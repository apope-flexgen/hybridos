# In the config folder's psm directory, Modbus client/server files and echo files
...
rm -rf $config_dir/psm/modbus_server
rm -rf $config_dir/psm/echo
mkdir $config_dir/psm/modbus_server
mkdir $config_dir/psm/echo
...
for component in "${components[@]}"; do
    ...
    mkdir $config_dir/psm/$component # make temp folder
    $bin_dir/echo -$config_flag=$config_dir/modbus_client/"$component"_client.json -mode=modbus -output=$config_dir/psm/$component # generate files
    mv $config_dir/psm/"$component"/"$component"_server.json $config_dir/psm/modbus_server/"$component"_server.json # move server file to modbus_server folder
    mv $config_dir/psm/"$component"/"$component"_echo.json $config_dir/psm/echo/"$component"_echo.json # move echo file to echo folder
    rm -rf $config_dir/psm/$component # delete temp folder
done
