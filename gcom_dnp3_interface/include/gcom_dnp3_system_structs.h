...
public:
  ...
  std::string crob_true =
      std::string("LATCH_ON"); // WHEN the crob is true, what is the string
                               // representation (e.g. "POWER_ON", "LATCH_ON",
                               // etc.)
  std::string crob_false =
      std::string("LATCH_OFF"); // WHEN the crob is false, what is the string
...
struct ProtocolDependencies {
  ...
  bool connected =
      false; // To tell main whether or not this workspace is up or not
...
template <> struct fmt::formatter<TMWSIM_POINT> {
  ...
  auto format(const TMWSIM_POINT &dbPoint, FormatContext &ctx) const {
    ...
    fmt::format_to(ctx.out(), R"("static_variation": {}, )",
                   dbPoint.defaultStaticVariation);
    fmt::format_to(ctx.out(), R"("event_variation": {}, )",
                   dbPoint.defaultEventVariation);
    if (dbPoint.type == TMWSIM_TYPE_ANALOG) {
      fmt::format_to(ctx.out(), R"("raw_value": {}, )",
                     dbPoint.data.analog.value);
      if (flexPoint->scale == 0) {
        fmt::format_to(ctx.out(), R"("scaled_value": {}, )",
                       dbPoint.data.analog.value);
      } else {
        fmt::format_to(ctx.out(), R"("scaled_value": {}, )",
                       dbPoint.data.analog.value / flexPoint->scale);
      }
      fmt::format_to(ctx.out(), R"("deadband": {}, )",
                     dbPoint.data.analog.deadband);
    } else {
      fmt::format_to(ctx.out(), R"("raw_value": {}, )",
    ...
    if (flexPoint->batch_pubs) {
      fmt::format_to(ctx.out(), R"("batch_pub_rate": {}, )",
                     flexPoint->batch_pub_rate);
    }
    if (flexPoint->interval_pubs) {
      fmt::format_to(ctx.out(), R"("interval_pub_rate": {}, )",
                     flexPoint->interval_pub_rate);
    }
    if (flexPoint->batch_sets) {
      fmt::format_to(ctx.out(), R"("batch_set_rate": {}, )",
                     flexPoint->batch_set_rate);
    }
    if (flexPoint->interval_sets) {
      fmt::format_to(ctx.out(), R"("interval_set_rate": {}, )",
                     flexPoint->interval_set_rate);
    }
    const TMWDTIME *tmpPtr = &(dbPoint.timeStamp);
    fmt::format_to(
        ctx.out(),
        R"("value_last_updated":"{:04}-{:02}-{:02} {:02}:{:02}:{:02}.{:03}")",
        tmpPtr->year, tmpPtr->month, tmpPtr->dayOfMonth, tmpPtr->hour,
...
template <> struct fmt::formatter<PointGroup> {
  constexpr auto parse(format_parse_context &ctx) { return ctx.end(); }
  ...
  auto format(const PointGroup &point_group, FormatContext &ctx) const {
    ...
    for (it = point_group.points.begin();
         it != std::prev(point_group.points.end()); it++) {
      const TMWSIM_POINT *dbPoint = it->second;
      fmt::format_to(ctx.out(), R"("{}": )", it->first);
      fmt::format_to(ctx.out(), R"({{)");
      fmt::format_to(ctx.out(), R"("uri": "{}", )",
