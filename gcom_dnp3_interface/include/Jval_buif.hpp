...
// Jval_buif is a structure defined to minimize storage space for mixed data types
// b = bool
// u = unsigned int
// i = int
// f = float
// It makes use of a union and includes several auxiliary funcitons that allow for
// comparisons.
...
// some utility functions that allow us to compare Jval_buif values
...
namespace buif_math
{
    ...
    template <typename T>
    inline constexpr bool is_tiny_unsigned_integral = std::is_same_v<uint8_t, T> || std::is_same_v<uint16_t, T> || std::is_same_v<uint32_t, T>;
