...
describe('FimsService', () => {
  ...
  describe('test get', () => {
    it('should use the arguments', async () => {
      const fimsSend = jest.spyOn(fimsService, 'send').mockResolvedValue({} as unknown as FimsMsg);
    ...
    it('should throw failed exception', async () => {
      jest.spyOn(fimsService, 'send').mockResolvedValue(FIMS_RESP_FAILED);
      expect(async () => {
        await fimsService.get(FIMS_URI, FIMS_BODY);
      }).rejects.toThrow(
        new FIMSSendFailedException({
          method: 'get',
          uri: FIMS_URI,
          body: FIMS_BODY,
        }),
    ...
    it('should throw timed out exception', async () => {
      jest.spyOn(fimsService, 'send').mockResolvedValue(FIMS_RESP_TIMED);
      expect(async () => {
        await fimsService.get(FIMS_URI, FIMS_BODY);
      }).rejects.toThrow(
        new FIMSSendTimedOutException({
          method: 'get',
          uri: FIMS_URI,
          body: FIMS_BODY,
        }),
  ...
  describe('test send', () => {
    ...
    it('should process argument', async () => {
      jest.spyOn(fimsService as any, 'newUID').mockReturnValue('42');
    ...
    it('should return promise', async () => {
      ...
      fimsSendSpy = jest.spyOn(FIMS, 'send').mockReturnValue(true);
      expect(await fimsService.send(FIMS_MSG)).toEqual(FIMS_RESPONSE);
    });

    it('should return error obj', async () => {
      fimsSendSpy = jest.spyOn(FIMS, 'send').mockReturnValue(false);
  ...
  describe('test subscribe', () => {
    ...
    it('should register', () => {
      ...
      createSubscribeObservableSpy = jest
        ...
        .mockReturnValue(SUB_OBS)
        .mockImplementation(jest.fn());
      registerSubscriptionSpy = jest.spyOn(fimsService as any, 'registerSubscription');
      fimsService.subscribe(SUB_URI, req);
  ...
  describe('test isUserAlreadySubscribed', () => {
    ...
    it('should have uid and uri', () => {
      mapHasSpy = jest.spyOn(Map.prototype as any, 'has').mockImplementation((input) => {
        if (input === SUB_UID) {
          return true;
        } else {
          return true;
      ...
      mapGetSpy = jest.spyOn(Map.prototype as any, 'get').mockReturnValue(new Map());
  ...
  describe('test registerSubscription', () => {
    ...
    it('should use args with existing URI set', () => {
      mapHasSpy = jest.spyOn(Map.prototype as any, 'has').mockReturnValue(true);
      mapGetSpy = jest.spyOn(Map.prototype as any, 'get').mockReturnValue(new Map());
    ...
    it('should create URI set', () => {
      mapHasSpy = jest.spyOn(Map.prototype as any, 'has').mockReturnValue(false);
      mapGetSpy = jest.spyOn(Map.prototype as any, 'get').mockReturnValue(new Map());
      mapSetSpy = jest.spyOn(Map.prototype as any, 'set').mockImplementation(jest.fn());
      fimsService.registerSubscription(SUB_URI, SUB_UID, SUB_OBS);
  ...
  describe('test openListener', () => {
    ...
    it('should call open', () => {
      fimsListenerOpenSpy = jest.spyOn(FimsListener.prototype as any, 'open');
      fimsService['openListener']();
      expect(fimsListenerOpenSpy).toBeCalled();
    });

    it('should return after opened', () => {
      fimsListenerOpenSpy = jest.spyOn(FimsListener.prototype as any, 'open').mockReturnValue(true);
  ...
  describe('test newUID', () => {
    ...
    it('should check for generated newUID', () => {
      stringSubstrSpy = jest.spyOn(String.prototype as any, 'substr').mockReturnValue(MY_UID);
    ...
    it('should recurse for existing URI', () => {
      ...
      let recurseUid = 1;
      stringSubstrSpy = jest.spyOn(String.prototype as any, 'substr').mockImplementation((x, y) => {
        return (recurseUid--).toString();
      });
      uidHasSpy = jest.spyOn(Set.prototype as any, 'has').mockImplementation((uid: string) => {
        if (parseInt(uid) <= 0) {
          return false;
        } else {
          return true;
  ...
  describe('test getListeningFunc', () => {
    ...
    it('should respond without issue', () => {
      eventRemoveListenerSpy = jest
        ...
        .mockImplementation(jest.fn());
      uidDeleteSpy = jest.spyOn(Set.prototype as any, 'delete').mockImplementation(jest.fn());
  ...
  describe('test doUnsubscribeCleanup', () => {
    ...
    it('should delete "uri from user" but not "user"', () => {
      mapHasSpy = jest.spyOn(Map.prototype as any, 'has').mockReturnValue(true);
      mapGetSpy = jest.spyOn(Map.prototype as any, 'get').mockReturnValue(
        new Map([
          [URI, 'some observable'],
          ['something', 'else'],
        ]),
