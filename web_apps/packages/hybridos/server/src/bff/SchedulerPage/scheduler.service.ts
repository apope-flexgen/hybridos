...
import {
    IValidJWTService,
    VALID_JWT_SERVICE,
} from 'src/auth/interfaces/validJWT.service.interface'
...
export class SchedulerService {
    ...
    async getSchedulerConfiguration(): Promise<
        string | Record<string, unknown>
    > {
        const configuration = await this.fimsService.get(
    ...
    async addSchedulerConfiguration(
        ...
        if (
            configDto.local_schedule?.name ||
            configDto.local_schedule?.name.length === 0
        ) {
    ...
    private getUriSpecificObservable = (
        ...
        const newObservable: Observable<MergedSchedulerData> =
            fimsSubscribe.pipe(
                map((event) => {
