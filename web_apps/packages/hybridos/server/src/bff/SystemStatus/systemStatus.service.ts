...
export class SystemStatusService {
  ...
  subscribeToSystemStatus = (): Observable<ServiceStatusResponse> => {
    ...
    const newObservable: Observable<ServiceStatusResponse> = fimsSubscribe.pipe(
      map((event) => {
  ...
  private parseSystemStatusData = (
    rawSystemStatusData: any,
    serviceName: string,
    ...
    const actions: ServiceAction[] = Object.keys(rawSystemStatusData.controls).map(
      (controlKey) => ({
  ...
  private parseSystemStatusGetData = (data: FimsMsg): ServiceStatusResponse[] => {
    ...
    const parsedData: ServiceStatusResponse[] = [];
    rawSystemStatusData.forEach((serviceObject) => {
      Object.keys(serviceObject).forEach((serviceName) => {
