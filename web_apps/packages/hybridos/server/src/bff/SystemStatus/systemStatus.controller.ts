...
export class SystemStatusController {
  ...
  @ApprovedRoles(Roles.Admin, Roles.Developer)
  async startService(@Param() serviceName: ServiceName): Promise<FimsMsg> {
    const serviceStartResponse = await this.systemStatusService.doServiceAction(
  ...
  @ApiOkResponse({ type: FimsMsg })
  @ApprovedRoles(Roles.Admin, Roles.Developer)
  async stopService(@Param() serviceName: ServiceName): Promise<FimsMsg> {
    const serviceStartResponse = await this.systemStatusService.doServiceAction(
      serviceName.serviceName,
      'stop',
    );
  ...
  @ApiOkResponse({ type: FimsMsg })
  @ApprovedRoles(Roles.Admin, Roles.Developer)
  async restartService(@Param() serviceName: ServiceName): Promise<FimsMsg> {
    const serviceStartResponse = await this.systemStatusService.doServiceAction(
      serviceName.serviceName,
      'restart',
    );
