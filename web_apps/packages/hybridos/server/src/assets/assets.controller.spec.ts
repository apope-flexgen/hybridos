...
import {
  ...
  ASSETS_POST,
  ASSETS_REDUCED,
  ASSETS_REDUCED_DUPLICATE,
  ASSETS_POST_DUPLICATE,
} from './assets.mocks';
...
describe('AssetsController', () => {
  ...
  describe('test getAssets', () => {
    it('should return assets', async () => {
      jest.spyOn(assetsService, 'getAssets').mockResolvedValue(ASSETS_RESPONSE);
      expect(await assetsController.getAssets()).toMatchObject(ASSETS_RESPONSE);
    });

    describe('test error throw', () => {
      beforeEach(async () => {
        jest.spyOn(assetsService, 'getAssets').mockRejectedValue(new Error('example error'));
      });
  ...
  describe('tests postAssets', () => {
    ...
    it('should return data body', async () => {
      jest.spyOn(assetsService, 'postAssets').mockResolvedValue(ASSETS_RESPONSE);
    ...
    it('should throw an exception', async () => {
      await expect(assetsController.postAssets('bad input' as unknown as Assets)).rejects.toThrow();
    });
  });
});
