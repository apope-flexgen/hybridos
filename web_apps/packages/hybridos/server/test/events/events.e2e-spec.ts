...
export const buildURI = (filters: any): string => {
  ...
  if (filters.severity != null) {
    filters.severity.forEach((singleSeverity: string) =>
      filtersArray.push(['severity', singleSeverity]),
    );
  }
...
describe('Events (e2e)', () => {
  ...
  it('/GET events - success', () => {
    ...
    jest
      .spyOn(eventsService, 'eventsFiltered')
      .mockImplementation(() => Promise.resolve(successResponse));

    return request(app.getHttpServer()).get(uri).expect(200).expect(successResponse);
  });
