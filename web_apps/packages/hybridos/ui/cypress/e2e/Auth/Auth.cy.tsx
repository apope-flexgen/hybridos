...
import {
  loginMfaMock,
  loginMock,
  loginMockUnauthorized,
  loginPassExpMock,
} from 'src/mocks/pages/Auth/Login.mock';
...
describe('<WebUILogin />', () => {
  ...
  it('FAIL: Log in with MFA, Incorrect Code.', () => {
    cy.window().then((window) => {
      const { worker } = window.msw;
      worker.use(loginMfaMock, mfaMockInvalidCode);
    });
  ...
  it('FAIL: Log in with Pass Exp, passwords to not match.', () => {
    ...
    cy.wait(250);
    cy.contains(
      'Invalid input. Ensure passwords are not empty and do not match each other.',
    ).should('exist');
    cy.getCookie('refreshToken').should('not.exist');
  });
  ...
  it('FAIL: Log in with Pass Exp, Invalid passwords.', () => {
    cy.window().then((window) => {
      const { worker } = window.msw;
      worker.use(loginPassExpMock, passExpMockOldPassword);
    });
