...
import { Roles, PasswordOptions } from 'shared/types/api/Users/Users.types';
...
const UserRow: FunctionComponent<UserRowProps> = ({
  ...
  const roles = Object.values(Roles).filter((role) => {
    return role !== Roles.RestReadWrite;
  });
  const rolesWithoutDeveloper = roles.filter((role) => {
  ...
  const generatePasswordRequirements = useCallback(async () => {
    try {
      ...
      const updatedPasswordRequirements = generatePasswordRequirementsToDisplay(
        passwordOptions,
        oldPasswords,
        newUser,
      );
    ...
    <Box sx={rowSx}>
      <Box sx={{ display: 'flex', width: '60%', gap: '8px' }}>
        ...
        <Select
          minWidth={175}
          label='Role'
          menuItems={showDeveloper ? roles : rolesWithoutDeveloper}
          onChange={(e) => {
        ...
        <TextField
          ...
          type='password'
          validator={
            userData?.password && (() => validatePassword(userData?.password, passwordOptions))
          }
          value={userData?.password ?? ''}
        />
      </Box>
      <Box sx={passwordReqsSx}>
        <Typography text='Password Requirements' variant='bodySBold' />
        {passwordRequirements.map((requirement) => (
          <Typography text={`- ${requirement}`} variant='helperText' />
        ))}
      </Box>
    </Box>
  );
