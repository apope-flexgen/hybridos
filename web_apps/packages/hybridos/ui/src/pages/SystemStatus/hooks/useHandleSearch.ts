...
const useHandleSearch = (systemStatusData: SystemStatusObject[]) => {
  const handleSearch = (searchTerms: string) => {
    ...
    const results: SystemStatusObject[] = systemStatusData.filter((systemStatusObject) => (
      formatServiceName(systemStatusObject.serviceName || '').toLowerCase().includes(searchTermsLC)
