...
import {
  SYSTEM_STATUS_URL,
  statusColorMapping,
} from 'src/pages/SystemStatus/SystemStatus.constants';
...
const useGenerateSystemStatusTable = () => {
  ...
  const generateActionButtons = (actionArray: ServiceActionObject[], serviceName: string) => {
    const handleOnClick = async (action: ServiceActionType) => {
      ...
      axiosInstance.put(serviceActionURL).then((res) => {
        if (res.data.body === 'done') notifCtx?.notif('success', `Succesfully completed ${action} on ${serviceName}`);
          ...
          notifCtx?.notif(
            'error',
            `Error completing ${action} on ${serviceName} - ${res.data.body}`,
          );
        }
      });
    };
    ...
    actionArray.forEach((serviceAction) => {
      enabledStatuses = {
        ...enabledStatuses,
        [serviceAction.action]: serviceAction.enabled,
      };
    });
  ...
  const generateDependencies = (dependencies: string[]) => {
    if (dependencies && dependencies.length > 0) {
      const dependenciesList = dependencies
        .map((dependency) => dependency.replace('.service', ''))
        .join(', ');
  ...
  const generateStatusTag = (serviceStatus: string) => (
    <Chip
  ...
  const generateConnectionStatus = (connectionStatus: ConnectionStatus | null) => {
  ...
  const generateRowsData = (
    ...
    sortedArray.sort((objectA, objectB) => {
      if (!reverseOrderToUse) {
    ...
    const returnData: SystemStatusRow[] = sortedArray.map((serviceData) => ({
      ...
      service_status: serviceData.serviceStatus
        ? generateStatusTag(serviceData.serviceStatus)
        : '-',
