...
import { ADD_CONTROL, CONTROL, DELETE_CONTROL, items, newControl } from './helpers/constants';
  ...
  const handleAdd = () => {
    setSelectedAsset(
      (prevSelectedAsset) =>
        ({
          ...prevSelectedAsset,
          controls: [...(prevSelectedAsset?.controls || []), newControl],
        } as Asset),
    );
              ...
              <Fragment key={key}>
                {select ? (
                  <Select
                    ...
                    value={control[key as keyof Control].toLowerCase()}
                  />
                ) : (
                  <TextField
