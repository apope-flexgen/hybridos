...
import { edgeSx, nodeBoxSx } from 'src/pages/ConfigurablePages/Dashboard/DiagramDashboard/diagramDashboard.styles';
import { AssetNodeProps, ValidAssetIcon } from 'src/pages/ConfigurablePages/Dashboard/DiagramDashboard/diagramDashboard.types';
import { useTheme } from 'styled-components';

const AssetNode: React.FC<NodeProps<AssetNodeProps>> = ({
  data,
}: NodeProps<AssetNodeProps>) => {
  const theme = useTheme() as ThemeType;
  const outerBoxSx = nodeBoxSx(theme);
  const handleSx = edgeSx(theme);

  const {
    assetType, label, hasParent,
  } = data;
