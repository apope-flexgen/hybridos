    ...
    if (controlsSets.withConfirm.includes(componentName)) {
        return {
            onCheck: (value: PossibleConfirmValues) => {
                ...
                console.log(`sending ${trueValue} to ${uri}`)
...
const organizeProps = (
    ...
    // TODO: change this to use proper file logging when it is implemented in the ui
    if (component === 'MaintModeSlider') {
        props.logging = (reason: string, comment: string) =>
            console.log(`Maintenance Mode Enabled (reason: ${reason} - comment: ${comment})`)
    }
