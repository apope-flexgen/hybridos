...
export const decideStatusIcon = (status: MaintenanceActionStatuses) => {
  if (status === MaintenanceActionStatuses.InProgress || status === MaintenanceActionStatuses.Exiting) return 'Loading';
...
export const decideStatusIconColor = (status: MaintenanceActionStatuses) => {
  if (status === MaintenanceActionStatuses.InProgress || status === MaintenanceActionStatuses.Exiting) return 'primary';
...
export const cardContainerSx = {
  boxShadow: 'none', gap: '12px', border: 'solid 1px #EAECF0', padding: '16px',
};
export const contentBoxSx = {
  display: 'flex', justifyContent: 'space-between', flex: 1, alignItems: 'flex-start',
};
...
export const modalContentSx = {
  width: '100%', display: 'flex', flexDirection: 'column', gap: '12px', padding: '0 24px 24px 24px',
};
