...
def fimsSet(uri):
    return subprocess.check_output([FIMS_SEND, "-m", "set", "-r","/me", "-u", uri, "1"], stderr=subprocess.STDOUT).strip()
    
def fimsSetTrue(uri):
    return subprocess.check_output([FIMS_SEND, "-m", "set", "-r", "/me", "-u", uri, "true"], stderr=subprocess.STDOUT).strip()
