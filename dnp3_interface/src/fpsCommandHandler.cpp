...
{
    const char* cmd = OperationTypeSpec::to_string(command.opType);
    sysdbAddtoRecord(sysdb,"CROB_SELECT", cmd, index);
    return CommandStatus::SUCCESS; 
}

CommandStatus fpsCommandHandler::Operate(const ControlRelayOutputBlock& command, uint16_t index, IUpdateHandler& handler, OperateType opType)
{
    const char* cmd = OperationTypeSpec::to_string(command.opType);
    if(sysdb->debug)
        FPS_ERROR_PRINT("                     ************ %s  CROB called, index: %d  cmd [%s]\n", __func__, (int) index, cmd );
    sysdb->setDbVarIx(Type_Crob, index, OperationTypeSpec::to_type(command.opType), command.status);
...
{ 
    if(sysdb->debug)
       FPS_ERROR_PRINT("                     ************ %s  AnalogInt16 Select called, index: %d \n", __func__, (int) index );
    sysdbAddtoRecord(sysdb, "AnalogInt16_SELECT", command, index);
    return CommandStatus::SUCCESS; 
}
 
CommandStatus fpsCommandHandler::Operate(const AnalogOutputInt16& command, uint16_t index, IUpdateHandler& handler, OperateType opType)
{ 
    if(sysdb->debug)
      FPS_ERROR_PRINT("                     ************ %s  AnInt16 Operate called, index: %d \n", __func__, (int) index );
    sysdbAddtoRecord(sysdb,"AnOPInt16", command, index);
    sysdb->setDbVarIx(AnIn16, index, command.value, command.status);
    return CommandStatus::SUCCESS; 
}
...
CommandStatus fpsCommandHandler::Operate(const AnalogOutputInt32& command, uint16_t index, IUpdateHandler& handler, OperateType opType)
{ 
    if(sysdb->debug)
        FPS_DEBUG_PRINT("                     ************ %s  AnInt32 called, index: %d \n", __func__, (int) index );
    sysdbAddtoRecord(sysdb, "AnOPInt32", command, index);
    sysdb->setDbVarIx(AnIn32, index, command.value, command.status);
    return CommandStatus::SUCCESS; 
}
...
CommandStatus fpsCommandHandler::Operate(const AnalogOutputFloat32& command, uint16_t index, IUpdateHandler& handler, OperateType opType)
{
    if(sysdb->debug)
        FPS_DEBUG_PRINT("                     ************ %s  AnF32 called, index: %d \n", __func__, (int) index );
    sysdbAddtoRecord(sysdb, "AnOPF32", command, index);
    sysdb->setDbVarIx(AnF32, index, command.value, command.status);    
    return CommandStatus::SUCCESS; 
}
