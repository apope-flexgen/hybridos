...
// When dealing with Modbus and you have a register type that consists of multiple contiguous blocks with gaps in between, it's not necessary to create multiple modbus_mapping_t objects for each block. Instead, you can handle this within a single modbus_mapping_t structure. The key is to define the mapping to cover the entire range from the lowest to the highest address used by your blocks, including the gaps.

// Here's how you can approach this:

// 1. Determine the Overall Range
// Calculate the start and end addresses for each register type across all devices. This range should cover the lowest address of the first block to the highest address of the last block, inclusive of any gaps.

// 2. Create a Single Modbus Mapping
// Create a single modbus_mapping_t object with the range determined above. This mapping will have spaces for all registers, including those in the gaps.

// 3. Initialize Registers
// Initialize the registers in your modbus_mapping_t object. For the registers within the gaps, you can set them to a default value (like zero). For the registers within your contiguous blocks, set them according to your actual data.

// 4. Handle Requests Appropriately
// When handling Modbus requests, you'll need to ensure that requests for addresses within the gaps are handled correctly - either by ignoring them, returning a default value, or signaling an error, depending on your application's requirements.
...
// cpp
// Copy code
// modbus_mapping_t* createModbusMapping() {
//     int start_address = ...; // Lowest address of your blocks
//     int end_address = ...; // Highest address of your blocks

//     // The total number of registers is the difference between the highest and lowest address, plus one
//     int num_registers = end_address - start_address + 1;

//     // Create the Modbus mapping
//     modbus_mapping_t *mb_mapping = modbus_mapping_new(start_address, num_registers, 0, 0, 0, 0);
//     if (mb_mapping == NULL) {
...
//         mb_mapping->tab_registers[start_address + i] = 0; // Or some default value
//     }

//     // Set values for actual registers in your blocks
//     // ...

//     return mb_mapping;
// }
//modbus_mapping_t* modbus_mapping_new(int nb_bits, int nb_input_bits, int nb_input_registers, int nb_registers);

// Considerations
// Memory Usage: This approach might lead to increased memory usage, especially if the gaps are large, as you are allocating space for unused registers.
// Handling Gaps: Decide how your system should respond to read/write requests to addresses within the gaps. This behavior should be consistent and well-defined.
// Modbus Conventions: Ensure that your approach aligns with Modbus conventions and the expectations of the Modbus master devices you are communicating with.
// This method simplifies the management of your Modbus mappings by using a single modbus_mapping_t object, while still allowing for gaps between contiguous blocks of registers.
#include <stdio.h>
#include <vector>
#include <string>
#include <string.h>
...
#include "gcom_modbus_server_id.h"

#define MICROSECOND_TO_MILLISECOND 1000
#define NANOSECOND_TO_MILLISECOND  1000000

// build/release/gcom_modbus_server_id configs/server/sel_350_slow_server_device_id.json
...
struct IODevice {
    IODevice(){};
    //IOPoint(int offset) :offset(offset) {};
    ...
    // Other attributes...
    //maps map;
    // Define a comparator for the set
...
using IOPointMap = std::map<std::string, IOPoint>; // Maps ID to IOPoint
using UriIOPointMap = std::map<std::string, IOPointMap>; // Maps URI to IOPointMap


// void addIOPoint(UriToIOPointMap& uriToIOPointMap,const char* uri, const char* id, const IOPoint& point) {
//     uriToIOPointMap[std::string(uri)][std::string(id)] = point;
// }

bool add_IOPoint(UriIOPointMap& uriIOPointMap, const char* uri, const char* id, const IOPoint& point) {
    auto& idMap = uriIOPointMap[std::string(uri)];
    if (idMap.find(std::string(id)) == idMap.end()) {
        // IOPoint not found, add it
        idMap[std::string(id)] = point;
        return true; // Indicate that the IOPoint was added
    }
    return false; // IOPoint already exists, not added
}


IOPoint* find_IOPoint(UriIOPointMap& uriIOPointMap,const char* uri, const char* id) {
    auto uriIt = uriIOPointMap.find(std::string(uri));
    if (uriIt != uriIOPointMap.end()) {
        auto& idMap = uriIt->second;
        auto idIt = idMap.find(std::string(id));
        if (idIt != idMap.end()) {
            return &(idIt->second);
        }
    }
...
// a list of io pointts in offset order
using IOPointSet = std::set<IOPoint>;
// map device types to device iopoints
using TypeIOPointMap = std::map<int, IOPointSet>;
// map device_ids to device types
using DeviceTypeMap = std::map<int, TypeIOPointMap>;

void add_IOType_to_deviceMap(DeviceTypeMap& deviceMap, int deviceId, int deviceType) {
    std::cout << " adding iopoint device type " << deviceId 
                << " device id " << deviceType
                << std::endl;
    deviceMap[deviceId][deviceType];
}

void add_IOPoint_to_deviceMap(DeviceTypeMap& deviceMap, int deviceId, int deviceType, const IOPoint& point) {
    std::cout << " adding iopoint device id " << deviceId 
                << " device id " << deviceType
                << std::endl;
    deviceMap[deviceId][deviceType].insert(point);
}


//using IOPointSet = std::set<IOPoint>;

// we break up each io_point set into a number of contiguous blocks based on reg type 
// then run through the device_id  mapping  
std::vector<std::vector<IOPoint>> createContiguousBlocks(const IOPointSet& ioPoints) {
    std::vector<std::vector<IOPoint>> contiguousBlocks;
    std::vector<IOPoint> currentBlock;
        //"size"

    for (const auto& point : ioPoints) {
        if (!currentBlock.empty() && 
            (point.offset != currentBlock.back().offset + (int)(currentBlock.back().map.num_regs))) {
            // If the current point is not contiguous with the last one in the block
            contiguousBlocks.push_back(currentBlock);
            currentBlock.clear();
        }
        currentBlock.push_back(point);
    }
...
void show_device_map(DeviceTypeMap& deviceMap, std::stringstream &ss)
{
    ss << " device map \n" ;
    for (const auto dd : deviceMap) {
        ss << " dev id    " << dd.first << "\n";
        for (const auto dt : dd.second) {
            ss << "         reg type      " << reg_types[dt.first] << "\n";
            if(dt.second.empty())
            {
                    ss << "                                no io points           \n"; 
            }
            ...
            {
                auto dp_first = *dt.second.begin();
                auto dp_last = *dt.second.rbegin();
                ss << "           ioPoint        first   offset         " << dp_first.offset<<"\n";
                ss << "           ioPoint        last   offset         " << dp_last.offset<< " num regs " << dp_last.map.num_regs<< "\n";
                ss << "           ioPoint        last   size         " << dp_last.offset + dp_last.map.num_regs<<"\n";


                for (const auto &dp : dt.second) {
                    //ss << "       ioPoint        offset         " << dp.offset <<"\n";
                    ss << "           ioPoint        offset            " << dp.offset 
                        << " uri [" <<dp.uri<<"]"
                        << " id [" <<dp.id<<"]"
                        << " reg_name [" <<dp.map.reg_name<<"]\n";
                }
            }
        }
    }
...
// we have to do something like this for each device_id
//  but then we need to also chunk up the data sets
// 
//             mapping.second->mb_mapping = modbus_mapping_new_start_address(
//                                            mapping.second->data[Coil].start_offset, mapping.second->data[Coil].num_regs,
//                                         mapping.second->data[Discrete_Input].start_offset,   mapping.second->data[Discrete_Input].num_regs,
//                                         mapping.second->data[Holding_Register].start_offset, mapping.second->data[Holding_Register].num_regs,
//                                         mapping.second->data[Input_Register].start_offset,   mapping.second->data[Input_Register].num_regs);

// Function to find an IOPoint by offset
IOPointSet::const_iterator findIOPointByOffset(const IOPointSet& ioPointSet, int offset) {
    IOPoint searchPoint(offset);  // Temporary IOPoint for the search
    return ioPointSet.find(searchPoint);
}

// // Find IOPoint with offset 20
//     auto it = findIOPointByOffset(ioPointSet, 20);
//     if (it != ioPointSet.end()) {
//         std::cout << "Found IOPoint with offset: " << it->offset << std::endl;
//     } else {
//         std::cout << "IOPoint with offset 20 not found." << std::endl;
//     }
//auto it = findIOPointByOffset(ioPointSet, 20);
// if (it != ioPointSet.end()) {
//     const IOPoint& foundIOPoint = *it; // Dereference the iterator to access the IOPoint
//     // Now you can use foundIOPoint
//     std::cout << "Found IOPoint with offset: " << foundIOPoint.offset << std::endl;

//     // If IOPoint has other members, you can access them here as well.
//     // For example:
//     // std::cout << "Other attribute: " << foundIOPoint.otherAttribute << std::endl;
// } else {
//     std::cout << "IOPoint with offset 20 not found." << std::endl;
// }


IOPointSet* getIOPoints(DeviceTypeMap& deviceMap, uint8_t deviceId, uint8_t deviceType) {
    auto deviceIt = deviceMap.find(deviceId);
    if (deviceIt != deviceMap.end()) {
        auto& typeMap = deviceIt->second;
        auto typeIt = typeMap.find(deviceType);
...
// IOPoint point{10}; // IOPoint with offset 10
//     addIOPoint(deviceMap, 1, 1, point); // Add to device_id 1, device_type 1

// int main() {
//     DeviceToTypeMap deviceMap;

//     // Example: Add an IO point
//     IOPoint point{10}; // IOPoint with offset 10
//     addIOPoint(deviceMap, 1, 1, point); // Add to device_id 1, device_type 1

//     // Retrieve and use IOPoints
//     IOPointSet* points = getIOPoints(deviceMap, 1, 1);
//     if (points) {
//         for (const auto& pt : *points) {
//             std::cout << "IOPoint offset: " << pt.offset << std::endl;
//         }
//     }
...
bool reload = false;
// map device_id , device_type, set items 
DeviceTypeMap deviceMap;
// map uri id to points
UriIOPointMap uriIOPointMap;



std::map<uint8_t, server_data*> device_map;
uri_map uri_to_register;
server_data *server_map = NULL; // the default map for device_ids that are NOT in the uri_map above
fims* p_fims = NULL;
...
int establish_connection(system_config* config)
{
    ...
    if (!config->service.empty())
    {
        ...
        ret = service_to_port((char*)config->service.c_str(), &sport);
        if (ret == 0) {
           FPS_ERROR_PRINT(" found service [%s] port [%d]\n", config->service.c_str(), sport);
           addr.sin_port = htons(sport);
           config->port = sport;
        } else {
            FPS_ERROR_PRINT("Failed to get port for service  %s: usinf default %d.\n", 
                     config->service.c_str(), config->port);
        }
    }
    if(!config->eth_dev.empty())
    {
        ifr.ifr_addr.sa_family = AF_INET;
        //read interface out of config file
        strncpy(ifr.ifr_name, config->eth_dev.c_str(), IFNAMSIZ - 1);
    ...
    else if(!config->ip_address.empty())
    {
        char ip[128];
        int ret;
        ret = hostname_to_ip((char*)config->ip_address.c_str() ,ip, 128);
        if (ret == 0)
        {
	      //free(config->ip_address);
            config->ip_address = ip;
        }
        addr.sin_addr.s_addr = inet_addr((char*)config->ip_address.c_str());
    }
    else
...
int parse_system(cJSON *system, system_config *config)
{
    ...
    if (serv != NULL)
    {
        ...
        {
            config->service = serv->valuestring;
    ...
    if (ip_addr != NULL)
    {
        ...
        config->ip_address = ip_addr->valuestring;
    }
    else if (serial_device != NULL)
    {
        ...
        config->serial_device = serial_device->valuestring;
...
int create_id_map(cJSON* registers, uint8_t device_id);
int create_io_point(IOPoint& point, cJSON *current_variable_JSON);



int parse_registers(cJSON *registers, system_config &config)
{
    std::cout << " parsing registers " << std::endl;
    int device_id;

    if (cJSON_IsObject(registers)) {
        std::cout << "[registers] is a cJSON Object" << std::endl;
        create_id_map(registers, 255);
    } else if (cJSON_IsArray(registers)) {
        std::cout << "[registers] is a cJSON Array" << std::endl;
        cJSON *reg;
        cJSON_ArrayForEach(reg, registers) {
            ...
            std::cout << "[registers] for id " << device_id << std::endl;
            create_id_map(reg, device_id);
        }
    } else {
        // 'registers' is neither an Object nor an Array
        std::cout << "[registers] is an invalid object" << std::endl;
    }
...
int create_id_map(cJSON* registers, uint8_t device_id)
{
    ...
    for(int i = 0; i < Num_Register_Types; i++)
    {
        ...
        if (register_array_JSON == NULL) {
            add_IOType_to_deviceMap(deviceMap, device_id, i);
            continue;
        }
        int reg_cnt = cJSON_GetArraySize(register_array_JSON);
        if(reg_cnt == 0)
        {
        ...
        //FPS_ERROR_PRINT
        printf ("Count %d variables included in %s config object.\n", reg_cnt, reg_types[i]);
        ...
        for(int j = 0; j < reg_cnt; j++)
        {
            ...
            if( !cJSON_IsNumber(current_variable_offset_json) ) {
                FPS_ERROR_PRINT("Missing variable offset for %s variable entry %d\n", reg_types[i], i);
                continue; //goto cleanup;
            }
            auto offset = current_variable_offset_json->valueint;
            printf("Found variable ID for %s variable entry %d   [%s]  offset %d \n"
                , reg_types[i], i, current_variable_id_json->valuestring,  offset);
            IOPoint point(offset); // IOPoint with offset 10
            create_io_point(point, current_variable_JSON);

            point.device_id = device_id;
            point.reg_type = i;

            add_IOPoint_to_deviceMap(deviceMap, device_id, i, point); // Add to device_id 1, device_type 1
...
int create_io_point(IOPoint& point, cJSON *current_variable_JSON)
{
    ...
    if( !cJSON_IsString(current_variable_id_json) ) {
        //FPS_ERROR_PRINT
        printf("Missing variable ID for variable entry \n");
        //goto cleanup;
    }
    point.map.reg_name = current_variable_id_json->valuestring;
    ...
    if( !cJSON_IsString(current_variable_uri_JSON) ) {
        FPS_ERROR_PRINT("Missing URI for %s\n",point.map.reg_name.c_str());
        //goto cleanup;
    }
    point.map.uri = current_variable_uri_JSON->valuestring;

    // parse variable's starting register offset
    cJSON* current_variable_offset_JSON = cJSON_GetObjectItem(current_variable_JSON, "offset");
    if( !cJSON_IsNumber(current_variable_offset_JSON) ) {
        FPS_ERROR_PRINT("Missing offset for %s\n",point.map.reg_name.c_str());
        //goto cleanup;
    }
    point.map.reg_off  = current_variable_offset_JSON->valueint;


    cJSON *byte_swap = cJSON_GetObjectItem(current_variable_JSON, "byte_swap");
    point.map.use_byte_swap = false;
    if(byte_swap) {
        point.map.byte_swap = cJSON_IsTrue(byte_swap);
        point.map.use_byte_swap = true;
    }
    ...
    point.map.num_regs = cJSON_IsNumber(multi_reg) ? multi_reg->valueint : 1;

    cJSON* bit_field = cJSON_GetObjectItem(current_variable_JSON, "bit_field");
    point.map.bit_field = cJSON_IsTrue(bit_field);

    cJSON* enum_type = cJSON_GetObjectItem(current_variable_JSON, "enum");
    point.map.enum_type = cJSON_IsTrue(enum_type);

    cJSON* random_enum = cJSON_GetObjectItem(current_variable_JSON, "random_enum");
    point.map.random_enum_type = cJSON_IsTrue(random_enum);

    cJSON* is_signed = cJSON_GetObjectItem(current_variable_JSON,"signed");
    point.map.sign = cJSON_IsTrue(is_signed);

    cJSON* is_bool = cJSON_GetObjectItem(current_variable_JSON, "bool");
    point.map.is_bool = cJSON_IsTrue(is_bool);

    cJSON* scale_value = cJSON_GetObjectItem(current_variable_JSON,"scale");
    point.map.scale = cJSON_IsNumber(scale_value) ? (scale_value->valuedouble) : 0.0;

    cJSON* is_float = cJSON_GetObjectItem(current_variable_JSON, "float");
    point.map.floating_pt = cJSON_IsTrue(is_float);
    point.map.sign |= point.map.floating_pt; // a float is signed by definition
    if (point.map.floating_pt && point.map.num_regs == 1) {
        FPS_ERROR_PRINT("%s is declared a float and has size 1 which is not allowed for floats.\n", point.map.reg_name.c_str());
        return -1;
    }
    point.uri = point.map.uri;
    point.id = point.map.reg_name;

   //add_IOPoint_to_deviceMap(deviceMap, point.device_id, point.reg_type, point);

    if (!add_IOPoint(uriIOPointMap, point.uri.c_str(), point.id.c_str(), point)) {
            FPS_ERROR_PRINT("%s duplicate io point\n", point.map.reg_name.c_str());
            return -1;
    }

    // this is tying to generate a list of points associated with a given uri
    //but we can do this in a bit easier way can we not
    if (0)
    {
        std::pair<std::string, body_map*> new_map_entry(point.map.uri, new body_map);
 ...
 */
// server_data* create_register_map(cJSON* registers, uint8_t device_id)
// {
//     server_data* srv_data = nullptr;

//     // can have duplicate device_id mappings just use the current one and create one if yu need to
//     // note thse the 255 map is the default map
//     if (device_map.find(device_id) != device_map.end()) {
//         FPS_ERROR_PRINT("device_id %d's mapping already exists\n", device_id);
//         srv_data = device_map[device_id];
//     }
//     else
//     {
//         srv_data = new server_data;
//         device_map[device_id] = srv_data;
//     }
...
//         // TODO this needs to be a std::set ordered by offset.
//         // std::map<Reg_type(int),  std::set<map>
    ...
    if(function == MODBUS_FC_WRITE_SINGLE_COIL)
    {
            ...
            offset <  data[Coil].start_offset + data[Coil].num_regs)
        {
            ...
            if(send_body != NULL)
            {
                ...
                {
                    char uri[512];
                    sprintf(uri, "%s/%s", reg->uri.c_str(), reg->reg_name.c_str());
...
int main(int argc, char *argv[])
{
    ...
    //do
    {
        ...
        if(-1 == parse_registers(registers, sys_cfg))
        {
            cJSON_Delete(config);
            return 1;
        }
// debug show deviceMap

        cJSON_Delete(config);
        std::stringstream ss;
        show_device_map(deviceMap, ss);
        std::cout << ss.str();
// map io_points for each type 

// Next we need a device structure to hold the mappings
...
//             server_map = create_register_map(registers, 255);
//             if (server_map == NULL)
//             {
...
//                 goto cleanup;
//             }
//             device_map[(uint8_t)255] = server_map;
//         } else if (cJSON_IsArray(registers)) {
//             cJSON *register;
//             cJSON_ArrayForEach(register, registers) {
//                 auto device_id = cJSON_GetObjectItem(register, "device_id");
//                 if (device_id == NULL || !cJSON_IsNumber(device_id)) {
//                     device_id = sys_cfg.device_id;
//                 }

//                 server_map = create_register_map(register, device_id);
//                 if (server_map == NULL)
//                 {
//                     FPS_ERROR_PRINT("Error creating register map for array  %s\n", sys_cfg.name);
//                     cJSON_Delete(config);
//                     goto cleanup;
//                 }
//                 //device_map[(uint8_t)device_id] = server_map;
...
//             mapping.second->mb_mapping = modbus_mapping_new_start_address(mapping.second->data[Coil].start_offset, mapping.second->data[Coil].num_regs,
//                                         mapping.second->data[Discrete_Input].start_offset,   mapping.second->data[Discrete_Input].num_regs,
//                                         mapping.second->data[Holding_Register].start_offset, mapping.second->data[Holding_Register].num_regs,
//                                         mapping.second->data[Input_Register].start_offset,   mapping.second->data[Input_Register].num_regs);
//             if (mapping.second->mb_mapping == NULL)
//             {
...
//         while(fims_connect < 5 && p_fims->Connect(sys_cfg.name) == false)
//         {
//             fims_connect++;
//             sleep(1);
//         }
...
//                         auto current_server_map = device_mapping == device_map.end() ? server_map : device_mapping->second;
...
//                         const uint8_t device_id = query[header_length - 1];
//                         auto device_mapping = device_map.find(device_id);
//                         auto server_map = device_map.find(255)->second;
...
//                         if(sys_cfg.mdebug > -1) {
//                             if(!sys_cfg.debug)
//                                 FPS_ERROR_PRINT("setting modbus mdebug  %d \n", sys_cfg.mdebug);
//                             else
...
//                 for (int i = 0; i < Num_Register_Types; i++)
//                 {
//                     if(mapping.second->data[i].register_map != NULL)
//                         delete[] mapping.second->data[i].register_map;
//                     if(mapping.second->regs_to_map[i] != NULL) delete[] mapping.second->regs_to_map[i];
//                 }
//                 if(mapping.second->mb_mapping != NULL)
//                     modbus_mapping_free(mapping.second->mb_mapping);
