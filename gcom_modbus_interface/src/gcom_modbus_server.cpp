...
//memset((void*)&sys_cfg, 0, sizeof(system_config));
...
int parse_system(cJSON *system, system_config *config)
{
    ...
    cJSON* low_debug = cJSON_GetObjectItem(system, "low_debug");
    if (low_debug)
    {
        config->low_debug = cJSON_IsTrue(low_debug); 
    } else {
...
{
    // can NOT have duplicate device_id mappings
    ...
    for(int i = 0; i < Num_Register_Types; i++)
    {
        ...
        for(int j = 0; j < reg_cnt; j++)
        {
            ...
            auto bmap = new body_map;
            std::pair<const char*, body_map*> new_map_entry(data[i].register_map[j].uri, bmap);
            ...
            std::pair<uri_map::iterator, bool > rtn_pair = uri_to_register.insert(new_map_entry);
            ...
            if(!rtn_pair.second) {
                //FPS_ERROR_PRINT("Failed to insert map entry 2 uri %s\n", data[i].register_map[j].uri);
                delete bmap;
            }
            ...
            if(var_id_map_it == uri_map_it->second->end())
            {
                ...
                Uri_Info reg_type_reg_config_arrays;
                reg_type_reg_config_arrays.device_id = device_id;
                reg_type_reg_config_arrays.reg_types = reg_type;
                reg_type_reg_config_arrays.mappings = reg_info;
                // key of map entry is the reg ID alone (URI is key in parent map). value is the pair of arrays for the URI/ID pair
                std::pair<const char*, Uri_Info > reg_id_to_arrays(data[i].register_map[j].reg_name, reg_type_reg_config_arrays);
            ...
            else
            {
                ...
                if(var_id_map_it->second.reg_types[i] == true || var_id_map_it->second.mappings[i] != NULL)
                {
                ...
                var_id_map_it->second.reg_types[i] = true;
                var_id_map_it->second.mappings[i] = &(data[i].register_map[j]);
            }
...
uint16_t json_to_uint16(maps* settings, cJSON* obj)
{
    cJSON* val = unwrap_if_clothed(obj);
    int val32 = val->valueint;  // Your 32-bit integer
    double scaled_val = val->valuedouble;
    ...
    //printf( "%s valueint %d %04x scaled %f \n", __func__, val32,val32, scaled_val);
    ...
    if (settings->is_bool)
    {
        ...
        encoded_val = static_cast<uint16_t> (cJSON_IsTrue(val) || val32 > 0);
    }
    else if(settings->bit_field || settings->enum_type || settings->random_enum_type)
    {
        encoded_val = static_cast<uint16_t> (extract_valueint_of_last_array_object(obj));
    }
    ...
    else if(settings->scale != 0.0)
    {
        ...
        if(settings->sign)
        {
            ...
            casted_val = static_cast<int16_t> (scaled_val);
            memcpy(&encoded_val, &casted_val, sizeof(encoded_val));
        }
        else
        {
            if (scaled_val > 0xffff)scaled_val = 0xffff;
            if (scaled_val < 0)scaled_val = 0;
            ucasted_val = static_cast<uint16_t> (scaled_val);
            memcpy(&encoded_val, &ucasted_val, sizeof(encoded_val));

        }
        //scaled_val = std::clamp((int)scaled_val, -32768, 32767);  // Clamp to signed 16-bit range
        //int casted_val = static_cast<int16_t> (scaled_val);
        //printf( "%s casted_val #2 %d x %f\n", __func__, casted_val, scaled_val);
        //memcpy(&encoded_val, &casted_val, sizeof(encoded_val));
        //printf( "%s casted_val #2 %d x %u\n", __func__, casted_val, encoded_val);
    }
    ...
    else
    {
        ...
        else
        {
            ...
            //val32 = std::clamp(val32, -32768, 32767);  // Clamp to signed 16-bit range
        // else
        //     val32 = std::clamp(val32, 0, 0xffff);  // Clamp to signed 16-bit range

        //uint16_t val16 = static_cast<uint16_t>(val32);
        //memcpy(&encoded_val, &(val16), sizeof(val16));
    }
    return encoded_val;
}
...
uint32_t json_to_uint32(maps* settings, cJSON* obj)
{
    ...
    int val32 = val->valueint;  // Your 32-bit integer
    double scaled_val = val->valuedouble;
    //printf( "%s val32  %d  scaled_val %f  %ld\n", __func__, val32, scaled_val, (long int)scaled_val);
    ...
    {
        //encoded_val = static_cast<uint32_t> (cJSON_IsTrue(val) || val->valueint == 1);
        encoded_val = static_cast<uint32_t> (cJSON_IsTrue(val) || val32 > 0);
    }
    else if(settings->bit_field || settings->enum_type || settings->random_enum_type)
    {
    ...
    else if(settings->floating_pt)
    {
        ...
        float dval = (float)scaled_val;
        memcpy(&encoded_val, &dval, sizeof(encoded_val));
        //printf( "%s float scaled_val %f  dval %f enc %08x  \n", __func__, scaled_val, dval, encoded_val);
    }
    // indirect float: scaled and truncated to int, then transmitted as int to be descaled on client side
    else if(settings->scale != 0.0)
    {
        ...
        if (settings->sign)
        {
            if (scaled_val > 2147483647)scaled_val = 2147483647;
            if (scaled_val < -2147483648)scaled_val = -2147483648;
            int casted_val = static_cast<int> (scaled_val);
        //printf( "%s scaled_val %f  scale %f enc %d .. %08x  \n", __func__, scaled_val, settings->scale, casted_val, casted_val);
            memcpy(&encoded_val, &casted_val, sizeof(encoded_val));
        }
        else
        {
            if (scaled_val > 2147483647.0*2)scaled_val = 2147483647.0*2;
            if (scaled_val < 0)scaled_val = 0;
            int casted_val = static_cast<uint32_t> (scaled_val);
        //printf( "%s scaled_val %f  scale %f enc %d .. %08x  \n", __func__, scaled_val, settings->scale, casted_val, casted_val);
            memcpy(&encoded_val, &casted_val, sizeof(encoded_val));
        }
        // int casted_val = static_cast<int> (scaled_val);
    ...
    else
    {
        ...
        //printf( "%s scaled_val %f  scale %f enc %d .. %08x  \n", __func__, scaled_val, settings->scale, casted_val, casted_val);
            ...
            //val32 = std::clamp(val32, -2147483648, 2147483647);  // Clamp to signed 32-bit range
        }
        else
        {
            if (scaled_val > 2147483647.0*2)scaled_val = 2147483647.0*2;
            if (scaled_val < 0)scaled_val = 0;
            encoded_val = static_cast<uint32_t> (scaled_val);
        }
...
{
    cJSON* val = unwrap_if_clothed(obj);
    int val32 = val->valueint;  // Your 32-bit integer this is all we can map unless we use the double
    double scaled_val = val->valuedouble;
    //printf( "%s val32  %d  scaled_val %f  %ld\n", __func__, val32, scaled_val, (long int)scaled_val);
    ...
    // source value may be in a variety of data types. use configuration settings to know which data type to expect
    // and parse the value into the uint32_t appropriately, including necessary type casts and/or direct memory copies.
    uint64_t encoded_val = val32;
    if (settings->is_bool)
    {
        //encoded_val = static_cast<uint64_t> (cJSON_IsTrue(val) || val->valueint == 1);
        encoded_val = static_cast<uint64_t> (cJSON_IsTrue(val) || val32 > 0);
    }
    else if(settings->bit_field || settings->enum_type || settings->random_enum_type)
    {
    ...
    else if(settings->scale != 0.0)
    {
        ...
        {
            if (scaled_val > 0xffffffffffffffff)
                scaled_val = 0xffffffffffffffff;            
            if (scaled_val < 0)
                scaled_val = 0;            
...
void update_holding_or_input_register_value(maps* settings, cJSON* value, uint16_t* regs, system_config* sys_cfg)
{
    ...
    {
        if (sys_cfg->low_debug)
            FPS_ERROR_PRINT(" size 2 Old value [%d.%d] New value [%d] .\n", regs[settings->reg_off], regs[settings->reg_off+1], val);
    ...
    else if(settings->num_regs == 4)
    {
        ...
        if (sys_cfg->low_debug)
            FPS_ERROR_PRINT(" size 4 Old value [%d.%d.%d.%d] New value [%ld] .\n"
                ,regs[settings->reg_off]
                , regs[settings->reg_off+1]
                , regs[settings->reg_off+2]
                , regs[settings->reg_off+3]
                , val64);
        if (!sys_cfg->byte_swap) // regular
        {
...
bool process_modbus_message(int bytes_read, int header_length, system_config* config, server_data* server_map, bool serial, uint8_t * query)
{
    datalog* data = server_map->data;
            ...
            if(send_body != NULL)
            {
                if(value == 0 || value == 0xFF00)
                {
                    ...
                    if (!invert)
                        ...
                        cJSON_AddBoolToObject(send_body, "value", (value == 0xFF00) ? false : true);

                    char* body_msg = cJSON_PrintUnformatted(send_body);
                    p_fims->Send("set", uri, NULL, body_msg);
                    free(body_msg);
...
bool process_fims_message(fims_message* msg, system_config* sys_cfg, bool* reload = nullptr)
{
    char* uri = msg->uri;
    // uri was sent to this processes URI so it needs to handle it
    //FPS_ERROR_PRINT("uri %s base %s.\n", uri, base_uri);
    if(strncmp(uri, base_uri, strlen(base_uri)) == 0)
    {
        ...
        if(strcmp("set", msg->method) == 0)
        {
            ...
            {
                uri = msg->pfrags[3] - 1;
                uri_map::iterator uri_it = uri_to_register.find(uri);
                if(uri_it == uri_to_register.end())
                {
    ...
    else // Check to see if this is a publish we care about
    {
        ...
        {
            //FPS_ERROR_PRINT("Received pub uri : [%s]\n", msg->uri);
            uri_map::iterator uri_it = uri_to_register.find(msg->uri);
            if(uri_it == uri_to_register.end())
            {
            ...
            for(cJSON* id_obj = body_obj->child; id_obj != NULL; id_obj = id_obj->next)
            {
                ...
                auto current_device = device_map.find(body_it->second.device_id);
                modbus_mapping_t* current_mapping = current_device != device_map.end() ? current_device->second->mb_mapping : server_map->mb_mapping;
                update_variable_value(current_mapping, body_it->second.reg_types, body_it->second.mappings, id_obj, sys_cfg);
...
bool initialize_map(server_data* server_map, system_config* sys_cfg, bool is_main_map)
{
    if (is_main_map) {

        if(p_fims->Subscribe((const char**)uris, num_uris + 1) == false)
        {
    ...
    for(uri_map::iterator it = uri_to_register.begin(); it != uri_to_register.end(); ++it)
    {
        ...
        while(p_fims->Connected() && found != true &&
             ...
             (timeout.tv_sec == current_time.tv_sec && timeout.tv_nsec > current_time.tv_nsec + 1000)))
        {
            ...
            fims_message* msg = p_fims->Receive_Timeout(us_timeout_left);
            if(msg == NULL)
                continue;
            bool rc = process_fims_message(msg, sys_cfg);
        ...
        if(found == false)
        {
            ...
            for (const auto& [uri, uri_info] : *it->second) {
                if (!uri_info.inited) {
                    FPS_ERROR_PRINT("Failed to initialize [%s]\n", uri);
                }
            }
            // FPS_ERROR_PRINT("Failed to find get response for %s.\n", it->first);
            return false;
...
int main(int argc, char *argv[])
{
    ...
    do
    {
        ...
        if (cJSON_IsObject(registers)) {
            ...
            int device_id = sys_cfg.device_id;
            server_map = create_register_map(registers, device_id);
            if (server_map == NULL)
            {
            ...
            // lets just use device_map;
            device_map[device_id] = server_map;
            server_map = nullptr;
        ...
        // configure Modbus mapping for devices
        ...
        // configure default Modbus mapping
        if(server_map)
        {
            server_map->mb_mapping = modbus_mapping_new_start_address(
                                        server_map->data[Coil].start_offset,             server_map->data[Coil].num_regs,
                                        server_map->data[Discrete_Input].start_offset,   server_map->data[Discrete_Input].num_regs,
                                        server_map->data[Holding_Register].start_offset, server_map->data[Holding_Register].num_regs,
                                        server_map->data[Input_Register].start_offset,   server_map->data[Input_Register].num_regs);
        ...
        while(running)
        {
            ...
            for(int current_fd = 0; current_fd <= fd_max; current_fd++)
            {
                ...
                else if(current_fd == fims_socket)
                {
                    ...
                    fims_message* msg = p_fims->Receive_Timeout(1*MICROSECOND_TO_MILLISECOND);
                    if(msg == NULL)
                    {
                        if(p_fims->Connected() == false)
                        {
        ...
        FPS_ERROR_PRINT("Main loop complete: Entering clean up device map.\n");
        if(!device_map.empty())
        {
            for (auto& mapping : device_map) {
                FPS_ERROR_PRINT("Main loop complete: Entering clean up device map id %d.\n", mapping.first);

                for (int i = 0; i < Num_Register_Types; i++)
                {
                    if(mapping.second->data[i].register_map != NULL)
                    {
                        FPS_ERROR_PRINT("Main loop complete: Entering clean up register map %d id %d.\n", i, mapping.first);
                        delete[] mapping.second->data[i].register_map;
                    }
                    if(mapping.second->regs_to_map[i] != NULL) 
                    { 
                        FPS_ERROR_PRINT("Main loop complete: Entering clean up regs to map %d id %d.\n", i, mapping.first);
                        delete[] mapping.second->regs_to_map[i];
                    }
                }
                if(mapping.second->mb_mapping != NULL)
                {
                    modbus_mapping_free(mapping.second->mb_mapping);
                    mapping.second->mb_mapping = nullptr;
                }
        ...
        FPS_ERROR_PRINT("Main loop complete: Entering uri_to_register.\n");
        uri_to_register.clear();
        FPS_ERROR_PRINT("Main loop complete: Entering server_map->data.\n");
        for (int i = 0; i < Num_Register_Types; i++)
        {
        ...
        if (uris != NULL) delete []uris;
        FPS_ERROR_PRINT("Main loop complete: Entering clean up server pfims.\n");
        if(p_fims != NULL)
        {
            if(p_fims->Connected() == true)
            {         
