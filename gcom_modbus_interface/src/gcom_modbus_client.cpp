...
void print_help() {
  std::cout << " Gcom Modbus Client :" << std::endl;
  std::cout
      << " use  gcom_modbus_client <-c | -u > <config name> [override options] "
      << std::endl;

  std::cout << "\n    override options :" << std::endl;
...
int main(const int argc, const char *argv[]) noexcept {
  ...
  // bool fileOk = gcom_load_overrides(myCfg, command_line_overrides_start,
  // argc, argv);
  bool fileOk = gcom_load_cfg_file(gcom_map, filename, myCfg, debug);
  fileOk |=
      gcom_load_overrides(myCfg, command_line_overrides_start, argc, argv);
  ...
  if (!myCfg.connection.is_RTU) {
    ...
    auto ret =
        hostname_to_ip(myCfg.connection.ip_address, new_ip, HOST_NAME_MAX);
    if (ret == 0) {
      myCfg.connection.ip_address = new_ip;
    } else {
      FPS_ERROR_LOG("ip_address \"%s\" isn't valid or can't be found from the "
                    "local service file",
