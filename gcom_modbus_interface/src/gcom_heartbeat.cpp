...
void hbSendWrite(cfg::heartbeat_struct *myhb)
{
    ...
    if(io_read_point && io_write_point)
    {
        ...
        {
            auto io_work = make_work(io_write_point->component, io_write_point->register_type, io_write_point->device_id, io_write_point->offset,  io_write_point->off_by_one,
                    io_write_point->size,
...
void hbCallback(std::shared_ptr<TimeObject> t, void *p)
{
    ...
    if(io_read_point)
    {
        ...
        if(myhb->init)
        {
            if ((tNow - myhb->last_time) > (myhb->timeout/1000.0))
            {
                if (!myhb->state_frozen)
                {
                    ...
                    std::string message = fmt::format("Heartbeat [{}] [{}] has entered the [frozen] state.", myhb->id, io_read_point->id);
            ...
            if ((tNow - tUpdate) > (myhb->timeout/1000.0))
            {
                if (!myhb->state_fault)
                {
                    if(debug)std::cout << " heartbeat faulted [" << io_read_point->id 
                        << "]  value : [" << raw_val 
                        << "] at time :"<< tNow <<  std::endl; 
                    myhb->state_fault = true;
                    myhb->state_str = "FAULT";
                    std::string message = fmt::format("Heartbeat [{}] id [{}] has entered the [timeout] state.", myhb->id, io_read_point->id);
        ...
        if(debug)std::cout << " heartbeat callback  #1 [" << myhb->id << "] time :"<< tNow << " value :" << myhb->value 
    ...
    if(myhb->enabled && io_read_point)
    {
            ...
            || (io_read_point->register_type == cfg::Register_Types::Input && !myCfg->pub_input)) 
        {
            ...
            auto io_work = make_work(io_read_point->component, io_read_point->register_type, io_read_point->device_id, io_read_point->offset,  io_read_point->off_by_one, io_read_point->size,
