...
void Stats::start(std::mutex &mtx) {
    std::unique_lock<std::mutex> lock(mtx); 
    start();
}

void Stats::snap(std::mutex &mtx) {
    snap();
}
...
void Stats::showNum(std::mutex &mtx, std::stringstream &ss, std::string _label) {
    std::unique_lock<std::mutex> lock(mtx); 
    showNum(ss, _label);
}

void Stats::showNum(std::stringstream &ss, std::string _label) 
{
    //std::unique_lock<std::mutex> lock(mtx); 
    if (count > 0)
    {
...
void Stats::showNum(std::mutex &mtx, std::stringstream &ss) {
    std::unique_lock<std::mutex> lock(mtx); 
    showNum(ss);
}

void Stats::showNum(std::stringstream &ss) 
{
    //std::unique_lock<std::mutex> lock(mtx); 
    if (count > 0)
    {
...
void Stats::clear(std::mutex &mtx) {
    std::unique_lock<std::mutex> lock(mtx); 
    clear();
}
...
void Stats::record_duration(std::mutex &mtx, double duration) {
    std::unique_lock<std::mutex> lock(mtx); 
...
void Stats::record_duration(std::mutex &mtx, int duration) {
    std::unique_lock<std::mutex> lock(mtx); 
    record_duration(duration);
}

void Stats::record_duration(int iduration) {
    //std::unique_lock<std::mutex> lock(mtx); 

    count++;
    double duration = (double)iduration;
