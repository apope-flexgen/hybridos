...
struct cfg
{
    ...
    struct inherited_fields_struct
    {
        ...
        double dbi_update_frequency = 0;  // in seconds
        double inter_message_delay = 0;   // in seconds (after being read in as milliseconds from config)
    ...
    struct io_point_struct
    {
        ...
        static bool compare(const io_point_struct& a, const io_point_struct& b) { return a.offset < b.offset; };
    };
    ...
    struct register_group_struct
    {
        ...
        std::string id;  // this is the second part of the uri. e.g. "comp_sel_2440" in the uri
                         // /components/comp_sel_2440 - inherited from the component that this register group belongs to
        std::string component_id;  // this is the first part of the uri. e.g. "components" in /components/comp_sel_2440
                                   // - inherited from the component that this register group belongs to
        component_struct* component;
    };
    ...
    struct component_struct
    {
        std::string component_id;  // this is the first part of the uri. e.g. "components" in /components/comp_sel_2440
        std::string
            id;  // this is the second part of the uri. e.g. "comp_sel_2440" in the uri /components/comp_sel_2440
        ...
        double inter_message_delay = 0;
        double last_message_time = 0;

        Fims_Format format = Fims_Format::Undefined;

        std::vector<std::shared_ptr<register_group_struct>> register_groups;
        double syncPct = 0.0;   // if polls take too long to retrieve data, what is our threshold (as a percent of pub
                                // frequency) for pushing back the next poll?
    ...
    struct heartbeat_struct
    {
        ...
        void setupHb(struct cfg& myCfg, const std::string& name, int device_id,
                     std::shared_ptr<struct io_point_struct> io_point, std::shared_ptr<struct io_point_struct> io_ref,
    ...
    struct watchdog_struct
    {
        ...
        void setupWatchdogTimer(struct cfg& myCfg, const std::string& name, double watchdog_frequency,
                                double watchdog_offset);
        void touchWatchdog();
        void checkWatchdog();
        void setNormalState();
        void setFaultState();
    ...
    struct request
    {
        ...
        request(const std::string& name, const std::string& help,
                std::function<bool(struct cfg&, std::shared_ptr<cfg::request>)> reqfun)
    ...
    // config_any
    void addPub(const std::string& base, const std::string& component_name, std::shared_ptr<component_struct> component,
                struct cfg* myCfg);
    void addWatchdog(const std::string& base, const std::string& component_name,
                     std::shared_ptr<component_struct> component, struct cfg* myCfg);
    void addWatchdog(std::shared_ptr<io_point_struct> io_point, std::string& pstr,
                     std::shared_ptr<component_struct> component, struct cfg* myCfg);
    void addOldWatchdog(const std::string& base, const std::string& component_name,
                        std::shared_ptr<component_struct> component, struct cfg* myCfg);
    void addOldWatchdog(std::shared_ptr<io_point_struct> io_point, std::string& pstr,
                        std::shared_ptr<component_struct> component, struct cfg* myCfg);
    void addHeartbeat(const std::string& base, const std::string& component_name,
                      std::shared_ptr<component_struct> component, struct cfg* myCfg);
    ...
    using Component_IO_point_map =
        std::map<std::string, std::map<std::string, std::map<std::string, std::shared_ptr<io_point_struct>>>>;
     ...
     */
    std::map<std::string, std::shared_ptr<cfg::io_point_struct>>* findIOPointMapFromUriFragments(
    ...
    std::string filename;
    std::map<std::string, std::shared_ptr<dbi_update_struct>>
        dbi_update_map;  // map of component uri to dbi update struct
    ...
    bool allow_gaps = false;  // do we allow gaps between registers when doing a single read?
                              // (e.g. We have [offset 100, size 1], and [offset 200, size 1]. Do we do a single read or
                              // two reads?) setting this to false for now because allowing gaps was causing issues
...
struct Uri_req
{
    ...
    void set_uri(std::string_view& uri_view)
    {
        ...
        for (int idx = 0; idx < num; ++idx)
        {
            ...
            {
                num_uris--;
                if (uri_vec[idx] == Raw_Request_Suffix)
                    is_raw_request = true;
                else if (uri_vec[idx] == Timings_Request_Suffix)
                    is_timings_request = true;
                else if (uri_vec[idx] == Reset_Timings_Request_Suffix)
                    is_reset_timings_request = true;
                else if (uri_vec[idx] == Reload_Request_Suffix)
                    is_reload_request = true;
                else if (uri_vec[idx] == Enable_Suffix)
                    is_enable_request = true;
                else if (uri_vec[idx] == Disable_Suffix)
                    is_disable_request = true;
                else if (uri_vec[idx] == Connect_Suffix)
                    is_connect_request = true;
                else if (uri_vec[idx] == Disconnect_Suffix)
                    is_disconnect_request = true;
                else if (uri_vec[idx] == Force_Suffix)
                    is_force_request = true;
                else if (uri_vec[idx] == Unforce_Suffix)
                    is_unforce_request = true;
                else if (uri_vec[idx] == Local_Suffix)
                    is_local_request = true;
                else if (uri_vec[idx] == Remote_Suffix)
                    is_remote_request = true;
                else if (uri_vec[idx] == Full_Suffix)
                    is_full_request = true;
                else if (uri_vec[idx] == Stats_Suffix)
                    is_stats_request = true;
                else if (uri_vec[idx] == Server_Suffix)
                    is_server_request = true;
                else if (uri_vec[idx] == Connection_Suffix)
                    is_debug_connection_request = true;
                else if (uri_vec[idx] == Response_Suffix)
                    is_debug_response_request = true;
                else if (uri_vec[idx] == Help_Suffix)
...
public:
    ...
    // Constructor
    PublishGroup(const std::string& component_id, const std::string& id, int freq, int offset)
...
using Component_IO_point_map =
    std::map<std::string, std::map<std::string, std::map<std::string, std::shared_ptr<cfg::io_point_struct>>>>;
 ...
 * @param raw_register_groups_json_array an std::any struct representing all register_groups in a component (as a json
 * list)
 * @param component a pre-populated component_struct, typically belonging to myCfg
 * @param myCfg config structure to load file data into
 * @param debug bool value representing whether or not to print messages to help debug code
 */
bool extract_register_groups(std::vector<std::shared_ptr<cfg::register_group_struct>>& register_groups,
                             const std::any& raw_register_groups_json_array,
                             std::shared_ptr<struct cfg::component_struct> component, struct cfg& myCfg, bool debug);
 ...
 * @param packed_io_point the parent io_point that contains packed registers within it. NULL if this is the parent
 * io_point or if the io_point is not packed.
...
bool extract_io_point_map(std::shared_ptr<struct cfg::register_group_struct> register_group,
                          std::shared_ptr<cfg::io_point_struct> packed_io_point, std::any& rawIOPointList,
                          struct cfg& myCfg, bool debug);
...
void checkNonZeroIOPointSize(std::shared_ptr<cfg::register_group_struct> register_group,
                             std::vector<std::shared_ptr<cfg::io_point_struct>> io_point_map, bool debug);
 ...
 * @param uri the URI request found in a fims message
 * @param regs16 a pointer to the 4x16-bit register to store the uint64 value. If nullptr, function will omit storing
 * the value.
 *
 * @return the 64-bit value (also stored in regs16 if not null)
 */
uint64_t get_any_to_uint64(std::shared_ptr<cfg::io_point_struct> io_point, std::any val, Uri_req& uri,
                           uint16_t* regs16);
 ...
 *
 * using Component_IO_point_map = std::map<std::string, std::map<std::string, std::map<std::string,
 * std::shared_ptr<cfg::io_point_struct>>>> (Map of component_prefix->component_id->io_point_id->io_point)
 *
 ...
 */
void addIOPointToComponentIOPointMap(Component_IO_point_map& imap, const char* component_uri_prefix,
                                     const char* component_id, std::shared_ptr<cfg::io_point_struct> io_point);
 ...
 */
bool ioPointExists(std::shared_ptr<cfg::io_point_struct>& io_point, const struct cfg& myCfg,
                   const std::vector<std::string>& uri_keys, int key_size, std::string io_point_key);
 ...
 */
void showDeviceRegisterGroup(struct cfg& myCfg, int dev_id, cfg::Register_Types reg_group_type,
                             std::map<int, cfg::io_point_struct*> io_point_map_lookup, std::stringstream& ss);
 ...
 * @param include_all_points bool value specifying whether or not to include all given points. If true, IO_Work objects
 * will include disabled and forced points. If false, IO_Work objects will exclude these points
 * @param debug bool value representing whether or not to print messages to help debug code
 */
void check_work_items(std::vector<std::shared_ptr<IO_Work>>& io_work_vec,
                      std::vector<std::shared_ptr<cfg::io_point_struct>>& io_map_vec, struct cfg& myCfg,
                      const char* work_type, bool include_all_points, bool debug);
...
std::shared_ptr<cfg::io_point_struct> findItem(
    const std::map<std::string, std::map<std::string, std::map<std::string, std::shared_ptr<cfg::io_point_struct>>>>&
        items,
    const std::string& component, const std::string& id, const std::string& name);
...
bool add_all_component_points_to_io_vec(std::vector<std::shared_ptr<cfg::io_point_struct>>& io_point_vec,
...
bool parseFimsMessage(struct cfg& myCfg, const Component_IO_point_map& items,
                      std::vector<std::pair<std::shared_ptr<cfg::io_point_struct>, std::any>>& result,
                      const std::string& method, const std::string& uri, const std::string& body);
bool compressResults(const std::vector<std::pair<std::shared_ptr<cfg::io_point_struct>, std::any>>& results,
...
bool extract_publish_groups(std::vector<std::shared_ptr<PublishGroup>>& publishGroups, const ItemMap& items,
