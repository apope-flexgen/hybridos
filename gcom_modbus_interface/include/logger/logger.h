...
#define FPS_DEBUG_LOG(...)                                  \
    do {                                                          \
        ...
        ::Logging::log_msg(spdlog::level::debug,                   \
                           ::Logging::pre_string(__FILE__,        \
                                                __FUNCTION__,     \
                                                __LINE__),        \
                           ::Logging::msg_string(__VA_ARGS__),    \
...
#define FPS_INFO_LOG(...)                                  \
    do {                                                          \
        std::unique_lock<std::mutex> lock(logger_mutex);          \
        ::Logging::log_msg(spdlog::level::info,                   \
                           ::Logging::pre_string(__FILE__,        \
                                                __FUNCTION__,     \
                                                __LINE__),        \
                           ::Logging::msg_string(__VA_ARGS__),    \
...
#define FPS_WARNING_LOG(...)                                  \
    do {                                                          \
        std::unique_lock<std::mutex> lock(logger_mutex);          \
        ::Logging::log_msg(spdlog::level::warn,                   \
                           ::Logging::pre_string(__FILE__,        \
                                                __FUNCTION__,     \
                                                __LINE__),        \
                           ::Logging::msg_string(__VA_ARGS__),    \
...
#define FPS_ERROR_LOG(...)                                 \
    do {                                                          \
        std::unique_lock<std::mutex> lock(logger_mutex);          \
        ::Logging::log_msg(spdlog::level::err,                    \
                           ::Logging::pre_string(__FILE__,        \
                                                __FUNCTION__,     \
                                                __LINE__),        \
                           ::Logging::msg_string(__VA_ARGS__),    \
...
#define FPS_LOG_IT(filename)                               \
    do {                                                          \
        std::unique_lock<std::mutex> lock(logger_mutex);          \
        ::Logging::log_it(filename);                              \
    } while (false)
