...
public:
    ...
    ~TimeObject()
    {
        std::cout << "Timer object ["<< name<<"] id " <<id << " deleted " << std::endl; 
    }
...
void addTimeObject(std::shared_ptr<TimeObject> obj, double offset, bool sync);
...
bool syncTimeObjectByName(const std::string& name, double error_margin_percent);
