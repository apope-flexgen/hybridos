...
TEST(LocalReadRegistersTest, ReadsDataCorrectly)
{
    cfg myCfg;
    ...
    local_read_registers(myCfg, io_work, false);

    EXPECT_EQ(io_work->buf16[0], 1);
    EXPECT_EQ(io_work->buf16[1], 2);
    EXPECT_EQ(io_work->buf16[2], 3);
    EXPECT_EQ(io_work->buf16[3], 4);
    EXPECT_EQ(io_work->buf16[4], 5);
    EXPECT_EQ(io_work->errors, 5);
}

TEST(LocalReadRegistersTest, HandlesEmptyIOPoints)
{
    cfg myCfg;
    auto io_work = std::make_shared<IO_Work>();
...
TEST(LocalReadRegistersTest, HandlesEmptyBuffer)
{
    cfg myCfg;
    auto io_work = std::make_shared<IO_Work>();
...
TEST(LocalReadRegistersTest, HandlesNullIOPoints)
{
    cfg myCfg;
    auto io_work = std::make_shared<IO_Work>();
...
TEST(LocalReadRegistersTest, ThreadSafety)
{
    cfg myCfg;
    auto io_work = std::make_shared<IO_Work>();
    ...
    std::thread t([&]() { local_read_registers(myCfg, io_work, false); });
