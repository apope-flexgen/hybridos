...
try:
    from user_global_utils import USER, INTERFACE, LOCAL_PYTHON_SCRIPT_DIR, \
    ...
    from global_utils import USER, INTERFACE, LOCAL_PYTHON_SCRIPT_DIR, \
    TEST_CONSOLE_OUTPUT_DIR, SERVER_CONTAINER, CLIENT_CONTAINER, \
    DOCKER_PYTHON_SCRIPT_DIR, CLIENT_PORT, SERVER_PORT, LOCAL_HYBRIDOS_DIR, \
    CONFIGS_DIR, DIRS, PATH_TO_SSH_TOKENS, USE_LATEST_RELEASE
...
def run_server(filename: str, stop: threading.Event) -> None:
    ...
    if "gcom_modbus" in INTERFACE:
        ...
        with open(f"{LOCAL_PYTHON_SCRIPT_DIR}/{TEST_CONSOLE_OUTPUT_DIR}/{filename}_{timestamp.file_fmt}.txt",
                  'w', newline='\n', encoding="utf-8") as file:
            with subprocess.Popen(["docker", "exec", "-it", f"{SERVER_CONTAINER}",
                                   f"{DOCKER_PYTHON_SCRIPT_DIR}/bin/{INTERFACE}_server", f"{DOCKER_PYTHON_SCRIPT_DIR}/{CONFIGS_DIR}/{filename}.json"],
                                   stdout=file, stderr=subprocess.STDOUT, text=True, bufsize=1) \
                                   as console_output_stream:
                while not stop.is_set():
                    try:
...
def run_client(filename: str, stop: threading.Event) -> None:
    ...
    with open(f"{LOCAL_PYTHON_SCRIPT_DIR}/{TEST_CONSOLE_OUTPUT_DIR}/{filename}_{timestamp.file_fmt}.txt",
        ...
        with subprocess.Popen(["docker", "exec", "-it", f"{CLIENT_CONTAINER}",
                               f"{DOCKER_PYTHON_SCRIPT_DIR}/bin/{INTERFACE}_client", f"{DOCKER_PYTHON_SCRIPT_DIR}/{CONFIGS_DIR}/{filename}.json"],
                               stdout=file, stderr=subprocess.STDOUT, text=True, bufsize=1) \
                               as console_output_stream:
            while not stop.is_set():
                try:
...
def kill_containers(client_container, server_container) -> None:
    ...
    '''
    flask_kill = f"pkill python*\n"
    client_kill = f"kill gcom\n"
    server_kill = f"pkill gcom\n"
    print(f"Killing {INTERFACE}_client...")
    command = client_kill
    command.replace("+","%2B")
    requests.get(f"http://localhost:{CLIENT_PORT}/docker/run_command?command={command}")
    ...
    #                       f"/docker/run_command?command={flask_kill}")

    print(f"Killing {INTERFACE}_server...")
    command = server_kill
    command.replace("+","%2B")
    requests.get(f"http://localhost:{SERVER_PORT}/docker/run_command?command={command}")

    # command = flask_kill
    # command.replace("+","%2B")
    # requests.get(f"http://localhost:{SERVER_PORT}/docker/run_command?command={command}")
    # # client_container.join(1, requests.get, f"http://localhost:{SERVER_PORT}" + \
    #                       f"/docker/run_command?command={flask_kill}")
    print(f"Not Stopping {CLIENT_CONTAINER}...")
    #subprocess_run_quiet(["docker","stop",CLIENT_CONTAINER])
    print(f"Not Stopping {SERVER_CONTAINER}...")
