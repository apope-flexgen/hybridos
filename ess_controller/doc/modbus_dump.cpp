...
/* FIMS Communication */
int handle_messages(fims* p_fims, char* base_uri, cJSON* data, connection_config* connection,
                    component_config* component);
...
bool get_conn_info(sys_t* sys, cJSON* config)  //, connection_config *connection)
{
    ...
    // cJSON* serial_device = cJSON_GetObjectItem(connection_json,
    // "serial_device");
    ...
    //     connection->baud = (baud_rate == nullptr) ? 115200 :
    //     baud_rate->valueint; cJSON* parity_str =
    //     cJSON_GetObjectItem(connection_json, "parity"); connection->parity =
    //     'N';
    ...
    // else
    // {
    //     FPS_ERROR_PRINT("Failed to find either an IP_Address or Serial_Device
    //     in JSON.\n"); free(connection->name); return -1;
    // }
    return 0;
}
...
// int get_component(sys_t* sys, cJSON *config, component_config* comp, cJSON
// *component)
    ...
    for (int reg_set = 0; reg_set < config->reg_cnt; reg_set++)
    {
        if (config->reg_datalog[reg_set].reg_type == reg_type)
        {
            for (unsigned int i = 0; i < config->reg_datalog[reg_set].map_size; i++)
            {
                ...
                else if (config->reg_datalog[reg_set]
                             .register_map[i]
    ...
    for (int i = 0; i < cnum; i++)
    {
        ...
        for (int j = 0; j < pcomp->reg_cnt; j++)
        {
            ...
            for (int k = 0; k < (int)regd->map_size; k++)
            {
                ...
                if (psys->sysmaps[regd->reg_type].find((int)ritem->reg_off) != psys->sysmaps[regd->reg_type].end())
                {
...
void dump_server(sys_t* sys, connection_config* cfg)
{
    ...
    //     pcomp->id, num, pcomp->reg_cnt); reg_datalog = pcomp->reg_datalog;
    //     //foreach regtype ??
    //     for (int j =0 ; j < pcomp->reg_cnt; j++)
    //     {
    //         regd=&reg_datalog[j];
    //         FPS_ERROR_PRINT("Component [%d] id [%s] num %d regd %p map_size
    ...
    //             else
    //             {
    //                 //psys->sysmaps[regd->reg_type][(int)ritem->reg_off] =
    //                 ritem;
    //                 // check for reg in urimap
    //                 add_sys_sysmap(sys, ritem);
    //                 add_sys_urimap(sys, ritem);
    ...
    for (int j = 0; j < Num_Register_Types; j++)
    {
        if (psys->sysmaps[j].size() > 0)
        {
            ...
            for (it = psys->sysmaps[j].begin(); it != psys->sysmaps[j].end(); ++it)
            {
                ...
                fprintf(fp, "\"uri\":\"/components/%s\" }", rmap->uri);
            }
            fprintf(fp, "       ]");
            if (j < last_reg)
                fprintf(fp, ",");
