...
private:
    AvarMap() { cout << "getInstance(): First instance\n"; };
};
...
public:
    ...
    // this is a bit clumsy but it we allocate a child to a non object then the
    ...
    AssetVar& operator[](string pname)
    {
        ...
        // cout << " this->name ["<< this->name << "] "<<this<<"  Params size :"<<
        // Params.size()<<endl;
        ...
        if (Params.find(pname) == Params.end())
        {
            ...
            {
                // cout << " Creating  [" << "value" << "] in ["<< this->name<<
                // "]"<<endl;
                AssetVar* av;
    ...
    // turns parent into an array and creates ann array entry Object which it
    // returns. 2/ parent is an array , finds / adds item
    //  returns entry.
    AssetVar& operator[](int idx)
    {
        ...
        // {
        // 	cout << __func__ << " old size : "<< aList.size()
        // << " idx : " << idx << " extending"<<endl;
        // }
    ...
    AssetVar* getParent() { return parent; }
    ...
    {
        if (Params.find(pname) != Params.end())
            return Params[pname]->valuedouble;
        return 0.0;
    }
    ...
    {
        if (Params.find(pname) != Params.end())
            return Params[pname]->valuestring;
        return nullptr;
    }
    ...
    void show(int level = 0);      // { cout << name << " type :" << atype << " int :"
                                   // <<valueint << " dbl :"<< valuedouble<<endl; }
    void showKids(int level = 0);  // { cout << name << " type :" << atype << " int
                                   // :" <<valueint << " dbl :"<<
                                   // valuedouble<<endl; }
};
...
// //static
// unsigned char utf16_literal_to_utf8(const unsigned char * const
// input_pointer, const unsigned char * const input_end, unsigned char
// **output_pointer);
