...
// the AssetVar cstring really means nothing it is an abstract object with one or more identities
...
// the Av will keep its original comp name (/components/sbmu_1) and its cname ( bms_soc)
// to do this we have to have the aVmap <local_name,Av*>
// and a vector aVec <pair::<Av*,local_name (char *)>
// the ->prev ->next stuff will be handled by the vector.
// arrays   will have null pointers for local_name 
...
int test_main()
{
    ...
    const char* sp2 = "{\"foo\":{\"value2\":22},\"foo3\":{\"value3\":33}"
   ...
   // avtest["one"]["two"]["three"] =  3;   //deep param map
	...
	svec.clear();
	const char* sp = "{\"foo\":{\"fvalue\":21,\"myval\":100,\"actions\":{\"onSet\":[{\"enum\":["
	"{\"eone\":1, \"two_1\":2, \"three_1\":\"sthree\"},"
	"{\"etwo\":1, \"two_2\":2, \"three_2\":\"sthree\"}"
	"]}]},\"value\":234}}";
    
    // as you can see, you no longer have to type "\" for escape characters anymore, it is also easier to read.
    // format is as follows: R"(everything inside these paranthesis is your raw string)";
    const char* raw = R"(
        {
            "this": "is", 
            "a": "json", 
	...
	//cout << " looking for foo :actions "<< avmap2.getParam(0)->gotParam("foo")->gotParam("actions") << endl;
    ...
    vm2.addAction("/components/pcs/fast/statusPower"
                ,"onSet", "enum"
...
int runAvFunc(AssetVar*vmap, AssetVar* sv, AssetVar* av, fims* p_fims )
{
    ...
    if (!av->gotParam("enabled") || av->getbParam("enabled"))
    {
        ...
        if(1)FPS_ERROR_PRINT("%s >> Running one at %2.6f name [%s] func %s \n"
                , __func__, tNow, av->cstring, myfcn?myfcn:"No FCN");
