...
double getSchedDelay(AssetVar* vmap, alist* sreqs, fims* p_fims)
{
    ...
    if(0)cout <<__func__ << " Running at time " << tNow<<endl;        
    int runit = 1;
    AssetVar* av=NULL;
    double delay = 2.0;
    if (sreqs->size() > 0)
    {
        ...
        while(runit)
        {
            ...
            if(av)
            {
                ...
                if(0)FPS_ERROR_PRINT("%s >> Got one at %2.6f name [%s]  size %d \n"
                   , __func__, tNow, av->cstring, (int)sreqs->size());

                if(endRun)
                {
                    if(0)FPS_ERROR_PRINT("%s >> EndSet for name [%s] at %2.3f \n"
                ...
                { 
                    ...
                    if(response)
                    {
                        if(1)FPS_ERROR_PRINT("          %s >> Response [%s] from %2.6f at %2.6f elapsed mS %2.6f \n"
                            , __func__
                            , response 
                            , tNow
                            , get_time_dbl()
                            , (get_time_dbl()-tNow) * 1000.0
                            );
                    }
                ...
                if(0)FPS_ERROR_PRINT("%s >> repTime av %2.3f new size  %d\n"
                    , __func__
                ...
                {
                    if(0)FPS_ERROR_PRINT(" added av [%s] at %2.6f \n"
                            , av->cstring
                    ...
                    if((endTime == 0) || (runTime < endTime))
                    {
                        ...
                        addSchedReq(sreqs, av);// new run_req(rn->t_shot+rn->t_rep, rn->t_rep,rn->args));    
                    }
                    else
                    {
                        if(1)FPS_ERROR_PRINT("%s >> deleted av  [%s] at %2.6f  runTime %2.3f endTime %2.3f \n"
                            , __func__
                            , av->cstring
                            , tNow
                            , runTime
...
void schedThread(AssetVar* av, alist* sreqs, const char* aname, fims* p_fims)
{
    ...
    while (run)
    {
        ...
        //         // look for id allready allocated
                FPS_ERROR_PRINT(
                    "%s >> Servicing  Sched Request %p id [%d] name[%s]  runCnt %d repTime %2.3f at %2.3f\n"
                    , __func__
                    , sv
                    , sv->id
                    , sv->cstring
