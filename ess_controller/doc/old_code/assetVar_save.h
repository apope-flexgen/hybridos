...
public:
    ...
    void write_cjson(const char* fname, cJSON* cj)
    {
        ...
        FPS_ERROR_PRINT(" %s >> Wrote %zu bytes to  file %s\n", __func__, bytes_written, fname);
        fclose(fp);
        free((void*)res);
    }
    ...
    assetVar* runActBitFieldfromCj(varsmap& vmap, assetVar* av, assetAction* aa)
    {
        ...
        for (auto& x : aa->Abitmap)
        {
            ...
            if (0)
                FPS_ERROR_PRINT(
                    " %s >> ######on Set Bitfield action bit %d mask "
                    "0x%04x hbit 0x%04x  aval %d masked val %d uri [%s] "
                    "var [%s] tmpval [%s]\n",
                    __func__, (int)bit, (int)mask, (int)bit, aval, (int)aval & (int)mask, uri, var, abf->tmpval);
            if (aval & (1 << (int)bit))
            {
    ...
    assetVar* runActEnumfromCj(varsmap& vmap, assetVar* av, assetAction* aa)
    {
        ...
        for (auto& x : aa->Abitmap)
        {
            ...
            if (0)
                FPS_ERROR_PRINT(
                    " %s >> ######on Set Enum action bit %d mask 0x%04x "
                    "hbit 0x%04x  aval %d masked val %d uri [%s] var [%s] "
                    "tmpval [%s] outValue[%s]\n",
                    __func__, (int)bit, (int)mask, (int)bit, aval, (int)aval & (int)mask, uri, var ? var : "NoVar",
                    abf->tmpval, outValue ? outValue : "noOutvalue");
    ...
    assetVar* setOldValfromCj(varsmap& vmap, const char* comp, const char* var, cJSON* cj)
    {
        ...
        if (0)
        {
            ...
            FPS_ERROR_PRINT(" %s Cj >>Setval for comp [%s] var  [%s] cjval >>%s<< \n", __func__, comp ? comp : "NoComp",
                            var ? var : "NoVar", tmp ? tmp : "NoCjTmp"
                            //, (void *)cjact
            );
            free((void*)tmp);
        }
        ...
        if (cJSON_IsNumber(cj) || cJSON_IsBool(cj) || cJSON_IsString(cj))
        {
        ...
        else
        {
            ...
            if (0)
            {
                char* tmp = cJSON_PrintUnformatted(cj);
                FPS_ERROR_PRINT(
                    "%s ..2  setValfrom Cj.value >>Setval for cj >>%s<<\n "
                    "type %d \n cjval %p \n",
                    __func__, tmp, cj->type, (void*)cjval
                    //, (void *)cjact
                );
                free((void*)tmp);
            }
            ...
            {
                if (0)
                    FPS_ERROR_PRINT(" >>Setting  value for [%s]\n", var);
                av = getVar(vmap, comp, var);
                if (av)
                {
                    av->setVal(cjval);
                ...
                if (0)
                    FPS_ERROR_PRINT(" >>After Setting  value for [%s] aval (f) %f  () %d \n", var,
                                    av->aVal->valuedouble, av->aVal->valueint);
            }
    ...
    assetVar* setValfromCj(varsmap& vmap, const char* comp, const char* var, cJSON* cj)
    {
        ...
        if (0)
            FPS_ERROR_PRINT(" >>Adding a new Variable comp [%s] var [%s] cj is %p\n", comp, var, (void*)cj);
        ...
        else
        {
            ...
            char* tmp = cJSON_PrintUnformatted(cj);
            if (0)
                FPS_ERROR_PRINT(" setValfrom_Cj >>Adding NEW cj >>%s<<\n cjval %p \n cjact %p\n", tmp, (void*)cjval,
                                (void*)cjact);
            free((void*)tmp);
            assetVar* av = nullptr;
            if (cJSON_IsArray(cj))
            ...
            {
                if (0)
                    FPS_ERROR_PRINT(" >>Adding CJ value for [%s]\n", var);
            ...
            {
                if (0)
                    FPS_ERROR_PRINT(" >>Adding actions for [%s] av [%p]\n", var, (void*)av);
                if (av)
                {
    ...
    assetVar* getVar(varsmap& vmap, const char* comp, const char* var = nullptr)
    {
        ...
        if (0)
            FPS_ERROR_PRINT("%s lookng for comp [%s] var [%s]\n", __func__, mycomp, myvar);
    ...
    {
        if (0)
            FPS_ERROR_PRINT("%s >> getting cj maps uri [%s] var [%s] \n", __func__, uri ? uri : "noURI",
                            var ? var : "noVar");
        ...
        else if (uri && var)
        {
            ...
            if (x != vmap.end())
            {
                ...
                if (y != vmap[uri].end())
                {
                    ...
                    if (0)
                        printf(" getting cj for uri [%s] var [%s] \n", uri, y->first.c_str());
    ...
    void processMsgGet(varsmap& vmap, const char* method, const char* uri, const char* body, cJSON** cjr)
    {
        ...
        {
            if (0)
                FPS_ERROR_PRINT("%s >>  running with uri [%s] nfrags %d\n", __func__, uri, nfrags);
            *cjr = getMapsCj(vmap, uri);
            if (0)
                FPS_ERROR_PRINT("%s >> returned from uri [%s] nfrags %d\n", __func__, uri, nfrags);
        }
    ...
    void processMsgSetPub(varsmap& vmap, const char* method, const char* uri, const char* body, cJSON** cjr)
    {
        ...
        if (cj)
        {
            ...
            *cjr = cJSON_CreateObject();
            ...
            {
                if (0)
                    FPS_ERROR_PRINT(" %s >> [%s] cj->type %d , cj->child %p cj->next %p\n", __func__, cjrx->string,
                                    cjrx->type, cjrx->child, cjrx->next);
    ...
    void processMsgSetReply(varsmap& vmap, const char* method, const char* uri, const char* replyto, const char* body,
                            cJSON** cjr)
    {
        int nfrags = get_nfrags(uri);
        char* var = nullptr;
        ...
        *cjr = cJSON_CreateObject();
        ...
        if (cj)
        {
            if (0)
                printf(
                    " %s >>> method %s uri [%s] body [%s] cj->string [%s] cj->type "
                    "%s , cj->child %d cj->next %p\n",
                    __func__, method, uri, body, var ? var : "noVar", cj->string, cj->type, cj->child, cj->next);
            if (var)
            {
                if (0)
                    printf("%s >>> [%s] cj->type %d , cj->child %p cj->next %p\n", __func__, var, cj->type, cj->child,
                           cj->next);

                setValfromCj(vmap, uri, var, cj);
                if (0)
                    FPS_ERROR_PRINT(" MSGSetReply we got a set with a replyto [%s]\n", replyto);
            ...
            else
            {
                ...
                {
                    if (0)
                        FPS_ERROR_PRINT(" [%s] cj->type %d , cj->child %p cj->next %p\n", cji->string, cji->type,
                                        cji->child, cji->next);
