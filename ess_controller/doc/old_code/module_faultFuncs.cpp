...
#define BMSStopResp 5
        int ShutdownSystem(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* aV)
        {
            ...
            if (reload < 2)
            {
                ...
                // other variables:
                linkVals(*vm, vmap, amap, aname, "/status", bval, "EStop", "UiShutdown", "SMShutdown", "FaultShutdown", "FullShutdown", "FullFaultShutdown");
            ...
            // bool ShutdownCompleted = amap["ShutdownCmd"]->getbParam("ShutdownCompleted");
            ...
            // deprecating FullShutdown...only time batteries shut off is FullFaultShutdown
            // if (FullShutdownAv->getbVal() && FullShutdownAv->valueChangedReset())
            // {
            //     amap["ShutdownCmd"]->setVal(true);
            //     amap["ShutdownCmd"]->setParam("FullShutdown", true);
            // }
            // want to stop PCS if BMS cuts off
            // if ((PCSStatusResp != Power_Electronics::OFF && PCSStatusResp != Power_Electronics::POWERUP) && (BMSStatusResp == CATL::POWEROFFREADY || BMSStatusResp == CATL::POWERONFAULT || BMSStatusResp == CATL::POWEROFFFAULT))
            // {
            //     // amap["ShutdownCmd"]->setVal(true);
            //     FPS_ERROR_PRINT("%s >>> E-STOPPING!!! PCSStatusResp [%d] BMSStatusResp [%d]\n", __func__, PCSStatusResp, BMSStatusResp);
            //     // amap["ShutdownCmd"]->setParam("FullShutdown", true);
            // }
            ...
            if (ShutdownCmd) // TODO: Make these one-shots
            {
                ...
                if (1) FPS_ERROR_PRINT("UIShutdown: %s, SMShutdown: %s, FaultShutdown: %s, FullFaultShutdown: %s\n",
                ...
                if (!ShutdownRequested) // we are having a shutdown requested here when you set ShutdownCompleted to false.
                {
                    ...
                    if (temp_av) temp_av->sendEvent("ESS", am->p_fims, Severity::Info, "Setting shutdown time at time %2.3f"
                        , vm->get_time_dbl()
                        );
                }
            }
            ...
            for (auto& ix : am->assetManMap) // should get (const?) references to pairs NOT copies.
            {
                ...
                if (amc->name == "bms")
                {
                    if (0) std::cout << std::boolalpha << "PCSFaulted?: " << PCSFaulted << '\n';
                    if (0) FPS_ERROR_PRINT("manager_name: [%s] BMSShutdown [%s] PCSShutdown [%s]\n"
                        , amc->name.c_str()
                        , BMSShutdown ? "true" : "false"
                        , PCSShutdown ? "true" : "false"
                    );

                    if (amap["ShutdownCmd"]->getbParam("FullShutdown")) // && (!strcmp(PCSStatusResp, "OFF") || !strcmp(PCSStatusResp, "FLT"))) // || PCSStatusResp == Power_Electronics::POWERUP )) //
                    {
                        if(!BMSFaulted && !BMSShutdown && PCSShutdown) // || PCSFaulted)) // This logic was added to guarantee no cascade is done without PCS being shutdown first. Might need to be changed.
                        {
                            if (0) FPS_ERROR_PRINT("%s >>>> cascading function to >>>>> Manager [%s] at time: %f\n ", __func__, amc->name.c_str(), tNow);
            ...
            else if (!amap["ShutdownCmd"]->getbParam("FullShutdown") && amap["PCSShutdown"]->getbVal())    ///*(!amap["ShutdownCmd"]->getbParam("FullShutdown") || */
            {
            ...
            else if (amap["PCSShutdown"]->getbParam("Fault"))
            {
                ...
                amap["ShutdownCmd"]->setParam("ShutdownRequested", false);
                amap["ShutdownCmd"]->setParam("HardShutdown", false);
                if (1) FPS_ERROR_PRINT("%s >> PCS Shutdown Failed, elapsed time: %f \n", __func__, (tNow - sdtime));
                vm->sendAlarm(vmap, "/status/ess:ShutdownRequested", "/components/ess:alarms", "Shutdown", "Shutdown Completed", 1);
            }
            else if (amap["BMSShutdown"]->getbParam("Fault"))
            {
                ...
                amap["ShutdownCmd"]->setParam("ShutdownRequested", false);
                amap["ShutdownCmd"]->setParam("HardShutdown", false);
                if (1) FPS_ERROR_PRINT("%s >> BMS Shutdown Failed, elapsed time: %f \n", __func__, (tNow - sdtime));
                vm->sendAlarm(vmap, "/status/ess:ShutdownRequested", "/components/ess:alarms", "Shutdown", "Shutdown Completed", 1);
            }
        ...
        */
        ...
        int ShutdownPCS(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* aV)
        {
            ...
            // FPS_ERROR_PRINT("%s /// pcsAv->getiVal() %d\n",__func__,pcsAv->getiVal());
            ...
            if (reload == 0)
            {
                ...
                linkVals(*vm, vmap, amap, "ess", "/status", bval, "PCSShutdown"); //, "ShutdownCmd", "HardShutdown");
                ...
                if (1) FPS_ERROR_PRINT("%s >>>>>>>>>>>>>>>>PCS Shutdown system step 1 reload [%d] at time: %f \n", __func__, reload, tNow);
            }
            if (reload < 2)
            {
                ...
                if (!amap["HardShutdown"]) amap["HardShutdown"]->setVal(false);
                ...
                if (1) FPS_ERROR_PRINT("%s >>>>>>>>>>>>>>>>PCS Shutdown system state [%s] reload [%d] at time: %f \n", __func__, amap["SystemState"]->getcVal(), reload, tNow);
            ...
            if (!PCSStatusResp)     // comms OK as well?
            {
                ...
                if (temp_av) temp_av->sendEvent("PCS", am->p_fims, Severity::Fault, "PCS comms lost at time %2.3f"
                    , vm->get_time_dbl()
                    );
            ...
            if (amap["SystemState"]->valueChangedReset() || amap["HardShutdown"]->getbVal() || !strcmp(PCSSystemStateStep, "Init"))
            {
                ...
                if (1) FPS_ERROR_PRINT("%s >>> State step changed to %s\n", __func__, SystemState);
                // Send Event - info
            ...
            if (strcmp(SystemState, "Running") == 0 && ((tNow - amap["PCSShutdownTime"]->getdVal()) > amap["maxShutdownTime"]->getdVal()))
            {
                ...
                else
                {
                    ...
                    amap["PCSSystemStateStep"]->setVal(PCSSystemStateStep);
                    if (1) FPS_ERROR_PRINT("%s >> PCS stuck in hold state %s, trying e-stop at time: %f \n", __func__, PCSStatusResp, tNow);
                    // Send Event - info
                    assetVar* temp_av = amap["ShutdownCmd"];
                    if (temp_av) temp_av->sendEvent("PCS", am->p_fims, Severity::Info, "PCS stuck in hold state [%s], trying estop at time %2.3f"
                        , PCSStatusResp
                        , vm->get_time_dbl()
                        );
                }
            }
            else if (strcmp(SystemState, "Shutdown") == 0 || strcmp(SystemState, "Fault") == 0 || strcmp(SystemState, "Off") == 0)
            {
                amap["PCSShutdown"]->setVal(true);
                PCSSystemStateStep = (char*)"PCS Shutdown Complete";
                amap["PCSSystemStateStep"]->setVal(PCSSystemStateStep);
                if (1) FPS_ERROR_PRINT("%s >> All PCS's succesfully shut down at time: %f \n", __func__, tNow);
                // Send Event - info
                assetVar* temp_av = amap["PCSShutdown"];
                if (temp_av) temp_av->sendEvent("PCS", am->p_fims, Severity::Info, "All PCS shutdown successfully at time %2.3f"
                    , vm->get_time_dbl()
                    );
                amap["schedShutdownPCS"]->setParam("endTime", 1);
                amap["schedShutdownPCS"]->setVal("schedShutdownPCS");
            ...
            else if (strcmp(SystemState, "Ready") == 0 && !amap["HardShutdown"]->getbParam("Latch"))
            {
                ...
                // Send Event - info
                assetVar* temp_av = amap["PCSShutdown"];
                if (temp_av) temp_av->sendEvent("PCS", am->p_fims, Severity::Info, "PCS hard-shutdown success at time %2.3f"
                    , vm->get_time_dbl()
                    );
                amap["schedShutdownPCS"]->setParam("endTime", 1);
                amap["schedShutdownPCS"]->setVal("schedShutdownPCS");
            ...
            else if (amap["KeyCmd"]->getbParam("KeyCmdDone"))
            {
                if (amap["HardShutdown"]->getbParam("Latch"))
                {
                    ...
                    amap["PCSSystemStateStep"]->setVal(PCSSystemStateStep);
                    if (1) FPS_ERROR_PRINT("%s >> PCS not responding still in state %s at time: %f \n", __func__, PCSStatusResp, tNow);
                    // Send Event - fault
                    assetVar* temp_av = amap["PCSShutdown"];
                    if (temp_av) temp_av->sendEvent("PCS", am->p_fims, Severity::Fault, "PCS not responding in state [%s] at time %2.3f"
                        , PCSStatusResp
                        , vm->get_time_dbl()
                        );
                    amap["schedShutdownPCS"]->setParam("endTime", 1);
                ...
                else
                {
                    ...
                    PCSSystemStateStep = (char*)"PCS Starting Hard Shutdown";
                    amap["PCSSystemStateStep"]->setVal(PCSSystemStateStep);
                    if (1) FPS_ERROR_PRINT("%s >> PCS not responding to stop, trying e-stop at time: %f \n", __func__, tNow);
                    // Send Event - info, alarm?
                    assetVar* temp_av = amap["PCSShutdown"];
                    if (temp_av) temp_av->sendEvent("PCS", am->p_fims, Severity::Info, "PCS not responding, trying e-stop at time %2.3f"
                        , vm->get_time_dbl()
                        );
                }
            }
        ...
        int ShutdownBMS(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* aV)
        {
            ...
            if (reload < 2)
            {
                ...
                RunKeyCmd(vmap, amap, aname, nullptr, aV);
                if (1) FPS_ERROR_PRINT("%s >>>>>>>>>>>>>>>>BMS Shutdown system state [%s] reload %d at time: %f \n", __func__, amap["BMSSystemState"]->getcVal(), reload, tNow);
            ...
            if (!BMSStatus || !BMSPowerOn)
            {
                ...
                if (temp_av) temp_av->sendEvent("BMS", am->p_fims, Severity::Fault, "BMS comms lost at time %2.3f"
                        , vm->get_time_dbl()
                        );
            ...
            if (0) FPS_ERROR_PRINT("%s >> reload [%d] status [%s] PowerOn [%s] BMSSystemStateStep [%s]\n", __func__, reload, BMSStatus, BMSPowerOn, BMSSystemStateStep);
            ...
            if (amap["BMSStatus"]->valueChangedReset() || !strcmp(BMSSystemStateStep, "Init"))
            {
                if (/*!strcmp(BMSStatus, "Warning") || */!strcmp(BMSStatus, "Fault"))
                {
                    ...
                    amap["BMSSystemStateStep"]->setVal(BMSSystemStateStep);
                    if (1) FPS_ERROR_PRINT("%s >>> BMSStatus changed? %d State step changed to %s\n", __func__, amap["BMSStatus"]->valueChangedReset(), BMSSystemStateStep);
                }
            }
            ...
            if (strcmp(BMSSystemStateStep, "Shutdown") == 0)
            {
                ...
                amap["BMSSystemStateStep"]->setVal(BMSSystemStateStep);
                if (1) FPS_ERROR_PRINT("%s >> All BMS's succesfully Shutdown at time: %f \n", __func__, tNow);
                // Send Event - info
                auto temp_av = amap["BMSShutdown"];
                if (temp_av) temp_av->sendEvent("BMS", am->p_fims, Severity::Info, "All BMS shutdown successfully at time %2.3f"
                        , vm->get_time_dbl()
                        );
                amap["schedShutdownBMS"]->setParam("endTime", 1);
                amap["schedShutdownBMS"]->setVal("schedShutdownBMS");
            ...
            else if ((tNow - amap["BMSShutdownTime"]->getdVal()) > amap["maxShutdownTime"]->getdVal())
            {
                ...
                amap["BMSSystemStateStep"]->setVal(BMSSystemStateStep);
                FPS_ERROR_PRINT("BMSShutdownTime %f maxShutdownTime %f\n",tNow - amap["BMSShutdownTime"]->getdVal(),amap["maxShutdownTime"]->getdVal());
                if (1) FPS_ERROR_PRINT("%s >> BMS Shutdown timed out in hold state %s at time: %f \n", __func__, BMSPowerOn, tNow);
                // Send Event - fault
                auto temp_av = amap["BMSShutdown"];
                if (temp_av) temp_av->sendEvent("BMS", am->p_fims, Severity::Fault, "BMS shutdown timed out in state [%s] at time %2.3f"
                        , BMSPowerOn
                        , vm->get_time_dbl()
                        );
                amap["schedShutdownBMS"]->setParam("endTime", 1);
            ...
            if (amap["KeyCmd"]->valueChangedReset())
            {
                ...
                if (amap["KeyCmd"]->getbVal())
                {
                    ...
                    // am->Send("set", "/components/catl_ems_bms_rw/ems_cmd", nullptr, "{\"value\":3}");
                }
                else
                {
                    amap["BMSKeyCmd"]->setVal(1); // todo: config
                    vm->addVlist(vlist, amap["BMSKeyCmd"]);
                    // am->Send("set", "/components/catl_ems_bms_rw/ems_cmd", nullptr, "{\"value\":1}");
                }
                vm->sendVlist(p_fims, "set", vlist);
                vm->clearVlist(vlist);
            }
