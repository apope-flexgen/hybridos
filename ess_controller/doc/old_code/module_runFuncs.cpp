...
int StartupSystem(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* aV)
{
    ...
    if (reload < 2)
    {
        ...
        // other variables:
        linkVals(*vm, vmap, amap, aname, "/status", bval, "UiStartup", "BMSStartupCmd", "UiStartupSeen", "StartupCmd",
                 "StartupRequest");
        ...
        // linkVals(*vm, vmap, amap, "pcs", "/status", cval, "PCSStatusResp");  //
        ...
        if (reload < 1)  // complete restart
        {
            ...
            FPS_ERROR_PRINT("%s >> after running setup , reloads PCS %d BMS %d \n", __func__,
                            amap["StartupPCS"]->getiVal(), amap["StartupBMS"]->getiVal());
        }
        reload = 2;
        essAv->setVal(reload);
    }
    ...
    // bool StartupCompletedOK =
    // amap["StartupCmd"]->getbParam("StartupCompletedOK");
    ...
    {
        if (1)
            FPS_ERROR_PRINT(" %s >>> Set UiStartup StartupCmd true\n", __func__);
        amap["StartupCmd"]->setVal(true);
    }
    if (BMSStartupCmdAv->getbVal() && BMSStartupCmdAv->valueChangedReset())
    {
        if (1)
            FPS_ERROR_PRINT(" %s >>> Set BMSStartupCmd true\n", __func__);
        amap["StartupCmd"]->setVal(true);
    }
    ...
    {
        if (1)
            FPS_ERROR_PRINT("%s >> UIStartup: %s, BMSStartupCmd: %s\n", __func__, UiStartup ? "true" : "false",
    ...
    {
        if (0)
            FPS_ERROR_PRINT("%s >> No startup cmd  at time: %f\n", __func__, tNow);
    }
    ...
    for (auto& ix : am->assetManMap)  // should get (const?) references to pairs NOT copies.
    {
        ...
        if (amc->name == "pcs")
        {
            if (0)
                FPS_ERROR_PRINT("%s>> manager_name: [%s] PCSStartup [%s] BMSStartup [%s]\n", __func__,
                                amc->name.c_str(), PCSStartup ? "true" : "false", BMSStartup ? "true" : "false");
    ...
    if (amap["PCSStartup"]->getbVal() && amap["BMSStartup"]->getbVal())  // then it goes here and does this.
    {
        ...
        vm->sendAlarm(vmap, "/status/ess:StartupRequested", "/components/ess:alarms", "Startup", "Startup Completed",
                      1);

        // assetVar* temp_av = amap["StartupRequested"];
        assetVar* temp_av = amap["StartupCmd"];
        if (temp_av)
            temp_av->sendEvent("ESS", am->p_fims, Severity::Info, "startup completed at time %2.3f",
                               vm->get_time_dbl());
    ...
    else if (amap["BMSStartup"]->getbVal() && UiStartupAv->gotParam("StartPCS") && !UiStartupAv->getbParam("StartPCS"))
    {
        ...
        assetVar* temp_av = amap["StartupCmd"];
        if (temp_av)
            temp_av->sendEvent("ESS", am->p_fims, Severity::Info, "UI startup completed at time %2.3f",
                               vm->get_time_dbl());
    ...
    // !(amap["StartupCmd"]->getbParam("StartupCompleted"))
    else if (amap["BMSStartup"]->getbParam("Fault"))
    {
        if (1)
            FPS_ERROR_PRINT("%s >>>> BMS Startup failed, quitting Startup at time: %f\n ", __func__, tNow);
        amap["StartupCmd"]->setParam("StartupRequested", false);
    ...
    {
        if (1)
            FPS_ERROR_PRINT("%s >>>> PCS Startup failed, quitting Startup at time: %f\n ", __func__, tNow);
        amap["StartupCmd"]->setParam("StartupRequested", false);
...
int StartupPCS(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* aV)
{
    ...
    // if (am == nullptr)
    if (0)
        FPS_ERROR_PRINT("%s >> Running  for aV [%s:%s] am %p, amap %p aname [%s] \n", __func__, aV->comp.c_str(),
                        aV->name.c_str(), aV->am, &amap, aname);
    if (0)
        FPS_ERROR_PRINT(
            "%s >>>>>>>>>>>PCS Startup check amap %p system state [%s] "
            "%p  >>pcssystem state [%s] %p \n",
            __func__, &amap, amap["SystemState"] ? amap["SystemState"]->getcVal() : "No System State",
            amap["SystemState"], amap["PCSSystemState"] ? amap["PCSSystemState"]->getcVal() : "No PCS System State",
            amap["PCSSystemState"]);
    VarMapUtils* vm = am->vm;
    double tNow = vm->get_time_dbl();
    essPerf ePerf(am, aname, __func__);
    ...
    if (reload == 0)
    {
        ...
        if (1)
            FPS_ERROR_PRINT("%s >>>>>>>>>>>PCS Startup reload 0 time %2.3f\n", __func__, tNow);
    }
    if (reload < 2)
    {
        ...
        // BUT do we wamt to reset the fault here ??    yes on a complete reset but
        // not after that time
        ...
        if (1)
            FPS_ERROR_PRINT(
                "%s >>>>>>>>>>>PCS Startup reload 1 amap %p system state "
                "[%s] %p  >>pcssystem state [%s] %p at time: %f \n",
                __func__, &amap, amap["SystemState"]->getcVal(), amap["SystemState"], amap["PCSSystemState"]->getcVal(),
                amap["PCSSystemState"], tNow);
        // Send Event - info
        assetVar* temp_av = amap["PCSStartup"];
        if (temp_av)
            temp_av->sendEvent("PCS", am->p_fims, Severity::Info, "PCS startup reload complete at time %2.3f",
                               vm->get_time_dbl());
    ...
    // if (1) FPS_ERROR_PRINT("%s >>>>>>>>>>>>>>>>PCS Startup system state [%s]
    // at time: %f \n",0, __func__,
    // amap["SystemStatsxe"]->getcVal(), tNow);
    ...
    char* PCSStatusResp = amap["PCSStatusResp"]->getcVal();
    if (0)
        FPS_ERROR_PRINT("%s >> Running reload %d StatusResp [%s]\n", __func__, reload, PCSStatusResp);
    if (0)
        FPS_ERROR_PRINT("aname %s comp %s name %s\n", aname, amap["PCSStatusResp"]->comp.c_str(),
                        amap["PCSStatusResp"]->name.c_str());
    ...
    if (!PCSStatusResp)
    {
        ...
        if (temp_av)
            temp_av->sendEvent("PCS", am->p_fims, Severity::Fault, "PCS startup comms failed at time %2.3f",
                               vm->get_time_dbl());
        amap["schedStartupPCS"]->setParam("endTime", 1);
        amap["schedStartupPCS"]->setVal("schedStartupPCS");
        pcsAv->setVal(1);
    ...
    else if (strcmp(SystemState, "Running") == 0)
    {
        ...
        amap["PCSSystemStateStep"]->setVal(PCSSystemStateStep);
        if (1)
            FPS_ERROR_PRINT("%s >> All PCS's succesfully Startup at time: %f \n", __func__, tNow);
        // Send Event - info
        assetVar* temp_av = amap["PCSStartup"];
        if (temp_av)
            temp_av->sendEvent("PCS", am->p_fims, Severity::Info, "PCS startup completed at time %2.3f",
                               vm->get_time_dbl());
        amap["schedStartupPCS"]->setParam("endTime", 1);
        amap["schedStartupPCS"]->setVal("schedStartupPCS");
        pcsAv->setVal(1);
    ...
    else if (strcmp(SystemState, "Shutdown") == 0 || strcmp(SystemState, "Fault") == 0)
    {
        ...
        amap["PCSSystemStateStep"]->setVal(PCSSystemStateStep);
        if (1)
            FPS_ERROR_PRINT("%s >> PCS failed in state %s at time: %f \n", __func__, PCSStatusResp, tNow);
        // Send Event - fault
        assetVar* temp_av = amap["PCSStartup"];
        if (temp_av)
            temp_av->sendEvent("PCS", am->p_fims, Severity::Fault, "PCS startup failed at time %2.3f",
                               vm->get_time_dbl());
        amap["schedStartupPCS"]->setParam("endTime", 1);
        amap["schedStartupPCS"]->setVal("schedStartupPCS");
        pcsAv->setVal(1);
    ...
    else if (amap["KeyCmd"]->getbParam("KeyCmdDone"))
    {
        ...
        amap["PCSSystemStateStep"]->setVal(PCSSystemStateStep);
        if (1)
            FPS_ERROR_PRINT("%s >> No response from PCS at time: %f \n", __func__, tNow);
        // Send Event - fault
        assetVar* temp_av = amap["PCSStartup"];
        if (temp_av)
            temp_av->sendEvent("PCS", am->p_fims, Severity::Fault, "PCS response failed at time %2.3f",
                               vm->get_time_dbl());
        amap["schedStartupPCS"]->setParam("endTime", 1);
        amap["schedStartupPCS"]->setVal("schedStartupPCS");
        pcsAv->setVal(1);
...
int StartupBMS(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* aV)
{
    ...
    if (reload < 2)  // complete reset
    {
        ...
        if (1)
            FPS_ERROR_PRINT("%s >>>>>>>>>>>>>>>>BMS Startup system state [%s]  at time: %f \n", __func__,
                            amap["BMSSystemState"]->getcVal(), tNow);
        bmsAv->setVal(2);
    ...
    if (0)
        FPS_ERROR_PRINT("%s >> status [%s] PowerOn [%s]\n", __func__, BMSStatus, BMSPowerOn);
    ...
    if (0)
        FPS_ERROR_PRINT(
            "%s >>> BMSPowerOn [%s] changed [%s] BMSSystemStateStep %s "
            "at time %2.3f \n",
            __func__, amap["BMSPowerOn"]->getcVal(), amap["BMSPowerOn"]->valueChanged() ? "true" : "false",
            BMSSystemStateStep, tNow);
    ...
    if (strcmp(BMSSystemStateStep, "Running") == 0)
    {
        ...
        amap["BMSSystemStateStep"]->setVal(BMSSystemStateStep);
        if (1)
            FPS_ERROR_PRINT("%s >> All BMS's succesfully Startup at time: %f \n", __func__, tNow);
        // Send Event - info
        auto temp_av = amap["BMSStartup"];
        if (temp_av)
            temp_av->sendEvent("BMS", am->p_fims, Severity::Info, "All BMS succesfully started up at time %2.3f",
                               vm->get_time_dbl());
    ...
    else if ((tNow - amap["BMSStartupTime"]->getdVal()) > amap["maxStartupTime"]->getdVal())
    {
        ...
        amap["BMSSystemStateStep"]->setVal(BMSSystemStateStep);
        if (1)
            FPS_ERROR_PRINT("%s >> BMS Startup timed out in hold state %s at time: %f \n", __func__, BMSPowerOn, tNow);
        // Send Event - fault
        auto temp_av = amap["BMSStartup"];
        if (temp_av)
            temp_av->sendEvent("BMS", am->p_fims, Severity::Fault, "BMS startup timed out in state [%s] at time %2.3f",
