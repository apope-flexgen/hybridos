...
int hbTestFunc::runFunc(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, asset* am)
{
    ...
    if (reload < 2)
    {
        ...
        ival = 2;
        CheckAssetHeartBeat->setVal(ival);
    }
    ...
    else
    {
        ...
        if (ival1 != ival2)
        {
            ...
            {
                if (0)
                    FPS_ERROR_PRINT("%s >>  ts  change for %s from [%d] to [%d] \n", __func__, aname, ival2, ival1);
                ival = Asset_Ok;
                amap["HeartBeatStateNum"]->setVal(ival);
            }
            lastHeartBeat = ival1;
...
int commsTestFunc::runFunc(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, asset* am)
{
    ...
    else
    {
        ...
        if (0)
            FPS_ERROR_PRINT("%s >>  ts  change for %s from [%s] to [%s] \n", __func__, aname,
                            lastTimestamp ? lastTimestamp : "no last Value", tval1);
...
int assetFeatDict::addCj(cJSON* cj, int uiObject, bool skipName)
{
    ...
    while (cjic)
    {
        if (0)
        {
            ...
            FPS_ERROR_PRINT(" %s >>   stmp2 >>%s<< name [%s] child %p next %p \n", __func__, stmp2, cjic->string,
                            (void*)cjic->child, (void*)cjic->next);
            free((void*)stmp2);
        }
        // addFeat(cjic);
        if (uiObject &&
        ...
        {
            if (0)
                FPS_ERROR_PRINT(" %s >> skipping [%s] as a base param  its special \n", __func__, cjic->string);
        }
        else
        {
            addFeat(cjic);
...
void assetFeatDict::setFeat(const char* name, cJSON* cj)
{
    ...
    {
        if (0)
            FPS_ERROR_PRINT("%s >> name [%s] cJSON type %d child %p\n", __func__, name, cj->type, cj->child);
        if (cj->child)
            cj = cj->child;
...
void assetFeatDict::setFeat(const char* name, int val)
{
    ...
    {
        if (0)
            FPS_ERROR_PRINT(" %s >> set old int Feat [%s] value %d\n", __func__, name, val);
...
void assetAction::showBitField(int show)
{
    ...
    {
        if (show)
            FPS_ERROR_PRINT("%s >> Bitfield [%d] name [%s]\n", __func__, x.first, name.c_str());
        assetBitField* abf = x.second;
        for (auto& y : abf->featDict->featMap)
        {
            if (show)
                FPS_ERROR_PRINT("%s >> Bitfield [%d] feature[%s]\n", __func__, x.first, y.first.c_str());
        }
    }
}
...
{
    if (0)
        FPS_ERROR_PRINT("%s >> 3 delete aList %p %s\n", __func__, &aList, name.c_str());
...
{
    if (0)
        FPS_ERROR_PRINT("%s >> adding options array [%s] child %p type %d\n", __func__,
                        cj->string ? cj->string : " no String", cj->child, cj->type);
...
{
    if (0)
        FPS_ERROR_PRINT(" %s >> setVal bool  called \n", __func__);
    valueint = val;
    valuebool = val;
    setTime = g_setTime;
}
...
{
    if (0)
        FPS_ERROR_PRINT(" %s >> running, cjson type %d \n", __func__, cj->type);
    if (cJSON_IsBool(cj))
    {
        if (setvar_debug_asset)
            FPS_ERROR_PRINT(" %s >>body  is a cjson bool value [%s]\n", __func__, cJSON_IsTrue(cj) ? "true" : "false");
    ...
    {
        if (setvar_debug_asset)
            FPS_ERROR_PRINT("%s >>  body  is a cjson numerical value [%f]\n", __func__, cj->valuedouble);
    ...
    {
        if (setvar_debug_asset)
            FPS_ERROR_PRINT(" %s >> body  is a cjson string value [%s]\n", __func__, cj->valuestring);
    ...
    {
        if (setvar_debug_asset)
            FPS_ERROR_PRINT(" %s >> body  is a cjson Object  try child [%p]\n", __func__, (void*)cj->child);
...
T assetVar::getVal(T val)
{
    ...
    // __func__, lVal?lVal->valuebool?"true":"false":"no
    // LVal",nVal?"true":"false");
    return nVal;
}
...
T assetVar::getLVal(T val)
{
    ...
    // FPS_ERROR_PRINT("%s --- last Val [%s] nVal [%s]\n",
    // __func__,
    // lVal->valuebool?"true":"false",nval?"true":"false");
    return nVal;
}
...
{
    if (0)
        FPS_ERROR_PRINT(" %s >> setVal  called here \n", __func__);
    if (linkVar)
    {
    ...
    if (0)
        FPS_ERROR_PRINT(" %s >> setVal  called here diff %d\n", __func__, diff);
    if (diff)
    {
        assetVal* tv = lVal;
        lVal = aVal;
...
bool assetVar::valueIsDiff(double db, const char* val)
{
    ...
    if (!aVal->valuestring && val)
        return true;
    if (0)
        FPS_ERROR_PRINT("assetVar.h %s >>  old [%s] aVal[%p] aValvs[%p]\n", __func__, val, aVal, aVal->valuestring);
    if (0)
        FPS_ERROR_PRINT("assetVar.h %s >>  old [%s] new [%s] strcmp [%d] diff [%s]\n", __func__, val, aVal->valuestring,
                        strcmp(aVal->valuestring, val), (bool)strcmp(aVal->valuestring, val) ? "true" : "false"
...
bool assetVar::setCjVal(cJSON* cj)
{
    ...
    if (0)
        FPS_ERROR_PRINT(" %s >> setCjVal  called here \n", __func__);
    if (cJSON_IsBool(cj))
    {
        if (setvar_debug_asset)
            FPS_ERROR_PRINT("%s >>   body child is a cjson bool value [%s]\n", __func__,
    ...
    {
        if (setvar_debug_asset)
            FPS_ERROR_PRINT("%s >> body child is a cjson numerical value [%f]\n", __func__, cj->valuedouble);
        return setVal(cj->valuedouble);
    }
    else if (cJSON_IsString(cj))
    {
        if (setvar_debug_asset)
            FPS_ERROR_PRINT("%s >> body child is a cjson string value [%s]\n", __func__, cj->valuestring);
...
void assetVar::showvarCJ(cJSON* cj, int opts)
{
    if (0)
        FPS_ERROR_PRINT(
            " %s >>  >>>>>>>name [%s] av %p link %p type %d atype %d "
            "opts 0x%04x ui_type %d naked [%s]\n",
            __func__, name.c_str(), this, linkVar, type, aVal->type, opts, ui_type, setNaked ? "true" : "false");
    // char *tmp = cJSON_Print(cj);
    // if (tmp)
    // {
    //     if (1) FPS_ERROR_PRINT(" >>>>> %s at start
    //     -->%s<<--\n",__func__, tmp); free((void*)tmp);
    // }
...
cJSON* assetVar::getAction(assetAction* aact)
{
    ...
    for (auto& x : aact->Abitmap)
    {
        ...
        if (debug_action)
            FPS_ERROR_PRINT("%s >>  >>>>> get action bitfield start abf [%d]<<--\n", __func__, x.first);
        ...
        //     switch (af->type) {
        //        case assFeat::AFLOAT:
        //             cJSON_AddNumberToObject(cjix, af->name.c_str(),
        //             af->valuedouble); break;
        //        case assFeat::AINT:
        //             cJSON_AddNumberToObject(cjix,  af->name.c_str(),
        //             af->valueint); break;
        //        case assFeat::ASTRING:
        //             cJSON_AddStringToObject(cjix,  af->name.c_str(),
        //             af->valuestring); break;
        //         case assFeat::ABOOL:
        //            if(af->valuebool)
        //                 cJSON_AddTrueToObject(cjix, af->name.c_str());
        //             else
...
void assetVar::showvarExtrasCJ(cJSON* cjv, int opts)
{
    if (0)
        if (1 || debug_action)
            FPS_ERROR_PRINT("%s >>  >>>>> looking for extras %p av %p [%s] opts 0x%04x link %p\n", __func__, extras,
                            this, name.c_str(), opts, linkVar);

    // char* tmp = cJSON_Print(cjv);
    // if (tmp)
    // {
    //     if (1) FPS_ERROR_PRINT(" >>>>> %s started with
    //     -->%s<<--\n",__func__, tmp); free((void*)tmp);
    // }
    // if (!actMap.empty())
    // {
    ...
    //         if (debug_action) FPS_ERROR_PRINT("%s >>  <<<<< action got cj
    //         -->%s<--  cja %p\n", __func__,
    ...
    //             if (tmp)
    //             {
    //                 if (0) FPS_ERROR_PRINT(" >>>>> show action found
    //                 -->%s<<--\n", tmp); free((void*)tmp);
    //             }
        ...
        if (!extras->actVec.empty())
        {
            ...
            for (auto& x : extras->actVec)
            {
                ...
                for (auto& y : x.second)
                {
                    ...
                    if (0 || debug_action)
                        FPS_ERROR_PRINT("%s >>  <<<<< action Vec got cj  -->%s<--  cja %p\n", __func__, x.first.c_str(),
                                        (void*)cja);
    ...
    // if (tmp)
    // {
    //     if (1) FPS_ERROR_PRINT(" >>>>> %s found
    //     -->%s<<--\n",__func__, tmp); free((void*)tmp);
    // }
    // if (baseDict)
    // {
    //     // This should take care of adding all the name:values in base dict to
    //     the existing cjson object baseDict->showCj(cjv);
    // }
}
// still need to split this up
...
void assetVar::setAlarm(asset_log* avAlarm)
{
    ...
    if (0)
        FPS_ERROR_PRINT("%s >> setting alarm in [%s:%s] msg [%s]\n", __func__, comp.c_str(), name.c_str(),
                        avAlarm->almsg.c_str());
...
void assetVar::showvarValueCJ(cJSON* cj, int opts)
{
    ...
    {
        if (0)
            FPS_ERROR_PRINT(" %s >>  >>>>>>>opts 0x%04x name [%s] type %d atype %d\n", __func__, opts, name.c_str(),
                            type, aVal->type);
...
void assetVar::showvarValueOnlyCJ(cJSON* cj, int opts)
{
    if (0)
    {
        FPS_ERROR_PRINT(" %s >>  >>>>>>>name [%s] comp [%s] type %d aval %p opts %d\n", __func__, name.c_str(),
                        comp.c_str(), type, aVal, opts);
        FPS_ERROR_PRINT(" %s >>  >>>>>>>name [%s] type %d atype %d\n", __func__, name.c_str(), type, aVal->type);
    }
...
{
    if (0)
        FPS_ERROR_PRINT(" %s >>   called here name [%s] \n", __func__, pname);
    if (!extras)
    {
        extras = new assetExtras;
    }
...
double assetVar::getdParam(const char* pname)
{
    double val;
    if (0)
        FPS_ERROR_PRINT(" %s >>   called here av name [%s] param [%s] extras %p \n", __func__, name.c_str(), pname,
                        extras);
    if (extras && extras->baseDict)
        extras->baseDict->getFeat(pname, &val);
    return val;
...
int assetVar::openLog(const char* fname, int logDepth)
{
    ...
    if (sp)
        extras->logfd = open(sp, O_RDWR | O_CREAT | O_APPEND, 0644);
    if (tmpsp)
        free(tmpsp);
    return 0;
}
...
int assetUri::setupUriVec()
{
    ...
    if (sp != Uri)
    {
        uriVec.push_back(sp);
        if (0)
            FPS_ERROR_PRINT("%s >> push back last [%s]\n", __func__, sp);
    }
    return (int)uriVec.size();
}
// was pull_one_uri
...
// to be developed .. the comman will be a fims message to be sent to all assets
...
void asset::configure(const char* fname, std::vector<std::pair<std::string, std::string>>* reps, asset_manager* am,
                      asset* ai)
{
    // VarMapUtils vm;
...
{
    if (0)
        FPS_ERROR_PRINT("%s >> asset manager setting vmap\n", __func__);
    vmap = _vmap;
}
...
{
    if (0)
        FPS_ERROR_PRINT("%s >> asset manager getting vmap\n", __func__);
    return vmap;
}
...
void asset_manager::assconfigure(varsmap* vmap, const char* fname, const char* aname)
{
    ...
    if (1)
        FPS_ERROR_PRINT(" %s >> seeking links  [%s] in file [%s] cja %p\n", __func__, assname, fname, (void*)cja);
    if (cja)
    {
        if (1)
            FPS_ERROR_PRINT(" %s >> found links  [%s]\n", __func__, cja->string);
        amConfig(vmap, cja, this);
    }
    // configure_vmap
}

asset* asset_manager::addAsset(cJSON* cja, cJSON* cjt, std::vector<std::pair<std::string, std::string>>& reps,
                               asset_manager* am)
{
    asset* ass = addInstance(cja->string);
    ass->setVmap(vmap);
...
// This is the asset_manager config we need the wake up function and teh asset
// manager pointer
void asset_manager::configure(varsmap* vmap, const char* fname, const char* aname, std::vector<std::string>* sysVec,
                              bool (*assWake)(asset*, int), asset_manager* am)
{
    // VarMapUtils vm; // use the asset_manager one
    ...
    {
        if (1)
            FPS_ERROR_PRINT(" %s >> error in file [%s]\n", __func__, fname);
        return;
    }
    if (1)
        FPS_ERROR_PRINT(" %s >> incoming vmap [%p]\n", __func__, vmap);
    ...
    if (cjs)
    {
        ...
        //     __func__, sumname, cjtmp); free((void*)cjtmp);
        // }
        // cJSON_Delete(cjs);
    }
    if (sumname)
    ...
    if (cjc)
    {
        ...
        {
            if (1)
                FPS_ERROR_PRINT(" %s >> found [%s] cjc >>%s<< \n", __func__, cfgname, cjtmp);
            free((void*)cjtmp);
        }
        // cJSON_Delete(cjc);
    }
    ...
    free((void*)assname);
    // This wont work .. put them inito the template file . Theywill be inserted
    // multiple tines but that's OK asprintf(&assname, "/components/%s", aname);
    // asprintf(&assname, "/components");
    // cja = cJSON_GetObjectItem(cjbase, assname);
    // if (0)FPS_ERROR_PRINT(" %s >> seeking components  [%s] in file [%s] cja
    // %p\n", __func__, assname, fname, (void*) cja); if(cja)
    // {
    //     if (1)FPS_ERROR_PRINT(" %s >> found components  [%s]\n",
    //     __func__, cja->string); amConfig(vmap, cja, am);
    // }
    // free((void*)assname);
}
...
bool asset_manager::runChildren(int wakeup)
{
    ...
    if (0)
        FPS_ERROR_PRINT(
            "%s >>>>>>>>%s Manager Loop >>>>>>>>>>>  looking for child "
            "Asset Manager\n",
            __func__, name.c_str());
            ...
            if (am2->run_wakeup)
            {
                if (0)
                    FPS_ERROR_PRINT(
                        ...
                        __func__, name.c_str(), ix.first.c_str());
                // first trigger the wakeup there is no thread in the lower leel
                // managers
...
// we are cutting down the complexit of this since we have developed the
// "cascade " concept.
void asset_manager::manager_loop()
{
    ...
    if (0)
        FPS_ERROR_PRINT("%s  >>>>>>>>    MANAGER LOOP Starting for [%s]\n", __func__, name.c_str());
    ...
    while (wakechan.get(wakeup, true))
    {
        ...
        //
        if (0)
            FPS_ERROR_PRINT("%s  %2.3f MANAGER LOOP Wakeup for [%s] wval %d\n", __func__, vm->get_time_dbl(),
                            name.c_str(), wakeup);
        ...
        {
            if (0)
                FPS_ERROR_PRINT("%s  %2.3f MANAGER LOOP Run Wakeup for [%s] wval %d\n", __func__, vm->get_time_dbl(),
                                name.c_str(), wakeup);
...
void asset_manager::man_timer_loop()
{
    ...
    while (*td->run)
    {
        ...
        {
            if (0)
                FPS_ERROR_PRINT("%s %2.3f MANAGER LOOP %s POLL WAKEUPS \n", __func__, vm->get_time_dbl(), name.c_str());
...
// asset timer loop
// Take the timed out asset from the top of the timer queue and throw it at the
// input channel. Queue upthe timer with the next entry
//
// class Compare
// {
// public:
...
//     while (!tqueue.empty())
//     {
//         printf("xtt %f somestuff
//         %d\n",tqueue.top()->tt,tqueue.top()->somestuff); delete tqueue.top();
//         tqueue.pop();
//     }
// }
// manager time loop
...
void asset_manager::ass_timer_loop()
{
    ...
    while (*td->run)
    {
        ...
        //
        // We'll have a wake up FD as well (socketpair perhaps) to allow a retest of
        // the queue after a new top guy appears
        poll(nullptr, 0, td->delay);
...
void asset_manager::timer_loop()
{
    ...
    while (*td->run)
        ...
        while (*td->run)
        {
            ...
            {
                if (0)
                    FPS_ERROR_PRINT("%s %2.3f TIMER LOOP %s POLL WAKEUPS \n", __func__, vm->get_time_dbl(),
                                    name.c_str());
...
void asset_manager::fims_loop()
{
    ...
    while (*td->run)
    {
        ...
        if (msg)
        {
            ...
            {
                if (0)
                    FPS_ERROR_PRINT("%s >> %2.3f  name %s  GET uri  [%s]\n", __func__, vm->get_time_dbl(), name.c_str(),
                                    msg->uri);
            }
...
void asset_manager::run_manager(fims* _p_fims)
{
    ...
    return;  // t_data.cthread;
             // return run(&t_data, (void *(*)(void*))timer_loop,  running, period,
             // &wakechan);
}
...
void asset_manager::run_message(int period)
{
    ...
    // return run(&m_data, (void *(*)(void*))message_loop,  running, period,
    // &wakechan);
    return;  // m_data.cthread;
}

// run the message loop eery 1.5 seconds
// pcs_man->run_message(&pcs_man->m_data, pcs_man->message_loop, &running, 1500,
// &pcs_man->wakechan);
...
void asset_manager::run_fims(int period, char** subs, const char* name, int numSubs)
{
    ...
    return;  // f_data.cthread;
             // return run(&f_data, (void *(*)(void*))fims_loop,  running, period,
             // &wakechan);
}
...
// thse guys are the wrappers to enable runing sub managers mmanaers and / or
// children.
...
// either can be nullptr allowing a cascade to children only or to sub managers
// only.
int asset_manager::cascadeAM(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, asset_manager* am,
                             int (*runAM)(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims,
                                          asset_manager* am),
                             int (*runAI)(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, asset* ai))
{
    if (am)
    {
        ...
        for (auto ix : am->assetManMap)
        {
            ...
            if (0)
                FPS_ERROR_PRINT("%s >>>> cascading function to >>>>> Manager [%s] \n ", __func__, amc->name.c_str());
            if (runAM)
                runAM(vmap, amc->amap, amc->name.c_str(), p_fims, amc);
        ...
        for (auto ix : am->assetMap)
        {
            ...
            if (0)
                FPS_ERROR_PRINT("%s >>>>>> cascading function to >>>> Asset [%s] \n ", __func__, amc->name.c_str());

            // runAI(vmap, amc->amap,amc->name.c_str(), p_fims, amc);
            if (runAI)
                cascadeAI(vmap, amc->amap, amc->name.c_str(), p_fims, amc, runAI);
        }
    }
    return 0;
}
...
assetVar* VarMapUtils::runActFuncfromCj(varsmap& vmap, assetVar* av, assetAction* aa)
{
    ...
    for (auto& x : aa->Abitmap)
    {
        ...
        if (func)
        {
            ...
            // vm->setFunc(vmap, "dcr_am", "TEST_VOLTAGE_LIMIT" , (void*)
            // &run_bms_asset_wakeup);  vm->setFunc(vmap, "dcr_ai", "CHECK_CONTACTS" ,
            // (void*) &run_bms_asset_wakeup);
        ...
        if (0)
            FPS_ERROR_PRINT(
                " %s >> running a Function av [%s] func [%s] am %p ai %p "
                "enable [%s]\n",
                __func__, av->name.c_str(), func, av->am, av->ai, enable ? "true" : "false");
        if (av->ai)
        {
            if (0)
                FPS_ERROR_PRINT(" %s >> running a Function  for an asset instance  [%s] \n", __func__,
                                av->ai->name.c_str());
        }
        else if (av->am)
        {
            ...
            if (av->am->vm)
            {
                ...
                res1 = av->am->vm->getFunc(vmap, av->am->name.c_str(),
                                           func);  // added change here -> should run
                                                   // func for arbitrary asset managers
                if (res1)
                {
            ...
            else
            {
                if (0)
                    FPS_ERROR_PRINT(
                        " %s >> running a Function  for an asset manager "
                        "with no VM please fix [%s] \n",
                        __func__, av->ai->name.c_str());
            }
            if (0)
                FPS_ERROR_PRINT(" %s >> running a Function  for an asset manager  [%s] res1 %p \n", __func__,
                                av->am->name.c_str(), res1);
        }
    }
    free((void*)strval);
    return av;
...
assetVar* VarMapUtils::runActValfromCj(varsmap& vmap, assetVar* av, assetAction* aa)
{
    ...
    {
        if (0)
            FPS_ERROR_PRINT(" %s >> #######on Set action aa->name [%s]\n", __func__, aa->name.c_str());
        av->aa = aa;
...
assetVar* VarMapUtils::runActBitMapfromCj(varsmap& vmap, assetVar* av, assetAction* aa)
{
    ...
    for (auto& x : aa->Abitmap)
    {
        ...
        if (1)
            FPS_ERROR_PRINT(
                " %s >> ######on Set BitMap action av %p mask 0x%04x "
                "hbit 0x%04x  shift %d aval %d masked val %d uri [%s] "
                "outval 0x%04x\n",
                __func__, av, (int)mask, (int)bit, shift, aval, (int)aval & (int)mask, uri ? uri : "NoUri", outval);
    }
    return av;
}
...
assetVar* VarMapUtils::runActEnumfromCj(varsmap& vmap, assetVar* av, assetAction* aa)
{
    ...
    for (auto& x : aa->Abitmap)
    {
        ...
        if (0)
            FPS_ERROR_PRINT(
                " %s >> ######on Set Enum action inValue %d mask 0x%04x "
                "hbit 0x%04x  shift %d aval %f masked val %d uri [%s] "
                "var [%s] tmpval [%s] outValue[%s]\n",
                __func__, (int)inValue, (int)mask, (int)bit, shift, aval, (int)aval & (int)mask, uri ? uri : "NoUri",
                var ? var : "NoVar", abf->getTmpval(), outValue ? outValue : "noOutvalue");
        ...
        if (fired)
        {
            ...
            if (0)
                FPS_ERROR_PRINT(" %s >> Mask Check OK tmpval [%s] cjov %p , uri [%s], var [%s]\n", __func__,
                                abf->getTmpval(), (void*)cjov, uri, var ? var : "NoVar");
...
assetVar* VarMapUtils::runActBitFieldfromCj(varsmap& vmap, assetVar* av, assetAction* aa)
{
    ...
    for (auto& x : aa->Abitmap)
    {
        ...
        if (0 || setvar_debug)
            FPS_ERROR_PRINT(
                " %s >> ######on Set Bitfield action bit %d mask 0x%04x "
                "hbit 0x%04x  value %d  bit value %d masked val %d uri "
                "[%s] var [%s] sstmp[%s] tmpval [%s]\n",
                __func__, (int)bit, (int)mask, (int)bit, aval, (int)(1 << bit), (int)(aval & (1 << bit)), uri, var,
                sstemp ? sstemp : "no outValuecj", abf->getTmpval());
        ...
        if (aval & (1 << (int)bit))
        {
            ...
            if (cjt)
            {
                // assetVar* setOldValfromCj(varsmap &vmap, const char* comp, const
                // char* var, cJSON* cj)
                if (0)
                    FPS_ERROR_PRINT(" %s >> ####### before setValfromCj  [%s:%s] body [%s] \n", __func__, uri, var,
                                    sstemp);
                ...
                if (avs)
                {
                    if (0)
                        FPS_ERROR_PRINT(
                            " %s >> #######Set  var [%s:%s] body [%s] "
                            "avs->name[%s] setFunc %p \n",
                ...
                {
                    if (0)
                        FPS_ERROR_PRINT(" %s >> #######Set  var [%s:%s] body [%s] NOAVS \n", __func__, uri, var,
                                        sstemp);
                }
                free((void*)stmp);
                // cJSON_Delete(cjt);
            ...
            {
                char* stmp;
                asprintf(&stmp, "{\"method\":\"set\", \"uri\":\"%s\",\"var\":%s,\"body\":%s}", uri, var,
                         abf->getTmpval());
                if (0)
                    FPS_ERROR_PRINT(" %s >> #######Set action [%s] cjit %p \n", __func__, stmp, (void*)cjt);
                free((void*)stmp);
            }
...
assetVar* VarMapUtils::runActBitSetfromCj(varsmap& vmap, assetVar* av, assetAction* aa)
{
    ...
    for (auto& x : aa->Abitmap)
    {
        ...
        if (0 || setvar_debug)
            FPS_ERROR_PRINT(
                " %s >> ######on Set BitSet action bit %d value %s  "
                "input val 0x%04x  output val 0x%04x "
                ...
                //, (int)bit
                ,
                bval ? "true" : "false", (int)(1 << bit), bval ? (int)(aval | (1 << bit)) : (int)(aval & ~(1 << bit)),
                uri, var, soloBit);
...
assetVar* VarMapUtils::makeVar(varsmap& vmap, const char* comp, const char* var, assetList* alist)
{
    ...
    if (0)
        FPS_ERROR_PRINT(
            "%s >> makeVar   comp [%s] var [%s] my.Uri [%s] my.Var[%s] "
            "my.Param [%s]\n",
            __func__, comp, var, my.Uri, my.Var, my.Param);
...
assetVar* VarMapUtils::makeAVar(varsmap& vmap, const char* comp, const char* var, assetVar* av)
{
    assetUri my(comp, var);
    if (0)
        FPS_ERROR_PRINT("%s >> makeVar for Av input av %p\n", __func__, (void*)av);
    if (0)
        FPS_ERROR_PRINT(
            "%s >> makeAVar   comp [%s] var [%s] my.Uri [%s] "
            "my.Var[%s] my.Param [%s] av %p\n",
            __func__, comp, var, my.Uri, my.Var, my.Param, (void*)av);
...
assetVar* VarMapUtils::replaceAv(varsmap& vmap, const char* comp, const char* var, assetVar* av)
{
    ...
    if (0)
        FPS_ERROR_PRINT(
            "%s >> replaceAv   comp [%s] var [%s] my.Uri [%s] "
            "my.Var[%s] my.Param [%s] av %p\n",
            __func__, comp, var, my.Uri, my.Var, my.Param, (void*)av);
    ...
    if (oldAv)
    {
        if (0)
            FPS_ERROR_PRINT(
                " %s >> NOT removing comp [%s] var [%s] old AV comp[%s] "
                "name[%s] old  %p new %p\n",
                __func__, comp, var, oldAv->comp.c_str(), oldAv->name.c_str(), (void*)oldAv, (void*)av);
                            ...
                            void* func)
{
    ...
    {
        if (0)
            FPS_ERROR_PRINT("%s >>  Func created ->>[%s:%s]\n", __func__, comp, fname);
...
void* VarMapUtils::getFunc(varsmap& vmap, const char* aname, const char* fname, assetVar* avi)
{
    ...
    if (var)
    {
        ...
        if (0)
            FPS_ERROR_PRINT(" %s value recovered [%s] res %p\n", __func__, var, res);
    }
    if (comp)
        free((void*)comp);
    return (void*)res;
...
assetVar* VarMapUtils::linkVal(varsmap& vmap, const char* comp, const char* var, T& defvalue)
{
    ...
    // [%]\n",__func__,comp,var); this gets the link comp may
    // have a : so remove it
    ...
    if (0)
        FPS_ERROR_PRINT(" %s >>looking for comp [%s] comp1 [%s] var [%s] got %p %s\n", __func__, comp, comp1, var,
                        (void*)av, av ? aVal->valuestring : "noval");
    free((void*)comp1);
    if (av)
    {
        if (aVal->valuestring)
        {
            ...
            if (0)
                FPS_ERROR_PRINT(" %s >>now looking for linked  comp3 [%s] var [%s] got %p\n", __func__, comp3, var,
                                (void*)av);
            if (!av)
            {
                av = makeVar(vmap, comp3, var, defvalue);
                if (0)
                    FPS_ERROR_PRINT(" %s >>now created linked  comp [%s] var [%s] av now  %p\n", __func__, comp3, var,
                                    (void*)av);
            }
            free((void*)comp3);
            return av;
...
assetVar* VarMapUtils::setLinkVal(varsmap& vmap, const char* aname, const char* cname, const char* var, T& defvalue)
{
    // printf(" %s looking for comp [%s] var
    // [%]\n",__func__,comp,var); this gets the link comp may
    // have a : so remove it char *comp1 = strdup(comp); char *sp =
    // strstr(comp1,":"); if(sp)
    //  *sp = 0;
    ...
    if (0)
        FPS_ERROR_PRINT("\n\n %s >>looking for linkcomp [%s] cname [%s] var [%s] got %p %s\n", __func__, linkcomp,
                        cname, var, (void*)lav, lav ? lav->aVal->valuestring : "noval");
    if (!lav)
    {
        // aname = bms_1
        // cname = /params
        ...
        // if (0)FPS_ERROR_PRINT(" %s >>created linkcomp [%s]  var [%s] linkvar [%s]
        // got %p %s\n"
        //     , __func__
        //     , linkcomp
        //     , var
    ...
    {
        if (0)
            FPS_ERROR_PRINT(" %s >>found linkcomp [%s]  var [%s] linkvar [%s] got %p %s\n", __func__, linkcomp, var,
                            linkvar, (void*)lav, lav ? lav->aVal->valuestring : "noval");
        ...
        if (lav->aVal->valuestring)
        {
            ...
            if (0)
                FPS_ERROR_PRINT(" %s >>now looking for link target comp3 [%s] var [%s] got %p\n", __func__, comp3, sp,
                                (void*)tav);
            ...
            if (!tav)
            {
                ...
                if (0)
                    FPS_ERROR_PRINT(" %s >> created target  comp [%s] var [%s] av now  %p\n", __func__, comp3, sp,
                                    (void*)tav);
            }
        }
        ...
        if (0)
            FPS_ERROR_PRINT(" %s >>now looking for amap  [%s] var [%s] got %p\n", __func__, acomp, var, (void*)aav);
        if (!aav)
        {
            aav = makeAVar(vmap, acomp, var, tav);
            if (0)
                FPS_ERROR_PRINT(" %s >>now creating var for  amap  [%s] var [%s] got %p\n", __func__, acomp, var,
                                (void*)aav);
        }
        else
        {
            if (aav != tav)
            {
                if (0)
                    FPS_ERROR_PRINT(
                        " %s >>now replacing aVar for  amap  [%s] var [%s]  "
                        "%p  with target av %p\n",
            ...
            else
            {
                if (0)
                    FPS_ERROR_PRINT(
                        " %s >> same var >>>> NOT replacing aVvar for  amap  "
                        "[%s] var [%s]  with target av\n",
                        __func__, acomp, var);
            }
        }
    }
    ...
    if (0)
        FPS_ERROR_PRINT(" %s >> link comp [%s] var [%s] completed\n", __func__, linkcomp, var);
    if (linkcomp)
        free((void*)linkcomp);
    if (linkvar)
        free((void*)linkvar);
...
assetVar* VarMapUtils::runActLimitsfromCj(varsmap& vmap, assetVar* av, assetAction* aa)
{
    ...
    if (0 || setvar_debug)
        FPS_ERROR_PRINT(" %s >> ###### av [%s] value [%f] LOW [%f] HIGH [%f]\n", __func__, av->name.c_str(), inVal, low,
                        high);
...
assetVar* VarMapUtils::runActRemapfromCj(varsmap& vmap, assetVar* av, assetAction* aa)
{
    ...
    if (0)
        FPS_ERROR_PRINT(
            " %s >> #######Remap action started av [%s] cvalue [%s]  "
            "int %d bool [%s]\n",
            __func__, av->name.c_str(), av->getcVal(), av->getiVal(), av->getbVal() ? "true" : "false");
    assetVal* aVal = av->aVal;
    if (av->linkVar)
    {
        aVal = av->linkVar->aVal;
    ...
    for (auto& x : aa->Abitmap)
    {
        ...
        abf->getFeat("enable", &ensp);
        if (0)
            FPS_ERROR_PRINT(" %s >> #######Remap action started enable [%s]\n", __func__, ensp);
        if (ensp)
        {
            assetVar* av = getVar(vmap, (const char*)ensp, nullptr);
            if (0)
                FPS_ERROR_PRINT(" %s >> #######Remap action started enable av [%p]\n", __func__, av);
            if (av)
            {
                enable = av->getbVal();
            }
        ...
        double scale = 1.0;
        abf->getFeat("scale", &scale);
        double offset = 0;
        abf->getFeat("offset", &offset);
        char* uri = nullptr;
        abf->getFeat("uri", &uri);
        char* var = nullptr;
        abf->getFeat("var", &var);
        ...
        if (cjiv)
        {
            ...
            if (0)
                FPS_ERROR_PRINT(
                    " %s >> #######Remap action enable [%s] looking for "
                    "match cjav \n[%s] cjiv \n[%s] match [%s]\n",
                    __func__, enable ? "true" : "false", cav, civ, match ? "true" : "false");
            free((void*)cav);
            free((void*)civ);
        }
        else
        {
            ...
            if (0)
                FPS_ERROR_PRINT(
                    " %s >> #######Remap action enable [%s] looking for "
                    "match cjav \n[%s]  match [%s]\n",
                    __func__, enable ? "true" : "false", cav, match ? "true" : "false");
            free((void*)cav);
        }
        ...
        if (enable && !match)
        {
            ...
            {
                if (0)
                    FPS_ERROR_PRINT(" %s >> ####### Setting Value Uri [%s] Var [%s] Param [%s] \n", __func__, auri.Uri,
                                    auri.Var, auri.Param);
            ...
            {
                {
                    ...
                    if (0)
                        FPS_ERROR_PRINT(
                            " %s >> #######Remap action on Param , enable [%s] "
                            " cjav >>%s<<  match [%s]\n",
                            __func__, enable ? "true" : "false", cav, match ? "true" : "false");
                    free((void*)cav);
                }
                // if (0)FPS_ERROR_PRINT(" %s >> ####### Setting Param Uri [%s] Var [%s]
                // Param [%s] \n", __func__, auri.Uri , auri.Var,
                // auri.Param);
            ...
            if (1)
            {
                ...
                char* stmp;
                asprintf(&stmp, "{\"method\":\"set\", \"uri\":\"%s\",\"var\":\"%s\",%s}", uri, var, scj);
                if (0)
                    FPS_ERROR_PRINT(" %s >> ####### Param [%s] Remap action uri [%s] var [%s] [%s]\n", __func__,
                                    auri.Param, uri, var ? var : "noVar", stmp);
                free((void*)stmp);
                free((void*)scj);
            }
            if (cjav)
    ...
    if (0)
        FPS_ERROR_PRINT(" %s >> #######Remap action completed av [%s] \n", __func__, av->name.c_str());
...
//     if (!av->actMap[act])
//     {
//         if (0)FPS_ERROR_PRINT("%s >>  setting new actMap entry for act [%s]
//         opt [%s]  \n"
//             , __func__
//             , act
//             , opt
...
//     1 if (0)FPS_ERROR_PRINT("%s >>  setting act [%s] opt [%s]   av->actMap
//     size %d\n"
//         , __func__
//         , act
//         , opt
...
{
    if (0)
        FPS_ERROR_PRINT(" %s >>  act [%s]  btype [%s]\n", __func__, act, opt);
    ...
    {
        if (0)
            FPS_ERROR_PRINT("%s >>  setting new actVec entry for act [%s] opt [%s]  \n", __func__, act, opt);
    ...
    if (0)
        FPS_ERROR_PRINT("%s >>  setting act [%s] opt [%s]   av->extras->actVec[act] size %d\n", __func__, act, opt,
...
assetVar* VarMapUtils::setActOptsfromCj(assetVar* av, const char* act, const char* opt, cJSON* cj)
{
    ...
    {
        if (0)
            FPS_ERROR_PRINT("%s >>  act [%s] opt [%s] type %d \n", __func__, act, opt, cjact->type);
...
{
    if (0)
        FPS_ERROR_PRINT("%s >>  starting for av->name [%s], act [%s] \n", __func__, av->name.c_str(), act
                        //, opt
        );
    if (!av->extras)
    {
    ...
    {
        if (0)
            FPS_ERROR_PRINT("%s >>  clearing  actVec entry for act [%s] \n", __func__, act
                            //, opt
            );
    ...
    //         ]
    // }
    //"actions":{"onSet":{"remap":[{"bit":0,"uri":"/alarms/bms:alarms"}]}},
    //"options":[{"name":"Clear Faults","return_value":"Clear"}]}]
    //"options":[{"name":"Clear Faults","return_value":"Clear"}]
    if (0)
        FPS_ERROR_PRINT(" %s >>  setting actions for name [%s] act [%s] \n\n", __func__, av->name.c_str(), act);
    cJSON* cji = cj;
    while (cji)
    {
        // hack to fix this
        ...
        if (0)
            FPS_ERROR_PRINT(
                "%s >>     >> string [%s] child [%p] child->string[%s] "
                "child->child->string[%s] child->child->next %p\n",
                __func__, cji->string, cji->child, cji->child->string, cji->child->child->string,
        ...
        {
            if (0)
                FPS_ERROR_PRINT(" %s >>  break setting actions for name [%s] act [%s] \n\n", __func__, av->name.c_str(),
                                act);
            break;
        }
        ...
        //                 "/fault/@@BMS_ID@@:TMS_mode_conflict", "outValue":
        //                 "Fault"}, { "shift": 3,"mask": 1,"inValue": 0,"uri":
        //                 "/fault/@@BMS_ID@@:temp_sensor_alarm", "outValue":
        //                 "Normal"}, { "shift": 3,"mask": 1,"inValue": 1,"uri":
        //                 "/fault/@@BMS_ID@@:temp_sensor_alarm", "outValue":
        //                 "Fault"}, { "shift": 4,"mask": 1,"inValue": 0,"uri":
        //                 "/fault/@@BMS_ID@@:smoke_sensor_alarm", "outValue":
        //                 "Normal"}, { "shift": 4,"mask": 1,"inValue": 1,"uri":
        //                 "/fault/@@BMS_ID@@:smoke_sensor_alarm", "outValue":
        //                 "Fault"}, { "shift": 5,"mask": 1,"inValue": 0,"uri":
        //                 "/fault/@@BMS_ID@@:aerosol_state", "outValue": "Normal"},
        //                 { "shift": 5,"mask": 1,"inValue": 1,"uri":
        //                 "/fault/@@BMS_ID@@:aerosol_state", "outValue": "Fault"},
        //                 { "shift": 6,"mask": 1,"inValue": 0,"uri":
        //                 "/fault/@@BMS_ID@@:aerosol_close", "outValue": "Normal"},
        //                 { "shift": 6,"mask": 1,"inValue": 1,"uri":
        //                 "/fault/@@BMS_ID@@:aerosol_close", "outValue": "Fault"},
        //                 { "shift": 7,"mask": 1,"inValue": 0,"uri":
        //                 "/fault/@@BMS_ID@@:aerosol_open", "outValue": "Normal"},
        //                 { "shift": 7,"mask": 1,"inValue": 1,"uri":
        //                 "/fault/@@BMS_ID@@:aerosol_open", "outValue": "Fault"},
        ...
        if (strcmp(cji->child->string, act) == 0)
        {
            ...
            {
                if (1)
                    FPS_ERROR_PRINT("%s >>     >> >> act [%s] cji2 is an Array  \n", __func__, act);
                cJSON* cjii;
            ...
            {
                if (0)
                    FPS_ERROR_PRINT("%s >>     >> >> act [%s] cji2 is an Object child func [%s]\n", __func__, act,
                                    cji2->child->string);
                cji2 = cji2->child;
            }
            ...
            {
                if (0)
                    FPS_ERROR_PRINT("%s >>     >> >> act [%s] cji2 string [%s] child [%p]\n", __func__, act,
                                    cji2->string, cji2->child);
    ...
    if (0)
        FPS_ERROR_PRINT(" %s >>  done setting actions for name [%s] act [%s] \n\n", __func__, av->name.c_str(), act);
...
assetVar* VarMapUtils::getVar(varsmap& vmap, const char* comp, const char* var)
{
    ...
    if (0)
        FPS_ERROR_PRINT(
            "%s >> getVar   comp [%s] var [%s] my.Uri [%s] my.Var[%s] "
            "my.Param [%s\n",
            __func__, comp, var, my.Uri, my.Var, my.Param);
    ...
    if (0)
        FPS_ERROR_PRINT("%s looking for comp [%s] var [%s]\n", __func__, my.Uri, my.Var);
    if (vmap.size() > 0)
    {
        ...
        {
            if (0)
                FPS_ERROR_PRINT("%s NOTE created comp [%s] var [%s]\n", __func__, my.Uri, my.Var);
        }
        if (ic != vmap.end())
        {
            if (0)
                FPS_ERROR_PRINT("%s found comp [%s] looking for var [%s] size %d\n", __func__, my.Uri, my.Var,
                                (int)vmap[my.Uri].size());
            // for (auto it : vmap[mycomp])
            // {
            //     if(0)FPS_ERROR_PRINT("%s found comp [%s] looking for var [%s] found
            //     [%s]\n"
            //         , __func__
            //         , mycomp
            //         , myvar
...
// individual uirs if needed options 0x0100 will produce a segmented list
...
int VarMapUtils::baseVec(std::string& bs, std::vector<std::string>& buri, std::vector<std::string>& turi)
{
    int rc = 0;
    if (0)
        FPS_ERROR_PRINT(" %s >> sizes buri [%d] turi [%d] \n", __func__, (int)buri.size(), (int)turi.size());
    while ((rc < (int)buri.size()) && (rc < (int)turi.size()))
    {
        if (buri[rc] != turi[rc])
        {
            if (0)
                FPS_ERROR_PRINT(" %s >> uri [%s] furi [%s] break rc %d\n", __func__, buri[rc].c_str(), turi[rc].c_str(),
                                rc);
    ...
    {
        if (0)
            FPS_ERROR_PRINT(" %s >> uri [%s] furi [%s] identical  rc %d\n", __func__, buri[rc - 1].c_str(),
                            turi[rc - 1].c_str(), rc);
        rc = 0;  // flag identical
    }
...
int VarMapUtils::uriSplit(std::vector<std::string>& uriVec, const char* _uri)
{
    ...
    do
    {
        ...
        else
        {
            ...
            if (0)
                FPS_ERROR_PRINT(" %s >> uri [%s] furi [%s]\n", __func__, _uri, furi.c_str());
            startf = endf + 1;
    ...
    if (0)
        FPS_ERROR_PRINT(" %s >> last >> uri [%s] furi [%s]\n", __func__, _uri, furi.c_str());
    return nfrags;
}
                                    ...
                                    std::vector<std::string>& uriVec)
{
    ...
    for (auto& x : uriVec)
    {
        ...
        if (0)
            FPS_ERROR_PRINT(" %s >> uri [%s] uriVec [%s] opts 0x%04x\n", __func__, inuri, myuri, options);
        ...
        if (0)
            FPS_ERROR_PRINT(
                "%s "
                ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>getting "
                "cj map for [%s] assetList %p  cj %p\n",
                __func__, myuri, alist, (void*)incj);
        ...
        if (!(options & 0x0100))
        {
            ...
            else
            {
                ...
                {
                    if (0)
                        FPS_ERROR_PRINT("%s >> getting cj for [%s] \n", __func__, y.first.c_str());
                    y.second->showvarCJ(cjii, options);
                    if (0)
                        FPS_ERROR_PRINT("%s >> got cj for [%s] \n", __func__, y.first.c_str());
        ...
        bs = bsx;
        if (0)
            FPS_ERROR_PRINT(" %s >> inuri [%s] uriVec [%s] opts 0x%04x bvec %d bs[%s]\n", __func__, inuri, x.c_str(),
        ...
        if (bvec == 0)
        {
            ...
            if (0)
                FPS_ERROR_PRINT(
                    " %s >> running getMapsCj [%s] into base node alist %p "
                    "cji->string [%s]\n",
                    __func__, x.c_str(), (void*)alist, cji->string ? cjii->string : "noname");
            // cjii = cJSON_CreateObject();
        ...
        {
            if (0)
                FPS_ERROR_PRINT(" %s >> bvec small %d need to find / create trees\n", __func__, bvec);
            while (bvec < (int)uVec.size())
            {
                if (0)
                    FPS_ERROR_PRINT(" %s >> bvec small %d  find / create tree [%s] inuri [%s] \n", __func__, bvec,
                                    uVec[bvec].c_str(), inuri);
                cjii = cJSON_GetObjectItem(cji, uVec[bvec].c_str());
                if (!cjii)
                {
                    if (0)
                        FPS_ERROR_PRINT(" %s >> bvec small %d  create tree [%s] inuri [%s] \n", __func__, bvec,
                                        uVec[bvec].c_str(), inuri);
            ...
            // now run getMapsCj on x.c_str() into cjii
            if (0)
                FPS_ERROR_PRINT(" %s >> running getMapsCj [%s ] into end node\n", __func__, x.c_str());
            if (alist)
            {
                unsigned int ix = 0;
                assetVar* av;
        ...
        else
        {
            // now run getMapsCj on x.c_str() into cjii
            if (0)
                FPS_ERROR_PRINT(" %s >> running getMapsCj [%s] into end node alist %p\n", __func__, x.c_str(),
                                (void*)alist);
            if (alist)
            {
                unsigned int ix = 0;
...
int VarMapUtils::createAssetListCj(varsmap& vmap, const char* uri, std::vector<std::string>* sysVec, int opts,
                                   std::vector<std::string>& nVec)
{
    ...
    for (auto& x : vmap)
    {
        if (process_fims_debug)
            FPS_ERROR_PRINT(" %s >> xVec push_back [%s]\n", __func__, x.first.c_str());
        xVec.push_back(x.first);
    }
    if (0)
        FPS_ERROR_PRINT(" %s >> xVec size [%d] uri [%s] \n", __func__, (int)xVec.size(), uri);
    yVecp = &xVec;
    ...
    if (sysVec && (opts & 0x0100))
    {
        for (auto& y : *sysVec)
        {
            ...
            if (0)
                FPS_ERROR_PRINT(" %s >> sysvec y [%s] ystart [%s] yend [%s]\n", __func__, y.c_str(), ystart.c_str(),
                                yend.c_str());
            for (auto z : xVec)
            {
                if (0)
                    FPS_ERROR_PRINT(" %s >> sysvec y [%s] z [%s]\n", __func__, y.c_str(), z.c_str());
                ...
                if ((y == z) || (z.find(yend) != std::string::npos))
                {
                    ...
                    {
                        if (0)
                            FPS_ERROR_PRINT(" %s >> >> >> yVec push_back [%s]\n", __func__, z.c_str());
                        yVec.push_back(z);
                    }
                }
            }
    ...
    for (auto& y : *yVecp)
    {
        ...
        auto x = y.find(suri);
        if (0)
            FPS_ERROR_PRINT(" %s >> >> >> suri [%s] yVec option [%s] x %d\n", __func__, suri.c_str(), y.c_str(),
                            (int)x);
        if (x == 0)
        {
            if (0)
                FPS_ERROR_PRINT(" %s >> >> >> >>nVec push_back [%s] suri [%s]\n", __func__, y.c_str(), suri.c_str());
            nVec.push_back(y);
        }
    }
    int rc = (int)nVec.size();
    if (0)
        FPS_ERROR_PRINT(" %s >> nVec size [%d]\n", __func__, rc);
    return rc;
}
...
asset_log* assetVar::sendAlarm(assetVar* destAv, const char* atype, const char* msg, int severity)
{
    ...
    if (0)
        FPS_ERROR_PRINT("%s >> calling setAlarm in destAv [%s:%s]\n", __func__, destAv->comp.c_str(),
                        destAv->name.c_str());
...
assetVar* VarMapUtils::setValfromCj(varsmap& vmap, const char* comp, const char* var, cJSON* cj, int uiObject)
{
    ...
    if (0)
        FPS_ERROR_PRINT(
            "%s  >> >>Seeking Variable comp 2 [%s] var [%s]  param "
            "[%s] uiObject %d alist %p\n",
            __func__, my.Uri, my.Var ? my.Var : " no Var", my.Param ? my.Param : " no Param", uiObject, alist);
    ...
    if ((uiObject > 0) || setvar_debug)
        FPS_ERROR_PRINT("%s  >> >>Suspected UI Object  comp [%s] my.Uri [%s] uiObject %d\n", __func__, comp, my.Uri,
                        uiObject);
    ...
    if ((my.Var == nullptr) && (cjval == nullptr) && (cjname == nullptr))
    {
        ...
        {
            if (1)
                FPS_ERROR_PRINT("%s >>  new assetVar ##1  [%s:%s] uiObject %d \n", __func__, my.Uri, cj3->string,
                                uiObject);
    ...
    if (av)
    {
        if (0 || setvar_debug)
            FPS_ERROR_PRINT(
                "%s >> >>Careful here Setting Old Vari comp [%s] var "
                "[%s] param [%s] cj is %p  extras [%p] cjisObject [%s]\n",
                __func__, my.Uri, my.Var, my.Param ? my.Param : "no Param", (void*)cj, av->extras,
                cJSON_IsObject(cj) ? "true" : "false");
        // this does not setup detiled components .. but note we may want to update
        // params.
        if (my.Param)
        {
    ...
    else
    {
        if (0 || setvar_debug)
            FPS_ERROR_PRINT(
                "%s >>  >>Setting New Variable comp [%s] var [%s] Param "
                "[%s] cj is %p \n",
                __func__, my.Uri, my.Var ? my.Var : "No Var", my.Param ? my.Param : "no Param", (void*)cj);
    }
    ...
    if (cJSON_IsObject(cj))
    {
        ...
        if (av == nullptr)
        {
            if (0 || setvar_debug)
                FPS_ERROR_PRINT(
                    " %s >> Adding a new Variable from OBJECT comp [%s] var [%s] cj is "
                    "%p cjval is %p cjname is %p cjui_type is %p\n",
                    __func__, my.Uri, my.Var ? my.Var : "No Var", (void*)cj, (void*)cjval ? cjval : nullptr,
                    (void*)cjname ? cjname : nullptr, (void*)cjui_type ? cjui_type : nullptr);
        }
        else
        {
            if (0 || setvar_debug)
                FPS_ERROR_PRINT(
                    " %s >> Using existing Variable from OBJECT comp [%s] var [%s] cj "
                    "is %p cjval is %p cjname is %p cjui_type is %p\n",
                    __func__, my.Uri, my.Var ? my.Var : "No Var", (void*)cj, (void*)cjval ? cjval : nullptr,
                    (void*)cjname ? cjname : nullptr, (void*)cjui_type ? cjui_type : nullptr);
        }
        // av = nullptr;
        ...
        if (!cjval)
        {
            ...
            if (!av && my.Var)
            {
                ...
                if (0 || setvar_debug)
                    FPS_ERROR_PRINT(" %s >>Adding a new ui control [%s]  OBJECT Uri [%s] var [%s]\n", __func__, ui_type,
                                    my.Uri, my.Var ? my.Var : "No Var");
                if (alist && av)
                {
                    alist->add(av);
                    // av = nullptr;
            ...
            else if (!av && cjname)
            {
                ...
                if (0 || setvar_debug)
                    FPS_ERROR_PRINT(
                        " %s >>Adding name  ui  [%s]  OBJECT Uri [%s] var "
                        "[%s] val [%s] \n",
                        __func__, ui_type, my.Uri, cjname->string, cjname->valuestring);
                ...
                while (cji)
                {
                    ...
                    if (1)
                        FPS_ERROR_PRINT(" %s >> added uiObject [%s] \n", __func__, cji->string);
                    cji = cji->next;
                }
            }
            // debug show alist
            if (alist)
            {
                ...
                for (unsigned int i = 0; i < ix; i++)
                {
                    ...
                    {
                        if (1)
                            FPS_ERROR_PRINT("  %s >>  alist item [%d] [%s] \n", __func__, i,
                                            alist->avAt(i)->name.c_str());
                    }
                }
            }
        }
        ...
        // right if we had a name but No Value or uiObject is set then we must add
        // params to the current av. no need to keep order for params ....
        ...
        //             if(0||setvar_debug)FPS_ERROR_PRINT(" %s >> Adding a new
        //             OBJECT comp [%s] varname [%s] vv [%s] \n"
        //                 , __func__
        //                 , mycomp
        //                 , cjname->string
        //                 , vv
        ...
        if (setvar_debug)
            FPS_ERROR_PRINT(
                " %s >>Adding a new Variable from OBJECT actions [%p] "
                "params[%p] options[%p]]\n",
                __func__, (void*)cjact, (void*)cjparam, (void*)cjopts);
        // what about setfunc
        if (cjact)
        {
            if (setvar_debug)
                FPS_ERROR_PRINT("%s >> >>  new Adding actions starting for [%s] av [%p]\n", __func__, var, (void*)av);
            if (av)
            {
            ...
            if (setvar_debug)
                FPS_ERROR_PRINT("%s >> >>  new Adding actions done for [%s] av [%p]\n", __func__, var, (void*)av);
        }
        // may need to add these to basedict
        if (cjparam)
        {
            if (0)
                FPS_ERROR_PRINT("%s >>Adding params for [%s] av [%p]\n", __func__, var, (void*)av);
            if (av)
            {
                if (!av->extras)
                {
        ...
        //        if (ui_control || ui_alarm || (uiObject > 0))  // TODO we may do
        //        this for any object now !!!
        {
            if (0)
                FPS_ERROR_PRINT("%s >>Adding base params starting for [%s] av [%p]\n", __func__, var, (void*)av);
            if (av)
            {
                if (!av->extras)
                {
            ...
            if (0)
                FPS_ERROR_PRINT("%s >>Adding base params done for [%s] av [%p]\n", __func__, var, (void*)av);
        }
        if (cjopts)
        {
            if (0)
                FPS_ERROR_PRINT("%s >>Adding options starting for [%s] av [%p] cjopts type %d\n", __func__, var,
                                (void*)av, cjopts->type);
            // it may be an array
            ...
            if (av)
            {
                ...
                {
                    if (0)
                        FPS_ERROR_PRINT("%s >>Adding options Vec for [%s] av [%p] cjopts type %d\n", __func__, var,
                                        (void*)av, cjopts->type);
                    if (!av->extras->optVec)
                    {
                        if (0)
                            FPS_ERROR_PRINT("%s >>New Options Vec  for [%s] av [%p] cjopts type %d\n", __func__, var,
                                            (void*)av, cjopts->type);
            ...
            if (0)
                FPS_ERROR_PRINT("%s >>Adding options done for [%s] av [%p] cjopts type %d\n", __func__, var, (void*)av,
                                cjopts->type);
        }
        return av;
    }
    ...
    //
    if (0 || setvar_debug)
        FPS_ERROR_PRINT("%s  >>NOT an OBJECT Var comp [%s] var [%s] cj is %p type %d\n", __func__, my.Uri,
                        my.Var ? my.Var : "No Var", (void*)cj, cj ? cj->type : -1);
    ...
    else
    {
        ...
        // if (setvar_debug)
        {
            ...
            if (0 || setvar_debug)
                FPS_ERROR_PRINT(
                    "%s >> NOT AN OBJECT  setValfrom_Cj >>Adding NEW cj "
                    ">>%s<<\n cjval %p \n cjact %p\n",
                    __func__, tmp, (void*)cjval, (void*)cjact);
            free((void*)tmp);
        }
        assetVar* av = nullptr;
        ///
        ...
        {
            if (0)
                FPS_ERROR_PRINT("%s  >>FUNNY STUFF Adding CJ value for [%s]\n", __func__, var);
        ...
        {
            if (1)
                FPS_ERROR_PRINT("%s  >>FUNNY STUFF Adding actions for [%s] av [%p]\n", __func__, var, (void*)av);
            if (av)
            {
        ...
        {
            if (1)
                FPS_ERROR_PRINT("%s >>FUNNY STUFF Adding params for [%s] av [%p]\n", __func__, var, (void*)av);
            if (av)
            {
                if (!av->extras)
                {
        ...
        if (cjopts)
        {
            if (1)
                FPS_ERROR_PRINT(
                    "%s >> FUNNY STUFF Adding options for [%s] av [%p] "
                    "cjopts type %d\n",
            ...
            if (av)
            {
                ...
                if (cjopts->type == cJSON_Array)
                {
                    ...
                    if (!av->extras->optVec)
                    {
                        if (1)
                            FPS_ERROR_PRINT(
                                "%s  >> FUNNY STUFF Adding New options for [%s] "
                                "av [%p] cjopts type %d\n",
                                __func__, var, (void*)av, cjopts->type);
                        av->extras->optVec = new assetOptVec;
                    }
...
int VarMapUtils::vListPartialSendFims(varsmap& vmap, std::vector<std::string>& vx, const char* method, fims* p_fims,
                                      const char* uri)
{
    char* msg;
    cJSON* cj = nullptr;
    ...
    for (auto ix : vx)
    {
        ...
        if (found == 0)
        {
            ...
            if (0)
                FPS_ERROR_PRINT("%s >> pmap ix.first [%s] cj [%p] opts 0x%04x\n", __func__, ix.c_str(), cj, opts);
        }

        // cJSON *cj = getVmapCJ(vmap[ix.c_str()]);
        if (0)
            FPS_ERROR_PRINT("%s >> pmap ix.first [%s] cj [%p] \n", __func__, ix.c_str(), cj);
        if (cj->child)
        ...
        cJSON_Delete(cj);
        if (0)
            FPS_ERROR_PRINT("%s >> pmap ix.first [%s] msg [%s] \n", __func__, ix.c_str(), msg);
        if (msg)
        {
            ...
            else
            {
                ...
                {
                    if (0)
                        FPS_ERROR_PRINT("%s >> comp [%s] msg [%s] \n", __func__, dest, msg);
                               ...
                               assetVar* avd)
{
    ...
    {
        if (1)
            FPS_ERROR_PRINT("%s >> No Fims Connection  \n", __func__);
        return -1;
    }
    char* msg;
    for (auto ix : vmap)
    {
        cJSON* cj = getVmapCJ(ix.second);  // Add flag for naked ,value or full dict
                                           // var: val var:value:val full =
                                           // var:{value:val ,scale: scale } html ???
                              ...
                              assetVar* avd)
{
    ...
    if (cj && cj->child)
    {
        ...
        {
            if (0)
                FPS_ERROR_PRINT(" %s >> meth [%s] comp [%s] msg [%s]\n", __func__, smeth, scomp, spsend);
            if (p_fims)
                                   ...
                                   const char* uri, const char* body)
{
    ...
    if (1)
        FPS_ERROR_PRINT(
            "%s >> we're working on this one uri [%s] nfrags %d my.Uri "
            "[%s] my.Var [%s] my.Param [%s]\n",
    ...
    if (strcmp(method, "get") == 0)
    {
        ...
        {
            if (process_fims_debug)
                FPS_ERROR_PRINT("%s >> Table found for  get uri  [%s]\n", __func__, uri);
        }
        else
        {
            if (process_fims_debug)
                FPS_ERROR_PRINT("%s >> Table NOT found for  get uri  [%s]\n", __func__, uri);
        }

        if (ix == vmap.end())
        {
            if (1)
                FPS_ERROR_PRINT("%s >> possible get single uri [%s] vv1 [%s] vv2 [%s] \n", __func__, uri, vv1, vv2);
            ...
            if (0)
                FPS_ERROR_PRINT(
                    "%s >> possible get single (0x%04x) uri  [%s] comp "
                    "[%s] len %ld var [%s]\n",
            ...
            *vName = varName ? strdup(varName) : nullptr;
            ...
            if (ix2 == vmap.end())
            {
                ...
                if (1)
                    FPS_ERROR_PRINT("%s >> Did not find  comp single uri  [%s] comp [%s] var [%s]\n", __func__, uri,
                                    *cName, varName);
                free((void*)*cName);
                if (*vName)
                    free((void*)*vName);
            ...
            if (ix3 == vmap[*cName].end())
            {
                ...
                if (0)
                    FPS_ERROR_PRINT("%s >> Did not find  var single uri  [%s] comp [%s] var [%s]\n", __func__, uri,
                                    uriName, varName);
                if (vv1)
                    free(vv1);
                if (vv2)
    ...
    else  // Set or Pub
    {
        ...
        {
            if (0)
                printf("%s >> possible single uri  [%s]\n", __func__, uri);
        }
        ...
        else
        {
            ...
            if (0)
                printf("%s >> cj OK, may not be single body [%s] type %d\n", __func__, body, cj->type);
            if (cj->type == cJSON_Object)
            {
                ...
                if (cji)
                {
                    if (0)
                        printf(
                            "%s >>cj OK  found value, confirmed SINGLE  uri [%s] body "
                            "[%s] type %d\n",
                ...
                else if (cjn)
                {
                    if (0)
                        FPS_ERROR_PRINT(
                            "%s >>cj no value but found Name , possible "
                            "uiasset  uri [%s] body [%s] type %d\n",
                            __func__, uri, body, cj->type);
            ...
            {
                if (0)
                    printf("cj OK  found false confirmed SINGLE  body [%s] type %d\n", body, cj->type);
                single |= 0x0101;
            }
            else if (cj->type == cJSON_Number)
            {
                if (0)
                    printf("cj OK  found number confirmed SINGLE  body [%s] type %d\n", body, cj->type);
                single |= 0x0101;
            }
            else if (cj->type == cJSON_String)
            {
                if (0)
                    printf("cj OK  found string confirmed SINGLE  body [%s] type %d\n", body, cj->type);
                single |= 0x0101;
            }
            else
            {
    ...
    if (single & 0x0001)
    {
        ...
        if (1)
            FPS_ERROR_PRINT("%s >>  got single, varName [%s] uriName [%s]\n", __func__, vv2, vv1);
        *vName = vv2;
        *cName = vv1;
    }
    else
...
bool VarMapUtils::checkedBlockedUri(varsmap& vmap, char** cName, int& opts, const char* method, const char* uri)
{
    ...
    *cName = strdup(sp);
    ...
    if (tblocked)
    {
        ...
        {
            if (0)
                FPS_ERROR_PRINT(" %s checking blocked uri [%s] \n", __func__, blockeduri);
            ...
            if (ix != vmap.end())
            {
                for (auto iy : vmap[blockeduri])
                {
                    ...
                    if (0)
                        FPS_ERROR_PRINT(" %s >>>> checking uri [%s] buri [%s] name [%s]  bval [%s]\n", __func__, uri,
                                        blockeduri, iy.first.c_str(), iy.second->getbVal() ? "true" : "false");
    ...
    if (0)
    {
        ...
        {
            if (1)
                FPS_ERROR_PRINT(" %s checked uri [%s] buri [%s] tblocked [%s] blocked [%s]\n", __func__, uri,
                                blockeduri, tblocked ? "true" : "false", blocked ? "true" : "false");
        }
    }
    ...
    {
        if (0)
            FPS_ERROR_PRINT("%s >> blocked uri  method [%s] uri [%s] ->[%s]\n", __func__, method, uri, *cName);
    }
    else
    {
        if (0)
            FPS_ERROR_PRINT("%s >> NOT blocked method [%s] uri [%s] ->[%s]\n", __func__, method, uri, *cName);
    }
...
bool VarMapUtils::runFimsMsg(varsmap& vmap, fims_message* msg, fims* p_fims, cJSON** cjri)
{
    ...
    if (0 || process_fims_debug)
        FPS_ERROR_PRINT("%s >> before  checkBlocked  uri [%s]\n", __func__, msg->uri);
    bool reject = checkedBlockedUri(vmap, &newUri, opts, msg->method, msg->uri);
    if (strcmp(msg->method, "pub") != 0)
    {
        if (0 || process_fims_debug)
            FPS_ERROR_PRINT(
                "%s >> after checkBlocked  method [%s] newUri [%s] "
                "reject [%s] opts 0x%04x\n",
                __func__, msg->method, newUri, reject ? "Rejected" : "OK to run", opts);
    }
    ...
    if (strcmp(msg->method, "get") == 0)
    {
        if (0 || process_fims_debug)
            FPS_ERROR_PRINT(
                "%s >> after checkBlocked  method [%s] orig uri [%s] "
                "newUri [%s] reject [%s] opts 0x%04x\n",
                __func__, msg->method, msg->uri, newUri, reject ? "Rejected" : "OK to run", opts);
    ...
    if (!reject)
    {
        ...
        if (0 || process_fims_debug)
            FPS_ERROR_PRINT(
                "%s  after checkSingle single 0x%04x vName [%s] cName2 "
                "[%s] newUri [%s] newBod [%s] cj %p\n",
                __func__, single, vName, cName2, newUri, msg->body, (void*)cj);
        ...
        if (single & 0x1000)
        {
            if (process_fims_debug)
                FPS_ERROR_PRINT(
                    "%s  ready for get/load uivars 0x%04x vName [%s] "
                    "cName2 [%s] newUri [%s] newBod [%s] cj %p\n",
                    __func__, single, vName, cName2, newUri, msg->body, (void*)cj);
            ...
            if (0 || process_fims_debug)
                FPS_ERROR_PRINT(
                    "%s  ready for get/load uivars 0x%04x vName [%s] "
                    "cName2 [%s] newUri [%s]  cj %p\n",
                    __func__, single, vName, cName2, newUri, (void*)cjr);
        }
        else if (single & 0x0100)
        {
            if (0 || process_fims_debug)
                FPS_ERROR_PRINT(
                    "%s  ready for get/load method [%s] single 0x%04x "
                    "vName [%s] cName2 [%s] newUri [%s] newBod [%s] cj "
                    "%p\n",
                    __func__, msg->method, single, vName, cName2, newUri, msg->body, (void*)cj);
            ...
            if (0 || process_fims_debug)
                FPS_ERROR_PRINT(
                    "%s  after get/load uivars 0x%04x vName [%s] cName2 "
                    "[%s] newUri [%s]  cjr %p\n",
                    __func__, single, vName, cName2, newUri, (void*)cjr);
            // vm.processRawMsg(vmap, msg->method, cName2.c_str(), msg->replyto,
            // newBod, &cjr);
            if (0)
                FPS_ERROR_PRINT("%s >> cj result %p\n", __func__, (void*)cjr);
            // if (cjr)
            // {
                                ...
                                asset_manager* am, asset* ai)
{
    ...
    *cjr = getMapsCj(vmap, my.Uri, my.Var, opts);
    ...
    //     if (0)FPS_ERROR_PRINT("%s >> we're working on this one uri [%s] nfrags
    //     %d smap [%s]\n", __func__, uri, nfrags, smap);
    //     *cjr = getMapsCj(vmap, smap, nullptr, opts); if (smap)
    //     free((void*)smap);
    // }
    // else if (nfrags == nbase + 2)
    // {
    //     if (0)FPS_ERROR_PRINT("%s >>  running with uri [%s] nfrags %d\n",
    //     __func__, uri, nfrags); *cjr = getMapsCj(vmap,
    //     uri, nullptr, opts); if (0)FPS_ERROR_PRINT("%s >> returned from uri
    //     [%s] nfrags %d\n", __func__, uri, nfrags);
    ...
    //         __func__, svar, sysnaked ? sysnaked :
    //         "dontknow"); FPS_ERROR_PRINT("%s  we got asuri as [%s]\n",
    //         __func__, asuri);
    //     }
                                     ...
                                     cJSON** cjr, asset_manager* am, asset* ai)
{
    ...
    if (0)
        FPS_ERROR_PRINT("%s >> we're working on this one uri [%s] nfrags %d \n", __func__, uri, nfrags);
    ...
    if (cj)
    {
        ...
        if (0 || process_fims_debug)
            FPS_ERROR_PRINT("%s >> method %s [%s] cj->type %d , cj->child %p cj->next %p\n", __func__, method,
        ...
        while (cjrx)
        {
            ...
            if (0 || setvar_debug)
                FPS_ERROR_PRINT(
                    " %s >> uri [%s] string [%s] cj->type %d (str %d), "
                    "cj->child %p cj->next %p\n",
                    __func__, uri, cjrx->string, cjrx->type, cJSON_String, cjrx->child, cjrx->next);
    ...
    if (cj)
    {
        ...
        if (uiObject > 0)
        {
            ...
            FPS_ERROR_PRINT("%s >> set  uiObject  name [%s] av %p naked [%s]\n", __func__,
                            cjname->valuestring ? cjname->valuestring : " No Name found", av,
        ...
        while (cjrx)
        {
            ...
            // todo what happened to the ui controls ??   they had name but no value
            // .... allow name and ui_type = control todo why is "name" corrupted ??
            // see above
            ...
            if (0 || setvar_debug)
                FPS_ERROR_PRINT(
                    " %s >> uri [%s] string [%s] cj->type %d (str %d), "
                    "cj->child %p cj->next %p av %p\n",
                    __func__, uri, cjrx->string, cjrx->type, cJSON_String, cjrx->child, cjrx->next, av);
            cJSON* cji2 = cJSON_Duplicate(cjrx, true);
            cJSON_AddItemToObject(*cjr, cjrx->string, cji2);
                                     ...
                                     const char* body, cJSON** cjr, asset_manager* am, asset* ai)
{
    ...
    if (cj)
    {
        if (0)
            FPS_ERROR_PRINT(
                " %s >>> uri [%s] method %s uri [%s] frgs %d body [%s] "
                "va [%s] cj->string [%s] cj->type %d , cj->child %p "
                "cj->next %p\n",
                __func__, uri, method, my.Uri, nfrags, body, my.Var ? my.Var : "noVar", cj->string, cj->type,
                (void*)cj->child, (void*)cj->next);
        if (my.Var)
        {
            if (0)
                FPS_ERROR_PRINT("%s >>> [%s] cj->type %d , cj->child %p cj->next %p\n", __func__, my.Var, cj->type,
                                cj->child, cj->next);
            ...
            if (1)
                FPS_ERROR_PRINT(" MSGSetReply we got a VAR set with a replyto [%s]\n", replyto ? replyto : "No Reply");
        ...
        else
        {
            ...
            while (cji)
            {
                ...
                if (1)
                    FPS_ERROR_PRINT("%s >> MSGSetReply we got a Multi set with a replyto [%s]\n", __func__, replyto);
...
bool VarMapUtils::cJSON_Compare(cJSON* a, cJSON* b)
{
    ...
    if (0)
        FPS_ERROR_PRINT(" %s >> a->type  %d b->type %d \n", __func__, a->type, b->type);
    if (a->type == b->type)
    {
        if ((a->type == cJSON_False) || (a->type == cJSON_True))
            return true;
        else if (a->type == cJSON_Number)
        {
            if (std::abs(a->valuedouble - b->valuedouble) < 0.001)
                return true;
        }
...
{
    if (process_fims_debug)
        FPS_ERROR_PRINT("%s >> getting cj maps uri [%s] var [%s] opts 0x%04x\n", __func__, inuri ? inuri : "noURI",
                        var ? var : "noVar", opts);
    int found = 0;
    ...
    if (uri && !var)
    {
        ...
        cJSON* cjm = createUriListCj(vmap, bs, uri, nullptr, opts, nVec);
        {
            if (process_fims_debug)
            {
                ...
                FPS_ERROR_PRINT(
                    "%s >> used createUriList from uri [%s] basevec [%s] "
                    "tmp \n>>%s<<\n",
    ...
    else if (uri && var)
    {
        ...
        if (x != vmap.end())
        {
            ...
            if (y != vmap[uri].end())
            {
                ...
                if (0)
                    FPS_ERROR_PRINT(" %s >> getting cj for uri [%s] var [%s] found %d opts 0x%04x\n", __func__, uri,
                                    y->first.c_str(), found, opts);
                // TODO need opts here
                if (opts & 0x0010)
                {
                    if (0)
                        FPS_ERROR_PRINT(
                            " %s >> showvarCJ  for uri [%s] var [%s] found %d "
                            "opts 0x%04x\n",
                ...
                // if(tmp)
                // {
                //     if(0)FPS_ERROR_PRINT(" %s >> getting cj for uri [%s] var [%s] as
                //     [%s] \n", __func__, uri, y->first.c_str(),
                //     tmp); free((void
                //     *)tmp);
                // }
                cJSON* cj = cji;
                ...
                if (opts & 0x0001 && cji->child)
                {
                    ...
                    // if(tmp)
                    // {
                    //     if(0)FPS_ERROR_PRINT(" %s >> final  cj child for uri [%s] var
                    //     [%s] as [%s] \n", __func__
                    //             , uri
                    //             , var?var:"noVar"
                    //             , tmp);
    ...
    // get all the objects
    {
        ...
        if (0)
            FPS_ERROR_PRINT(" %s >>  get them all opts [0x%04x] \n", __func__, opts);
        int found = 0;

        for (auto& x : vmap)
        {
            if (0)
                FPS_ERROR_PRINT(" %s >>  get them all comp [%s] \n", __func__, x.first.c_str());

            // cJSON* cjx = loadAssetList(vmap, x.first.c_str(), found, opts);
            if (0)
                FPS_ERROR_PRINT(" %s >>  get them all comp [%s]  assetlists \n", __func__, x.first.c_str());

            cJSON* cji = cJSON_CreateObject();
            // assetList* alist = getAlist(vmap, x.first.c_str());

            // if (0)FPS_ERROR_PRINT(" %s >>       >>>>>>>>>>>>>>>>>>>>query alist
            // >>>>>>>>>>>>>>>>>>>>>>>>>>getting cj for comp [%s]  alist %p\n"
            //     , __func__
            //     , x.first.c_str()
            //     , (void*)alist
            // );
            ...
            // else
            {
                for (auto& y : vmap[x.first])
                {
                    ...
                    {
                        if (0)
                            FPS_ERROR_PRINT(" %s >>       NOTE running  for var [%s:%s]  av %p cji %p\n", __func__,
    ...
    if (cj)
    {
        ...
        {
            if (0)
                FPS_ERROR_PRINT(" %s >> getting cj for uri [%s] var [%s] as [%s] \n", __func__, uri,
                                var ? var : "noVar", tmp);
            free((void*)tmp);
        }
    }
...
{
    if (0)
        FPS_ERROR_PRINT("%s >> getting vm maps uri [%s] var [%s] \n", __func__, uri ? uri : "noURI",
                        var ? var : "noVar");
...
bool VarMapUtils::strMatch(const char* str, const char* key)
{
    // char* key2=nullptr;
    if (0)
        printf("  Match test  for str [%s] key [%s] \n", str, key);
    ...
    {
        if (0)
            printf(" >>>> Match true  for str [%s] key [%s] \n", str, key);
        return true;
    }
    if (!str)
    {
    ...
    {
        if (0)
            printf("     >>>> Match true  for str [%s] key [%s] mlen %d \n", str, key, mlen);
        return true;
    }
...
{
    if (0)
        FPS_ERROR_PRINT("%s >> getting cj comps key [%s] var [%s] \n", __func__, key ? key : "noKey",
                        var ? var : "noVar");
...
void VarMapUtils::free_fims_message(fims_message* msg)
{
    if (msg)
    {
        ...
        if (msg->uri)
            free((void*)msg->uri);
        if (msg->replyto)
            free((void*)msg->replyto);
        if (msg->body)
            free((void*)msg->body);
        if (msg->pfrags)
            free((void*)msg->pfrags);
...
{  // vmap["/links/bms_1"]
    ...
    if (ix != vmap.end())
    {
        ...
        FPS_ERROR_PRINT(
            "%s >>  We found our links , we should be able to set up "
            "our link amap\n",
            __func__);
        for (auto iy : ix->second)
        {
            if (iy.second->type == assetVar::ASTRING)
            {
                ...
                FPS_ERROR_PRINT("%s >> lets link [%s] to [%s]\n", __func__, iy.first.c_str(), aVal->valuestring);
...
int VarMapUtils::getSubCount(char* scopy)
{
    ...
    while (*sp)
    {
        ...
        {
            if (0)
                FPS_ERROR_PRINT(" %s >> found comma in [%s] ccnt %d\n", __func__, sp, i);
            i++;
        }
        sp++;
    }
                                ...
                                asset_manager* am)
{
    ...
    if (0)
        FPS_ERROR_PRINT(" %s >> seeking [%s] Found ccnt as %d\n", __func__, alistname, ccnt);
    if (ccnt > 0)
    {
        if (0)
...
char** VarMapUtils::getList(vecmap& vecs, varsmap& vmap, varmap& amap, const char* aname, const char* vname, int& ccnt)
{
    ...
    {
        if (0)
            FPS_ERROR_PRINT(" %s >> found list string dbsubs [%s] ccnt [%d]\n", __func__, dbsubs, ccnt);
...
cJSON* VarMapUtils::loadVmap(varsmap& vmap, int single, const char* comp, const char* var, const char* body)
{
    ...
    if (single & 1)
    {
        ...
        if (0)
            FPS_ERROR_PRINT(" %s >> running single value [%s]\n", __func__, xsp);
...
cJSON* VarMapUtils::getVmap(varsmap& vmap, int& single, const char* key, const char* var, int opts, const char* baseUri)
{
    ...
    if (process_fims_debug)
        FPS_ERROR_PRINT(
            "%s >> RUNNING  key   [%s] var [%s] single 0x%04x opts "
            "0x%04x baseUri %s\n",
    ...
    if (single & 0x0001)
    {
        ...
        if (process_fims_debug)
            FPS_ERROR_PRINT("%s >> Running getMapsCj key   [%s] var [%s] opts 0x%04x\n", __func__, key, var, opts);
    ...
    else
    {
        ...
        if (process_fims_debug)
            FPS_ERROR_PRINT("%s >> Running getMapsCj key   [%s] var nullptr opts 0x%04x\n", __func__, key, opts);
        cJSON* cjall = getMapsCj(vmap, key, nullptr, opts);
        if (0)
            FPS_ERROR_PRINT("%s >> Ran key   [%s] var [%s] single 0x%04x cjall %p \n", __func__, key, var, single,
                            (void*)cjall);
...
cJSON* VarMapUtils::getUimap(varsmap& vmap, int& single, const char* key, const char* var)
{
    ...
    if (0)
        FPS_ERROR_PRINT("%s >> RUNNING  key   [%s] var [%s] single 0x%04x\n", __func__, key, var, single);
    int opts = 0;
    cJSON* cj = nullptr;
    // {
    ...
    //     __func__, key, (void *)cjx, (void *)tmp); if(tmp)
    //     {
    //         if(0)FPS_ERROR_PRINT("%s >> full item key   [%s] tmpall [%s] \n",
    //         __func__, key, tmp); free((void*)tmp);
    //     }
    ...
    //         if(tmpall)
    //         {
    //             if(0)FPS_ERROR_PRINT("%s >>  full item JUST DATA key   [%s]
    //             data [%s] \n", __func__, key, tmpall);
    //             free((void*)tmpall);
    //         }
    //     }
                          ...
                          int retval)
{
    ...
    asprintf(&auri, "/%s/%s", base, aname);

    if (0)
        FPS_ERROR_PRINT("%s >> Alarm uri [%s] Body  [%s]\n", __func__, auri, body ? body : "aint got no body");
                                  ...
                                  const char* msg, int severity)
{
    ...
    {
        if (0)
            FPS_ERROR_PRINT("%s >> falied to make var for [%s]\n", __func__, srcUri);
        return nullptr;
    }
                                  ...
                                  const char* msg, int severity)
{
    ...
    if (0)
        FPS_ERROR_PRINT("%s >> Alarm dest Uri [%s] av %p\n", __func__, destUri, destAv);
                            ...
                            int severity)
{
    ...
    {
        if (0)
            FPS_ERROR_PRINT("%s >> falied to find srcVar for [%s]\n", __func__, srcUri);
        return -1;
    }
                            ...
                            int severity)
{
    ...
    {
        if (0)
            FPS_ERROR_PRINT("%s >> falied to find destVar for [%s]\n", __func__, destUri);
        return -1;
    }
...
int VarMapUtils::setAmFunc(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, asset_manager* am,
                           const char* vname,
...
bool VarMapUtils::loadSiteMap(varsmap& vmap, cJSON* cjss)
{
    ...
    if (cjregs)
    {
        ...
        if (cjinput)
        {
            ret = true;
            if (0)
                FPS_ERROR_PRINT("%s >> found input registers type %d child %p\n", __func__, cjinput->type,
                                cjinput->child);
            cji = cjinput->child;
            while (cji)
            {
        ...
        if (cjhold)
        {
            ret = true;
            if (0)
                FPS_ERROR_PRINT("%s >> found holding registers type %d child %p\n", __func__, cjinput->type,
                                cjinput->child);
            cji = cjhold->child;
            while (cji)
            {
...
assetList* VarMapUtils::getAlist(varsmap& vmap, const char* uri)
{
    ...
    if (tmp)
    {
        ...
        if (0)
            FPS_ERROR_PRINT(" %s >> looking for alist [%s] av [%p] alist [%p]\n", __func__, tmp, av, alist);
...
// :%d,\"date_min\":%d,\"date_sec\":%d}", ltm->tm_year + 1900, ltm->tm_mon,
...
void VarMapUtils::clearVmapxx(varsmap& vmap, bool delAv)
{
    for (auto& x : vmap)
    {
        for (auto& y : vmap[x.first])
        {
            ...
            {
                if (0)
                    FPS_ERROR_PRINT("%s >> running .... YES deleting [%s:%s]\n", __func__, x.first.c_str(),
                                    y.first.c_str());
...
{
    if (0)
        FPS_ERROR_PRINT("%s >> 2 aList %p add av [%s] size %d\n", __func__, &aList, av->name.c_str(),
                        (int)aList.size());
...
assetVar* VarMapUtils::makeVar(varsmap& vmap, const char* comp, const char* var, T& value)
{
    ...
    if (my.Var)
    {
        ...
        vmap[my.Uri][my.Var] = av;
        if (0)
            FPS_ERROR_PRINT("%s >> seeking compFunc for Av input av [%s:%s]\n", __func__, my.Uri, my.Var);
        void* compFunc = getFunc(vmap, "comp", my.Uri,
        ...
        if (compFunc)
        {
            ...
            if (0)
                FPS_ERROR_PRINT("%s >> FOUND compFunc for Av %p input av [%s:%s]\n", __func__, av, av->comp.c_str(),
                                av->name.c_str());
        }

        if (0)
            FPS_ERROR_PRINT(">> %s >> created [%s:%s] av %p\n", __func__, my.Uri, my.Var, (void*)av);
    }
    else
    {
        if (1)
            FPS_ERROR_PRINT(">> %s >> FAILED [%s:%s]\n", __func__, comp, var);
    }
...
assetVar* VarMapUtils::setVal(varsmap& vmap, const char* comp, const char* var, T& value)
{
    ...
    if (my.Var)
    {
        ...
        if (!av)
        {
            ...
            auto& lt = vmap.find("/config/locks");
            if (lt != vmap.end())
            {
                for (auto& xx : lt.second)
                {
                    if (xx.first == my.Uri)  // my.Uri exists in /config/locktable
                    {
                        av->lock = xx.second->getbVal();  // xx.second is lock state.
                        varCreated = true;                // allow newly created variable to have value
                                                          // changed but still add lock for next time.
                    }
                }
            }
        }
        ...
        else
        {
            ...
            if (my.Param)
            {
                // if(paramLocked(av, my.Param)) // TODO GB
                // FPS_ERROR_PRINT("[%s] got to point 1 \n",
                // __func__); auto& ltb =
                // vmap.find("/config/locks"); if(ltb != vmap.end())
                // {
                //     FPS_ERROR_PRINT("[%s] got to point 2 \n",
                //     __func__); for(auto& xy : ltb.second)
                //     {
                //         FPS_ERROR_PRINT("[%s] got to point 3. xy.first is [%s] \n",
                //         __func__ , xy.first); if(xy.first ==
                //         my.Uri) //TODO make this look for parameter
                //         {
                //             if(xy.first->gotParam(my.Param.c_str()))
                //             {
                //                 paramSkip = xy.first->getbParam(my.Param.c_str())
                //             }
                //         }
                //     }
                // }
                if (0)
                    FPS_ERROR_PRINT("%s >> running set Param for [%s:%s@%s] (av->extras %p)\n", __func__, my.Uri,
                                    my.Var, my.Param, av->extras);
                // if(!paramSkip)
                // {
                ...
                // else
                // {
                // if(true)FPS_ERROR_PRINT("[%s] NOTE parameter [%s:%s@%s] locked.
            ...
            // TODO check isDIFF
            if (0 && av->extras)
                FPS_ERROR_PRINT("%s >> >> running  for [%s:%s@%s] (av->extras %p) compFunc %p\n", __func__, my.Uri,
                                my.Var, my.Param, av->extras, av->extras ? av->extras->compFunc : 0);
            ...
            if (av->extras)
            {
                ...
                {
                    if (0)
                        FPS_ERROR_PRINT("%s >> running SetFunc for [%s:%s]\n", __func__, my.Uri, my.Var);
                    setTime();
                ...
                if (av->extras->actVec.size() > 0)
                {
                    if (av->extras->actVec.find("onSet") != av->extras->actVec.end())
                    {
                        if (0)
                            FPS_ERROR_PRINT(
                                "%s >> After Setting  value for [%s] aval "
                                "(float) %f  (int) %d (char) [%s]\n",
                                __func__, my.Var, aVal->valuedouble, aVal->valueint,
                        ...
                        if (0)
                            FPS_ERROR_PRINT("%s >> setActValfrmoCj onSet value for [%s]\n", __func__, my.Var);
