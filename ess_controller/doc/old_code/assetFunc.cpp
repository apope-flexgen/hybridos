    ...
    if (my.Var)
    {
        ...
        else
        {
            ...
            if(my.Param)
            {
                ...
                // auto& ltb = vmap.find("/config/locks");
                // if(ltb != vmap.end())
                // {
                //     FPS_ERROR_PRINT("[%s] got to point 2 \n", __func__);
                //     for(auto& xy : ltb.second)
                //     {
                //         FPS_ERROR_PRINT("[%s] got to point 3. xy.first is [%s] \n", __func__ , xy.first);
                //         if(xy.first == my.Uri) //TODO make this look for parameter
                //         {
                ...
                // else
                // {
                    // if(true)FPS_ERROR_PRINT("[%s] NOTE parameter [%s:%s@%s] locked. Parameter value will not be changed.\n" , __func__, my.Uri, my.Var, my.Param);
                // }
