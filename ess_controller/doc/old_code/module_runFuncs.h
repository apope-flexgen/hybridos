...
#define BMSstartResp 5
        void StartupSystem(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, asset_manager* am)
        {
            ...
            if (reload < 2)
            {
                ...
                linkVals(*vm, vmap, amap, aname, "/status", bval, "EstartSeen", "UiStartupSeen", "SMStartupSeen", "StartupCmd", "FaultStartupSeen");
                linkVals(*vm, vmap, amap, "ess", "/status", bval, "EMMUFaulted", "BMSFaulted", "PCSFaulted", "EMUFaulted");
                linkVals(*vm, vmap, amap, aname, "/status", bval, "StartupRequest", "StartupRequested", "StartupCompleted", "PCSStartupForced", "BMSStartupForced");
                // NOTE: These are all under "ess" where it should be, Estart especially needs this.
                linkVals(*vm, vmap, amap, "ess", "/status", bval, "PCSStartup", "BMSStartup", "StartupSim", "StartupSimSeen", "StartupCompleted");
                ...
                if (reload == 0) // complete restart
                {
                    ...
                    // amap["Estart"]->setVal(false); // Startup from estart command - replaced, wording is probably wrong.
            ...
            if (StartupSim)
            {
                if (!StartupSimSeen)
                {
                    if (1) FPS_ERROR_PRINT("%s >> Starting Startup Sim at time: %f\n", __func__, tNow);
                ...
                {
                    if (1) FPS_ERROR_PRINT("%s >> startping StartupSim at time: %f (tNow-StartupTime) %f\n"
                        , __func__, tNow
                        , (tNow - StartupTime)
                    );
                    //amap["StartupSim"]->setVal(false);
                ...
                else if (tNow - StartupTime > 10.0)
                {
                    ...
                    if (!BMSStartup)
                    {
                        if (1) FPS_ERROR_PRINT("%s >>Forcing  Startup BMS at time: %f\n", __func__, tNow);
                ...
                else if (tNow - StartupTime > 5.0)
                {
                    if (!PCSStartup)
                    {
                        if (1) FPS_ERROR_PRINT("%s >>Forcing  Startup PCS at time: %f\n", __func__, tNow);
                ...
                else if (tNow - StartupTime > 4.0)
                {
                    ...
                    {
                        if (1) FPS_ERROR_PRINT("%s >> Forcing pcs start reply at time: %f\n", __func__, tNow);
            ...
            {
                if (1) FPS_ERROR_PRINT("UIStartup: %s, SMStartup: %s, FaultStartup: %s\n",
                    UiStartup ? "true" : "false",
                    SMStartup ? "true" : "false",
                    FaultStartup ? "true" : "false"
                );
            ...
            {
                asset_manager* amc = ix.second;
                if (0)printf("manager_name: [%s] PCSStartup [%s] BMSStartup [%s]\n"
                    , amc->name.c_str()
                ...
                {
                    if (0)printf("manager_name: [%s] BMSStartup [%s]\n"
                        , amc->name.c_str()
                        , BMSStartup ? "true" : "false"
                    );
                ...
                if (amc->name == "pcs")
                {
                    if (0)printf("manager_name: [%s] PCSStartup [%s]\n"
                        , amc->name.c_str()
                        , PCSStartup ? "true" : "false"
                    );
             ...
             *
             * Everything below here probably needs to be removed, we aren't "forcing" a startup or timing out.
             *
             */
            ...
            if (!PCSStartup && (tNow - StartupTime) > maxPCSStartupTime)
            {
                ...
                vm->sendAlarm(vmap, "/status/ess:StartupRequested", "/components/ess:alarms", "PCSStartup", "PCS Startup Forced", 1);

                // TODO: put an alarm here
                if (1) FPS_ERROR_PRINT(" %s >>>> Forced PCS Startup State at time %f SDtime %f elap %f max %f \n"
                    , __func__
                    , tNow
                    , StartupTime
                    , (tNow - StartupTime)
            ...
            if (PCSStartup && (!BMSStartup && (tNow - StartupTime) > maxBMSStartupTime))
            {
                ...
                // TODO: put an alarm here
                if (1) FPS_ERROR_PRINT(" %s >> Forced BMS Startup State at time %f \n", __func__, tNow);
                vm->sendAlarm(vmap, "/status/ess:StartupRequested", "/components/ess:alarms", "BMSStartup", "BMS Startup Forced", 1);
            }
        ...
        */
        void StartupPCS(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, asset_manager* am)
        {
            ...
            if (reload < 2)
            {
                ...
                linkVals(*vm, vmap, amap, "ess", "/status", bval, "PCSFaulted", "PCSFaultSeen", "StartupRequest", "StartupRequested");
                ...
                linkVals(*vm, vmap, amap, aname, "/status", ival, "PCSStatusResp", "PCSKeyStart", "PCSKeyCmdTries");   // returned status    PCSKEYCmd perhaps
            ...
            if (PCSStartup)
            {
                if (strcmp(SystemStateStep, "Waiting PCS Startup") == 0)
                {
                    ...
                    if (1) FPS_ERROR_PRINT("%s >> All PCS's succesfully Startup at elapsed time: %f \n", __func__, tNow);
                }
                return;
            }
            ...
            if (0) FPS_ERROR_PRINT("%s >>>> PCS Startup system state [%s]  at time: %f \n", __func__, PCSSystemState, tNow);

            // OK to Run Startup ??
            if (/*(strcmp(PCSSystemState, tValInit) == 0) ||*/ (strcmp(PCSSystemState, tValRun) == 0) || (strcmp(PCSSystemState, tValReady) == 0))
            {
                if (1) FPS_ERROR_PRINT("%s >>>>>>>>>>>>>>>>PCS Startup system state [%s]  at time: %f \n", __func__, PCSSystemState, tNow);
                if (1) FPS_ERROR_PRINT("%s >>>> PCS ActivePower %f ReactivePower %f  time: %f \n", __func__
                    , PCSActivePower
                    , PCSReactivePower
                    , tNow);
            ...
            // TODO check timeout
            // if we have not sent the Keystart then wait for the power setpoints to settle
            if (PCSStatusResp == Power_Electronics::OFF || PCSStatusResp == Power_Electronics::READY)
            {
                if (PCSKeyStart == 0 && PCSStartup)
                {
                    ...
                    if (1) FPS_ERROR_PRINT("%s >> PCS power OK sending Keystart at time: %f elspsed: %f \n", __func__, tNow, (tNow - PCSStartupStart));
            ...
            else if (PCSKeyStart && PCSStatusResp != Power_Electronics::ON)
            {
                ...
                {
                    // We need a warning if it was due to timeout.
                    if (1) FPS_ERROR_PRINT("%s >> PCS Timeout sending Keystart at elapsed time: %f \n", __func__, (tNow - PCSStartupStart));
                    ...
                    vm->sendAlarm(vmap, "/status/ess:StartupRequested", "/components/pcs:alarms", "PCSStartup", "PCS Forced Startup", 1);
                }
            }
            ...
            {
                if (PCSStatusResp == PCSstartResp) // TODO: Will be configed - probably come in as a text stream  JIMMY
                {
                    if (1) FPS_ERROR_PRINT("%s >> PCS Status start Reponse found at elapsed time: %f \n", __func__, (tNow - PCSStartupStart));
            ...
            // KeystartSent used to turn off command Keystart used to indicate command sent
            // Return Cmd to zero after PCSCmdTime ?? Config
            ...
            if (PCSKeyStartSent != 0)
            {
                if ((tNow - PCSKeyCmdTime) > maxPCSKeyCmdOnTime)
                {
                    ...
                    //}
                    // vm->sendAlarm(vmap ,amap["PCSKeystart"],"/components/pcs:alarms", "PCSKEYstart","PCS Ignored Command",1);
                }
            }
            // Allow a resend
            if (0) FPS_ERROR_PRINT("%s >> PCS start waiting for PCSStatusResp (%d) cmdTime %f Max %f at  elap time: %f \n", __func__, PCSStatusResp, PCSKeyCmdTime, maxPCSKeyCmdTime, (tNow - PCSKeyCmdTime));
            if (PCSKeyStart != 0)
            {
                if ((tNow - PCSKeyCmdTime) > maxPCSKeyCmdTime)
                {
                    if (PCSKeyCmdTries < maxPCSKeyCmdTries)
                    {
                        ...
                        vm->sendAlarm(vmap, amap["PCSKeyCmd"], "/components/pcs:alarms", "PCSKEYstart", "PCS Ignored Command", 1);
                        if (1) FPS_ERROR_PRINT("%s >> PCS start No PCSStatusResp (%d != %d)  resend after %d tries  attime: %f \n", __func__, PCSStatusResp, PCSstartResp, PCSKeyCmdTries, tNow);
                        //amap["PCSFaulted"]->setVal(true);
                    }
                    else if (PCSKeyCmdTries == maxPCSKeyCmdTries)
                    {
                        amap["PCSKeyCmdTries"]->addVal(1);
                        if (1) FPS_ERROR_PRINT("%s >> PCS start No Cmd Response startped after %d tries  attime: %f \n", __func__, PCSKeyCmdTries, tNow);
                    }
                }
                // TODO Fault the PCS
            }
            ...
            if (((tNow - PCSStartupStart) > maxPCSStartupTime) && !PCSStartup)
            {
                ...
                // ToDo Send Alarm
                if (1) FPS_ERROR_PRINT("%s >>>>>> Time Forced PCS Startup at time %f start %f elap %f  \n ", __func__, tNow, PCSStartupStart, (tNow - PCSStartupStart));
            }
        }
        ...
        /*
            ...
            ////OK linkVals(*vm, vmap, amap, "ess", "/config", dval, "ActivePowerSetpoint", "ReactivePowerSetpoint", "StartupTime");
            ////OK linkVals(*vm, vmap, amap, "ess", "/config", dval, "maxBMSStartupTime");
            ...
            ////OK linkVals(*vm, vmap, amap, aname, "/status", dval, "BMSStartupTime", "BMSStartupStart");
            ////Fixed linkVals(*vm, vmap, amap, "ess", "/config", dval, "maxBMSStartupTime");
            ////OK linkVals(*vm, vmap, amap, "ess", "/status", bval, "BMSStartup", "BMSStatus");
            ...
            ////Remove for now linkVals(*vm, vmap, amap, aname, "/status", ival, "BmsStatus");
                ...
                ////linkVals(*vm, vmap, amap, "ess", "/status", bval, "BMSFaulted", "BMSFaultSeen", "StartupRequest", "StartupRequested");
            ////Fixed linkVals(*vm, vmap, amap, aname, "/status", ival, "BMSKeyCmdSent");
                ...
                //Estart:
                ////linkVals(*vm, vmap, amap, "ess", "/status", bval, "Estart"); // Might not need this?
        ...
        */
        ...
        void StartupBMS(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, asset_manager* am)
        {
            ...
            if (reload < 2)
            {
                ...
                linkVals(*vm, vmap, amap, aname, "/status", bval, "BMSStartup", "BMSStatus");
                linkVals(*vm, vmap, amap, aname, "/status", ival, "BMSnumFaults", "BMSnumPresent"); // Need keyCmd here?

                // other variables:
                linkVals(*vm, vmap, amap, "ess", "/status", bval, "BMSFaulted", "BMSFaultSeen", "StartupRequest", "StartupRequested");
                linkVals(*vm, vmap, amap, "ess", "/status", ival, "BMSKeyStatus");  // maybe /asset/bms
                linkVals(*vm, vmap, amap, aname, "/status", ival, "BMSKeyCmdSent", "BMSKeyCmdTries"); // is Status necessary?
            ...
            if (BMSStartup)
            {
                if (0) FPS_ERROR_PRINT("%s >> All BMS's succesfully Started up at time: %f \n", __func__, tNow);
                return;
            }

            if (1) FPS_ERROR_PRINT("%s >> Startup BMS called for (BMS_Manager: %s) at time: %f \n", __func__, aname, tNow);

            //double  BMSStartupTime = amap["BMSStartupTime"]->getdVal();
            //double  maxBMSStartupTime = amap["maxBMSStartupTime"]->getdVal(); // necessary for a timeout?
            ...
            if (BMSStatus == CATL::INIT) BMSStatusString = (char*)"Init";
            else if (BMSStatus == CATL::NORMAL) BMSStatusString = (char*)"Normal";
            else if (BMSStatus == CATL::FULLCHARGE) BMSStatusString = (char*)"Full Charge";
            else if (BMSStatus == CATL::FULLDISCHARGE) BMSStatusString = (char*)"Full Discharge";
            else if (BMSStatus == CATL::WARNING) BMSStatusString = (char*)"Warning";

            if (1) FPS_ERROR_PRINT("%s >> BMS Status [%d] string [%s] Startup Requested [%d] BMS Key Cmd [%d]\n", __func__, BMSStatus, BMSStatusString, StartupRequested, BMSKeyCmd);

            if ((strcmp(BMSSystemState, tValInit) == 0) || (strcmp(BMSSystemState, tValRun) == 0) || (strcmp(BMSSystemState, tValReady) == 0))
            {
            ...
            {
                if (1) FPS_ERROR_PRINT("Sending Key Cmd Power ON at time %f\n", tNow);
            ...
            if (BMSKeyCmdSent && !BMSStartup && (BMSStatus == CATL::INIT) && ((tNow - BMSKeyCmdTime) > BMSKeyCmdWaitTime)) // annoying bool here - just to get past init call
            {
            ...
            {
                if (1) FPS_ERROR_PRINT("%s >>>>>> BMS Left Init stage at %f \n ", __func__, tNow);
                char* cval = (char*)"BMS Startup";
                char* cvals = (char*)"Startup Complete";
                amap["BMSSystemState"]->setVal(cval);
                amap["BMSSystemStateStep"]->setVal(cvals);
