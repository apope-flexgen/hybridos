...
int  process_fault(varsmap &vmap, varmap &amap, const char* aname, fims* p_fims, assetVar* av)
{
    ...
    if (av)
    {
        ...
        if (0) FPS_ERROR_PRINT(" %s >>  name [%s] value [%s]  lastValue[%s] \n"
            , __func__
            , av->name.c_str()
            , avVal
            , avLVal
        ...
        if (!av->am->amap["FaultDestination"])            av->am->amap["FaultDestination"] = vm->setLinkVal(vmap, av->am->name.c_str(), "/config", "FaultDestination", fltDest);
        if (!av->am->amap["FaultDestination"]->getcVal()) av->am->amap["FaultDestination"]->setVal(noFltMsg);

        if (!av->am->amap["NoFaultMsg"])                  av->am->amap["NoFaultMsg"]       = vm->setLinkVal(vmap, av->am->name.c_str(), "/config", "NoFaultMsg", noFltMsg);
        if (!av->am->amap["NoFaultMsg"]->getcVal())       av->am->amap["NoFaultMsg"]->setVal(noFltMsg);

        if (0) FPS_ERROR_PRINT("%s >> Fault Destination [%s]\n", __func__
                                , av->am->amap["FaultDestination"]->getcVal() ? av->am->amap["FaultDestination"]->getcVal() : "did not find val from config");
        if (0) FPS_ERROR_PRINT("%s >> No fault msg [%s]\n", __func__
                                , av->am->amap["NoFaultMsg"]->getcVal() ? av->am->amap["NoFaultMsg"]->getcVal() : "did not find val from config");
...
int  process_alarm(varsmap &vmap, varmap &amap, const char* aname, fims* p_fims,assetVar* av)
{
    ...
    if (av)
    {
        ...
        if (!av->am->amap["AlarmDestination"])            av->am->amap["AlarmDestination"] = vm->setLinkVal(vmap, av->am->name.c_str(), "/config", "AlarmDestination", alrmDest);
        if (!av->am->amap["AlarmDestination"]->getcVal()) av->am->amap["AlarmDestination"]->setVal(noAlrmMsg);

        if (!av->am->amap["NoAlarmMsg"])                  av->am->amap["NoAlarmMsg"]       = vm->setLinkVal(vmap, av->am->name.c_str(), "/config", "NoAlarmMsg", noAlrmMsg);
        if (!av->am->amap["NoAlarmMsg"]->getcVal())       av->am->amap["NoAlarmMsg"]->setVal(noAlrmMsg);

        if (0) FPS_ERROR_PRINT("%s >> Alarm Destination [%s]\n", __func__
                                , av->am->amap["AlarmDestination"]->getcVal() ? av->am->amap["AlarmDestination"]->getcVal() : "did not find val from config");
        if (0) FPS_ERROR_PRINT("%s >> No alarm msg [%s]\n", __func__
                                , av->am->amap["NoAlarmMsg"]->getcVal() ? av->am->amap["NoAlarmMsg"]->getcVal() : "did not find val from config");
        ...
        else if (strcmp(avVal, avLVal) != 0)
        {
            ...
            if (strcmp(avVal, noAlrmMsg) != 0)
            {
                ...
                {
                    // see if it works now
                    if(1) vm->sendAlarm(vmap, av, dest, nullptr, msg, 2);
                    if (0)FPS_ERROR_PRINT(" %s >> AlarmSent dest [%s] msg [%s]  am %p \n", __func__, dest, msg, (void*)av->am);
...
int  process_status(varsmap &vmap,  varmap &amap, const char* aname, fims* p_fims, assetVar* av)
{
    ...
    if (av)
    {
        ...
        if (strcmp(avVal, avLVal) != 0) av->setLVal((const char*)avVal);
