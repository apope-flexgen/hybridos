...
{
    std::string vtot(lname); vtot += "tot";
    std::string vavg(lname); vavg += "avg";
    std::string vmax(lname); vmax += "max";
    std::string vcnt(lname); vcnt += "cnt";
...
void run_ess_init(asset_manager* am)
{
    if (1)FPS_ERROR_PRINT(">>>>>>>>>ESS>>>>>>>>>>>%s running for ESS Manager vm %p \n", __func__, am->vm);
    am->vm->setTime();
    double dval = 0.0;
    double dvalHB = 1.0;
    bool bval = false;
 ...
 * Note: only process_sys_alarm is used here. Add additional functions here (perhaps there's a better way to do this...)
 * 
...
void initFuncs(asset_manager* am)
{
    ...
    if (1) FPS_ERROR_PRINT("%s >> About to set process_sys_alarm for %s\n", __func__, am->name.c_str());
    for (auto& ix : am->assetManMap)
    {
        asset_manager* amc = ix.second;
        if (1) FPS_ERROR_PRINT("%s >> Got asset manager %s. Setting func now...\n", __func__, amc->name.c_str());
        ...
        {
            asset* ami = iy.second;
            if (1) FPS_ERROR_PRINT("%s >> Got asset instance %s. Setting func now...\n", __func__, ami->name.c_str());
...
void dummyPub(asset_manager* am, const char* uri, const char* puri)
{
    ...
    if(0)FPS_ERROR_PRINT("%s >> aname [%s] << publish status cjp %p \n",__func__, am->name.c_str(), cjp);
    // pname ="Unknown2";
    // res = "After pub";
    ...
    if (cjp && cjp->child && cjp->child->string && cjp->child->child)
    {
        ...
        if (0) FPS_ERROR_PRINT("%s >>>>>>>Publish fims  res >>%s<< cjp %p\n", __func__, res, cjp);
        ...
        if (0) FPS_ERROR_PRINT("%s >>>>>>>Able to publish to [%s] [%s] fims \n", __func__, uri, puri);
        am->p_fims->Send("pub", cjp->child->string, nullptr, res);
        if(res)free(res);
        res = nullptr;
        if(cjp)cJSON_Delete(cjp);
        cjp = nullptr;
    }
    else
    {
        ...
        if(cjp && cjp->child)
        {
            ...
            if (0) FPS_ERROR_PRINT("%s >>>>>>>Able to publish to [%s] [%s] fims cjp %p cjp->child %p >>%s<<\n"
                , __func__
                , uri
                , puri
                , cjp
        ...
        {
            if (1) FPS_ERROR_PRINT("%s >>>>>>>Unable to publish to [%s] [%s] fims cjp %p \n"
                , __func__
                , uri
                , puri
                , cjp
...
bool run_ess_wakeup(asset_manager* am, int wakeup)
{
    ...
    if (am->reload != 2)
    {
        ...
        if(!setup)
        {
            if (1)FPS_ERROR_PRINT("%s MANAGER LOOP %s  setup MonitorVar\n", __func__, am->name.c_str());

            if(0)am->vm->setMonitorList(*am->vmap, "bms");
            ...
            setup = 1;
            if (1)FPS_ERROR_PRINT("%s MANAGER LOOP %s  completed setup MonitorVar\n", __func__, am->name.c_str());

            // double dval = 45.0;
            // assetVar* av =  am->amap["MaxCellTemperature"] = vm->setLinkVal(*am->vmap, "ess", "/status", "MaxCellTemperature", dval);
            // av->am = am;
            // int ival = 10;
...
//avarmap avmap;
    ...
    if (wakeup == WAKE_LEVEL1000)
    {
        ...
        if( am->amap["RunWake1000"])
        {
            ...
            if (!runMe)FPS_ERROR_PRINT("%s MANAGER LOOP %s WAKE1000 run_secs %d\n", __func__, am->name.c_str(), run_secs);
        }
        ...
        if(am->amap["SendPcs"])
        {
            ...
            if (!runMe)FPS_ERROR_PRINT("%s MANAGER LOOP %s WAKE1000 sendPcs %s am2 %p [%s]\n", __func__, am->name.c_str()
                , sendPcs?"true":"false"
                , am2
                , am2->name.c_str()
                );
...
//int defaultFault_module::ShutdownSystem(varsmap &vmap, varmap &amap, const char* aname, fims* p_fims, asset*am);
//int defaultRun_module::StartupSystem(varsmap &vmap, varmap &amap, const char* aname, fims* p_fims, asset*am);
        ...
        if( am->amap["RunWake100"]->getbVal())
        {
            ...
            //int UpdateSysTime(varsmap &vmap, varmap &amap, const char* aname, fims* p_fims, asset_manager* av);
            //int UpdateSysTime(varsmap &vmap, varmap &amap, const char* aname, fims* p_fims, asset_manager*am);
            ...
            if(1)UpdateSysTime(*am->vmap, am->amap, am->name.c_str(), am->p_fims, &Av);
            ...
            if (0)FPS_ERROR_PRINT("%s MANAGER LOOP %s UpdateSysTime\n", __func__, am->name.c_str());

            if(0)myCompFcn(*am->vmap, am->amap, am->name.c_str(), am->p_fims, &Av);
            ...
            //     TimestampAv =  am->amap["Timestamp"] = vm->setLinkVal(*am->vmap, "ess", "/status", "Timestamp", initTimestamp);
            //     TimestampAv->am = am;
            // }
            // if(TimestampAv && !TimestampAv->am) TimestampAv->am = am;
            // if (TimestampAv && TimestampAv->am && TimestampAv->am->vm)
            //     CheckAmComms(*am->vmap, am->amap, am->name.c_str(), am->p_fims, &Av);

            //CheckEssStatus(*am->vmap, am->amap, am->name.c_str(), am->p_fims, am);
            // SetupLimitsVec(*am->vmap, am->amap, avmap, am->name.c_str(), "MaxCellCurrent", am);
            // SetupLimitsVec(*am->vmap, am->amap, avmap, am->name.c_str(), "MaxCellVolt", am);
            // CheckOverLimitsVec(*am->vmap, avmap, am->name.c_str(), "MaxCellCurrent", am->p_fims, am);
            // CheckOverLimitsVec(*am->vmap, avmap, am->name.c_str(), "MaxCellVolt", am->p_fims, am);
            defaultFault_module::ShutdownSystem(*am->vmap, am->amap, am->name.c_str(), am->p_fims, &Av);
            defaultRun_module::StartupSystem(*am->vmap, am->amap, am->name.c_str(), am->p_fims, &Av);

            
            if(0)FPS_ERROR_PRINT("%s >> running new monitorList2  ess \n", __func__);
            am->vm->runMonitorList2(*am->vmap, am->amap, "ess",am->p_fims, "wake_monitor");//am->p_fims,/*am->amap,*/ 0.1, 0.0);
            if(0)FPS_ERROR_PRINT("%s >> completed running new monitorList2  ess\n", __func__);

            if(0)FPS_ERROR_PRINT("%s >> running new monitorList2  bms \n", __func__);
            am->vm->runMonitorList2(*am->vmap, am->amap, "bms",am->p_fims, "wake_monitor");//am->p_fims,/*am->amap,*/ 0.1, 0.0);
            if(0)FPS_ERROR_PRINT("%s >> completed running new monitorList2  bms\n", __func__);

            if(0)FPS_ERROR_PRINT("%s >> running new monitorList2  pcs \n", __func__);
            ...
            if(0)FPS_ERROR_PRINT("%s >> completed running new monitorList2  pcs  vmap %p res21 %p \n", __func__, am->vmap, res21);

            if(0)FPS_ERROR_PRINT("%s >> running old monitorList \n",__func__);
            if(0)am->vm->runMonitorList(*am->vmap, am->amap, "ess",am->p_fims,/*am->amap,*/ 0.1, 0.0);
...
//using namespace defaultFault_module;
            // if(1) UpdateSysTime(*am->vmap, am->amap,  am->name.c_str(), am->p_fims, am);//test/test_walker.cpp:
            // if(0) HandleAssetHeartbeat(*am->vmap, am->amap,  am->name.c_str(), am->p_fims, am);//test/test_walker.cpp:
    ...
    {    
        ...
        if (0)FPS_ERROR_PRINT("%s >> aname [%s] >> publish status RunPub %p ->%s time %2.3f \n"
            , __func__
            , am->name.c_str()
            , am->amap["RunPub"]
            , runPub ? "true":"false"
        ...
        if(runPub)
        {
            ...
            for (auto sv : sysVec)
            {
                ...
                if (sv != "site/summary")
                {
                    ...
                    if (1)dummyPub(am, auri.c_str(), auri.c_str());
                }
            }
            // HACK HACK HACK
...
//         char* msg;
//         asprintf(&msg, "%s alarm generated [%s]", av->name.c_str(), av->aVal->valuestring ? av->aVal->valuestring : "No Data");
//         av->am->vm->sendAlarm(vmap, av->am->name.c_str(), dest, nullptr, msg, 2);
//         FPS_ERROR_PRINT(" %s >> dest [%s] msg [%s]  \n", __func__, dest, msg);
...
int main(int argc, char* argv[])
{
    ...
    // vm->setFunc(vmap, "sbmu_1", "process_sys_alarm", (void*)&process_sys_alarm);
    // vm->setFunc(vmap, "sbmu_2", "process_sys_alarm", (void*)&process_sys_alarm);
    // vm->setFunc(vmap, "sbmu_3", "process_sys_alarm", (void*)&process_sys_alarm);
    // vm->setFunc(vmap, "sbmu_4", "process_sys_alarm", (void*)&process_sys_alarm);
    // vm->setFunc(vmap, "sbmu_5", "process_sys_alarm", (void*)&process_sys_alarm);
    // vm->setFunc(vmap, "sbmu_6", "process_sys_alarm", (void*)&process_sys_alarm);
    // vm->setFunc(vmap, "sbmu_7", "process_sys_alarm", (void*)&process_sys_alarm);
    // vm->setFunc(vmap, "sbmu_8", "process_sys_alarm", (void*)&process_sys_alarm);
    // vm->setFunc(vmap, "sbmu_9", "process_sys_alarm", (void*)&process_sys_alarm);
   ...
   // this is just a check
    ...
    // }
    // todo mo
    cfgname = vm->getFname("patch_configs/test_ess.json");// TODO move this into asset manager assconfig
...
//        }
    ...
    //         {
    //             FPS_ERROR_PRINT("%s >> unable to connect to fims server after %d attempts. All is lost.\n", __func__, fims_count);
    //             exit(0);
    //         }
    //         FPS_ERROR_PRINT("%s >> failed  to connect to fims server on attempt %d \n", __func__, fims_count);
    //         running = 0;
    //         fims_count++;
    //         sleep(5);
    //     }
    ...
    {
         poll(nullptr, 0, 1000);
         FPS_ERROR_PRINT("%s >> name %s waiting to connect to FIMS,attempt %d\n", __func__, "ess_controller", cattempt++);
    }
    ...
    // this will start serviceing the incoming fims messages but , as of yet , no one is listening.
    ess_man->p_fims = p_fims;
        ...
        for (auto iy : ixa->second)
        {
            if (iy.second->type == assetVar::ASTRING)
            {
                ...
                if (iy.first == "ess" || iy.first == "bms" || iy.first == "pcs" || iy.first == "site")
                {
                    ...
                    Av->am = ass_man;
                    // vm->setAvFunc(vmap, "comp", "/alarms", aname, Av, (void*)&process_alarm);
                    // vm->setAvFunc(vmap, "comp", "/faults", aname, Av, (void*)&process_fault);
                    // vm->setAvFunc(vmap, "comp", "/status", aname, Av, (void*)&process_status);
    ...
    // set up vlinks
    //std::map<std::string, 
    //   std::map<std::string, assetVar*, std::less<std::string>, std::allocator<std::pair<std::string const, assetVar*> > >, std::less<std::string>,
    //     std::allocator<std::pair<std::string const, std::map<std::string, assetVar*, std::less<std::string>, std::allocator<std::pair<std::string const, assetVar*> > > > > >&, 
    //         char const*)
    ...
    // NOTE this could collide with the  wake manager thread so lets put it in there
