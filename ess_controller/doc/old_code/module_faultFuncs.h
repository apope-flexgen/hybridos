...
namespace defaultFault_module
{
    ...
    extern "C"
    {
        ...
        // Put dummy functions inside each of the managers for their Shutdown's
         ...
         *     1) Send to PCS_Manager the ShutdownPCS's command (SIDENOTE: have PCS_Manager set [ActivePowerSetPoint && ReactivePowerSetPoint] to 0 )
         ...
         *  // Statuses, etc.:
                ...
                // other variables:
                bool
                /status/ess/EStop             estop input flag , several possible sources. TODO Should be attached directly to Shutdown.
                /status/ess/UiShutdown        UserInterface shutdown request
                /status/ess/SMShutdown        Site Manage Shutdown request
                /status/ess/FaultShutdown     Fault Shutdown Request. Set by the fault system
                ...
                /status/ess/ShutdownRequest      Main trigger for a shutdown start , set by any system that wants to initiate a shutdown. We'll attach an alarm to it later.
                /status/ess/ShutdownRequested    Indication that we have responded to the latest shutdown request.
                /status/ess/ShutdownCompleted    Indication that we have completed the latest Shutdown Request
                 bool
                /status/ess/PCSShutdownForced     Indication that we forced the PCS to shutdown It did not  resond to the current request.
                /status/ess/BMSShutdownForced     Indication that we forces the BMS shutdown.

                /status/ess/PCSShutdown           Indication that the PCSShutdown has completed
                /status/ess/BMSShutdown           Indication that the BMSShutdown has completed

                /status/ess/ShutdownSim           Request for simulation code path tester  reset as soon as SimSeen is set.
                /status/ess/ShutdownSimSeen       Indication that the simulation request has been seen , reset when the simulation has completed

                /status/ess/ShutdownCompleted     Indication that Shutdown has been completed.
...
#define BMSStopResp 5
        void ShutdownSystem(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, asset_manager* am)
        {
            ...
            if (reload < 2)
            {
                ...
                //linkVals(*vm, vmap, amap, aname, "/status", ival, "BmsStatus", "PcsStatus", "EmuStatus", "EmmuStatus");

                // other variables:
                linkVals(*vm, vmap, amap, aname, "/status", bval, "EStop", "UiShutdown", "SMShutdown", "FaultShutdown");
                linkVals(*vm, vmap, amap, aname, "/status", bval, "EStopSeen", "UiShutdownSeen", "SMShutdownSeen", "ShutdownCmd", "FaultShutdownSeen");
                linkVals(*vm, vmap, amap, "ess", "/status", bval, "EMMUFaulted", "BMSFaulted", "PCSFaulted", "EMUFaulted");
                linkVals(*vm, vmap, amap, aname, "/status", bval, "ShutdownRequest", "ShutdownRequested", "ShutdownCompleted", "PCSShutdownForced", "BMSShutdownForced");
                // NOTE: These are all under "ess" where it should be, EStop especially needs this.
                linkVals(*vm, vmap, amap, "ess", "/status", bval, "PCSShutdown", "BMSShutdown", "ShutdownSim", "ShutdownSimSeen", "ShutdownCompleted");
                linkVals(*vm, vmap, amap, "pcs", "/config", ival, "PCSKeyCmd");
                linkVals(*vm, vmap, amap, "pcs", "/status", ival, "PCSStatusResp");  // is this needed here ??
                linkVals(*vm, vmap, amap, "bms", "/config", ival, "BMSKeyCmd");
                linkVals(*vm, vmap, amap, "bms", "/status", ival, "BMSStatusResp");  // is this needed here ??

                // other variables:
                //linkVals(*vm, vmap, amap, aname, "/status", ival, "PCSKeyStatus", "PCSKeyCmd");
            ...
            if (0) FPS_ERROR_PRINT("%s >> Running Shutdown  at time: %f\n", __func__, tNow);

            if (ShutdownSim)
            {
                if (!ShutdownSimSeen)
                {
                    if (1) FPS_ERROR_PRINT("%s >> Starting Shutdown Sim at time: %f\n", __func__, tNow);
                ...
                {
                    if (1) FPS_ERROR_PRINT("%s >> Stopping ShutdownSim at time: %f (tNow-ShutdownTime) %f\n"
                        , __func__, tNow
                        , (tNow - ShutdownTime)
                    );
                    //amap["ShutdownSim"]->setVal(false);
                ...
                else if (tNow - ShutdownTime > 10.0)
                {
                    ...
                    if (!BMSShutdown)
                    {
                        if (1) FPS_ERROR_PRINT("%s >>Forcing  Shutdown BMS at time: %f\n", __func__, tNow);
                ...
                else if (tNow - ShutdownTime > 5.0)
                {
                    if (!PCSShutdown)
                    {
                        if (1) FPS_ERROR_PRINT("%s >>Forcing  Shutdown PCS at time: %f\n", __func__, tNow);
                ...
                else if (tNow - ShutdownTime > 4.0)
                {
                    ...
                    {
                        if (1) FPS_ERROR_PRINT("%s >> Forcing pcs stop reply at time: %f\n", __func__, tNow);
            ...
            {
                if (0)FPS_ERROR_PRINT("""""""""""""I AM COMPLETE!!!!""""""""""\n");
            ...
            if (ShutdownCmd) // TODO: Make these one-shots
            {
                if (amap["ShutdownRequested"]->getbVal())
                {
                    ...
                    if (1) FPS_ERROR_PRINT("%s >> Shutdown Request Ignored,  In request at time: %f elap %f \n", __func__, tNow, (tNow - ShutdownTime));

                }
                if (1) FPS_ERROR_PRINT("UIShutdown: %s, SMShutdown: %s, FaultShutdown: %s\n",
                    UiShutdown ? "true" : "false",
                    SMShutdown ? "true" : "false",
                    FaultShutdown ? "true" : "false"
                );
                if (!ShutdownRequested) // we are having a shutdown requested here when you set ShutdownCompleted to false.
                {
                    if (1) FPS_ERROR_PRINT("%s >> Func Starting Shutdown at time: %f elap %f \n", __func__, tNow, (tNow - ShutdownTime));
                    amap["ShutdownCmd"]->setVal(false);
                    ...
                    if (1) FPS_ERROR_PRINT("%s >> Setting ShutdownTime to %f\n", __func__, tNow);

                    vm->sendAlarm(vmap, "/status/ess:ShutdownRequested", "/components/ess:alarms", "Shutdown", "Shutdown Started", 1);
                }
            }
            ...
            if (!ShutdownRequested)
            {
                if (0) FPS_ERROR_PRINT("%s >> Func Quitting Shutdown at  time: %f\n", __func__, tNow);
                return;
            }
            ...
            if (amap["SystemState"]->valueChangedReset() | amap["SystemStateStep"]->valueChangedReset())
            {
                ...
                if (1) FPS_ERROR_PRINT("%s >> Shutdown  state [%s] step [%s]  time elapsed : %f\n", __func__, cstate, csstate, tNow - sdtime);
            }
            ...
            {
                asset_manager* amc = ix.second;
                if (0)FPS_ERROR_PRINT("manager_name: [%s] PCSFaulted [%s] BMSFaulted [%s]\n"
                    , amc->name.c_str()
                    , PCSFaulted ? "true" : "false"
                    , BMSFaulted ? "true" : "false"
                );

                if (amc->name == "pcs")
                {
                    if (0) FPS_ERROR_PRINT("manager_name: [%s] PCSShutdown [%s]\n"
                        , amc->name.c_str()
                    ...
                    {
                        if (1) FPS_ERROR_PRINT("%s >>>> cascading function to >>>>> Manager [%s] at time: %f\n ", __func__, amc->name.c_str(), tNow);
                        ShutdownPCS(vmap, amc->amap, amc->name.c_str(), p_fims, amc);
                    }
                }
                if (amc->name == "bms")
                {
                    ...
                    if (0) FPS_ERROR_PRINT("manager_name: [%s] BMSShutdown [%s]\n"
                        , amc->name.c_str()
                    ...
                    {
                        if(!BMSFaulted && ((PCSShutdown && !BMSShutdown) || PCSFaulted)) // This logic was added to guarantee no cascade is done without PCS being shutdown first. Might need to be changed.
                        {
                            if (1) FPS_ERROR_PRINT("%s >>>> cascading function to >>>>> Manager [%s] at time: %f\n ", __func__, amc->name.c_str(), tNow);
                            ShutdownBMS(vmap, amc->amap, amc->name.c_str(), p_fims, amc);
                        }
            ...
            //     vm->sendAlarm(vmap, "/status/ess:ShutdownRequested", "/components/ess:alarms", "PCSShutdown", "PCS Shutdown Forced", 1);

            //     // TODO: put an alarm here
            //     if (1) FPS_ERROR_PRINT(" %s >>>> Forced PCS Shutdown State at time %f SDtime %f elap %f max %f \n"
            //         , __func__
            //         , tNow
            //         , ShutdownTime
            //         , (tNow - ShutdownTime)
            ...
            // if (PCSShutdown && (!BMSShutdown && (tNow - ShutdownTime) > maxBMSShutdownTime))
            // {
            //     amap["BMSShutdown"]->setVal(true);
            //     amap["BMSShutdownForced"]->setVal(true);
            //     // TODO: put an alarm here
            //     if (1) FPS_ERROR_PRINT(" %s >> Forced BMS Shutdown State at time %f \n", __func__, tNow);
            //     vm->sendAlarm(vmap, "/status/ess:ShutdownRequested", "/components/ess:alarms", "BMSShutdown", "BMS Shutdown Forced", 1);
            // }
            ...
            if (BMSShutdown && PCSShutdown) // then it goes here and does this.
            {
                if (!ShutdownCompleted)
                {
                    ...
                    if (1) FPS_ERROR_PRINT("%s >> Full Shutdown Completed, elapsed time: %f \n", __func__, (tNow - sdtime));
                    vm->sendAlarm(vmap, "/status/ess:ShutdownRequested", "/components/ess:alarms", "Shutdown", "Shutdown Completed", 1);
                }
            }
            else if (!amap["ShutdownCmd"]->getbParam("HardShutdown") && PCSShutdown)
            {
                if (!ShutdownCompleted)
                {
                    ...
                    amap["ShutdownRequested"]->setVal(false);
                    amap["ShutdownCompleted"]->setVal(true);
                    if (1) FPS_ERROR_PRINT("%s >> PCS Shutdown Completed, elapsed time: %f \n", __func__, (tNow - sdtime));
                    vm->sendAlarm(vmap, "/status/ess:ShutdownRequested", "/components/ess:alarms", "Shutdown", "Shutdown Completed", 1);
                }
            }
        ...
        /*
                ...
                // check this linkVals(*vm, vmap, amap, aname, "/status", ival, "PCSStatusResp", "PCSKeyStop");   // returned status

                // other variables:
                // todo: link EStop to aname = "ess" not itself.
                // Check this linkVals(*vm, vmap, amap, "ess", "/status", bval, "PCSFaulted", "PCSFaultSeen", "ShutdownRequest", "ShutdownRequested");
        ...
        */
        void ShutdownPCS(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, asset_manager* am)
        {
            ...
            if (reload < 2)
            {
                ...
                linkVals(*vm, vmap, amap, "ess", "/status", bval, "PCSFaulted", "PCSFaultSeen", "ShutdownRequest", "ShutdownRequested", "ShutdownCmd");
                ...
                linkVals(*vm, vmap, amap, aname, "/status", dval, "PCSActivePower", "PCSReactivePower");   // ModBus responses
                linkVals(*vm, vmap, amap, aname, "/status", ival, "PCSStatusResp", "PCSKeyStop", "PCSKeyCmdTries");   // returned status    PCSKEYCmd perhaps
                ...
                if (reload == 0) // complete restart 
                {
                    ...
                    if (0) FPS_ERROR_PRINT("%s >>>>>>>>>>>>>>>>PCS Shutdown system state [%s]  at time: %f \n", __func__, amap["PCSSystemState"]->getcVal(), tNow);

                }
                reload = 2;
                pcsAv->setVal(reload);
            }
            const char* stopUri = amap["ShutdownCmd"]->getbParam("HardShutdown") ? "/components/remote_control/e_stop" : "/components/remote_control/stop";
            ...
            {
                if (1) FPS_ERROR_PRINT("%s >> PCS State [%s] Step [%s] time: %f \n", __func__, PCSSystemState, PCSSystemStateStep, tNow);
            }
            bool PCSShutdown = amap["PCSShutdown"]->getbVal();
            ...
            if (PCSShutdown)
            {
                if (strcmp(SystemStateStep, "Waiting PCS Shutdown") == 0)
                {
                    ...
                    if (1) FPS_ERROR_PRINT("%s >> All PCS's succesfully Shutdown at elapsed time: %f \n", __func__, tNow);
                }
                return;
            }
            ...
            if (PCSFaulted)
            {
                if (strcmp(SystemStateStep, "Waiting PCS Shutdown") == 0)
                {
                    ...
                    amap["SystemStateStep"]->setVal(SystemStateStep);
                    if (1) FPS_ERROR_PRINT("%s >> PCS Faulted at time: %f \n", __func__, tNow);
                }

                // May have to do other stuff for example
                vm->sendAlarm(vmap, "/status/ess:ShutdownRequested", "/components/pcs:alarms", "Shutdown", "PCS Fault Shutdown", 1);
            ...
            // Do not run if in Init The PCS monitor will bring things out of init or we have to do it by hand 
            if (0) FPS_ERROR_PRINT("%s >>>> PCS Shutdown system state [%s] step [%s]  at time: %f \n"
                , __func__
                , PCSSystemState
                , PCSSystemStateStep
                , tNow);
            ...
            if (ShutdownRequested)
            {
                if (strcmp(PCSSystemState, tValInit) == 0)
                {
                    ...
                    if (!PCSShutdown || !PCSSystemStateStep || (strcmp(PCSSystemStateStep, cvals) != 0))
                    {
                        ...
                        amap["PCSShutdown"]->setVal(true);
                        if (1) FPS_ERROR_PRINT("%s >>>> PCS Shutdown  Forced because we're in system state [%s]  at time: %f \n", __func__, PCSSystemState, tNow);

                    }
                    return;
                }
                else if (PCSStatusResp == Power_Electronics::STOP || PCSStatusResp == Power_Electronics::FAULT) // TODO: Will be configed - probably come in as a text stream  JIMMY
                {
                    if (1) FPS_ERROR_PRINT("%s >> PCS Status Stop Reponse found at elapsed time: %f \n", __func__, (maxPCSKeyCmdTime * PCSKeyCmdTries) + (tNow - PCSShutdownStart));
                    ...
                    amap["PCSKeyCmd"]->setVal(0);
                    p_fims->Send("set", stopUri, nullptr, "{\"value\":0}");      //need to turn off stop command?
            ...
            if (PCSKeyStop == 0)
            {
                ...
                if (1) FPS_ERROR_PRINT("%s >> PCS power OK sending KeyStop at time: %f elspsed: %f \n", __func__, tNow, (tNow - PCSShutdownStart));
            ...
            if (!PCSShutdown)// && (PCSKeyStop != 0))
            {
                if ((tNow - PCSKeyCmdTime) > maxPCSKeyCmdTime)
                {
                    if (PCSKeyCmdTries < maxPCSKeyCmdTries - 1)
                    {
                        ...
                        vm->sendAlarm(vmap, amap["PCSKeyCmd"], "/components/pcs:alarms", "PCSKEYSTOP", "PCS Ignored Command", 1);
                        if (1) FPS_ERROR_PRINT("%s >> PCS Stop No PCSStatusResp (%d != %d)  resend after %d tries  attime: %f \n", __func__, PCSStatusResp, PCSStopResp, PCSKeyCmdTries+1, tNow);
                        //amap["PCSFaulted"]->setVal(true);
                    }
                    else if (PCSKeyCmdTries == maxPCSKeyCmdTries - 1)
                    {
                        amap["PCSKeyCmdTries"]->addVal(1);
                        if (1) FPS_ERROR_PRINT("%s >> PCS Stop No Cmd Response Stopped after %d tries  attime: %f \n", __func__, PCSKeyCmdTries+1, tNow);
                        // p_fims->Send("set", "/components/remote_control/e_stop", nullptr, "{\"value\":0}");
                        ...
                        // ToDo Send Alarm
                        if (1) FPS_ERROR_PRINT("%s >>>>>> Time Forced PCS Shutdown at time %f start %f elap %f  \n ", __func__, tNow, PCSShutdownStart);
                    }
                }
                if (1) FPS_ERROR_PRINT("%s >>> PCS Shutdown attempt no. %d time since shutdown start %f\n ", __func__, PCSKeyCmdTries+1, tNow - PCSShutdownStart);

                vm->sendAlarm(vmap, "/status/ess:ShutdownRequested", "/components/pcs:alarms", "PCSShutdown", "PCS Forced Shutdown", 1);
            }
            ...
            if (PCSKeyStopSent != 0)
            {
                ...
                {
                    if (1) FPS_ERROR_PRINT("%s >> PCS Reset Cmd  at elapsed time: %f \n", __func__, (tNow - PCSKeyCmdTime));

                    amap["PCSKeyStopSent"]->setVal(0);   // force a resend
                    amap["PCSKeyCmd"]->setVal(0);
                    p_fims->Send("set", stopUri, nullptr, "{\"value\":0}");
                    // vm->sendAlarm(vmap ,amap["PCSKeyStop"],"/components/pcs:alarms", "PCSKEYSTOP","PCS Ignored Command",1);
                }
            }
            ...
            if (((maxPCSKeyCmdTime * PCSKeyCmdTries) + (tNow - PCSShutdownStart) > maxPCSShutdownTime) && !PCSShutdown)
            {
                ...
                // ToDo Send Alarm
                if (1) FPS_ERROR_PRINT("%s >>>>>> Time Forced PCS Shutdown at time %f start %f elap %f  \n ", __func__, tNow, PCSShutdownStart, (tNow - PCSShutdownStart));
            }
        ...
        //  *     ... add other system as needed
         // Statuses, etc.:
        /*
                / Should an asset be accessing another asset's states? - could this not lead to error?
                // TODO: In future make a read-only linkVal (as to prevent errors from occuring due to shared links)
                // reload
                linkVals(*vm, vmap, amap, aname, "/reload", reload, __func__);
                bmsAv = amap[__func__];
                // Statuses, etc.:
            ////OK  linkVals(*vm, vmap, amap, "ess", "/status", tValInit, "SystemState", "SystemStateStep");
            ////OK linkVals(*vm, vmap, amap, "ess", "/config", dval, "ActivePowerSetpoint", "ReactivePowerSetpoint", "ShutdownTime");
            ////OK linkVals(*vm, vmap, amap, "ess", "/config", dval, "maxBMSShutdownTime");
            ////Remove linkVals(*vm, vmap, amap, "ess", "/config", dval, "PowerSetpointDeadband");
            ////OK linkVals(*vm, vmap, amap, aname, "/status", tValInit, "BMSSystemState", "BMSSystemStateStep");
            ////OK linkVals(*vm, vmap, amap, aname, "/status", dval, "BMSShutdownTime", "BMSShutdownStart");
            ////Fixed linkVals(*vm, vmap, amap, "ess", "/config", dval, "maxBMSShutdownTime");
            ////OK linkVals(*vm, vmap, amap, "ess", "/status", bval, "BMSShutdown", "BMSStatus");
            ////Remove linkVals(*vm, vmap, amap, "bms", "/status", ival, "BMSnumFaults", "BMSnumPresent"); // Need keyCmd here?
                // linkVals(*vm, vmap, amap, aname, "/status", ival, "BMSKeyCmd", "BMSKeyStatus");


            ////Remove for now linkVals(*vm, vmap, amap, aname, "/status", ival, "BmsStatus");

                // other variables:
                // TODO: Estop to aname = "ess" not itself.
                ////linkVals(*vm, vmap, amap, "ess", "/status", bval, "BMSFaulted", "BMSFaultSeen", "ShutdownRequest", "ShutdownRequested");
            ////Fixed linkVals(*vm, vmap, amap, aname, "/status", ival, "BMSKeyCmdSent");
                ////linkVals(*vm, vmap, amap, aname, "/status", ival, "BMSKeyCmd", "BMSKeyStatus"); // is Status necessary yes for feedback
                //linkVals(*vm, vmap, amap, aname, "/status", bval, "BMSShutdown", "BMSStatus");
                // linkVals(*vm, vmap, amap, aname, "/config", ival, "PCSKeyStopCmd");
                ////linkVals(*vm, vmap, amap, aname, "/config", ival, "BMSFaultStatus");

                //EStop:
                ////linkVals(*vm, vmap, amap, "ess", "/status", bval, "EStop"); // Might not need this?
                ...
                /status/bms/BMSStatusResp            Code indicating the current BMS status.  from ModBudMap Jimmy
                /status/pcs/PCSKeyCmd            Place to send  PCS command.  from ModBus Map Jimmy
                /status/bms/BMSKeyCmdSent        Place record of  BMS command sent.
                /status/bms/BMSKeyCmdTries       Place record of  BMS command send tries.
                /status/bms/BMSKeyCmd            Place to Send  BMS command.  from Modbus Map  Jimmy
        ...
        */
        ...
        void ShutdownBMS(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, asset_manager* am)
        {
            ...
            // TODO: keyCMD is a shared mbmu register that will shut down all of the BMS's.
            ...
            if (0) FPS_ERROR_PRINT("%s >> Shutdown BMS called for (BMS_Manager: %s) at time: %f \n", __func__, aname, tNow);
            ...
            if (reload < 2)
            {
                ...
                linkVals(*vm, vmap, amap, "bms", "/status", ival, "BMSnumFaults", "BMSnumPresent"); // Need keyCmd here?
                // linkVals(*vm, vmap, amap, aname, "/status", ival, "BMSKeyCmd", "BMSKeyStatus");

                // other variables:
                linkVals(*vm, vmap, amap, "ess", "/status", bval, "BMSFaulted", "BMSFaultSeen", "ShutdownRequest", "ShutdownRequested", "ShutdownCmd");
                linkVals(*vm, vmap, amap, "ess", "/status", ival, "BMSKeyStatus");  // maybe /asset/bms
                linkVals(*vm, vmap, amap, aname, "/status", ival, "BMSKeyCmd", "BMSKeyCmdSent", "BMSKeyCmdTries"); // is Status necessary?
                linkVals(*vm, vmap, amap, aname, "/config", dval, "BMSKeyCmdTime"); // is Status necessary?
                linkVals(*vm, vmap, amap, aname, "/config", ival, "BMSFaultStatus");
                // linkVals(*vm, vmap, amap, aname, "/status", ival, "num_hv_subsystem");
                // amap["NumSBMUConnected"] = vm->setLinkVal(vmap, aname, "/status", "num_hv_subsystem", ival);
            ...
            {
                if (1) FPS_ERROR_PRINT("%s >>  BMS State [%s] Step [%s] time: %f \n", __func__, BMSSystemState, BMSSystemStateStep, tNow);
            }
            bool BMSShutdown = amap["BMSShutdown"]->getbVal();
            ...
            if (BMSShutdown)
            {
                if (strcmp(SystemStateStep, "Waiting BMS Shutdown") == 0)
                {
                    ...
                    amap["SystemStateStep"]->setVal(SystemStateStep);
                    if (1) FPS_ERROR_PRINT("%s >> All BMS's succesfully Shutdown at elapsed time: %f \n", __func__, tNow);
                }
                return;
            }
            ...
            if (BMSFaulted)
            {
                if (strcmp(SystemStateStep, "Waiting BMS Shutdown") == 0)
                {
                    ...
                    amap["SystemStateStep"]->setVal(SystemStateStep);
                    if (1) FPS_ERROR_PRINT("%s >> BMS Faulted at time: %f \n", __func__, tNow);
                }

                // May have to do other stuff for example
                vm->sendAlarm(vmap, "/status/ess:ShutdownRequested", "/components/bms:alarms", "Shutdown", "BMS Fault Shutdown", 1);
            ...
            if (ShutdownRequested)
            {
                ...
                else if (BMSKeyCmdSent == 0)
                {
                    if (strcmp(SystemStateStep, "Waiting BMS Shutdown") == 0)
                    {
                        ...
                        amap["SystemStateStep"]->setVal(SystemStateStep);
                        if (1) FPS_ERROR_PRINT("%s >> BMSEStop at time: %f \n", __func__, tNow);
                    }

                    vm->sendAlarm(vmap, "/status/ess:ShutdownRequested", "/components/bms:alarms", "Shutdown", "PCS EStop Shutdown", 1);
            ...
            if (BMSKeyCmdSent != 0)
            {
                if (BMSKeyCmd != 0)
                {
                    ...
                    {
                        if (1) FPS_ERROR_PRINT("%s >> BMS Reset Cmd  at elapsed time: %f \n", __func__, (tNow - BMSKeyCmdTime));
                        amap["BMSKeyCmd"]->setVal(0);
                ...
                if ((tNow - BMSKeyCmdTime) > BMSKeyCmdWaitTime)
                {
                    if (BMSKeyCmdTries < maxBMSKeyCmdTries - 1)
                    {
                        ...
                        if (1) FPS_ERROR_PRINT("%s >>>>>> BMS cmd status [%d!=%d] Retry Cmd attempt %d at %f \n "
                            , __func__
                            , BMSKeyStatus
                            , BMSKeyStatusStop
                            , BMSKeyCmdTries
                    ...
                    {
                        if (1) FPS_ERROR_PRINT("%s >>>>>> BMS Retry Cmd failed at %f \n ", __func__, tNow);
                        amap["BMSShutdown"]->setVal(true); //add "failedShutdown"?
                        amap["BMSKeyCmdTries"]->setVal(0);
                        amap["BMSKeyCmdSent"]->setVal(0);
                    }
                }
                if (((((BMSKeyCmdWaitTime * (BMSKeyCmdTries - 1)) + (tNow - BMSKeyCmdTime)) > maxBMSShutdownTime)) && !BMSShutdown)
                {
                    FPS_ERROR_PRINT("time %f max time %f\n",(BMSKeyCmdWaitTime * BMSKeyCmdTries) + (tNow - BMSKeyCmdTime), maxBMSShutdownTime);
                    char* cval = (char*)"BMS Shutdown";
                    char* cvals = (char*)"Shutdown Timeout";
                    amap["BMSSystemState"]->setVal(cval);
                    amap["BMSSystemStateStep"]->setVal(cvals);
                    if (1) FPS_ERROR_PRINT("%s >>>>>> Forced BMS Shutdown at %f \n ", __func__, tNow);
                    amap["BMSShutdown"]->setVal(true);
                    amap["BMSKeyCmdTries"]->setVal(0);
                    amap["BMSKeyCmdSent"]->setVal(0);  // force retry
                }
            ...
            //     // amap["BMSKeyCmd"]->setVal(PowerOffCmd); // Is this necessary? Isn't this an error state?
            // }
        }
        ...
        // NOTE: Needs aname to be a particular bms_number - Might be enum instead.
        // IMPORTANT: This is DEPRECATED, we only control a single register to tell all of the BMS's to Shutdown.
        void ShutdownBMSasset(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, asset* ai)
        {
            //Timer ShutdownBMSTimer("ShutdownBMSTimer", true);
            ...
            if (0) FPS_ERROR_PRINT("%s >> Shutdown BMSAsset called for (BMS: %s) at time: %f \n", __func__, aname, tNow);
            ...
            if (reload < 2)
            {
                ...
                // reload
                linkVals(*vm, vmap, amap, "ess", "/status", tValInit, "SystemState", "SystemStateStep");
                //linkVals(*vm, vmap, amap, "ess", "/config", dval, /*"ActivePowerSetpoint", "ReactivePowerSetpoint",*/ "ShutdownTime");
                // linkVals(*vm, vmap, amap, "ess", "/config", dval, "PCSKeyActivePowerSetpoint", "PCSKeyReactivePowerSetpoint");
            ...
            // want to see: timer for regular Shutdown vs. EStop Shutdown. Use tNow (differences) in FPS_ERROR_PRINTs.
            ...
            if (((tNow - BMSShutdownTime) > maxBMSShutdownTime) && !BMSShutdown)
            {
                ...
                amap["BMSShutdown"]->setVal(true);
                // ToDo Send Alarm
                if (1) FPS_ERROR_PRINT("%s >>>>>> Timeout Forced BMS Shutdown at %f \n ", __func__, tNow);
                amap["BMSnumPresent"]->subVal(1);
                amap["BMSnumFaults"]->addVal(1);
            }
        ...
        // void sendKeyStopCmd(/* faultFuncArgs, other args? */) //LC send KEYSTOP command to PCS
        // {
        ...
        //     //     {
        //     //         bool bval;
        //     //         amap[KeyStop] = vm->setLinkVal(vmap, "pcs", "/controls", "KeyStopCmd", bval);
        //     //     }
        //     //
        //     //     // Need to check if pcs has already received keystop cmd (keystop = true)
        //     //     
        //     //     bool keyStopCmdActive = amap[KeyStop]->getVal(keyStopCmdActive);
        //     //     int sysState = amap[SystemStateNum]->getVal(sysState);
        //     //     if (!keyStopCmdActive && sysState == System_Fault && aname != "pcs")
        //     //     {
        //     //         amap[keyStopCmdActive]->setVal(true);
        //     //         vm->sendAssetVar(amap[keyStopCmdActive], p_fims);
        //     //     }
        //     //
        //     //     // After KeyStopCmd has been sent to PCS, we'll probably need to check if the cmd actually went through
