...
int HandlePower(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, asset_manager* am)
{
    ...
    if (Pset->valueChangedReset() | PmaxC->valueChangedReset() | PmaxD->valueChangedReset() |
    ...
    std::cout << "Pset: " << Pset->getdVal() << " PmaxC: " << PmaxC->getdVal() << " Pcmd: " << Pcmd->getdVal()
              << std::endl;
    return rc;
}
...
int HandleBMSChargeL2(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, asset* am)
{
    ...
    else
    {
        ...
        ival = 1;
        numActBms->addVal(ival);
    }

    printf("%s --- Active BMS's: [%d] max C [%f] max D [%f]\n", aname, numActBms->getiVal(), PmaxC->getdVal(),
           PmaxD->getdVal());
...
bool run_ems_wakeup(asset_manager* am, int wakeup)
{
    // FPS_ERROR_PRINT("%s %2.3f >>>>>>>>>>>>>>>>>>>>> running for %s Manager wval
    // %d \n",__func__, am->vm->get_time_dbl(),
    // am->name.c_str(), wakeup); then service the other channels
    //
    char* item3;
    fims_message* msg;
