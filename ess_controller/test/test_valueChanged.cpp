/*
 * vmap basic test
 * more advance use where we pull a reference to a data point an run some calculations on it
...
These will become unordered_maps sometime soon they are 10X faster but use 2X space.
...
int main(int argc, char *argv[])
{
    ...
    printf(" First Test Value changed       [%s] expect [true] at chg %2.6f set %2.6f  last %d currrent  %d  time diffuS %f\n"
        , bval?"true":"false"
        , dtime3
        , dtime2
    ...
    printf(" Second Test Value changedReset [%s] expect [true] at chg %2.6f set %2.6f last %d currrent  %d  time diffuS %f\n"
        , bval?"true":"false"
        , dtime3
        , dtime2
        , ival1, ival2, (dtime2-dtime1)*1000000.0);
    ...
    ival2 =StandbyTestI->getiVal();
    bval = StandbyTestI->valueChanged();
    printf(" Third Test Value changed       [%s] expect [false] last %d currrent  %d  time diffuS %f\n"
        , bval?"true":"false"
        , ival1, ival2, (dtime2-dtime1)*1000000.0);

    printf(" set it again to %d  and retest  we should still register no change possibly also check chgtime\n", ixval);
    ...
    ival2 =StandbyTestI->getiVal();
    bval = StandbyTestI->valueChangedReset();
    printf(" Third Test Value changedReset) [%s] expect [false] at chg %2.6f set %2.6f last %d currrent  %d  time diffuS %f\n"
        , bval?"true":"false"
        , dtime3
        , dtime2
        , ival1, ival2, (dtime2-dtime1)*1000000.0);
    ...
    printf(" First Test Value changedReset [%s] expect [true]  at chg %2.6f set %2.6f last %2.6f currrent  %2.6f  time diffuS %f db *10^6 %2.6f\n"
        , bval?"true":"false"
        , dtime3
        , dtime2
        ...
        , dval3*1000000.0);
    bval = av2->valueChangedReset();
    printf(" First Test Value changedReset [%s] expect [false] at chg %2.6f set %2.6f last %2.6f currrent  %2.6f  time diffuS %f\n"
        , bval?"true":"false"
        , dtime3
        , dtime2
        , dval1, dval2, (dval2-dval1)*1000000.0);
    ...
    dval2 =av2->getdVal();
    bval = av2->valueChangedReset();
    printf(" First Test Value changedReset [%s] expect [true] at chg %2.6f set %2.6f last %2.6f currrent  %2.6f  time diffuS %f\n"
        , bval?"true":"false"
        , dtime3
        , dtime2
        , dval1, dval2, (dval2-dval1)*1000000.0);
