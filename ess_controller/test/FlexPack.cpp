...
// /usr/local/bin/fims/fims_send -m set -r /$$ -u /gpio/control/gpio/getPubs true
...
// /usr/local/bin/fims/fims_send -m set -r /$$ -u /gpio/full/sched/gpio '{"schedGpioRwPub":{"value":"schedGpioRwPub","repTime":20}}'
...
void signal_handler(int sig)
{
    ...
    {
        flex_man->running = 0;
        if(flex_man->wakeChan)flex_man->wakeChan->put(-1);
    }
    signal(sig, SIG_DFL);
}
...
//         FPS_ERROR_PRINT("%s >> dump config  av [%s] table [%s]  file [%s] len [%d]\n"
//             , __func__
//             , aV->getfName()
//             , table?table:"none"
//             , outFile
...
// int PostHtml(varsmap &vmap, VarMapUtils*vm, char*host, int port,  std::string hdr, std::string msg)
// {
//     std::string response;
//     struct iovec iv[2];
...
// // if we dont have one then create one and start the thread (if it is not running)
...
//     if(debug)FPS_ERROR_PRINT("%s >> Processing [%s] publishing db [%s] table [%s] as [%s]\n"
//         , __func__
//         , aV->getfName()
//         , db
//         , table
...
//     PostHtml(vmap, vm, host, port, hdr.str(), msg.str());
...
//     }
//     if(debug)FPS_ERROR_PRINT("%s >> Processing [%s] publishing table [%s] as [%s]\n"
//         , __func__
//         , aV->getfName()
//         , table
//         , sendas
...
//     FPS_ERROR_PRINT("%s >> Flex >>Done Setting vlinks\n",__func__);
//     return 0;
//}
// "/links/rack_6":{
//     "Year": {
...
//     FPS_ERROR_PRINT("%s >> Flex >>Done Setting links\n",__func__);
//     if(aV->gotParam("amap"))
//     {
...
//     }
//     FPS_ERROR_PRINT("%s >> setting up manager [%s->%s] filename [%s]\n",__func__, pname, aname, fname);
...
// /status/flex/chargeCurrent = system charge current -ve to charge +ve to discharge
// /status/flex/chargeTime = simulated time between cycles 
...
double getCellChargeAlloc(double cellCharge, double totalCharge, int numCells, int soc, int avg_soc)
{
    ...
    else
    {
        ...
        if(0)dval = dval + (1.0 - socadjust) * dval;
    }
    return dval; 
}
...
int RunCell(varsmap &vmap, varmap &amap, const char* aname, fims* p_fims, assetVar* aV)
{
    ...
    FPS_ERROR_PRINT("%s >> Running for assetVar [%s] with aname [%s] and asset manager [%s]\n"
                  , __func__, aV->getfName(), aname, aV->am->name.c_str());

    asset_manager* fixed_am = aV->am->vm->getaM(vmap, aname);
    // Make sure we are using the right asset manager for the assetVar based on the
    // given aname
    if (fixed_am) aV->am = fixed_am;
    FPS_ERROR_PRINT("%s >> Running for assetVar [%s] with aname [%s] and asset manager (fixed)  [%s]\n"
        , __func__, aV->name.c_str(), aname, fixed_am->name.c_str());

	VarMapUtils* vm = aV->am->vm;
    // reload is pitched at the base level we'll call this flex shouls probably be bms
    // any cell being calculated after that time will use the same base options.
    // if we have different cabinets with different characteristics then the reload can be rooted at bms_1 / bms_2  etc
    // the cell itself will have its nesting specified 
    // /bms_x/rack_y/module_z
    // that will map out the asset_master layout
    // the cell itself will have a defined flat table based on this /bms_x{delim}rack_y{delim}module_z:{"cell_xx":{},...}
    // why ... well it helps make the system work.
    // the interface (modbus) maps require the flat structure although some are getting more modular 
    //  we need the asset_manager->asset structure  am->am->am->ai to manage the recursive detail collecting data 
    ...
    if (1) FPS_ERROR_PRINT("%s >> Reload val for asset manager %s reload name %s is %d.\n"
                         , __func__, aname, reloadStr.c_str(), reload);
    if (reload < 2)
    {
        FPS_ERROR_PRINT("%s >> Reload val for asset manager %s reload name %s is %d. Reloading...\n"
             , __func__, aname, reloadStr.c_str(), reload);
        ...
        }
        {
            ...
            FPS_ERROR_PRINT("%s >>\tsoc,\tvolts,\tcharge,(charge power), (alloc),\t\tdischarge,(dischargepower),(alloc)\n",__func__);
            for (int soc = 0; soc< 100 ; soc+=5)
            {
                ...
                dischargeAlloc   = getCellDischargeAlloc(dischargeCurrent*volts*timeSlice, totalDischarge, numCells, soc, soc);
                chargeAlloc      = getCellChargeAlloc(chargeCurrent*volts*timeSlice, totalCharge, numCells, soc, soc);
                FPS_ERROR_PRINT("%s >>\t%02d,\t%2.3f,\t%2.3f,\t%2.3f,\t%2.3f,\t%2.3f,\t%2.3f,\t%2.3f\n"
                    , __func__
                    , soc
                    , volts
                    , chargeCurrent
                    , (volts * chargeCurrent * 0.1)
                    , chargeAlloc
                    , dischargeCurrent
                    , (volts * dischargeCurrent * 0.1)
                    , dischargeAlloc
                    );
            }
        }
        int ival = 2;
        //av->am->amap[reloadStr] = vm->setLinkVal(vmap, aname, "/reload", reloadStr.c_str(), ival);
    ...
    FPS_ERROR_PRINT("%s >> av [%s] soc %f volts %f discharge %f \n"
            , __func__, av->getfName(), soc, volts, dischargeCurrent) ;
    // are we discharging 
    if(chargePower > 0.0)
    {
...
// these allow us to stop the program /control/flex:runTime  /control/flex:stopTime
    ...
    while (*sc->run)
    {
        ...
        if(sc->reqChan.get(si, false)) 
        {
            if(si) 
            {
                ...
                FPS_ERROR_PRINT("%s >> Servicing  Sched Request %p id [%s] (%p)  "
        ...
        if (sc->fimsChan.get(msg, false))
        {
            if(msg)
            {
                ...
                {
                    if(0)FPS_ERROR_PRINT("%s >> Setting up for a fims message old actions %d  \n"
                               ,__func__, sc->vm->schedActions);
                }
        ...
        if(debug)FPS_ERROR_PRINT("%s>> new delay = %2.6f\n", __func__, ddelay);
        delay = ddelay;
...
int main_test_new_flex(int argc, char *argv[])
{
    ...
    //vm.setSequence(*flex_man->vmap, flexName, "help",     (void*)nullptr,"used to save sequences of operations ");    
    ...
    if (debug)FPS_ERROR_PRINT(" %s >> %s start load controller \n", __func__, flexName); 
    {
        char *sp;
    ...
    if (debug)FPS_ERROR_PRINT(" %s >> %s done load controller rc %d \n", __func__, flexName, rc ); 
    ...
    {
       if (1)FPS_ERROR_PRINT(" %s >> flex_man >> No Subs found in flex config \n", __func__); 
       return 0;
    }

    //p_fims = sched.p_fims;
    flex_man->p_fims = p_fims;
    if (debug)FPS_ERROR_PRINT(" %s >> %s >> p_fims %p\n", __func__, flexName, p_fims); 
    //return 0;
...
int main(int argc, char *argv[])
{
    ...
    // setting up the default console logger for the FlexPack Controller from now on
