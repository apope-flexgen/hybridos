...
funVec* parse_funItem(cJSON* cj, varsmap& varMaps)
{
    ...
    if (cji)
    {
        ...
        cJSON_ArrayForEach(ci, cji)
        {
            ...
            if (1)
                printf(" found input array item [%s] [%s]\n", civ ? civ->valuestring : "no var",
    ...
    if (cji)
    {
        ...
        cJSON_ArrayForEach(ci, cji)
        {
            ...
            if (1)
                printf(" found out array item [%s] [%s] [%s]\n", cio ? cio->valuestring : "no op",
                       civ ? civ->valuestring : "no var", cii ? cii->valuestring : "no id");
...
int main()
{
    ...
    void* res = vm.getFunc(varMaps2, "ess", "MyTestFunc");
    ...
    dvalue = 9000.0;
    vm.setVal(varMaps, "/site/bss_params", "min_dc_volts", dvalue);
    dvalue = 10000.0;
    vm.setVal(varMaps, "/site/bss_params", "max_dc_volts", dvalue);
    ...
    // varMaps["/features/bss_status"]["max_dc_volts"]= new
    // assetVar("max_dc_volts",900.5);
    // varMaps["/features/bss_status"]["avg_dc_volts"]= new
    // assetVar("avg_dc_volts",901.5);
    // varMaps["/features/bss_status"]["min_dc_volts"]= new
    // assetVar("max_dc_volts",902.5);  varMaps["base"]["d1"]= new
    // assetVar("d1",true);
    ...
    /// std::vector<assetVar*> ins = make_vector<assetVar*>() << d1 << v1 << v2 <<
    /// v3 << v4 << v5;
    // std::vector<assetVar*> outs = make_vector<assetVar*>() << d2 << sum << avg
    // << max << min;
    ...
    printf("after first run \n sum %f \n avg %f \n min %f\n max %f \n ", varMap["sum"]->getdVal(),
           varMap["avg"]->getdVal(), varMap["min"]->getdVal(), varMap["max"]->getdVal());
    ...
    else
    {
        ...
        printf(" for second run \n bss_1 %f \n bss_2 %f \n bss_3 %f\n\n ",
               varMaps["/components/bss_1"]["dc_volts"]->getdVal(), varMaps["/components/bss_2"]["dc_volts"]->getdVal(),
