...
// todo: get this to work with spdlog::info, error, warn, etc. (the default
// logger) spdlog has a stderr color sink, but I might need to make a singleton
// for that. it even has an _mt and _st version of that, so the singleton
// wouldn't be too bad as we could just swap out the _st with _mt when moving to
// a multi-threaded environment. Therefore we could get all the benefits of
// spdlog when printing to the console.
#define NEW_FPS_ERROR_PRINT(...) fmt::print(stderr, __VA_ARGS__);

// my implementaiton of the new FPS singleton pattern using spdlogs
// stderr_color_sink possible problem: we can't have more than one guy going to
// the same sink, not without sharing the same sink_ptr.
class FPS_Printer final
{
    // todo: could also set this to _mt for a multithreaded environment, singleton
    // pattern could then stay. this is a neat trick because we can get spdlog
    // functionality while still providing macros to access the singleton. the _mt
    // will also allow for thread synchronization when printing to the console if
    // needed. will take it slow, but this will, in my mind, inevitably replace
    // FPS_ERROR_PRINT
    spdlog::logger mLogger;

    FPS_Printer() : mLogger("FPS_PRINTER", std::make_shared<spdlog::sinks::stderr_color_sink_st>())
    {
        // if we don't like the redundant [FPS_PRINTER] we can just change the
        // pattern here
...
public:
    ...
    // There is no set pattern, I think the default pattern is correct for
    // printing to console. Unless we absolutely need to change the pattern, I
    // don't plan on providing that functionality.
...
// alternatively the "third way" -> just use the regular stdout
// spdlog::func(fmt, args); this is by default _mt (thread safe) so threads
// trying to stdout to console will be no problem. NOTE: it's not stderr it's
// stdout so we might need to use the stderr singleton above.

// spdlog::set_level(spdlog::level::debug); // will have to include this for the
// debug macros, doesn't work in global space though.
...
// #define OUT_PRINT_DEBUG(...) spdlog::debug(__VA_ARGS__); // todo: work on
// this, default log doesn't print out debug
...
int main()
{
    ...
    // this is how I could make the new FPS_ERROR_PRINT, just wrap this in a
    // singleton actually this has been implemented in a much more succinct manner
    // above.
    ...
    // If you want to do the print for time since epoch without the "e-0x" then
    ...
    {{
        ...
        "{}": {:.9}
                     ...
                     "json's and raw strings", "are fun", "so is the time since January 1st of this year",
                     flex::time_since_epoch());

    // with the new macros the above fps_logger would just look like this in our
    // code: uses the above FPS_Printer singleton to stderr
    FPS_PRINT_CRITICAL("this is the {} pattern that we can use",
                       "new");  // this provide colors and timestamps to our console output now.
