...
{
    FPS_ERROR_PRINT("%s >> Testing valid case - vars are within range of key-pair values in table...\n", __func__);
    ...
    if (expResults == actResults) FPS_ERROR_PRINT("soc [%f] temp [%f]    expResults [%f] == actResults [%f] --> SUCCESS\n\n", soc, temp, expResults, actResults);
    else                          FPS_ERROR_PRINT("soc [%f] temp [%f]    expResults [%f] != actResults [%f] --> FAILURE\n\n", soc, temp, expResults, actResults);
...
{
    FPS_ERROR_PRINT("%s >> Testing valid case - vars are within the range of key-pair values in the table (edge case)...\n", __func__);

    FPS_ERROR_PRINT("Test 1: Var one and var two values are in the lower bound in p_charge_tbl.csv\n");
    double soc = 0;
    double temp = -30;
    ...
    FPS_ERROR_PRINT("Test 2: Var one and var two values are in the upper bound in p_charge_tbl.csv\n");
    soc = 100;
    temp = 65;
    expResults = 0.000000;
    actResults = getTableValue(soc, temp, tbl);
...
{
    FPS_ERROR_PRINT("%s >> Testing invalid valid case - vars are outside the range of key-pair values in the table...\n", __func__);

    FPS_ERROR_PRINT("Test 1: Var one and var two are less than the lower bound in p_charge_tbl.csv\n");
    double soc = -1;
    double temp = -36;
    double expResults = 0.000000;
    double actResults = getTableValue(soc, temp, tbl);
    ...
    FPS_ERROR_PRINT("Test 2: Var one and var two are greater than the upper bound in p_charge_tbl.csv\n");
    soc = 101;
    temp = 66;
    expResults = 0.000000;
    actResults = getTableValue(soc, temp, tbl);
