...
public:
    ...
    {
        // mLogger->enable_backtrace(1); // stores only one message to go to file at
        // a time.
    }

    const std::chrono::time_point<std::chrono::steady_clock> getCreateTime() const { return mCreationTime; }
...
public:
    ...
    WindowLog(const char* loggerName,
              const std::vector<spdlog::sink_ptr>& sinks = { std::make_shared<spdlog::sinks::rotating_file_sink_st>(
