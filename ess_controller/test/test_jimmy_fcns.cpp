 ...
 * @brief File that contains functions for testing (either modified functions
 * from previous files like test_phil_fcns.cpp or new functions)
 * @version 0.1
 * @date 2020-12-07
 *
...
//         "/status", "Timestamp",          initTimestamp);
...
//         amap["essCommsFaultCnt"]      = vm->setLinkVal(vmap, "ess",
//         "/errors",     "essCommsFaultCnt",     ival);
//         amap["essCommsAlarmCnt"]      = vm->setLinkVal(vmap, "ess",
//         "/errors",     "essCommsAlarmCnt",     ival); amap["essCommsInit"]
//         = vm->setLinkVal(vmap, "ess",                "/status",
//         "essCommsInit",     ival); amap["essCommsFaultTimeout"]  =
//         vm->setLinkVal(vmap, "ess",                "/config",
//         "essCommsFaultTimeout",     toFault); amap["essCommsAlarmTimeout"]  =
//         vm->setLinkVal(vmap, "ess",                "/config",
//         "essCommsAlarmTimeout",     toAlarm); amap["essCommsResetTimeout"]  =
//         vm->setLinkVal(vmap, "ess",                "/config",
//         "essCommsResetTimeout",     toReset);

//         if(am->am)
//         {
//             amap["amCommsFaultCnt"]  = vm->setLinkVal(vmap,
//             am->am->name.c_str(), "/status",    "CommsFaultCnt", ival);
//             amap["amCommsAlarmCnt"]  = vm->setLinkVal(vmap,
//             am->am->name.c_str(), "/status",    "CommsAlarmCnt", ival);
//             amap["amCommsInit"]      = vm->setLinkVal(vmap,
//             am->am->name.c_str(), "/status",    "CommsInit",           ival);
//         }

//         amap["CommsFaultCnt"]        = vm->setLinkVal(vmap, aname,
//         "/status",     "CommsFaultCnt",       ival); amap["CommsAlarmCnt"]
//         = vm->setLinkVal(vmap, aname,                "/status",
//         "CommsAlarmCnt",       ival); amap["CommsInit"]            =
//         vm->setLinkVal(vmap, aname,                "/status",
//         "CommsInit",         ival); amap["CommsState"]           =
//         vm->setLinkVal(vmap, aname,                "/status",
//         "CommsState",        cval); amap["BypassComms"]          =
//         vm->setLinkVal(vmap, aname,                "/config",
//         "BypassComms",       bval); amap["CommsStateNum"]        =
//         vm->setLinkVal(vmap, aname,                "/status",
//         "CommsStateNum",      ival); amap["CommsOK"]              =
//         vm->setLinkVal(vmap, aname,                "/status",     "CommsOK",
//         bval);
...
//             // time remaining before fault amap["Timestamp"]
//             ->setParam("rdAlarm", toAlarm);                      // time
//             reamining before alarm amap["Timestamp"]
//             ->setParam("rdReset", toReset);                      // time
//             remaining before reset amap["Timestamp"]     ->setParam("rdLast",
//             dval);                         // time when last to event was
//             seen
...
//         amap["Timestamp"] ->getdParam("rdReset"); amap["Timestamp"]
//         ->setParam("rdLast",tNow);
...
//         {
//             amap["Timestamp"]     ->setParam("lastTimestamp",
//             currentTimestamp);
...
//                 currentTimestamp, rdReset, (tNow - rdLast), rdAlarm,
//                 rdFault);
...
//                     cleared at %2.3f\n", __func__,
//                     aname, tNow); amap["Timestamp"]
//                     ->setParam("seenFault",false);
...
//                 Asset_Ok; // seen Timestamp change amap["CommsStateNum"]
//                 ->setVal(ival); ival = 0; amap["CommsInit"]->setVal(ival);
//                 char *tval; asprintf(&tval," Comms OK last set %2.3f Alarm
...
//             // This seems redundant. Fault/alarm time already incremented
//             (see above)
//             // if(rdFault < toFault)
//             // {
//             //     rdFault += (tNow - rdLast);
...
//             last Value", Ts); amap["Timestamp"]
//             ->setParam("lastTimestamp",currentTimestamp);
 ...
 *
 */
// int CheckAmHeartbeat(varsmap &vmap, varmap &amap, const char* aname, fims*
// p_fims, asset_manager*am)
// {
//     //double dval = 0.0;
//     int ival = 0;
...
//         amap["Heartbeat"]            = vm->setLinkVal(vmap, aname,
//         "/status", "Heartbeat",          initHeartbeat);
//         if(1)FPS_ERROR_PRINT("%s >>  aname TimeStamp %p comp [%s] name [%s]
//         \n"
//                 , __func__
//                 , aname
...
//         amap["essHeartbeatFaultCnt"]     = vm->setLinkVal(vmap, "ess",
//         "/status",     "essHeartbeatFaultCnt",         ival);
//         amap["essHeartbeatAlarmCnt"]     = vm->setLinkVal(vmap, "ess",
//         "/status",     "essHeartbeatAlarmCnt",         ival);
//         amap["essHeartbeatInit"]         = vm->setLinkVal(vmap, "ess",
//         "/status",     "essHeartbeatInit",             ival);
//         amap["essHeartbeatFaultTimeout"] = vm->setLinkVal(vmap, "ess",
//         "/config",     "essHeartbeatFaultTimeout",     toFault);
//         amap["essHeartbeatAlarmTimeout"] = vm->setLinkVal(vmap, "ess",
//         "/config",     "essHeartbeatAlarmTimeout",     toAlarm);
//         amap["essHeartbeatResetTimeout"] = vm->setLinkVal(vmap, "ess",
//         "/config",     "essHeartbeatResetTimeout",     toReset);
//         amap["essHeartbeatHoldTimeout"]  = vm->setLinkVal(vmap, "ess",
//         "/config",     "essHeartbeatHoldTimeout",      toHold);

//         if(am->am)
//         {
//             amap["amHeartbeatFaultCnt"]  = vm->setLinkVal(vmap,
//             am->am->name.c_str(), "/status",    "HeartbeatFaultCnt", ival);
//             amap["amHeartbeatAlarmCnt"]  = vm->setLinkVal(vmap,
//             am->am->name.c_str(), "/status",    "HeartbeatAlarmCnt", ival);
//             amap["amHeartbeatInit"]      = vm->setLinkVal(vmap,
//             am->am->name.c_str(), "/status",    "HeartbeatInit", ival);
//         }

//         amap["HeartbeatFaultCnt"]        = vm->setLinkVal(vmap, aname,
//         "/status",     "HeartbeatFaultCnt",            ival);
//         amap["HeartbeatAlarmCnt"]        = vm->setLinkVal(vmap, aname,
//         "/status",     "HeartbeatAlarmCnt",            ival);
//         amap["HeartbeatInit"]            = vm->setLinkVal(vmap, aname,
//         "/status",     "HeartbeatInit",                ival);
//         amap["HeartbeatState"]           = vm->setLinkVal(vmap, aname,
//         "/status",     "HeartbeatState",               cval);
//         amap["BypassHeartbeat"]          = vm->setLinkVal(vmap, aname,
//         "/config",     "BypassHeartbeat",              bval);
//         amap["HeartbeatStateNum"]        = vm->setLinkVal(vmap, aname,
//         "/status",     "HeartbeatStateNum",            ival);
//         amap["HeartbeatOK"]              = vm->setLinkVal(vmap, aname,
//         "/status",     "HeartbeatOK",                  bval);
...
//             amap["Heartbeat"]     ->setParam("rdFault", toFault);
//             // time remaining before fault amap["Heartbeat"]
//             ->setParam("rdAlarm", toAlarm);                      // time
//             reamining before alarm amap["Heartbeat"]
//             ->setParam("rdReset", toReset);                      // time
//             remaining before reset
//             //amap["Heartbeat"]     ->setParam("rdHold", toHold);
//             // time to wait before no change test amap["Heartbeat"]
//             ->setParam("tLast", dval);                         // time when
//             last to event was seen
...
//     int lastHeartbeat    =
//     amap["Heartbeat"]->getiParam("lastHeartbeat");//amap["lastHeartBeat"]->getiVal();
//     // are we the ess_controller
//     if(!am->am)
//     {
//         //bool initSeen =             amap["Heartbeat"]
//         ->getbParam("initSeen");
...
//         {
//             asset_manager * amc = ix.second;
//             CheckAmHeartbeat(vmap, amc->amap, amc->name.c_str(), p_fims,
//             amc); icnt++;
//         }
...
//         int lastHbAlarms =
//         amap["essHeartbeatAlarmCnt"]->getiParam("lastHbAlarms"); int
//         lastHbFaults =
//         amap["essHeartbeatFaultCnt"]->getiParam("lastHbFaults");
...
//             else
//             {
//                 FPS_ERROR_PRINT("%s >> %d essHbFaults cleared at time
//                 %2.3f\n", __func__, essHbFaults, tNow);
//             }
//         }
...
//             else
//             {
//                 FPS_ERROR_PRINT("%s >> %d essHbAlarms cleared at time
//                 %2.3f\n", __func__, essHbAlarms, tNow);
//             }
//         }
...
//     // If we are in the init state wait for comms to start count down reset
//     time if (currentHeartbeat == initHeartbeat)
//     {
//         // if not toally set up yet then quit this pass
...
//         //if(0)FPS_ERROR_PRINT("%s >>  ts  change for %s from [%s] to [%s]
//         \n", __func__, aname,
//         lastHeartbeat?lastHeartbeat:"no last Value", tval1) if
//         (currentHeartbeat != lastHeartbeat)
//         {

//             if(0)FPS_ERROR_PRINT("%s >> %s Heartbeat change detected,  from
//             [%d] to [%d] tNow %2.3f seenHB [%s]\n"
//                 , __func__, aname, currentHeartbeat,
//                 lastHeartbeat, tNow, seenHB?"true":"false");

//             amap["Heartbeat"]     ->setParam("lastHeartbeat",
//             currentHeartbeat);
...
//                 {
//                     if(0)FPS_ERROR_PRINT("%s >> %s Heartbeat stall detected
//                     tNow %2.3f seebTime %2.3f .stalll time %2.3f toHold %2.3f
//                     \n"
//                             , __func__, aname, tNow,
//                             HBseenTime, (tNow - HBseenTime), toHold);
...
//                 if(seenFault)
//                 {
//                     if(1)FPS_ERROR_PRINT("%s >>  Heartbeat fault for  %s
//                     cleared at %2.3f\n", __func__,
//                     aname, tNow); amap["Heartbeat"]
//                     ->setParam("seenFault",false); seenFault = false;
//                 }
//                 if(seenAlarm)
//                 {
//                     if(1)FPS_ERROR_PRINT("%s >>  Heartbeat Alarm for  %s
//                     cleared at %2.3f\n", __func__,
//                     aname, tNow); amap["Heartbeat"]
//                     ->setParam("seenAlarm",false); seenAlarm = false;
//                 }
...
//                 if(1)FPS_ERROR_PRINT("%s >>  Heartbeat OK for  %s at
//                 %2.3f\n", __func__, aname, tNow); ival =
//                 Asset_Ok; // seen Heartbeat change amap["HeartbeatStateNum"]
//                 ->setVal(ival); ival = 0;
//                 amap["HeartbeatInit"]->setVal(ival);
//                 char *tval;
//                 asprintf(&tval," Hb OK last set  Alarm %3.2f max %3.2f",
...
//             if(0)FPS_ERROR_PRINT("%s >>  Heartbeat stall for  %s at %2.3f
//             rdFault %2.3f rdAlarm %2.3f HBOk [%s] seenHB [%s] tDiff %2.3f \n"
//                                         , __func__,
//                                         aname, tNow, rdFault, rdAlarm,
//                                         amap["HeartbeaetOK"]?"true":"false",
...
//                 if(1)FPS_ERROR_PRINT("%s >>  Heartbeat  Fault  for %s at
//                 %2.3f \n", __func__, aname, tNow); char
//                 *tval; asprintf(&tval," Hb Fault last set Alarm %3.2f max
...
//                 int totalHbFaults =
//                 amap["Heartbeat"]->getiParam("totalHbFaults");
//                 totalHbFaults++;
...
//                 char *tval;
//                 asprintf(&tval,"Hb Alarm last set Alarm %3.2f max %3.2f",
//                 toAlarm, toFault); if(tval)
//                 {
//                     amap["HeartbeatState"]->setVal(tval);
//                     free((void *)tval);
...
//                 amap["Heartbeat"]->setParam("seenOk", false);
//                 int totalHbAlarms =
//                 amap["Heartbeat"]->getiParam("totalHbAlarms");
//                 totalHbAlarms++;
...
//             else
//             {
//                 if(0)FPS_ERROR_PRINT("%s >> Hb for [%s] [%s] Stalled at %2.3f
//                 Fault %2.3f Alarm %2.3f \n"
//                         , __func__
//                         , aname
...
// int CheckTimestamp(varsmap &vmap, varmap &amap, const char* aname, fims*
// p_fims, asset_manager*am)
// {
//     //double dval = 0.0;
//     int ival = 0;
...
//         amap["essTimestampFaultCnt"]       = vm->setLinkVal(vmap, "ess",
//         "/status",     "essTimestampFaultCnt",     ival);
//         amap["essTimestampAlarmCnt"]       = vm->setLinkVal(vmap, "ess",
//         "/status",     "essTimestampAlarmCnt",     ival);
//         amap["essTimestampInit"]         = vm->setLinkVal(vmap, "ess",
//         "/status",     "essTimestampInit",     ival);
//         amap["essTimestampFaultTimeout"] = vm->setLinkVal(vmap, "ess",
//         "/config",     "essTimestampFaultTimeout",     toFault);
//         amap["essTimestampAlarmTimeout"] = vm->setLinkVal(vmap, "ess",
//         "/config",     "essTimestampAlarmTimeout",     toAlarm);
//         amap["essTimestampResetTimeout"] = vm->setLinkVal(vmap, "ess",
//         "/config",     "essTimestampResetTimeout",     toReset);
//         amap["essTimestampHoldTimeout"]  = vm->setLinkVal(vmap, "ess",
//         "/config",     "essTimestampHoldTimeout",      toHold);

//         if(am->am)
//         {
//             amap["amTimestampFaultCnt"]  = vm->setLinkVal(vmap,
//             am->am->name.c_str(), "/status",    "TimestampFaultCnt", ival);
//             amap["amTimestampAlarmCnt"]  = vm->setLinkVal(vmap,
//             am->am->name.c_str(), "/status",    "TimestampAlarmCnt", ival);
//             amap["amTimestampInit"]    = vm->setLinkVal(vmap,
//             am->am->name.c_str(), "/status",    "TimestampInit", ival);
//         }

//         amap["TimestampFaultCnt"]        = vm->setLinkVal(vmap, aname,
//         "/status",     "TimestampFaultCnt",       ival);
//         amap["TimestampAlarmCnt"]        = vm->setLinkVal(vmap, aname,
//         "/status",     "TimestampAlarmCnt",       ival);
//         amap["TimestampInit"]          = vm->setLinkVal(vmap, aname,
//         "/status",     "TimestampInit",         ival); amap["TimestampState"]
//         = vm->setLinkVal(vmap, aname,                "/status",
//         "TimestampState",        cval); amap["BypassTimestamp"]        =
//         vm->setLinkVal(vmap, aname,                "/config",
//         "BypassTimestamp",       bval); amap["AssetState"]         =
//         vm->setLinkVal(vmap, aname,                "/status",
//         "AssetState",        ival); amap["TimestampStateNum"]      =
//         vm->setLinkVal(vmap, aname,                "/status",
//         "TimestampStateNum",      ival);
...
//             // time to wait before no change test amap["Timestamp"]
//             ->setParam("tLast", dval);                         // time when
//             last to event was seen
...
//         int lastTsAlarms =
//         amap["essTimestampAlarmCnt"]->getiParam("lastTsAlarms"); int
//         lastTsFaults =
//         amap["essTimestampFaultCnt"]->getiParam("lastTsFaults");
...
//                 if(1)FPS_ERROR_PRINT("%s >>  Timestamp OK for  %s at
//                 %2.3f\n", __func__, aname, tNow); ival =
//                 Asset_Ok; // seen Timestamp change amap["TimestampStateNum"]
//                 ->setVal(ival); ival = 0;
...
//             else
//             {
//                 if(0)FPS_ERROR_PRINT("%s >> Ts for [%s] [%s] Stalled at %2.3f
//                 Fault %2.3f Alarm %2.3f \n"
//                         , __func__
//                         , aname
//                         , amap["Timestamp"]->getcVal()
...
int CheckPCSStatus(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, asset_manager* am)
{
    ...
    if (reload < 2)
    {
        ...
        // amap["AssetState"]             = vm->setLinkVal(vmap, aname,
        // "/status",     "AssetState",             ival);
        ...
        // reset reload
        ival = 2;
        amap["CheckPCSStatus"]->setVal(ival);
    }
...
int CheckBMSStatus(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, asset_manager* am)
{
    ...
    if (reload < 2)
    {
        ...
        // reset reload
        ival = 2;
        amap["CheckBmsStatus"]->setVal(ival);
    }
...
int CheckESSStatus(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, asset_manager* am)
{
    ...
    if (reload < 2)
    {
        ...
        // amap["essPCSStatusState"]          = vm->setLinkVal(vmap, aname,
        // "/status",     "essPCSStatusState",        cval);
        ...
        // amap["essBMSStatusState"]          = vm->setLinkVal(vmap, aname,
        // "/status",     "essBMSStatusState",        cval);
