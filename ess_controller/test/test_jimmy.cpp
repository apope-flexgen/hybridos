...
void dummyPub(asset_manager* am, const char* uri, const char* puri)
{
    ...
    // status\n",__func__, am->name.c_str()); pname
    // ="Unknown2"; res = "After pub";
...
bool run_ess_wakeup(asset_manager* am, int wakeup)
{
    ...
    if (wakeup == WAKE_LEVEL1)
    {
        ...
        if (1)
        {
            ...
            {
                double dval = 45.0;
                assetVar* av = am->amap["mbmu_avg_cell_voltage"] = vm->setLinkVal(*am->vmap, "ess", "/status",
                                                                                  "mbmu_avg_cell_voltage", dval);
                av->am = am;
                int ival = 10;
            ...
            if (1)
                HandleAssetHeartbeat(*am->vmap, am->amap, am->name.c_str(), am->p_fims, am);
 ...
 * alarm if the bms is sending out fault values, and proceeds to shutdown the
 * system due to fault
 *
            ...
            if (iy.second->type == assetVar::ASTRING)
            {
...

                {
                    ...
                    if (iy.first == "bms")
                    {
                        ...
                        // vm->setAmFunc(vmap, "bms", "CheckMonitorVar", aname, ass_man,
                    ...
                    else
                    {
                        ...
                        // vm->setAmFunc(vmap, "pcs", "CheckMonitorVar", aname, ass_man,
                        // (void*)&CheckMonitorVar);
                    }
