...
// something like this......
// char * valuestr = "{\"test_int\":
// 123,\"test_float\":456.78,\"test_string\":\"some string thing\" }";  char *
// resstr = "{\"test_int\": 123,\"test_float\":456.78,\"test_string\":\"some
// string thing\" }";  cJSON * valuecj = cJSON_Parse(valuestr);
// EXPECT_STREQ(restr, vm->setValfromCJ(vmap,"/components/vmap_test",
// nullptr,valuecj);
// );
...
int main_test_vmap0(int argc, char* argv[])
{
    ...
    int rc;
    rc = vm.testRes(" Test 1", vmap, "set", "/components/vmap_test",
                    "{\"test_int\": "
                    "123,\"test_float\":456.78,\"test_string\":\"some string "
                    "thing\" }",
                    "{\"test_int\":123,\"test_float\":456.78,\"test_string\":"
                    "\"some string thing\"}");
    rc = vm.testRes(" Test 1a", vmap, "set", "/components/vmap_test",
                    "{\"test_int\":123,\"test_float\":456.78,\"test_string2\":{"
                    "\"value\":\"some string value\"}}",
                    "{\"test_int\":123,\"test_float\":456.78,\"test_string2\":{"
                    "\"value\":\"some string value\"}}");

    rc = vm.testRes(" Test 2", vmap, "set", "/system/status",
                    "{\"status\": \"standby\",\"soc\":100,\"active_current_setpoint\":2000 }",
                    "{\"status\":\"standby\",\"soc\":100,\"active_current_setpoint\":2000}");

    rc = vm.testRes(" Test 3", vmap, "get", "/system/status", nullptr,
                    "{\"active_current_setpoint\":{\"value\":2000},\"soc\":{"
                    "\"value\":100},\"status\":{\"value\":\"standby\"}}");

    rc = vm.testRes(" Test 4", vmap, "get", "/system/status/soc", nullptr, "100");
    ...
    // rc = 0;
    if (0 && rc == 1)
        printf("vmap at end opts 0 tNow %2.3f\n%s\n", tNow1, res);
    ...
    printf(
        " Temps \n package %2.5f core0 %2.5f core1 %2.5f core2 %2.5f core3 "
        "%2.5f \n",
        package, core0, core1, core2, core3);
    printf(" Memory \n currReal  %d peakReal  %d currVirt %d peakVirt %d \n", currRealMem, currVirtMem, peakRealMem,
           peakVirtMem);
    return 0;
}
