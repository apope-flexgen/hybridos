...
//        return 0;
    ...
    am->p_fims = p_fims;
    //int assetVar::sendEvent(const char* source, int severity, const char *msg,...)
    rc = av->sendEvent("TestEvent", 2, "This a test event from [%s:%s] at time %2.3f"
        , av->comp.c_str()
        , av->name.c_str()
        , vm.get_time_dbl()
        );
    ...
    while(!done)
    {
        ...
        else
        {
           ...
           rc = av->sendEvent("TestEvent", 2, "This a test event %d  [%s:%s] at time %2.3f"
            , count
            , av->comp.c_str()
            , av->name.c_str()
            , vm.get_time_dbl()
