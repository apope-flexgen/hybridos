...
void showResult(VarMapUtils*vm ,varsmap &vmap, const char* testId, const char* res, assetVar* av, double tval)
{
    ...
    printf("[%s] Set Value %2.3f changedReset   [%5s] expect [%5s] last %2.6f this %2.6f dbV %2.6f\n\n"
        , testId
        , tval
        , bval?"true":"false"
        , res
...
int main_test_deadband(int argc, char *argv[])
{
    ...
    assetVar* DeadbandTestD  = 
        vm.setVal(vmap,"/system/test","DeadbandTestD",dval);
    ...
    showResult(&vm ,vmap, "Test 3", "false", DeadbandTestD, dval + 0.150);  // below deadband level
