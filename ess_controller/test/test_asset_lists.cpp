 ...
 * The key goal is to get the system to load aan asset var(s) or some var params
 on demand
 * For example set  mode gridfollow then load a bunch of variables from a file
 (or a DB)
 * First we review some of the list uses in the current development model of the
 ess_controller
 ...
 * These populate vecs with lists of strings based on the contents of
 /config/ess/Pubs  , /config/ess/Subs
 ...
 *
 * "/config/ess": {
 *   "Subs": "/ess /components, /assets, /params, /status, /controls,
 /variables, /config, /tests, /default, /reload, /misc2 ",
 *   "Pubs": "/status/ess, /variables/ess, /config/ess, /params/ess ",
 *   "BlockSets": "/status/ess, /status/bms, /status/bms_1, /status/bms_2 "
 * },
 * The vecmap will contain a list of strings associated with the key word (s)
 Subs, Pubs, BlockSets etc
 ...
 *
 * here is it working
 * getListStr >> recovered [Subs] as [/ess /components, /assets, /params,
 /status, /controls, /variables, /config, /tests, /default, /reload, /misc2 ]
 ...
 *
 *  vLists
 *   These are a cool way to sent a block of sets or pubs to Fims from inside
 func.
 * The selected vars  can be sent to their default locations (defined by the
 comp element) or to an optional  different location(and :var)
 * They are mini varsmaps and can be created and deleted.
 *
 *
 * varsmap *vlist = am->vm->createVlist();
 * am->vm->addVlist(vlist,
 amap["lastActiveCurrentSetpoint"],"/components/pcr/pcr_1:lastSetpoint");
 ...
 * This was created to maintain the order of a config file so that it can be
 used for a consistent UI presentation.
 * it is saved in varsmap as "_assetList/uri"
 *     contains the order of assetVars for publish.
 * its a "normal" assetVar with a value (bool at the moment) with a special
 component  an assetlist type
 *
 * typedef std::vector<assetVar*>  assetlist;
 *
 * When parsing the config file, the assetList iscreated as a named collection
 of assetVars.
 * When wanting to publish the list we just simply run through the list and
 produce the cjson objects.
 *
 ...
 *                   do
 *                   {
 *                       av = alist->avAt(ix++);  // returns nullptr when the
 list is done.
 *                       if(av) av->showvarCJ(cji, opts);
 *                   } while(av);
 *               }
 * currently any "get" operation that just includes a uri will in fact first
 look for an asset list to define
 * the list of assetVars to be presented.
 * This is, in effect, a viable interface.
 *
 * So how do we make an assetList
 *
 * There is a VarMapUtils function for this. The uri can be any string. but they
 need to be unique.
 *
 ...
 *   }
 *
 * So once you have tour list simply add assets to it. They will be placed in
 the cJSON file in order.
 *
 *   alist->add(av);
 *
 * A getMapsCj with the created uri   is all you need
 *
 * cJSON* getMapsCj(varsmap &vmap , const char* uri = nullptr, const char* var =
 nullptr, int opts = 0)
 *
 *
 * The other use for asset lists is to collect assetVars suitable to drop into
 or read from a file or DB.
 * Ideally this will done in a config file but it could be in a Func.
 * This config file option is the best but we have to make it so that the assets
 ...
 *
 * "/config/bms/bms_1": {
 *    "/asset/bms/bms_1/MaxLimits": "/asset/bms/bms_1:MaxVoltageLimit,
 /asset/bms/bms_1:MaxCurrentLimit ..."
 *   "AssetLists": "/asset/bms/bms_1/MaxLimits,
 /asset/bms/bms_1/DischargeLimits",
 *   "BlockSets": "/status/ess, /status/bms, /status/bms_1, /status/bms_2 "
 * },

 * We use the "AssetLists" keyword to get a list of the lists. then process the
 lists specified from that string
 * Lets simply put the string into the assetList object at this time.
 * The first time the assetList is used it will translate  the string into the
 actual assetVars.
 * This will allow late runtime binding after the config is all set up.
 * Any vars in the list not in the config wil have to be created. But what is
 the type for those.
 * We'll  have an undefined type perhaps and then set the type on the first
 value set.
 *
 ...
 * This will be performed by a set on a regular assetVar with the asset_instance
 as the string.
...
int TestEssStart(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* av)
{
    ...
    if (StartCmd > StartLimit)
    {
        ...
        // vmp->addVlist(vlist, amap["EssStartCmd"]);  // this would cause a tight
        // loop
...
//                        \"amap\":\"ess\",\"func\":\"readConfigFile\","
//                        "\"configType\":\"PowerConfig\",\"onOk\":\"/status/ess:PowerConfigOK\",\"onErr\":
//                        \"/status/ess:PowerConfigErr\"}"
int readConfigFile(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* av)
{
    ...
    if (reload < 2)
    {
        ...
        {
            ival = 0;
            amap["readConfigProfile"]->setVal(tval);
        }
        ival = 2;
        readConfigFile->setVal(ival);
    }
    int ret = 0;
    if (av->aVal->valuestring && av->abf && av->am && av->am->vm)
    {
        ...
        {
            if (1)
                FPS_ERROR_PRINT("%s >> attempting to read config file [%s]\n", __func__, aload);
            // now load
...
int saveConfigFile(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* av)
{
    ...
    if (reload < 2)
    {
        ...
        {
            ival = 0;
            amap["saveConfigProfile"]->setVal(tval);
        }
        ival = 2;
        saveConfigFile->setVal(ival);
    }
    int ret = 0;
    if (av->aVal->valuestring && av->abf && av->am && av->am->vm)
    {
        ...
        if (aload)
        {
            ...
            cJSON_Delete(cjbm);
            if (1)
                FPS_ERROR_PRINT("%s >> attempting to save config file [%s]\n", __func__, aload);
            free((void*)aload);
            free((void*)asname);
            free((void*)sname);
        }
    ...
    {
        if (1)
            FPS_ERROR_PRINT("%s >> error with setup av->abf %p av->am %p av->am->vm %p\n", __func__, av->abf, av->am,
                            av->am ? av->am->vm : 0);
        ret = 1;
    }
...
// //  vm->setFunc(vmap, "dcr", "run_asset_wakeup" , (void*)
// &run_bms_asset_wakeup);
...
// int getAssetListVersion(varsmap &vmap, const char* alistname, const char*
// alistVersion)
// {
//     char* aload;
...
// sets up the internal structure for an assetList either from a config string
// or a template file aname = "ess" int setUpAssetList(varsmap &vmap, const
// char* aname, const char* alistname, const char* alistVersion, asset_manager*
// am)
// {
//     int ccnt = 0;
//     vecmap vecs;  // may not be used
//     char** assets  = vm.getList(vecs, vmap, am->amap, am->name.c_str(),
//     alistname, ccnt);
//     // if ccnt == 0 then look for a template file

//     if(1)FPS_ERROR_PRINT(" %s >> seeking [%s] Found ccnt as %d\n",
//     __func__, alistname, ccnt); if(ccnt > 0)
//     {
...
int main(int argc, char* argv[])
{
    ...
    // Hack to set up the am in the av object not normally needed in a "normal"
    // system config cJSON* cj2 = cJSON_Parse("{\"value\":\"InitValcJ\"}");
    ...
    const char* var1 =
        ...
        "\"amap\":\"ess\",\"func\":\"TestEssStart\","
        "\"onOK\":\"/asset/ess/controls/startOK\",\"onErr\": "
        "\"/asset/ess/controls/startErr\"}"
        "]"
        "}"
        "}"
        "}"
    ...
    const char* var2 =
        "{"
        "\"/config/ess:ChargeLimits\": \"/config/ess:MaxVoltageLimit, "
        "/config/ess:MaxCurrentLimit, /config/ess:MinCurrentLimit, "
        "/config/ess:MinVoltageLimit\","
        "\"/config/ess:DischargeLimits\": \"/config/ess:MaxVoltageLimit, "
        "/config/ess:MaxCurrentLimit, /config/ess:MinCurrentLimit, "
        "/config/ess:MinVoltageLimit\","
        "\"/config/ess:StartConfig\": \"/config/ess:MaxVoltageLimit, "
        "/config/ess:MaxCurrentLimit, /config/ess:MinCurrentLimit, "
        "/config/ess:MinVoltageLimit\","

        "\"AssetLists\": \"/config/ess:ChargeLimits, "
        "/config/ess:DischargeLimits, /config/ess:StartConfig\""
        "},";

    const char* var4 =
        "{\"readConfig\":{\"value\":\"GridFollow\","
        "\"actions\":{"
        "\"onSet\":{"
        "\"func\":["
        "{ \"enable\":\"/assets/ess:run_ok\", "
        "\"amap\":\"ess\",\"func\":\"readConfigFile\","
        "\"configType\":\"PowerConfig\",\"onOK\":\"/status/"
        "ess:PowerConfigOK\",\"onErr\": \"/status/ess:PowerConfigErr\"}"
        "]"
        "}"
        "}"
        "}"
    ...
    const char* var5 =
        "{\"saveConfig\":{\"value\":\"GridFollow\","
        "\"actions\":{"
        "\"onSet\":{"
        "\"func\":["
        "{ \"enable\":\"/assets/ess:run_ok\", "
        "\"amap\":\"ess\",\"func\":\"saveConfigFile\","
        "\"configType\":\"/config/ess:StartConfig\",\"onOK\":\"/status/"
        "ess:StartConfigOK\",\"onErr\": \"/status/ess:StartConfigErr\"}"
        "]"
        "}"
        "}"
        "}"
    ...
    const char* rep1 =
        ...
        "[{\"enable\":\"/assets/"
        "ess:start_enable\",\"amap\":\"ess\",\"func\":\"TestEssStart\","
        "\"onOK\":\"/asset/ess/controls/startOK\",\"onErr\":\"/asset/ess/"
        "controls/startErr\"}]}}}}";

    const char* rep3 =
        "{\"/assets/ess\":{"
        "\"readConfig\":{\"value\":\"GridFollow\",\"actions\":{\"onSet\":{"
        "\"func\":"
        "[{"
        "\"amap\":\"ess\","
        ...
        "}]}}},"
        "\"start_stop\":{\"value\":0,\"actions\":{\"onSet\":{\"func\":"
        "[{\"amap\":\"ess\",\"enable\":\"/assets/"
        "ess:start_enable\",\"func\":\"TestEssStart\","
        "\"onErr\":\"/asset/ess/controls/startErr\",\"onOK\":\"/asset/ess/"
        "controls/startOK\"}]}}}}}";
    ...
    const char* rep4 =
        ...
        "\"func\":"
        "[{\"enable\":\"/assets/"
        "ess:run_ok\",\"amap\":\"ess\",\"func\":\"readConfigFile\","
        "\"configType\":\"PowerConfig\","
        "\"onOK\":\"/status/ess:PowerConfigOK\",\"onErr\":\"/status/"
        "ess:PowerConfigErr\"}]}}}}";

    const char* rep5 =
        "{\"saveConfig\":{\"value\":\"GridFollow\",\"actions\":{\"onSet\":{"
        "\"func\":"
        "[{\"enable\":\"/assets/"
        "ess:run_ok\",\"amap\":\"ess\",\"func\":\"saveConfigFile\","
        "\"configType\":\"StartConfig\","
        "\"onOK\":\"/status/ess:StartConfigOK\",\"onErr\":\"/status/"
        "ess:StartConfigErr\"}]}}}}";

    rc = vm.testRes(" Test 1", vmap, "set", "/assets/ess", var1, rep1, ess_man, nullptr);

    rc = vm.testRes(" Test 1.1", vmap, "set", "/assets/ess", var4, rep4, ess_man, nullptr);

    rc = vm.testRes(" Test 2", vmap, "get", "/assets/ess", nullptr, rep3);

    rc = vm.testRes(" Test 2.1", vmap, "set", "/assets/ess", var5, rep5, ess_man, nullptr);

    // this is broken we think
    rc = vm.testRes(" Test 3.0", vmap, "set", "/assets/ess:start_stop", "0", "{\"start_stop\":0}");
    rc = vm.testRes(" Test 3.1", vmap, "set", "/assets/ess", "{\"start_stop\":1000}", "{\"start_stop\":1000}");
    rc = vm.testRes(" Test 3.2", vmap, "set", "/assets/ess", "{\"start_stop\":2000}", "{\"start_stop\":2000}");

    rc = vm.testRes(" Test 4", vmap, "set", "/assets/ess", "{\"start_stop\":13000}", "{\"start_stop\":13000}");

    rc = vm.testRes(" Test 5", vmap, "set", "/assets/ess", "{\"readConfig\":\"gridFollow\"}",
                    "{\"readConfig\":\"gridFollow\"}");

    rc = vm.testRes(" Test 6", vmap, "set", "/assets/ess", "{\"saveConfig\":\"StartUp\"}",
                    "{\"saveConfig\":\"StartUp\"}");
    ...
    free((void*)res);
    cJSON_Delete(cj);
    rc = vm.testRes(" Test 10", vmap, "set", "/config/ess", var2, nullptr, ess_man, nullptr);
