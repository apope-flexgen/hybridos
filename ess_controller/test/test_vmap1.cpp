/*
 * vmap basic test
 * more advance use wher we pull a reference to a data point an run some
calculations on it
...
Thise will become unordered_maps sometime soon they are 10X faster but use 2X
space.
...
// int CheckAssetDisable(varsmap &vmap, varmap &amap, const char* aname, fims*
// p_fims, asset *am)
// {
...
//     double dval = 0.0;
//     int reload = am->vm->CheckReload(vmap, amap, aname,
//     __func__);
...
//         //int ival = 0;
//         bool bval = false;
//         //setAmapAi(am,  amap,          CheckAssetDisable,
//         am->name.c_str(),      /reload,       ival); setAmapAi(am,  amap,
//         DisableCmd,               am->name.c_str(),      /controls, ival);
//         setAmapAi(am,  amap,          EnableCmd,
//         am->name.c_str(),      /controls,     ival); setAmapAi(am,  amap,
//         EnableCnt,                am->name.c_str(),      /status,     ival);
//         setAmapAi(am,  amap,          DisableCnt,
//         am->name.c_str(),      /status,     ival); setAmapAi(am,  amap,
//         Enabledxx,                am->name.c_str(),      /controls, bval);
//         setAmapAi(am,  amap,          CheckAssetInit,
//         am->name.c_str(),      /status,     tNow); setAmapAi(am,  amap,
//         CheckAssetCmdRun,         am->name.c_str(),    /status,     tNow);
//         setAmapAi(am,  amap,          CheckAssetCmdRuns,
//         am->name.c_str(),    /status,     dval); setAmapAi(am,  amap,
//         DisableCmdRun,            am->name.c_str(),    /status,     tNow);
//         setAmapAi(am,  amap,          EnableCmdRun,
//         am->name.c_str(),    /status,     tNow); ival = 2;
