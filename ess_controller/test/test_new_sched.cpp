/*
 * using Phil's old schedule tester to move function calling out of json files
 * and into code base this code allows us to statically add functions into the
 * scheduler
 */
...
// this takes the assetVar and  creates the sched item and triggers the channel
// write things like repTime and runTime are left in place as params and used in
// the sched list
int add_sched_item(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* av)
{
    ...
    printf(
        "\n\n***** %s >> running for  av [%s:%s] am %p cj %p  aV [%s] value "
        "[%s]\n\n",
    ...
    if (avi && am)
    {
        ...
        as->setUp(avi->getcParam("id"), avi->name.c_str(), aV, "run_some_func", avi->getdParam("refTime"),
                  avi->getdParam("runTime"), avi->getdParam("repTime"), avi->getdParam("endTime"),
                  avi->getcParam("targ"));
        as->func = (char*)vm.getFunc(*am->vmap, "ess", "run_some_func");

        // TODO phase out the function here... just send the time to the scheduled
        // assetVar  let its actions list take care of things.
        ...
        if (am->wakeUpChan)
            am->wakeUpChan->put((int)(tNow * 1000));
        if (0)
        {
...
int setup_sched_item_actions(varsmap& vmap, const char* uri, const char* name, cJSON* cja)
{
    ...
    asprintf(&rbuf,
             ...
             "{\"%s\":{\"value\":0.0, \"actions\":{\"onSet\":[{\"func\":%s}]}}}}",
             uri, name, acts);
...
int setup_sched_item(varsmap& vmap, const char* uri, const char* name, const char* id, const char* aname,
                     double runTime, double repTime)
{
    char* rbuf;
    // uri example /schedule/ess
    asprintf(&rbuf,
             ...
             "\"aname\":\"%s\","
             "\"runTime\":%f,\"repTime\":%f}}}",
             uri, name, id, aname, runTime, repTime);
    fims_message* msg = vm.bufferToFims((const char*)rbuf);
    if (msg)
    {
        printf(
            "fims message:\nmethod: [%s] | uri: [%s] | replyto: [%s] | body: [%s]  "
            "| fims pfrags [1] [%s]\n",
            msg->method ? msg->method : "no method", msg->uri ? msg->uri : "no uri",
            msg->replyto ? msg->replyto : "no replyto",
            msg->body ? msg->body : "no body"
            // , msg->process_name ? msg->process_name : "no pname"         these
            // dont work and cause faults -> process_name: [%s] | username: [%s] |
...
int setup_sched_function(varsmap& vmap, const char* uri, const char* oper, const char* func, asset_manager* am)
{
    ...
    asprintf(&rbuf,
             "{\"method\":\"set\",\"uri\":\"%s\",\"body\": "
             "{\"%s\":{\"value\":\"test_string\",\"actions\":{\"onSet\":[{"
             "\"func\":[{\"func\":\"%s\",\"amap\":\"ess\"}]}]}}}}",
             uri, oper, func);
    ...
    if (msg)  // prints msg
    {
        printf(
            "fims message:\nmethod: [%s] | uri: [%s] | replyto: [%s] | body: [%s]  "
            "| process_name: [%s] | username: [%s] | fims pfrags [1] [%s]\n",
            msg->method ? msg->method : "no method", msg->uri ? msg->uri : "no uri",
            msg->replyto  //      ? msg->replyto : "no replyto"
            ,
            msg->body ? msg->body : "no body", msg->process_name ? msg->process_name : "no pname",
            msg->username ? msg->username : "no username", msg->nfrags > 0 ? msg->pfrags[1] : "no pfrags");
    }
    else
    {
        printf(" no message from \n>>>%s<<<\n", rbuf);
...
int asset_manager::getSchedDelay()
{
    ...
    if (schreqs.size() > 0)
    {
            ...
            if (av)
            {
                ...
                if (av->gotParam("repTime"))
                {
                    if (av->getdParam("repTime") > 0.0)
                    {
                        ...
                        // addAmSchedReq(as);// new run_req(rn->t_shot+rn->t_rep,
                        // rn->t_rep,rn->fname,rn->args));
                        if (am->reqChan)
                        {
                            channel<schedItem*>* reqChan = (channel<schedItem*>*)am->reqChan;
                            reqChan->put(as);
        ...
        if (oneran)
            printf("Done running at %2.3f   ran %d\n", tNow, oneran);
    }
...
int runChans(asset_manager* am)  // TODO: check if this does all the same things
                                 // as regular ess_c when putting things on
                                 // reqChan
{
    running = 1;
    int delay = 1000;  // ms
    ...
    while (running)
    {
        ...
        if (am->reqChan)
        {  // was originally if(am->reqChan->get(as, false))

            // printf("Adding  Sched Request [%s]  at %2.3f\n", as->id, tNow); //
            // as->id creates a segfault because as is a nullptr
        ...
        if (am->fimsChan->get(msg, false))
        {
            ...
            if (msg)
            {
                ...
                bool runFims = true;
                if (runFims)
                    vm.runFimsMsg(vmap, msg, am->p_fims);  // am->p_fims);//fims* p_fims)
...
void setupControls(scheduler* sc, varsmap* vmap, schlist* rreqs, asset_manager* am, fims* p_fims)
{
    ...
    if (!fimsav)
    {
        ...
        (*vmap)[av->comp][av->name] = av;
        FPS_PRINT_INFO("Created Fims var [{}:{}]", av->comp, av->name);
        fimsav = av;
    }
    ...
    if (!runav)
    {
        ...
        assetVar* av = new assetVar(my.Var, my.Uri, dval);
        (*vmap)[av->comp][av->name] = av;
        FPS_PRINT_INFO("Created Runvar var [{}:{}]", av->comp, av->name);
        runav = av;
    }
    ...
    if (!stopav)
    {
        ...
        assetVar* av = new assetVar(my.Var, my.Uri, dval);
        (*vmap)[av->comp][av->name] = av;
        FPS_PRINT_INFO("Created Stop var [{}:{}]", av->comp, av->name);
        stopav = av;
    }
...
void fimsThread(scheduler* sc, fims* p_fims)
{
    ...
    while (*sc->run)
    {
        ...
        if (fimsok)
        {
            ...
            if (0)
                FPS_PRINT_INFO("Fims Tick {} msg {} p_fims {}", tick, fmt::ptr(msg), fmt::ptr(p_fims));
            tick++;
        }
        if (msg)
        {
            ...
            {
                if (1)
                    FPS_PRINT_INFO("GET uri [{}]", msg->uri);
            }
            if (*sc->run)
            {
                sc->fimsChan.put(msg);
                sc->wakeChan.put(0);  // but this does not get serviced immediatey -> is
                                      // done in schedThread
    ...
    while (*sc->run)
    {
        ...
        if (sc->reqChan.get(si, false))
        {
            if (si)
            {
                ...
                if (0)
                    FPS_PRINT_INFO(
                        "Servicing Sched Request {} id [{}] ({})  uri [{}] "
                        "repTime {:2.3f}",
                        fmt::ptr(si), fmt::ptr(si->id), fmt::ptr(si->id), si->uri, si->repTime);
                ...
                if (si)
                {
                    ...
                    if (oldsi)
                    {
                        FPS_PRINT_INFO("this is a replacement schedItem {} id {} uri {}", fmt::ptr(oldsi),
                                       cstr{ oldsi->id }, cstr{ oldsi->uri });
                        oldsi->repTime = 0;
                        oldsi->endTime = 0.1;
                    ...
                    {
                        if (0)
                            FPS_PRINT_INFO("schedItem added {} id {} uri {}", fmt::ptr(si), cstr{ si->id },
                                           cstr{ si->uri });
        ...
        if (0)
            FPS_PRINT_INFO("new delay = {:2.6f}", ddelay);
        delay = ddelay;
...
int main_test_new_sched(int argc, char* argv[])  // most of the code here is
                                                 ...
                                                 // ess_controller
{
    ...
    {
        if (1)
            FPS_PRINT_INFO(" flex_man >> No Subs found in flex config");
        if (nsubkeys == 0)
        {
        ...
        if (1)
            FPS_PRINT_INFO(" flex_man >> p_fims [{}]  sched [{}]", fmt::ptr(p_fims), fmt::ptr(sched.p_fims));
    }
    else
    {
        if (1)
    ...
    // int setup_sched_item(varsmap& vmap, const char*name , const char* id, const
    // char*aname, double runTime , double reptime)
    ...
    // runChans(am);   // is this the same as starting fimsThread and schedThread?
    // -> this exists in scheduler.h (diff params) printf("\n\n runchans done
    // ...\n");
