...
bool run_bms_wakeup(asset_manager* am, int wakeup)
{
    ...
    if (0)FPS_ERROR_PRINT("%s %2.3f >>>>>>>>>>>>>>>>>>>>> running for %s Asset Manager wval %d \n", __func__, am->vm->get_time_dbl(), am->name.c_str(), wakeup);
    if (wakeup == 0)
    {
    ...
    for (auto ix : am->assetManMap)
    {
        ...
        {
            if (0)FPS_ERROR_PRINT("%s >>>>>>>>> %s Manager Loop NO Wakeup >>>>>>>>>>> running for Asset Manager [%s] \n", __func__, am->name.c_str(), ix.first.c_str());
        }
    ...
    //     //TODO ass->wakechan.put(wakeup); this will only work if the assets are threaded
    //     if (ass->run_wakeup)
...
bool run_bms_asset_wakeup(asset* am, int wakeup)
{
    ...
    if (0)FPS_ERROR_PRINT("%s >>>>>>>>>>>>>>>>>>>>> running for %s Asset wval %d \n", __func__, am->name.c_str(), wakeup);
        ...
        for (auto iy : ixa->second)
        {
            if (iy.second->type == assetVar::ASTRING)
            {
                ...
                else if (iy.first == "pcs")
                {
                    ...
                    // we should be able to do things like get status from the pcs_manager.
                    // first see it it unmaps correctly.
