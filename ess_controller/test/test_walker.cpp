...
// however the asset manager may want to tune this on behalf of one of the
// managed assets inwhich case the aname will be bms_1, bms_2 or what ever.
...
// Task from Phil:
/*
The second task entails reading input from the pcs modbus_client like
current_fault and setting a fault mode. The first task should detect the fault
mode and set the active power set point to 0.
*/

// FaultManager (if it will exist in the future) could override the level of all
// of the managers. Fault level = -1? FaultManager could take control of the
// other managers if the other managers aren't doing their jobs. Do faultMode
// logic here based on various factors, subject to change in the future:
// amap/aname will most likely be faultManager - which will call this.
// p_fims will subscribe to (or faultManager will) faultChannel to get the
// faultMessages Format of faultMessages will be: (?) How does hybridOS handle
// faults? How will they be handled in Phil's new system? Maybe make this a
// template function. Specialize based on which asset is calling it (bms vs.
// pcs)?
 ...
 * - If a fault is of higher severity than an asset can handle it passes it up
 * to its "manager." This needs to be added in the future to asset.h
 * - Init links for handling what variables to change and where to send fims
 * when handling faults
 * - fims channel that sends faults around (this is a direct line and ignores
 * normal channel "wakeup" rules, should immediately be handled depending on
 * severity)
 * -
 *
 * FaultTypes:
 * - Level 1 asset - bms_x or pcs_x)
 *      - cellDrawButEmpty (battery cell is empty but tried to draw power from
 * it), switch to another one
 *      - drawingTooMuchPower (trying to draw 1 million mW from a battery),
 *      - lose connection (look at heartbeat/something - need feedback from
 * system)
 * - Level 2 assetManager - bms_manager or pcs_manager)
 *      - batteryEmpty (bms_x's battery is out and needs to be charged,
 * bms_manager needs to draw from another bms_x entirely)
 * - Level 3 ess_controller - manager of managers)
 *      - A fire has occured! (panic!?)
 * - Level 4 fleet_manager - manager of managers of managers)? - Are these
 * emergencies?
 *      - No connection (lost ess_controller connection)
 *      - rack out of power entirely!
 *
 * What this is NOT:
 * - Emergency handler: handles fires and other odd emergencies that must be
 * immediately dealt with.
 * - Handles faults only: It does NOT identify them! That is for each asset to
 * determine individually
 *
 *
 * TODO:
 * - Change asset.h to get a reference to asset_manager so an asset can send
 * faults up the management chain.
 * - figure out how to setup a line to go up chain of command fluently. Is a
 * function needed?
 * - Figure out how to make this configurable? John wanted something that is
 * configurable, maybe have user-defined faultTypes? If so, this complicates
 * things.
 *      - Format could be: "faultTypesAndInstructions": {'faultType_x':
 * {instructions (needs to be constrained)}, ...}
 * -
 */
// vmp = regular old vmap, amap = regular old assetMap, aname = regular old
int HandleFault(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, asset* ass = nullptr,
                asset_manager* am = nullptr)
{
     ...
     *  - depth (value changes adds to depth - this is how far along the fault is
     * compared to the others) {subject to change}
     *  -
     *
     ...
     *      switch (faultType) {
     *          case 'faulttype_x':
     *              do faulttype_x stuff (vm.sendAssetVar(APSP, p_fims,
     * "destination"); // This is where we change data per fault Mode. Where to
     * go?) break;
     *          ...
     *          default:
     *              break; // handle unknown faults here - that or have type 'u'
     * and send it up to some manager immediately.
     *      }
     * } else {
    ...
    double dval;
    int rc = 0;
    char* faultMode = (char*)"\0";  // Change this to a string/char and use
                                    // switches. Maybe add configuration to it?
    ...
    amap["CurrentFault"] = vm.setLinkVal(vmap, "bms_1", "/status", "CurrentFault",
                                         faultMode);  // Change this to modbus_client:current fault
    assetVar* CF = amap["CurrentFault"];              // This comes from "modbus_client:
                                                      // current_fault". Could turn into
                                                      // something more specific.
    assetVar* APSP = amap["ActivePowerSetpoint"];
    // Reload might not be necessary for handling faults, there should be no need
    // to reload fault Handling: if (!CF || !APSP) //
    // {
    ...
    // {
    //     //reload = 0;
    //     amap["ActivePowerSetpoint"]      = vm.setLinkVal(vmap, "bms_1",
    //     "/status",    "ActivePowerSetpoint",            dval);
    //     amap["CurrentFault"]      = vm.setLinkVal(vmap, "bms_1", "/status",
    //     "CurrentFault",            faultMode); //Change this to
    //     modbus_client:current fault
    //     // amap["current_fault"]      = vm.setLinkVal(vmap, "modbus_client",
    //     "/uri?",    "current_fault",            faultMode);
    ...
    // Question: Is FaultMode going to have its own function/thread like this? How
    // will that be handled? Question: Where do I "send" the assetVar to in order
    // to get an actual change in the system? PCS?
    char* fm;
    fm = CF->getcVal();
    if (fm)
    {
        switch (fm[0])
        {
            case 'f':
                ...
                vm.sendAssetVar(APSP, p_fims, "/status/bms_1");  // This is where we change
                                                                 // data per fault Mode.
                                                                 // Where to go?
                rc++;
                break;
...
// Function that could be used to determine the type of fault that an asset is
// encountering. Might be unnecessary. No way to make this general enough for
// each asset type.
...
int main(int argc, char* argv[])
{
    ...
    // vm.addSub(dbsubs, "power_electronics_pcs_1", ccnt, scnt);  // added
    // power_electronics sub here - Jimmy vm.showSubs(dbsubs, "bms", ccnt, scnt);
    ...
    while (bms_man->wakechan.get(wakeup, true))
    {
        ...
        {
            if (bms_debug)
                std::cout << " BMS >> process another channel\n";
            // break;
        }
        ///  got some big plans for messages and wakeups
        // we really have two events (poll : 1 pub :2 )
        // but we could (will ) set up more event groups to force an immediate
        // response.
        //
        ...
        if (wakeup == WAKE_LEVEL1)  // POLL EVENT
        {
            ...
            HandleFault(vmap, *amap, "bms", p_fims, nullptr,
                        bms_man);  // Need to fix segmentation fault - modify jsons.
        }

        if (wakeup == WAKE_LEVEL_PUB)  // PUB EVENT
        {
            if (bms_debug)
                std::cout << " BMS >> publish status\n";
        ...
        // we cant issue the event "send" yet but when this all runs as a thread
        // inside the bms_manager object we'll have direct access to the wakeup and
        // message channels then a fims set to /controls/bms_1:wakeup '{"value":3}'
        // will cause this to happen bms_man->wakechan->put(wakeval); once we do
        // this the assetVars will have to have locking but no big deal on that.

        // if(wakeup == 3) // HANDLESETPOINT REQUEST EVENT do it now dont wait for a
        // poll
        // {
        //     vm.setTime();
        ...
        //     HandleLoadRequest(vmap, *amap, "bms", p_fims);
        // }
        // if(wakeup == 4) // HANDLE TEST REQUEST EVENT do it now dont wait for a
        // poll
        // {
        //     //vm.setTime();
        ...
        if (bms_man->msgchan.get(item3, false))
        {
            sendTestMessage(p_fims, tnum);
            if (bms_debug)
                std::cout << " BMS >>item3-> data  " << item3 << " tnum :" << tnum << "\n";
            tnum++;
            sendTestMessage(p_fims, tnum);
            if (bms_debug)
                std::cout << " BMS >>item3-> data  " << item3 << " tnum :" << tnum << "\n";
        ...
        if (bms_man->fimschan.get(msg, false))
        {
            ...
            {
                std::cout << " BMS >>fims_msg method :" << msg->method << " uri :" << msg->uri << " body :"
                          << "NoBody"
                          << "\n";
            }
