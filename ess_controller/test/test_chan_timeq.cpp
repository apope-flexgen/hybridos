 ...
 * the timer thread can be woken up with a message to add a new event to the
 * timer list the new event may change the timeout and events
 */
    ...
    while (t_now < 200)
    {
        ...
        if (rreqs.size() > 0)
        {
            ...
            if (rr->t_shot <= t_now)
            {
                ...
                {
                    rr->t_shot += rr->t_rep;
                    add_rreq(rreqs, rr);  // new run_req(rn->t_shot+rn->t_rep,
...
int main_test_chan_timeeq(int argc, char* argv[])
{
    ...
    if (0)
        run(&r_data, timer_loop, &running, 1000, &wakechan);
    ...
    // the second also uses channels but that gets scheduling messages  Do this
    // every xyz seconds or di this once now or do this in .2 seconds. this is the
    // loop that will work out when to wake up the first loop. at last we get to
    // use our message channel.
    ...
    while (running)
    {
        ...
        {
            if (0)
                std::cout << " wakeup bflag true ival " << wakeup << "\n";
            // this is a true wake up so wake up the main scheduler
            // pop rhe first time off the list and have at it. This also sets the next
            // delay
        }
        else
        {
            if (0)
                std::cout << " wakeup bflag false ival " << wakeup << "\n";
        }
