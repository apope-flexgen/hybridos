...
int CheckEssStart(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* av)
{
    ...
    else
    {
        ...
        if (1)
            FPS_ERROR_PRINT(" %s >> Ok found assetBitField for number %d as %p\n", __func__, av->abNum, abf);
        ...
        {
            if (1)
                FPS_ERROR_PRINT(" %s >> Ok found assetFunc feature \"func\" as [%s] \n", __func__, af->valuestring);
        }
        ...
        if (av->extras && av->extras->featDict)
        {
            ...
            {
                if (1)
                    FPS_ERROR_PRINT("%s >> param name [%s]\n", __func__, x.first.c_str());
            }
        }
        else
        {
            if (1)
                FPS_ERROR_PRINT("%s >> no params found for  [%s]\n", __func__, av->name.c_str());
        }
    }
...
int main(int argc, char* argv[])
{
    ...
    const char* var1 =
        ...
        "\"amap\":\"bms\",\"testCheckArg\":234,"
        "\"func\":\"CheckEssStart\",\"onOK\":\"/controls/ess/"
        "startOK\",\"onErr\": \"/controls/ess/startErr\"},"
        "{ \"enable\":\"/controls/ess:start_enable\", "
        "\"amap\":\"ess\",\"testTestArg\":234,"
        "\"func\":\"TestEssStart\",\"onOK\":\"/controls/ess/"
        "startOK\",\"onErr\": \"/controls/ess/startErr\"}"
        "]"
        "}]"
        "}"
    ...
    const char* rep1 =
        "{\"start_stop\":{\"value\":0,\"params\":{\"param1\":1,\"param2\":\"p2\"}"
        ",\"actions\":{\"onSet\":[{\"func\":"
        "[{\"enable\":\"/controls/ess:start_enable\",\"amap\":"
        "\"bms\",\"testCheckArg\":234,\"func\":\"CheckEssStart\",\"onOK\":\"/"
        "controls/ess/startOK\",\"onErr\":\"/controls/ess/startErr\"},"
        "{\"enable\":\"/controls/ess:start_enable\",\"amap\":\"ess\","
        "\"testTestArg\":234,\"func\":\"TestEssStart\",\"onOK\":\"/controls/ess/"
        "startOK\",\"onErr\":\"/controls/ess/startErr\"}]}]}}}";
    ...
    rc = vm.testRes(" Test 1", vmap, "set", "/controls/ess", var1, rep1, ess_man, nullptr);

    rc = vm.testRes(" Test 2", vmap, "get", "/controls/ess", nullptr, rep3);

    // this is broken we think
    rc = vm.testRes(" Test 3.0", vmap, "set", "/cotrols/ess:start_stop", "0", "{\"start_stop\":0}");
    rc = vm.testRes(" Test 3.1", vmap, "set", "/controls/ess",
                    "{\"start_stop\":{\"value\":1000,\"params\":{\"maxVal\":"
                    "23456,\"minVal\":555}}}",
                    "{\"start_stop\":{\"value\":1000,\"params\":{\"maxVal\":"
                    "23456,\"minVal\":555}}}");
    rc = vm.testRes(" Test 3.2", vmap, "set", "/controls/ess", "{\"start_stop\":2000}", "{\"start_stop\":2000}");

    rc = vm.testRes(" Test 4", vmap, "set", "/controls/ess", "{\"start_stop\":9000}", "{\"start_stop\":9000}");
