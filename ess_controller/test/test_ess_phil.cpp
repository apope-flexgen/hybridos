    ...
    if(reload < 2)
    {
        ...
        if(reload == 0) // complete restart 
        {
            ...
            if(vmp->notMissing(amap, __func__, "HeartBeatLast") 
                && vmp->notMissing(amap, __func__, "HandleBMSHeartBeat")
                && vmp->notMissing(amap, __func__, "HeartBeat")
                )
            {
...
int oldHandlePower(varsmap &vmap, varmap &amap, fims* p_fims)
{
    ...
    //     amap["lastActivePowerSetpoint"]->setVal(  amap["ActivePowerSetpoint"]->getbVal());
    //     if (abs(amap["ActivePowerSetpoint"]->getbVal()) < amap["maxActivePower"]->getbVal())
    //     {
