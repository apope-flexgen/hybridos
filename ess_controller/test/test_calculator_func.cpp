    ...
    "every100mS": {    // assetvar
        ...
        "actions":{
            "onSet" [ { "exec" :[                        // special param functions
                ...
                {"func":"CheckMonitorVar", "Av":"/components/catl_mbmu_stat_r:bms_heartbeat",                 "enable": true,"aname":"bms"},
                {"func":"CheckMonitorVar", "Av":"/components/catl_mbmu_stat_r:bms_timestamp",                 "enable": true,"aname":"bms"}},   
...
* 2/ define a component function compFun   executed  on any writes to that component (read table)
...
int test_main(int argc, char* argv[])
{
    ...
    //asprintf(&rbuf,"{\"method\":\"set\",\"uri\":\"/assets/sungrow_ess_1\" ,\"body\":\"{\"close_dc_contactors\":{\"value\":true}}}");
    // asprintf(&rbuf,"{\"method\":\"set\",\"uri\":\"/assets/sungrow_ess_1\",\"body\":{\"Some body\"}}"); //{\"close_dc_contactors\":{\"value\":true}}}");
    ...
    vm.funMapp = &funMap;
// varmaputils .. thanks
    asset_manager* essm = setupEssManager(&vm, vmap, vecs, syscpVec, "ess", nullptr);
    asset_manager* bmsm = setupEssManager(&vm, vmap, vecs, syscpVec, "bms", nullptr);
    asset_manager* pcsm = setupEssManager(&vm, vmap, vecs, syscpVec, "bms", nullptr);

    
    vm.syscVec = &syscVec;
    vm.setFunc(vmap, "bms", "TestFunc1", (void*)&TestFunc1);
    // this is acop function and runs (with no config) whenver any var in /alarms/bms is written to 
...
//     bm->setVmap(&vmap);
    ...
    printf("asset test OK So Far essm %p bmsm %p pcsm %p\n", essm, bmsm, pcsm);
//     // TODO read the top level file
//     // std::vector<std::pair<std::string, std::string>> reps =
//     // {
//     //     { "@@DUMMY@@", "bss_1" }, { "@@IP_ADDR@@", "192.168.1.2" }, { "@@PORT@@", "502" }
//     // };
...
// OK this puts it into an optVec never to be seen again.....
// we need it added to the optDict someting to do with uiObject
    asprintf(&rbuf,
       "{\"method\":\"set\",\"uri\":\"/components/testOptVec\",\"body\":{\"usesOptVec\":"
       "{\"value\":true,\"stuff\":1234,"
       "\"options\":["
       "{\"name\":\"BmsState\"},"
       "{\"name\":\"BmsVal1\"},"
       "{\"name\":\"BmsVal2\"}"
       "]"
       "}}}"
       ); //{\"close_dc_contactors\":{\"value\":true}}}");
    //fims_message *msg = bufferToFims((const char *)rbuf);
...
//     //bm->processFimsMessage(msg);
    ...
    free((void *)rbuf);//     //vm.getMapsCj
    cJSON* cj2 = vm.getMapsCj(vmap, nullptr, nullptr, 0x0110);
    char* buf = cJSON_Print(cj2);
    cJSON_Delete(cj2);
    free(buf);
...
// we need it added to the optDict someting to do with uiObject
    asprintf(&rbuf,
       "{\"method\":\"set\",\"uri\":\"/components/testOptDict\",\"body\":{\"usesOptDict\":"
       "{\"value\":true,\"stuff\":1234,"
       "\"variables\":"
       "{\"name1\":\"BmsState\","
       "\"name2\":\"BmsVal1\","
...
//     //bm->processFimsMessage(msg);
    vm.free_fims_message(msg);
    free((void *)rbuf);//     //vm.getMapsCj
    cj2 = vm.getMapsCj(vmap, nullptr, nullptr, 0x0110);
    buf = cJSON_Print(cj2);
    cJSON_Delete(cj2);


    FPS_ERROR_PRINT("%s >> result so far \n>>%s<<\n", __func__, buf);
    free((void*)buf);
    delete essm;
    delete bmsm;
    delete pcsm;
...
//     asprintf(&rbuf,"{\"method\":\"set\",\"uri\":\"/components/sungrow_ess_1\",\"body\":{\"close_dc_contactors\":true,\"stop_start\":345}}"); //{\"close_dc_contactors\":{\"value\":true}}}");
//     msg = vm.bufferToFims((const char *)rbuf);
//     if(msg)
//     {
//         cJSON* cj =cJSON_CreateObject();
...
//     asprintf(&rbuf,"{\"method\":\"set\",\"uri\":\"/setup/bms\",\"body\":{\"test_f1\":1234,\"test_f2\":456}}"); //{\"close_dc_contactors\":{\"value\":true}}}");
//     msg = vm.bufferToFims((const char *)rbuf);
//     if(msg)
//     {
//         cJSON* cj =cJSON_CreateObject();
...
//     asprintf(&rbuf,"{\"method\":\"set\",\"uri\":\"/alarms/bms\",\"body\":{\"fault_f1\":1234,\"fault_f2\":456}}"); //{\"close_dc_contactors\":{\"value\":true}}}");
//     msg = vm.bufferToFims((const char *)rbuf);
//     if(msg)
//     {
//         cJSON* cj =cJSON_CreateObject();
...
//     // this will run the process_alarms   function for test_f1
//     asprintf(&rbuf,"{\"method\":\"set\",\"uri\":\"/alarms/bms\",\"body\":{\"fault_f1\":12345,\"fault_f2\":4567}}"); //{\"close_dc_contactors\":{\"value\":true}}}");
//     msg = vm.bufferToFims((const char *)rbuf);
//     if(msg)
//     {
//         cJSON* cj =cJSON_CreateObject();
...
//     asprintf(&rbuf,"{\"method\":\"set\",\"uri\":\"/test/bms\",\"body\":{\"test_f1\":12345,\"test_f2\":4567}}"); //{\"close_dc_contactors\":{\"value\":true}}}");
//     msg = vm.bufferToFims((const char *)rbuf);
//     if(msg)
//     {
//         cJSON* cj =cJSON_CreateObject();
...
//     //assetVar* setActVecfromCj(assetVar* av, "onSet", "enum", cjbf, cJSON* cj)
//         // add an action.
...
//     cjbf = cJSON_Parse("[{ \"enable\":\"/status/ess:UiEnabled\",\"inValue\":true,\"uri\":\"/controls/ess:maint_active_power_setpoint@enabled\",\"outValue\":true}]");
...
// // Also how do we remove / replace lines... reinstall the bit field.on write the bit field spaces by 5 to allow insert
...
// //                         "onSet":        [{
// //                                         "remap":        [{
// //                                                         "enable":       "/status/ess:UiEnabled",
// //                                                         "inValue":      true,
// //                                                         "outValue":     true,
// //                                                         "uri":  "/controls/ess:maint_active_power_setpoint@enabled"
// //                                                 }]
// //                                 }]
// //                 },
// //                 "intParam":     1234
...
//     vm.setVal(vmap2, "/config/ess/test", "testIntVar", ival);  //Well this seemed to work.

//     FPS_ERROR_PRINT("%s >> setval via vm to 5678 done \n", __func__);

//     bval = true;
//     FPS_ERROR_PRINT("%s >> setval via vm to [true] \n", __func__);
//     vm.setVal(vmap2, "/config/ess/test", "testBoolVar", bval);  //Well this seemed to work.
