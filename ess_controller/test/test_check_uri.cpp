 ...
 * get /ess/vars/pattern ... will show all components and vars matching a pattern
 * set /ess/  bypasses blocks
 * set /components ( or any allowed Sets will create havoc .. not really will create components for us)
 * set/pub /status ( or any Blocked sets are disabled) 
 * set/pub /ess/status ( or or any Blocked sets are allowed) 
 * uri /x/y/z   may be comp /x/y/z if body is cJSON and does not have a "value" key     body can have {"vname":val,"vname":val...}
 *                                                                                      or {"vname":{"value":val}}
 * uri /x/y/z  may be comp /x/y if it's body is NOT cJSON or does have  a "value" first level key , in this case "z" is the variable and we are denoted as a "SINGLE"
 * 
...
// bool checkedBlockedUri(VarMapUtils &vm, varsmap &vmap, std::string &cName, const char* method, const char* uri)
// {
//     bool blocked = false;
...
//                     if (strncmp(&uri[1], ix.first.c_str(), strlen(ix.first.c_str())-1)==0)
//                     {
//                         blocked = true;
//                         break;
//                     }
...
// cJSON *checkSingleUri(VarMapUtils &vm, varsmap &vmap, int& single, std::string &vName, std::string &cName, const char* uri=nullptr,const char* body=nullptr)
// {
//     single = 0;
...
//            if(cji)
//            {
//                 printf("cj OK  found value confirmed SINGLE  body [%s] type %d\n", body, cj->type);
//                 single |= 1;
//                 single |= 2;
...
// // do this for singles
// cJSON* loadVmap(VarMapUtils &vm, varsmap& vmap, int single, const char* comp, const char* var, const char* body)
// {
//     char* xsp = nullptr;
//     cJSON* cjr = nullptr;
//     if(single & 1)
...
// cJSON* runFimsMsg(VarMapUtils &vm, varsmap &vmap, fims_message* msg , fims* p_fims)
// {
//     cJSON* cjr = nullptr;
...
//         int single;
//         // send in the new uri
//         cJSON* cj = checkSingleUri(vm, vmap, single, vName, cName2, newUri.c_str(), msg->body);
        
//         printf(" after checkSingle single %d vName [%s] cName2 [%s] newUri [%s] newBod [%s] cj %p\n"
...
//         else
//             cjr = loadVmap(vm, vmap, single, cName2.c_str(), vName.c_str(), newBod);
...
//         if(0)FPS_ERROR_PRINT(" %s >> >>>>>>>>>>>  %2.3f  %s  FIMS MESSAGE  method [%s] uri [%s]\n"
//                 ,__func__
//                 , vm->get_time_dbl()
//                 , am->name.c_str()
//                 , msg->method         
...
//        std::string cName2;   // new comp (uri) name
//        bool reject = checkedBlockedUri(vm, vmap, cName, msg->method, msg->uri)

//         if (!reject)
//         {
//         int ret = checkSingleUri(vm, vmap, vName, cName2, cNmae1.c_str(), msg->body);
//
//             am->vm->processFims(*am->vmap,  msg, &cj);
//          char * new_body = (char *)msg->body;
//          cJSON * cjr = nullptr; 
//             am->vm->processRawMsg(*am->vmap, msg->method, cName2.c_str(), msg->replyto, newbody &cjr)
//             am->vm->processFimsMsg(*am->vmap,  msg, &cj);
//         }
//         if (cj)
//         {
//             if(msg->replyto)
...
//             if(strcmp(msg->method,"get") == 0)
//             {
//                 if(1)FPS_ERROR_PRINT(" %s >> >>>>>>>>>>>  %2.3f  %s  FIMS MESSAGE  method [%s] uri [%s]\n"
//                 ,__func__
//                 , vm->get_time_dbl()
//                 , am->name.c_str()
//                 , msg->method         
...
//                 {
//                     if(0)std::cout << am->name << "  >> fims_msg uri "<< msg->uri  << " REJECTED\n";
//                     reject = true;
//                 }
