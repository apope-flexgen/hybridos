...
// #include "spdlog/fmt/bundled/format.h" // erorr when fmt and spdlog are used
// together, blegh! above is unnecessary it is a part of regular spdlog.h
...
// todo: get compile time log type in here so we're not stuck with just
// rotating_file_log_st always another template default param will work.
//  Time stuff
...
public:
    timedLogger(const char* name,
                const std::vector<spdlog::sink_ptr>& sinks)  // loggers should be named after the asesetVar's name
    ...
    std::chrono::time_point<std::chrono::high_resolution_clock> getCreateTime() const { return mCreationTime; }

    template <typename FormatString, typename... Args>
    void info(const FormatString& fmt, Args&&... args)
    {
        mLogger->info(fmt, std::forward<Args>(args)...);  // might need this to be
    ...
    {
        mLogger->debug(fmt, std::forward<Args>(args)...);  // might need this to be
                                                           // different depending on
                                                           // log level.
    }
    ...
    {
        mLogger->trace(fmt, std::forward<Args>(args)...);  // might need this to be
                                                           // different depending on
                                                           // log level.
    }
    ...
    {
        mLogger->warn(fmt, std::forward<Args>(args)...);  // might need this to be
                                                          // different depending on
                                                          // log level.
    }
    ...
    {
        mLogger->error(fmt, std::forward<Args>(args)...);  // might need this to be
                                                           // different depending on
                                                           // log level.
    }
    ...
    {
        mLogger->critical(fmt, std::forward<Args>(args)...);  // might need this to
                                                              // be different
                                                              // depending on log
                                                              // level.
    }
    ...
    void flush() { mLogger->flush(); }
};

// this is your "timed window" attached to an assetVar (goes underneath extras),
// I'll get around to making the ostream opeartor<< for the assetVars
// eventually. this uses a deque so not as efficient as it could be, but it
// works well enough. might need to benchmark this eventually, wouldn't be
// suprised in the performance loss from it. "pure" spdlog would, of course, be
// the fastest. the multisink idea is cool, but slows it down quite a bit due to
// all the heap allocations considering making a much more simplified version
// without the multisink, or just the regular file_sink.
...
class WindowLog
{
    ...
    const std::vector<spdlog::sink_ptr>
        mSinks;  // for storing the sink locations when creating new logs
                 // could make above only a reference, but this would cause a segFault if you
                 // give it a temporary.

public:
    std::deque<timedLogger<MaxFileSize, RotFileNum>> mLogWindow;
    // creates logs that go to this filename with this logger name:
    // possible todo: make it to where loggers can change name and file location
    // during runtime:
    WindowLog(const char* loggerName,
              const std::vector<spdlog::sink_ptr>& sinks = { std::make_shared<spdlog::sinks::basic_file_sink_st>(
                  LogFile, MaxFileSize, RotFileNum) })
        : mLoggerName(loggerName), mSinks(sinks)
    {
    }

    ~WindowLog()  // might not need to flush on destruction. If so then comment
                  // out.
    {
        this->flush();  // gauranteed logging even when the program ends, just in
                        // case.
    }

    void shiftWindow()
    {
        // if (!mLogWindow.empty()) // this is logically unecessary, so it is
        ...
        if (0)
        {
            ...
            while (dif.count() >= WindowSize)  // if x (5 by default) seconds have
                                               // elapsed start popping off from the
                                               // queue
            {
