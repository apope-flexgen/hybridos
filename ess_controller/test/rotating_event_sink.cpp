...
int main()
{
    ...
    ESSLogger::get().setPattern("%D -> %v", spdlog::pattern_time_type::local);  // make sure %v is there to
                                                                                // keep your own message
    ...
    ESSLogger::get().debug("Is spdlog slow? I think not, observe -> {:%F} {:%H:%M:%S}", now,
                           now.time_since_epoch() + 20_h);
