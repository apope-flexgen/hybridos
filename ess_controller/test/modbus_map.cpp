...
double get_time_dbl()
{
    return static_cast<double>(get_time_us()) - g_base_time;
}
...
int setup_register_map(sys_t* sys, cJSON* reg, const component_config* comp)
{
    ...
    rlog->reg_type = static_cast<Type_of_Register>(decode_type(cjtype->valuestring));
    ...
    //    ++)
    {
        ...
        if (ritem->reg_off < rlog->start_offset || ritem->reg_off > rlog->start_offset + rlog->num_regs)
        {
            FPS_ERROR_PRINT("offset %d is outside of registers read %d - %d.\n", ritem->reg_off, rlog->start_offset,
                            rlog->start_offset + rlog->num_regs);
            delete[] rmap;
            rlog->register_map = nullptr;
            return -1;
        }
        ...
        ritem->num_regs = (multi_reg != nullptr && multi_reg->type == cJSON_Number) ? multi_reg->valueint : 1;
        ...
        ritem->bit_field = (bit_field != nullptr && bit_field->type == cJSON_True && (++string_type != 0));

        cJSON* indv_bits = cJSON_GetObjectItem(arg1, "individual_bits");
        ritem->individual_bits = (indv_bits != nullptr && indv_bits->type == cJSON_True && (++string_type != 0));

        cJSON* enum_type = cJSON_GetObjectItem(arg1, "enum");
        ritem->enum_type = (enum_type != nullptr && enum_type->type == cJSON_True && (++string_type != 0));

        cJSON* random_enum_type = cJSON_GetObjectItem(arg1, "random_enum");
        ritem->random_enum_type = (random_enum_type != nullptr && random_enum_type->type == cJSON_True &&
                                   (++string_type != 0));
            ...
            if (ritem->random_enum_type)
            {
                for (int i = 0; i < array_size; i++)
                {
                    ...
                    if (bit_string_item != nullptr)
                    {
                        ...
                        if (value != nullptr && value->type == cJSON_Number && bit_string != nullptr &&
                            bit_string->valuestring != nullptr)
                        {
        ...
        {
            FPS_ERROR_PRINT(
                "Bit_strings object defined but no corresponding "
                "bit_field type values.\n");
            delete[] rmap;
            rlog->register_map = nullptr;
            return -1;
        }
        ...
        if ((uri_reg != nullptr) && (uri_reg->valuestring != nullptr))
        {
...
// TODO(root): use the sys conn_info
bool get_conn_info(sys_t* sys, cJSON* config)  //, connection_config *connection)
{
    ...
    if (ret)
    {
        ret = getCJstr(cj, "name", &sys->conn_info->name, "Dummy Name", true);
    }
    if (ret)
    {
        ret = getCJstr(cj, "ip_address", &sys->conn_info->ip_address, "127.0.0.1", false);
    }
    if (ret)
    {
        ret = getCJint(cj, "port", sys->conn_info->port, 502, false);
    }
...
int get_component(sys_t* sys, cJSON* config, component_config* comp, cJSON* component)
{
    ...
    if (ret)
    {
        ret = getCJstr(cj, "id", &comp->id, "Dummy Name", true);
    }
    if (ret)
    {
        ret = getCJbool(cj, "byte_swap", comp->byte_swap, false, false);
    }
    if (ret)
    {
        ret = getCJbool(cj, "off_by_one", comp->off_by_one, false, false);
    }
    if (ret)
    {
        ret = getCJint(cj, "frequency", comp->frequency, 500, false);
    }
    if (ret)
    {
        ret = getCJint(cj, "offset_time", comp->offset_time, 100, false);
    }
    if (ret)
    {
        ret = getCJint(cj, "debounce", comp->debounce, 0, false);
    }
    comp->debounce *= 10;
    if (ret)
    {
        ret = getCJint(cj, "device_id", comp->device_id, -1, false);
    }
    ...
    if (ret)
    {
        ret = getCJbool(cj, "heartbeat_enabled", comp->heartbeat_enabled, false, false);
    }
    ...
    if (comp->heartbeat_enabled)
    {
        if (ret)
        {
            ret = getCJint(cj, "modbus_heartbeat_freq_ms", comp->modbus_heartbeat_freq_ms, 500, false);
        }
        if (ret)
        {
            ret = getCJint(cj, "component_heartbeat_timeout_ms", comp->component_heartbeat_timeout_ms, 500, false);
        }

        if (ret)
        {
            ret = getCJstr(cj, "component_heartbeat_read_uri", &comp->read_name, nullptr, true);
        }
        ...
        if (ret)
        {
            ret = getCJstr(cj, "component_heartbeat_write_uri", &comp->write_name, nullptr, false);
        }
    }

    // parsing the configuration file to get the Type of the registers, its start
    // and end address, and to get the value of the specified registers  We first
    // get the object and then store it in the data structure  to get Register
    // object
...
{
    int rc = 0;
    // int get_comp_registers(sys_t* sys,component_config* comp, cJSON* registers,
    // cJSON* component)
    ...
    for (int reg_set = 0; reg_set < config->reg_cnt; reg_set++)
    {
        if (config->reg_datalog[reg_set].reg_type == reg_type)
        {
            for (unsigned int i = 0; i < config->reg_datalog[reg_set].map_size; i++)
            {
                ...
                if (config->reg_datalog[reg_set]
                        .register_map[i]
                        .individual_bits)  // override for individual_bits, register
                                           // name read as bit_strings
                {
...
// TODO(root): place all the components and reg_types in an overall system object
void add_sys_sysmap(sys_t* sys, maps_t* ritem)
{
    sys->sysmaps[ritem->reg_type][static_cast<int>(ritem->reg_off)] = ritem;
}

// TODO(root): place all the components and reg_types in an overall system object
void add_sys_urimap(sys_t* sys, maps_t* ritem)
{
    char* uri = ritem->ruri != nullptr ? ritem->ruri : ritem->uri;
    ...
    for (int i = 0; i < cnum; i++)
    {
        ...
        for (int j = 0; j < pcomp->reg_cnt; j++)
        {
            ...
            FPS_ERROR_PRINT("Component [%d] id [%s]  regd %p map_size %d\n", i, pcomp->id, regd,
                            regd ? regd->map_size : 0);
            fprintf(stdout, "   [%s] \n", get_reg_stype(regd->reg_type));

            //    regdata [%d] type [%s] start [%d] num %d map_size [%d].\n"
            //                 , i, get_reg_stype(regd->reg_type),
            //                 (int)regd->start_offset , (int)regd->num_regs ,
            //                 (int)regd->map_size
            //                 );
            // for each reg
            for (int k = 0; k < static_cast<int>(regd->map_size); k++)
            {
                ...
                if (sys->sysmaps[regd->reg_type].find(static_cast<int>(ritem->reg_off)) !=
                    sys->sysmaps[regd->reg_type].end())
                {
                    fprintf(stdout, " name [%s] already found at index %d\n", ritem->reg_name, ritem->reg_off);
                }
                else
                {
    ...
    for (int j = 0; j < Num_Register_Types; j++)
    {
        if (!sys->sysmaps[j].empty())
        {
            ...
            for (it = sys->sysmaps[j].begin(); it != sys->sysmaps[j].end(); ++it)
            {
                ...
                fprintf(fp, "          { \"name\":\"%s\",\"id\":\"%s\",\"offset\":%d,", rmap->reg_name, rmap->reg_name,
                        static_cast<int>(rmap->reg_off));
                if (rmap->scale != 0.0)
                {
                    fprintf(fp, "\"scale\": %f, ", rmap->scale);
                }
                if (rmap->shift != 0)
                {
                    fprintf(fp, "\"shift\": %d, ", rmap->shift);
                }
                if (rmap->num_regs != 1)
                {
                    fprintf(fp, "\"size\": %d, ", rmap->num_regs);
                }

                // signed
                if (rmap->sign)
                {
                    fprintf(fp, "\"signed\": true, ");
                }
                if (rmap->ruri != nullptr)
                {
                    fprintf(fp, "\"uri\":\"%s\" }", rmap->ruri);
                }
                else
                {
                    fprintf(fp, "\"uri\":\"%s\" }", rmap->uri);
                }
            }
            fprintf(fp, "       ]");
            if (j < last_reg)
    ...
    int mcount = 0;
    {
        ...
        for (std::map<std::string, urimap_t*>::iterator it = sys->urimaps.begin(); it != sys->urimaps.end(); ++it)
        {
            ...
            for (int i = 0; i < Num_Register_Types; i++)
            {
                ...
                FPS_ERROR_PRINT(" uri  [%s] type %d size (%d)\n", it->first.c_str(), i, (int)imap->size());
                if (static_cast<int>(imap->size()) > 0)
                {
                    if (need_comma)
                    {
                        fprintf(fp, ",");
                    ...
                    {
                        phead = 0;
                        fprintf(fp, "/usr/local/bin/fims/fims_echo -u %s -b '{\n", it->first.c_str());
                    }
                    // map for each
                    // the map should already have the min offset at the hed and the max
                    // offset at the end
                    ...
                    for (xit = imap->begin(); xit != imap->end(); ++xit)
                    {
                        ...
                        if (max_offset < static_cast<int>(ritem->reg_off) + static_cast<int>(ritem->num_regs))
                        {
                            max_offset = ritem->reg_off + ritem->num_regs;
                        }
                        if (min_offset > static_cast<int>(ritem->reg_off))
                        {
                            min_offset = ritem->reg_off;
                        }

                        FPS_ERROR_PRINT("  ....  name [%s]\n", ritem->reg_name);
                        fprintf(fp, " \"%s\":{\"value\":0}", ritem->reg_name);
                        if (k < static_cast<int>(imap->size()) - 1)
                        {  // TODO(root): may need to add a "," if there are more to print
                            fprintf(fp, ",");
                        }
                        fprintf(fp, "\n");
                        k++;
...
const char* reg_types[] = { "Coil", "Discrete_Input", "Input_Register", "Holding_Register" };
const char* reg_stypes[] = { "coils", "discrete_inputs", "input_registers", "holding_registers" };
...
int decode_type(const char* val)
{
    ...
    if (strcmp(val, "Discrete Inputs") == 0)
    {
