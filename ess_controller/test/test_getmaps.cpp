...
// include/varMapUtils.h: In member function ‘int
// VarMapUtils::vListPartialSendFims(varsmap&,
// std::vector<std::basic_string<char> >&, const char*, fims*, const char*)’:
// ./include/varMapUtils.h:140:55: error: ‘loadAssetList’ was not declared in
// this scope
//              int found = loadAssetList(vmap, cj, newuri);
//                                                        ^
// ./include/varMapUtils.h: In member function ‘cJSON*
// VarMapUtils::getMapsCj(varsmap&, const char*, const char*, int)’:
// ./include/varMapUtils.h:1828:58: error: ‘loadAssetList’ was not declared in
// this scope
//              found += loadAssetList(vmap, cj, newuri, opts);
//                                                           ^
// ./include/varMapUtils.h:1899:65: error: ‘loadAssetList’ was not declared in
// this scope
                       ...
                       std::vector<std::string>& uriVec)
{
    ...
    for (auto& x : uriVec)
    {
        ...
        bs = bsx;
        if (0)
            FPS_ERROR_PRINT(" %s >> uri [%s] uriVec [%s] opts 0x%04x bvec %d bs[%s]\n", __func__, inuri, x.c_str(),
...
int createAssetListCj(varsmap& vmap, const char* uri, std::vector<std::string>* sysVec, int opts,
                      std::vector<std::string>& nVec)
{
    ...
    if (sysVec)
    {
        for (auto& y : *sysVec)
        {
            ...
            if (0)
                FPS_ERROR_PRINT(" %s >> sysvec y [%s] yend [%s]\n", __func__, y.c_str(), yend.c_str());
            for (auto z : xVec)
            {
    ...
    for (auto& y : *yVecp)
    {
        ...
        auto x = y.find(suri);
        if (0)
            FPS_ERROR_PRINT(" %s >> >> >> yVec option [%s] x %d\n", __func__, y.c_str(), (int)x);
        if (x == 0)
        {
            if (0)
                FPS_ERROR_PRINT(" %s >> >> >> nVec push_back [%s]\n", __func__, y.c_str());
            nVec.push_back(y);
        }
    }
                 ...
                 const char* origuri = nullptr, cJSON* cji = nullptr)
{
    ...
    //         __func__, inuri); uri =
    //         strdup(&inuri[strlen(uri) - strlen("/naked")]);
    ...
    else if (uri && var)
    {
        ...
        if (x != vmap.end())
        {
            ...
            if (y != vmap[uri].end())
            {
                ...
                if (0)
                    FPS_ERROR_PRINT(" %s >> getting cj for uri [%s] var [%s] found %d \n", __func__, uri,
                                    y->first.c_str(), found);
                // TODO need opts here
    ...
    // get all the objects
    {
        ...
        for (auto& x : vmap)
        {
            ...
            if (0)
                FPS_ERROR_PRINT(
                    " %s >>       >>>>>>>>>>>>>>>>>>>>query alist "
                    ">>>>>>>>>>>>>>>>>>>>>>>>>>getting cj for comp [%s]  "
                    "alist %p\n",
                    __func__, x.first.c_str(), (void*)alist);
            if (alist)
            {
                unsigned int ix = 0;
                assetVar* av;
...
int main(int argc, char* argv[])
{
    ...
    const char* rep2 =
        "{\"/assets/bms_1\":"
        "{\"start_stop\":{\"value\":0,\"actions\":{\"onSet\":{\"bitfield\":["
        "{\"inValue\":0,\"outValue\":\"nice\",\"uri\":\"/system/"
        "new_controls\",\"var\":\"oncmd\"},"
        "{\"inValue\":1,\"outValue\":34.5,\"uri\":\"/system/"
        "new_controls\",\"var\":\"kacclosecmd\"},"
        "{\"inValue\":8,\"outValue\":true,\"uri\":\"/system/"
        "new_controls\",\"var\":\"offcmd\"},"
        "{\"inValue\":9,\"outValue\":true,\"uri\":\"/system/"
        "new_controls\",\"var\":\"kacopencmd\"}"
        "]}}}}}";
    ...
    rc = vm.testRes(" Test 2", vmap, "get", "/components/bms_1", nullptr, rep2);
    ...
    rc = vm.testRes(" Test 5", vmap, "get", "/system/new_controls/kacclosecmd", nullptr, "{\"value\":34.5}");

    rc = vm.testRes(" Test 6", vmap, "get", "/system/new_controls/oncmd", nullptr, "{\"value\":\"nice\"}");
    ...
    //         }
    // }
    // this is probably broken we should get this  actually for now we cannot
    // represent the strucure properly need to change the loader...
