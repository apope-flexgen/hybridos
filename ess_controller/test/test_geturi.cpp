...
// the sysVec keeps track of the components as they are loaded.
//
...
char* getUri(varsmap &vmap, VarMapUtils *vm, const char* uri, std::string& fopts, int options = 0x100)
{
    ...
    if(vmap.find(myx.Uri) == vmap.end())
    {
        ...
        {
            if (debug) FPS_ERROR_PRINT(" %s >> path 0 table/var [%s/%s] found  \n"
                    , __func__
                    , suri.c_str()
                    , svar.c_str()
                    );
    ...
    if(vmap.find(my.Uri) == vmap.end())
    {
        ...
        if (debug) FPS_ERROR_PRINT(" %s >> path 1 no table match seek similar tables \n"
                    , __func__);
        if(vm->syscVec) // if we hav a sysvec ( from load config) then follow that order
        {
        ...
        else
        {
            ...
            for (auto x: vmap)
            {
                if (debug) FPS_ERROR_PRINT(" %s >> vmap idx [%d] entry [%s] \n"
        ...
        if(mvec.size() > 0)
        {
            ...
            {
                if(debug)FPS_ERROR_PRINT(" %s >>  [%03d]->[%s]\n", __func__, idx, mvec[idx]);
            ...
            if(debug)
            {
                ...
                if(debug)FPS_ERROR_PRINT(" %s >>  cj so far\n[%s]\n", __func__, tmp);
                free(tmp);
            }
            if(debug)printf(" segment 1\n");
        ...
        {
            if(debug)FPS_ERROR_PRINT(" got nothing from [%d] --> [%s]\n", idx, my.Uri);
            std::string uri;
            std::string var; 
    ...
    {
        if (debug) FPS_ERROR_PRINT(" %s >> path 2 found table match \n"
                    , __func__);
        ...
        else
        {
            ...
            if (my.Var == nullptr)
            {
                if (1||debug) FPS_ERROR_FMT(" {} >> path 2 found table {} match no var \n"
                    , __func__
                    , my.Uri
                    );
                if (cj->child)
            ...
            {
                if (debug) FPS_ERROR_PRINT(" %s >> path 2 found table match with var [%s] \n"
                    , __func__, my.Var);
                if (cj->child)
                {
                    cJSON* cjc = cj->child;
...
void clearAlist(VarMapUtils* vm, varsmap &vmap)
{
    ...
    {
        if(debug)FPS_ERROR_PRINT("%s >> no aList found\n",__func__);
    }
    else
    {
...
int test_geturi(int argc, char*argv[])
{
    ...
    if(debug)FPS_ERROR_PRINT("%s >> opts = 0x%04x asort %s \n result =>> \n[%s]\n"
