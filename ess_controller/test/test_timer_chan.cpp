 ...
 * the main thread will issue timer requests. The timer thread will call the
 * main thread back at the requested time. we can even find out how long it took
 * to process the round trip. The timer server will have A request channel A
 * priority queue of timer requests with the closest one at the top
 *
 */
...
// this is the makings of a reverse prioity queue the smallest value ot tt will
// be the top of the queue normal priority queues do it the other way round. The
// compare function included in the queue definition reverses this. The timeVar
// will contain the details of the thing we want to do at the time out time. for
// now it just contains somestuff
//
...
// then run the system and collect the output  ?? how do we do that something
// like std::string result=""; result.append(last_test);

// test the output string against an expected output string
// note this may be tricky because the numbers will not be the same due to times
// .. have to think about that test 2 send multiple  requests to the message
// channel again inspect the output

// this is all the get_time_dbl stuff .. there is better std::chron stuff now in
// c++ but we'll use the double for now.
...
{
    // Jimmy - may need to look into assetVar.h in more detail and see how vars
    // are stored/utilized
    ...
    char* body;
    asprintf(&body,
             "{\"date_yr\":%d,\"date_mon\":%d,\"date_day\":%d,\"date_hr\":%d,"
             "\"date_min\":%d,\"date_sec\":%d}",
...
public:
    ...
    // Added this constructor to allow timeVar to store and run a function (ex.:
    // HandleHeartBeat)
    ...
    // this is not used
    bool greater(timeVar* tv) { return (tv->tt > tt); }

    double tt;  // wake up time in seconds base on get_time_dbl()
    int somestuff;
    char* actions;
    int (*func)(fims*);  // Pointer to a function - executes whenever the timeVal
                         // run() is called
};
...
{
public:
    bool operator()(timeVar* t1, timeVar* t2) { return (t1->tt > t2->tt); };
};
...
// the  tqueue provides a queue of objects with the smallest value of tt at the
// top. tt is the time we want this thing to wake up and to the actions

// this is a structure used to pass channel data to the running thread.  We will
// use this to send instructions ti the timer object
typedef struct mchan_data_t
{
    ...
    int delay;
    int* run;
    int (*func)(fims*);  // Pointer to a function - each time request will perform
                         // specific action
    pthread_t thread;
...
void* timer_loop(void* data)
{
    ...
    while (*td->run)
    {
        ...
        // give me a wake up time in the future new timeVar(get_time_dbl()
        // +td->dcount, td_count));
        // td->wake_up_chan->put(new timeVar(get_time_dbl() +td->dcount,
        // td->count));
...
void* message_loop(void* data)
{
    ...
    while (*td->run)
    {
        ...
        asprintf(&mdata1,
                 "\"timer\":{ "
                 "\"action\":\"oneshot\",\"delay\":500,\"send\":\"/system/"
                 "bss_1:start_off\",\"to\":\"/component/"
                 "bss_1:system_start\"}");
        td->message_chan->put(mdata1);  // HMM what happens if mdata is changed
        asprintf(&mdata2,
                 "\"timer\":{ "
                 "\"action\":\"oneshot\",\"delay\":10,\"send\":\"/system/"
                 "bss_1:start_on\",\"to\":\"/component/"
                 "bss_1:system_start\"}");
...
void* timer_main(void* data)
{
    ...
    // Start timer request threads, each with different time delay and task
    // function
    ...
    while (running)
    {
        ...
        if (!tqueue.empty())
        {
            ...
            if (tv->tt < tnow)
            {
                ...
                {
                    tv = tqueue.top();
                    // reset tdel remember the NEXT time to wake up is at the top of the
                    // queue
                    tdel = tv->tt;
                }
            }
        ...
        // this will return after the tdel timeout or when we get another timer
        // request. the tdel timeout is the time perion between now and the lowest
        // value request at the top of the tqueue Ie the next time to run. it will
        // block until the timeout or the message comes. if it returned true a new
        // timer request was sent to the timer server. push that request and the
        // tqueue will bubble the next request to the top.
        ...
        //     // TODO this will create a timVar structure with a delay and action
        //     and add it ti tqueue
        //     //tqueue.push(new tv)
