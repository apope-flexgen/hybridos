...
protected:
    virtual void SetUp()
    {
        ...
        // const char* ival = R"({"value": 0, "EnableConditionCheck": false,
        // "AlarmTimeout": 5, "FaultTimeout": 7, "RecoverTimeout": 4})"; const char*
        // dval = R"({"value": 0.0, "EnableConditionCheck": false, "AlarmTimeout":
        // 5, "FaultTimeout": 7, "RecoverTimeout": 4})"; const char* cval =
        // R"({"value": "init", "EnableConditionCheck": false, "AlarmTimeout": 5,
        // "FaultTimeout": 7, "RecoverTimeout": 4})";
        const char* ival =
            "{\"value\": 0, \"EnableConditionCheck\": false, "
            "\"AlarmTimeout\": 5, \"FaultTimeout\": 7, "
            "\"RecoverTimeout\": 4}";
        const char* dval =
            "{\"value\": 0.0, \"EnableConditionCheck\": false, "
            "\"AlarmTimeout\": 5, \"FaultTimeout\": 7, "
            "\"RecoverTimeout\": 4}";
        const char* cval =
            "{\"value\": \"init\", \"EnableConditionCheck\": false, "
            "\"AlarmTimeout\": 5, \"FaultTimeout\": 7, "
            "\"RecoverTimeout\": 4}";
...
TEST_F(ConditionMonitorTest, condition_monitor_params_valid)
{
    ...
    EXPECT_STREQ("int",
                 int_av->getcParam("Type"));  // Default type is int if not defined in config
...
// Test if the alarm/fault/reset time are not changed and the alarm/fault/reset
// status flags are not changed when condition check is disabled
...
// Test if the alarm/fault time and flag are changed when the value != expected
// value(s) with no conditions Also test if the reset time and flag are changed
// when the value == expected value(s) with no conditions assetVar type is int
TEST_F(ConditionMonitorTest, condition_monitor_int_alarm_fault_reset_state)
{
    ...
    CheckMonitorVar(vmap, amap, "bms", nullptr, int_av);
    EXPECT_GT(5,
              int_av->getdParam("AlarmTime"));  // Alarm time should be decremented
    EXPECT_GT(7,
              int_av->getdParam("FaultTime"));  // Fault time should be decremented
    EXPECT_EQ(4, int_av->getdParam("RecoverTime"));
    EXPECT_FALSE(int_av->getbParam("seenFault"));
    EXPECT_FALSE(int_av->getbParam("seenAlarm"));
    EXPECT_FALSE(int_av->getbParam("seenReset"));
    ...
    CheckMonitorVar(vmap, amap, "bms", nullptr, int_av);
    EXPECT_LT(0,
              int_av->getdParam("AlarmTime"));  // Alarm time should be incremented
    EXPECT_LT(0,
              int_av->getdParam("FaultTime"));       // Fault time should be incremented
    ...
    // This time, we'll expect reset state. Fault/alarm states should be set to
    // false
    int_av->setParam("RecoverTime", 0.00001);
    EXPECT_EQ(0.00001, int_av->getdParam("RecoverTime"));
    CheckMonitorVar(vmap, amap, "bms", nullptr, int_av);
    EXPECT_EQ(5, int_av->getdParam("AlarmTime"));    // Alarm time should still be
                                                     // equal to alarm time config
    EXPECT_EQ(7, int_av->getdParam("FaultTime"));    // Fault time should still be
                                                     // equal to fault time config
...
// Test if the alarm/fault time and flag are changed when the value != expected
// value(s) with no conditions Also test if the reset time and flag are changed
// when the value == expected value(s) with no conditions assetVar type is
// double
TEST_F(ConditionMonitorTest, condition_monitor_dbl_alarm_fault_reset_state)
{
    ...
    CheckMonitorVar(vmap, amap, "bms", nullptr, dbl_av);
    EXPECT_GT(5,
              dbl_av->getdParam("AlarmTime"));  // Alarm time should be decremented
    EXPECT_GT(7,
              dbl_av->getdParam("FaultTime"));  // Fault time should be decremented
    EXPECT_EQ(4, dbl_av->getdParam("RecoverTime"));
    EXPECT_FALSE(dbl_av->getbParam("seenFault"));
    EXPECT_FALSE(dbl_av->getbParam("seenAlarm"));
    EXPECT_FALSE(dbl_av->getbParam("seenReset"));
    ...
    CheckMonitorVar(vmap, amap, "bms", nullptr, dbl_av);
    EXPECT_LT(0,
              dbl_av->getdParam("AlarmTime"));  // Alarm time should be incremented
    EXPECT_LT(0,
              dbl_av->getdParam("FaultTime"));       // Fault time should be incremented
    ...
    CheckMonitorVar(vmap, amap, "bms", nullptr, dbl_av);
    EXPECT_EQ(5, dbl_av->getdParam("AlarmTime"));    // Alarm time should still be
                                                     // equal to alarm time config
    EXPECT_EQ(7, dbl_av->getdParam("FaultTime"));    // Fault time should still be
                                                     // equal to fault time config
...
TEST_F(ConditionMonitorTest, condition_monitor_str_alarm_fault_reset_state)
{
    ...
    CheckMonitorVar(vmap, amap, "bms", nullptr, str_av);
    EXPECT_GT(5,
              str_av->getdParam("AlarmTime"));  // Alarm time should be decremented
    EXPECT_GT(7,
              str_av->getdParam("FaultTime"));  // Fault time should be decremented
    EXPECT_EQ(4, str_av->getdParam("RecoverTime"));
    EXPECT_FALSE(str_av->getbParam("seenFault"));
    EXPECT_FALSE(str_av->getbParam("seenAlarm"));
    EXPECT_FALSE(str_av->getbParam("seenReset"));
    ...
    CheckMonitorVar(vmap, amap, "bms", nullptr, str_av);
    EXPECT_LT(0,
              str_av->getdParam("AlarmTime"));  // Alarm time should be incremented
    EXPECT_LT(0,
              str_av->getdParam("FaultTime"));       // Fault time should be incremented
    ...
    CheckMonitorVar(vmap, amap, "bms", nullptr, str_av);
    EXPECT_EQ(5, str_av->getdParam("AlarmTime"));    // Alarm time should still be
                                                     // equal to alarm time config
    EXPECT_EQ(7, str_av->getdParam("FaultTime"));    // Fault time should still be
                                                     // equal to fault time config
...
// Test if alarm/fault time and flag are changed when there are more than one
// expected value Also test if reset time and flag are changed when there are
// more than one expected value
TEST_F(ConditionMonitorTest, condition_monitor_alarm_fault_reset_state_multiple_exp_vals)
{
    ...
    // Run condition monitor and check the current state of condition params now
    // Since current value == expectedVal1, we shouldn't expect alarm/fault time
    // and state to be changed
    CheckMonitorVar(vmap, amap, "bms", nullptr, int_av);
    ...
    // Setting current value = expectedVal2 should also not affect the alarm/fault
    // time and state
    int_av->setVal(5);
...
// Test if the alarm/fault time and flag are changed when the value != expected
// value(s) with conditions Also test if the reset time and flag are changed
// when the value == expected value(s) with conditions
TEST_F(ConditionMonitorTest, condition_monitor_alarm_fault_reset_state_with_cond)
{
    ...
    // Test case - value == expected value, but current condition value !=
    // condition(s) Expected behavior: reset time and state are not changed
    int_av->setVal(2);
    EXPECT_EQ(int_av->getiParam("expectedVal1"), int_av->getiVal());
    CheckMonitorVar(vmap, amap, "bms", nullptr, int_av);
    ...
    // Test case - value != expected value, but current condition value !=
    // condition(s) Expected behavior: alarm/fault time and state are not changed
    int_av->setVal(1);
    EXPECT_NE(int_av->getiParam("expectedVal1"), int_av->getiVal());
    CheckMonitorVar(vmap, amap, "bms", nullptr, int_av);

    EXPECT_EQ(5,
              int_av->getdParam("AlarmTime"));  // Alarm time should not be changed
    EXPECT_EQ(7,
              int_av->getdParam("FaultTime"));  // Fault time should not be changed
    EXPECT_EQ(4, int_av->getdParam("RecoverTime"));
    ...
    // Test case - value != expected value and current condition value == one, but
    // not all, conditions Expected behavior - alarm/fault time and state are not
    // changed
    int_av->setVal(0);
    ...
    // Test case - value == expected value and current condition value == one, but
    // not all, conditions Expected behavior - reset time and state are not
    // changed
    int_av->setVal(2);
    amap["condVar1"]->setVal(10);
    EXPECT_EQ(int_av->getiParam("expectedVal1"), int_av->getiVal());
    ...
    // Expected behavior - reset time should be incremented. alarm/fault time
    ...
    // Test case - value == expected value and all conditions are met
    // Expected behavior - reset time should be decremented. alarm/fault time
