...
/*
 *    After init we must get a continual heartbeat otherwise we alarm and then
 fault.
...
//  *
//  */
// int CheckAmTimestamp(varsmap& vmap, varmap& amap, const char* aname, fims*
...
//         amap["essTsFaults"] = vm->setLinkVal(vmap, "ess", "/status",
//         "essTsFaults", ival); amap["essTsAlarms"] = vm->setLinkVal(vmap,
//         "ess", "/status", "essTsAlarms", ival); amap["essTsInit"] =
//         vm->setLinkVal(vmap, "ess", "/status", "essTsInit", ival);
//         amap["essTsTimeoutFault"] = vm->setLinkVal(vmap, "ess", "/config",
//         "essTsTimeoutFault", toFault); amap["essTsTimeoutAlarm"] =
//         vm->setLinkVal(vmap, "ess", "/config", "essTsTimeoutAlarm", toAlarm);
//         amap["essTsTimeoutReset"] = vm->setLinkVal(vmap, "ess", "/config",
//         "essTsTimeoutReset", toReset); amap["essTsTimeoutHold"] =
//         vm->setLinkVal(vmap, "ess", "/config", "essTsTimeoutHold", toHold);

//         if (am->am)
//         {
//             amap["amTsFaults"] = vm->setLinkVal(vmap, am->am->name.c_str(),
//             "/status", "TsFaults", ival); amap["amTsAlarms"] =
//             vm->setLinkVal(vmap, am->am->name.c_str(), "/status", "TsAlarms",
//             ival); amap["amTsInit"] = vm->setLinkVal(vmap,
//             am->am->name.c_str(), "/status", "TsInit", ival);
//         }

//         amap["TsFaults"] = vm->setLinkVal(vmap, aname, "/status", "TsFaults",
//         ival); amap["TsAlarms"] = vm->setLinkVal(vmap, aname, "/status",
//         "TsAlarms", ival); amap["TsInit"] = vm->setLinkVal(vmap, aname,
//         "/status", "TsInit", ival); amap["TsState"] = vm->setLinkVal(vmap,
//         aname, "/status", "TsState", cval); amap["BypassTs"] =
//         vm->setLinkVal(vmap, aname, "/config", "BypassTs", bval);
//         amap["AssetState"] = vm->setLinkVal(vmap, aname, "/status",
//         "AssetState", ival); amap["TsStateNum"] = vm->setLinkVal(vmap, aname,
//         "/status", "TsStateNum", ival);
...
//             amap["Timestamp"]->setParam("tLast", dval);
...
//     char* lastTimestamp =
//     amap["Timestamp"]->getcParam("lastTimestamp");//amap["lastTimestamp"]->getiVal();
...
//             CheckAmTimestamp(vmap, amc->amap, amc->name.c_str(), p_fims,
//             amc); icnt++;
//         }
...
//             else
//             {
//                 FPS_ERROR_PRINT("%s >> %d essTsFaults cleared at time
//                 %2.3f\n", __func__, essTsFaults, tNow);
//             }
//         }
...
//             else
//             {
//                 FPS_ERROR_PRINT("%s >> %d essTsAlarms cleared at time
//                 %2.3f\n", __func__, essTsAlarms, tNow);
//             }
//         }
...
//         \n", __func__, aname,
//         lastTimestamp?lastTimestamp:"no last Value", tval1) if
//         (strcmp(currentTimestamp, lastTimestamp) != 0)
//         {

//             if (0)FPS_ERROR_PRINT("%s >> %s Timestamp change detected,  from
//             [%s] to [%s] tNow %2.3f seenTS [%s]\n"
//                 , __func__, aname, currentTimestamp,
//                 lastTimestamp, tNow, seenTS ? "true" : "false");

//             amap["Timestamp"]->setParam("lastTimestamp", currentTimestamp);

//             //if(!seenTS)
//             {
//                 if (0)FPS_ERROR_PRINT("%s >> %s Timestamp set TSseenTime
//                 %2.3f \n"
//                     , __func__, aname, tNow);
...
//         }
//         else   // No Change , start tracking faults and alarms  but wait for
//         hold time
//         {
...
//                 {
//                     if (0)FPS_ERROR_PRINT("%s >> %s Timestamp stall detected
//                     tNow %2.3f seebTime %2.3f .stalll time %2.3f toHold %2.3f
//                     \n"
//                         , __func__, aname, tNow,
//                         TSseenTime, (tNow - TSseenTime), toHold);
...
//                 amap["TsStateNum"]->setVal(ival); ival = 0;
//                 amap["TsInit"]->setVal(ival);
//                 char* tval;
//                 asprintf(&tval, " Ts OK last set  Alarm %3.2f max %3.2f",
...
//             if (0)FPS_ERROR_PRINT("%s >>  Timestamp stall for  %s at %2.3f
//             rdFault %2.3f rdAlarm %2.3f TSOk [%s] seenTS [%s] tDiff %2.3f \n"
//                 , __func__, aname, tNow, rdFault,
//                 rdAlarm, TSOk ? "true" : "false", seenTS ? "true" : "false",
//                 tDiff);
//             if (rdFault > 0.0)
//             {
//                 rdFault -= tDiff;
...
//                 tval; asprintf(&tval, " Ts Fault last set Alarm %3.2f max
//                 %3.2f", toAlarm, toFault); if (tval)
//                 {
//                     amap["TsState"]->setVal(tval);
//                     free((void*)tval);
//                 }
...
//                 int totalTsFaults =
//                 amap["Timestamp"]->getiParam("totalTsFaults");
//                 totalTsFaults++;
...
//             {
//                 if (1)FPS_ERROR_PRINT("%s >> Timestamp  Alarm  for %s at
//                 %2.3f \n", __func__, aname, tNow);

//                 char* tval;
//                 asprintf(&tval, "Ts Alarm last set Alarm %3.2f max %3.2f",
//                 toAlarm, toFault); if (tval)
//                 {
//                     amap["TsState"]->setVal(tval);
//                     free((void*)tval);
...
//                 amap["Timestamp"]->setParam("seenOk", false);
//                 int totalTsAlarms =
//                 amap["Timestamp"]->getiParam("totalTsAlarms");
//                 totalTsAlarms++;
...
//             else
//             {
//                 if (0)FPS_ERROR_PRINT("%s >> Ts for [%s] [%s] Stalled at
 ...
 */
// BMS Single Cell Charge Over Voltage max 3.60 for 8 seconds Open DC Breaker
// (latched)  BMS Single Cell Charge Over Voltage less than 3.55 for 10 seconds
// not in charging state reset DC Breaker (latched)
...
// I have yet to get these triggered from a config function but I'm working on
// it.
...
// the MaxBusBarCurrent is the root name and all the other components are based
// on that toot. then call the standard function CheckOver/UnderLimits with the
// same "MaxBusBarCurrent" the aname holds it alltogether. The key is to get the
// order right in the setup function. I'll think about a way to get the
// SetupFunction automaticaly called if needed. done it see checkover

// SetupLimitsVec(*am->vmap, am->amap, avmap, am->name.c_str(),
// "MaxBusBarCurrent", am);  CheckOverLimitsVec(*am->vmap, avmap,
// am->name.c_str(), "MaxBusBarCurrent", am->p_fims, am);
...
// std::map<std::string, std::map<std::string,   std::vector<assetVar*>>>
// AvMaxCellVoltage;
int SetupLimitsVec(varsmap& vmap, varmap& amap, avarmap& avmap, const char* aname, const char* vname,
                   asset_manager* am);
/**
 * @brief Periodically check if the variable contains a value that is greater
 * than the limit value after a given time. If this is true, proceed to run
 * fault-handling functions and then recover
 *
 ...
 */
int CheckOverLimitsVec(varsmap& vmap, avarmap& avmap, const char* aname, const char* vname, fims* p_fims,
                       asset_manager* am)
{
    ...
    // help us transition to fault state . Latch the value set
    //             ***
    // ***********************************************ValueLimit****************************
    //          *      *
    //        *          *
    ...
    if (0)
        FPS_ERROR_PRINT(
            ...
            ValueFault ? "true" : "false", ValueErrTime, ValueResetTime, eSim);
    //////////////////////////////////////////////////////////////////////////////////
    if (Sim == 1)
    {
    ...
    if (Sim != 0)
    {
        if (1)
            FPS_ERROR_PRINT(
                "%s >> Value [%s] [%f]->[%f] Seen [%s] OK [%s] Fault "
                "[%s] ErrTime %f  ResetTime %f \n",
                __func__, ValueAv->name.c_str(), ValueAv->getdLVal(), ValueAv->getdVal(), ValueSeen ? "true" : "false",
                ValueOK ? "true" : "false", ValueFault ? "true" : "false", ValueErrTime, ValueResetTime);
    }
...
// typedef std::map<std::string, std::map<std::string, std::vector<assetVar* >>>
// avarmap;
/**
 * @brief Periodically check if the variable contains a value that is less than
 * the limit value after a given time. If this is true, proceed to run
 * fault-handling functions and then recover
 *
 * @param vmap the global data map all assets/asset managers have access to
 * @param avmap the vector that contains asset vars
 ...
 * @param am the asset manager
 */
int CheckUnderLimitsVec(varsmap& vmap, avarmap& avmap, const char* aname, const char* vname, fims* p_fims,
                        asset_manager* am)
{
    using namespace LimitsCheck;
    ...
    // exceeded ValueLimit ValueOK set by the assetVar holding the Valueage
    // component value when it went below ValueReset ValueFault is the assetVar
    // indicating that we have a fault.
    if (reload < 2)
    {
        // SetupValueage
    }
    ...
    if (Sim != 0)
    {
        if (1)
            FPS_ERROR_PRINT(
                "%s >> Sim [%d] Value [%s] [%f]->[%f] Limit [%f] Reset [%f] Seen "
                "[%s] OK [%s] Fault [%s] ErrTime %f  ResetTime %f \n",
                __func__, Sim, ValueAv->name.c_str(), ValueAv->getdLVal(), ValueAv->getdVal(), ValueLimit, ValueReset,
                ValueSeen ? "true" : "false", ValueOK ? "true" : "false", ValueFault ? "true" : "false", ValueErrTime,
                ValueResetTime);
        // fix crazy stuff
    ...
    // // If the current variable is greater than the reset value, then we need to
    // be
    // // aware of this. This will help us transition out of fault state
    // if (!ValueOK)
    // {
    ...
    // // than the limit value after a given time, then reset variables and
    // transition
    // // out of fault state
    // if (ValueFault)
    // {
...
int SetupLimitsVec(varsmap& vmap, varmap& amap,
                   std::map<std::string, std::map<std::string, std::vector<assetVar*>>>& avmap, const char* aname,
                   const char* vname, asset_manager* am)
{
    using namespace LimitsCheck;
    ...
    if (reload < 2)
    {
        ...
        // "Value",  dval);  eErrTime, eResetTime,
        mvar = mname + "ErrCfg";
        amap[mvar.c_str()] = vm->setLinkVal(vmap, aname, "/config", mvar.c_str(), dval);
        avmap[aname][vname].push_back(amap[mvar.c_str()]);
        mvar = mname + "ResetCfg";
        ...
        FPS_ERROR_PRINT(
            "%s >> use: fims_send -m set -r /$$ -u /config/%s "
            "'{\"%sSim\":true}' to run Simulation \n",
            __func__, aname, vname);
    }
    return 0;
}
// MaxCellVoltSeen set by the assetVar holding the MaxCellVoltage component
// value when it exceeded MaxCellVoltLimit MaxCellVoltOK set by the assetVar
// holding the MaxCellVoltage component value when it went below
// MaxCellVoltReset MaxCellVoltFault is the assetVar indicating that we have a
// fault.
...
// int SendAssetCmd(varsmap &vmap, varmap &amap, const char* aname, fims*
// p_fims, asset_manager* am)  int TestAssetCmdAm(varsmap &vmap, varmap &amap,
// const char* aname, fims* p_fims, asset_manager*am)
...
int SetupCmdVec(varsmap& vmap, varmap& amap,
                std::map<std::string, std::map<std::string, std::vector<assetVar*>>>& avmap, const char* aname,
                const char* vname, asset_manager* am)
{
    ...
    assetVar* CheckValue = amap[mvar.c_str()];
    // char *tval = (char *)" Asset Init";
    // int reload = CheckReload(vmap, am->amap, aname,
    // mvar.c_str(),&CheckMaxCellVoltage);
    int reload = 0;
    bool bval = false;
    int ival = -1;
    double dval = 0.0;
    ...
    if (reload < 2)
    {
        ...
        avmap[aname][vname].push_back(amap[mvar.c_str()]);
        mvar = mname + "Cmd";
        amap[mvar.c_str()] = vm->setLinkVal(vmap, aname, "/controls", mvar.c_str(), ival);
        avmap[aname][vname].push_back(amap[mvar.c_str()]);
        mvar = mname + "Resp";
        amap[mvar.c_str()] = vm->setLinkVal(vmap, aname, "/controls", mvar.c_str(), ival);
        avmap[aname][vname].push_back(amap[mvar.c_str()]);
        ...
        // "ValueSeen",  bval);  amap["ValueOK"]               =
        // vm->setLinkVal(vmap, aname, "/status", "ValueOK",  bval);
        mvar = mname + "MaxRetries";
        amap[mvar.c_str()] = vm->setLinkVal(vmap, aname, "/config", mvar.c_str(), dval);
        avmap[aname][vname].push_back(amap[mvar.c_str()]);
        mvar = mname + "RetryCount";
        amap[mvar.c_str()] = vm->setLinkVal(vmap, aname, "/config", mvar.c_str(), dval);
        avmap[aname][vname].push_back(amap[mvar.c_str()]);
...
int SendAssetCmd(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, asset_manager* am)
{
    ...
    // amap["AssetRespTime"]         = vm->setLinkVal(vmap, aname,    "/status",
    // "AssetRespTime",            dval);
    ...
    if (reload < 2)
    {
        ...
        //    here and here
        amap[LAssetStatusSendReg] = vm->setLinkVal(vmap, aname, "/configs", LAssetStatusSendReg,
                                                   tVal);  // statusval  dunno yet
        ...
        reload = 2;
        TestAssetStatusAv->setVal(reload);
    }
    ...
    double tNow = vm->get_time_dbl();
    if (0)
        FPS_ERROR_PRINT(" %s >> AssetStatusVal %d lastAssetStatusVal %d , time  %2.3f  \n", __func__, AssetStatusVal,
                        lastAssetStatusVal, tNow);
    if (AssetStatusVal != lastAssetStatusVal)
    {
        ...
        if (AssetStatusVal != resetAssetStatusVal)
        {
            ...
            // TODO             amap[LAssetStatusSendReg]->setFimsVal(AssetStatusVal,
            // am->p_fims);

            amap[LAssetRespTime]->setVal(tNow);

            vm->sendAssetVar(amap[LAssetStatusSendReg],
                             am->p_fims);  // can use different comp if required
    ...
    if (0)
        FPS_ERROR_PRINT(" %s >> resetAssetStatusVal %d lastAssetStatusVal %d , time  %2.3f  \n", __func__,
                        AssetStatusVal, lastAssetStatusVal, tNow - AssetRespTime);
    ...
    if (0)
        FPS_ERROR_PRINT(" %s >> AssetRespVal %d lastAssetRespVal %d , time  %2.3f  \n", __func__, AssetRespVal,
                        lastAssetRespVal, tNow - AssetRespTime);
    ...
    if (AssetRespVal != lastAssetRespVal)
    {
        ...
        {
            if (1)
                FPS_ERROR_PRINT(" %s >> got correct asset resp %d in %2.3f secs \n", __func__, AssetRespVal, tDiff);
            bool bval = true;
            amap[LAssetRespOk]->setVal(bval);
            bval = false;
            amap[LAssetRespErr]->setVal(bval);
        ...
        {
            if (1)
                FPS_ERROR_PRINT(" %s >> got incorrect asset resp %d in %2.3f secs \n", __func__, AssetRespVal, tDiff);
        }

        amap[LlastAssetRespVal]->setVal(AssetRespVal);
    }
    if (0)
        FPS_ERROR_PRINT(" %s >> AssetRespErr %s AssetRespOk %s , time  %2.3f  \n", __func__,
                        AssetRespErr ? "true" : "false", AssetRespOk ? "true" : "false", tNow - AssetRespTime);
    // not OK and Not Err'd yet check for timeout
    if (!AssetRespErr && !AssetRespOk && (tDiff > tRespTime))
    {
        if (1)
            FPS_ERROR_PRINT(
                " %s >> warning, timeout ..   asset resp %d after %2.3f "
                "secs, attempt %d\n",
                __func__, AssetRespVal, tDiff, maxAssetSendNum - AssetSendNum);
        ...
        if (AssetSendNum == 0)
        {
            if (1)
                FPS_ERROR_PRINT(
                    " %s >> Error, timeout ..   asset resp %d after %2.3f "
                    "secs, attempt %d\n",
                    __func__, AssetRespVal, tDiff, maxAssetSendNum - AssetSendNum);
            amap[LAssetRespErr]->setVal(bval);
            AssetSendNum = -1;
...
int InitAMHeartBeat(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, asset_manager* am)
{
    ...
    int ival = 0;
    amap["HeartBeatErrors"]->setVal(ival);

    return 0;
}
// these guys check to see if the individual bypass bools have been set.
// if so then ignore the comms errors or lack of first comms data preventing the
// system getting into ready if CommsBypass is set  then if asset in Coms Init
// then put it into Comms Ready bypassing the first data then capture CommsErr
// and ifnore the error
int CheckAssetCommsOk(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, asset* ai)
{
    ...
    // if the bypass flag is set we pretend to get the firstComms message and
    // ignore Comms errors
    // return BypassComms;
...
int CheckCommsOk(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, asset_manager* am)
{
    ...
    for (auto ass_man : am->assetManMap)
    {
        ...
        if (0)
            FPS_ERROR_PRINT("%s >>>>>> ESS [%s]  Asset Man [%s] \n ", __func__, aname, amc->name.c_str());
    ...
    for (auto x : am->assetMap)
    {
        asset* ai = x.second;
        if (0)
            FPS_ERROR_PRINT("%s >>>>>> ESS [%s]  Asset [%s]\n ", __func__, aname, ai->name.c_str());
    ...
    // Set comms to ok (to get system into ready state) if all assets' bypass
    // comms have been set
    if (commsReady)
    {
        amap["CommsOk"]->setVal(true);
...
// these guys check to see if the individual bypass bools have been set.
// if so then ignore theheartbeat errors or lack of first heartbeat data
// preventing the system getting into ready
int CheckAssetHeartBeatOk(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, asset* ai)
{
    ...
    // if the bypass flag is set we pretend to get the first HB message and gnore
    // HB errors
    // return BypassHB;
...
int CheckHeartBeatOk(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, asset_manager* am)
{
    ...
    // Set heartbeat state to ok (to get system into ready state) if all assets'
    // bypass comms have been set
    if (hbReady)
    {
        amap["HBOk"]->setVal(true);
    }
...
// },
//                                            { "field": "kacclosecmd", "value":
//                                            true }]},
//                 {"value": 206,"controls": [{ "field": "offcmd", "value": true
//                 },
//                                            { "field": "kacopencmd", "value":
//                                            true }]}],
// "ctrlword2cfg":[{"value": 2,"controls": [{ "field": "kdcclosecmd", "value":
// true }]},
//                 {"value": 1,"controls": [{ "field": "kdcopencmd", "value":
//                 true }]}],
// "ctrlword4cfg": [{"value": 205,"controls": [{ "field": "standbycmd", "value":
// true }]},
...
//   set /components/ess:control_word1 207 > /controls/ess/OnCmd /controls/ess
//   AcContactorCloseCmd set /components/ess:control_word1 208 >
//   /controls/ess/OffCmd /controls/ess AcContactorOpenCmd
...
//    we'll have to finish the current wakeup but apart from that it should be
//    good.
...
// 6/ wait for pcs to respond .....  Phil to provide infrastructure to wait for
// command  completion. 7 /  monitor PCS output and
...
// }; (edited)
// 9:05
// RunMode1 is able to accept power commands, RunMode2 we don't have yet,
// Standby is running but no power features enabled 9:07 Normal behavior will be
// it's in Ready mode, gets a start signal, goes through Startup to RunMode1
//
...
// unlike the CommsState these will be general pupose functions with NO local
// variables
//  So the path is to send a command to the pcr AFTER we have set up the command
//  response to the feedback var from the PCR to run the handle response
//  function.
// Then send the command to thepcr.
// The normall poll function will look for an indication that the response has
// changed and if so that function will be disabled. the response (" I am turned
// ON ") will cause the system to continue to the next phase. IF the poll
// function finds NO response in a timeout period then the system will revert to
// an error state. Lets try and ascii draw this...
//
//   HandleCmd -> sets up function to HandlePCRON ,sends "turn on to PCR",
//   continues polling until we get PCR CMD timeout.
//  if we have issued a "turn ON PCR" command HandlePCR ON will eiher set the
//  state to Ready or Error depending on the response from the PCR status. IF we
//  want, the HandlePCRReady command can be run directly by the asset RunVar
//  command.
//
//
// enum States
...
// int CheckSystemState(varsmap &vmap, varmap &amap, const char *aname, fims*
// p_fims, asset_manager *am)
// {
//     VarMapUtils*vm = am->vm;
//     vm->setTime();
...
//         if ((DcContactor != DcContactorFbk) && ((dval
//         -amap["DcContactor"]->getSetTime()) >respTime))
//         {
//             dval = vm->get_time_dbl();
//             FPS_ERROR_PRINT("%s >> %s --- Running  statenum %d Dc [%s] DcFbk
//             [%s] settime %f lastset %f\n"
...
//             char* fval;
//             asprintf(&fval,"%s  DcContactor Did not respond in %f seconds",
//             aname, dval); if(fval)
//             {
//                 amap["SystemState"]->setVal(fval);
//                 free((void*)fval);
...
//         if ((AcContactor != AcContactorFbk) && ((dval -
//         amap["AcContactor"]->getSetTime()) >respTime) )
//         {
//             dval = vm->get_time_dbl();
//             FPS_ERROR_PRINT("%s >> %s --- Running  statenum %d Ac [%s] AcFbk
//             [%s] settime %f lastset %f\n"
//                     , __func__, aname, sState
...
//             char* fval;
//             asprintf(&fval,"%s  AcContactor Did not respond in %f seconds",
//             aname, dval); if(fval)
//             {
//                 amap["SystemState"]->setVal(fval);
//                 free((void*)fval);
...
// //      100 Warning status
// //      101 Fault status
// int HandleAssetCmd(varsmap &vmap, varmap &amap, const char *aname, fims*
// p_fims, asset* am, int sstate)
// {
//     VarMapUtils * vm = am->vm;
//     printf("%s >> %s --- Running  state %d\n", __func__,
//     aname, sstate); assetVar* HandleAssetCmd = amap["HandleAssetCmd"]; char
//     *tval = (char *)" Asset Init"; int reload = 0; bool bval =  false; int
//     ival = -1; reload = CheckReload(vmap, amap, aname,
//     __func__);

//     if(reload < 2)
//     {
//         // note links must set these values
//         amap["HandleAssetCmd"]    = vm->setLinkVal(vmap, aname, "/controls",
//         "HandleAssetCmd", reload); amap["AssetCmd"]          =
//         vm->setLinkVal(vmap, aname, "/controls", "AssetCmd", ival);
//         amap["AssetOn"]           = vm->setLinkVal(vmap, aname, "/params",
//         "AssetOn", ival); amap["AssetOff"]          = vm->setLinkVal(vmap,
//         aname, "/params",   "AssetOff", ival); amap["AssetStandby"]      =
//         vm->setLinkVal(vmap, aname, "/params",   "AssetStandby", ival);
//         amap["AssetInit"]         = vm->setLinkVal(vmap, aname, "/params",
//         "AssetInit", ival); amap["OnCmd"]             = vm->setLinkVal(vmap,
//         aname, "/controls", "OnCmd", bval); amap["OffCmd"]            =
//         vm->setLinkVal(vmap, aname, "/controls", "OffCmd", bval);
//         amap["StandbyCmd"]        = vm->setLinkVal(vmap, aname, "/controls",
//         "StandbyCmd", bval); amap["AssetState"]        = vm->setLinkVal(vmap,
//         aname, "/status",   "AssetState", tval); amap["BypassHB"]          =
//         vm->setLinkVal(vmap, aname, "/controls", "BypassHB", ival);
//         amap["BypassComms"]       = vm->setLinkVal(vmap, aname, "/controls",
//         "BypassComms", ival); if(reload < 1)
//         {
...
// int HandleManagerCmd(varsmap &vmap, varmap &amap, const char *aname, fims*
// p_fims, asset_manager *am, int sstate)
// {
//     VarMapUtils * vm = am->vm;
//     char* tVal = (char *)" Init";
//     printf("%s >> %s --- Running state %d\n", __func__,
//     aname, sstate); assetVar*HandleManCmd      = amap["HandleManCmd"];
//     printf("%s >> %s --- Running\n", __func__, aname);
//     bool bval = false;
//     int ival = 0;
//     int reload = 0;
...
//     if(reload < 2)
//     {
//         amap["On"]                = vm->setLinkVal(vmap, aname, "/status",
//         "On", bval); amap["Off"]               = vm->setLinkVal(vmap, aname,
//         "/status",   "Off", bval); amap["Standby"]           =
//         vm->setLinkVal(vmap, aname, "/status",   "Standby", bval);
//         amap["Fault"]             = vm->setLinkVal(vmap, aname, "/status",
//         "Fault", bval); amap["ResetFaultCmd"]     = vm->setLinkVal(vmap,
//         aname, "/controls", "ResetFaultCmd", bval); amap["OnCmd"]
//         = vm->setLinkVal(vmap, aname, "/controls", "OnCmd", bval);
//         amap["OffCmd"]            = vm->setLinkVal(vmap, aname, "/controls",
//         "OffCmd", bval); amap["StandbyCmd"]        = vm->setLinkVal(vmap,
//         aname, "/controls", "StandbyCmd", bval); amap["ResetFaultCmd"]     =
//         vm->setLinkVal(vmap, aname, "/controls", "ResetFaultCmd", bval);
//         amap["ResetCmd"]          = vm->setLinkVal(vmap, aname, "/controls",
//         "ResetCmd", bval); amap["GridForming"]       = vm->setLinkVal(vmap,
//         aname, "/status",   "GridForming", bval); amap["GridFollowing"]     =
//         vm->setLinkVal(vmap, aname, "/status",   "GridFollowing", bval);
//         amap["SystemState"]       = vm->setLinkVal(vmap, aname, "/status",
//         "SystemState", tVal); amap["SystemStateNum"]    =
//         vm->setLinkVal(vmap, aname, "/status",   "SystemStateNum", ival);
//         amap["AcContactor"]       = vm->setLinkVal(vmap, aname, "/status",
//         "AcContactor", bval); amap["DcContactor"]       =
//         vm->setLinkVal(vmap, aname, "/status",   "DcContactor", bval);
//         amap["AcContactorFbk"]    = vm->setLinkVal(vmap, aname, "/controls",
//         "AcContactorFbk", bval); amap["DcContactorFbk"]    =
//         vm->setLinkVal(vmap, aname, "/controls", "DcContactorFbk", bval);
//         amap["AcContactorCmd"]    = vm->setLinkVal(vmap, aname, "/controls",
//         "AcContactorCmd", bval); amap["DcContactorCmd"]    =
//         vm->setLinkVal(vmap, aname, "/controls", "DcContactorCmd", bval);

//         amap["CommsOk"]           = vm->setLinkVal(vmap, aname, "/status",
//         "CommsOk", bval); amap["HBOk"]              = vm->setLinkVal(vmap,
//         aname, "/status",    "HBOk", bval);
//         // amap["BMSOk"]             = vm->setLinkVal(vmap, "bms", "/status",
//         "BMSOk", bval);
//         // amap["PCROk"]             = vm->setLinkVal(vmap, "pcr", "/status",
//         "PCROk", bval);
//         // amap["DRCOk"]             = vm->setLinkVal(vmap, "drc", "/status",
//         "DRCOk", bval);
//         // amap["EMMOk"]             = vm->setLinkVal(vmap, "emm", "/status",
//         "EMMOk", bval);
...
//             amap["SystemState"]->setVal(tVal);
//             amap["SystemStateNum"]->setVal(System_Init); // starts init
//             timeout
...
//         sstate); if(0)FPS_ERROR_PRINT(" %s >> manager child [%s]  with %d
...
//     {
//         asset* ami = ix.second;
//         HandleAssetCmd(vmap, ami->amap, ami->name.c_str(), p_fims, ami,
//         sstate);
...
// int HandleFaultShutdown(varsmap &vmap, varmap &amap, const char *aname, fims*
// p_fims, asset_manager *am)
// {
//     printf("%s >> %s --- Running\n", __func__, aname);
//     char* fval;
...
//     if (reload) < 2 )
//     {
//         amap["RunPCRCmd"]          = vm->setLinkVal(vmap, aname, "/status",
//         "RunPCRCmd",     reload); amap["PCRCmd"]             =
//         vm->setLinkVal(vmap, aname, "/components",    "PCRCmd",     reload);

//         if ( reload < 1)
//         {
//             RunPCRCmd =(assetFunc *) new assetFunc(aname);
//             amap["RunPCRCmdFunc"] = (assetVar *)RunPCRCmd;
//             RunPCRCmd->setupRamFunc(HandlePCRCmd,vmap, amap, aname, p_fims,
//             am);
//             // the following causes HandlePCRCmd to be run on eery Set or Pub
//             on PCRCmd
...
// int HandleESSInput(varsmap &vmap, varmap &amap,  const char *aname, fims*
// p_fims, asset_manager *am)
// {
...
//     if(0)
//     {
//         FPS_ERROR_PRINT("%s >> %s --- at Start AcContactorOpenCmd [%s]
//         AcContactorCloseCmd [%s] DcContactorOpenCmd [%s] DcContactorCloseCmd
//         [%s] \n"
//                 , __func__, aname
...
//                 );
//         FPS_ERROR_PRINT("%s >> %s ---          OffCmd [%s] OnCmd [%s]
//         StandbyCmd [%s] \n"
//                 , __func__, aname
//                 , OffCmd?"true":"false"
//                 , OnCmd?"true":"false"
...
//                 );
//         FPS_ERROR_PRINT("%s >> %s ---         readyOkSetCmd [%s]
//         readyOkClearCmd [%s]\n"
//                 , __func__, aname
...
//         FPS_ERROR_PRINT("%s >> %s --- at End AcContactorOpenCmd [%s]
//         AcContactorCloseCmd [%s] DcContactorOpenCmd [%s] DcContactorCloseCmd
//         [%s] \n"
//                 , __func__, aname
//                 , AcContactorOpenCmd?"true":"false"
...
//         = vm->setLinkVal(vmap, aname, "/status",    "HeartBeat", dval);
//         amap["Timestamp"]             = vm->setLinkVal(vmap, aname,
//         "/status",    "Timestamp",               tVal); amap["CommsDummy"]
//         = vm->setLinkVal(vmap, aname, "/status",    "CommsDummy", dval);
//         amap["SimPcsComms"]           = vm->setLinkVal(vmap, aname,
...
//         = vm->setLinkVal(vmap, aname, "/configsim",    "SimBms_1HB", bval);
//         amap["SimBms_2Comms"]         = vm->setLinkVal(vmap, aname,
//         "/configsim",    "SimBms_2Comms",           bval); amap["SimBms_2HB"]
//         = vm->setLinkVal(vmap, aname, "/configsim",    "SimBms_2HB", bval);
//         amap["SimBms_3Comms"]         = vm->setLinkVal(vmap, aname,
//         "/configsim",    "SimBms_3Comms",           bval); amap["SimBms_3HB"]
//         = vm->setLinkVal(vmap, aname, "/configsim",    "SimBms_3HB", bval);
//         amap["SimBms_4Comms"]         = vm->setLinkVal(vmap, aname,
//         "/configsim",    "SimBms_4Comms",           bval); amap["SimBms_4HB"]
//         = vm->setLinkVal(vmap, aname, "/configsim",    "SimBms_4HB", bval);
//           dval = 1.0;
//         amap["HeartBeatPeriod"]       = vm->setLinkVal(vmap, aname,
//         "/config",    "HeartBeatPeriod",         dval); dval = 255.0;
//         amap["HeartBeatMax"]          = vm->setLinkVal(vmap, aname,
//         "/config",    "HeartBeatMax",            dval);
...
// how do we have multiples of these running ??? maybe give the asset its own
// amap... or  use the param list
...
//                                         "func": [{
//                                                         "amap": "ess",
//                                                         "sendCmd":"/controls/bms_1/start_stop_cmd",
//                                                         << send requested
//                                                         command here
//                                                         "expCmdResponse":"/controls/bms_1/start_stop_resp",
//                                                         << place expected
//                                                         resp here "resetCmd":
//                                                         0,
//                                                         << after send reset
...
//                                                         "CmdRetries": 5,
//                                                         <<  try 5 times
//                                                         "sendVal":"/controls/bms_1/start_stop"
//                                                         << where the fimssend
//                                                         goes to
//                                                         "respVal":"/controls/bms_1/bms_status"
//                                                         << monitor response
//                                                         here "enable":
//                                                         "/controls/ess:start_enable",
//                                                         "func":
//                                                         "TestAssetStatusAv",
//                                                         "onErr":
//                                                         "/controls/ess/startErr",
//                                                         "onOK":
//                                                         "/controls/ess/startOK"
//                                                 }]
//                                 }
//                         }
//                 }
    ...
    if (dval > 0.0)
    {
        if (tNow - dval < 1.0)
        {
            if (tNow - dval > 0.5)
            {
                ...
                vm->sendAssetVar(amap[LAssetRespVal],
                                 am->p_fims);  // can use different comp if required
            }
...
int TestAssetCmdAv(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* av)
{
    ...
    vm->sendAssetVar(amap[LAssetStatusVal],
                     av->am->p_fims);  // can use different comp if required
    return 0;
}

// // just run this and the commands should all fall into place ( ie crash)
// // this waits  for coms and hartbeat  and then waits for the assets to get
// into a ready mode.
...
// // we'll have the last requested sate and power command but that is for a
// different sprint.
// // this guy only handles the cold start.
// // so we force the assets into subordination.
// // then wait for a command.
// //
// int HandleESSCmd(varsmap &vmap, varmap &amap,  const char *aname, fims*
// p_fims, asset_manager* am)
// {
//     int rc = 0;
//     bool bval = false;
...
//     reload = vm->CheckReload(vmap, amap, aname,
//     __func__);
...
//         amap["On"]                = vm->setLinkVal(vmap, aname, "/status",
//         "On",                bval); amap["On2"]                =
//         vm->setLinkVal(vmap, aname, "/status",   "On2",               bval);
//         amap["Off"]               = vm->setLinkVal(vmap, aname, "/status",
//         "Off",               bval); amap["Standby"]           =
//         vm->setLinkVal(vmap, aname, "/status",    "Standby",           bval);
//         amap["Fault"]             = vm->setLinkVal(vmap, aname, "/status",
//         "Fault",             bval); amap["ResetFaultCmd"]     =
//         vm->setLinkVal(vmap, aname, "/controls",  "ResetFaultCmd",     bval);
//         amap["OnCmd"]             = vm->setLinkVal(vmap, aname, "/controls",
//         "OnCmd",             bval); amap["OffCmd"]            =
//         vm->setLinkVal(vmap, aname, "/controls",  "OffCmd",            bval);
//         amap["StandbyCmd"]        = vm->setLinkVal(vmap, aname, "/controls",
//         "StandbyCmd",        bval); amap["ResetFaultCmd"]     =
//         vm->setLinkVal(vmap, aname, "/controls",  "ResetFaultCmd",     bval);
//         amap["ResetCmd"]          = vm->setLinkVal(vmap, aname, "/controls",
//         "ResetCmd",          bval); amap["GridForming"]       =
//         vm->setLinkVal(vmap, aname,   "/status",  "GridForming",     bval);
//         amap["GridFollowing"]     = vm->setLinkVal(vmap, aname,   "/status",
//         "GridFollowing",   bval); amap["SystemState"]       =
//         vm->setLinkVal(vmap, aname,   "/status",  "SystemState",     tVal);
//         amap["SystemStateNum"]    = vm->setLinkVal(vmap, aname,   "/status",
//         "SystemStateNum",     ival); amap["lastSystemStateNum"] =
//         vm->setLinkVal(vmap, aname,  "/status",  "lastSystemStateNum", ival);
//         amap["AcContactor"]       = vm->setLinkVal(vmap, aname,   "/status",
//         "AcContactor",      bval); amap["DcContactor"]       =
//         vm->setLinkVal(vmap, aname,   "/status",  "DcContactor",       bval);
//         amap["AssetStateNum"]        = vm->setLinkVal(vmap, "ess", "/status",
//         "AssetStateNum",     ival); amap["CommsStateNum"]        =
//         vm->setLinkVal(vmap, "ess", "/status",   "CommsStateNum",     ival);
//         amap["HeartBeatStateNum"]    = vm->setLinkVal(vmap, "ess", "/status",
//         "HeartBeatStateNum", ival); amap["CommsState"]           =
//         vm->setLinkVal(vmap, "ess", "/status",   "CommsState",        tVal);
//         amap["AssetState"]           = vm->setLinkVal(vmap, "ess", "/status",
//         "AssetState",        tVal); amap["HeartBeatState"]       =
//         vm->setLinkVal(vmap, "ess", "/status",   "HeartBeatState",    tVal);
//         amap["SystemState"]           = vm->setLinkVal(vmap, "ess",
//         "/status",   "SystemState",       tVal); amap["SystemStateStep"]
//         = vm->setLinkVal(vmap, "ess", "/status",   "SystemStateStep", tVal);
//         amap["CurrentSetpoint"]       = vm->setLinkVal(vmap, "ess",
//         "/controls",   "CurrentSetpoint",   dval); amap["PcsCurrentSetpoint"]
//         = vm->setLinkVal(vmap, "ess", "/status",     "PcsCurrentSetpoint",
//         dval);

//         amap["AcContactorCloseCmd"]  = vm->setLinkVal(vmap, aname,
//         "/controls", "AcContactorCloseCmd", bval);
//         amap["DcContactorCloseCmd"]  = vm->setLinkVal(vmap, aname,
//         "/controls", "DcContactorCloseCmd", bval); amap["AcContactorOpenCmd"]
//         = vm->setLinkVal(vmap, aname, "/controls", "AcContactorOpenCmd",
//         bval); amap["DcContactorOpenCmd"]   = vm->setLinkVal(vmap, aname,
//         "/controls", "DcContactorOpenCmd", bval);

//         amap["CommsOk"]           = vm->setLinkVal(vmap, aname, "/status",
//         "CommsOk", bval); amap["HBOk"]              = vm->setLinkVal(vmap,
//         aname, "/status",    "HBOk", bval); amap["BMSOk"]             =
//         vm->setLinkVal(vmap, "bms", "/status",    "BMSOk", bval);
//         amap["PCROk"]             = vm->setLinkVal(vmap, "pcr", "/status",
//         "PCROk", bval); amap["DRCOk"]             = vm->setLinkVal(vmap,
//         "drc", "/status",    "DRCOk", bval); amap["EMMOk"]             =
//         vm->setLinkVal(vmap, "emm", "/status",    "EMMOk", bval);
//         amap["readyOk"]           = vm->setLinkVal(vmap, aname, "/status",
//         "readyOk", bval); amap["readyOkSetCmd"]     = vm->setLinkVal(vmap,
//         aname, "/controls",  "readyOkSetCmd", bval); amap["readyOkClearCmd"]
//         = vm->setLinkVal(vmap, aname, "/controls",  "readyOkClearCmd", bval);
//         amap["SimPCSCommsCmd"]   = vm->setLinkVal(vmap, aname, "/controls",
//         "SimPCSCommsCmd", bval); amap["SimPCSHBCmd"]      =
//         vm->setLinkVal(vmap, aname, "/controls",  "SimPCSHBCmd", bval);
//         amap["PcsCmd"]           = vm->setLinkVal(vmap, aname, "/controls",
//         "PcsCmd", ival); amap["PcsStatus"]        = vm->setLinkVal(vmap,
//         aname, "/status",    "PcsStatus", ival);
...
//             runEssInputFunc->setupRamFunc(HandleESSInput,vmap, amap, aname,
//             p_fims, am); amap["readyOkSetCmd"]       ->SetPubFunc =
//             (assetVar* )runEssInputFunc; amap["readyOkClearCmd"]
//             ->SetPubFunc = (assetVar* )runEssInputFunc;
//             amap["AcContactorCloseCmd"] ->SetPubFunc = (assetVar*
//             )runEssInputFunc; amap["AcContactorOpenCmd"]  ->SetPubFunc =
//             (assetVar* )runEssInputFunc; amap["DcContactorCloseCmd"]
//             ->SetPubFunc = (assetVar* )runEssInputFunc;
//             amap["DcContactorOpenCmd"]  ->SetPubFunc = (assetVar*
//             )runEssInputFunc; amap["OnCmd"]               ->SetPubFunc =
//             (assetVar* )runEssInputFunc; amap["OffCmd"]
//             ->SetPubFunc = (assetVar* )runEssInputFunc; amap["StandbyCmd"]
//             ->SetPubFunc = (assetVar* )runEssInputFunc;
//         }
...
//             amap["SystemState"]->setVal(tVal);
//             amap["SystemStateNum"]->setVal(System_Init); // starts init
//             timeout amap["lastSystemStateNum"]->setVal(System_Startup); //
//             starts init timeout
...
//     if(lastState != sState)
//     {
//         FPS_ERROR_PRINT("%s >> %s ---ssnum %d --> %d  OnCmd [%s] On [%s] last
//         set time %f\n"
//                 , __func__
//                 , aname
//                 , lastState
...
//     if(0)FPS_ERROR_PRINT("%s >> [%s] Looking for System Startup got [%s] Step
//     [%s] CommsState [%s] num %d time %f\n"
//                     , __func__
//                     , aname
//                     , sVal2
//                     , sValStep2
...
//     {
//         if(0)FPS_ERROR_PRINT("  %s >> [%s] Found System Startup [%s]  Step
//         [%s] CommsState [%s] HeartBeatState [%s] AssetState [%s] time %f\n"
//                     , __func__
//                     , aname
//                     , sVal2
//                     , sValStep2
...
//         {
//             if(0)FPS_ERROR_PRINT("    %s >> [%s] Looking for CommsOK got %d
//             time %f\n"
//                     , __func__
//                     , aname
//                     , ival5
...
//             if (ival5 == Asset_Ok)
//             {
//                 FPS_ERROR_PRINT("%s >> [%s] CommsOK looking for HeartBeat at
//                 time %f\n"
//                     , __func__
//                     , aname
//                     , dval
...
//             if (ival5 == Asset_Ok)
//             {
//                 FPS_ERROR_PRINT("%s >> [%s] HeartBeat OK Reset Assets at time
//                 %f\n"
//                     , __func__
//                     , aname
//                     , dval
//                     );
//                 FPS_ERROR_PRINT("%s >> send \"Assets Ready\" to
//                 /status/ess/AssetState to continue\n"
//                     , __func__
//                     , aname
//                     , dval
//                     );
...
//             {
//                 FPS_ERROR_PRINT("%s >> [%s] Assets OK move to ready at time
//                 %f\n"
//                     , __func__
//                     , aname
//                     , dval
...
//         {
//             // from here we can go to "System On" or "System Standby" or
//             "System Fault'" OnCmd           = amap["OnCmd"]->getbVal();
//             //On              = amap["On"]->getbVal();
//             if (OnCmd)
//             {
//                 FPS_ERROR_PRINT("%s >> [%s]Got On Cmd at time %f Check
//                 DcContactor and SOC etc\n"
//                     , __func__
//                     , aname
//                     , dval
//                     );
...
//             if (DcContactor)
//             {
//                 FPS_ERROR_PRINT("%s >> [%s] DCContactor closed at time %f
//                 Check Current Setpoint and SOC etc\n"
//                     , __func__
//                     , aname
//                     , dval
//                     );
...
//             {
//                 FPS_ERROR_PRINT("%s >> [%s] currentSetpoint set at %f at time
//                 %f Check Pcstatus feedback and SOC etc\n"
//                     , __func__
//                     , aname
//                     , currentSetpoint
//                     , dval
...
//             int pcsStatus = amap["PcsStatus"]->getiVal();            // we
//             can get faults int pcsOkStatus = 1; if (pcsStatus == pcsOkStatus)
//             {
//                 FPS_ERROR_PRINT("%s >> [%s] PcsStatus %d at time %f move to
...
//             int pcsStatus = amap["PcsStatus"]->getiVal();            // we
//             can get faults int pcsOkStatus = 1;
//             // TODO check SOC and capacity
//             if (pcsStatus != pcsOkStatus)
//             {
//                 FPS_ERROR_PRINT("%s >> [%s] PcsStatus %d at time %f move to
//                 System Fault, check SOC etc\n"
//                     , __func__
//                     , aname
//                     , pcsStatus
//                     , dval
...
//         amap["SystemStateStep"]->setVal(sValStep2);
//         // from here we can go to "System On" or "System Standby" but we have
//         to get the Fault Reset Command
//         // from here we can go to "System On" or "System Standby" or "System
//         Fault'" bool resetFaultCmd           =
//         amap["ResetFaultCmd"]->getbVal();

//         if (resetFaultCmd)
//         {
//             resetFaultCmd  = false ;;
//             amap["resetFaultCmd"]->setVal(resetFaultCmd); sValStep2 = (char
//             *)"Waiting for Command";
//             amap["SystemStateStep"]->setVal(sValStep2);
//             sValStep2 =  (char *)"System Ready";
//             amap["SystemState"]->setVal(sValStep2);
...
//     if(0)FPS_ERROR_PRINT("%s >> %s -After Check  -- SystemState [%s] rc %d
//     nCmd [%s] On [%s] Standby [%s] readyOk [%s] AcContactor [%s] DcContactor
//     [%s] go  [%s] go2  [%s]\n"
//                 , __func__
//                 , aname
//                 , sVal ? sVal:"NotSet"
...
//                 , DcContactor?"true":"false"
//                 , (OnCmd && !On )?"Go":"NoGo"
//                 , (OnCmd && (!On || Standby) && AcContactor &&
//                 DcContactor)?"Go":"NoGo"
//                 );
//     if (rc != 0)
//     {
//         if(1)FPS_ERROR_PRINT("%s >> %s -Running Fault Shutdown rc %d -- OnCmd
//         [%s] On [%s] readyOk [%s] \n"
//                 , __func__
//                 , aname
//                 , rc
...
//             OnCmd           = amap["OnCmd"]->getbVal();
//             On              = amap["On"]->getbVal();
//             if(1)FPS_ERROR_PRINT("%s >> %s -After OnCmd Process rc %d --
//             OnCmd [%s] On [%s]\n"
//                     , __func__
//                     , aname
//                     , rc
...
//             OnCmd           = amap["OnCmd"]->getbVal();
//             On              = amap["On"]->getbVal();
//             if(0)FPS_ERROR_PRINT("%s >> %s -After Handle Manager rc %d --
//             OnCmd [%s] On [%s]\n"
//                     , __func__
//                     , aname
//                     , rc
...
// THis is run from the setVar action oNSET for estop .. yet to be completed put
// in place. we Simply register an ESTOP assetVar and run it....
//  We can call this function and if ESTOP is not asserted we can just proceed
//  with the setup
// NOTE that, during setup,  the assetvar will be given a reference to the asset
// manager here and a wakeup number the incoming fims messsage will call
// vm.setvar. this will then trigger a channel wakeup. the next thing the asset
// manager will do is service that wakeup, from a channel put, after processing
// the fims message. this will probabbly respond in about 100 uSecs or so.
...
int HandleEStop(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, asset_manager* am)
{
    ...
    {
        if (1)
            FPS_ERROR_PRINT("%s >> for [%s]  reload [%d]  amap[HandleEStop]  %p amap %p\n", __func__, aname, reload,
                            (void*)amap["HandleEStop"], (void*)&amap);

        // double warnVal = 3.5;
        // //double errVal = 5.0;
        // double pper = 0.250;
        // amap["CheckComms"]         = vm->setLinkVal(vmap, aname, "/reload",
        // "CheckComms",     reload); amap["CheckCommsPeriod"]   =
        // vm->setLinkVal(vmap, aname, "/config",    "CheckCommsPeriod",  pper);
        // amap["CheckCommsRun"]      = vm->setLinkVal(vmap, aname, "/config",
        // "CheckCommsRun",     dval);
        amap["EStopCmd"] = vm->setLinkVal(vmap, "ess", "/component", "EStopCmd", bval);
        amap["EStop"] = vm->setLinkVal(vmap, "ess", "/component", "EStop", bval);
        // amap["essCommsWarns"]      = vm->setLinkVal(vmap, "ess", "/status",
        // "essCommsWarns",     ival); if(am->am)
        // {
        //     amap["amCommsErrors"]     = vm->setLinkVal(vmap,
        //     am->am->name.c_str(), "/status",    "CommsErrors",    ival);
        //     amap["amCommsWarns"]      = vm->setLinkVal(vmap,
        //     am->am->name.c_str(), "/status",    "CommsWarns",     ival);
        // }
        // amap["CommsErrors"]        = vm->setLinkVal(vmap, aname, "/status",
        // "CommsErrors",       ival); amap["CommsWarns"]         =
        // vm->setLinkVal(vmap, aname, "/status",    "CommsWarns",        ival);
        amap["EStopState"] = vm->setLinkVal(vmap, "ess", "/status", "EStopState", csVal);

        // // amap["errCommsTimeout"]    = vm->setLinkVal(vmap, aname, "/config",
        // "errCommsTimeout",   errVal);
        // // amap["warnCommsTimeout"]   = vm->setLinkVal(vmap, aname, "/config",
        // "warnCommsTimeout",  warnVal);
        ...
        if (reload == 0)  // complete restart
        {
            ...
            amap["EStopCmd"]->setVal(bval);  //     = vm->setLinkVal(vmap, "ess",
                                             //     "/component",    "EStopCmd", bval);
            amap["EStop"]->setVal(bval);     //        = vm->setLinkVal(vmap, "ess",
                                             //        "/component",    "EStop",    bval);
        }
        ival = 2;
        amap[__func__]->setVal(ival);
    }
    ...
    if (!am->am)
    {
        bool esCmd = false;
        bool esIn = false;
        esCmd = amap["EStopCmd"]->getbVal();  //     = vm->setLinkVal(vmap, "ess",
                                              //     "/component",    "EStopCmd",
                                              //     bval);
        esIn = amap["EStop"]->getbVal();      //        = vm->setLinkVal(vmap, "ess",
                                              //        "/component",    "EStop", bval);

        if (1)
            FPS_ERROR_PRINT("%s >> OK Test Again  EStop >>esCmd: %d esIn: %d \n", __func__, esCmd, esIn);
        if (!esCmd && esIn)
        {
            amap["EStopCmd"]->setVal(true);  //     = vm->setLinkVal(vmap, "ess",
                                             //     "/component",    "EStopCmd", bval);
        }
        else
        {
    ...
    if (runme)
    {
        ...
        if (am->am)
        {
            ...
            // if(1)FPS_ERROR_PRINT("%s >>>>>> AM [%s]  Manager [%s] warns %d errs
            // %d\n "
            //         ,__func__
            //         , aname
            //         , am->am->name.c_str()
...
//   char* _aname, fims* _p_fims, asset_manager *_am)
//  This only runs when we get a set or Pub  PCRCmd ( Default
//  /components/pcr/PCRCmd)
// int HandlePCRCmd(varsmap &vmap, varmap &amap, const char* aname, fims*
// p_fims, asset_manager *am)
// {
//     VarMapUtils* vm;
//     vm = am->vm;
...
//     if (reload < 2 )
//     {
//         amap["PCRCmd"]             = vm->setLinkVal(vmap, aname,
//         "/components",    "PCRCmd",        ival); amap["PCRCmdStatus"]
//         = vm->setLinkVal(vmap, aname, "/status",        "PCRCmdStatus",
//         tVal); if(reload < 1)
//         {
//             // do init stiff here
//             amap["PCRCmdStatus"]->setVal(tVal); //       =
//             vm->setLinkVal(vmap, aname, "/status",    "PCRCmdStatus", tVal);
//             amap["PCRCmd"]->setVal(ival); //       = vm->setLinkVal(vmap,
//             aname, "/status",    "PCRCmdStatus",     tVal);
...
//     "/status",    "PCRCmdStatus",     tVal);

//     asprintf(&cval, "%s >> Set PCRCmd Status to %d at
//     %f",__func__, ival, cmdTime); if(cval)
//     {
//         amap["PCRCmdStatus"]->setVal(cval); //       = vm->setLinkVal(vmap,
//         aname, "/status",    "PCRCmdStatus",     tVal); free((void *)cval);
//     }
//     // Now do stuff to handle change in PCSCmd
//     return 0;
// }

// this sets upthe PCRCmd to run the HandlePCRCmd function every time the PCRCmd
// is "set" this instance must be set up by the asset manager this really should
// be an init function. to test set a value in /components/ess/PCRCmd  and check
// /ess/status/ess/PCRCmdStatus

// int SetupRunPCRCmd(varsmap &vmap, varmap &amap, const char* aname, fims*
// p_fims, asset_manager *am)
// {

//     FPS_ERROR_PRINT(">>>>>>>>>ESS>>>>>>>>>>>%s running for ESS
//     Manager\n",__func__);
...
//     if (reload < 2)
//     {
//         //amap["SetupRunPCRCmd"]     = vm->setLinkVal(vmap, aname, "/reload",
//         "SetupRunPCRCmd",     reload); amap["RunPCRCmd"]          =
//         vm->setLinkVal(vmap, aname, "/status",        "RunPCRCmd", reload);
//         amap["PCRCmd"]             = vm->setLinkVal(vmap, aname,
//         "/components",    "PCRCmd",     reload);
