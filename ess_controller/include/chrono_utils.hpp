...
// copy and paste this macro into your test files so you don't have to define
// things over and over. for release, do NOT include this macro and compile
// using chrono_utils.cpp (or .o) with the extern variables defined once right
// before your main -> please define epoch before base_time inside the flex
// namespace #define CHRONO_UTILS_SINGLE_INCLUDE

// loss-less conversion between durations as double representation
namespace flex
{
// NOTE: might need to change these to long doubles to prevent overflow, they
// seem good for now
...
{
    static_assert(std::is_same<TimeType, dbl_nano>::value || std::is_same<TimeType, dbl_micro>::value ||
                      std::is_same<TimeType, dbl_milli>::value || std::is_same<TimeType, dbl_sec>::value,
...
// new template version, default is in seconds. Template version makes it fast
// and safe
template <typename TimeType = dbl_sec>
