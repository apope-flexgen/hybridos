...
#include "Ouroboros.hpp"  // important, do NOT remove
...
struct event_time_filename_calculator
{
    static spdlog::filename_t calc_filename(const spdlog::filename_t& filename, bool add_timestamp)
    {
        ...
        if (index == basename.npos)
        {
            ...
            {
                return fmt::format(SPDLOG_FILENAME_T("{:%F}_{:%H:%M:%S}_{}{}"), now,
                                   now.time_since_epoch() + std::chrono::hours(20), basename, ext);
            }
        }
        ...
        {
            return fmt::format(SPDLOG_FILENAME_T("{}{:%F}_{:%H:%M:%S}_{}{}"), folder_path, now,
                               now.time_since_epoch() + std::chrono::hours(20), basename, ext);
        }
    }
};
...
public:
    ...
    void set_backtrace(std::size_t size) { buf_.reSize(size); }

    // actually dumps current messages to file
    void log_it_(const spdlog::filename_t& fileName, bool add_timestamp = false) { rotate_(fileName, add_timestamp); }
...
protected:
    ...
    // todo: default file name is not setup right now, maybe maybe will include
    // this feature.
...
public:
    ...
    void setBacktrace(std::size_t size) { mSink->set_backtrace(size); }

    // neat trick for flushing the previous logs and starting fresh, almost no
    // performance cost. NOTE: it is your responsibility to set size > 0 and not
    // flush other people's messages unecessarily.
    ...
    void setFormatter(std::unique_ptr<spdlog::formatter> formatter) { mLogger.set_formatter(std::move(formatter)); }
