...
namespace Math
{
    ...
    {
        "+", "-", "*", "/", "%", "avg", "pctOf", "max", "min", "sqrt", "scale", "and", "or", "stddev", ">", "<"
    };

    inline double add(const std::vector<double>& operands)
    {
        double result = 0;
        for (const double& val : operands) { result += val; }
        return result;
    }
    ...
    inline double divide(const std::vector<double>& operands)
    {
        ...
        for (const double& val : operands)
        {
            ...
            {
                if (0) FPS_PRINT_ERROR("Division by 0");
                if (0) ESSLogger::get().error("[zero division] on [{}] in [{}]",
                    result, __func__);
                return 0;
            }
            else
    ...
    inline double modulus(const std::vector<double>& operands)
    {
        ...
        for (const double& val : operands)
        {
            ...
            {
                if (0) FPS_PRINT_ERROR("Modulus division by 0");
                if (0) ESSLogger::get().error("[modulus zero division] on [{}] in [{}]",
                    result, __func__);
                return 0;
            }
            else
    ...
    inline double scale(double val, double scale) { return scale > 0 ? val / scale : val; }

    inline double squareRoot(double val) { return val > 0 ? sqrt(val) : 0; }

    inline double max(const std::vector<double>& operands) 
    {
        if (operands.size() <= 0)
        {
            FPS_PRINT_DEBUG("operands is empty");
            return 0;
        }
    ...
    inline double stddev(const std::vector<double>& operands)
    {
        if (operands.size() <= 0)
        {
            FPS_PRINT_WARN("operands is empty");
            return 0;
        }
...
namespace Expression
{
    ...
    enum class Associativity { none, leftAssociative, rightAssociative };
    struct OperatorInfo { Precedence prec; Associativity assoc; };

    enum DataType {NUMERIC, BOOL, STRING};
    struct Operand
    {
        DataType type;
        double dval;
    ...
    // The map of all operators and the corresponding precedence and associativity rules
    static std::map<Operator, OperatorInfo> operatorMap = 
    {
        {"!",  { 12, Associativity::rightAssociative}},
        {"not",{ 12, Associativity::rightAssociative}},
        {"~",  { 12, Associativity::rightAssociative}},
        {"**", { 11, Associativity::rightAssociative}},
        {"*",  { 10, Associativity::leftAssociative }},
        {"/",  { 10, Associativity::leftAssociative }},
        {"%",  { 10, Associativity::leftAssociative }},
        {"+",  { 9, Associativity::leftAssociative  }},
        {"-",  { 9, Associativity::leftAssociative  }},
        {"<<", { 8, Associativity::leftAssociative  }},
        {">>", { 8, Associativity::leftAssociative  }},
        {"<",  { 7, Associativity::leftAssociative  }},
        {"<=", { 7, Associativity::leftAssociative  }},
        {">",  { 7, Associativity::leftAssociative  }},
        {">=", { 7, Associativity::leftAssociative  }},
        {"==", { 6, Associativity::leftAssociative  }},
        {"!=", { 6, Associativity::leftAssociative  }},
        {"&",  { 5, Associativity::leftAssociative  }},
        {"^",  { 4, Associativity::leftAssociative  }},
        {"|",  { 3, Associativity::leftAssociative  }},
        {"&&", { 2, Associativity::leftAssociative  }},
        {"and",{ 2, Associativity::leftAssociative  }},
        {"||", { 1, Associativity::leftAssociative  }},
        {"or", { 1, Associativity::leftAssociative  }},
    };
     ...
     */
    inline bool isBoolean(const std::string& token) { return token == "true" || token == "false"; }
     ...
     */
    inline bool isOperator(const std::string& token) { return operatorMap.find(token) != operatorMap.end(); }
     ...
     */
    inline bool isFunction(const std::string& token) { return std::find(funcs.begin(), funcs.end(), token) != funcs.end(); }
     ...
     */
    inline int getPrecedence(const std::string& oper) { return operatorMap.find(oper) != operatorMap.end() ? operatorMap[oper].prec : -1; }
    ...
    inline bool evaluate(std::stack<Expression::Operator>& operators, std::stack<Expression::Operand>& operands)
    {
        ...
        if (operators.empty())
        {
            FPS_PRINT_DEBUG("The list of operators is empty");
            return false;
        }
        ...
        if (oper == "!" || oper == "not" || oper == "~")
        {
            if (operands.empty())
            {
                FPS_PRINT_DEBUG("The list of operands is empty");
                return false;
            }

            Expression::Operand operand = operands.top(); operands.pop();
            ...
            // Bitwise operator
                ...
                operands.push(Expression::Operand{ Expression::DataType::NUMERIC, (double)~(int)operand.dval });
        ...
        if (operands.size() < 2)
        {
            FPS_PRINT_DEBUG("The list of operators is empty. Two operands are needed to run logical, comparison, arithmetic, and bitwise operations");
            return false;
        }

        // Operator that takes two operands
        Expression::Operand operand2 = operands.top(); operands.pop();
        Expression::Operand operand1 = operands.top(); operands.pop();

        if (operand1.type != operand2.type)
        {
            if ((operand1.type == Expression::DataType::NUMERIC || operand1.type == Expression::DataType::BOOL) && operand2.type == Expression::DataType::STRING)
            {
                FPS_PRINT_WARN("Type mismatch between operand 1 [{}:{}] and operand 2 [{}:string]"
                    ...
                    , operand1.type == Expression::DataType::NUMERIC ? "numeric" : "boolean"
                    , operand2.sval
                );
                return false;
            }
            if ((operand2.type == Expression::DataType::NUMERIC || operand2.type == Expression::DataType::BOOL) && operand1.type == Expression::DataType::STRING)
            {
                FPS_PRINT_WARN("Type mismatch between operand 1 [{}:{}] and operand 2 [{}:string]"
                    , operand2.type == Expression::DataType::NUMERIC ? operand2.dval : operand2.bval
                    , operand2.type == Expression::DataType::NUMERIC ? "numeric" : "boolean"
                    , operand1.sval
                );
                return false;
            }
        ...
        // Arithmetic operators
            ...
            operands.push(Expression::Operand{ Expression::DataType::NUMERIC, operand1.dval + operand2.dval });
        else if (oper == "-")
            operands.push(Expression::Operand{ Expression::DataType::NUMERIC, operand1.dval - operand2.dval });
        else if (oper == "*")
            operands.push(Expression::Operand{ Expression::DataType::NUMERIC, operand1.dval * operand2.dval });
        ...
        else if (oper == "%") 
        {
            ...
            operands.push(Expression::Operand{ Expression::DataType::NUMERIC, fmod(operand1.dval, operand2.dval) });
        }
        else if (oper == "**")
            operands.push(Expression::Operand{ Expression::DataType::NUMERIC, pow(operand1.dval, operand2.dval) });

        // Bitwise operators
        else if (oper == "&")
            operands.push(Expression::Operand{ Expression::DataType::NUMERIC, (double)((int)operand1.dval & (int)operand2.dval) });
        else if (oper == "|")
            operands.push(Expression::Operand{ Expression::DataType::NUMERIC, (double)((int)operand1.dval | (int)operand2.dval) });
        else if (oper == "^")
            operands.push(Expression::Operand{ Expression::DataType::NUMERIC, (double)((int)operand1.dval ^ (int)operand2.dval) });
        ...
        // Pop the operator from the operator stack and proceed to the next operator, if possible
        operators.pop();
    ...
    /**
     * @brief Helper function for running a supported function and evaluating the results
     * 
     * @param operators the operators stack
     * @param operands the operands stack
    ...
    {
        // If the operands/operators is empty, then we have an invalid operands/operators stack
        if (operators.empty())
        {
            FPS_PRINT_DEBUG("The list of operators is empty. Skipping func");
            return false;
        }
        ...
        if (funcName == "sqrt")
        {
            if (operands.empty())
            {
                FPS_PRINT_DEBUG("The list of operands is empty. We need at least one operand to run sqrt func. Skipping func");
                return false;
            }
            ...
            {
                FPS_PRINT_ERROR("The operand type is not NUMERIC, which is needed for sqrt func. Skipping func");
                return false;
            }
            double result = Math::squareRoot(operand.dval);
            Expression::Operand resultOperand{ Expression::DataType::NUMERIC, result };
        ...
        if (funcName == "max")
        {
            if (operands.size() < 2)
            {
                FPS_PRINT_DEBUG("The list of operands is less than two. We need at least two operands to run max func. Skipping func");
                return false;
            }
            ...
            {
                FPS_PRINT_ERROR("The operand(s) type is not NUMERIC, which is needed for max func. Skipping func");
                return false;
            }
        ...
        if (funcName == "min")
        {
            if (operands.size() < 2)
            {
                FPS_PRINT_DEBUG("The list of operands is less than two. We need two operands to run min func. Skipping func");
                return false;
            }
            ...
            if (operand1.type != Expression::DataType::NUMERIC || operand2.type != Expression::DataType::NUMERIC)
            {
                FPS_PRINT_ERROR("The operand(s) type is not NUMERIC, which is needed for min func. Skipping func");
                return false;
            }
        ...
        if (funcName == "if")
        {
            // We need three arguments to compose conditional if like so: if [boolean], [expr_if_true], [expr_if_false]
            if (operands.size() < 3)
            {
                FPS_PRINT_DEBUG("The list of operands is less than two. We need at three operands to run if statement. Skipping conditional evaluation");
                return false;
            }

            Expression::Operand valIfFalse   = operands.top(); operands.pop();
            Expression::Operand valIfTrue    = operands.top(); operands.pop();
            Expression::Operand conditionVal = operands.top(); operands.pop();

            if (conditionVal.type == Expression::DataType::STRING)
            {
                FPS_PRINT_WARN("The condition operand type is not NUMERIC, which is needed for if func. Skipping conditional evaluation");
                return false;
            }

            bool condition = false;
            // If the condition operand is a numeric type, convert stored value to a boolean
     ...
     * 
     * @return true if the expression is valid, meaning not null and does not contain n/a
     */
    inline bool validExpr(assetVar* av, const char* exprParam)
    {
        ...
        {
            FPS_PRINT_DEBUG("expression [{}] not defined for assetVar [{}:{}]"
                            , (av->gotParam(exprParam) && av->getcParam(exprParam)) ? av->getcParam(exprParam) : "Not Defined"
                            , av->comp
                            , av->name);
            return false;
        }
...
{
    /**
     * @brief Adds the assetVar's value to the list of operands, if the data type of the assetVar
     * is supported
     * 
    ...
    inline bool addOperandForAv(int type, assetVar* av, std::vector<Expression::Operand>& operands)
    {
        ...
        case assetVar::ATypes::AINT:
        {
            ...
            Expression::Operand operand{Expression::DataType::NUMERIC, (double)av->getiVal()};
        ...
        case assetVar::ATypes::AFLOAT:
        {
            ...
            Expression::Operand operand{Expression::DataType::NUMERIC, av->getdVal()};
            operands.push_back(operand);
            break;
        }
        ...
        default:
            FPS_PRINT_ERROR("Type not supported for assetVar {}. Supported types are: AINT, AFLOAT, ASTRING, and ABOOL", av->name);
            return false;
        }
        return true;
    }
    ...
    /**
     * @brief Adds the assetVar parameter's value to the list of operands, if the data type of the parameter
     * is supported
     * 
     ...
     * @return true if the value is added to the list of operands or false otherwise
     */
    inline bool addOperandForParam(int type, assetVar* av, const std::string avParam, std::vector<Expression::Operand>& operands)
    {
        switch (type)
        {
        case assFeat::AINT:
        {
            ...
            Expression::Operand operand{ Expression::DataType::NUMERIC, (double)av->getdParam(avParam.c_str()) };
            operands.push_back(operand);
            break;
        }
        case assFeat::AFLOAT:
        {
            ...
            Expression::Operand operand = { Expression::DataType::NUMERIC, av->getdParam(avParam.c_str()) };
            operands.push_back(operand);
            break;
        }
        case assFeat::ABOOL:
        ...
        case assFeat::ASTRING:
        {
            ...
            Expression::Operand operand{};
            operand.type = Expression::DataType::STRING;
            operand.sval = av->getcParam(avParam.c_str()) ? fmt::format("{}", av->getcParam(avParam.c_str())) : "no string val";
            operands.push_back(operand);
            break;
        }
        default:
    ...
    inline bool getOperands(varsmap& vmap, varmap& amap, assetVar* av, std::vector<Expression::Operand>& operands)
    {
        ...
        // If includeCurrVal is true, add the assetVar's current value to the list of operands
        ...
        if (numVars > 0)
        {
            ...
            for (int i = 1; i <= numVars; i++)
            {
                ...
                if (!avParamName)
                {
                    FPS_PRINT_WARN("assetVar [{}:{}] does not have param {} or param value is null", av->comp, av->name, varParam);
                    return false;
                }
                ...
                FPS_PRINT_DEBUG("assetVar [{}:{}] has parameter [{}]. Attempting to adding to list of operands", av->comp, av->name, varParam);
                ...
                // If we have something like bms:RackCurrent, then this is an aggregation for all asset managers or instances under the asset manager
                // In this case, we'll want to add all of the asset manager or instance's assetVar value to the list of operands
                if (strcmp(my.Uri, av->am->name.c_str()) == 0)
                {
                    ...
                    // If the asset manager map is not empty, we'll take the aggregation of all the asset managers under the parent asset manager
                    if (!av->am->assetManMap.empty())
                    {
                        // Iterate through all asset managers in the asset map and see if the asset manager contains the assetVar targeted for aggregation
                        for (auto& pair : av->am->assetManMap)
                        {
                            FPS_PRINT_DEBUG("pair.first: {} pair.second: {} avName: {}", pair.first, pair.second->name, avName);
                            ...
                            if (!amiVar)
                            {
                                ...
                                const std::string auri = fmt::format("{}/{}:{}", av->comp.substr(0, av->comp.find_last_of("/")), ami->name, avName);
                                FPS_PRINT_DEBUG("auri: {} av->comp: {} ami->name: {} avName: {}", auri, av->comp, ami->name, avName);
                            ...
                            // If we have an assetVar param, we'll include that in the list of operands instead of the assetVar's value 
                            if (!avParam.empty() && amiVar->gotParam(avParam.c_str()))
                            {
                                FPS_PRINT_DEBUG("We got an asset param {} for av {}. We'll include that in the list of operands instead", avParam, amiVar->name);
                                if (!amiVar->extras)
                                {
                    ...
                    {
                        // Iterate through all asset instances in the asset map and see if the asset instance contains the assetVar targeted for aggregation
                        for (auto& pair : av->am->assetMap)
                        {
                            ...
                            if (!aiVar)
                            {
                                ...
                                const std::string auri = fmt::format("{}/{}:{}", av->comp.substr(0, av->comp.find_last_of("/")), ai->name, avName);
                                FPS_PRINT_DEBUG("auri: {} av->comp: {} ai->name: {} avName: {}", auri, av->comp, ai->name, avNname);
                            ...
                            {
                                FPS_PRINT_DEBUG("We got an asset param {} for av {}. We'll include that in the list of operands instead", avParam, aiVar->name);
                                if (!aiVar->extras)
                                {
                ...
                // No aggregation. We'll try to add the assetVar's value to the list of operands as normal
                else
                {
                    ...
                    // If a param is defined in the assetVar uri, check if that param exists and include that value to the list of operands instead
                    const std::string avParam = fmt::format("{}", my.Param ? my.Param : "");
                    if (!avParam.empty() && anotherAv->gotParam(avParam.c_str()))
                    {
                        FPS_PRINT_DEBUG("We got an asset param {} for av {}. We'll include that in the list of operands instead", avParam, av->name);
                        if (!anotherAv->extras)
                        {
     ...
     * 
     * Ex.: If we have an assetVar with the following info: /status/pcs:ActivePowerSetpoint,
     * then the operand info we'll get is /status/pcs:ActivePowerSetpoint
     * 
     * @param vmap the global data map shared by all assets/asset managers
     * @param amap the local data map used by an asset/asset manager
    ...
    inline bool getOperandInfo(varsmap& vmap, varmap& amap, assetVar* av, std::vector<std::string>& operandInfo)
    {
        ...
        // If includeCurrVal is true, add the assetVar's info to the list of operand info
        if (av->gotParam("includeCurrVal") && av->getbParam("includeCurrVal")) 
        ...
        if (numVars > 0)
        {
            for (int i = 1; i <= numVars; i++)
            {
                ...
                FPS_PRINT_INFO/*DEBUG*/("assetVar [{}] has parameter [{}]. Attempting to adding to list of operands", av->name, varParam);
                assetUri my(avUri.c_str());
                ...
                FPS_PRINT_INFO/*DEBUG*/("assetVar {} in parameter [{}] of assetVar [{}] exists. Adding info (uri, name, parameter) to list of operand info"
    ...
    inline bool getOperandsDbl(varsmap& vmap, varmap& amap, assetVar* av, std::vector<double>& operandsDbl)
    {
        ...
        for (const auto& operand : operands)
        {
            ...
            {
                FPS_PRINT_DEBUG("operand data type {} is a string, which we cannot add to the list of operands of type double", operand.type);
                return false;
            }
    ...
    /**
     * @brief Helper function for getting the value of the operand based on the operand's
     * data type
     * 
     * @return the value of the operand or empty string if the given data type is not supported
     */
    inline const std::string getOperandVal(Expression::Operand operand)
    {
        ...
        default:
            FPS_PRINT_ERROR("Type {} not supported for operand. Supported expression data types are: NUMERIC, BOOL, and STRING", operand.type);
            return {};
        }
    }

    /**
     * @brief Helper function that replaces all occurrences of a substring with a new substring
     * 
    ...
    /**
     * @brief Construct and return a string, which contains the expression and the assetVar uri and name associated with the
     * expression, if placeholders are used
     * 
     ...
     * @param exprParam the expression parameter name
     * 
     * @return the string containing the expression and associated assetVar info if placeholders are present
     */
    inline const std::string getExpressionInfo(varsmap& vmap, varmap& amap, assetVar* av, const char* exprParam)
    {
        ...
        // If we are unable to retrieve the list of operands, return empty string to indicate invalid expression
        ...
        while (startIdx != std::string::npos && endIdx != std::string::npos)
        {
            ...
            // Convert placeholder value to int and check if that new index value is within the valid index ranges in the operands vector
            int idx = std::stoi(val) - 1;
            if (idx < 0 || idx > (int)operandInfo.size() - 1 || idx > (int)operands.size() - 1)
            {
                FPS_PRINT_DEBUG("placeholder value {} is not within the valid index range in the list of operands and/or operand info", idx);
                return {};
            }
    ...
    /**
     * @brief Parse the expression. Perform variable substitution if placeholders are
     * found in the expression and the corresponding operand can be found
     * 
    ...
    inline const std::string parseExpr(varsmap& vmap, varmap& amap, assetVar* av, const char* exprParam)
    {
        ...
        // First pass - Replace commas with whitespaces and add whitespace to parenthesis in preparation for parsing
        ...
        // Second pass - Replace placeholders with the values in the operands vector in the expression
        // The placeholder value is the index value in the operands vector
        // Ex.: {1} + 1 + {2} evaluates to 2 + 1 + 3, where {1} = 2 and {2} = 3
        size_t startIdx = expr.find("{");
        size_t endIdx = expr.find("}");
        while (startIdx != std::string::npos && endIdx != std::string::npos)
        {
            ...
            // Check if the placeholder is a number. If so, this is a positional placeholder
            // Ex.: {1} represents the first element in the list of operands
            ...
            {
                FPS_PRINT_DEBUG("placeholder value {} is the valid index range in the list of operands", idx);
                return {};
            }
    ...
    inline Expression::Result evaluateExpr(const std::string& expr)
    {
        ...
        while (ss >> token)
        {
            if (Expression::isNumber(token))
            {
                ...
                Expression::Operand operand{Expression::DataType::NUMERIC, stod(token)};
            ...
            {
                // If operator at the top of the stack has greater precedence or the operator at the top of the stack has equal precendence and token is left associative
                // and the operator is not a (, pop operators from the operator stack and perform calculation on the two operands in the operands stack
                while (!operators.empty() 
                    && (Expression::getPrecedence(operators.top()) > Expression::getPrecedence(token) 
                        || (Expression::getPrecedence(operators.top()) == Expression::getPrecedence(token) && Expression::operatorMap[token].assoc == Expression::Associativity::leftAssociative))
                    && operators.top() != "(")
                {
                    if (!Expression::evaluate(operators, operands))
                    {
                        FPS_PRINT_WARN("Unsuccessful evaluation of expression {}", expr);
                        return Expression::Result{Expression::DataType::NUMERIC, 0};
                    }
                }
            ...
            {
                FPS_PRINT_DEBUG("Pushing ( to operator stack");
                operators.push(token);
            }
            else if (token == ")")
            {
                FPS_PRINT_DEBUG("Found ) token");
                while (operators.top() != "(")
                {
                    if (!Expression::evaluate(operators, operands))
                    {
                ...
                if (operators.top() == "(")
                {
                    FPS_PRINT_DEBUG("Popping off ( from the operator stack");
                    operators.pop();
                }
                // If there is a function token on top of the operator stack, pop the function out and run function
        ...
        // There should be no more tokens to read. At this stage, results should also be calculated. In that case, return the result
        while (!operators.empty())
        {
            FPS_PRINT_DEBUG("About to evaluate operands with size {} operators with size {}", operands.size(), operators.size());
            if (!Expression::evaluate(operators, operands))
            {
                FPS_PRINT_WARN("Unsuccessful evaluation of expression {}", expr);
                return Expression::Result{Expression::DataType::NUMERIC, 0};
        ...
        // If the operands stack is not empty, we may have an invalid expression. In that case, exit out of function
        // without reporting results
        if (operands.size() != 1)
        {
            FPS_PRINT_WARN("The final size of the operands stack [{}] != 1, so expression {} is invalid", operands.size(), expr);
            return Expression::Result{Expression::DataType::NUMERIC, 0};
        }
        ...
        {
        case Expression::DataType::NUMERIC:
            return Expression::Result{Expression::DataType::NUMERIC, operands.top().dval};
        case Expression::DataType::BOOL:
        ...
        default:
            FPS_PRINT_ERROR("Type {} not supported for result. Supported expression data types are: NUMERIC, BOOL, and STRING", operands.top().type);
            return Expression::Result{Expression::DataType::NUMERIC, 0};
        }
    }
}
