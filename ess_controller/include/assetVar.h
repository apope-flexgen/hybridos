...
// DONE valueChanged valueDiff .. perhaps we need valueIfDiff and ValueDiff  value callback
// DONE set/get AssetVar  detects 1 layer component and adds assetID  everything must now b for an asset
// DONE publist a Vlist ot set of asset vars when required (tod example test_phil)
// DONE set both valueint and valuedouble  in the assetvals to help with the cJson representation
// DONE function refs in varsmap vm.add/getFunction ( name , void* ptr) not yet in use but works a treat
// DONE remove hard coded asset manager setup
// DONE register functions for init and wakeups
// DONE getLastSetDiff gets time after last set
// DONE setFunc
// DONE assetFeatDict. availalbe now for assetVar
// DONE TESTING add valueChanged valueDiff getStime 
// DONE I think collect pubs blocks and subs using vecMap
// DONE create assetFunc.h .cpp

// INPROGRESS vlist to different components
// INPROGRESS fims accept, reject and fward methods.
// DONE fix PUBS
// INPROGRESS macros to stop crashes
// DEFERRED  Fix comp logic  only going with two levels for no but comp:var solves some problemse an asset example /status:soc becomes /status/bms_4:soc 
...
// DONE response operation. We set a value on the peripheral and want to handle  the reply verifing the set.....
// DONE faults etc.
// DONE add lastvar 
// DONE rework amap links just do it for functions as needed
// DONE complete full start up.
...
// DONE   fixed in getMapsCj  fix asset config  . we can have three layers /asset/bms/bms_1
// DEFERRED amap is amap leave it at that ..turn amap into a full varsmap test_bms
// DEFERRED for now ther are some common aspects but ..  subclass asset_manager from asset...... 
// DEFERRED  add hos asset
...
// DEFFERED add auto cjson detect to find nfrags on a set or get ..... hmm how do we do get 
// DEFFERED add fims set and get with notification 
// DEFERRED file set and get this allows auto save and recall
// DEFERRED mongodb set and get ... same thing
// DONE limit checking on set against limits in variable names.  ( do it on remap) use max/min = /table:var  onMax onMin max/minerr/time = /table:var etc to map it...


// Response operation/
// DONE
// See doc/promise.txt
// We need a way to send a command to the asset and then get notified when that asset has confirmed the command through a status message.
// So set /controls/bms_1:OnCmd true will cause a response in /status/bms_1:status or some other response value.
...
// TODO AFTER MVP use featdict for options 
class assFeat {
...
public:
    ...
    enum AFTypes { AINT, AFLOAT, ASTRING, ABOOL, AAVAR, AVOID, AEND };
    std::string name;
    //std::string fname;
    AFTypes type;
    double valuedouble;
...
public:
  ...
  //     void setupRunFunc(int (*_runFunc)(varsmap &vmap, varmap &amap, const char* aname, fims* p_fims, asset *am),varsmap *_vmap, varmap *_amap,const char* _aname, fims* _p_fims, asset *_am)
  //     {
  //         runFunc = _runFunc;
  //         vmap = _vmap;
  //         amap = _amap;
...
public:
    ...
    enum ATypes { AINT, AFLOAT, ASTRING, ABOOL, AVAR, AVOID, AEND };
...
// TODO after MVP loose IsDiff
    ...
    int getVecVals(int depth, double& avval, double& minval, 
                                              double& maxval, double& spval, bool debug = false);
...
public:
    VarsMap();
    ~VarsMap();
// TODO map_lock  is not used remove after <MVP
...
class assetExtras {
    //assetExtras::
public:
    enum ATypes { AINT, AFLOAT, ASTRING, ABOOL, AVAR, AEND };
    assetExtras();
    ~assetExtras();
public:
    int cval;
    ...
    //std::map<std::string,assFeat*>featMap;
    //std::map<std::string, assetAction*>actMap;   //list of actions we can have a single action or a vector ( but probably not both)
    std::map<std::string, std::vector<assetAction*>>actVec;   //list of actions for each category
    //adds 48 bytes (264 -> 312)
    //std::map<std::string, std::vector<assetAction*>>actVec2;   //list of actions for each category
    assetFeatDict* featDict;
