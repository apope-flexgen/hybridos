...
public:
    ...
    bool processActRemapOptions(varsmap& vmap, assetVar* av, assetVar** inAv, cJSON** cjov, assetBitField* abf,
                                bool debug = false);
    bool processActEnumOptions(varsmap& vmap, assetVar* av, assetVar** inAv, cJSON** cjovp, assetBitField* abf,
                               bool debug);
    int _vListSendFims(varsmap& vmap, const char* method, fims* p_fims, const char* dbi, const char* uri = nullptr,
                       bool sim = false, assetVar* avd = nullptr);
    int vListSendFims(varsmap& vmap, const char* method, fims* p_fims, const char* uri = nullptr, bool sim = false,
                      assetVar* avd = nullptr);
    int DbivListSendFims(varsmap& vmap, const char* method, fims* p_fims, const char* uri = nullptr, bool sim = false,
                         assetVar* avd = nullptr);
    ...
    // assetVar* makeCJVar(varsmap& vmap, const char* comp, const char* var,
    // cJSON* cj);
    ...
    void setAmFunc(varsmap& vmap, const char* aname, const char* fname, const char* amname, asset_manager* am,
                   void* func);
    ...
    assetVar* setVLink(varsmap& vmap, const char* vname, const char* vlink, bool mkfrom = false, bool mkto = false,
                       assetVar* aV = nullptr);
    ...
    //                         [
    //                            {
    //                            "enable":"/controls/ess_controls:active_power_enable","offset":0,
    //                            "scale":10,
    ...
    //  **"uri":"/cmds/ess_controls:active_power_cmd@Param "    extended to set
    //  PAram
    //  **outValue  if we are going to do the remap than set the outValue to this
    //  number
    ...
    assetVar* setParamfromCj(varsmap& vmap, const char* comp, const char* var, const char* param, cJSON* cj,
                             int uiObject = 0);
    ...
    // assetVar* orig_setValfromCj(varsmap& vmap, const char* comp, const char*
    // var, cJSON* cj, int uiObject = 0);  assetVar* new_setValfromCj(varsmap&
    // vmap, const char* comp, const char* var, cJSON* cj, int uiObject = 0);
    ...
    cJSON* createUriListCj(varsmap& vmap, std::string& bs, const char* inuri, cJSON* incj, int options,
                           std::vector<std::string>& uriVec);
    int createAssetListCj(varsmap& vmap, const char* uri, std::vector<char*>* syscVec, int opts,
    ...
    cJSON* getMapsCj(varsmap& vmap, const char* inuri = nullptr, const char* var = nullptr, int opts = 0,
                     const char* origuri = nullptr, cJSON* cji = nullptr);
    ...
    void processMsgSetPubUi(varsmap& vmap, const char* method, const char* uri, int& single, const char* body,
                            cJSON** cjr, asset_manager* am = nullptr, asset* ai = nullptr);

    // make an assetList var only for uiObjects
    // This is a real hack we store the assetList away in the assetList area of
    // varsmap
    ...
    // dont want to mess up this cde .. We could ussr the cjr to capture the exact
    // order. the vars map coild contain an assetList I guess.

    void processMsgSetPub(varsmap& vmap, const char* method, const char* uri, int& single, const char* body,
                          cJSON** cjr, asset_manager* am = nullptr, asset* ai = nullptr);

    void processMsgSetReply(varsmap& vmap, const char* method, const char* inuri, const char* replyto, const char* body,
                            cJSON** cjr, asset_manager* am = nullptr, asset* ai = nullptr);
    ...
    int testRes(const char* tname, varsmap& vmap, const char* method, const char* uri, const char* body,
                const char* res, asset_manager* am = nullptr, asset* ai = nullptr);
    int configure_vmapStr(varsmap& vmap, const char* body, asset_manager* am, asset* ai, bool delcj);
    int configure_vmap(varsmap& vmap, const char* fname,
                       std::vector<std::pair<std::string, std::string>>* reps = nullptr, asset_manager* am = nullptr,
                       asset* ai = nullptr);
    ...
    int sendAssetVar(assetVar* av, fims* p_fims, const char* meth = nullptr, const char* comp = nullptr,
                     const char* var = nullptr, assetVar* avd = nullptr);
    ...
    int setupAssetList(varsmap& vmap, const char* aname, const char* alistname, const char* alistVersion,
                       asset_manager* am);
    ...
    void handleCfile(varsmap& vmap, cJSON* cj, const char* method, const char* uri, int& single, const char* body,
    ...
    cJSON* loadVmap(varsmap& vmap, int single, const char* comp, const char* var, const char* body,
                    asset_manager* am = nullptr, char* uri = nullptr);
    ...
    // if method == set we do it by detecting the body content to see if the last
    // pfag was the variable
    // cJSON* cj = checkSingleUri(vmap,        single,        &vName,     &cName2,
    // msg->method, newUri,              msg->body);
    // will recover uri and var from a possible single uri
    cJSON* checkSingleUri(varsmap& vmap, int& single, char** vName, char** cName, const char* method,
                          const char* uri = nullptr, const char* body = nullptr);

    cJSON* getVmap(varsmap& vmap, int& single, const char* key = nullptr, const char* var = nullptr, int opts = 0,
                   const char* baseUri = nullptr);
    cJSON* loadUimap(varsmap& vmap, int single, const char* comp, const char* var, const char* body,
    ...
    bool runFimsMsgAm(varsmap& vmap, fims_message* msg, asset_manager* am, fims* p_fims = nullptr,
                      cJSON** cjri = nullptr);
    ...
    asset_log* sendAlarm(varsmap& vmap, const char* srcUri, const char* destUri, const char* atype, const char* msg,
                         int severity);
    asset_log* sendAlarm(varsmap& vmap, assetVar* srcAv, const char* destUri, const char* atype, const char* msg,
                         int severity);
    int clearAlarm(varsmap& vmap, assetVar* srcAv, assetVar* destAv, const char* atype, const char* msg, int severity);
    int clearAlarm(varsmap& vmap, const char* srcUri, const char* destUri, const char* atype, const char* msg,
                   int severity);
    int clearAlarm(varsmap& vmap, assetVar* srcAv, const char* destUri, const char* atype, const char* msg,
                   int severity);
    ...
    int setAmFunc(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, asset_manager* am, const char* vname,
                  int (*func)(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, asset_manager* am));

    int setAvFunc(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* am, const char* vname,
                  int (*func)(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* am));
    // void setSequence(varsmap& vmap, const char* aname, const char* fname, void*
    // func, const char* help);
    ...
    // //void runMonitorList(varsmap& vmap, const char* aname, double rate, double
    // offset); void runMonitorList(varsmap& vmap, varmap &amap, const char*
    // aname, fims* p_fims, double rate, double offset);
    void runMonitorList2(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, const char* wname,
                         bool debug = false);
...
int remapSendFims(varsmap& vmap, const char* fims, assetUri& auri, assetVar* av, assetVar* aV2, assetBitField* abf,
                  cJSON* cjov, cJSON* cjav, bool debug);

int remapSendAv(varsmap& vmap, VarMapUtils* vm, assetUri& auri, assetVar* av, assetBitField* abf, cJSON* cjov,
                cJSON* cjav, bool debug);

int remapSendAvParam(varsmap& vmap, assetUri& auri, assetVar* av, assetBitField* abf, cJSON* cjov, cJSON* cjav,
                     bool debug);
...
// Variadic template and base case for setting links, very useful/powerful stuff
// (CAUTION: arg/args can be anything, so check for compiler error):
...
inline void linkVals(VarMapUtils& vm, varsmap& vmap, varmap& amap, const char* aname, const char* URI, dataType& type,
                     Arg&& arg, Args&&... args)
{
    // ((amap[std::forward<Args>(args)] = vm->setLinkVal(vmap, aname, URI,
    // std::forward<Args>(args), type)), ...); // C++17 and up way to do it!
    amap[std::forward<Arg>(arg)] = vm.setLinkVal(vmap, aname, URI, std::forward<Arg>(arg), type);
    linkVals(vm, vmap, amap, aname, URI, type, args...);  // With C++17 and up this
                                                          // will become obsolete
                                                          // along with the base
                                                          // case function.
}

// Variadic template and base case for linking Vector Vals, as seen in
// test_release_funcs.cpp under function : SetupLimitsVec
template <typename dataType>
inline void linkAvarmapVals(VarMapUtils& vm, varsmap& vmap, varmap& amap, avarmap& avmap, const char* aname,
                            const char* vname, std::string& mvar, std::string& mname, const char* URI, dataType& type)
{
    return;
}
template <typename dataType, typename Arg, typename... Args>
...
int debugSystemLoad(VarMapUtils* vm, varsmap& vmap, vecmap& vecs, std::vector<char*>* syscpVec, const char* aname,
                    const char* logdir);
// int debugSystemLoad(VarMapUtils*vm, vecmap &vecs, std::vector<char*>*
// syscpVec, const char *aname);
