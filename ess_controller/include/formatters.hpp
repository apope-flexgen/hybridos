...
// some useful helper macros for logging to console (this is the new standard I want, for safety and extensibility reasons)
...
// NOTE: Please change "tweakme.h" inside of /usr/local/include/spdlog/ to get this macro to expand out
// uncomment and change line 114 to -> #define SPDLOG_ACTIVE_LEVEL SPDLOG_LEVEL_DEBUG
// also don't forget to call "spdlog::set_level(spdlog::level::debug);" once at the start of main of your program
...
// p = prettified (spaces and indenting - might be tricky to implement, probably the last thing todo)
...
namespace fmt
{
    ...
    struct formatter<assetVar>
    {
        ...
        auto format(const assetVar& aVr, FormatContext& ctx) -> decltype(ctx.out()) const
        {
            ...
            case 'u':
                ...
                {
                    if(0)FPS_PRINT_INFO("FOUND EXTRAS for [{}]", aVr.name);
                    if (aVr.extras->optDict) {
                        if(0)FPS_PRINT_INFO("FOUND OPTS for [{}]", aVr.name);
                        return format_to(ctx.out(), R"({{"value":{},"options":[{}],"enabled":{}}})",
                            aVr.linkVar ? *aVr.linkVar->aVal : *aVr.aVal,
                            *aVr.extras->optDict,
                ...
                else if (aVr.extras && (aVr.extras->optVec))// here for optVec
                {
                    ...
                    if(options)free((void*)options);
                    return fmtOutput;

                }
                else
                {
                    return format_to(ctx.out(), R"({{"value":{},"enabled":true}})", aVr.linkVar ? *aVr.linkVar->aVal : *aVr.aVal);
                    //return format_to(ctx.out(), "{}", aVr.linkVar ? *aVr.linkVar->aVal : *aVr.aVal);
                }
            case 'i':
                if(aVr.extras )
                {
                    ...
                    if (aVr.extras->optDict)
                    {
                        return format_to(ctx.out(), R"("{}":{{"value":{},"options":[{}],"enabled":{},"name":"{}","scaler":{},"type":"{}","ui_type":"{}","unit":"{}"}})",
                            aVr.name,
                            aVr.linkVar ? *aVr.linkVar->aVal : *aVr.aVal,
                            *aVr.extras->optDict,
                            aVr.getbParam("enabled"),
                            cstr{aVr.getcParam("name")}, // might need to check this
                            aVr.getiParam("scaler"),
                            cstr{aVr.getcParam("type")},
                            cstr{aVr.getcParam("ui_type")},
                    ...
                    else // here for optVec
                    {
                        ...
                        //const char* options = aVr.extras->optVec ? cJSON_PrintUnformatted(aVr.extras->optVec->cjopts) : "[]";
                        auto xx = format_to(ctx.out(), R"("{}":{{"value":{},"options":{},"enabled":{},"name":"{}","scaler":{},"type":"{}","ui_type":"{}","unit":"{}"}})",
                            aVr.name,
                            aVr.linkVar ? *aVr.linkVar->aVal : *aVr.aVal,
                            options,
                            aVr.getbParam("enabled"),
                        ...
                        if(options)free((void*)options); // here for safety, just in case cJSON allocates (NEVER trust a C API)
                        return xx;
                    }
                }
            case 'f': // TODO(WALKER): format cjopts here as well then make sure to delete the char* to prevent a memory leak
                // format value first:
    ...
    struct formatter<asset_log*>
    {
        ...
        {
            return format_to(ctx.out(), R"({{"name":"{}","return_value":{}}})",
                aLog->almsg, *static_cast<assetVal*>(aLog->aVal));
        }
    };
    ...
    struct formatter<std::pair<const std::string, std::vector<assetAction*>>>
    {
        ...
        template <typename FormatContext>
        auto format(const std::pair<const std::string, std::vector<assetAction*>>& actVp, FormatContext& ctx) -> decltype(ctx.out()) const
        {
    ...
    struct formatter<std::map<std::string, std::vector<assetAction*>>>
    {
        ...
        template <typename FormatContext>
        auto format(const std::map<std::string, std::vector<assetAction*>>& actV, FormatContext& ctx) -> decltype(ctx.out()) const
        {
    ...
    //     template <typename FormatContext>
    //     auto format(const std::pair<const std::string, asset*>& p, FormatContext& ctx) -> decltype(ctx.out()) const
    //     {
    //         return format_to(ctx.out(), R"("{}":{})", p.first, *p.second);
    //     }
    // };
    ...
    //     template <typename FormatContext>
    //     auto format(const asset_manager& am, FormatContext& ctx) -> decltype(ctx.out()) const
    //     {
    ...
    //     template <typename FormatContext>
    //     auto format(const std::pair<const std::string, asset_manager*>& p, FormatContext& ctx) -> decltype(ctx.out()) const
    //     {
    //         return format_to(ctx.out(), R"("{}":{})", p.first, *p.second);
    //     }
    // };
