...
public:
    ...
    engine(SeedSeq&& seedSeq,
           typename std::enable_if<!stream_mixin::can_specify_stream && !std::is_convertible<SeedSeq, itype>::value &&
                                   ...
                                   no_specifiable_stream_tag>::type /*unused*/
           = {})
    ...
    engine(SeedSeq&& seedSeq,
           typename std::enable_if<stream_mixin::can_specify_stream && !std::is_convertible<SeedSeq, itype>::value &&
                                       !std::is_convertible<SeedSeq, engine>::value,
                                   can_specify_stream_tag>::type /*unused*/
           = {})
    {
        itype seeddata[2];
...
struct xsh_rs_mixin
{
    static xtype output(itype internal)
    {
        ...
        bitcount_t rshift = opbits != 0u ? bitcount_t(internal >> (bits - opbits)) & mask : 0;
...
struct xsh_rr_mixin
{
    static xtype output(itype internal)
    {
        ...
        bitcount_t rot = opbits != 0u ? bitcount_t(internal >> (bits - opbits)) & mask : 0;
...
struct rxs_m_xs_mixin
{
    ...
    static itype unoutput(itype internal)
    {
        ...
        bitcount_t rshift = opbits != 0u ? (internal >> (bits - opbits)) & mask : 0;
...
private:
    ...
    static constexpr state_type tick_mask = may_tick
                                                ? state_type(
                                                      (uint64_t(1) << (advance_pow2 * static_cast<int>(may_tick))) - 1)
...
void extended<table_pow2, advance_pow2, baseclass, extvalclass, kdd>::advance(state_type distance, bool forwards)
{
    ...
    if (may_tick)
    {
        state_type ticks = distance >> (advance_pow2 * static_cast<int>(may_tick));
