...
* each state will perform measurements on data , produce alerts and warnings if needed
* states will also have transitions to other states.
* these trasitions take the form of delivering commands and monitoring system responses to those commnds.
* a state will have a command input 
* lets take the "power up" state as an example.
* the system powers up, 
*  configures a default environment.
*  starts communications with other systems.
*  issues start up commands to its subsystems.
*  runs data monitoring functions and transitions to either a  ready state or a failed state.
*
*  BMS in more detail
...
*      any alert or alarm will cause the system to issue warnings sound alarms and even maybe shutdown.
*      hybridOS determined about 5 states
*      STOP STANDBY START RUN1 RUN2 ESTOP lets consider some more RESET POWERUP and FAULT
*      The HYBRIDOS model of Sequences/ paths and steps look like a good one so we'll try and follow that.
*       
*
*/
/*
   ...
   Th request can then proced to the first or next step  ( in fact the iniital conditions are the first step)
A step will set up some commands and evaluate results within a time constraint.
Warnings alarms and other errors are logged.
...
We then  monitor the dc contactor state and consider the step complete when done.
"commands"
    "request":"standby"
	"fromstates":["init","stopped","standby"]
...
"monitor"
  "allstates" freq 100mS  
      monitor "bms_heartbeats" "incrementing" time 1.0 wrap 255  errorstate "Estop" 
      monitor "pcs_heartbeats" "incrementing" time 1.0 wrap 255  errorstate "Estop"
      monitor "bms_mode" "value=OK"  errorstate "Estop"
      monitor "pcs_mode" "value=OK"  errorstate "Estop"


  "standby"
	"monitor":[
		{"comp":"/system/components","var":"grid_volts", "min":900, "max":1000, "onError": Stop"},
		{"comp":"/system/components","var":"dc_volts", "min":900, "max":1000, "onError": Stop"}
		]
...
ess updateMode
this does things with control words like if you are off and you get an on command.
look for set_active_power and stuff like that
...
processCtrlWordConfig(e, e.CtrlWord1Cfg, e.CtrlWord1)
	...
	processCtrlWordConfig(e, e.CtrlWord4Cfg, e.CtrlWord4)  // get incoming commands
...
Each system manager will add commands to the asset name space based on the incoming commands.
The assets will provide responses in their namespace
the asset manager will then formulate the plan and pass it down 
lastly the systems will provide an update on what they are doing.
...
example 
   ...
   back up to the top and the charge load for each will be calculated based on these responses.
   back down to give allowed charge commands to each . They will respond
   Finally the last runupdates the state with the results.
