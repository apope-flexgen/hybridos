...
// Namespace for common utility operations used in ESS (ex.: getting an assetVar
// from another assetVar's parameter)
namespace ESSUtils
{
/**
...
{
    FPS_PRINT_DEBUG("Decrementing time for av [{}:{}].   {} [{:2.3f}] tDiff [{:2.3f}]", av->comp, av->name, param,
                    currTime, tDiff);
    currTime -= tDiff;
 ...
 * @param timeCfg the max time defined in the config that the current time can
 * be updated to
 * @param param the name of the time param to update
 */
...
{
    FPS_PRINT_DEBUG(
        "Incrementing time for av [{}:{}].   {} [{:2.3f}] tDiff "
        "[{:2.3f}] timeCfg [{:2.3f}]",
        av->comp, av->name, param, currTime, tDiff, timeCfg);
    currTime += tDiff;
 ...
 */
inline void setAvStatus(varsmap& vmap, varmap& amap, const char* aname, assetVar* av, const std::string& targetAvName,
                        bool result, const std::string& logName, bool logToFile = true)
{
    ...
    const std::string fileName = fmt::format("{}_{}_{}_{}", essName, aname, av->name,
                                             logName.empty() ? targetAvName : logName);
    const std::string dir = fmt::format("{}/{}.txt", logDir, fileName);
    const std::string msg = fmt::format("Setting /status/{}:{} for [{}] to {}", aname, targetAvName, av->getfName(),
                                        result);
 ...
 */
inline void setAvStatus(varsmap& vmap, varmap& amap, const char* aname, assetVar* av, const std::string& targetAvName,
                        const std::string& result, const std::string& logName, bool logToFile = true)
{
    VarMapUtils* vm = av->am->vm;
 ...
 *
 * @return the assetVar if the assetVar exists as defined in the param or null
 * otherwise
 */
inline assetVar* getAvFromParam(varsmap& vmap, varmap& amap, assetVar* av, const std::string& param)
{
    ...
    if (!avUri.empty())
    {
        ...
        // If we have found a ":", then we may have a uri and a variable name.
        // Something like this: /component/asset_name:var_name
        else
        {
            VarMapUtils* vm = av->am->vm;
...
/**
 * @brief Checks if the condition(s) are satisfied, meaning if the evaluated
 ...
 *
 * @return true if conditions are satisfied or false if the expression is
 * invalid or conditions are not satisfied
 */
inline bool checkConditions(varsmap& vmap, varmap& amap, assetVar* av, const char* exprParam)
{
    ...
    else
    {
        ...
        {
            FPS_PRINT_ERROR(
                "expression [{}] result is not a boolean, which is not "
                "valid for [{}:{}]",
                cstr{ exprParam }, av->comp, av->name);
            return false;
        }
        else
        {
...
inline std::string getEventSourceName(VarMapUtils* vm, varsmap& vmap, const char* aname)
{
    ...
    if (avEventSrc)
    {
        ...
        if (formatParam)
        {
            ...
            {
                // Use essName<sep>assetName as the source name, where <sep> is the
                // separator Default to -> if sep is not defined
...
inline void record(varsmap& vmap, varmap& amap, const char* aname, assetVar* av, const std::string& msg, double tNow,
                   const std::string& reason, Severity severity, bool sendAlarm = false, bool fault = false,
                   bool logToFile = true)
{
    UNUSED(tNow);
    VarMapUtils* vm = av->am->vm;
...
inline bool getAvList(varsmap& vmap, varmap& amap, assetVar* av, std::vector<assetVar*>& operands)
{
    ...
    if (numVars > 0)
    {
        ...
        for (int i = 1; i <= numVars; i++)
        {
            ...
            FPS_PRINT_DEBUG(
                "assetVar [{}] has parameter [{}]. Attempting to adding "
                "to list of operands",
                av->name, varParam);
            assetVar* anotherAv;
            if (!my.Var)  // If the parameter contains just name, then check the amap
                          // for the variable
            {
                if (!amap[avUri])
                {
            ...
            else  // The parameter contains uri:name, so check the vmap for the
                  // variable
            {
            ...
            FPS_PRINT_DEBUG(
                "assetVar {} in parameter [{}] of assetVar [{}] exists. "
                "Adding val to list of operands",
