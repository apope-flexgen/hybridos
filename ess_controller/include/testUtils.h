...
// Very useful operation that is not in C++11, so I imported it here (USED IN
// DUMMY CLASS! DO NOT DELETE!):
...
// Class that tells you what is going on to it (for data structures, algorithms,
// etc.). Uses the rule-of-five
class Dummy
{
public:
    std::string m_name;
    std::vector<std::string> event_history;
    bool m_debug = true;  // Needed for testing data structures, do NOT remove the
                          // "= true" (bools are "= false" by default)

    // Default constructor:
    explicit Dummy(std::string name = "default", bool debug = true) : m_name(std::move(name)), m_debug(debug)
    {
        if (m_debug)
        {
            printf("(Dummy: %s) was constructed.\n", m_name.c_str());
        }
    }
    // Destructor:
    ~Dummy()
    {
        if (m_debug)
        {
            printf("(Dummy: %s) was destroyed, event_history(begin->end): \n", m_name.c_str());
        }
        ...
        if (m_debug && !event_history.empty())
        {
            printf("\n");
        }
    }
    ...
    // Copy constructor:
    Dummy(const Dummy& other, bool debug = true) : m_name(other.m_name), m_debug(debug)
    {
        if (other.m_debug)
        {
            printf("(Dummy: %s) was copied by constructor!\n", other.m_name.c_str());
        }
    ...
    Dummy& operator=(const Dummy& other)
    {
        if (this != &other)
        {
            if (m_debug)
            {
                printf("(Dummy: %s) is copying (Dummy: %s) by copy assignment!\n", m_name.c_str(),
                       other.m_name.c_str());
            }
        ...
        else
        {
            if (m_debug)
            {
                printf("(Dummy: %s) is copying ITSELF by copy assignment!\n", m_name.c_str());
            }
        ...
        : m_name(custom_exchange_ns::exchange(other.m_name, "null")), m_debug(debug)
    {
        if (other.m_debug)
        {
            printf("(Dummy: %s) was moved by constructor!\n", m_name.c_str());
        }
    ...
    Dummy& operator=(Dummy&& other) noexcept
    {
        if (this != &other)
        {
            if (m_debug)
            {
                printf("(Dummy: %s) is turning into (Dummy: %s) by move assignment!\n", m_name.c_str(),
                       other.m_name.c_str());
            }
        ...
        else
        {
            if (m_debug)
            {
                printf("(Dummy: %s) is turning into ITSELF by move assignment!\n", m_name.c_str());
            }
...
// Timer class to help you figure out how long a block of code takes to run (for
// algorithms, etc.)
class Timer
{
...
public:
    explicit Timer(std::string name, bool debug = true) : m_name(std::move(name)), m_debug(debug)
    {
...
// use this to pause a test to check changes, then press y to continue to the
// next step (disable it by passing in false):
