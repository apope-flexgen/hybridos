...
{
    BOOLEAN_T,
    INT_T, UINT_T, INT16_T, UINT16_T, INT32_T, UINT32_T, INT64_T, UINT64_T, ULONG_T, ULONGLONG_T, 
    DOUBLE_T, REAL_T, REAL32_T, REAL64_T, TIME_T,
    dflt
};
...
struct DataMap {
    ...
    void sendToAmap(varsmap &vmap, std::string bname, asset_manager* am, uint8_t* dataArea);                        // transfer data , using a named transfer block , from the dataMap to the amap
    bool setDataItemToAmap(varsmap &vmap, asset_manager *am, const std::string &amapName, const std::string &mapName, uint8_t* dataArea);
    void getFromAmap(std::string bname, asset_manager* am, uint8_t* dataArea);                                      // transfer data , using a named transfer block , from the amap to the dataMap
    bool getDataItemFromAmap(asset_manager *am, const std::string &amapName, const std::string &mapName, uint8_t* dataArea);
};
...
class ess_thread {
    ...
    channel <std::pair<int, assetVar*>> wakeChan;       // set as a pair to pass signal along with context ("i want you to run this signal against this aV")
    int wakeup;
    bool running;
    double delay;
    int core;
...
extern std::unordered_map<std::string, DataMap*> dataMaps;         // a map of all dataMaps and their names. used to access dataMaps outside of their creation function
