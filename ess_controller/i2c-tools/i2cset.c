...
static int confirm(const char* filename, int address, int size, int daddress, int value, int vmask,
                   const unsigned char* block, int len, int pec)
{
    int dont = 0;
...
int main_i2cset(int argc, char* argv[])
{
    ...
    {
        /* All other commands */
        if (strlen(argv[argc - 1]) > 2 || (strlen(argv[argc - 1]) == 2 && argv[argc - 1][1] != 'p'))
        {
    ...
    switch (size)
    {
        ...
        case I2C_SMBUS_WORD_DATA:
            ...
            if ((size == I2C_SMBUS_BYTE_DATA && value > 0xff) || (size == I2C_SMBUS_WORD_DATA && value > 0xffff))
            {
    ...
    if (maskp)
    {
        ...
        if (((size == I2C_SMBUS_BYTE || size == I2C_SMBUS_BYTE_DATA) && vmask > 0xff) || vmask > 0xffff)
        {
    ...
    if (file < 0 || check_funcs(file, size, pec) || set_slave_addr(file, address, force))
        exit(1);

    if (!yes && !confirm(filename, address, size, daddress, value, vmask, block, len, pec))
        exit(0);
    ...
    if (vmask)
    {
        ...
        if (!yes)
        {
            fprintf(stderr,
                    ...
                    "0x%02x\n",
                    size == I2C_SMBUS_WORD_DATA ? 4 : 2, oldvalue, size == I2C_SMBUS_WORD_DATA ? 4 : 2, vmask,
    ...
    {
        printf(
            "Warning - data mismatch - wrote "
            "0x%0*x, read back 0x%0*x\n",
            size == I2C_SMBUS_WORD_DATA ? 4 : 2, value, size == I2C_SMBUS_WORD_DATA ? 4 : 2, res);
    }
    else
    {
...
int i2cset(unsigned int i2cbus, unsigned int address, unsigned int daddress, unsigned int data)
{
    ...
    if (file < 0 || check_funcs(file, I2C_SMBUS_BYTE_DATA, 0) || set_slave_addr(file, address, 1))
        return -8;
...
int i2cset_word(unsigned int i2cbus, unsigned int address, unsigned int daddress, unsigned int data)
{
    ...
    file = open_i2c_dev(i2cbus, filename, sizeof(filename), 1);
    if (file < 0 || check_funcs(file, I2C_SMBUS_WORD_DATA, 0) || set_slave_addr(file, address, 1))
        return -8;
