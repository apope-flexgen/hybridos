...
static int confirm(const char* filename, int address, int size, int daddress, int pec)
{
    ...
    fprintf(stderr, ", using %s.\n",
            size == I2C_SMBUS_BYTE ? (daddress < 0 ? "read byte" : "write byte/read byte")
...
int main_i2cget(int argc, char* argv[])
{
    ...
    if (file < 0 || check_funcs(file, size, daddress, pec) || set_slave_addr(file, address, force))
        exit(1);
...
int i2cget(unsigned int i2cbus, unsigned int address, unsigned int daddress, unsigned int* data)
{
    ...
    *data = 0;
    ...
    if (file < 0 || check_funcs(file, I2C_SMBUS_BYTE_DATA, daddress, 0) || set_slave_addr(file, address, 1))
        return -8;
...
int i2cget_word(unsigned int i2cbus, unsigned int address, unsigned int daddress, unsigned int* data)
{
    ...
    *data = 0;
    ...
    file = open_i2c_dev(i2cbus, filename, sizeof(filename), 1);
    if (file < 0 || check_funcs(file, I2C_SMBUS_WORD_DATA, daddress, 0) || set_slave_addr(file, address, 1))
        return -8;
