...
# Tests power limitation for batteries based on protections (ex.: soc and temperature)
#
...
setupTableVar()
{
    echo -e "Initializing table variable for power limitation test..."
    fimsSet "/ess/status/bms" '{"pChargeLimit":0}'
    fimsSet "/ess/status/bms" '{"pChargeLimit":{"tableName":"p_charge_tbl.csv"}}'
    fimsSet "/ess/status/bms" '{"pChargeLimit":{"tableVar1":"mbmu_soc"}}'
    fimsSet "/ess/status/bms" '{"pChargeLimit":{"tableVar2":"mbmu_avg_cell_temperature"}}'
    fimsSet "/ess/reload/bms" '{"CheckTableVar_pChargeLimit":0}'
...
echo -n "getting /reload/bms:CheckTableVar_pChargeLimit " && fimsGet "/ess/reload/bms/CheckTableVar_pChargeLimit"
...
echo -n "getting pChargeLimit                           " && fimsGet "/ess/full/status/bms/pChargeLimit"
sleep 2
...
echo -n "setting soc to 4%                              " && fimsSet "/ess/components/catl_mbmu_summary_r" '{"mbmu_soc":4}'
sleep 1
echo -n "setting temperature to -5 degrees Celsius      " && fimsSet "/ess/components/catl_mbmu_summary_r" '{"mbmu_avg_cell_temperature":-5}'
sleep 3
echo -n "getting pChargeLimit after set (expect: 0)     " && fimsGet "/ess/full/status/bms/pChargeLimit"
sleep 3
echo -n "setting soc to 13%                             " && fimsSet "/ess/components/catl_mbmu_summary_r" '{"mbmu_soc":13}'
sleep 1
echo -n "setting temperature to 18 degrees Celsius      " && fimsSet "/ess/components/catl_mbmu_summary_r" '{"mbmu_avg_cell_temperature":18}'
sleep 3
echo -n "getting pChargeLimit after set (expect: 0.5)   " && fimsGet "/ess/full/status/bms/pChargeLimit"
sleep 3
echo -n "setting soc to 95%                             " && fimsSet "/ess/components/catl_mbmu_summary_r" '{"mbmu_soc":95}'
sleep 1
echo -n "setting temperature to 25 degrees Celsius      " && fimsSet "/ess/components/catl_mbmu_summary_r" '{"mbmu_avg_cell_temperature":25}'
sleep 3
echo -n "getting pChargeLimit after set (expect: 1)     " && fimsGet "/ess/full/status/bms/pChargeLimit"
