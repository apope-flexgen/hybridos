...
# set up a test link
/usr/local/bin/fims/fims_send  -m set -r /$$ -u /flex/full/test/bms '
                 {"bms_vlink":{"value":1234.5,"$aopts":true, "$vlink":"/status/bms:bms_vlinked"}}'
...
/usr/local/bin/fims/fims_send  -m get -r /$$ -u /flex/full/test/bms/bms_vlink
/usr/local/bin/fims/fims_send  -m get -r /$$ -u /flex/full/status/bms/bms_vlinked
echo "set value"
/usr/local/bin/fims/fims_send  -m set -r /$$ -u /flex/test/bms/bms_vlink 44.5
echo "result"
/usr/local/bin/fims/fims_send  -m get -r /$$ -u /flex/full/test/bms   | jq
