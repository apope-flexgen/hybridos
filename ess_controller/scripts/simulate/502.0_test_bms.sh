...
# { 
#     "sbmu_warning_1":0,"sbmu_warning_21":0,
#     "sbmu_door_state":0,"sbmu_fan_in_box":0,
...
echo setup the sim function for the scheduler
/usr/local/bin/fims/fims_send -m set -r /$$ -u /$SIM/full/control/bms '
                    { "SimBms":{"value":1,"amap":"bms","fcn":"SimHandleBms",
                                  "enabled":true, "actions":{"onSet":[{"func":[{"func":"SimHandleBms"}]}]}}}'

echo run Bms Sim operations 
/usr/local/bin/fims/fims_send -m set -r /$$ -u /$SIM/full/system/commands/run '
                    {"value":22,"uri":"/control/bms:SimBms","every":1.0,"offset":0,"debug":1}'
sleep 2
/usr/local/bin/fims/fims_send -m set -r /$$ -u /$SIM/full/control/bms '
                    { "SimBms":{"value":1,"fcn":"SimHandleBms"}
                    }'
