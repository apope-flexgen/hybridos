...
/usr/local/bin/fims_send -m set -r /$$ -u /ess/components/bms_info '{
        "max_cell_voltage": {
            "value": 0, "EnableFaultCheck": true, "EnableMaxValCheck": true,"MaxAlarmThreshold": 25.4,"MaxFaultThreshold": 28,"MaxResetValue": 22.4,
...
/usr/local/bin/fims_send -m get -r /$$ -u /ess/components/bms_info

wait_pause

echo "examine the variables in full detail "
echo;echo;echo
/usr/local/bin/fims_send -m get -r /$$ -u /ess/full/components/bms_info
...
echo;echo;echo
/usr/local/bin/fims_send -m get -r /$$ -u /ess/full/components/bms_info | jq
...
echo;echo;echo
/usr/local/bin/fims_send -m get -r /$$ -u /ess/full/schedule/wake_monitor/bms | jq 
...
/usr/local/bin/fims_send -m get -r /$$ -u /ess/full/system/commands | jq 
wait_pause
...
/usr/local/bin/fims_send -m set -r /$$ -u /ess/full/system/commands '{"runMon": {"value":22,"aname":"bms","monitor":"wake_monitor","debug":1}}'

wait_pause
#exit

#echo " check results"
#/usr/local/bin/fims_send -m get -r /$$ -u /ess/full/ | jq 

echo "set up bad values for max_cell_voltage 26.4 "
/usr/local/bin/fims_send -m set -r /$$ -u /ess/components/bms_info '{"max_cell_voltage": {"value":26.4}}'
...
/usr/local/bin/fims_send -m get -r /$$ -u /ess/full/components/bms_info/max_cell_voltage | jq 
wait_pause

echo " check alarm results"
/usr/local/bin/fims_send -m get -r /$$ -u /ess/full/assets/ess/summary | jq |  grep -B 5 -A 15 alarms

echo "set up bad values for max_cell_voltage 28.4 "
/usr/local/bin/fims_send -m set -r /$$ -u /ess/components/bms_info '{"max_cell_voltage": {"value":28.4}}'

echo " wait  another second  for a fault"
sleep 1.1
echo ; echo " >>run RunMonitor for bms wake_monitor again "
/usr/local/bin/fims_send -m set -r /$$ -u /ess/full/system/commands '{"runMon": {"value":23,"aname":"bms","monitor":"wake_monitor","debug":1}}'
wait_pause

echo " check faults results"
/usr/local/bin/fims_send -m get -r /$$ -u /ess/full/assets/ess/summary | jq | grep -B 5 -A 15 faults
...
/usr/local/bin/fims_send -m set -r /$$ -u /ess/components/bms_info '{"max_cell_voltage": {"value":22.3}}'

echo " wait  two seconds  for a fault reset"
sleep 2.2
echo ; echo " >>run RunMonitor for bms wake_monitor again "
/usr/local/bin/fims_send -m set -r /$$ -u /ess/full/system/commands '{"runMon": {"value":24,"aname":"bms","monitor":"wake_monitor","debug":1}}'

wait_pause

echo " check alarm results for a third time"
/usr/local/bin/fims_send -m get -r /$$ -u /ess/full/assets/ess/summary | jq 
...
/usr/local/bin/fims_send -m get -r /$$ -u /ess/full/status/ess | jq
wait_pause
...
echo " Something like "
echo "/usr/local/bin/fims_send -m set -r /$$ -u /ess/full/system/commands/run 
