...
/usr/local/bin/fims_send -m set -r /me -u /ess/status/bms ' 
      {"MaxCellVoltage": {"value": 0, "MaxSetVal": 20},
        "MinCellVoltage": {"value": 0, "MaxSetVal": 20},
        "MaxCellTemp": {"value": 0, "MaxSetVal": 20}
      }'

SOC_START=$(/usr/local/bin/fims_send -m get -r /me -u /ess/naked/status/bms/MaxCellVoltage)
SOH_START=$(/usr/local/bin/fims_send -m get -r /me -u /ess/naked/status/bms/MinCellVoltage)
VOLTAGE_START=$(/usr/local/bin/fims_send -m get -r /me -u /ess/naked/status/bms/MaxCellTemp)



echo "MaxCellVoltage is: ${green}$SOC_START${reset}. MinCellVoltage is: ${green}$SOH_START${reset}. MaxCellTemp is: ${green}$VOLTAGE_START${reset} "
...
echo "Set up /system/bms for test purposes"; echo
/usr/local/bin/fims_send -m set -r /me -u /ess/status/bms ' 
      {"MaxCellTemp": {"value": 60},
        "MinCellVoltage": {"value": 3},
        "MaxCellVoltage": {"value": 4}
      }'
...
sohtext=$green
voltagetext=$green
SOC_UPDT=$(/usr/local/bin/fims_send -m get -r /me -u /ess/naked/status/bms/MaxCellVoltage)
SOH_UPDT=$(/usr/local/bin/fims_send -m get -r /me -u /ess/naked/status/bms/MinCellVoltage)
VOLTAGE_UPDT=$(/usr/local/bin/fims_send -m get -r /me -u /ess/naked/status/bms/MaxCellTemp)
...
echo -e "Actual MaxCellVoltage: ${soctext}$SOC_UPDT${reset}. MinCellVoltage: ${sohtext}$SOH_UPDT${reset}. MaxCellTemp: ${voltagetext}$VOLTAGE_UPDT${reset}."
