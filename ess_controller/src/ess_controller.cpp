...
void fimsThread(scheduler* sc, fims* p_fims)
{
    ...
    FPS_PRINT_INFO("fims shutting down", NULL);
...
void initFuncs(asset_manager* am)
{
    ...
    if (1)
        FPS_PRINT_INFO("About to set process_sys_alarm for {}", am->name);
    ...
    for (auto& ix : am->assetManMap)
    {
        ...
        if (1)
            FPS_PRINT_INFO("Got asset manager {}. Setting func now...", amc->name);
        ...
        am->vm->setFunc(*am->vmap, amc->name.c_str(), "SiteBMSContactorControl",
                        (void*)&InputHandler::SiteBMSContactorControl);
        am->vm->setFunc(*am->vmap, amc->name.c_str(), "SitePCSStatusControl",
                        (void*)&InputHandler::SitePCSStatusControl);
        am->vm->setFunc(*am->vmap, amc->name.c_str(), "CloseContactorsEnable",
                        (void*)&ScheduledEnableFunctions::CloseContactorsEnable);
        am->vm->setFunc(*am->vmap, amc->name.c_str(), "OpenContactorsEnable",
                        (void*)&ScheduledEnableFunctions::OpenContactorsEnable);
        ...
        for (auto& iy : amc->assetMap)
        {
            asset* ami = iy.second;
            if (1)
                FPS_PRINT_INFO("Got asset instance {}. Setting func now...", ami->name);
...
void schedThread(scheduler* sc, varsmap* vmap, schlist* rreqs, asset_manager* am, fims* p_fims)
{
    ...
    while (*sc->run)
    {
        ...
        tNow = sc->vm->get_time_dbl();
        if (0)
            FPS_PRINT_INFO("Sched Tick", NULL);
        if (0)
            sc->showReqs(*rreqs);
        ...
        if (sc->msgChan.get(cmsg, false))
        {
            ...
            {
                FPS_PRINT_INFO("wakeup value to stop", NULL);
                *sc->run = 0;
            }
            free((void*)cmsg);
        }
        ...
        if (sc->reqChan.get(si, false))
        {
            if (si)
            {
                ...
                if (0)
                    FPS_PRINT_INFO("Servicing Sched Request {} id [{}] ({})  uri [{}] repTime {:2.3f}", fmt::ptr(si),
                                   fmt::ptr(si->id), fmt::ptr(si->id), si->uri, si->repTime);
                if (!si->id || (strcmp(si->id, "None") == 0))
                {
                    FPS_PRINT_INFO("this is not a schedItem", NULL);
                    delete si;
                    si = nullptr;
                }
                ...
                if (si)
                {
                    ...
                    if (oldsi)
                    {
                        ...
                        FPS_PRINT_INFO("schedItem deleted, seting repTime to 0.0", NULL);
                    }
                    // else
                    {
                        if (0)
            ...
            {
                FPS_PRINT_INFO("got nullptr si request !!", NULL);
            }
        }
    ...
    tNow = sc->vm->get_time_dbl();
    FPS_PRINT_INFO("shutting down", NULL);
    return;
}

int loadAssetManagers(varsmap& vmap, asset_manager* ess_man, std::vector<char*>* syscpVec, fims* p_fims, char* essName)
{
    UNUSED(essName);
    ...
    if (ixa != vmap.end())
    {
        ...
        FPS_PRINT_INFO("ESS >> We found our assets, we should be able to set up our system", NULL);
        for (auto iy : ixa->second)
        {
            if (iy.second->type == assetVar::ASTRING)
            {
                FPS_PRINT_INFO("lets run assets for  [{}] from [{}]", iy.first, cstr{ iy.second->aVal->valuestring });
                ...
                if (iy.first == "ess" || iy.first == "bms" || iy.first == "pcs" || iy.first == "site")
                {
                    ...
                    // ccnt = 0;
                    if (1)
                        FPS_PRINT_INFO("running with vmap [{}]", fmt::ptr(&vmap));
                    if (1)
                        FPS_PRINT_INFO("syscVec size [{}]", syscpVec->size());
                    ...
                    if (iy.first == "pcs")
                        PCSInit(vmap, Av->am->amap, aname, p_fims, Av);
...
int loadSiteManager(VarMapUtils* vm, varsmap& vmap, const char* sname, const char* aname)
{
    ...
    {
        if (1)
            FPS_PRINT_INFO("ess_man >> no site interface found", NULL);
        rc = -1;
    }
    return rc;
}
...
void print_version();
void print_help(VarMapUtils* vm, const char* fdir, const char* fname, const char* cfg, const char* cfile,
                const char* subs)
{
    UNUSED(vm);
    UNUSED(fdir);
    ...
    auto tmp = fmt::format(
        ...
        fname ? fname : "Undefined", cfg ? cfg : "Undefined", cfile ? cfile : "Undefined", subs ? subs : "Undefined");
    FPS_PRINT_INFO("{}\n", tmp.c_str());
}
...
void load_config_file(const char* cfile)
{
    ...
    else
    {
        ...
        av = vm.getVar(vmap, "/sysconfig/default", "Subs");
        if (av)
        {
            FlexSubs = av->getcVal();
        }
        av = vm.getVar(vmap, "/sysconfig/default", "Config");
        if (av)
        {
            FlexConfig = av->getcVal();
        }
        av = vm.getVar(vmap, "/sysconfig/default", "EssName");
        if (av)
        {
            FlexName = av->getcVal();
            essName = (char*)FlexName;
        }
...
int main_test_new_ess(int argc, char* argv[])
{
    ...
    if (!useArgs)  // run "ess_controller" to get into this block (crashes bc of filepath rn)
    {
        ...
        if (0)
            FPS_PRINT_INFO("ess_man >> syscVec size [{}]", syscpVec->size());
        ...
        if (1)
            FPS_PRINT_INFO("ess_man >> Setting up site manager rc {}", rc);
        ...
        {
            if (1)
                FPS_PRINT_INFO("ess_man >> Subs found in ess config", NULL);

            p_fims = sched.fimsSetup((const char**)mysubs, mysublen, "EssSched",
                                     vm.argc);  // TODO v1.0.1  set up fims AFter we get all the subs
            vm.clearList(mysubs, FlexName, mysublen);
            if (1)
                FPS_PRINT_INFO("ess_man >> p_fims {} sched {}", fmt::ptr(p_fims), fmt::ptr(sched.p_fims));
        }
        else
        {
            if (1)
                FPS_PRINT_INFO("ess_man >> No Subs found in ess config", NULL);
            return 0;
        }
        ...
        if (1)
            FPS_PRINT_INFO("ess_man >> p_fims {}", fmt::ptr(p_fims));
        ...
        FPS_PRINT_INFO("ESS >> Setting vlinks", NULL);
        vm.setVLinks(vmap, essName);  //**
        FPS_PRINT_INFO("ESS >> Done Setting vlinks", NULL);
    ...
    else
    {
        ...
        {
            if (1)
                FPS_PRINT_INFO(" flex_man >> No Subs found in flex config", NULL);
            if (nsubkeys == 0)
            {
    ...
    if (sThread.joinable())
        sThread.join();
    FPS_PRINT_INFO("threads done cleaning up ...", NULL);
    ...
    delete ess_man;
    int idx = 0;
    FPS_PRINT_INFO("things found in syscpVec ... ", NULL);
    for (auto x : *syscpVec)
    {
