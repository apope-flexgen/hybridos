...
struct DataMap;
std::unordered_map<std::string, DataMap*> dataMaps;  // a map of all dataMaps
                                                     // and their names. used to
                                                     // access dataMaps outside
                                                     // of their creation
                                                     // function
...
// for all simulink types, our naming convention drops the _T from each type
// (ex: uint32_T is the "uint32" case)
bool DataMap::getDataItemFromAmap(asset_manager* am, DataMap* dataMap, const std::string& amapName,
...
// false if not found for all simulink types, our naming convention drops the _T
// from each type (ex: uint32_T is the "uint32" case)
bool DataMap::setDataItemToAmap(asset_manager* am, DataMap* dataMap, const std::string& amapName,
                                const std::string& mapName, uint8_t* dataArea)
{
    if (am->amap.find(amapName) != am->amap.end() && dataMap->dataItems.find(mapName) != dataMap->dataItems.end() &&
    ...
    {
        DataItem* dataItem = dataMap->dataItems[mapName];
        if (dataItem->type == "int" || dataItem->type == "uint" || dataItem->type == "int16" ||
            dataItem->type == "uint16" || dataItem->type == "int32" || dataItem->type == "uint32" ||
            dataItem->type == "int64" || dataItem->type == "uint64" || dataItem->type == "ulong" ||
            dataItem->type == "ulonglong")
        {
            am->amap[amapName]->setVal(*(int*)(&dataArea[dataItem->offset]));
        }
        else if (dataItem->type == "double" || dataItem->type == "real" || dataItem->type == "real32" ||
                 dataItem->type == "real64" || dataItem->type == "time")
        {
...
void setupDMTest()
{
    ...
    dm->addDataItem((char*)"DMTestDirection", offsetof(Datamaps_test::ExtUPointer_Reference_T, DMTestDirection),
                    (char*)"bool");

    // output
    dm->addDataItem((char*)"DMTestOut", offsetof(Datamaps_test::ExtYPointer_Reference_T, DMTestOut), (char*)"int32");
    dm->addDataItem((char*)"AdderResult", offsetof(Datamaps_test::ExtYPointer_Reference_T, AdderResult),
                    (char*)"int32");
    dm->addDataItem((char*)"GainResult", offsetof(Datamaps_test::ExtYPointer_Reference_T, GainResult), (char*)"int32");
    dm->addDataItem((char*)"DMTestOutDblNoise", offsetof(Datamaps_test::ExtYPointer_Reference_T, DMTestOutDblNoise),
                    (char*)"real");
    dm->addDataItem((char*)"DirectionFeedback", offsetof(Datamaps_test::ExtYPointer_Reference_T, DirectionFeedback),
                    (char*)"bool");
...
int runDataMaps(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* aV)
{
    ...
    {
        if (debug)
            FPS_PRINT_INFO("Reloading ...");
        // datamap and amap init functions (what used to be setup_demo_rack and
        // setup_demo_rack_amap)
        setupDMTest();

        setupDMtestAmap(vm, vmap, testDMam);
        if (debug)
            FPS_PRINT_INFO("amap set up");
        ...
        if (debug)
            FPS_PRINT_INFO("Finished reloading");
    }
    ...
    if (debug)
    {
        ...
        FPS_PRINT_INFO("dmTestOutput.DirectionFeedback: {}",
                       dmTestOutput.DirectionFeedback);  // NEW! testing
        FPS_PRINT_INFO("dmTestOutput.DMTestOut: {}", dmTestOutput.DMTestOut);
        FPS_PRINT_INFO("dmTestOutput.DMTestOutDblNoise: {}",
                       dmTestOutput.DMTestOutDblNoise);  // NEW! testing
...
TO RUN THINGS
    ...
    This runs the func twice (change inValue to != 0 if you only want it to run
once) fims_send -m set -r /$$ -u /ess/demo/code '{ "runExtCode":{ "value":0,
        "actions":{
            "onSet":[
                {"func":
                    [
...
'{"runExtCode":{"value":15,"cmd":"setup_demo","pname":"rack","num_racks":2,
"num_modules":2}}'


    to test set_vlink
fims_send -m set -r /$$ -u /ess/demo/code
'{"runExtCode":{"value":0,"cmd":"set_vlink","amap":"someVar","amname":"ext_1",
"uri":"/components/ext_1:someVar","val":1111}}'


    to test set_amap
fims_send -m set -r /$$ -u /ess/demo/code
'{"runExtCode":{"value":0,"cmd":"set_amap","amap":"someVar","amname":"ext_1",
"uri":"/sys/status:someVar","val":2222}}'


    this sets up the parent and the asset  using set_mapping
fims_send -m set -r /$$ -u /ess/demo/code
'{"runExtCode":{"value":3,"cmd":"set_mapping","pname":"ext","amname":"ext_1"}}'
fims_send -m set -r /$$ -u /ess/demo/code
'{"runExtCode":{"value":3,"cmd":"set_mapping","pname":"ext","amname":"ext_2"}}'
...
fims_send -m get -r /$$ -u /ess/control | jq
    ...
    add /field to look at specific value (fims_send -m get -r /$$ -u
/ess/control/rack_0/max_charge_capacity | jq for example)
...
fims_send -m set -u /ess/control/rack_X_Y/field_to_be_set 15
    only fields rn are charge_capacity, charge_current, max_charge_capacity, &
soc
