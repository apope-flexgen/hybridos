    ...
    while (*sc->run)
    {
        ...
        if (sc->reqChan.get(si, false))
        {
            if (si)
            {
                ...
                FPS_ERROR_PRINT(
                    "%s >> Servicing  Sched Request %p id [%s] (%p)  uri "
                    "[%s] repTime %2.3f at %2.3f\n",
...
int loadAssetManagers(varsmap& vmap, asset_manager* ess_man, std::vector<char*>* syscpVec, fims* p_fims)
{
    ...
    if (ixa != vmap.end())
    {
        ...
        FPS_ERROR_PRINT(
            "%s >> GPIO >>We found our assets, we should be able to "
            "set up our system\n",
            __func__);
...
int main_test_new_gpio(int argc, char* argv[])
{
    ...
    if (1)
        FPS_ERROR_PRINT(" %s >> gpio start load controller \n", __func__);
    rc = loadEssConfig(&vm, vmap, "gpio_controller.json", gpio_man, &sched);
    if (1)
        FPS_ERROR_PRINT(" %s >> gpio done load controller rc %d \n", __func__, rc);
    ...
    if (0)
    {
        ...
        {
            if (1)
                FPS_ERROR_PRINT(" %s >> gpio_man >> Error in setting up site manager \n", __func__);
            return 0;
        }
    }
    ...
    if (1)
        FPS_ERROR_PRINT(" %s >> ess_man >> p_fims %p\n", __func__, (void*)p_fims);
    // return 0;
    ...
    // "gpio_schedule.json", gpio_man, &sched); if (1)FPS_ERROR_PRINT(" %s >> gpio
    // done  load schedule rc %d \n", __func__, rc);

    double tNow = vm.get_time_dbl();
    assetVar* aV = vmap["/config/gpio"]["gpios"];
    if (1)
        FPS_ERROR_PRINT(" %s >> gpio aV %p \n", __func__, (void*)aV);
    if (aV)
    {
    ...
    {
        if (1)
            FPS_ERROR_PRINT(" %s >> gpio aV error %p \n", __func__, (void*)aV);
        return 0;
    }
...
int main(int argc, char* argv[])
{
    ...
    // offers safety and extensibilty, even more safety once we move to C++17 and
    // up
