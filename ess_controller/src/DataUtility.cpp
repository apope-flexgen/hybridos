...
{
/**
 * @brief gives the ability to populate the amap with a singular given assetVar based on the information given in the
 * "info" variable
 *
    ...
    if (!av)
    {
        switch (info->type)
        {
            ...
            case assetVar::ATypes::ASTRING:
                ...
                break;
            default:
                FPS_PRINT_ERROR("Error in Switch Statement of PopulateAmapWithAv - No known type given for AV",
                                nullptr);
        }
    }
    amap[amId] = av;
...
/**
 * @brief allows to populate multiple assetVars within the amap at once by calling with a vector list of the desired
 * assetVars, this will then call the PopulateAmapWithAv for each one
 *
 * @param vmap the global data map shared by all assets/asset managers
 * @param amap the local data map used by an asset/asset manager
 * @param vm  VarMapUtils pointer
...
void PrintAssetVar(assetVar* aV, assetVar::ATypes assetVarValType)
{
    ...
    if (0)
        FPS_PRINT_INFO("Start of PrintAssetVar", NULL);
    ...
    switch (assetVarValType)
    {
        case assetVar::ATypes::AINT:
            if (0)
                FPS_PRINT_INFO("Before iParam", NULL);
            assetVarString += fmt::format("    \"value\": {},\n", aV->getiVal());
            break;
        case assetVar::ATypes::AFLOAT:
            if (0)
                FPS_PRINT_INFO("Before dVal", NULL);
            assetVarString += fmt::format("    \"value\": {},\n", aV->getdVal());
            break;
        case assetVar::ATypes::ABOOL:
            if (0)
                FPS_PRINT_INFO("Before bVal", NULL);
            assetVarString += fmt::format("    \"value\": {},\n", aV->getbVal());
            break;
        case assetVar::ATypes::ASTRING:
            if (0)
                FPS_PRINT_INFO("Before cVal", NULL);
            assetVarString += fmt::format("    \"value\": \"{}\",\n", aV->getcVal());
            break;
        default:
            break;
    ...
    for (auto it = paramToDataTypeDict.begin(); it != paramToDataTypeDict.end(); ++it)
    {
        ...
        if (0)
            FPS_PRINT_INFO("paramString {}", paramString);

        if (aV->gotParam(paramString.c_str()))
        {
            if (0)
                FPS_PRINT_INFO("Param was found", NULL);
            switch (paramType)
            {
                case assetVar::ATypes::AINT:
                    if (0)
                        FPS_PRINT_INFO("iParam {}", aV->getiParam(paramString.c_str()));
                    assetVarString += fmt::format("    \"{}\": {}", paramString, aV->getiParam(paramString.c_str()));
                    break;
                case assetVar::ATypes::AFLOAT:
                    if (0)
                        FPS_PRINT_INFO("dParam {}", aV->getdParam(paramString.c_str()));
                    assetVarString += fmt::format("    \"{}\": {}", paramString, aV->getdParam(paramString.c_str()));
                    break;
                case assetVar::ATypes::ABOOL:
                    if (0)
                        FPS_PRINT_INFO("bParam {}", aV->getbParam(paramString.c_str()));
                    assetVarString += fmt::format("    \"{}\": {}", paramString, aV->getbParam(paramString.c_str()));
                    break;
                case assetVar::ATypes::ASTRING:
                    if (0)
                        FPS_PRINT_INFO("cParam {}", aV->getcParam(paramString.c_str()));
                    assetVarString += fmt::format("    \"{}\": \"{}\"", paramString,
                                                  aV->getcParam(paramString.c_str()));
                    break;
                default:
                    break;
            }
...
std::unordered_map<std::string, std::string> controlsEnabledLogicMap = { { "close_contactors", "" },
                                                                         { "open_contactors", "" },
                                                                         { "start", "" },
                                                                         { "stop", "" },
                                                                         { "standby", "" }
