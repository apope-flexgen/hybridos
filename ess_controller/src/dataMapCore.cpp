...
bool checkParams(varsmap& vmap, assetVar* aV)
{
    ...
    if (enable == false || enabled == false)
    {
        ...
        if (debug)
            FPS_PRINT_INFO("This assetVar [{}] not enabled", aV->name);
        return false;
    }
...
void reConfigure(assetVar* aV)
{
    ...
    while (++num)
    {
        ...
        else
        {
            ...
            if (debug)
                FPS_PRINT_INFO("No {} param found", funcX);
            break;
        }
    }
    ...
    for (int x = 1; x < (int)funcNamesVec.size(); x++)
    {
        ...
        if (debug)
            FPS_PRINT_INFO(" editing aV [{}]'s parameter [{}] to be [{}]", aV->name, funcX, funcNamesVec.at(x));
    }
...
void addAVtoThread(assetVar* aV, ess_thread* ess, assetVar* threadAV)
{
    ...
    else
    {
        ...
        while (++num)
        {
            ...
            if (debug)
                FPS_PRINT_INFO("Adding \"{}: {}\" as a param on thread AV", funcGroupNum, funcNumName);
        }
    }
...
void setTransferAV(assetVar* aV, assetVar* transferAV, std::string comp)
{
    ...
    if (debug)
        FPS_PRINT_INFO("adding aV [{}] to asset manager [{}]'s amap with key [{}_transfer]", transferAV->name,
                       aV->am->name, dmName);
...
int limitSetupTime(assetVar* aV, assetVar* threadAV)
{
    ...
    if (!aV->gotParam("checkSetupLimit"))
    {
        ...
        if (every == 0)
            every = 1;  // if somehow our repTime is 0, we dont want to divide by 0
        ...
        if (debug)
            FPS_PRINT_INFO("SETUP_CHECK will check {} times before signaling an error", iteration_limit);
    }
    ...
    if (debug)
        FPS_PRINT_INFO("On iteration [{}]. iteration limit is [{}]", check_setup_iteration, check_setup_limit);
    ...
    if (ess)
    {
        ...
        if (reload < 2)
        {
            if (reload < 1)
            {
                ...
                if (debug)
                    FPS_PRINT_INFO("   FULL RELOAD for [{}]", aV->name);
            ...
            // partial reload
            if (debug)
                FPS_PRINT_INFO("   partial RELOAD for [{}]", aV->name);
        ...
        if (aV->getbParam("done") && !aV->getbParam("error"))
        {
            ...
            aV->setParam("done", false);
            if (debug)
                FPS_PRINT_INFO("function(s) for aV [{}] have finished running, ready to run again", aV->name);
        }
        ...
        if (ess->core == SETUP_CONTEXT)  // 11
        {
            ...
            if (setup_state == SETUP_CHECK)
            {
                ...
                if (!setupDone(aV, setup_iteration))
                    return 0;  // if setup is not done, return and wait for next iteration

                // if we get here it is because setupDone returned true
                if (debug)
                    FPS_PRINT_INFO("Set up of aV [{}] is done, setting \"runStatus\" to RUN_CONTEXT", aV->name);
                aV->setParam("runStatus", RUN_CONTEXT);
        ...
        if (ess->core == ERROR_ON_CONTEXT)  // 19
        {
            ...
            char* faultOrAlarm = fault ? am->amap["FaultDestination"]->getcVal()
                                       : am->amap["AlarmDestination"]->getcVal();
            ...
            if (debug)
                FPS_PRINT_INFO(" aV [{}] is sending an error to [{}] with message [{}]", aV->name, destUri, errMsg);
            vm->sendAlarm(vmap, aV, destUri.c_str(), nullptr, errMsg.c_str(),
                          SEVERITY);  // eventually, have severity variable
            ...
            if (fault)
            {
                ...
                if (debug)
                    FPS_PRINT_INFO("Setting threadAV [{}] param [{}] to false", threadAV->name, funcNumRunning);
            }
            else
            {
        ...
        if (ess->core == WAITING_FOR_DONE)  // 10
        {
            ...
            ESSLogger::get().critical("Datamap [{}] function has overrun on assetVar [{}]",
                                      aV->getcParam("datamapName"), aV->name);
            if (logging_enabled)
            {
                ...
                if (aV->getbParam("heartbeatEnable"))
                {
                    ...
                    if (aV->gotParam("lastHeartbeat"))
                    {
                        ...
                        else
                        {
                            ...
                            if (debug)
                                FPS_PRINT_INFO("Setting \"{}\" to its last heartbeat value of [{}]", param,
                                               lastResetTime);
                        }
                    }
                }
