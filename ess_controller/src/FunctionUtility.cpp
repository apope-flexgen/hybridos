 ...
 * @param scheduledFuncName function on the scheduler that is the origin of the run process ex: LocalStartBMS,
 * LocalStopPCS, etc
 * @param updatedAssetVarName the string on the /assets/X/summary page that got updated and caused the scheduled
 * function to get scheduled ex: /assets/bms/summary/[close_contactors], /assets/pcs/summary/[start], etc
 * @param infoMessage a string message that could give any helpful information on the status of the run process
 */
void FunctionResultHandler(int returnValue, varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* aV,
                           const char* scheduledFuncName, const char* updatedAssetVarName, const char* infoMessage)
{
    UNUSED(vmap);
    if (0)
        FPS_PRINT_INFO("{}", __func__);
    ...
    if (event)
    {
        ...
        if (0)
            FPS_PRINT_INFO("aname [{}]", cstr{ aname });
        if (0)
            FPS_PRINT_INFO("val [{}]", aV->getdVal());
        if (0)
            FPS_PRINT_INFO("assetVarName [{}] ", aV->getName());
        if (0)
            FPS_PRINT_INFO("\nassetVar [{}] \n", aV->getfName());
        if (0)
            FPS_PRINT_INFO("schedUri [{}] ", aV->comp.c_str());
        if (0)
            FPS_PRINT_INFO("scheduledFuncUri [{}] ", aV->getName());
...
{
    if (0)
        DataUtility::PrintAssetVar(aV, assetVar::ATypes::AFLOAT);
    aV->setParam("endTime", 1);

    if (!amap[updatedAssetVarName])
        return;
...
void SharedInputHandlerLocalFunction(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* aV,
                                     const char* inputHandlerFuncName)
{
    ...
    std::function<FunctionUtility::FunctionReturnObj(varsmap&, varmap&, const char*, fims*, assetVar*, const char*)>
    ...
    std::vector<DataUtility::AssetVarInfo> assetVarVector = { DataUtility::AssetVarInfo(
        assetsUri.c_str(), uiUriName.c_str(), uniqueAssetUri.c_str(), assetVar::ATypes::ABOOL) };
    ...
    if (!amap[uniqueAssetUri.c_str()]->getbVal())
        return;
    ...
    FunctionUtility::FunctionReturnObj returnObject = outputHandlerFunction(vmap, amap, aname, p_fims, aV,
                                                                            uniqueAssetUri.c_str());
    int returnValue = returnObject.statusIndicator;
    std::string message = returnObject.message;

    FunctionUtility::FunctionResultHandler(returnValue, vmap, amap, aname, p_fims, aV, scheduledFuncName.c_str(),
                                           uniqueAssetUri.c_str(), message.c_str());
}

int SharedInputHandlerRemoteFunction(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* aV,
                                     std::string uri, std::string scheduledFuncName, std::string outputHandlerFuncName)
{
    if (0)
        FPS_PRINT_INFO("{}", __func__);
    ...
    std::string message = returnObject.message;
    FunctionUtility::FunctionResultHandler(returnValue, vmap, amap, aname, p_fims, aV, scheduledFuncName.c_str(),
                                           uri.c_str(), message.c_str());

    return returnValue;
}

/**
 * @brief Shared function for all of the OutputHandler functions for resetting certain parameters that are necessary to
 * make the process work properly
 *
 * @param vmap the global data map shared by all assets/asset managers
 * @param amap the local data map used by an asset/asset manager
 * @param uiUriKey the string for the uri from /assets/X/summary    ex: close_contactors, open_contactors, start, stop,
 * standby
 * @param controlsUriKey the string for the uri in /ess/controls/X    ex: CloseContactors, OpenContactors, Start, etc.
 * @param verifyControlsUriKey the string for the uri in /ess/controls/X that verifies the actual control    ex:
 * VerifyCloseContactors, VerifyStart, etc.
 */
varmap& SharedAmapReset(varmap& amap, VarMapUtils* vm, const char* uiUriKey, const char* controlsUriKey,
                        const char* verifyControlsUriKey)
{
    if (0)
        FPS_PRINT_INFO("{}", __func__);
    ...
    if ((!amap[controlsUriKey]->getbParam("triggerCmd") && !amap[controlsUriKey]->getbParam("cmdSent")) ||
        !amap[controlsUriKey]->gotParam("triggerCmd"))
    {
...
/**
 * @brief Shared function for all OutputHandler functions which walks through 4 phases: Start, Control, Trigger
 * VerifyControl, VerifyControl. There are two calls to HandleCmd for the control and the verify control ex:
 * CloseContactors and VerifyCloseContactors
 *
 * @param vmap the global data map shared by all assets/asset managers
 * @param amap the local data map used by an asset/asset manager
 * @param aname the name of the asset/asset manager
 ...
 */
FunctionReturnObj SharedHandleCmdProcess(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* aV,
                                         HandleCmdProcessUris uris)
{
    ...
    if (reload == 0)
    {
        ...
        reload = 1;
        if (0)
            FPS_PRINT_INFO("Completed Phase 0 (Start)", nullptr);
    }
    ...
    if (reload == 1)
    {
        ...
        switch (returnObject.statusIndicator)
        {
            case SUCCESS:
                ...
                reload = 2;
                if (0)
                    FPS_PRINT_INFO("Completed Phase 1 (Control)", nullptr);
                break;
            case RESET:
                hcAV->setVal(0);
                if (0)
                    FPS_PRINT_INFO("Resetting From Phase 1", nullptr);
            default:
                break;
        }
    }
    ...
    if (reload == 2)
    {
        ...
        reload = 3;
        if (0)
            FPS_PRINT_INFO("Completed Phase 2 (Trigger VerifyControl)", nullptr);
    }
    ...
    if (reload == 3)
    {
        ...
        switch (returnObject.statusIndicator)
        {
            case RESET:
                hcAV->setVal(0);
                if (0)
                    FPS_PRINT_INFO("Resetting From Phase 3", nullptr);
            default:
                break;
        }
    }
 ...
 * @param p_fims the interface used for data interchange
 * @param av the assetVar that contains the command value to send
 * @param controlString the string for the /ess/controls/X/[] uri **Can be either control or verifyControl**    ex:
 * CloseContactors, VerifyCloseContactors, Start, VerifyStart, etc.
 */
FunctionReturnObj SharedIndividualHandleCmdLogic(varmap& amap, const char* aname, fims* p_fims, assetVar* aV,
                                                 HandleCmdLogicUris uris)
{
    if (0)
        FPS_PRINT_INFO("{}", __func__);
    ...
    {
        FPS_PRINT_INFO("NO CONTROL STRING", NULL);
    ...
    if (!amap[control.c_str()]->getbParam("triggerCmd") && !amap[control.c_str()]->getbParam("cmdSent"))
    {
        if (amap[controlSuccess.c_str()])
        {
            ...
            {
                amap[controlAlarm.c_str()]->setVal(
                    (InfoMessageUtility::controlAlarmMessage(control, assetName)).c_str());
                returnObject.statusIndicator = FAILURE;
                returnObject.message = InfoMessageUtility::handleCmdTimedOutFail(control, controlSuccess);
                returnObject.message += fmt::format("  -> Condition(s): [{}:{}] == true",
        ...
        else
        {
            ...
            returnObject.message = fmt::format("{} doesn't exist and therefore the control, {}, cannot be validated.",
                                               controlSuccess, control);
        }
    }
    else
    {
