...
int assetUri::setupUriVec()
{
    ...
    do
    {
        ...
        if (sp1)
        {
            *sp1++ = 0;
            uriVec.push_back(sp);
            if (0)
                FPS_PRINT_INFO("push back [{}]", cstr{ sp });
            sp = sp1;
        }
    } while (sp1);
    if (sp != vecUri)
    {
        uriVec.push_back(sp);
        if (0)
            FPS_PRINT_INFO("push back last [{}]", cstr{ sp });
    }
    return (int)uriVec.size();
}
// was pull_one_uri
...
char* assetUri::pullPvar(int idx)
{
    ...
    if (idx > ix)
        return nullptr;
    if (idx <= 0)
        return nullptr;  // Phil said this is okay;
...
int asset::configure(const char* fname, std::vector<std::pair<std::string, std::string>>* reps, asset_manager* am,
                     asset* ai)
{
    // VarMapUtils vm;
    int rc = -1;
    ...
    while (cj)
    {
        ...
        if (0)
            FPS_PRINT_INFO("cj->string [{}] child [{}] body [{}]", cstr{ cj->string }, fmt::ptr(cj->child),
                           cstr{ body });
        ...
        if (0)
            FPS_PRINT_INFO("configured [{}]", cstr{ buf });
        free((void*)buf);
...
// //int asset::Send(const char* method, const char*uri, const char*rep, const
// char* body)
// {
...
{
    if (1)
        FPS_PRINT_INFO("asset manager running cleanup", NULL);
    cleanup();
}
...
{
    if (0)
        FPS_PRINT_INFO("asset manager setting vmap", NULL);
    vmap = _vmap;
}
...
{
    if (0)
        FPS_PRINT_INFO("asset manager getting vmap", NULL);
    return vmap;
}
...
// TODO after MVP remove the asset/asset manager split make them the same
// structure
void asset_manager::assconfigure(varsmap* vmap, const char* fname, const char* aname)
{
    ...
    while (cja)
    {
        ...
        if (0)
            FPS_PRINT_INFO("found asset [{}]", cstr{ cja->string });
        if (0)
            FPS_PRINT_INFO("found asset [{}] template [{}]", cstr{ cja->string },
                           cjt->valuestring ? cjt->valuestring : "No template");

        cJSON* cjs = cJSON_GetObjectItem(cja, "subs");
        if (0)
            FPS_PRINT_INFO("found asset [{}] subs {} isArray {}", cja->string, fmt::ptr(cjs), cJSON_IsArray(cjs));
        if (cJSON_IsArray(cjs))
        {
            ...
            cJSON_ArrayForEach(cjsi, cjs)
            {
                ...
                {
                    if (1)
                        FPS_PRINT_INFO("found asset [{}] sub from [{}] to [{}]", cstr{ cja->string },
                                       cstr{ cjsr->valuestring }, cstr{ cjsw->valuestring });
    ...
    if (1)
        FPS_PRINT_INFO("seeking links [{}] in file [{}] cja {}", cstr{ assname }, fname, fmt::ptr(cja));
    if (cja)
    {
        if (1)
            FPS_PRINT_INFO("found links [{}]", cstr{ cja->string });
...
int asset_manager::amConfig(varsmap* vmap, cJSON* cj, asset_manager* am)
{
    char* body = cJSON_Print(cj);
    if (1)
        FPS_PRINT_INFO("cj->string [{}] child [{}] body [{}]", cstr{ cj->string }, fmt::ptr(cj->child), cstr{ body });
    ...
    if (1)
        FPS_PRINT_INFO("configured [{}] [{}]", cstr{ cj->string }, cstr{ buf });
    if (buf)
        free(buf);
    return 0;
}
// Note syscVec is for Uri ordering
// This is the asset_manager config we need the wake up function and the asset
// manager pointer
// Used here if(ass_man->configure(&vmap, fname, aname, &syscVec, myAwake,
// ass_man) < 0)
int asset_manager::configure(varsmap* vmap, const char* fname, const char* aname, std::vector<char*>* syscVec,
                             bool (*assWake)(asset*, int), asset_manager* am)
{
    ...
    {
        if (1)
            FPS_PRINT_ERROR("error in fileName [{}] fname [{}]", cstr{ fileName }, fname);
        return rc;
    }
    rc = 0;
    if (1)
        FPS_PRINT_INFO("incoming vmap [{}]", fmt::ptr(vmap));
    ...
    cja = cJSON_GetObjectItem(cjbase, assname);
    if (0)
        FPS_PRINT_INFO("seeking links [{}] in file [{}] cja {}", assname, fname, fmt::ptr(cja));
    if (cja)
    {
        if (1)
            FPS_PRINT_INFO("found links [{}]", cstr{ cja->string });
    ...
    free((void*)assname);
    // This wont work .. put them inito the template file . They will be inserted
    // multiple tines but that's OK asprintf(&assname, "/components/%s", aname);
    // asprintf(&assname, "/components");
    // cja = cJSON_GetObjectItem(cjbase, assname);
    // if (0)FPS_PRINT_INFO(" %s >> seeking components  [%s] in file [%s] cja
...
bool asset_manager::runChildren(int wakeup)
{
    // return true;
    if (0)
        FPS_PRINT_INFO("%s Manager Loop >>>>>>>>>>> looking for child Asset Manager", name);
    ...
    if (1)
    {
        ...
        {
            if (0)
                FPS_PRINT_INFO("{} Manager Loop >>>>>>>>>>> running for Asset Manager [{}]", name, ix.first);
            ...
            if (am2->run_wakeup)
            {
                if (0)
                    FPS_PRINT_INFO(
                        "{} Manager Loop Wakeup >>>>>>>>>>> running for Asset "
                        "Manager [{}]",
                        name, ix.first);
            ...
            {
                FPS_PRINT_INFO(
                    "%s Manager Loop NO Wakeup >>>>>>>>>>> running for "
                    "Asset Manager [{}]",
                    name, ix.first);
            }
    ...
    if (1)
    {
        ...
        for (auto ix : assetMap)
        {
            ...
            if (0)
                FPS_PRINT_INFO("{} ASSETS >>>>>>>>>>> running for Asset [{}]", name, ix.first);
            // TODO after MVP remove the asset / asset manager wakeup structure is is
            // deprecated
            if (ass->run_wakeup)
...
// we are cutting down the complexity of this since we have developed the
// "cascade " concept.
void asset_manager::manager_loop()
{
    ...
    if (0)
        FPS_PRINT_INFO("MANAGER LOOP Starting for [{}]", name);
    ...
    while (wakechan.get(wakeup, true))
    {
        ...
        //
        if (0)
            FPS_PRINT_INFO("MANAGER LOOP Wakeup for [{}] wval {}", name, wakeup);
        ...
        {
            if (0)
                FPS_PRINT_INFO("MANAGER LOOP Run Wakeup for [{}] wval {}", name, wakeup);
            ...
            if (wakeup != 0)  // dont pass on controller level wakeups
            {
                ...
                if (0)
                {
                    ...
                    {
                        if (0)
                            FPS_PRINT_INFO("{} Manager Loop >>>>>>>>>>> running for Asset Manager [{}]", name,
                                           ix.first);
                        ...
                        if (am2->run_wakeup)
                        {
                            if (0)
                                FPS_PRINT_INFO(
                                    "{} Manager Loop Wakeup >>>>>>>>>>> running for "
                                    "Asset Manager [{}]",
                                    name, ix.first);
                            // first trigger the wakeup there is no thread in the lower leel
                            // managers
                        ...
                        {
                            FPS_PRINT_INFO(
                                "{} Manager Loop NO Wakeup >>>>>>>>>>> running "
                                "for Asset Manager [{}]",
                                name, ix.first);
                        }
...
void asset_manager::man_timer_loop()
{
    ...
    while (*td->run)
    {
        ...
        {
            if (0)
                FPS_PRINT_INFO("MANAGER LOOP {} POLL WAKEUPS", name);
...
void asset_manager::timer_loop()
{
    ...
    while (*td->run)
        ...
        while (*td->run)
        {
            ...
            if (0)
                FPS_PRINT_INFO("count {} mod {} count500 {}", td->count, (500 / td->delay),
            ...
            {
                if (tchange)
                    FPS_PRINT_INFO("TIMER LOOP {} Wake1 Time", name);
            ...
            {
                if (tchange)
                    FPS_PRINT_INFO("TIMER LOOP {} Wake2 Time", name);
                td->wake_up_chan->put(WAKE_LEVEL2);  // 100Ms
            }
            if ((td->count % (wake3Time)) == 0)
            {
                if (tchange)
                    FPS_PRINT_INFO("TIMER LOOP {} Wake3 Time", name);
            ...
            {
                if (tchange)
                    FPS_PRINT_INFO("TIMER LOOP {} Wake4 Time", name);
...
void asset_manager::fims_loop()
{
    ...
    while (*td->run)
    {
        ...
        if (msg)
        {
            ...
            {
                if (0)
                    FPS_PRINT_INFO("name {} GET uri [{}]", name, cstr{ msg->uri });
            }
            if (*td->run)
            {
    ...
    FPS_PRINT_INFO("name {} fims shutting down", name);
...
{
    if (1)
        FPS_PRINT_INFO("STARTING for {}", name);
    if (1)
        FPS_PRINT_INFO("STARTING for {} amap.size {}", name, amap.size());
    for (auto& x : amap)
    {
        assetVar* av = x.second;
        if (1)
            FPS_PRINT_INFO("Running (no delete) for {} item [{}] {}", name, x.first, fmt::ptr(x.second));

        if (0 && av)
            FPS_PRINT_INFO("Running (no delete) for {} item [{}] {} [{}:{}]", name, x.first, fmt::ptr(x.second),
                           av ? av->comp.c_str() : "NoAvComp", av ? av->name.c_str() : "NoAvName");
        // if(x.second) delete x.second;
    }
    amap.clear();
    if (1)
        FPS_PRINT_INFO("{} assetMap.size {}", name, assetMap.size());
    for (auto aa : assetManMap)
    {
        asset_manager* am = aa.second;
        if (1)
            FPS_PRINT_INFO("asset_manager [{}] ->> [{}]", aa.first, am->name);
        delete am;
    }
    if (1)
        FPS_PRINT_INFO("{} assetMap.size {}", name, assetMap.size());
    for (auto aa : assetMap)
    {
        asset* ai = aa.second;
        if (1)
            FPS_PRINT_INFO("asset [{}] ->> [{}]", aa.first, ai->name);
        delete ai;
    }
    ...
    if (vecs)
    {
        ...
        if (1)
            FPS_PRINT_INFO("vecs->size {}", vsize);
            ...
            {
                {
                    ...
                    if (0)
                        FPS_PRINT_INFO("vecs [{}] vp {} size {}", aa.first, fmt::ptr(vp), vp->size());
                    for (auto ap : *vp)
                    {
                        if (0)
                            FPS_PRINT_INFO("vec [{}]", ap);
                    }
                }
            }
            // vecs->clear();
                             ...
                             int (*runAI)(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, asset* ai))
{
    ...
    if (am)
    {
        ...
        for (auto ix : am->assetManMap)
        {
            ...
            if (0)
                FPS_PRINT_INFO("cascading function to >>>>> Manager [{}]", amc->name);
            if (runAM)
        ...
        for (auto ix : am->assetMap)
        {
            ...
            if (0)
                FPS_PRINT_INFO("cascading function to >>>> Asset [{}]", amc->name);
...
// int asset_manager::Send(const char* method, const char*uri, const char*rep,
// const char* body)
// {
//     //p_fims->Send("pub", comp, nullptr, dvar);
//     int ival = p_fims->Send(method, uri, rep, body);
