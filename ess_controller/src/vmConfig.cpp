...
// notes find /path/backups/ -type f -mtime +2 -exec rsync -vPhd -e "ssh -p 512"
// {} --delete --ignore-existing me@host:/remote/path/server-backups/ \;
...
const std::map<std::string, const char*> autoCfgMap = { { "std_bms_manager_controls", std_bms_manager_controls_s },
                                                        ...
                                                        { "std_ess_controller_controls",
                                                          std_ess_controller_controls_s },
...
// fims_send -m set -u /dbi/ess_controller/configs_ess_controller -f
// /usr/local/etc/config/site_controller/assets.json  fims_send -m set -u
// /dbi/ess_controller/configs_risen_bms_manager -f
// /usr/local/etc/config/site_controller/sequences.json  fims_send -m set -u
// /dbi/ess_controller/configs_risen_bms_template -f
// /usr/local/etc/config/site_controller/variables.json
...
// int SimBmsOpenContactor(varsmap& vmap, varmap& amap, const char* aname, fims*
// p_fims, assetVar* aV);
...
// move to lib
// creates a schedule var (if needed) and runs a mapVar from the amap every x
// seconds

// move to lib
// creates a schedule var (if needed) and runs a mapVar from the amap every x
// seconds
void RunAv(varsmap& vmap, VarMapUtils* vm, varmap& amap, const char* aname, asset_manager* am, fims* p_fims,
           ...
           double tNow, double every /*0.1*/, double offset)
{
    UNUSED(tNow);
    if (0)
        FPS_PRINT_INFO(" running schedvar [{}] mapVal [{}]->[{}]\n", schedVar, mapVar, amap[mapVar]->getfName());
    ...
    if (0)
        FPS_PRINT_INFO(" after running schedvar [{}] mapVal [{}]->[{}]\n", schedVar, mapVar, amap[mapVar]->getfName());
}

// creates a schedule var (if needed) and stops a mapVar from the amap in x
// seconds
            ...
            double tNow, double in /*0.1*/)
{
    ...
    avs->am = am;
    if (0)
        FPS_PRINT_INFO(" >>>> stopping uri [{}] schedVar [{}] \n", amap[mapVar]->getfName(), schedVar);
    StopSched(vmap, amap, aname, p_fims, avs);
    if (0)
        FPS_PRINT_INFO(" >>>> stopped uri [{}] schedVar [{}] \n", amap[mapVar]->getfName(), schedVar);
}
...
// this looks for conifg files /config/cfiles and templates /config/tmpl  for
// items yet to be loaded and issues reload requests unti they arrive it also
// checks the  /config/load system for complete collections of files to load.
// cfg/cfile/ess/ess_controller.json     will cause a config file to be loaded
// immediately this can cause other /config/cfiles and config/tmpl  requests to
// be registered. /config/load will also specify a  list of cfiles and templates
// and cause the template evalustion system to run when all files are loaded.
// the tmpl files are loaded into a extras->tbody string  so they can be
// recovered and template substitution performed. one of the design goals was to
// make the substutution progressive to allow a bms to have racks , modules, and
// cells the list of keys (replace/with) is progressive
// rack_01/module_01/cell_01   etc. an evaluated cell template will be repeated
// a number of times each with a different aname ("rack_xx_module_yy_cell_zz")
...
//         "aname":"bms",
//         "options":[
//            { "tmpl":"bms_rack_tmpl.json", "from": 1, "to": 5, "type":"am",
//            "aname":"rack_{:02d}",
//                               "reps":[
//                                {"replace":"##RACK_ID##"
//                                ,"with":"rack_{:02d}"},
...
//                                ]
//           },
//           { "tmpl":"bms_module_tmpl.json","from": 1, "to": 38, "type":"am",
//           "aname":"module_{:02d}",
//                               "reps":[
//                                {"replace":"##MODULE_ID##"
//                                ,"with":"module_{:02d}"},
...
//                                ]
//           },
//            { "tmpl":"bms_cell_tmpl.json","from": 1, "to": 10,
//            "type":"ai""aname":"cell_{:02d}",
//                               "reps":[
//                                {"replace":"##CELL_ID##"
//                                ,"with":"cell_{:02d}"},
...
//                         // first look for the av , it may have already been
//                         loaded. assetVar*avf =
//                         vm->getVar(vmap,fr.c_str(),nullptr);
...
// they get loaded  into /config/cfiles:<file name> , they need an asset
// (ainame) or an asset manager (amname) who may need a parent (pname)
// }
bool checkcFile(varsmap& vmap, VarMapUtils* vm, cJSON* cjsi, const char* cfile)
{
    ...
    else
    {
        ...
        if (cjsi)
        {
            ...
            if (cjaname)
                aname = cjaname->valuestring;
            if (cjam)
                amname = cjam->valuestring;  // this will be an encoded string
            if (cjai)
                ainame = cjai->valuestring;
            if (cjap)
                pname = cjap->valuestring;
            if (cjbase)
                useConfigDoc = cJSON_IsTrue(cjbase);
        }
        if (av)
        {
            if (aname)
            ...
            if (amname)
                av->setcParam("amname", amname);
            if (pname)
                av->setcParam("pname", pname);
            if (ainame)
                av->setcParam("ainame", ainame);
            if (!useConfigDoc)
                av->setParam("useConfigDoc", useConfigDoc);
        }
    }

    if (!ret)
        if (0)
            FPS_PRINT_INFO(" seeking [{}]", cfile);
    return ret;
}
// use this for all files that are not templates
// they get loaded  into /config/tmpl:<file name> , then need an asset manager
// who may need a parent                       }
...
bool checkLoad(varsmap& vmap, VarMapUtils* vm, cJSON* cjsi, const char* tmpl)
{
    ...
    else
    {
        ...
        av = vm->setVal(vmap, lstr.c_str(), nullptr, ret);
        if (0)
            FPS_PRINT_INFO(" creating [{}] av[{}] --> [{}]", lstr, fmt::ptr(av),
...
{
    if (0)
        FPS_PRINT_INFO("   aname [{}] fname [{}]", aname ? aname : " No aname", fname ? fname : " No fname");
    ...
    if (fname && aname)
    {
        ...
        if (0)
            FPS_PRINT_INFO(" fims send [{}] reply [{}] cfile [{}]", fs, fr, fname);
        if (!vm->simdbi)
        {
            // if (vm->p_fims)
        ...
        else
        {
            ...
            {
                if (1)
                    FPS_PRINT_INFO(" >>>>>> no xname for [{}]", fname);
            }
            auto cms = fmt::format("{}fims_send -f {}.json -m set  -u {} &", FimsDir, xname ? xname : fname, fr);
            // system(cms.c_str());
            if (0)
                FPS_PRINT_INFO(" >>>>>> simdbi {}", cms);
        }
    }
    return 0;
}
...
int requestCfile(varsmap& vmap, VarMapUtils* vm, assetVar* av)
{
    ...
    // If the file parameter does not exist, we'll use the name of the assetVar as
    // part of the request for the cfile
    if (!fname)
    ...
    if (0)
        FPS_PRINT_INFO(" fims send fname [{}] simdbi [{}] aname [{}] useConfigDoc [{}]", cstr{ fname }, vm->simdbi,
                       cstr{ aname }, useConfigDoc);
    if (fname && aname)
    {
        // fname += strlen("cfile:");
        ...
        auto fr = fmt::format("/{}/cfg/cfile/{}/{}", vm->getSysName(vmap), aname, fname);
        if (0)
            FPS_PRINT_INFO(" fims send [{}] reply [{}] cfile [{}]", fs.c_str(), fr.c_str(), fname);
        ...
        else
        {
            ...
            auto xname = vm->getFileName(xfile.c_str());
            if (0)
                FPS_PRINT_INFO(" >>>>>> xname {}", fmt::ptr(xname));
...
int requestFfile(varsmap& vmap, VarMapUtils* vm, const char* final)
{
    ...
    // bool bval = false;
    // set this as false
    // it will come back as a cfile so /configs/cfile/<final> will be set true
    // when it arrives
    ...
    if (final && aname)
    {
        ...
        if (1)
            FPS_PRINT_INFO(" fims send [{}] reply [{}] cfile [{}]", fs.c_str(), fr.c_str(), final);
        if (!vm->simdbi)
        {
            if (vm->p_fims)
                vm->p_fims->Send("get", fs.c_str(), fr.c_str(), nullptr);
        ...
        else
        {
            ...
            if (0)
                FPS_PRINT_INFO(" >>>>>> xname {}", fmt::ptr(xname));
            auto cms = fmt::format("{}fims_send -f {}.json -m set  -u {} &", FimsDir, xname ? xname : final, fr);
            // system(cms.c_str());
            if (0)
                FPS_PRINT_INFO(" >>>>>> simdbi {}", cms);
        }
...
int requestTmpl(varsmap& vmap, VarMapUtils* vm, assetVar* av)
{
    ...
    if (0)
        FPS_PRINT_INFO(" fims send fname [{}] simdbi [{}] aname [{}]", cstr{ fname }, vm->simdbi, cstr{ aname });
    if (fname && aname)
    {
        auto fr = fmt::format("/{}/cfg/ctmpl/{}/{}", aname, aname, fname);
        if (0)
            FPS_PRINT_INFO(" fims reply [{}] cfile [{}] dbifs [{}]", fr.c_str(), fname, dbifs);
        auto fs = fmt::format("{}{}", dbifs, fname);
        if (0)
            FPS_PRINT_INFO(" fims send 1 [{}] cfile [{}]", fs, fname);
        if (vm->autoLoad && strstr(fname, "std_"))
        {
            int single = 0;
        ...
        {
            if (0)
                FPS_PRINT_INFO(" fims send 1a [{}] cfile [{}]", fs, fname);
            if (vm->p_fims)
                vm->p_fims->Send("get", fs.c_str(), fr.c_str(), nullptr);
        }
        else
        {
            if (1)
                FPS_PRINT_INFO(" fims send 1b fname [{}]", fname);
            auto xname = vm->getFileName(fname);
            if (0)
                FPS_PRINT_ERROR(" getFileName possibly  no xname [{}]", xname ? xname : "no xname");
            if (xname)
            {
                auto cms = fmt::format("{}fims_send 2 -f {}.json -m set  -u /{}{} &", FimsDir, xname, aname, fr);
                // system(cms.c_str());
                if (1)
                    FPS_PRINT_INFO(" >>>>>> simdbi {}", cms);
            }
...
int parseItem::Init(varsmap* _vmap, VarMapUtils* _vm)
{
    ...
    if (cjfrom)
        from = cjfrom->valueint;
    if (cjto)
        to = cjto->valueint;
    if (cjstep)
        step = cjstep->valueint;
    if (cjmult)
        mult = cjmult->valueint;
    if (cjadd)
        add = cjadd->valueint;
    ...
    if (cjrange)
    {
        ...
        if (1)
            FPS_PRINT_INFO(" range found [{}]", cjrange->valuestring);
        if (rangeav)
        {
            if (1)
                FPS_PRINT_INFO(" rangevar found [{}]", rangeav->getfName());
            bool bval = true;
            if (rangeav->gotParam("from"))
                from = rangeav->getiParam("from");
            if (rangeav->gotParam("to"))
                to = rangeav->getiParam("to");
            if (rangeav->gotParam("step"))
                step = rangeav->getiParam("step");
            if (rangeav->gotParam("mult"))
                mult = rangeav->getiParam("mult");
            if (rangeav->gotParam("add"))
                add = rangeav->getiParam("add");
            rangeav->setVal(bval);
        }
    }
    ...
    if (cjap)
        if (0)
            FPS_PRINT_INFO(" pname found [{}]", pname);
    if (cjai)
        if (0)
            FPS_PRINT_INFO(" ainame found [{}]", ainame);
...
int parseItem::eValIdx(int _idx)
{
    ...
    cJSON_ArrayForEach(cjri, cjra)
    {
        ...
        int radd = add;
        if (cjmult)
            rmult = cjmult->valueint;
        if (cjadd)
            radd = cjadd->valueint;

        // if(0)FPS_PRINT_INFO(" loaded rep [{}] with
        // [{}]",cjrep->valuestring,cjwith->valuestring);

        // If we are given cjwithids (withIds), which contains [bms_a, bms_b], for
        // example, then we'll replace the placeholder defined in cjrep (replace)
        // with each entry in cjwithids
        if (cjwithids && cJSON_IsArray(cjwithids))
        {
            ...
            if (entry)
            {
                ...
                if (0)
                    FPS_PRINT_INFO(" loaded rep [{}] with [{}] => [{}]  idx [{}]", cjrep->valuestring,
                                   entry->valuestring, vstr, _idx);

                repmap[cjrep->valuestring] = vstr;
            }
        }

        // Otherwise, replace the placeholder defined in cjrep (replace) with the
        // value defined in cjwith (with)
        else
        {
            auto vstr = fmt::format(cjwith->valuestring, (_idx * rmult) + radd);
            if (0)
                FPS_PRINT_INFO(" loaded rep [{}] with [{}] => [{}]", cjrep->valuestring, cjwith->valuestring, vstr);
...
int parseItem::setBody()
{
    ...
    if (0)
        FPS_PRINT_INFO(" av  [{}] cjtmpl [{}]->[{}]  body param av [{}] param [{}]", fmt::ptr(av), fmt::ptr(cjtmpl),
                       cjtmpl->valuestring, fmt::ptr(av), av ? av->gotParam("md5sum") : false);
    ...
    {
        if (0)
            FPS_PRINT_INFO(" av  [{}]->[{}] has no body param", fmt::ptr(av), av->getfName());
    }
...
{
    if (0)
        FPS_PRINT_INFO("ainame fixup ainame [{}] *rmap->size [{}] ", fmt::ptr(ainame), rmap->size());
    for (auto& xx : *rmap)
    {
        auto& fstr = xx.first;
        auto& tstr = xx.second;
        ...
        if (ainame)
        {
            start_pos = 0;
            if (0)
                FPS_PRINT_INFO("ainame [{}] fixup #1 [{}]", ainame, aistr);
            ...
            if (0)
                FPS_PRINT_INFO("ainame fixup #2 [{}]", aistr);
        }
        if (pname)
        {
            start_pos = 0;
            if (0)
                FPS_PRINT_INFO("pname fixup #1 [{}]", pstr);
            ...
            if (0)
                FPS_PRINT_INFO("pname fixup #2 [{}]", pstr);
        }
    }
...
bool setVecAmAi(std::vector<parseItem*>& piVec, int lvl)
{
    ...
    if (piVec[lvl]->pname)
    {
        ...
        if (!piVec[lvl]->pam)
        {
            ...
            if (0)
                FPS_PRINT_INFO(" pname Setting piVec  PAM [{}]", piVec[lvl]->pstr.c_str());
            ...
            if (lvl > 0)
            {
                ...
                if (1)
                    FPS_PRINT_INFO(" pname Setting  pam [{}] parent am [{}] ", piVec[lvl]->pstr.c_str(),
                                   piVec[lvl - 1]->am->name);
            }
            else
            {
                ...
                if (1)
                    FPS_PRINT_INFO(" pname Setting  pam [{}] parent base [{}] ", piVec[lvl]->pstr.c_str(),
                                   ambase->name);
            }
        }
        if (piVec[lvl]->pam)
    ...
    if (piVec[lvl]->amname)
    {
        ...
        if (!piVec[lvl]->am)
        {
            ...
            if (0)
                FPS_PRINT_INFO(" amname Setting piVec  AM [{}]", piVec[lvl]->amstr.c_str());
            ...
            if (lvl > 0)
            {
                ...
                if (1)
                    FPS_PRINT_INFO(" amname Setting am  [{}] parent am [{}] ", piVec[lvl]->amstr.c_str(),
                                   piVec[lvl - 1]->amstr.c_str());
            }
            else
            {
                ...
                if (1)
                    FPS_PRINT_INFO(" amname Setting am  [{}] parent base [{}] ", piVec[lvl]->amstr.c_str(),
                                   ambase->name);
            }
        }
    ...
    if (piVec[lvl]->ainame)
    {
        ...
        if (!piVec[lvl]->ai)
        {
            ...
            if (1)
                FPS_PRINT_INFO(" ainame Setting piVec  AI [{}]", piVec[lvl]->aistr.c_str());
            ...
            if (lvl > 0)
            {
                ...
                if (1)
                    FPS_PRINT_INFO(" ainame Setting ai  [{}] parent am [{}] ", piVec[lvl]->aistr.c_str(),
                                   piVec[lvl - 1]->amstr.c_str());
            }
            else
            {
                ...
                if (1)
                    FPS_PRINT_INFO(" ainame Setting ai  [{}] parent ambase [{}] ", piVec[lvl]->aistr.c_str(),
                                   ambase->name);
            }
        }
    }
...
bool runpiVec(std::vector<parseItem*>& piVec, int& lvl, int& idx)
{
    UNUSED(idx);
    if (0)
        FPS_PRINT_INFO(" running  level is [{}] av [{}] av->name [{}]", lvl, fmt::ptr(piVec[lvl]->av),
                       piVec[lvl]->av->name);

    if (!piVec[lvl]->av)
    {
        if (0)
            FPS_PRINT_INFO("   level [{}] has no av [{}] stop the process", lvl, fmt::ptr(piVec[lvl]->av));
        return false;
    }

    if (0)
        FPS_PRINT_INFO(" running  level is [{}] idx [{}] nextIdx [{}] step [{}]", lvl, piVec[lvl]->idx,
                       piVec[lvl]->nextIdx, piVec[lvl]->step);
    ...
    if (piVec[lvl]->idx != piVec[lvl]->nextIdx)
    {
        ...
        if (0)
            FPS_PRINT_INFO(" running  eValIdx level [{}] next [{}]", lvl, piVec[lvl]->nextIdx);
        piVec[lvl]->eValIdx(piVec[lvl]->nextIdx);
        piVec[lvl]->idx = piVec[lvl]->nextIdx;
        if (0)
            FPS_PRINT_INFO(" running  setBody level [{}] next [{}] ", lvl, piVec[lvl]->nextIdx);
        piVec[lvl]->setBody();
        if (0)
            FPS_PRINT_INFO(" running  applyRemap level [{}] next ", lvl, piVec[lvl]->nextIdx);
        for (int j = 0; j <= lvl; j++)
        {
            if (0)
                FPS_PRINT_INFO(" running  applyRemap level [{}] j [{}] ", lvl, j);
            piVec[lvl]->applyRepMap(&piVec[j]->repmap);
        }
        // set up assets and asset_managers
        setVecAmAi(piVec, lvl);
        // set config into vmap
        piVec[lvl]->vm->configure_vmapStr(*piVec[lvl]->vmap, piVec[lvl]->bstr.c_str(), piVec[lvl]->am, piVec[lvl]->ai,
                                          true);
        ...
        if (lvl < (int)piVec.size() - 1)
        {
            lvl += 1;
            if (0)
                FPS_PRINT_INFO(" setting level to [{}]", lvl);
        }
        else
        {
            piVec[lvl]->nextIdx = piVec[lvl]->nextIdx + (piVec[lvl]->step * piVec[lvl]->mult);
...
int configLoad(varsmap& vmap, VarMapUtils* vm, const char* xfile)
{
    ...
    if (av)
    {
        ...
        {
            if (0)
                FPS_PRINT_INFO("  FOUND  av [{}]  configDone", av->getfName());
            return 0;
        }
        if (0)
            FPS_PRINT_INFO("  SETTING av [{}]  configDone", av->getfName());
        ...
        {
            if (0)
                FPS_PRINT_INFO("  no cjopts options on av [{}] skipping ", av->getfName());
            return 0;
        }

        cJSON* cj = av->extras->optVec->cjopts;
        if (0)
            FPS_PRINT_INFO("  config load [{}] found cj [{}]  av [{}]", xfile, fmt::ptr(cj), av->getfName());
        auto tmp = cJSON_Print(cj);
        if (0)
            FPS_PRINT_INFO("  running cjopts  [{}]", tmp);
        free(tmp);
        ...
        cJSON_ArrayForEach(cji, cj)
        {
            ...
            // If we have a ctmpls field, then we'll want to grab the template data
            // (body) stored in each template assetVar's tbody and add them in one
            // parse item's assetVar's tbody
            if (cjtmpls)
            {
                FPS_PRINT_INFO(
                    "tmpls field is found in av [{}]. We'll perform "
                    "template expansions with tmpls instead of tmpl",
                    av->getfName());
                char* xtmp = cJSON_Print(cji);
                if (0)
                    FPS_PRINT_INFO(" adding parseItem [{}] for tmpls", xtmp);
                free(xtmp);
                ...
                // Add the first template assetVar that we get to the parse item's
                // assetVar field Exclude the first { and last } from the template data.
                // This is because we are combining other template data from other
                // template assetVars to the parse item's assetVar's tbody
                ...
                // Iterate through tmpls and add the template data to the parseItem's
                // assetVar's template body field (av->extras->tbody)
                ...
                for (int i = 1; i < sz; i++)
                {
                    ...
                    if (cJSON_IsString(currTmpl) && currTmpl->valuestring)
                    {
                        ...
                        if (end > start)
                        {  // prevent empty files from adding commas
                            pi->av->extras->tbody += fmt::format(",{}",
                                                                 tmplAv->extras->tbody.substr(start, end - start));
                        }
                        FPS_PRINT_INFO("pi's av tbody now: {}", pi->av->extras->tbody);
                    }
                }

                // Finally, add the starting { and ending } so that the parse item's
                // assetVar's tbody is a valid JSON object
            ...
            else if (cjtmpl)
            {
                char* xtmp = cJSON_Print(cji);
                if (0)
                    FPS_PRINT_INFO(" adding parseItem [{}] for tmpl", xtmp);
                free(xtmp);
            ...
            if (cjsite)
            {
                ...
                if (cja && cja->valuestring)
                {
                    ...
                    if (aV)
                        loadSiteMapAv(vmap, vm, aV);
                }
                // int loadSiteMapAv(varsmap &vmap, varmap &amap, const char* aname,
                // fims* p_fims, assetVar* aV)
            }
        ...
        int count = 0;
        if (0)
            FPS_PRINT_INFO(" Note start initpiVec piVec.size [{}]", piVec.size());
        if (piVec.size() > 0)
        {
            initpiVec(piVec);
            if (0)
                FPS_PRINT_INFO(" Note start runpiVec", NULL);
            bool ok = true;
            // TODO run unlimited perhaps
            while (ok && count < MAX_PIVEC)
            {
                if (0)
                    FPS_PRINT_INFO(" runpiVec at lvl [{}]", lvl);
                count++;
                ok = runpiVec(piVec, lvl, idx);
            }
            if (0)
                FPS_PRINT_INFO(" Note end runpiVec ok [{}] count [{}]", ok, count);
            cleanpiVec(piVec);
        }
    }
    else
    {
        if (0)
            FPS_PRINT_INFO(" ERROR config load [{}] not found", xfile);
    }
    return 0;
}
// runs every second
...
int runConfig(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* av)
{
    ...
    if (0)
        FPS_PRINT_INFO(" running am [{}] vm [{}] val [{:2.3f}]", am->name.c_str(), fmt::ptr(vm), av->getdVal());
    bool nofiles = true;
    ...
    if (vmap.find("/config/cfile") != vmap.end())
    {
        ...
        for (auto xx : vmap["/config/cfile"])
        {
            ...
            if (0)
                FPS_PRINT_INFO(" found cfile [{}]  av [{}] value [{}]", xx.first, avx->getfName(), avx->getbVal());
            if (!avx->getbVal())
            {
                if (0)
                    FPS_PRINT_INFO(" found pending cfile [{}]  av [{}] value [{}]", xx.first, avx->getfName(),
                                   avx->getbVal());
                ...
                if (tNow > avx->getdParam("reqTimeout"))
                {
                    ...
                    {
                        if (1)
                            FPS_PRINT_INFO(" requesting cfile [{}] ", xx.first);
                    ...
                    {
                        FPS_PRINT_ERROR("Failed to receive cfile [{}] after {} tries", xx.first, ccount);
                        getsFailed = true;
    ...
    if (0)
        FPS_PRINT_INFO("looking for tmpl files", NULL);
    ...
    if (vmap.find("/config/ctmpl") != vmap.end())
    {
        ...
        {
            if (0)
                FPS_PRINT_INFO(" found tmpl #0 [{}] ", xx->first);
            ...
            {
                if (0)
                    FPS_PRINT_INFO("No avx  [{}] moving on...", xx->first);
                continue;
            }
            if (0)
                FPS_PRINT_INFO(" found tmpl #1 [{}]  av [{}] value [{}]", xx->first, avx->getfName(), avx->getbVal());
            if (!avx->getbVal())
            {
                if (0)
                    FPS_PRINT_INFO(" found tmpl #2 [{}]  av [{}] value [{}]", xx->first, avx->getfName(),
                                   avx->getbVal());
                if (tNow > avx->getdParam("reqTimeout"))
                {
                    int ccount = avx->getiParam("reqCount");
                    ...
                    {
                        if (1)
                            FPS_PRINT_INFO(" requesting ctmpl [{}] try number [{}] ", xx->first, ccount + 1);
                        requestTmpl(vmap, vm, avx);
                        if (0)
                            FPS_PRINT_INFO(" requested  ctmpl [{}] ", xx->first);
                        ccount++;
                        avx->setParam("reqCount", ccount);
                        double dval = tNow + 5.0;
                        avx->setParam("reqTimeout", dval);
                    ...
                    {
                        FPS_PRINT_ERROR("Failed to receive tmpl file [{}] after {} tries", xx->first, ccount);
                        getsFailed = true;
                    }
                }
            }
            ...
            {
                FPS_PRINT_ERROR("Failed to receive 1 or more template files", NULL);
                // Stop running runConfig()
                am->amap["runConfig"]->setParam("endTime", 1);
            }
        }
    ...
    if (0)
        FPS_PRINT_INFO("looking for load files", NULL);
    ...
    // if a loader has templates , they two have to be loaded before starting the
    // loader.
    ...
    if (vmap.find("/config/load") != vmap.end())
    {
        ...
        for (auto xx : vmap["/config/load"])
        {
            ...
            if (0)
                FPS_PRINT_INFO(" request load #1  [{}]  av [{}] value [{}]", xx.first, avx->getfName(), avx->getbVal());
            ...
            {
                if (0)
                    FPS_PRINT_INFO(" request load #2 [{}]  av [{}] value [{}]", xx.first, avx->getfName(),
                                   avx->getbVal());
                ...
                if (!avx->gotParam("file"))
                {
                    if (0)
                        FPS_PRINT_INFO(
                            " load <<file>> param missing;  bypass loader "
                            "xx.first [{}]  av [{}] value [{}]",
                            xx.first, avx->getfName(), avx->getbVal());
                ...
                {
                    // currently every loader must have a file.. if not specified the file
                    // is the loader name may fix that later this week. 09/21/2021
                    xfile = avx->getcParam("file");
                    if (!xfile)
                    {
                        if (0)
                            FPS_PRINT_INFO(" load file param NULL  xx.first [{}]  av [{}] value [{}]", xx.first,
                                           avx->getfName(), avx->getbVal());
                        xfile = xx.first.c_str();
                        // skip this loader nothing mre we can do with it
                        // bool bval = true;
                        avx->setParam("file", xfile);
                    ...
                    // else
                    {
                        ...
                        if (!avf)
                        {
                            ...
                            avf = vm->setVal(vmap, fr.c_str(), nullptr, bval);
                            if (aname)
                                avf->setcParam("aname", aname);
                            if (pname)
                                avf->setcParam("pname", pname);
                            if (ainame)
                                avf->setcParam("ainame", ainame);
                        }
                ...
                if (0)
                    FPS_PRINT_INFO(" found load  [{}] xfile [{}]  av [{}] value [{}]", xx.first, xfile, avx->getfName(),
                                   avx->getbVal()
                ...
                if (!runLoad)
                {
                    ...
                    if (0)
                        FPS_PRINT_INFO(
                            " aborting load  FILE MISSING  [{}] xfile [{}]  av "
                            "[{}] value [{}]",
                            xx.first, xfile, avx->getfName(), avx->getbVal()
                ...
                if (!avx->extras || !avx->extras->optVec || !avx->extras->optVec->cjopts)
                {
                    if (0)
                        FPS_PRINT_INFO(
                            " load options missing;  bypass loader xx.first "
                            "[{}]  av [{}] value [{}]",
                            xx.first, avx->getfName(), avx->getbVal());
                    bool bval = true;
                    avx->setVal(bval);
                ...
                if (0)
                    FPS_PRINT_INFO(" xfile [{}] completed", xfile);
            }
            // avf is the file
            ...
            // if(avx->extras && avx->extras->optVec)
            {
                ...
                cJSON_ArrayForEach(cjsi, avx->extras->optVec->cjopts)
                {
                    ...
                    // If we have a tmpls field, then we'll request for all template files
                    // defined in this field Otherwise, request for template file defined
                    // in tmpl field, if it exists
                    ...
                    if (cjtmpls)
                    {
                        if (0)
                            FPS_PRINT_INFO(
                                "tmpls field is found in avx [{}]. Requests for "
                                "template files will be made for tmpls instead of "
                                "tmpl",
                                avx->getfName());
                        if (optionsComplete)
                        {
                    ...
                    {
                        FPS_PRINT_INFO("tmpl field is found in avx [{}], which is {}", avx->getfName(),
                                       cstr{ cjtmpl->valuestring });
                        if (optionsComplete)
                        {
            ...
            if (0)
                FPS_PRINT_INFO(" avx [{}] optionsComplete [{}] loadComplete [{}]", avx->getfName(), optionsComplete,
                               loadComplete);
            // free(stmp);
            ...
            {
                if (0)
                    FPS_PRINT_INFO(" LOAD >>> Still Waiting for files for  [{}] in /config/load", xx.first);
                continue;
            }
            ...
            if (!runitall)
            {
                ...
                {
                    if (1)
                        FPS_PRINT_INFO(" All files loaded for loader [{}] ready to run av [{}] load", xx.first,
                                       avx->getfName());
                    ...
                    // bval =  false;
                    // vm->setVal(vmap,"/config/cfile:bms_manager_test.json", nullptr,
                    // bval);
                    char* fname = avx->getfName();
                    if (0)
                        FPS_PRINT_INFO(" After load, set avx [{}] true", fname);
                    ...
                    if (avx->gotParam("final"))
                    {
                        ...
                        {
                            if (0)
                                FPS_PRINT_INFO(" After load, requesting final [{}]", final);
                            bval = false;
                            ...
                            if (!bval)
                            {
                                ...
                                if (1)
                                    FPS_PRINT_INFO(" After requestFfile, requesting final [{}]", final);
                                // Stop running runConfig()
                                am->amap["runConfig"]->setParam("endTime", 1);
                            }
                        }
                ...
                if (0)
                    FPS_PRINT_INFO(" After load, requested final", NULL);
            }
            // this is the end of this load xx.first
        }
    }
    if (0)
        FPS_PRINT_INFO("done looking for files", NULL);

    if (nofiles)
    {
        if (0)
            FPS_PRINT_INFO("no file requests found", NULL);
    }
    return 0;
}
...
assetVar* setupCfile(varsmap& vmap, VarMapUtils* vm)
{
    ...
    if (!am)
    {
        ...
        if (0)
            FPS_PRINT_INFO(" Setting base AM [{}]", aname);
    ...
    if (!av)
    {
        double dval = 0.0;
        if (0)
            FPS_PRINT_INFO(" Setting up config system  1 ", NULL);
        av = vm->setVal(vmap, cfgsp, nullptr, dval);
        av->am = am;

        am->amap["runConfig"] = av;

        vm->setAvFunc(vmap, am->amap, (const char*)aname, vm->p_fims, av, "runConfig", &runConfig);
        ...
        //         , double tNow, double every /*0.1*/, double offset);
        if (0)
            FPS_PRINT_INFO(" Setting up config system  2 ", NULL);
        RunAv(vmap, vm, am->amap, aname, am, vm->p_fims, "/schedule/ess:run_config", "runConfig", tNow, 0.1, 0.0);
        if (0)
            FPS_PRINT_INFO(" Setting up config system  3 ", NULL);
    }
...
void getManagement(varsmap& vmap, VarMapUtils* vm, assetVar* av, asset_manager** pamp, asset_manager** amp, asset** aip)
{
    ...
    // cfiles will have asset_managers {am} defined in aname  also pname and
    // asname for simple assets if this screws up for templates we may have to
    // preload them in the manager files. no biggie an asset will need a pname
    char* aname = av->getcParam("aname");
    char* pname = av->getcParam("pname");
    char* ainame = av->getcParam("ainame");
    FPS_PRINT_DEBUG("av: [{}] aname: [{}] pname: [{}] ainame: [{}]", av->getfName(), cstr{ aname }, cstr{ pname },
                    cstr{ ainame });
    if (pname)
    {
    ...
    if (aname)
    {
        ...
        if (0)
            FPS_PRINT_INFO(" >>>>>   seeking aname [{}]  found am [{}]", aname, fmt::ptr(am));
        if (!am)
        {
            am = new asset_manager(aname);
            vm->setaM(vmap, aname, am);
            ...
            if (0)
                FPS_PRINT_INFO(" >>>>>   created am  aname [{}]  found am [{}]", aname, fmt::ptr(am));
        }
    }
    if (ainame)
    {
 ...
 * @param filterItems the collection of fields to remove from the cJSON message
 * body
 */
...
int VarMapUtils::configure_vmapStr(varsmap& vmap, const char* body, asset_manager* am, asset* ai, bool delcj)
{
    ...
    if (cj)
    {
        ...
        const char* ainame = nullptr;
        const char* pname = nullptr;
        FPS_PRINT_INFO("process file seeking header stuff am [{}] ai [{}]", am ? am->name : "no am",
                       ai ? ai->name : "no ai");
        ...
        if (0)
            FPS_PRINT_INFO(
                "process file seeking header stuff am [{}] ai [{}] cjap "
                "[{}] cjam [{}] cjai [{}]",
                am ? am->name : "no am", ai ? ai->name : "no ai", cjap ? cjap->valuestring : "..",
                cjam ? cjam->valuestring : "..", cjai ? cjai->valuestring : "..");

        // Delete fields in the cJSON object that should not be included before
        // processing
        filtercJSONMsg(cj, { "_doc", "_id", "_version" });

        // Iterate through each object in the cJSON body and replace keys, which are
        // uris, containing _ with /
        ...
        {
            FPS_PRINT_ERROR(
                "Unable to process file , please add amname or ainame, "
                "and possible pname fields ",
                NULL);
            cJSON_Delete(cj);
            return -1;
        }
 ...
 * }
 *
 * This function iterates through the outer two objects to construct uris in the
 * form of /controls/pcs, and adds the value of the third level to the map as
 * /controls/pcs/active_power_gradient.
 */
void VarMapUtils::readMapfromDbi(varsmap& vmap, const char* fims_message_body)
{
                ...
                while (cj_level_2)
                {
                    if (cj_level_2->type == cJSON_Object)
                    {
                        ...
                        while (cj_assetVar)
                        {
                            ...
                            std::string asset_uri = "/" + std::string(cj_level_1->string) + "/" +
                                                    std::string(cj_level_2->string);
    ...
    // Don't forget to free the cJSON object after you're done with it to avoid
    // memory leaks
...
// vm->handleCfile(vmap, nullptr, "set", "/cfg/crun", single, nullptr, nullptr,
// nullptr, nullptr);
void VarMapUtils::handleCfile(varsmap& vmap, cJSON* cj, const char* method, const char* uri, int& single,
                              const char* body, cJSON** cjr, asset_manager* am, asset* ai)
{
    ...
    if (strncmp(uri, "/cfg/setup", strlen("/cfg/setup")) == 0)
    {
        ...
        if (!cjb)
        {
            FPS_PRINT_INFO("  no body I'm outta here", NULL);
            if (cj)
                cJSON_Delete(cj);
            return;
        }
        ...
        {
            FPS_PRINT_INFO("  no function in body I'm outta here", NULL);
            if (cj)
                cJSON_Delete(cj);
            if (cjb)
                cJSON_Delete(cjb);
        ...
        {
            FPS_PRINT_INFO("  no name in body I'm outta here", NULL);
            if (cj)
                cJSON_Delete(cj);
            if (cjb)
                cJSON_Delete(cjb);
        ...
        if (res1 == nullptr)
        {
            // FPS_PRINT_ERROR(" Running Setup  setting up function [{}] uri [{}]
            // aname [{}] body [{}]"
            //             fun, uri, aname?aname:"I aint got no name",body?body:"I
            //             aint got no body");
    ...
    {
        if (0)
            FPS_PRINT_INFO(" >>>>>> handling uri [{}] ", uri);
        ...
        spav = sp + strlen(sp) + 1;
        if (0)
            FPS_PRINT_INFO(" >>>>>> uri [{}] am [{}] spav [{}]", spuri, sp, spav);
        ...
        if (avt)
        {
            if (1)
                FPS_PRINT_INFO(
                    " >>>>>> before getting management #1 info from  [{}]  "
                    "am at start [{}]",
                    avt->getfName(), am ? am->name : "no Am");
            getManagement(vmap, this, avt, &pam, &am, &ai);
            if (0)
                FPS_PRINT_INFO(
                    " >>>>>> after getting management info from  [{}]  am "
                    "after getManagement [{}]",
                    avt->getfName(), am ? am->name : "no Am");
        }
        else
        {
            ...
            if (0)
                FPS_PRINT_INFO(" NOTE setting up  uri [{}]", cfile);
            bool bval = false;
        ...
        {
            if (1)
                FPS_PRINT_INFO(" ERROR >>>>>> am not found using data from file", NULL);
    ...
    //        fims_send -f bms_manager_tmpl.json -m set -r /$$ -u
    //        /flex/cfg/ctmpl/bms/bms_catl_new_tmpl.json
    ...
    if (strncmp(uri, "/cfg/ctmpl/", strlen("/cfg/ctmpl/")) == 0)
    {
        ...
        spav = sp + strlen(sp) + 1;
        if (0)
            FPS_PRINT_INFO(" >>>>>> #1 uri [{}]", spuri);
        if (0)
            FPS_PRINT_INFO(" >>>>>> #2 uri [{}] sp [{}] ", spuri, sp);
        if (0)
            FPS_PRINT_INFO(" >>>>>> #3 uri [{}] sp [{}] spav [{}]", spuri, sp, spav);
        ...
        if (!avt)
        {
            bool bval = false;
            if (1)
                FPS_PRINT_INFO(" Created Template file  [{}]", cfile);
            avt = vm->setVal(vmap, cfile.c_str(), nullptr, bval);
        }
        if (avt)
        {
            // if(1)FPS_PRINT_INFO(" >>>>>> before getting management #2 info from
            // [{}]  am at start [{}]"
            //         , avt->getfName()
            //         , am?am->name:"no Am"
            //         );
        ...
        else
        {
            ...
            if (1)
                FPS_PRINT_INFO(" NO /config/ctmpl def found ", NULL);
        }
        if (cj)
            cJSON_Delete(cj);
        if (spuri)
    ...
    if (strncmp(uri, "/cfg/cstop", strlen("/cfg/cstop")) == 0)
    {
        ...
        // void StopAv(varsmap& vmap, VarMapUtils*vm, varmap& amap, const char*
        // aname, asset_manager *am, fims * p_fims
        //             , const char* schedVar/*"/schedule/bms:bms_rack"*/
        //             , const char* mapVar/*"runRackCloseContactor"*/
        //             , double tNow, double in /*0.1*/)
        StopAv(vmap, vm, am->amap, aname, am, vm->p_fims, "/schedule/ess:run_config", "runConfig", tNow, 0.0);
        FPS_PRINT_INFO(" Stopped Loader ", NULL);
    }
    ...
    if (strncmp(uri, "/cfg/crun", strlen("/cfg/crun")) == 0)
    {
        ...
        RunAv(vmap, vm, am->amap, aname, am, vm->p_fims, "/schedule/ess:run_config", "runConfig", tNow, 0.1, 0.0);
        FPS_PRINT_INFO(" Running Loader ", NULL);
    }
...
int deepDive(cJSON* cjii, VarMapUtils* vm, asset_manager* am, std::string& item, int opts)
{
    ...
    {
        if (0)
            FPS_PRINT_INFO("am   [{}]  child am [{}]  ", am->name, xx.first);
    ...
    {
        if (0)
            FPS_PRINT_INFO("am   [{}]  child ai [{}]  ", am->name, xx.first);
        if (xx.second->amap.find(item) != xx.second->amap.end())
        {
            auto x = xx.second->amap[item];
            if (x)
                ...
                asset_manager* am, char* uri)
{
    ...
    if (1)
        FPS_PRINT_INFO("uri  [{}] nmaps [{}]  ", uri, nmaps);
    for (int i = 0; i < nmaps; i++)
    {
        if (1)
            FPS_PRINT_INFO("uri  [{}] idx [{}] item [{}] ", uri, i, amVec[i]);
    }
    if (nmaps == 0)
    {
        return nullptr;
    ...
    if (amVec[0] == "amap")  // strncmp(uri,"/amap/", strlen("/amap/"))==0)
    {
        ...
        asset_manager* am = nullptr;
        if (aname)
            am = vm->getaM(vmap, (const char*)aname);
        asset* ai = nullptr;
        if (aname)
            ai = vm->getaI(vmap, (const char*)aname);
        if (1)
            FPS_PRINT_INFO("looking for amap [{}] [{}] am {} ai {}", uri, aname ? aname : "no aname", fmt::ptr(am),
                           fmt::ptr(ai));
        if (am || ai)
        {
        ...
        if (nmaps > 2)
        {
            for (auto x : am->amap)
            {
                ...
                {
                    if (1)
                        FPS_PRINT_INFO("nmaps [{}] item [{}] filt [{}] match [{}]", nmaps, x.first,
                ...
                if (match)
                {
                    // if(nmaps <=3)
                    {
                        if (1)
                            FPS_PRINT_INFO(
                                "     nmaps [{}] item [{}] {} filt [{}] match "
                                "[{}] body [{}] [{}]",
                                nmaps, x.first, fmt::ptr(x.second), (nmaps > 2) ? amVec[2] : "none",
                                (bool)(x.first == amVec[2]), body, fmt::ptr(cjbody));
                    }

                    // if(x.second) x.second->showvarCJ(cjii, opts);
                    if (x.second && cjbody)
                        vm->setValfromCj(vmap, x.second->getfName(), nullptr, cjbody);
        ...
        if (am)
        {
                ...
                if (nmaps > 2)
                {
                    ...
                    if (match)
                    {
                        if (nmaps <= 3)
                        {
                            if (1)
                                FPS_PRINT_INFO("     nmaps [{}] item [{}] {} filt [{}] match [{}]", nmaps, x.first,
                                               fmt::ptr(x.second), (nmaps > 2) ? amVec[2] : "none",
                                               (bool)(x.first == amVec[2]));
                        }

                        if (x.second)
                            x.second->showvarCJ(cjii, opts);
                        deepDive(cjii, vm, am, amVec[2], opts);
                    }
                }
                else
            ...
            if (nmaps < 3)
            {
                ...
                if (vmap.find(tmp) != vmap.end())
                {
                    ...
                    {
                        if (0)
                            FPS_PRINT_INFO("link [{}] {}", x.first, fmt::ptr(x.second));
                        if (x.second)
                            x.second->showvarCJ(cjil, opts);
                    }
                ...
                if (vmap.find(tmp) != vmap.end())
                {
                    ...
                    {
                        if (0)
                            FPS_PRINT_INFO("vlink [{}] {}", x.first, fmt::ptr(x.second));
                        if (x.second)
                            x.second->showvarCJ(cjil, opts);
                    }
            ...
            for (auto x : ai->amap)
            {
                if (nmaps > 2)
                {
                    ...
                    if (match)
                    {
                        if (1)
                            FPS_PRINT_INFO("nmaps [{}] item [{}] {} filt [{}] match [{}]", nmaps, x.first,
                                           fmt::ptr(x.second), (nmaps > 2) ? amVec[2] : "none", match);
...
// if(my.Var == nullptr) this is a reference to a full table (e.g. my.Uri is a
// full table)
int runLocks(varsmap& vmap, assetUri& my, const char* pname, bool lockState,
             assetVar* aV)  // maybe void or bool? - return true if locks
                            // changed, false if locks not changed?
{
    bool debug = 0;
    ...
    if (debug)
        FPS_ERROR_PRINT("[%s] function called\n", __func__);
    if (debug)
        FPS_ERROR_PRINT("[%s] Changing lock status of [%s:%s@%s] to [%s]\n", __func__, my.Uri ? my.Uri : "no Uri",
                        my.Var ? my.Var : "no Var", pname ? pname : "no parameter", lockState ? "true" : "false");
    if (aV->am)
    {
    ...
    {
        if (debug)
            FPS_ERROR_PRINT("[%s] aV->am not found\n", __func__);
    }
    ...
    else  // object to be locked is a single assetVar or parameter
    {
        if (!aV->am)
        {
            if (debug)
                FPS_ERROR_PRINT(
                    "[%s] NOTE assetManager cannot be found for while "
                    "locking elements in [%s:%s]\n",
        ...
        else if (!aV->am->vm)
        {
            if (debug)
                FPS_ERROR_PRINT(
                    "[%s] NOTE varMapUtils cannot be found for while "
                    "locking elements in [%s:%s]\n",
                    __func__, my.Uri, my.Var);
        }
        else
            ...
            if (pname != nullptr)  // Object to be locked is a parameter
            {
                if (localav->extras)
                {
                    ...
                    if (fmap.find(pname) != fmap.end())
                    {
                        ...
                        if (debug)
                            FPS_PRINT_INFO(
                                "[%s] updated locking for parameter [%s:%s@%s] to "
                                "lock state [%s]\n",
                                __func__, my.Uri, my.Var, pname, lockState ? "true" : "false");
                        return 0;
                    }
                    else
                    {
                        if (debug)
                            FPS_ERROR_PRINT("[%s] parameter [%s] not found in assetVar [%s:%s]\n", __func__, pname,
                                            my.Uri, my.Var);
                    }
                }
                else
                {
                    if (debug)
                        FPS_ERROR_PRINT("[%s] extras not found in assetVar [%s:%s]\n", __func__, my.Uri, my.Var);
                }
            }
...
int runAllLocks(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* av)
{
    ...
    if (1)
        FPS_PRINT_INFO("function called for {}", cstr{ av->getfName() });

    if (av->gotParam("uri"))
    {
        tblptr = av->getcParam("uri");
        if (debug)
            FPS_PRINT_INFO("URI?? {}", tblptr);
    }
    else
    {
        FPS_PRINT_ERROR("No Lock table found", NULL);
        return 0;
    }
    auto x = vmap.find(tblptr);
    {
        if (x == vmap.end())
        {
            if (debug)
                FPS_PRINT_ERROR(
                    "NOTE configuration locking table not found. No locks "
        ...
        {
            if (debug)
                FPS_PRINT_INFO("[{}] Configuration locking table found: [{}]\n", __func__, tblptr);
            for (auto& xx : vmap[tblptr])  // xx.first is object to be locked.
                                           // xx.second is lockState.
            {
                assetUri my(xx.first.c_str());  // my contains assetVar that is to be
                                                // locked or contains a parameter to be
                                                // locked.
                if (debug)
                    FPS_PRINT_INFO("[line 2747] assetUri my.Uri: [{}]. my.Var: [{}].\n", my.Uri ? my.Uri : "no URI",
                                   my.Var ? my.Var : "no Var");
                if (xx.second->extras)
                {
                    assetFeatDict* lockDict = xx.second->extras->baseDict;  // TODO gate this with
                                                                            // xx.second->extras check. This does
                                                                            // not get actions (vector in extras?)
                                                                            // or options (optDict?).
                    auto& lockParam = lockDict->featMap;
                    if (lockParam.empty())
                    {
                        if (debug)
                            FPS_PRINT_INFO("NOTE no parameters found in {}. Locking assetVar or table\n", xx.second);
                    }
                    else
                    {
                        for (auto& yy : lockParam)
                        {
                            ...
                            if (xx.second->gotParam(pname))
                            {
                                ...
                                lockState = false;  // default behavior should be to unlock, so
                                                    // re-set lockState here.
                            }
                            // lockParam(vmap, my, yy.first.c_str(), /*yy.second*/ true, *av);
                            // // yy.first is param name, yy.second is lock state
                        }
                    }
                }
                lockState = xx.second->getbVal();
                if (debug)
                    FPS_PRINT_INFO("NOTE configuration locking item, {}", xx.first.c_str());
                if (debug)
                    FPS_PRINT_INFO("NOTE configuration locking uri, {}", my.Uri ? my.Uri : "no URI");
                if (debug)
                    FPS_PRINT_INFO("NOTE configuration locking var, {}", my.Var ? my.Var : "no Var");
                if (debug)
                    FPS_PRINT_INFO("NOTE configuration locking state, {}", lockState ? "true" : "false");
                runLocks(vmap, my, nullptr, lockState, av);  // passing nullptr to pname
                                                             // indicates to runLocks()
                                                             // that this is an assetVar
                                                             // not a param
                lockState = false;
            }
        }
