...
def getbat_volts( constc, constv, time, cur_charge, cap):
    cc = cur_charge
    v = 0
    #use cc
...
max_battery_voltage = 4.1 # volts
min_battery_voltage = 2.5 # volts

const_current_charge = 10  # amps

battery_curr_charge = 0   # %
c = 0
vhist = []
cchist = []
thist = []
for t in range (0,100):
    ...
    v,c = getbat_volts(10, 4.5, 10, c, 100 )
    vhist.append(v)
    cchist.append(c)
    thist.append(t)

    print (" Volts %f cap %f"%(v,c))

plt.figure(1)
plt.figure(1)
plt.subplot(311)
#plt.title(('Simulation while sensing every {0} sec and PIR {1} with {2} events').format(sens_time_orig, PIR, PIR_events))
plt.plot(thist, vhist, 'b-', label = 'Voltage', markersize = 10)
plt.ylabel('volts', fontsize=15)
plt.legend(loc=9, prop={'size': 10})
plt.grid(True)
plt.subplot(312)
plt.plot(thist, cchist, 'r-', label = 'Charge', markersize = 10)
plt.grid(True)
plt.show()
...
plt.grid(True)
plt.subplot(312)
plt.plot(time_hist, SC_volt_hist, 'r.', label = 'SC Voltage', markersize = 10)
plt.ylabel('Super Capacitor\nVoltage [V]', fontsize=15)
plt.legend(loc=9, prop={'size': 10})
plt.ylim(2.2,5.6)
plt.grid(True)
plt.subplot(313)
plt.plot(time_hist, PIR_hist, 'k.', label = 'PIR detection', markersize = 10)
