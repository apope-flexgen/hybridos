...
std::unordered_map<std::string, std::unordered_map<int, std::unique_ptr<HighLevelController>>>
    HighLevelControllerObjects;
...
void HighLevelControllerRun(std::string uri, int instance)
{
    ...
    FPS_PRINT_INFO("aV [{}]  HLC[{}] In: [{}]    HLC Out: [{}]", uri, instance, dmHLCObject->HighLevelController_U.In1,
                   dmHLCObject->HighLevelController_Y.Out1);
}
...
void setupHighLevelControllerAmap(VarMapUtils* vm, varsmap& vmap, asset_manager* am, int instance, std::string uri)
{
    ...
    if (debug)
        FPS_PRINT_INFO("Setting up datamap to amap interface for {} using amap of asset manager: [{}]", ctrlHLC,
                       am->name);
...
void setupHighLevelController(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* aV)
{
    ...
    tmp.erase(tmp.begin());
    std::string instanceAMname = tmp + "_HighLevelController_" + std::to_string(instance) + "_asset_manager";
