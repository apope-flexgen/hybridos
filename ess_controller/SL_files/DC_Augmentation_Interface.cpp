...
{
    // create a new instance of the DC_Augmentation inputs struct, output struct,
    // and object
...
void setupDC_Augmentation(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* aV)
{
    ...
    {
        FPS_PRINT_ERROR(
            "Could not find parent AV of [{}] using comp [{}]. Cannot "
    ...
    while (++num)
    {
        ...
        if (parent_AV->gotParam((char*)funcNum.c_str()))
        {
            ...
            {
                // we have found our function, set its "setupX" flag high to signify
                // that we are done
