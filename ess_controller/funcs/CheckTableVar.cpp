...
std::shared_ptr<two_way_tbl> parseCSVFile(const char* fileName)
{
    ...
    {
        FPS_PRINT_DEBUG("File {} is opened", fileName);
    }
...
/**
 * @brief Retrieves the value from the two-way-table, given two values that
 * serve as key-pair values
 *
...
double getTableValue(double var1, double var2, std::shared_ptr<two_way_tbl> tbl)
{
    ...
    // If var1 or var2 is outside the range in the table, set value to 0 by
    // default
    if (var1 > varOneUpperBound || var1 < varOneLowerBound || var2 > varTwoUpperBound || var2 < varTwoLowerBound)
    {
        FPS_PRINT_DEBUG("Value not found (due to key value). Value is now set to 0 by default", NULL);
    ...
    else
    {
        ...
        FPS_PRINT_DEBUG("varOneKey found [{}] varTwoKey found [{}] var value found [{}]", varOneKey, varTwoKey,
                        tbl->find(keyvar)->second);
        return tbl->find(keyvar)->second;
    }
}

/**
 * @brief Checks the two-way-table for a value based on key-pair values and
 * updates the table value to the assetVar
 *
...
int CheckTableVar(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* av)
{
    ...
    if (reload < 2)
    {
        FPS_PRINT_DEBUG("Reload val for asset manager {} reload name {} is %d. Reloading...", aname, reloadStr.c_str(),
                        reload);
        if (reload < 1)
        {
            ...
            // // Check if the table exists. If the table exists, delete the table
            // since SetupTableVar dynamically allocates the table void* tblPtr =
            // av->getvParam("tbl"); if (tblPtr)
            // {
            //     FPS_PRINT_DEBUG("We're setting up a new table, but table %p in {}
            //     already exists. Deleting current table now...", av->name.c_str(),
            //     tblPtr); two_way_tbl* tbl = (two_way_tbl*)tblPtr; tbl->clear();
            //     delete tbl;
            // }
            ...
            if (tblName)
            {
                // Check if table already exists in the table map. If so, replace the
                // existing table with a new one
                if (vm->tblMap.find(tblName) != vm->tblMap.end())
                {
                    FPS_PRINT_DEBUG(
                        "We're setting up a new table for {} to use, but "
                        "table {} already exists in the table map. Deleting "
                        "current table now...",
                        av->name.c_str(), tblName);
                ...
                if (tbl)
                    vm->tblMap.insert(std::pair<std::string, std::shared_ptr<two_way_tbl>>(tblName, tbl));
            }
        }
    ...
    // Make sure the table var params are defined in the config file or database
    ...
    if (tblIter != vm->tblMap.end())
    {
        ...
        {
            FPS_PRINT_DEBUG(
                "tableVar param(s) are null. tableVar1: {}  tableVar2: "
                "{}. Skipping table check.",
                tblVar1Name ? tblVar1Name : "null", tblVar2Name ? tblVar2Name : "null");
            return 0;
        }
        ...
        {
            FPS_PRINT_DEBUG(
                "tableVar assetVar(s) are null. tableVar1: {}  "
                "tableVar2: {}. Skipping table check.",
                (void*)tblVar1, (void*)tblVar2);
            return 0;
        }
        ...
        FPS_PRINT_DEBUG(
            "Table [{}] --> tblVar1 [{}] val1 [{}]   tblVar2 [{}] val2 [{}] tblVal "
            "[{}]",
            av->getcParam("tableName") ? av->getcParam("tableName") : "null", tblVar1->name.c_str(),
            tblVar2->name.c_str(), val1, val2, tblVal);
