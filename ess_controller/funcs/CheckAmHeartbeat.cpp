 ...
 * toReset time after changes start being seen again before resetting faults and
 Alarms
 *
 */
int CheckAmHeartbeat(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* av)
{
    ...
    if (reload < 2)
    {
        ...
        if (1)
            FPS_ERROR_PRINT("\n\n%s >>  aname [%s] Heartbeat %p comp [%s] name [%s] \n", __func__, aname,
                            (void*)amap["HeartbeatRead"], amap["HeartbeatRead"]->comp.c_str(),
        ...
        amap["essHeartbeatFaultTimeout"] = vm->setLinkVal(vmap, essName, "/config", "essHeartbeatFaultTimeout",
                                                          toFault);
        amap["essHeartbeatAlarmTimeout"] = vm->setLinkVal(vmap, essName, "/config", "essHeartbeatAlarmTimeout",
                                                          toAlarm);
        amap["essHeartbeatResetTimeout"] = vm->setLinkVal(vmap, essName, "/config", "essHeartbeatResetTimeout",
                                                          toReset);
        ...
        if (reload == 0)  // complete restart
        {
            ...
            amap["HeartbeatRead"]->setParam("rdFault",
                                            toFault);  // time remaining before fault
            amap["HeartbeatRead"]->setParam("rdAlarm",
                                            toAlarm);  // time reamining before alarm
            amap["HeartbeatRead"]->setParam("rdReset",
                                            toReset);  // time remaining before reset
            // amap["HeartbeatRead"]     ->setParam("rdHold", toHold);
            // // time to wait before no change test
            amap["HeartbeatRead"]->setParam("tLast",
    ...
    if (!am->am)
    {
        // bool initSeen =             amap["HeartbeatRead"]
        // ->getbParam("initSeen");
          ...
          // then fault
    {
        ...
        else  // not changed not onHold look out for errors
        {
            ...
            if (rdFault > 0.0)
            {
                if (1)
                    FPS_ERROR_PRINT(
                        "%s >>  Heartbeat stall for  %s at %2.3f rdFault "
                        "%2.3f rdAlarm %2.3f HBOk [%s] seenHB [%s] tDiff "
                        "%2.3f \n",
            ...
            if (rdFault <= 0.0 && !seenFault)
            {
                ...
                {
                    bool bval = true;
                    // May need to change essName to "pcs" or "bms" if other asset
                    // managers can also initiate shutdown process Right now,
                    // ShutdownSystem function is called from ess wake up
                    amap["FaultShutdown"] = vm->setLinkVal(vmap, essName, "/status", "FaultShutdown", bval);
                }
            ...
            else
            {
                if (0)
                    FPS_ERROR_PRINT(
                        ...
                        "%2.3f Alarm %2.3f \n",
                        __func__, aname, amap["HeartbeatRead"]->getcVal(), tNow, rdFault, rdAlarm);
            }
        }
        if (seenFault)
        {
