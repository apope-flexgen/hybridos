...
int StartupPCS(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* aV)
{
    ...
    // if (am == nullptr)
    if (0)
        FPS_PRINT_INFO("Running for aV [{}:{}] am {}, amap {} aname [{}]", aV->comp, aV->name, fmt::ptr(aV->am),
                       fmt::ptr(&amap), aname);
    if (0)
        FPS_PRINT_INFO(
            ">>>>>>>>>>>PCS Startup check amap {} system state [{}] {} "
            ">> pcssystem state [{}] {}",
            fmt::ptr(&amap), amap["SystemState"] ? cstr{ amap["SystemState"]->getcVal() } : cstr{ "No System State" },
    ...
    {
        if (0)
            FPS_PRINT_INFO("Running reload {}", reload);
    ...
    if (reload == 0)
    {
        ...
        ival = 1;
        linkVals(*vm, vmap, amap, aname, "/config", ival, "PCSon");
        ival = 0;
        linkVals(*vm, vmap, amap, aname, "/config", ival, "PCSoff");
        ival = 1;
        linkVals(*vm, vmap, amap, aname, "/config", ival, "PCSstart");
        // ival= 0; linkVals(*vm, vmap, amap, aname, "/config", ival, "PCSstop");
        ival = 0;
        linkVals(*vm, vmap, amap, aname, "/config", ival, "PCSstay");

        linkVals(*vm, vmap, amap, aname, "/controls", ival,
                 "PCSRun");  // pcs selector status variable
        linkVals(*vm, vmap, amap, "bms", "/status", ival, "DCClosed");

        linkVals(*vm, vmap, amap, aname, "/config", dval, "maxStartupTime", "maxFullStartupTime", "maxKeyCmdOnTime",
                 "maxKeyCmdTime");
        ...
        if (1)
            FPS_PRINT_INFO(">>>>>>>>>>>PCS Startup reload 0 ; PCSon {} PCSoff {}", amap["PCSon"]->getiVal(),
                           amap["PCSoff"]->getiVal());
    ...
    if (reload < 2)
    {
        ...
        if (1)
            FPS_PRINT_INFO(
                ">>>>>>>>>>> PCS Startup reload 1 amap {} system state "
                "[{}] {} >> pcssystem state [{}] {}",
                fmt::ptr(&amap), cstr{ amap["SystemState"]->getcVal() }, fmt::ptr(amap["SystemState"]),
                cstr{ amap["PCSSystemState"]->getcVal() }, fmt::ptr(amap["PCSSystemState"]));

        pcsAv->setVal(2);
    }
    if (0)
        FPS_PRINT_INFO(">>>>>>>>>>>>>>>>PCS Startup system state [{}]", cstr{ amap["SystemState"]->getcVal() });

    char* SystemState = amap["SystemState"]->getcVal();
    char* PCSSystemStateStep = amap["PCSSystemStateStep"]->getcVal();
    if (0)
        FPS_PRINT_INFO("Running reload {} state [{}] step [{}]", reload, cstr{ SystemState },
                       cstr{ PCSSystemStateStep });

    char* PCSStatusResp = amap["PCSStatusResp"]->getcVal();
    if (0)
        FPS_PRINT_INFO("Running reload {} StatusResp [{}]", reload, cstr{ PCSStatusResp });
    if (0)
        FPS_PRINT_INFO("aname {} comp {} name {}", aname, amap["PCSStatusResp"]->comp, amap["PCSStatusResp"]->name);

    // Command values for the PCS control registers (values can be defined in
    // config)
    ...
    if (!PCSStatusResp)
    {
        ...
        if (1)
            FPS_PRINT_WARN("No communication with PCS", NULL);
        // Send Event - fault
        ...
        if (temp_av)
            temp_av->sendEvent("PCS", vm->p_fims, Severity::Fault, "PCS startup comms failed at time %2.3f",
                               now.count());
    ...
    if (amap["SystemState"]->valueChangedReset() || strcmp(PCSSystemStateStep, "Init") == 0)
    {
        ...
        if (0)
            FPS_PRINT_INFO("State step changed from {} to {}", cstr{ PCSSystemStateStep }, cstr{ SystemState });
    }
    ...
    if (strcmp(SystemState, "Off") == 0)
    {
        if (!amap["DCClosed"]->getiVal())
        {
            ...
            amap["PCSSystemStateStep"]->setVal(PCSSystemStateStep);
            if (1)
                FPS_PRINT_WARN("PCS initial DC voltage condition not met", NULL);
            const auto now = flex::get_time_dbl();
            ESSLogger::get().info("[PCS initial DC voltage condition not met]");
            assetVar* temp_av = amap["PCSStartup"];
            if (temp_av)
                temp_av->sendEvent("PCS", vm->p_fims, Severity::Info,
                                   "PCS initial DC voltage condition not met at time %2.3f", now.count());
        ...
        else if (!amap["PCSStartup"]->getbParam("FullStartup"))
        {
            ...
            amap["PCSSystemStateStep"]->setVal(PCSSystemStateStep);
            if (1)
                FPS_PRINT_INFO("Sending Start to PCS", NULL);
            const auto now = flex::get_time_dbl();
            ESSLogger::get().info("[Sending Start to PCS]");
            assetVar* temp_av = amap["PCSStartup"];
            if (temp_av)
                temp_av->sendEvent("PCS", vm->p_fims, Severity::Info, "Sending Start to PCS at time %2.3f",
                                   now.count());
        ...
        else if (amap["PCSRun"]->getdVal() != PCSon)
        {
            if ((tNow - amap["PCSStartupTime"]->getdVal()) > amap["maxStartupTime"]->getdVal())
            {
                ...
                amap["PCSSystemStateStep"]->setVal(PCSSystemStateStep);
                if (1)
                    FPS_PRINT_WARN("PCS Start command not setting in state {}", cstr{ PCSStatusResp });
                // Send Event - fault
                const auto now = flex::get_time_dbl();
                ...
                assetVar* temp_av = amap["PCSStartup"];
                if (temp_av)
                    temp_av->sendEvent("PCS", vm->p_fims, Severity::Fault,
                                       "PCS Start command not setting at time %2.3f", now.count());
    ...
    else if (amap["SoftStartup"]->getbVal() && strcmp(SystemState, "Ready") == 0)
    {
        ...
        amap["PCSSystemStateStep"]->setVal(PCSSystemStateStep);
        if (1)
            FPS_PRINT_INFO("PCS succesfully precharged, in Standby", NULL);
        // Send Event - info
        const auto now = flex::get_time_dbl();
        ESSLogger::get().info("[PCS DC Contactors closed, in Standby]");
        assetVar* temp_av = amap["PCSStartup"];
        if (temp_av)
            temp_av->sendEvent("PCS", vm->p_fims, Severity::Info, "PCS DC Contactors closed, in Standby at time %2.3f",
                               now.count());
    ...
    else if (amap["PCSStartup"]->getbParam("FullStartup") && strcmp(SystemState, "Ready") == 0)
    {
        ...
        amap["PCSSystemStateStep"]->setVal(PCSSystemStateStep);
        if (1)
            FPS_PRINT_INFO("PCS succesfully precharged, in Standby going to start", NULL);
        const auto now = flex::get_time_dbl();
        ESSLogger::get().info("[PCS DC Contactors closed, in Standby going to start ]");
        assetVar* temp_av = amap["PCSStartup"];
        if (temp_av)
            temp_av->sendEvent("PCS", vm->p_fims, Severity::Info,
                               "PCS DC Contactors closed, in Standby going to start  at time %2.3f", now.count());
             ...
             ((tNow - amap["PCSStartupTime"]->getdVal()) > amap["maxFullStartupTime"]->getdVal()))
    {
        ...
        const std::string msg = fmt::format("PCS stuck in hold state {} for {} seconds at time {:2.3f}", PCSStatusResp,
                                            amap["maxFullStartupTime"]->getdVal(), now.count());
        ...
        assetVar* temp_av = amap["PCSStartup"];
        if (temp_av)
            temp_av->sendEvent("PCS", vm->p_fims, Severity::Fault, msg.c_str());
             ...
             ((tNow - amap["PCSStartupTime"]->getdVal()) > amap["maxStartupTime"]->getdVal()))
    {
        ...
        amap["PCSSystemStateStep"]->setVal(PCSSystemStateStep);
        if (1)
            FPS_PRINT_INFO("PCS stuck in hold state {}", cstr{ PCSStatusResp });
        // Send Event - fault
        const auto now = flex::get_time_dbl();
        ...
        assetVar* temp_av = amap["PCSStartup"];
        if (temp_av)
            temp_av->sendEvent("PCS", vm->p_fims, Severity::Fault, "PCS stuck in hold state [%s] at time %2.3f",
                               PCSStatusResp, now.count());

        vm->schedStop(vmap, amap, "schedStartupPCS", 1);
        pcsAv->setVal(1);  // force reload init of this function
    }
    // not in full startup (cold start needs  a long time, warm start needs a
    // shorter timer)
    else if (strcmp(SystemState, "Starting") == 0 && !amap["PCSStartup"]->getbParam("FullStartup") &&
             ((tNow - amap["PCSStartupTime"]->getdVal()) > amap["maxStartupTime"]->getdVal()))
    {
    ...
    else if (strcmp(SystemState, "Running") == 0)
    {
        ...
        amap["PCSSystemStateStep"]->setVal(PCSSystemStateStep);
        if (1)
            FPS_PRINT_INFO("All PCS's succesfully Startup", NULL);
        // Send Event - info
        const auto now = flex::get_time_dbl();
        ESSLogger::get().info("[PCS startup completed]");
        assetVar* temp_av = amap["PCSStartup"];
        if (temp_av)
            temp_av->sendEvent("PCS", vm->p_fims, Severity::Info, "PCS startup completed at time %2.3f", now.count());
    ...
    else if (strcmp(SystemState, "Shutdown") == 0 || strcmp(SystemState, "Fault") == 0)
    {
        ...
        amap["PCSSystemStateStep"]->setVal(PCSSystemStateStep);
        if (1)
            FPS_PRINT_ERROR("PCS failed in state {}", cstr{ PCSStatusResp });
        // Send Event - fault
        const auto now = flex::get_time_dbl();
        ...
        assetVar* temp_av = amap["PCSStartup"];
        if (temp_av)
            temp_av->sendEvent("PCS", vm->p_fims, Severity::Fault, "PCS startup failed at time %2.3f", now.count());
    ...
    {
        if (0)
            FPS_PRINT_INFO("Waiting for Full Startup", NULL);
    }
    ...
    else if (amap["KeyCmd"]->getbParam("KeyCmdDone"))
    {
        ...
        amap["PCSSystemStateStep"]->setVal(PCSSystemStateStep);
        if (1)
            FPS_PRINT_WARN("No response from PCS", NULL);
        // Send Event - fault
        const auto now = flex::get_time_dbl();
        ...
        assetVar* temp_av = amap["PCSStartup"];
        if (temp_av)
            temp_av->sendEvent("PCS", vm->p_fims, Severity::Fault, "PCS response failed at time %2.3f", now.count());
...
int StartupBMS(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* aV)
{
    ...
    if (reload == 0)
    {
        ...
        ival = 3;
        linkVals(*vm, vmap, amap, aname, "/config", ival, "BMSstop");
        ival = 2;
        linkVals(*vm, vmap, amap, aname, "/config", ival, "BMSstart");
        ival = 1;
        linkVals(*vm, vmap, amap, aname, "/config", ival, "BMSstay");
        ival = 0;
        linkVals(*vm, vmap, amap, aname, "/config", ival, "BMSreset");
    ...
    if (reload < 2)  // complete reset
    {
        ...
        if (1)
            FPS_PRINT_INFO(">>>>>>>>>>>>>>>BMS Startup system state [{}]", cstr{ amap["BMSSystemState"]->getcVal() });
        bmsAv->setVal(2);
    }

    // Command values for the BMS control registers (values can be defined in
    // config)
    ...
    if (0)
        FPS_PRINT_INFO("Running reload {} state [{}] step [{}]", reload, cstr{ BMSSystemState },
    ...
    {
        if (0)
            FPS_PRINT_INFO("BMS State [{}] Step [{}]", cstr{ BMSSystemState }, cstr{ BMSSystemStateStep });
    }
    ...
    if (!BMSStatus || !BMSPowerOn)
    {
        ...
        if (1)
            FPS_PRINT_INFO("No communication with BMS", NULL);
        // TODO Preston to check
        return 0;
    }
    if (0)
        FPS_PRINT_INFO("status [{}] PowerOn [{}]", cstr{ BMSStatus }, cstr{ BMSPowerOn });
    ...
    if (0)
        FPS_PRINT_INFO("BMSPowerOn [{}] changed [{}] BMSSystemStateStep {}", cstr{ amap["BMSPowerOn"]->getcVal() },
                       amap["BMSPowerOn"]->valueChanged(), cstr{ BMSSystemStateStep });
        ...
        strcmp(BMSSystemStateStep, "Startup Failed"))
    {
        ...
        {
            FPS_PRINT_INFO("State step changed from [{}] to [{}]", amap["BMSSystemStateStep"]->getcVal(),
                           cstr{ BMSSystemStateStep });
            amap["BMSSystemStateStep"]->setVal(BMSSystemStateStep);
        }
    }
    ...
    if (strcmp(BMSSystemStateStep, "Running") == 0)
    {
        ...
        amap["BMSSystemStateStep"]->setVal(BMSSystemStateStep);
        if (1)
            FPS_PRINT_INFO("All BMS's succesfully Startup", NULL);
        // Send Event - info
        const auto now = flex::get_time_dbl();
        ESSLogger::get().info("[All BMS successfully started] up");
        auto temp_av = amap["BMSStartup"];
        if (temp_av)
            temp_av->sendEvent("BMS", vm->p_fims, Severity::Info, "All BMS succesfully started up at time %2.3f",
                               now.count());
    ...
    else if ((tNow - amap["BMSStartupTime"]->getdVal()) > amap["maxStartupTime"]->getdVal())
    {
        ...
        amap["BMSSystemStateStep"]->setVal(BMSSystemStateStep);
        if (1)
            FPS_PRINT_INFO("BMS Startup timed out in hold state {}", cstr{ BMSPowerOn });
        if (1)
            FPS_PRINT_INFO("maxStartupTime {} BMSStartupTime {}", amap["maxStartupTime"]->getdVal(),
                           amap["BMSStartupTime"]->getdVal());
        // Send Event - fault
        const auto now = flex::get_time_dbl();

        ESSLogger::get().critical("BMS startup timed out in state [{}]", BMSPowerOn);
        if (logging_enabled)
        {
        ...
        auto temp_av = amap["BMSStartup"];
        if (temp_av)
            temp_av->sendEvent("BMS", vm->p_fims, Severity::Fault, "BMS startup timed out in state [%s] at time %2.3f",
    ...
    else if (strcmp(BMSSystemStateStep, "Startup Failed") == 0)
    {
        ...
        amap["BMSSystemStateStep"]->setVal(BMSSystemStateStep);
        if (1)
            FPS_PRINT_ERROR("BMS in failed state", NULL);
        // Send Event - fault
        const auto now = flex::get_time_dbl();
        ...
        auto temp_av = amap["BMSStartup"];
        if (temp_av)
            temp_av->sendEvent("BMS", vm->p_fims, Severity::Fault, "BMS in failed state at time %2.3f", now.count());
    ...
    else if (amap["KeyCmd"]->getbParam("KeyCmdDone"))
    {
        ...
        amap["BMSSystemStateStep"]->setVal(BMSSystemStateStep);
        if (1)
            FPS_PRINT_ERROR("No response from BMS", NULL);
        // Send Event - fault
        const auto now = flex::get_time_dbl();
        ...
        auto temp_av = amap["BMSStartup"];
        if (temp_av)
            temp_av->sendEvent("BMS", vm->p_fims, Severity::Fault, "BMS not responding at time %2.3f", now.count());
