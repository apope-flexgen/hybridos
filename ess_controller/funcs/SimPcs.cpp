...
// for Pcs modules
int SimSetupPcsModules(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* aV, const char* modfmt)
{
...
int SimHandlePcsStatus(varsmap& vmap, varmap& amap, const char* xaname, fims* p_fims, assetVar* aV)
{
    ...
    if (current_status == 6)
    {
        ...
        if (current_cmd == 2)  // stop
        {
            lastTime = now.count();
            amap["lastTime"]->setVal(lastTime);
            current_status = 8;  // Discharge   // we are now ready  we can accept
                                 // charge / discharge
        }
    }
    if (current_status == 8)
    {
        ...
        if (now.count() - lastTime > discharge_time)
        {
            ...
            // current_status = 2;// Stop but we need to charge     // we are now
            // ready  we can accept charge / discharge
        }
    }
    amap["current_status"]->setVal(current_status);
    return 0;
...
int SimHandlePcs(varsmap& vmap, varmap& amap, const char* xaname, fims* p_fims, assetVar* aV)
{
    ...
    if (!am)
    {
        ...
        if (debug)
            FPS_ERROR_FMT(" {} >> adding pcs asset manager \n", __func__);
    ...
    if (1)  // (amap["Heartbeat"]->getLastSetDiff(dval) > 1.0)
    {
        ...
        for (auto& iy : am->assetMap)
        {
            if (idx < num_modules)
            {
                ...
                aV->am = am;
                if (1)
                    FPS_ERROR_FMT("{} >> Running asset  {} module_count {}..\n", __func__, ami->name, module_count);
        ...
        if (1)
            FPS_ERROR_FMT("{} >> After loop  [{}] as {} output power {}\n", __func__,
                          amap["num_running_modules"]->getfName(), ival, dval);
