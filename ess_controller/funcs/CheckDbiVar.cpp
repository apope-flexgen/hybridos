...
/**
 * @brief Check if the parameters in av are also in dbiAv and if the param values are different. If so, 
 *        update av parameters by copying the parameter values in dbiAv to av
 * 
...
void updateParamsFromDbi(assetVar* av, assetVar* dbiAv)
{
    ...
    // For error-checking, make sure both dbiAv and av have assetExtras, a baseDict, and values in the featMap (where parameters are stored)
    if (!dbiAv->extras || !av->extras)
    {
        FPS_PRINT_ERROR("assetExtras is null for either dbiAv or av %s. Skipping param set to dbi and assetVar\n",  av->name);
        return;
    }
    ...
    if (!dbiDict || !avDict)
    {
        FPS_PRINT_ERROR("baseDict is null for either dbiAv or av %s. Skipping param set to dbi and assetVar\n", av->name);
        return;
    }
    ...
    {
        FPS_PRINT_ERROR("%s >> featMap that contains the params is empty for dbiAv %s. Skipping param set to dbi and assetVar\n", av->name);
        return;
    }
    ...
    for (auto& param : dbiAvParams)
    {
        ...
        // In the future, we can possibly have a list of parameters found in the dbiAv that we do not want to include in av
        // For now, we don't want dbiResp to be included
        if (paramName == "dbiSet")
        {
            FPS_DEBUG_PRINT("%s >> param %s is found in dbiAv %s. This will not be included in av.\n", __func__, paramName.c_str(), dbiAv->name.c_str());
            return;
        }
        switch (dbiAvParams[paramName]->type)
        {
            case assFeat::AFTypes::AINT:
            {
                ...
                FPS_DEBUG_PRINT("%s >> av param %s is in dbiAv and both param values are different. Updating av param to int val %d\n", __func__, paramName.c_str(), ival);
            ...
            case assFeat::AFTypes::AFLOAT:
            {
                ...
                FPS_DEBUG_PRINT("%s >> av param %s is in dbiAv and both param values are different. Updating av param to double val %f\n", __func__, paramName.c_str(), dval);
            ...
            case assFeat::AFTypes::ASTRING:
            {
                ...
                FPS_DEBUG_PRINT("%s >> av param %s is in dbiAv and both param values are different. Updating av param to string val %s\n", __func__, paramName.c_str(), cval);
            ...
            case assFeat::AFTypes::ABOOL:
            {
                ...
                FPS_DEBUG_PRINT("%s >> av param %s is in dbiAv and both param values are different. Updating av param to bool val %s\n", __func__, paramName.c_str(), bval ? "true" : "false");
                avDict->setFeat(paramName.c_str(), bval);
                break;
            }
            default:
                FPS_ERROR_PRINT("%s >> assFeat type not supported for dbi variable. Supported types are: AINT, AFLOAT, ASTRING, and ABOOL. Skipping param set to dbi and assetVar %s\n", __func__, dbiAv->name.c_str());
        }
    }
}

/**
 * @brief Check if the actions in av are also in dbiAv and if the action values are different. If so, 
 *        update av actions by copying the action values in dbiAv to av
 * 
 * @param av the assetVar referencing the dbi
 * @param dbiAv the dbi assetVar
 */
void updateActionsFromDbi(assetVar* av, assetVar* dbiAv)
{
    ...
    if (!dbiAv->extras || !av->extras)
    {
        FPS_ERROR_PRINT("%s >> assetExtras is null for either dbiAv or av %s. Skipping action set to dbi and assetVar\n", __func__, av->name.c_str());
        return;
    }
...
/**
 * @brief Check if the options in av are also in dbiAv and if the option values are different. If so, 
 *        update av options by copying the option values in dbiAv to av
 *
 * @param av the assetVar referencing the dbi
 * @param dbiAv the dbi assetVar
 */
...
{
    // For error-checking, make sure both dbiAv and av have assetExtras, an optDict, and values in the featMap (where options are stored)
    if (!dbiAv->extras || !av->extras)
    {
        FPS_ERROR_PRINT("%s >> assetExtras is null for either dbiAv or av %s. Skipping option set to dbi and assetVar\n", __func__, av->name.c_str());
        return;
    }
    ...
    if (!dbiDict || !avDict)
    {
        FPS_ERROR_PRINT("%s >> optDict is null for either dbiAv or av %s. Skipping option set to dbi and assetVar\n", __func__, av->name.c_str());
        return;
    }
    ...
    {
        FPS_ERROR_PRINT("%s >> featMap that contains the options is empty for dbiAv %s. Skipping option set to dbi and assetVar\n", __func__, av->name.c_str());
        return;
    }
    ...
    for (auto& option : dbiAvOptions)
    {
        ...
        switch (dbiAvOptions[optionName]->type)
        {
            case assFeat::AFTypes::AINT:
            {
                ...
                FPS_DEBUG_PRINT("%s >> av option %s is in dbiAv and both option values are different. Updating av option to int val %d\n", __func__, optionName.c_str(), ival);
            ...
            case assFeat::AFTypes::AFLOAT:
            {
                ...
                FPS_DEBUG_PRINT("%s >> av option %s is in dbiAv and both option values are different. Updating av option to double val %f\n", __func__, optionName.c_str(), dval);
            ...
            case assFeat::AFTypes::ASTRING:
            {
                ...
                FPS_DEBUG_PRINT("%s >> av option %s is in dbiAv and both option values are different. Updating av option to string val %s\n", __func__, optionName.c_str(), cval);
            ...
            case assFeat::AFTypes::ABOOL:
            {
                ...
                FPS_DEBUG_PRINT("%s >> av option %s is in dbiAv and both option values are different. Updating av option to bool val %s\n", __func__, optionName.c_str(), bval ? "true" : "false");
                avDict->setFeat(optionName.c_str(), bval);
                break;
            }
            default:
                FPS_ERROR_PRINT("%s >> assFeat type not supported for dbi variable. Supported types are: AINT, AFLOAT, ASTRING, and ABOOL. Skipping option set to dbi and assetVar %s\n", __func__, dbiAv->name.c_str());
        }
    }
}
 ...
 *        An assetVar will not have a dbi value on start up, so this indicates that the dbi needs to be fetched.
 *        This should then trigger the CheckDbiResp function
 * 
...
ReplyTo: (null)
Body:    {"status_pcs":{"MaxDeratedChargeCmd":{"value":-3510,"EnableDbiUpdate":true,"UpdateTimeCfg":5,"UpdateTimeRemain":5,"dbiStatus":"OK","tLast":1542.074574999977},"MaxDeratedDischargeCmd":{"value":3510,"EnableDbiUpdate":true,"UpdateTimeCfg":5,"UpdateTimeRemain":0,"dbiStatus":"OK","tLast":1542.0746829999844}}}
Timestamp:   2021-07-16 12:32:44.492924
so anything coming in for dbi gets the first object child inside the body rather than the body 
*/
int GetDbiVar(varsmap &vmap, varmap& amap, fims* p_fims, const char* aname, assetVar* av)
{
    ...
    // /dbi/config/bms:DemoChargeCurrent will have an action to check and then update the value 
    ...
    if(0)FPS_ERROR_PRINT("%s >> Sending get request to uri %s with replyto %s from assetVar %s\n"
...
// This runs whenever /config/bms:DemoChargeCurrent changes state or every 5 seconds anyway 
/**
 * @brief Set the dbi assetVar to the dbi data storage system. This will update all dbi assetVar's parameters and actions
 * 
 * @param vmap the global data map shared by all assets/asset managers
 * @param amap the local data map used by an asset/asset manager
...
int SetDbiVar(varsmap &vmap, varmap& amap, fims* p_fims, const char* aname, assetVar* av)
{
    ...
    // In order to properly update a variable in the dbi, we'll need to compose a list of
    // the rest of the dbi variables that contains the same component uri in the vlist
    // before updating the dbi. This is to ensure all of the dbi variables are intact
    // after the update

    // we need to take the comp and replace /a/b/c_d with /a_b_c__d
    // used in clone
    //char* res = vm->run_replace(const char* inStr, const char* replace, const char* with)
...
int SetupAvParams(const char* aname, assetVar* av)
{
    ...
    if (!av->gotParam("UpdateTimeCfg")) av->setParam("UpdateTimeCfg", dval);
    av->setParam("UpdateTimeRemain", av->getdParam("UpdateTimeCfg"));
    if (!av->gotParam("EnableDbiUpdate")) av->setParam("EnableDbiUpdate", bval);  // Enable dbi update by default if param is not already provided
...
{
    FPS_DEBUG_PRINT("%s >>  setup for  %s  %s\n", __func__, aname, dbiAv->name.c_str());   
    bool bval = false;          
 ...
 *        If the assetVar value is different to the dbi value, then the assetVar needs to be sent to the dbi.
 *        The time of last update is also saved. An update will also be sent to the dbi after a certain amount of time.
 * 
 * @param vmap the global data map shared by all assets/asset managers
 * @param amap the local data map used by an asset/asset manager
 * @param aname the name of the asset/asset manager
...
{
    if(0)FPS_PRINT_INFO("checking dbi aname [{}] with assetVar [{}]"
        , aname, av->getfName());
    ...
    {
        if(0)FPS_PRINT_INFO("dbiStatus [fetch] for assetVar [{}] tDiff [{:2.3f}]"
                        ,  av->getfName(), tDiff);
    ...
    // Otherwise, check if the assetVar changes its value or 5 seconds (or configurable time) have passed and then set to dbi
    else
    {
        dbiCnt = av->getiParam("dbiCnt");
        dbiCnt++;
        ...
        if (!enableDbiUpdate)
        {
            FPS_PRINT_INFO("enableDbiUpdate for assetVar [{}] with uri {} is false. Skipping update to dbi", av->name, av->comp);
            return;
        }
        ...
        if (av->valueChangedReset() /*|| updateTimeRemain <= 0.0*/)
        {
            ...
            FPS_PRINT_INFO("calling SetDbiVar assetVar [{}] with uri [{}]."
                                , av->name, av->comp);
 ...
 *        If the dbi assetVar value has changed, or if the dbiSet param is true, then the assetVar referencing the dbi needs to be updated.
 * 
 * @param vmap the global data map shared by all assets/asset managers
 * @param amap the local data map used by an asset/asset manager
 * @param aname the name of the asset/asset manager
...
void UpdateAvFromDbi(varsmap &vmap, varmap &amap, const char* aname, fims* p_fims, assetVar* dbiAv)
{
    ...
    vm = am->vm;    
    // Since this function is expected to run periodically, we'll want to update av if the dbiAv value has
    // changed or if the param dbiSet is true

    // dbiSet may be set to true here. If so, set this back to false so we don't repeatedly run the code block below
    if (dbiAv->gotParam("dbiSet") && dbiAv->getbParam("dbiSet")) dbiAv->setParam("dbiSet", false);

    if(0)FPS_DEBUG_PRINT("%s >> dbiAv val has changed or dbiSet is true. Updating av and dbi %s\n", __func__, dbiAv->name.c_str());
    ...
    if (idx != std::string::npos && idx == 0) {
        ...
        if(!update)
        {
            if(0)FPS_PRINT_INFO("dbiAv val has not changed or dbiSet is false."
        ...
        if (av->type == dbiAv->type)
        {
            switch (av->type)
            {
                ...
                case assetVar::ATypes::ABOOL:
                    ...
                    break;
                default:
                    FPS_PRINT_ERROR("Type not supported for dbi variable."
                    " Supported types are: AINT, AFLOAT, ASTRING, and ABOOL. Skipping set to dbi and assetVar {}"
                    , dbiAv->getfName()
                    );
                    break;
            }

            // Check dbi parameters, actions, and options and set those values to av as appropriate
        ...
        {
            FPS_PRINT_ERROR("The data types for dbiAv and the assetVar referencing the dbiAv are not the same."
    ...
    {
        FPS_PRINT_ERROR("dbiAv uri {} does not contain /dbi. Skipping set to dbi and assetVar {}"
                       , dbiAv->comp, dbiAv->name);
    }
}

/**
 * @brief Periodically check the response received by the dbi assetVar before setting to the dbi and the assetVar the dbi is referencing.
 * 
 * @param vmap the global data map shared by all assets/asset managers
 * @param amap the local data map used by an asset/asset manager
 * @param aname the name of the asset/asset manager
...
int CheckDbiResp(varsmap &vmap, varmap &amap, const char* aname, fims* p_fims, assetVar* dbiAv)
{
    ...
    if(0)FPS_PRINT_INFO("Running for assetVar [{}] with aname [{}] value [{:2.3f}]"
            , dbiAv->getfName()
            , aname
            , dbiAv->getdVal()
            );
    ...
    //     if (!amap[logName]->getcVal()) amap[logName]->setVal(logFile);

    //     if(reload < 1)
    //     {
    //         FPS_DEBUG_PRINT("%s >> Not set up for [%s] reloadStr [%s]\n", __func__, dbiAv->name.c_str(), reloadStr.c_str());
    ...
    //         if (!logFile)
    //         {
    //             // If the assetVar does not have the log file, check if the local map has a log file
    //             if (amap[logName])
    //             {
    ...
    //     }
    //     ival = 2;
    //     amap[reloadStr] = dbiAv->am->vm->setLinkVal(vmap, aname, "/reload", reloadStr.c_str(), ival);
...
/**
 * @brief Periodically check if the variables need to be updated in the dbi system.
 * 
 * @param vmap the global data map shared by all assets/asset managers
 * @param amap the local data map used by an asset/asset manager
 * @param aname the name of the asset/asset manager
...
{
    //FPS_DEBUG_PRINT("%s >> Running \n", __func__);
    if(0)FPS_PRINT_INFO("Running for assetVar [{}] with aname [{}]\n", dbiAv->getfName(), aname);
    //return 0;
    assetVar * av = dbiAv;
    VarMapUtils* vm = av->am->vm;
...
int SendDbiVar(varsmap &vmap, varmap &amap, const char* aname, fims* p_fims, assetVar* av)
{
    ...
    if(debug)FPS_PRINT_INFO("Running for assetVar [{}] with aname [{}]", av->getfName(), aname?aname:"no aname");
    ...
    if(reload < 2)
    {
        ...
        if(reload < 1)
        {
            if(debug)FPS_PRINT_INFO("Not set up for [{}] reloadStr [{}]\n", av->name, reloadStr);
            ...
            if (!logFile)
            {
                ...
                {
                    logFile = amap[logName]->getcVal();
                    if(debug)FPS_PRINT_INFO("Log file name from amap [{}] is [{}]", aname, logFile);
                }
                else
                {
                    logFile = (char*)"file:DBIVarLog.txt";
...
//            SetupAvParams(aname, av);
    ...
    if(debug)FPS_PRINT_INFO(" vact for  [{}] is [{}]", av->getfName(), vact);
    ...
    if(vact =="set")
    {
        ...
        {
            if(debug)FPS_PRINT_INFO("[{}] db update too soon ", av->getfName());
            return 0;
        }
        ...
        if(comp)
        {
            for ( auto xx : av->extras->baseDict->featMap)
            {
                ...
                if(debug)FPS_PRINT_INFO(" [{}] name  [{}] var [{}]"
                    , idx++
                    , xx.first
                    , var?var->getfName(): "noVar"
                    );
            }
        }
        ...
        if(debug)FPS_PRINT_INFO(" [{}] body  [{}]", av->getfName(), body);
        //auto uristr = fmt::format("/dbi/ess_controller/dbivars{}",av->getfName());
        auto uristr = fmt::format("{}/{}",av->comp,av->name);
        auto uripart = vm->getDbiNameAv(uristr);
        if(debug)FPS_PRINT_INFO(" [{}] uri  [/dbi/ess_controller/dbivars{}]", av->getfName(), uripart);
    ...
    else if(vact == "resp")
    {
        ...
        if(debug)FPS_PRINT_INFO("DBI RESP 1>>>av [{}]  handle resp params :", av->getfName());
        char* dname = nullptr;
        ...
        if(debug)FPS_PRINT_INFO("DBI RESP 2>>>av [{}]  handle resp params :", av->getfName());
        ...
        else
        {
            for ( auto xx : av->extras->baseDict->featMap)
            {
                ...
                {
                    auto var = vm->getVar(vmap, comp, xx.first.c_str());
                    if(debug)if(var)FPS_PRINT_INFO("DBI RESP 3>>> xx.first [{}:{}]  var [{}] handle resp params :"
                            , comp
                            , xx.first
                            , fmt::ptr(var)
                            );
                    ...
                    if (var)
                    {
                        ...
                        //av->extras->baseDict->setFeatfromAv(xx.first.c_str(), var);
                        if(debug)FPS_PRINT_INFO("DBI RESP >>>  idx  [{}] setting value for av [{}] name  [{}]", idx, var->getfName(), xx.first);
                    }
                    else
                    {
                        if(0)FPS_PRINT_ERROR("DBI RESP >>>  idx  [{}] var [{}:{}] not found", idx, comp, xx.first);

                    }
                    idx++;
                }
            }
            if(debug)FPS_PRINT_INFO("DBI RESP 4++>>>av [{}]  deleting Params :", av->getfName());
...
/**
 * @brief Sets up a variable as a member of a DbiDoc(auto created) , updates the DbiDoc Param and pending count when it changes value
 * 
 * @param vmap the global data map shared by all assets/asset managers
 * @param amap the local data map used by an asset/asset manager
 * @param aname the name of the asset/asset manager
...
int SetDbiDoc(varsmap &vmap, varmap &amap, const char* aname, fims* p_fims, assetVar* av)
{
    ...
    if(debug)FPS_PRINT_INFO("Running for assetVar [{}] with aname [{}]", av->getfName(), aname);
