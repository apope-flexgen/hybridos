...
int RunPub(varsmap &vmap, varmap &amap, const char* aname, fims* p_fims, assetVar* aV)
{
    ...
    {
        if(debug)FPS_PRINT_INFO("{} >> Found table param for   [{}]"
            , __func__
            , aV->getfName()
            );
    ...
    if(!table)
    {
        if(aV->gotParam("uri"))
        {
            if(debug)FPS_PRINT_INFO("{} >> Found uri param for   [{}]"
                , __func__
                , aV->getfName()
                );
    ...
    if(table == nullptr)
    {
        FPS_PRINT_ERROR("{} >> please provide table or  uri param for   [{}]"
            , __func__
            , aV->getfName()
            );
        return 0;
    ...
    if(runaV)
    {
        ...
        FPS_PRINT_INFO("  config var found   [{}] value [{}]"
            , runaV->getfName()
            , pval?pval:"no Value"
            );
        if(pval)
        {
            ...
            FPS_PRINT_INFO("  config var value  [{}], nmaps [{}]"
                , pval
                , nmaps
                );
            int idx = 0;
    ...
    if(debug)FPS_PRINT_ERROR("{} >> Processing [{}] publishing table [{}] as [{}]"
        , __func__
        , aV->getfName()
        , table
...
void SendPub(varsmap& vmap, asset_manager* am, const char* uri, const char* puri, assetVar* aV)
{
    ...
    // fmt::basic_memory_buffer<char, 10000> buf; // for some reason this doesn't work. Might be a compiler bug?
    std::string output; // I would like to use the buf but it appears to not want to work with the buf for some reason?
    ...
    if (pos != std::string::npos)
    {
        ...
        if(debug)FPS_PRINT_INFO(" now seeking [{}]", suri);
        for (auto xx : vmap)
        {
            if(debug)FPS_PRINT_INFO(" current table [{}] suri [{}]", xx.first, suri);
            if (strncmp(xx.first.c_str(),suri.c_str(), suri.size()) == 0)
            {
                if(debug)FPS_PRINT_INFO(" found [{}]", xx.first);
...

    {
        ...
        else
        {
            ...
            {
                output = std::move(fmt::format(R"({{"{}":{{"value":"uri {} nonexistent"}}}})", suri , suri));
            }
        }
    }
    if(!puri)
    ...
    if(rc <= 0)
    {
        FPS_PRINT_ERROR("Pub Failed rc {} size {}"
        , rc
        , size
        );
