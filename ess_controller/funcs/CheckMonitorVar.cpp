...
// Namespace for functions dealing with time adjustments for alarm, fault, and
// recovery states
...
void alert(varsmap& vmap, varmap& amap, const char* aname, assetVar* av, const std::string& msg, bool fault)
{
    ...
    const auto now = flex::get_time_dbl();  // might need to use this instead of
                                            // direct calls to ensure times are the
                                            // same.
    if (fault)
    {
...
void SetupLimitParams(const char* aname, assetVar* av)
{
    ...
    if (!av->gotParam("EnableFaultCheck"))
        av->setParam("EnableFaultCheck", false);
    if (!av->gotParam("EnableMaxValCheck"))
        av->setParam("EnableMaxValCheck", false);
    if (!av->gotParam("EnableMinValCheck"))
        av->setParam("EnableMinValCheck", false);
    if (!av->gotParam("MaxAlarmThreshold"))
        av->setParam("MaxAlarmThreshold", 0.0);
    if (!av->gotParam("MaxFaultThreshold"))
        av->setParam("MaxFaultThreshold", 0.0);
    if (!av->gotParam("MaxResetValue"))
        av->setParam("MaxResetValue", 0.0);
    if (!av->gotParam("MinAlarmThreshold"))
        av->setParam("MinAlarmThreshold", 0.0);
    if (!av->gotParam("MinFaultThreshold"))
        av->setParam("MinFaultThreshold", 0.0);
    if (!av->gotParam("MinResetValue"))
        av->setParam("MinResetValue", 0.0);
    if (!av->gotParam("MaxAlarmTimeout"))
        av->setParam("MaxAlarmTimeout", 0.0);
    if (!av->gotParam("MaxFaultTimeout"))
        av->setParam("MaxFaultTimeout", 0.0);
    if (!av->gotParam("MaxRecoverTimeout"))
        av->setParam("MaxRecoverTimeout", 0.0);
    if (!av->gotParam("MinAlarmTimeout"))
        av->setParam("MinAlarmTimeout", 0.0);
    if (!av->gotParam("MinFaultTimeout"))
        av->setParam("MinFaultTimeout", 0.0);
    if (!av->gotParam("MinRecoverTimeout"))
        av->setParam("MinRecoverTimeout", 0.0);
...
void SetupStateParams(const char* aname, assetVar* av)
{
    ...
    if (!av->gotParam("AlarmTimeout"))
        av->setParam("AlarmTimeout", 0.0);
    if (!av->gotParam("FaultTimeout"))
        av->setParam("FaultTimeout", 0.0);
    if (!av->gotParam("RecoverTimeout"))
        av->setParam("RecoverTimeout", 0.0);
    if (!av->gotParam("Type"))
        av->setParam("Type", (char*)"int");
    if (!av->gotParam("lastVal"))
    {
        if (!strcmp(av->getcParam("Type"), "string"))
            av->setParam("lastVal", (char*)"Init val");
        else
            av->setParam("lastVal", 0);
    }
...
void SetupConditionParams(varsmap& vmap, varmap& amap, const char* aname, assetVar* av)
{
    ...
    if (!av->gotParam("numConditionVars"))
        av->setParam("numConditionVars", 0);
    for (int i = 1; i <= av->getiParam("numConditionVars"); i++)
    {
        ...
        if (!condType)
            av->setParam(condTypeParam, (char*)"int");
        if (condTypeParam)
            free(condTypeParam);
    }
...
void CheckState(varsmap& vmap, varmap& amap, const char* aname, assetVar* av, T& val, T& lastVal)
{
    ...
    if (!strcmp(type, "int"))
    {
        if (debug > 0 && enableStateCheck)
            FPS_PRINT_INFO(
                "{} Value [{}] lastVal [{}]  Fault Time [{:.4f}] Alarm "
                "Time [{:.4f}] Recovery Time [{:.4f}]",
                av->name, av->getiVal(), av->getiParam("lastVal"), currFaultTime, currAlarmTime, currRecoverTime);
        if (debug > 0)
            av->setParam("debug", debug - 1);
    }
    else if (!strcmp(type, "double"))
    {
        if (debug > 0 && enableStateCheck)
            FPS_PRINT_INFO(
                "{}  Value [{:.4f}] lastVal [{:.4f}]  Fault Time [{:.4f}] "
                "Alarm Time [{:.4f}] Recovery Time [{:.4f}]",
                av->name, av->getdVal(), av->getdParam("lastVal"), currFaultTime, currAlarmTime, currRecoverTime);
        if (debug > 0)
            av->setParam("debug", debug - 1);
    }
    else if (!strcmp(type, "string"))
    {
        if (debug > 0 && enableStateCheck)
            FPS_PRINT_INFO(
                "{} Value [{}] lastVal [{}] Fault Time [{:.4f}] Alarm "
                "Time [{:.4f}] Recovery Time [{:.4f}]",
                av->name, cstr{ av->getcVal() }, cstr{ av->getcParam("lastVal") }, currFaultTime, currAlarmTime,
                currRecoverTime);
        if (debug > 0)
            av->setParam("debug", debug - 1);
    }
    ...
    {
        if (currFaultTime > 0)
            ESSUtils::decrementTime(av, currFaultTime, tDiff, "FaultTime");
        if (currAlarmTime > 0)
            ESSUtils::decrementTime(av, currAlarmTime, tDiff, "AlarmTime");
        ...
        if (currFaultTime <= 0.0 && !seenFault)
        {
            ...
            if (enableAlert)
            {
                ...
                std::string msg;
                if (!strcmp(type, "int"))
                    msg = fmt::format("[{}] value [{}] is not changing for {:.2f} seconds at {}", av->name,
                                      av->getiVal(), FaultTimeCfg, timestamp);
                else if (!strcmp(type, "double"))
                    msg = fmt::format("[{}] value [{}] is not changing for {:.2f} seconds at {}", av->name,
                                      av->getdVal(), FaultTimeCfg, timestamp);
                else if (!strcmp(type, "string"))
                    msg = fmt::format("[{}] value [{}] is not changing for {:.2f} seconds at {}", av->name,
                                      av->getcVal(), FaultTimeCfg, timestamp);
            ...
            // If we got EnableCommsCheck as a param, and if that's set to true, then
            // set CommsOK to false
        ...
        if (currAlarmTime <= 0.0 && !seenAlarm)
        {
            ...
            if (enableAlert)
            {
                ...
                std::string msg;
                if (!strcmp(type, "int"))
                    msg = fmt::format("[{}] value [{}] is not changing for {:.2f} seconds at {}", av->name.c_str(),
                                      av->getiVal(), FaultTimeCfg, timestamp);
                else if (!strcmp(type, "double"))
                    msg = fmt::format("[{}] value [{}] is not changing for {:.2f} seconds at {}", av->name.c_str(),
                                      av->getdVal(), FaultTimeCfg, timestamp);
            ...
            vm->setVal(vmap, amap["MonitorVarAlarm"]->comp.c_str(), amap["MonitorVarAlarm"]->name.c_str(),
                       monAlrmAvVal);
        }
    }
    else
    {
        if (!strcmp(type, "int"))
            av->setParam("lastVal", av->getiVal());
        else if (!strcmp(type, "double"))
            av->setParam("lastVal", av->getdVal());
        else if (!strcmp(type, "string"))
            av->setParam("lastVal", av->getcVal());

        if (currRecoverTime > 0.0)
            ESSUtils::decrementTime(av, currRecoverTime, tDiff, "RecoverTime");
        ...
        // If the value has changed, reset the current alarm/fault time back to the
        // configurable time
        if (currAlarmTime < AlarmTimeCfg)
            av->setParam("AlarmTime", AlarmTimeCfg);
        if (currFaultTime < FaultTimeCfg)
            av->setParam("FaultTime", FaultTimeCfg);
    }
}
...
{
/**
 * @brief Helper function for findExpectedVal and findConditionVal to compare
 * the current value to the expected value(s)
 *
...
/**
 * @brief Helper function for CheckCondition function to determine if the
 * current value is equal to the expected value(s)
 *
 * @param av the assetVar to check expected values for
 * @return true if the value is equal to at least one expected value, the
 * numExpectedVals is 0, or the expectedVal parameter(s) do not exist
 */
bool findExpectedVal(assetVar* av)
{
    // If we do not have the correct parameters initialized before the check,
    // immediately exit this function No need to perform alarm/fault check
    if (!av->gotParam("numExpectedVals") || av->getiParam("numExpectedVals") <= 0)
    {
        FPS_DEBUG_PRINT(
            "%s >> av [%s] numExpectedVals either does not exist or is "
            "<= 0. Skipping condition check\n",
            __func__, av->name.c_str());
        return false;
    }

    // Get the list of expected value(s) of specified type before doing the
    // comparison check
    const char* type = av->getcParam("Type");
    if (!strcmp(type, "int"))
    {
        ...
        for (int i = 1; i <= av->getiParam("numExpectedVals"); i++)
        {
            ...
            if (!av->gotParam(expectedValParam))
            {
                FPS_DEBUG_PRINT("%s >> %s does not exist in av %s. Skipping condition check\n", __func__,
                                expectedValParam, av->name.c_str());
    ...
    {
        FPS_DEBUG_PRINT(
            "%s >> assetVar %s does not have a type. Type needs to be "
            "defined for condition check\n",
            __func__, av->name.c_str());
        return false;
    }
}
...
/**
 * @brief Helper function for findCondition function to determine if the given
 * condition var has a value that is equal to the condition value(s)
 *
 ...
 * @return true if the condtion var has a value that is equal to at least one
 * condition value
 */
bool findConditionVal(assetVar* av, assetVar* condAv, int id)
{
    // TODO: use smart ptr to reduce # of times we need to use free()
    // std::unique_ptr<std::string> numCondParam(new std::string("numConditions" +
    // std::to_string(id)));
    ...
    {
        FPS_DEBUG_PRINT(
            "%s >> av [%s] %s either does not exist or is <= 0. "
            "Skipping condition check\n",
            __func__, av->name.c_str(), numCondValParam);
        free(numCondValParam);
        return false;
    }

    // Get the list of condition value(s) of specified type before doing the
    // comparison check
    char* condTypeParam = nullptr;
    ...
    if (!strcmp(condType, "int"))
    {
        ...
        for (int i = 1; i <= av->getiParam(numCondValParam); i++)
        {
            ...
            {
                FPS_DEBUG_PRINT(
                    "%s >> %s of type %s does not exist in av %s. Skipping "
                    "condition check\n",
    ...
    {
        FPS_DEBUG_PRINT(
            "%s >> assetVar %s does not have a condition type. Type "
            "needs to be defined for condition check\n",
            __func__, av->name.c_str());
        free(condTypeParam);
        free(numCondValParam);
        return false;
...
/**
 * @brief Helper function for CheckCondition function to find the condition var
 * and the one or more condition(s) that matches the current value of the
 * condition var
 *
 * @param amap the local data map used by an asset/asset manager
 * @param av the assetVar that contains the condition values to check for
 * @return true if the condition var's value is equal to at least one of the
 * condition value(s)
 */
bool findCondition(varmap& amap, assetVar* av)
{
    ...
    {
        FPS_DEBUG_PRINT(
            "%s >> av [%s] numConditionVars either does not exist or "
            "is <= 0. Skipping condition check\n",
            __func__, av->name.c_str());
        return false;
    }
    ...
    for (int i = 1; i <= av->getiParam("numConditionVars"); i++)
    {
        ...
        if (av->gotParam(condVarParam) && av->getcParam(condVarParam))
        {
            ...
            // Check if the assetVar in the variable parameter already exists. If not,
            // skip calculations
            ...
            {
                FPS_DEBUG_PRINT(
                    "%s >> assetVar in parameter [%s] of assetVar [%s] "
                    "does not exist. Skipping condition check\n",
                    __func__, condVarParam, av->name.c_str());
                free(condVarParam);
                return false;
            }

            // If we did find a condition value match, then set foundConditions to
            // false. This indicates search failure since we expect to find value
            // matches for all condition vars
            if (!findConditionVal(av, condAv, i))
            {
                FPS_DEBUG_PRINT("%s >> findConditionVal for conditionAv %s and av %s is false\n", __func__,
                                condVarParam, av->name.c_str());
                free(condVarParam);
                return false;
            }
        }
        ...
        {
            FPS_DEBUG_PRINT(
                "%s >> assetVar [%s] does not have parameter [%s]. "
                "Skipping condition check\n",
                __func__, av->name.c_str(), condVarParam);
            free(condVarParam);
            return false;
    ...
    // We have reached the end of the loop, which means we have found a value
    // match for all condition vars Return true, indicating search success
    return true;
}
}  // namespace Condition
...
void CheckCondition(varsmap& vmap, varmap& amap, const char* aname, assetVar* av)
{
    ...
    // If the condition check is disabled, reset alarm/fault/reset states and
    ...
    // Find the condition val. If return value is true, then we have found at
    // least one condition value equal to the condition av value
    bool foundCondVal = Condition::findCondition(amap, av);
    // Find the expected val. If return value is true, then we have found at least
    // one expected value equal to the av value
    bool foundExpectedVal = Condition::findExpectedVal(av);

    // If we have found the expected value and our conditions are satisfied,
    // perform recovery tasks
    if (foundExpectedVal)
    {
        FPS_DEBUG_PRINT("%s >> Found expected value for av %s\n", __func__, av->name.c_str());
        // If we did not find the condition value match when we have condition vars,
        // then skip recovery tasks
        if (!foundCondVal && av->gotParam("numConditionVars") && av->getiParam("numConditionVars") > 0)
            return;
        ...
        if (currAlarmTime < AlarmTimeCfg)
            ESSUtils::incrementTime(av, currAlarmTime, tDiff, AlarmTimeCfg, "AlarmTime");
        if (currFaultTime < FaultTimeCfg)
            ESSUtils::incrementTime(av, currFaultTime, tDiff, FaultTimeCfg, "FaultTime");
    }
    // We did not find the expected value match
    // If the condition values are not satisfied, proceed to decrement alarm/fault
    // time and send out alarm/fault event once alarm/fault time has elapsed
    else
    {
        FPS_DEBUG_PRINT("%s >> Expected value not found for av %s   foundCondVal %s\n", __func__, av->name.c_str(),
                        foundCondVal ? "true" : "false");
        // If we did not find a condition value match when we have condition vars,
        // skip alarm/fault event tasks
        if (!foundCondVal && av->gotParam("numConditionVars") && av->getiParam("numConditionVars") > 0)
            return;

        // If numExpectedVal is <= 0, then we should exit immediately. Should have
        // to send out alarm/fault if we can't compare the current value to the
        // expected value(s)
        if (!av->gotParam("numExpectedVals") || av->getiParam("numExpectedVals") <= 0)
            return;
        ...
        if (currFaultTime <= 0.0 && !seenFault)
        {
            ...
            if (enableAlert)
            {
                ...
                std::string msg;
                if (!strcmp(type, "int"))
                    msg = fmt::format("[{}] value [{}] != expected value(s) for {:.2f} seconds at {}", av->name.c_str(),
                                      av->getiVal(), FaultTimeCfg, timestamp);
                else if (!strcmp(type, "double"))
                    msg = fmt::format("[{}] value [{}] != expected value(s) for {:.2f} seconds at {}", av->name.c_str(),
                                      av->getdVal(), FaultTimeCfg, timestamp);
...
/**
 * @brief Check if the current value of the assetVar is above or below the
 * threshold value
 *
 * @param vmap the global data map shared by all assets/asset managers
 * @param amap the local data map used by an asset/asset manager
...
void CheckLimits(varsmap& vmap, varmap& amap, const char* aname, assetVar* av)
{
    ...
    if (debug > 0 && (enableMaxCheck || enableMinCheck))
    {
        if (enableMaxCheck)
            FPS_PRINT_INFO(
                "{} Value [{:.3f}] Max Alarm [{:.3f}] Max Fault [{:.3f}] "
                "Max Reset [{:.3f}] Fault Time [{:.4f}] Alarm Time "
                "[{:.4f}] Recovery Time [{:.4f}]",
                av->name, val, maxAlrmThreshld, maxFltThreshld, maxReset, currMaxFaultTime, currMaxAlarmTime,
                currMaxRecoverTime);

        if (enableMinCheck)
            FPS_PRINT_INFO(
                "{} Value [{:.3f}] Min Alarm [{:.3f}] Min Fault [{:.3f}] "
                "Min Reset [{:.3f}]   Fault Time [{:.4f}] Alarm Time "
                "[{:.4f}] Recovery Time [{:.4f}]",
                av->name, val, minAlrmThreshld, minFltThreshld, minReset, currMinFaultTime, currMinAlarmTime,
                currMinRecoverTime);
        if (debug > 0)
            av->setParam("debug", debug - 1);
    }
    ...
    // PSW we may have several different thresholds which may be operationally
    // dependent PSW So we may have a limit gradient based on some other value
    ...
    if (enableMaxCheck)
    {
        if (enableFaultCheck && val > maxFltThreshld)
        {
            ...
            if (currMaxFaultTime > 0)
                ESSUtils::decrementTime(av, currMaxFaultTime, tDiff, "MaxFaultTime");
            // Set monitor var to fault state
            if (currMaxFaultTime <= 0.0 && !seenMaxFault)
            {
                ...
                if (enableAlert)
                {
                    ...
                    std::string msg = fmt::format(
                        "[{}] value [{:.3f}] > max fault threshold [{:.3f}] "
                        "for {:.2f} seconds at {}",
                        av->name.c_str(), val, maxFltThreshld, maxFaultTimeCfg, timestamp);
                ...
                const char* monFltAvVal = val.c_str();
                vm->setVal(vmap, amap["MonitorVarFault"]->comp.c_str(), amap["MonitorVarFault"]->name.c_str(),
                           monFltAvVal);
            }
        }
        if (val > maxAlrmThreshld)
        {
            ...
            if (currMaxAlarmTime > 0)
                ESSUtils::decrementTime(av, currMaxAlarmTime, tDiff, "MaxAlarmTime");
            // Set monitor var to alarm state
            if (currMaxAlarmTime <= 0.0 && !seenMaxAlarm)
            {
                ...
                if (enableAlert)
                {
                    ...
                    std::string msg = fmt::format(
                        "[{}] value [{:.3f}] > max alarm threshold [{:.3f}] "
                        "for {:.2f} seconds at {}",
                        av->name.c_str(), val, maxAlrmThreshld, maxAlarmTimeCfg, timestamp);
        ...
        if (val < maxReset)
        {
            ...
            if (currMaxRecoverTime > 0.0)
                ESSUtils::decrementTime(av, currMaxRecoverTime, tDiff, "MaxRecoverTime");
            ...
            // Increment both the current alarm/fault time
            if (currMaxAlarmTime < maxAlarmTimeCfg)
                ESSUtils::incrementTime(av, currMaxAlarmTime, tDiff, maxAlarmTimeCfg, "MaxAlarmTime");
            if (currMaxFaultTime < maxFaultTimeCfg)
                ESSUtils::incrementTime(av, currMaxFaultTime, tDiff, maxFaultTimeCfg, "MaxFaultTime");
        }
    }
    else
    {
        // If max limit monitoring is disabled, then reset the alarm/fault/reset
        // states and timers
        av->setParam("MaxAlarmTime", av->getdParam("MaxAlarmTimeout"));
        av->setParam("MaxFaultTime", av->getdParam("MaxFaultTimeout"));
        av->setParam("MaxRecoverTime", av->getdParam("MaxRecoverTimeout"));
    ...
    if (enableMinCheck)
    {
        if (enableFaultCheck && val < minFltThreshld)
        {
            ...
            if (currMinFaultTime > 0)
                ESSUtils::decrementTime(av, currMinFaultTime, tDiff, "MinFaultTime");
            // Set monitor var to fault state
            if (currMinFaultTime <= 0.0 && !seenMinFault)
            {
                ...
                if (enableAlert)
                {
                    ...
                    std::string msg = fmt::format(
                        "[{}] value [{:.3f}] < min fault threshold [{:.3f}] "
                        "for {:.2f} seconds at {}",
                        av->name.c_str(), val, minFltThreshld, minFaultTimeCfg, timestamp);
        ...
        if (val < minAlrmThreshld)
        {
            ...
            if (currMinAlarmTime > 0)
                ESSUtils::decrementTime(av, currMinAlarmTime, tDiff, "MinAlarmTime");
            // Set monitor var to alarm state
            if (currMinAlarmTime <= 0.0 && !seenMinAlarm)
            {
                ...
                if (enableAlert)
                {
                    // Prepare alarm msg to send out
                    std::string msg = fmt::format(
                        "[{}] value [{:.3f}] < min alarm threshold [{:.3f}] "
                        "for {:.2f} seconds at {}",
                        av->name.c_str(), val, minAlrmThreshld, minAlarmTimeCfg, timestamp);
        ...
        {
            // Decrement recovery time
            if (currMinRecoverTime > 0.0)
                ESSUtils::decrementTime(av, currMinRecoverTime, tDiff, "MinRecoverTime");
            // Reset current state of monitor var
            ...
            // Increment both the current alarm/fault time
            if (currMinAlarmTime < minAlarmTimeCfg)
                ESSUtils::incrementTime(av, currMinAlarmTime, tDiff, minAlarmTimeCfg, "MinAlarmTime");
            if (currMinFaultTime < minFaultTimeCfg)
                ESSUtils::incrementTime(av, currMinFaultTime, tDiff, minFaultTimeCfg, "MinFaultTime");
        }
    }
    else
    {
        // If min limit monitoring is disabled, then reset the alarm/fault/reset
        // states and timers
        av->setParam("MinAlarmTime", av->getdParam("MinAlarmTimeout"));
        av->setParam("MinFaultTime", av->getdParam("MinFaultTimeout"));
        av->setParam("MinRecoverTime", av->getdParam("MinRecoverTimeout"));
...
/**
 * @brief Periodically check if the variables are either over or under the
...
int CheckMonitorVar(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* av)
{
    ...
    if (reload < 2)
    {
        ...
        FPS_PRINT_INFO("Fault Destination for {} with assetVar {} is {}", aname, av->getfName(),
                       cstr{ amap["FaultDestination"]->getcVal() });
        FPS_PRINT_INFO("Alarm Destination for {} with assetVar {} is {}", aname, av->getfName(),
    ...
    if (av->gotParam("EnableStateCheck"))
    {
        ...
        {
            FPS_PRINT_INFO(
                "{} with assetVar {} does not have a type. Type needs to "
                "be defined for state check",
                aname, av->name);
        }
    }
    else if (av->gotParam("EnableConditionCheck"))
        CheckCondition(vmap, amap, aname, av);
    else
        CheckLimits(vmap, amap, aname, av);
