...
int ShutdownPCS(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* aV)
{
    ...
    if (reload == 0)
    {
        ...
        linkVals(*vm, vmap, amap, essName, "/status", bval, "PCSShutdown", "ShutdownBMS"); //, "ShutdownCmd", "HardShutdown");
    ...
    if (reload < 2)
    {
        ...
        if (!amap["FullShutdown"]) amap["FullShutdown"]->setVal(false);
    ...
    if (amap["SystemState"]->valueChangedReset() || amap["HardShutdown"]->getbVal() || !strcmp(PCSSystemStateStep, "Init"))
    {
        ...
        if (1) FPS_ERROR_PRINT("%s >>> #1 State step changed to %s\n", __func__, SystemState);
    }

    // if (1) FPS_ERROR_PRINT("%s >>> APS DEADBAND %f\n", __func__, amap["ActivePowerSetpoint"]->dbV, amap["ReactivePowerSetpoint"]->dbV.value());
    ...
    {
        const auto now = flex::get_time_dbl();
        if (1) FPS_ERROR_PRINT("%s >> Sending 0kW 0kVAr to PCS at time %f\n", __func__, tNow);
        ESSLogger::get().info("Sending 0kW 0kVAr to PCS");
        assetVar* temp_av = amap["PCSShutdown"];
        if (temp_av) temp_av->sendEvent("PCS", am->p_fims, Severity::Info, "Sending 0kW 0kVAr to PCS at time %2.3f"
            , now.count()
            );
    ...
    if ((strcmp(SystemState, "Running") == 0 || strcmp(SystemState, "Ready") == 0) && ((tNow - amap["PCSShutdownTime"]->getdVal()) > amap["maxShutdownTime"]->getdVal()))
    {
        if (amap["HardShutdown"]->getbParam("Latch"))
        {
        ...
        else if (!amap["FullShutdown"]->getbParam("WaitingDCCurrent"))
        {
            ...
            amap["PCSSystemStateStep"]->setVal(PCSSystemStateStep);
            amap["PCSShutdownTime"]->setVal(tNow);
            if (1) FPS_ERROR_PRINT("%s >> PCS stuck in hold state %s, trying e-stop and resetting PCSShutdown timer at time: %f \n", __func__, PCSStatusResp, tNow);
            // Send Event - Alarm
            const auto now = flex::get_time_dbl();
            ESSLogger::get().warn("PCS stuck in hold state [{}], trying estop",
                PCSStatusResp);
            assetVar* temp_av = amap["ShutdownCmd"];
            if (temp_av) temp_av->sendEvent("PCS", am->p_fims, Severity::Alarm, "PCS stuck in hold state [%s], trying estop at time %2.3f"
                , PCSStatusResp
                , now.count()
                );
        }
        else
        {
            ...
            amap["PCSSystemStateStep"]->setVal(PCSSystemStateStep);
            amap["PCSShutdownTime"]->setVal(tNow);
            if (1) FPS_ERROR_PRINT("%s >> PCS Current still above shutdown threshold at time: %f\n", __func__, tNow);
            // Send Event - info
            const auto now = flex::get_time_dbl();
            ESSLogger::get().info("[PCS Current holding above threshold]");
            assetVar* temp_av = amap["PCSShutdown"];
            if (temp_av) temp_av->sendEvent("PCS", am->p_fims, Severity::Info, "PCS Current holding above threshold at time %2.3f"
                , now.count()
                );
            amap["FullShutdown"]->setParam("WaitingDCCurrent", false);
    ...
    else if (!amap["HardShutdown"]->getbParam("Latch") && !amap["FullShutdown"]->getbParam("Latch") && !amap["FullShutdown"]->getbParam("WaitingDCCurrentDone"))
    {
        if (amap["BMSCurrentCheckStop"]->getbParam("seenMaxAlarm") || amap["BMSCurrentCheckStop"]->getbParam("seenMinAlarm"))
        {
            if (!amap["FullShutdown"]->getbParam("WaitingDCCurrent"))
            {
                double Idc = amap["BMSCurrentCheckStop"]->getdVal();
                if (1) FPS_ERROR_PRINT("%s >> PCS Stop waiting for [DC Current %f A] to decrease\n", __func__, Idc);
                const auto now = flex::get_time_dbl();
                ESSLogger::get().warn("PCS Stop waiting for [DC Current {} A] to decrease", Idc);
                assetVar* temp_av = amap["PCSShutdown"];
                if (temp_av) temp_av->sendEvent("PCS", am->p_fims, Severity::Alarm, "PCS Stop waiting for [DC Current %f A] to decrease at time %2.3f"
                    , Idc
                    , now.count()
                    );
                amap["FullShutdown"]->setParam("WaitingDCCurrent", true);
        ...
        else
        {
            ...
            amap["PCSSystemStateStep"]->setVal(PCSSystemStateStep);
            if (1) FPS_ERROR_PRINT("%s >> PCS Current below shutdown threshold at time: %f\n", __func__, tNow);
            // Send Event - info
            const auto now = flex::get_time_dbl();
            ESSLogger::get().info("[PCS Current below turnoff threshold]");
            assetVar* temp_av = amap["PCSShutdown"];
            if (temp_av) temp_av->sendEvent("PCS", am->p_fims, Severity::Info, "PCS Current below turnoff threshold at time %2.3f"
                , now.count()
                );
            amap["FullShutdown"]->setParam("WaitingDCCurrent", false);
            amap["FullShutdown"]->setParam("WaitingDCCurrentDone", true);
    ...
    else if (strcmp(SystemState, "Off") == 0 || strcmp(SystemState, "Fault") == 0)
    {
        ...
        {
            // todo: this kinda cheats, a little, npos check is in, but they can still give me an empty string or a single
            // character
            std::string faultType{amap.at("sys_fault")->getcVal()};
            auto pos = faultType.find(')');
            if (pos != faultType.npos)
            {
                // caveat: requires that first character is "(" - will skip the first character on this assumption
            ...
            ESSLogger::get().critical("PCS faulted with message [{}]",
                    amap.at("sys_fault")->getcVal());
            if (logging_enabled)
            {
            ...
            assetVar* temp_av = amap["PCSShutdown"];
            if (temp_av) temp_av->sendEvent("PCS", am->p_fims, Severity::Fault, "PCS faulted with message [%s] at time %2.3f"
                , amap.at("sys_fault")->getcVal()
                , now.count()
                );
        }
        else
        {
            ...
            assetVar* temp_av = amap["PCSShutdown"];
            if (temp_av) temp_av->sendEvent("PCS", am->p_fims, Severity::Fault, "PCS shutdown complete at time %2.3f"
                , now.count()
                );
        }
    ...
    else if (strcmp(SystemState, "Ready") == 0 && !amap["HardShutdown"]->getbParam("Latch") && !amap["FullShutdown"]->getbParam("Latch"))
    {
        if (amap["FullShutdown"]->getbVal())
        {
            ...
            vm->clearVlist(vlist);
            if (1) FPS_ERROR_PRINT("%s >> Setting PCSRun to 0 at time %f\n", __func__, tNow);
        }
        else
        {
            PCSSystemStateStep = (char*)"PCS In Standby";
            amap["PCSSystemStateStep"]->setVal(PCSSystemStateStep);
            if (1) FPS_ERROR_PRINT("%s >> PCS In Standby at time: %f. ShutdownBMS? [%d] \n", __func__, tNow, amap["FullShutdown"]->getbParam("ShutdownBMS"));
            // Send Event - info
            const auto now = flex::get_time_dbl();
            ESSLogger::get().info("[PCS In Standby]");
            assetVar* temp_av = amap["PCSShutdown"];
            if (temp_av) temp_av->sendEvent("PCS", am->p_fims, Severity::Info, "PCS In Standby at time %2.3f"
                , now.count()
                );
            vm->schedStop(vmap, amap,"schedShutdownPCS", 0.001);
            pcsAv->setVal(1);
    ...
    else if (amap["FullShutdown"]->getbParam("Latch") && amap["PCSRun"]->getdVal() != 0)
    {
        if ((tNow - amap["PCSShutdownTime"]->getdVal()) > amap["maxShutdownTime"]->getdVal())
        {
            ...
            amap["PCSSystemStateStep"]->setVal(PCSSystemStateStep);
            if (1) FPS_ERROR_PRINT("%s >> PCS Start command not setting: [%s] tnow %2.3f \n", __func__, PCSStatusResp, tNow);
            // Send Event - fault
            const auto now = flex::get_time_dbl();
            ...
            if (temp_av) temp_av->sendEvent("PCS", am->p_fims, Severity::Fault, "PCS Start command not setting at time %2.3f"
                , now.count()
                );
    ...
    else if (amap["KeyCmd"]->getbParam("KeyCmdDone"))
    {
        if (amap["HardShutdown"]->getbParam("Latch"))
        {
            ...
            ESSLogger::get().critical("PCS not responding in state [{}]",
                PCSStatusResp);
            if (logging_enabled)
            {
        ...
        else
        {
            ...
            assetVar* temp_av = amap["PCSShutdown"];
            if (temp_av) temp_av->sendEvent("PCS", am->p_fims, Severity::Alarm, "PCS not responding, trying e-stop at time %2.3f"
                , now.count()
                );
        }
    }
    else if ((strcmp(SystemState, "Running") == 0 || strcmp(SystemState, "Ready") == 0 || amap["HardShutdown"]->getbParam("Latch"))
...
// TODO: review ShutdownBMS BEFORE MVP Will just send out a single command to an MBMU register.
int ShutdownBMS(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* aV)
{
    int reload = 0;
    bool bval = false;
    ...
    if (reload == 0)
    {
        ...
        // Statuses, etc.:
        // linkVals(*vm, vmap, amap, essName, "/status", tValInit, "SystemState", "SystemStateStep");
    ...
    else if ((tNow - amap["BMSShutdownTime"]->getdVal()) > amap["maxShutdownTime"]->getdVal())
    {
        ...
        ESSLogger::get().critical("BMS shutdown timed out in state [{}]",
            BMSPowerOn);
        if (logging_enabled)
        {
...
// TODO after MVP put the command definitions into config 
            ...
            // am->Send("set", "/components/catl_ems_bms_rw/ems_cmd", NULL, "{\"value\":3}");
        }
        else
        {
            amap["BMSKeyCmd"]->setVal(1);
            vm->addVlist(vlist, amap["BMSKeyCmd"]);
            // am->Send("set", "/components/catl_ems_bms_rw/ems_cmd", NULL, "{\"value\":1}");
        }
        vm->sendVlist(p_fims, "set", vlist);
        vm->clearVlist(vlist);
    }
...
int CheckGPIO(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* aV)
{
    ...
    assetVar* cgpioAv = amap[__func__]; // Need to figure out the resolution here!
    ...
    if (reload < 2)
    {
        ...
        // other variables:
        linkVals(*vm, vmap, amap, aname, "/status", bval, "EStop", "Surge Arrester", "Disconnect Switch", "Fire Alarm", "Fuse Monitoring", "Door Latch", "gpioFault");
    ...
    if (amap["gpioBitfield"]->getiVal() != gbf)     amap["gpioBitfield"]->setVal(gbf);

    if (0) FPS_ERROR_PRINT("%s >>> gbf = %d\n", __func__, gbf);
            ...
            for (auto x : vmap["/components/gpio"])
            {
                ...
                if (avg && avg->gotParam("pin"))
                {
                    if (avg->getbVal() && avg->getiParam("pin") > 2)
                    {
                        ...
                        if (1) FPS_ERROR_PRINT("%s, fltDest %s\n", msg, fltDest);
