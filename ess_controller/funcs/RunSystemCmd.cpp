 ...
 * @brief Information related to system requests (ex.: bms power on, power off, etc.) are encapulated in a command object.
 *        The command object will execute an action depending on the request
 * @date 2021-05-17
 */
...
public:
    ...
    void runCmd(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* av)
    {
        ...
        if (m_syscmd) m_syscmd->execute(vmap, amap, aname, p_fims, av);
    }
};
...
public:
    explicit SystemCmd(std::shared_ptr<SystemCmdReceiver> receiver, std::string& cmd) : m_receiver(std::move(receiver)), m_cmd(cmd) {}
    ...
    void execute(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* av) const override {
        ...
        if (m_cmd == "Power On")  { m_receiver->PowerOn(vmap, amap, aname, p_fims, av); }
        if (m_cmd == "Power Off") { m_receiver->PowerOff(vmap, amap, aname, p_fims, av); }
    }
};
...
int RunSystemCmd(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* av)
{
    ...
    // Or, in config, have key cmd value remapped to different value with string data type
