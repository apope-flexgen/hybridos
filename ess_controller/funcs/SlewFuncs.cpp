...
int SlewVal(varsmap &vmap, varmap &amap, const char* aname, fims* p_fims, assetVar*Av)
{
    if (!Av)
    {
        ...
        FPS_PRINT_ERROR("{}","cannot get Av to run slew function on, please check that an Av is provided for this function.");
        return -1;
    }
    if (!Av->am)
    {
  //      FPS_PRINT_ERROR("cannot get Av->am for Av: {}, please make sure it has one.", Av->getfName());
        return -1;
    }
    if (!Av->am->vm)
    {
    //    FPS_PRINT_ERROR("cannot get Av->am->vm for Av: {}, please make sure it has one.", Av->getfName());
        return -1;
    }
    // check for correct type:
    if (Av->aVal->type != assetVal::ATypes::AFLOAT)
    {
      //  FPS_PRINT_ERROR("non floating point types are not-supported for SlewVal. Av name is: {}", Av->getfName());
        return -1;
    }
    ...
    {
        //FPS_PRINT_ERROR("could not get SlewedVal for Av {}, please provide one.", Av->getfName());
        return -1;
    }
    ...
    if (!var_to_slew)
    {
        //FPS_PRINT_ERROR("could not get the var to slew using uri: {} for Av: {}", var_to_slew_uri, Av->getfName());
        return -1;
    }
    if (var_to_slew->type != assetVar::ATypes::AFLOAT) // might need to check for linkVal?
    {
        //FPS_PRINT_ERROR("the value to be slewed -> {}, does not contain a float.", var_to_slew->getfName());
        return -1;
    }
    ...
    {
        //FPS_PRINT_ERROR("can't find one of RatedValue or SlewRate for Av: {}, please provide them", Av->name);
        return -1;
    }
    ...
    if (SlewRate < 0.0)
    {
        //FPS_PRINT_ERROR("SlewRate cannot be negative, for Av: {}", Av->getfName());
        return -1;
    }
    double elapsedTime;
    ...
    {
        //FPS_PRINT_ERROR("elapsed time since LastSlewTime is negative for some reason, for Av: {}", Av->getfName());
        if(LastslewedTime > 0)
            Av->setParam("LastSlewedTime", current_time);
        return -1;
    }
    ...
    if (elapsedTime == 0.0) // can't have negative time (someone changed it)
    {
        //FPS_PRINT_ERROR("elapsed time since LastSlewTime is 0 for some reason, for Av: {}", Av->getfName());
        if(LastslewedTime > 0)
            Av->setParam("LastSlewedTime", current_time);
        return -1;
    }
    ...
    const auto to_add_or_sub = std::abs(RatedValue * SlewRate / 100 * elapsedTime); // make sure this is always positive
    ...
    // NOTE(WALKER): this determines whether or not we are slewing in the right direction
    // positive value means slew in the positive direction
    // negative value means slew in the negative direction
    ...
    if (!enabled)
    {
        ...
        if(LastslewedTime > 0)
            Av->setParam("LastSlewedTime", current_time); // make sure to reset LastSlewedTime even if not enabled (so we don't get buildup when we re-enable it)
        return 0;
    }
