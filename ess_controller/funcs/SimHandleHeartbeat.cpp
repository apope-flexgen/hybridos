...
int SimHandleHeartbeat(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* aV)
{
    ...
    if (0)
        FPS_ERROR_PRINT("Heartbeat [%s] val [%f] last set diff [%f]", tVal, dval,
                        amap["Heartbeat"]->getLastSetDiff(dval));

    free((void*)tVal);
    if (amap["Heartbeat"]->getLastSetDiff(dval) > 1.0)
    {
        if (0)
            FPS_ERROR_PRINT("Heartbeat >> Running val [%f] \n", dval);
        ...
        // dval = hb->getdVal();
        if (0)
            FPS_ERROR_PRINT("Heartbeat aname %s  val after set %f SimPcsComms [%s]\n", aname, dval,
                            SimPcsComms ? "true" : "false");
        char* sp = cd->getcVal();
        dval = hb->getdVal();
        if (SimPcsComms)
        {
            if (0)
                FPS_ERROR_PRINT(
                    "Heartbeat aname %s  val after set %f SimPcsComms [%s] "
                    "cd %p p_fims %p\n",
                    aname, dval, SimPcsComms ? "true" : "false", (void*)cd, (void*)p_fims);
            if (SimPub)
                vm->sendAssetVar(cd, p_fims, "pub", "/components/pcs_sim", "sys_timestamp");
            vm->setVal(vmap, "/components/pcs_sim", "pcs_timestamp", sp);
        }
        if (SimPcsHB)
        {
            if (SimPub)
                vm->sendAssetVar(hb, p_fims, "pub", "/components/pcs_sim", "sys_heartbeat");
        ...
        {
            if (SimPub)
                vm->sendAssetVar(cd, p_fims, "pub", "/components/bms_sim", "bms_timestamp");
            vm->setVal(vmap, "/components/bms_sim", "bms_timestamp", sp);
        }
        if (SimBmsHB)
        {
            if (SimPub)
                vm->sendAssetVar(hb, p_fims, "pub", "/components/bms_sim", "bms_heartbeat");
