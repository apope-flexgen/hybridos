...
// TODO after MVP get these GPIO names from config file
        ...
        linkVals(*vm, vmap, amap, aname, "/components", bval,
            "EStop", "Disconnect Switch", "Door Latch", "Surge Arrester", "Fire Alarm", "Fuse Monitoring");

        linkVals(*vm, vmap, amap, aname, "/components", dval,
            "GPIORefTime");
    ...
    // These are here so we don't "log(n)" search through the map again over and over when using the assetVars. Silly, but it works.
...
// TODO after MVP navigate through al the config GPIO items and get pin numbers from them

    // exclusive ors between the current gpio bit and the "inverted" status, just change the config
    for (std::size_t i = 0; i < gpio_bitfield.size(); ++i)
    {
        ...
        if (i == 0 && EStop->getbParam("enabled"))
        {
            if ((gpio_bitfield[i] ^ EStop->getbParam("inverted"))) 
                { EStop->setVal(true); vm->addVlist(vlist, EStop);countflag++; } // continuously sends when on.
            else if (!(gpio_bitfield[i] ^ EStop->getbParam("inverted")) && EStop->valueChangedReset()) 
                { EStop->setVal(false); vm->addVlist(vlist, EStop); countflag++;} // Reset EStop
        }
        if (i == 1 && Disconnect_Switch->getbParam("enabled"))
        {
            // Disconnect Switch:
            if ((gpio_bitfield[i] ^ Disconnect_Switch->getbParam("inverted"))) 
                { Disconnect_Switch->setVal(true); vm->addVlist(vlist, Disconnect_Switch);countflag++; } // Continuously sends when on.
            else if (!(gpio_bitfield[i] ^ Disconnect_Switch->getbParam("inverted")) && Disconnect_Switch->valueChangedReset()) 
                { Disconnect_Switch->setVal(false); vm->addVlist(vlist, Disconnect_Switch); countflag++;} // Reset Disconnect Switch
        }
        if (i == 2 && Door_Latch->getbParam("enabled"))
        {
            // Door Latch:
            if ((gpio_bitfield[i] ^ Door_Latch->getbParam("inverted"))) 
                { Door_Latch->setVal(true); vm->addVlist(vlist, Door_Latch); countflag++;} // Continuously sends when on.
            else if (!(gpio_bitfield[i] ^ Door_Latch->getbParam("inverted")) && Door_Latch->valueChangedReset())
                { Door_Latch->setVal(false); vm->addVlist(vlist, Door_Latch); countflag++;} // Reset Door Latch
        }
        if (i == 3 && Surge_Arrester->getbParam("enabled"))
        {
            // Surge Arrrester:
            if ((gpio_bitfield[i] ^ Surge_Arrester->getbParam("inverted"))) 
                { Surge_Arrester->setVal(true); vm->addVlist(vlist, Surge_Arrester); countflag++;} // Continuously sends when on.
            else if (!(gpio_bitfield[i] ^ Surge_Arrester->getbParam("inverted")) && Surge_Arrester->valueChangedReset()) 
                { Surge_Arrester->setVal(false); vm->addVlist(vlist, Surge_Arrester); countflag++;} // Reset Surge Arrester
        }
        // Fire Alarm:
        if (i == 4 && Fire_Alarm->getbParam("enabled"))
        {
            ...
            if ((gpio_bitfield[i] ^ Fire_Alarm->getbParam("inverted"))) 
                { Fire_Alarm->setVal(true); vm->addVlist(vlist, Fire_Alarm); countflag++;} // Continuously sends when on.
            else if (!(gpio_bitfield[i] ^ Fire_Alarm->getbParam("inverted")) && Fire_Alarm->valueChangedReset())
                { Fire_Alarm->setVal(false); vm->addVlist(vlist, Fire_Alarm); countflag++;} // Reset Fire Alarm
        }
        // Fuse Monitoring:
        if (i == 5 && Fuse_Monitoring->getbParam("enabled"))
        {
            if ((gpio_bitfield[i] ^ Fuse_Monitoring->getbParam("inverted"))) 
                { Fuse_Monitoring->setVal(true); vm->addVlist(vlist, Fuse_Monitoring); countflag++;} // Continuously sends when on.
            else if (!(gpio_bitfield[i] ^ Fuse_Monitoring->getbParam("inverted")) && Fuse_Monitoring->valueChangedReset()) 
                { Fuse_Monitoring->setVal(false); vm->addVlist(vlist, Fuse_Monitoring); countflag++;} // Reset Fuse Monitoring
        }
...
int GpioSystemInit(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* aV)
{
    if (1)FPS_ERROR_PRINT(">>>>>>>>SCHED >>>>%s running for GPIO Manager\n", __func__);
    asset_manager* am = aV->am;
...
int GpioPub(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* aV)
{
    ...
    // {
        ...
        // {
            ...
            if(0)FPS_ERROR_PRINT("%s  sysVec -> [%s]\n",__func__, auri.c_str());
