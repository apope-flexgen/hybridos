...
{
/**
 * @brief Initialize the alarm/fault variables (ex.: destination, normal
 * fault/alarm condition messages, etc.)
 *
...
void initAlarmFaultVars(varsmap& vmap, varmap& amap, const char* aname, assetVar* av)
{
    ...
    FPS_PRINT_DEBUG("Fault Destination [{}]", amap["FaultDestination"]->getcVal() ? amap["FaultDestination"]->getcVal()
    ...
    FPS_PRINT_DEBUG("Alarm Destination [{}]", amap["AlarmDestination"]->getcVal() ? amap["AlarmDestination"]->getcVal()
                                                                                  : "did not find val from config");
    FPS_PRINT_DEBUG("No alarm msg [{}]",
                    amap["NoAlarmMsg"]->getcVal() ? amap["NoAlarmMsg"]->getcVal() : "did not find val from config");
}

/**
 * @brief Resets the monitor assetVar's alarm/fault state. Don't reset the
 * alarm/fault/recover time. This will allow fault/alarm messages to be reported
 * again if the alarm/fault still exists
 *
...
void resetMonitorVar(assetVar* monitorAv, const std::string& type)
{
    ...
    // Reset custom monitor var alarm/fault state (e.g. monitoring variables that
    // use CheckMonitorVar_v2)
...
/**
 * @brief Clear all alarm/fault messages and reset the monitor assetVar's
 * alarm/fault state, if applicable
 *
 * @param vmap the global data map shared by all assets/asset managers
 * @param amap the local data map used by an asset/asset manager
 * @param aname the name of the asset/asset manager
...
void clearAlarms(varsmap& vmap, varmap& amap, const char* aname, assetVar* av)
{
    ...
    if (debug)
        FPS_PRINT_INFO("Clearing all alarms/faults for asset [{}] with [{}]", cstr{ aname }, av->getfName());
    if (vmap.find(av->comp) != vmap.end())
    {
        ...
        for (auto& pair : avMap)
        {
            ...
            if (debug)
            {
                FPS_PRINT_DEBUG("avMap key {} assetVar {} lastVal before reset is {}", pair.first, var->name,
                                cstr{ var->getcLVal() });
            }
            ...
            if (var->name != av->name)
            {
                ...
                if (debug)
                {
                    FPS_PRINT_DEBUG("assetVar {} lastVal after reset is now {}", var->name, cstr{ var->getcLVal() });
                }
            }
        }
    }
    if (debug)
        FPS_PRINT_INFO("Clearing all alarms/faults 2 for asset [{}] with [{}]", cstr{ aname }, av->getfName());
    // If the assetVar name has been provided to the clear fault/alarms assetVar,
    // see if there  is anything else we need to do with that particular assetVar
    // (ex.: reset monitor assetVar's fault/alarm state)
    ...
    {
        if (debug)
            FPS_PRINT_INFO("numVars {}", av->getiParam("numVars"));
        for (int i = 1; i <= av->getiParam("numVars"); i++)
        {
            ...
            {
                if (debug)
                    FPS_PRINT_INFO("assetVar [{}] has parameter [{}]", av->name, varParam);
                ...
                // Check if the assetVar in the variable parameter already exists. If
                // not, skip reset step
                ...
                if (!monitorAv && defComp != nullptr)
                {
                    ...
                    {
                        if (debug)
                            FPS_PRINT_INFO(" setting up amap for  [{}:{}]", defComp, vp);
                ...
                if (monitorAv)
                {
                    if (debug)
                        FPS_PRINT_INFO(
                            "monitor assetVar {} in parameter [{}] of assetVar "
                            "[{}] exists. Starting reset step",
                            monitorAv->name, varParam, av->name);
                    std::string type = "";
                    ...
                    FPS_PRINT_INFO(
                        "monitor assetVar [{}] in parameter [{}] of assetVar [{}] does "
                        "not exist. Skipping alarm/fault reset step.",
                        av->getcParam(varParam.c_str()) ? av->getcParam(varParam.c_str()) : "no monitor var name",
                        varParam, av->name);
            }
            else
                FPS_PRINT_INFO(
                    "assetVar [{}] does not have parameter [{}]. Skipping "
                    "alarm/fault reset step",
                    av->getfName(), varParam);
        }
    }
    if (debug)
        FPS_PRINT_INFO("Clearing all alarms/faults 3 for asset [{}] with [{}]", cstr{ aname }, av->getfName());
}
}  // namespace AlarmFaultHandlerUtils

/**
 * @brief Report and record the alarm/fault produced either from the hardware
 * (ex.: BMS, PCS) or the ESS Controller itself
 *
 * @param vmap the global data map shared by all assets/asset managers
 * @param amap the local data map used by an asset/asset manager
 * @param aname the name of the asset/asset manager
...
int process_sys_alarm(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* av)
{
    ...
    if (!av->am)
    {
        FPS_PRINT_ERROR("name [{}] has no associated AM", av->name);
        return 0;
    }

    VarMapUtils* vm = av->am->vm;

    // Initialize the alarm/fault variables (ex.: Alarm/FaultDestination; done
    // once)
    ...
    // Initialize the destination, normal state, and the current/last value
    // (default values related to the alarm assetVar)
    ...
    // If we have a fault variable, reassign the destination and normal state
    // variables to the values related to the fault variable
    if (av->gotParam("type") && !strcmp(av->getcParam("type"), "fault"))
    {
    ...
    // If the assetVar has been set to Clear, then proceed to clear all
    // alarm/fault msgs and restore state
    if (avVal == "Clear")
    {
        ...
        // Reset the state of the fault/alarm assetVars (not including the clear
        // fault/alarm assetVar)
        AlarmFaultHandlerUtils::clearAlarms(vmap, amap, aname, av);

        // If we are the asset manager, clear all alarms/faults for asset managers
        // and instances
        if (strcmp(aname, av->am->name.c_str()) == 0)
        {
            // Iterate through all asset managers in the asset map and clear
            // faults/alarms for each manager
            for (auto& pair : av->am->assetManMap)
            {
                asset_manager* ami = pair.second;

                // To clear asset manager alarms/faults, we'll require that the name of
                // the variable responsible for clearing alarms/faults will be the same
                // for both the asset manager and the asset instance (ex.:
                // /faults/[asset]:clear_faults, /alarms/[asset]:clear_alarms)
                const std::string vmapKey = fmt::format("{}/{}", av->comp.substr(0, av->comp.find_last_of("/")),
                                                        ami->name);
                ...
                {
                    FPS_PRINT_DEBUG(
                        "asset manager [{}] under parent asset manager [{}] "
                        "has [{}] variable. Clearing alarms/faults for asset "
                        "manager",
            ...
            // Iterate through all asset instances in the asset map and clear
            // faults/alarms for each instance
            for (auto& pair : av->am->assetMap)
            {
                asset* ai = pair.second;

                // To clear asset instance alarms/faults, we'll require that the name of
                // the variable responsible for clearing alarms/faults will be the same
                // for both the asset manager and the asset instance (ex.:
                // /faults/[asset]:clear_faults, /alarms/[asset]:clear_alarms)
                const std::string vmapKey = fmt::format("{}/{}", av->comp.substr(0, av->comp.find_last_of("/")),
                ...
                {
                    FPS_PRINT_DEBUG(
                        "asset instance [{}] under asset manager [{}] has "
                        "[{}] variable. Clearing alarms/faults for asset "
                        "instance",
    ...
    // If the alarm/fault assetVar has a value change, check to see if that value
    // is a new alarm/fault state
