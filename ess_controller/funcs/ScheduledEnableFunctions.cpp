...
void CloseContactorsEnable(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* aV)
{
    ...
    if (reload == 0)
    {
        ...
        std::vector<DataUtility::AssetVarInfo> assetVarVector = {
            ...
            DataUtility::AssetVarInfo(assets.c_str(), "close_contactors", close_contactors.c_str(),
            ...
            DataUtility::AssetVarInfo(status.c_str(), "CloseContactorsEnabled", CloseContactorsEnabled.c_str(),
                                      assetVar::ATypes::ABOOL),
            // /controls/bms/CloseContactorsEnable
            DataUtility::AssetVarInfo(controls.c_str(), "CloseContactorsEnable", CloseContactorsEnable.c_str(),
                                      assetVar::ATypes::ABOOL)
        };
    ...
    if (reload == 1)
    {
        ...
        if (!amap[DCClosed.c_str()]->getbVal() && amap[maint_mode.c_str()]->getbVal() &&
        ...
        {
            message += fmt::format(" ---> Condition(s): [{}:{}] == false && [{}:{}] == true && [{}:{}] == false",
                                   amap[DCClosed.c_str()]->getfName(), amap[DCClosed.c_str()]->getbVal(),
                                   amap[maint_mode.c_str()]->getfName(), amap[maint_mode.c_str()]->getbVal(),
                                   amap[IsFaulted.c_str()]->getfName(), amap[IsFaulted.c_str()]->getbVal());
...
void OpenContactorsEnable(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* aV)
{
    ...
    if (reload == 0)
    {
        ...
        std::vector<DataUtility::AssetVarInfo> assetVarVector = {
            ...
            DataUtility::AssetVarInfo(assets.c_str(), "open_contactors", open_contactors.c_str(),
                                      assetVar::ATypes::ABOOL),
            // /status/bms/DCClosed
            DataUtility::AssetVarInfo(status.c_str(), "DCClosed", DCClosed.c_str(), assetVar::ATypes::ABOOL),
            // /status/bms/OpenContactorsEnabled
            DataUtility::AssetVarInfo(status.c_str(), "OpenContactorsEnabled", OpenContactorsEnabled.c_str(),
                                      assetVar::ATypes::ABOOL),
            // /controls/bms/OpenContactorsEnable
            DataUtility::AssetVarInfo(controls.c_str(), "OpenContactorsEnable", OpenContactorsEnable.c_str(),
                                      assetVar::ATypes::ABOOL),
            // /status/pcs/DCClosed
    ...
    if (reload == 1)
    {
        ...
        if (amap[DCClosed.c_str()]->getbVal() && !amap["DCClosed_PCS"]->getbVal() &&
        ...
        {
            message += fmt::format(" ---> Condition(s): [{}:{}] == true && [{}:{}] == false && [{}:{}] == true",
                                   amap[DCClosed.c_str()]->getfName(), amap[DCClosed.c_str()]->getbVal(),
                                   amap["DCClosed_PCS"]->getfName(), amap["DCClosed_PCS"]->getbVal(),
                                   amap[maint_mode.c_str()]->getfName(), amap[maint_mode.c_str()]->getbVal());
...
void StartEnable(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* aV)
{
    ...
    if (reload == 1)
    {
        ...
        if (amap["DCClosed_bms"]->getbVal() && amap["maint_mode"]->getbVal() && (systemState) &&
        ...
        else
        {
            ...
            message += fmt::format(
                ...
                amap["DCClosed_bms"]->getfName(), amap["DCClosed_bms"]->getbVal(), amap["maint_mode"]->getfName(),
                amap["maint_mode"]->getbVal(), amap["SystemState"]->getfName(), systemStateVal,
                amap["IsFaulted"]->getfName(), amap["IsFaulted"]->getbVal());
...
void StopEnable(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* aV)
{
    ...
    if (reload == 1)
    {
        ...
        else
        {
            ...
            message += fmt::format(" ---> Condition(s): [{}:{}] == true && [{}:{}] != Stop",
                                   amap["maint_mode"]->getfName(), amap["maint_mode"]->getbVal(),
                                   amap["SystemState"]->getfName(), systemStateVal);
...
void StandbyEnable(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* aV)
{
    ...
    if (reload == 1)
    {
        ...
        else
        {
            ...
            message += fmt::format(" ---> Condition(s): [{}:{}] == true && [{}:{}] == true && [{}:{}] == (Stop or Run)",
                                   amap["DCClosed"]->getfName(), amap["DCClosed"]->getbVal(),
                                   amap["maint_mode"]->getfName(), amap["maint_mode"]->getbVal(),
                                   amap["SystemState"]->getfName(), systemStateVal);
