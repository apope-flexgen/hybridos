...
int initVec(varsmap &vmap, VarMapUtils* vm, varmap &amap,  const char* mname
                   /* "MaxBMSDischargePowerEstFiltIn"*/, double initVal, double defdepth)
{
...
// just write a value to an assetVar with "depth" and "mathVec" params set up with this function triggered onSet.
// this function will reset the depth vector and restart the movingAvg process 
// you can also do this directly in appliction code using the  mathAv->setVecDepth(depth) function.
...
int SetVecDepth(varsmap &vmap, varmap &amap, const char* aname, fims* p_fims, assetVar* aV)
{
    ...
    if (mathAv && depth > 0 && depth < MAX_VEC_DEPTH)
    {
        ...
        if(debug)FPS_ERROR_FMT("{} >> setVecDepth for  [{}] to {}\n"
...
int MathMovAvg(varsmap &vmap, varmap &amap, const char* aname, fims* p_fims, assetVar* aV)
{
    ...
    if(aV->gotParam("window"))
    {
        ...
        if(debug)FPS_ERROR_FMT("{} >> found window param for [{}] value {}\n"
    ...
    { 
        if(debug)FPS_ERROR_FMT(
            "{} >> [{}] wind {} lval {} dval {} diff {} lim {} corrected {}\n"
                ...
                , std::abs(lval-dval)
                , lval*wind
                , (std::abs(lval-dval)> lval*wind)?
                     (lval > dval)?lval*(1-wind):lval*(1+wind):dval
                );
    }
    ...
    if(!vecAv)
    {
        ...
        {
            if(debug)FPS_ERROR_FMT("{} >> no vecAv Param for  [{}]\n"
                , __func__, aV->getfName());
            //return 0;
        }
    }
    ...
    if(vecAv) 
    {
        ...
        if(debug)FPS_ERROR_FMT("{} depth {} avg {:2.3f}, max {:2.3f} , min {:2.3f} spread {:2.3f} num {:2.3f}   \n"
        ...
        if(aV->gotParam("outAvg"))
        {
            ...
            if(!outAv->am)
                outAv->am = aV->am;
            if(debug)FPS_ERROR_FMT("{} >> set outAvg  [{}] to [{:2.3f}] \n"
                , __func__, spv, avval);
        }
        if(aV->gotParam("outMax"))
        {
            ...
            if(!outAv->am)
                    outAv->am = aV->am;
            if(debug)FPS_ERROR_FMT("{} >> set outMax  [{}] to [{:2.3f}] \n"
                , __func__, spv, maxval);
        }
        if(aV->gotParam("outMin"))
        {
            ...
            if(!outAv->am)
                    outAv->am = aV->am;
            if(debug)FPS_ERROR_FMT("{} >> set outMin  [{}] to [{:2.3f}] \n"
                , __func__, spv, minval);
        }
        // span
        if(aV->gotParam("outSp"))
        {
            ...
            if(!outAv->am)
                    outAv->am = aV->am;
            if(debug)FPS_ERROR_FMT("{} >> set outSp  [{}] to [{:2.3f}] \n"
                , __func__, spv, spval);
        }
        if(aV->gotParam("outSum"))
        {
            ...
            if(!outAv->am)
                    outAv->am = aV->am;
            if(debug)FPS_ERROR_FMT("{} >> set outSum  [{}] to [{:2.3f}] \n"
    ...
    if(aV->gotParam("outFilt"))
    {
        ...
        if(outAv)
        {
            ...
            if(debug)FPS_ERROR_FMT("{} >> old/dval [{:2.3f}]/[{:2.3f}] set outFilt  [{}] to [{:2.3f}] \n"
    ...
    if(aV->gotParam("outTimeFilt"))
    {
        ...
        else
        {
            ...
            if(outAv)
            {
                ...
                vm->setVal(vmap, spv, NULL, filt);
                if(1 || debug)FPS_ERROR_FMT("{} >> set outTimeFilt  [{}] to [{:2.3f}] \n"
                    , __func__, spv, filt);
            }
        }
    }
    ...
    // pus
    if(debug)FPS_ERROR_FMT("{} >> name [{}] input [{}] depth {}\n"
