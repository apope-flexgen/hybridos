...
extern "C++" {
    ...
    int HandleSplitItem(varsmap& vmap, assetVar* av, const char* newId, const char* newFunc
        , const char* newUri, double refInc, double fastRep, double runAfter, double newEnd);
    ...
    int SchedRunFunc(varsmap& vmap, const char* sname, const char* func, double runAfter, double refTime, double repTime, assetVar* aV);
    ...
    // int  SetupGit(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* av
    //              , const char*gbranch
    //              , const char*gcommit
    //              , const char*gtag
    //              , const char*gversion
...
// after MVP no need to reister functions for diffent am names unless we need to do so.
// the default "vm->getSysName()" will suffice 
int SetupEssSched(scheduler* sched, asset_manager* am)
{
    ...
    // am->vm->setFunc(*am->vmap, aname, "SetLoggingSize", (void*)&SetLoggingSize);
...
int  TestTriggerFunc(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* av)
{
    ...
    FPS_PRINT_INFO(
        "enabled [{}] rcount [{}] for av [{}] schedaV [{}] runTime [{:2.3f}] runCnt [{}]"
        , enabled
        , rcount
        , cstr{av->getfName()}
        , cstr{am->vm->schedaV->getfName()}
        , runTime
        , runCnt
    ); // spdlog
...
// NOTE: For the doubles you could also say {:2.6f} but fmt defaults to 
    ...
    if (runCnt > 10)
    {
        ...
        FPS_PRINT_INFO("shutting down for av [{}] runTime {:2.3f} runCnt {}"
            , cstr{am->vm->schedaV->getfName()}
            , runTime
            , runCnt
        ); // spdlog
...
// 	"addSchedItem":	{"value":	"None","actions":{"onSet":	[{ "func":	[{"func":	"HandleSchedItem","amap":	essName}]}]}},
...
// and create an new scheditem with a faster repeat time fastRep and a small offset (refInc)
// the Uri will need to be different 
// HandleSplitItem(vmap, av, "SlowPubOne", "SlowPubOne","/shed/ess/slowPubOne"
//   refTime+0.010, 0.010, 0.0)
int  HandleSplitItem(varsmap& vmap
    , assetVar* av, const char* newId, const char* newFcn, const char* newUri
    , double refInc, double fastRep, double newrunAfter, double newEnd)
{
    ...
    int runCnt = 0; //av->getiParam("runCnt");  runCnt is used to index into the array of things to process
    assetUri my(newUri, nullptr);
    bool debug = av->getbParam("debug");
    debug = false;
    if (debug) FPS_PRINT_INFO(
        "running for av [{}:{}] am {} vm {} schedId [{}] schedUri [{}] schedFcn [{}] refTime {:2.3f} runTime {:2.3f} repTime {:2.3f} runAfter {:2.3f}\n"
        , cstr{my.Uri}
        , cstr{my.Var}
        , fmt::ptr(am)
        , fmt::ptr(am->vm)
        , schedId ? schedId : "no Id"
        , schedUri ? schedUri : "no Uri"
        , schedFcn ? schedFcn : "no Fcn"
        , refTime
        , runTime
        , repTime
        , runAfter
    ...
    {
        FPS_PRINT_INFO("no SchedUri quitting"); // spdlog
        return 0;
    }

    if (!schedId || strcmp(schedId, "None") == 0)
    {
        FPS_PRINT_INFO("no SchedId quitting"); // spdlog
        return 0;
    }
    ...
    if (!avi)
    {
        ...
        if (debug)FPS_PRINT_INFO("added new schedUri {} [{}]"
            , fmt::ptr(avi), cstr{schedUri}); // spdlog
        int ival = 0;
        avi->setParam("runCnt", ival);
        avi->setParam("fcn", schedFcn);   // we'll run the function this way without adding actions.
    ...
    if (avi)
    {
        if (refTime > 0)avi->setParam("refTime", refTime);
        if (runTime > 0)avi->setParam("runTime", runTime);
        if (runCnt >= 0)avi->setParam("runCnt", runCnt);
        if (repTime > 0)avi->setParam("repTime", repTime);
        if (runAfter > 0)avi->setParam("runAfter", runAfter);


        //         if(endTime> 0)avi->setParam("endTime",endTime);
        // TODO  (look in FlexPack.cpp) after MVP we use the repTime and endTime to create single shots or run forever categories 
        // but this could be done a bit better
        //         dval = 0.0;
    ...
    if (avi && avi->am)
    {
        ...
        if (debug) FPS_PRINT_INFO("created new schedItem {}"
            , fmt::ptr(as)); // spdlog
        as->av = avi;
        if (avi->am)
            am = avi->am;
...
// TODO after MVP add amap to the schedItem 
        ...
        if (debug)FPS_PRINT_INFO(
            "@@@@@@ reset old values {} avi {} runTime {:2.3f}"
            , cstr{schedUri}
            , fmt::ptr(avi)
        ...
        if (debug)FPS_PRINT_INFO(
            "@@@@@@ done with old values {}   avi {} runTime {:2.3f}"
            , cstr{schedUri}
            , fmt::ptr(avi)
            , avi->getdParam("runTime")
        ); // spdlog
    ...
    {
        FPS_PRINT_INFO(
            "@@@@@ did not find var to activate avi {} avi->am {}"
            , fmt::ptr(avi), fmt::ptr(avi ? avi->am : nullptr)
        ); // spdlog
...
int  HandleSchedLoad(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* av)
{
    ...
    //debug = true;
    if (debug) FPS_PRINT_INFO("***** running for av [{}] am [{}] vm [{}] func aname [{}] av amap [{}]"
        , cstr{av->getfName()}
        , fmt::ptr(am)
        , fmt::ptr(am->vm)
        , aname ? aname : " No amap"
        , schedamap ? schedamap : "No amap"
    ); // spdlog
    ...
    if (!schedUri || strcmp(schedUri, "None") == 0)
    {
        FPS_PRINT_ERROR("no SchedUri (use uri)quitting"); // spdlog
        return 0;
    }

    if (!schedId || strcmp(schedId, "None") == 0)
    {
        FPS_PRINT_ERROR("no SchedId (use value) quitting"); // spdlog
        return 0;
    }
    ...
    //       runTime    refTime repTime  runAfter  runFor             startTime                                  endTime
    //       x          -       -        -         -                  tNow+runTime                               0 
    //       x          -       x        -         -                  tNow+runTime                               0
    //       x          x       -        -         -                  tNow+runTime                               0
    //       x          x       x        -         -                  refTime+(x*repTime)>tNow+runTime           0
    //       x          -       -        x         -                  tNow+runTime+runAfter                      0 
    //       x          -       x        x         -                  tNow+(runTime +(x *repTime) > tNow+runAfter 0 
    //       x          x       -        x         -                  tNow+runAfter                              0 
    //       x          x       x        x         -                  refTime +(x*repTime) > tNow+ runAfter      0 
    //       .          .       .        .         x                  .                                          startTime + runFor


    //       runTime plus runAfter plus runRef plus runRep        runTime = inc runRef by runRep untill past tNow plus runAfter
    //       runTime plus runAfter no runRef plus runRep          runTime = inc tNow by runRep untill past tNow plus runAfter
    //
    ...
    if (av && av->am)
    {
        ...
        if (debug)FPS_PRINT_INFO(
            "@@@@@@ done with old values {}   av {} runTime {:2.3f}"
            , cstr{schedUri}
            , av->getfName()
    ...
    else
    {
        FPS_PRINT_INFO(
            "@@@@@ did not find var to activate avi {} avi->am {}"
            , fmt::ptr(av), fmt::ptr(av ? av->am : nullptr)); // spdlog
    }
    return 0;
}

// this should be it....
//                        schedId        func,     initial delay, priority , repeat time, assetVar 
//SchedRunFunction(vmap, "GpioPubId",    GpioPub,  2.0,           0.01,       0.1,        aV);
...
int SchedRunFunc(varsmap& vmap, const char* sname, const char* func, double runAfter, double refTime, double repTime, assetVar* aV)
{
    ...
    if (avs && am)
    {
        ...
        //as->setUp(myid, am->name.c_str(), schedUri, schedFcn,  refTime, runTime, repTime, endTime, schedTarg);
        as->setUp(myid, am->name.c_str()
            , stmp.c_str(), func, refTime, runTime, repTime, endTime, aV->getfName());
        if (debug)
        {
            FPS_PRINT_INFO("created new schedItem {} [{}]"
                , fmt::ptr(as), cstr{aV->getfName()}
            ); // spdlog
            as->show();

            FPS_PRINT_INFO("@@@@@@ created schedVar [{}] runTime {}"
                , stmp
                , avs->getdParam("runTime")
            ); // spdlog
        }
        if (am->reqChan)
        {
            ...
            if (debug)FPS_PRINT_INFO("@@@@@@ activated {}   av {} runTime {:2.3f}"
                , stmp
                , fmt::ptr(avs)
                , avs->getdParam("runTime")
            ); // spdlog
        ...
        {
            FPS_PRINT_INFO("@@@@@ You need to set up am->reqChan and am->wakeChan @@@@"
            ); // spdlog
            delete as;
            as = nullptr;
        }
    ...
    {
        FPS_PRINT_INFO("@@@@@ did not find var to activate asi {} avi->am {}"
            , fmt::ptr(avs)
            , fmt::ptr(aV ? aV->am : nullptr)
        ); // spdlog
    }
...
// take the options list and send a list of uris to a target function like HandleSchedItem
// or send each option to a uri as a body.
// option to send a value to a assetVar and then send that assetvar to a function if one is defined
...
int  SchedItemOpts(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* av)
{
    ...
    if(!vm)
    {
        FPS_PRINT_INFO(" error no vm in av [{}] cannot proceed "
        , av->getfName()
        );
        return -1;
    }
    ...
    {
        FPS_PRINT_INFO("@@@@@ [{}] found cjopts ready to schedule res1 [{}] tav [{}]"
            , av->getfName()
            , fmt::ptr(res1)
        ...
        { 
            ...
            if (cjuri && cjuri->valuestring)
            {
                ...
                if(debug)FPS_PRINT_INFO( " idx [{}] uri ->[{}] avi [{}]"
                    , idx
                    , cjuri->valuestring
            ...
            if (cjuri && cjuri->valuestring && targVal)
            {
                ...
                if(!avi->am) avi->am=ams;
                if(!avi->am) avi->am=av->am;
            ...
            if (res1 && avi)
            {
                ...
                if(ams)
                {
                    if(debug)FPS_PRINT_INFO( " idx [{}] AMS @@@@ uri ->[{}] func [{}]"
                        , idx
                ...
                if(ais)
                {
                    if(debug)FPS_PRINT_INFO( " idx [{}] AIS @@@@ uri ->[{}] func [{}]"
                        , idx
                        , avi?avi->getfName():av->getfName()
                        , tfun?tfun:"no tfun"
                        );
...
int  RunScript(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* av)
{
    ...
    // auto cmd = fmt::format("sh /home/scripts/RunScript.sh {} {}&", av->comp, av->name);
...
int  HandleSchedItem(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* av)
{
    ...
    {
        FPS_PRINT_INFO("no schedFcn (fcn) quitting"); // spdlog
        return 0;
    }
    ...
    if (avi && avi->am)
    {
        ...
        FPS_PRINT_INFO("@@@@@@ reset old values {}   avi {} runTime {:2.3f}"
            , cstr{schedUri}
            , fmt::ptr(avi)
            , avi->getdParam("runTime")
        ); // spdlog
        char* nonestr = strdup("None");
        //once we replce the Params the old values are no longer valid , they have been free'd
        if (schedUri)
        {
        ...
        free(nonestr);
        if (debug)FPS_PRINT_INFO("@@@@@@ done with old values {}   avi {} runTime {}"
            , cstr{schedUri}
            , fmt::ptr(avi)
            , avi->getdParam("runTime")
        ); // spdlog
        if (am->reqChan)
        {
            ...
            if (debug)FPS_PRINT_INFO("@@@@@@ activated {}   avi {} runTime {}"
                , cstr{schedUri}
                , fmt::ptr(avi)
                , avi->getdParam("runTime")
            ); // spdlog
    ...
    {
        FPS_PRINT_INFO("@@@@@ did not find var to activate avi {} avi->am {}"
            , fmt::ptr(avi), fmt::ptr(avi ? avi->am : nullptr)); // spdlog
    }
    return 0;
}
...
int EssSystemInit(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* av)
{
    ...
    if (am->reload != 2)
    {
        if (debug) FPS_PRINT_INFO("running init for {} Manager"
            , am->name); // spdlog
            //am->run_init(am);
            //static int setup = 0;
        ...
        if (!am->setup)
        {
            if (debug)FPS_PRINT_INFO("MANAGER LOOP {}  setup MonitorVar"
                ,am->name); // spdlog
        ...
        //get base am from vm->getSysName and set up all the direct reports
            ...
            am->setup = 1;
            if (debug)FPS_PRINT_INFO("MANAGER LOOP {} completed setup MonitorVar"
                , am->name); // spdlog
        }
        bool bval = false;
    ...
    if (debug)FPS_PRINT_INFO(">>>>>>>>>>>>>>>>>>>>> done running init for {} Manager"
        , am->name); // spdlog
...
// this can now be attached to different assetVars to allow it to run at different rates
int SendTrue(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* aV)
{
    asset_manager* am = aV->am;
    if (!am || !am->vm)
    ...
    if(0)FPS_PRINT_INFO(" send [{}] to [{}]"
            , bval, aV->getfName()); // spdlog
    return 0;
}
// this can now be attached to different assetVars to allow it to run at different rates
int SendTime(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* aV)
...
int RunMonitorList(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* aV)
{
    ...
    if(debug)
        FPS_PRINT_ERROR("  RUNNING am ({}) (or vm) for av [{}] aname [{}] debug [{}]"
            , fmt::ptr(am), cstr{aV->getfName()}, aname, debug); // spdlog
    ...
    if(debug)FPS_PRINT_ERROR("running for [{}] amname [{}] monitor_name [{}]"
                ,  cstr{aV->getfName()}, amname, mname); // spdlog
    ...
    if(runChild)
    {
        for (auto& ix : am->assetManMap)
        {
            ...
            if (debug) FPS_PRINT_INFO("Got asset manager {}. Running func now..."
                , amc->name); // spdlog
        // Run monitor list for asset managers
            ...
            {
                asset* ami = iy.second;
                if (debug) FPS_PRINT_INFO("Got asset instance {} {} with asset manager {}. Running monitor list now..."
                    , ami->name, fmt::ptr(ami), ami->am ? ami->am->name : "null"); // spdlog
...
int HandlePowerRackEst(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* aV)
{
    ...
    if (!am || !am->vm)
    {
        if(debug)FPS_PRINT_ERROR("no am ({}) (or vm) for [{}] aname [{}]"
            , fmt::ptr(am), cstr{aV->getfName()}, aname); // spdlog
        return 0;
    }
    VarMapUtils* vm = am->vm;
    ...
    if (!ai)
    {
        FPS_PRINT_ERROR("no ai ({}) (or vm) for [{}] aname [{}]"
            , fmt::ptr(ai), cstr{aV->getfName()}, aname); // spdlog
        return 0;
    }

    if (amap.find("SBMUMaxDischargeCurrent") == amap.end())
    {
        FPS_PRINT_ERROR("no [{}] in  aname [{}]"
            , "SBMUMaxDischargeCurrent", aname ); // spdlog
        return 0;
    }
    if (amap.find("SBMUMaxChargeCurrent") == amap.end())
    {
        FPS_PRINT_ERROR("no [{}] in  aname [{}]"
            , "SBMUMaxChargeCurrent", aname ); // spdlog
        return 0;
    }
    ...
    {
        FPS_PRINT_ERROR("no [{}] in  aname [{}]"
            , "MBMUMaxChargeCurrent", aname ); // spdlog
        amap["MBMUMaxChargeCurrent"]    = vm->setLinkVal(vmap, ai->am->name.c_str(), "/status", "MBMUMaxChargeCurrent", dval);
        amap["MBMUMaxChargeVolts"]      = vm->setLinkVal(vmap, ai->am->name.c_str(), "/status", "MBMUMaxChargeVolts", dval);

        amap["MBMUMinChargeCurrent"]    = vm->setLinkVal(vmap, ai->am->name.c_str(), "/status", "MBMUMinChargeCurrent", dval);
        amap["MBMUMinChargeVolts"]      = vm->setLinkVal(vmap, ai->am->name.c_str(), "/status", "MBMUMinChargeVolts", dval);

        amap["MBMUMaxDischargeCurrent"] = vm->setLinkVal(vmap, ai->am->name.c_str(), "/status", "MBMUMaxDischargeCurrent", dval);
        amap["MBMUMaxDischargeVolts"]   = vm->setLinkVal(vmap, ai->am->name.c_str(), "/status", "MBMUMaxDischargeVolts", dval);

        amap["MBMUMinDischargeCurrent"] = vm->setLinkVal(vmap, ai->am->name.c_str(), "/status", "MBMUMinDischargeCurrent", dval);
        amap["MBMUMinDischargeVolts"]   = vm->setLinkVal(vmap, ai->am->name.c_str(), "/status", "MBMUMinDischargeVolts", dval);
    ...
    //double SBMUMinDischargeCurrent = amap["SBMUMinDischargeCurrent"]->getdVal();
    ...
    //double MBMUMinDischargeCurrent = amap["MBMUMinDischargeCurrent"]->getdVal();
    ...
    FPS_PRINT_INFO("MBMUMaxDischargeCurrent [{}] in  aname [{}] numSbmus {}"
            , amap["MBMUMaxDischargeCurrent"]->getdVal(), aname , MBMUNumSbmus); // spdlog
...
int HandlePowerEst(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* aV)
{
   ...
   //double ival = 0;
    asset_manager* am = aV->am;
     ...
     if(0)FPS_PRINT_INFO(" Running  am ({}) for [{}] aname [{}]"
            , fmt::ptr(am), cstr{aV->getfName()}, aname); // spdlog
    if (!am || !am->vm)
    {
        FPS_PRINT_ERROR("no am ({}) (or vm) for [{}] aname [{}]"
    ...
    {
        FPS_PRINT_ERROR("no am ({}) (or vm) for [{}] aname [{}]"
            , fmt::ptr(am), cstr{aV->getfName()}, "bms"); // spdlog
        return 0;
    }
    if(0)FPS_PRINT_ERROR(" Running  am ({}) for [{}] aname [{}]"
            , fmt::ptr(am), am->name, aname); // spdlog
    // get the maxcharge / disharge current and volts for each rack
    if (am->amap.find("MaxChargeCurrent") == am->amap.end())
    {
        FPS_PRINT_ERROR("no [{}] in  aname [{}]"
            , "MaxChargeCurrent", aname ); // spdlog
        am->amap["MaxChargeCurrent"]    = am->vm->setLinkVal(vmap, am->name.c_str(), "/status", "MaxChargeCurrent", dval);
        am->amap["MaxDischargeCurrent"] = am->vm->setLinkVal(vmap, am->name.c_str(), "/status", "MaxDischargeCurrent", dval);
        // amap["MBMUMaxChargeVolts"]      = am->vm->setLinkVal(vmap, am->name.c_str(), "/status", "MBMUMaxChargeVolts", dval);

        // amap["MBMUMinChargeCurrent"]    = am->vm->setLinkVal(vmap, am->name.c_str(), "/status", "MBMUMinChargeCurrent", dval);
        // amap["MBMUMinChargeVolts"]      = am->vm->setLinkVal(vmap, am->name.c_str(), "/status", "MBMUMinChargeVolts", dval);

        // amap["MBMUMaxDischargeCurrent"] = am->vm->setLinkVal(vmap, am->name.c_str(), "/status", "MBMUMaxDischargeCurrent", dval);
        // amap["MBMUMaxDischargeVolts"]   = am->vm->setLinkVal(vmap, am->name.c_str(), "/status", "MBMUMaxDischargeVolts", dval);

        // amap["MBMUMinDischargeCurrent"] = am->vm->setLinkVal(vmap, am->name.c_str(), "/status", "MBMUMinDischargeCurrent", dval);
        // amap["MBMUMinDischargeVolts"]   = am->vm->setLinkVal(vmap, am->name.c_str(), "/status", "MBMUMinDischargeVolts", dval);
        // amap["MBMUNumSbmus"] = am->vm->setLinkVal(vmap, am->name.c_str(), "/status", "MBMUNumsbmus", ival);
    }
    if (1)
    {   
        dval = 0.0;
        ...
        // {
        //     asset_manager* amc = ix.second;
        //     if (debug) FPS_PRINT_INFO("{} >> Got asset manager {}. Running HandlePowerEst now...\n"
        //              , __func__, amc->name.c_str());
        //     // Handle asset manager asset Power
        //     HandlePowerEst(*am->vmap, am->amap, amc->name.c_str(), am->p_fims, aV);
        ...
        //     {
        //         asset* ami = iy.second;
        //         if (debug) FPS_PRINT_INFO("{} >> Got asset instance {}. Running HandlePowerRackEst now...\n"
        //                 , __func__, ami->name.c_str());
        //         HandlePowerRackEst(*ami->vmap, ami->amap, ami->name.c_str(), ami->p_fims, aV);
        //     }
        // }
    }
    return 0;
...
int Every100mSP1(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* aV)
{
    ...
    //     {
    //         asset_manager* amc = ix.second;
    //         if (0) FPS_PRINT_INFO("{} >> Got asset manager {}. Running func now...\n"
    //                  , __func__, amc->name.c_str());
    //         // Run monitor list for asset manager
    //         am->vm->runMonitorList2(*am->vmap, am->amap, amc->name.c_str(), am->p_fims, "wake_monitor");

    //         // Run monitor list for asset instances
    //         for (auto& iy : amc->assetMap)
    //         {
    //             asset* ami = iy.second;
    //             if (0) FPS_PRINT_INFO("{} >> Got asset instance {}. Running monitor list now...\n"
    //                  , __func__, ami->name.c_str());
    //             am->vm->runMonitorList2(*am->vmap, am->amap, ami->name.c_str(), am->p_fims, "wake_monitor");
    //         }
    //     }
    // }
    ...
    // // am->vm->runMonitorList2(*am->vmap, am->amap, essName,am->p_fims, "wake_monitor");//am->p_fims,/*am->amap,*/ 0.1, 0.0);
    // // am->vm->runMonitorList2(*am->vmap, am->amap, "bms",am->p_fims, "wake_monitor");//am->p_fims,/*am->amap,*/ 0.1, 0.0);
    // // am->vm->runMonitorList2(*am->vmap, am->amap, "pcs",am->p_fims, "wake_monitor");//am->p_fims,/*am->amap,*/ 0.1, 0.0);
    // // am->vm->runMonitorList2(*am->vmap, am->amap, "site",am->p_fims, "wake_monitor");
    return 0;
...
int Every1000mS(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* av)
{
    ...
    if (am->amap["SendPcs"])
    {
        ...
        if (!runMe)
            FPS_PRINT_INFO(
                "MANAGER LOOP {} WAKE1000 sendPcs {} am2 {} [{}]"
                , am->name
                , sendPcs
                , fmt::ptr(am2)
                , am2->name
    ...
    if (am->run_secs == 30)
    {
        ...
        //char* fname = nullptr;
        auto fname = fmt::format("{}_{}_after_30_seconds.json"
    ...
    if (am->run_secs == 32)
    {
        ...
        auto fname = fmt::format("{}_{}_after_32_seconds.json"
            , am->vm->runCfgDir ? am->vm->runCfgDir : "run_configs"
        ...
        //if(0)
    {
        ...
        auto fname = fmt::format("{}_{}_after_31_seconds.json"
            , am->vm->runCfgDir ? am->vm->runCfgDir : "run_configs"
            , cstr{am->vm->getSysName(vmap)}
        );
        ...
        else
        {
            ...
            FPS_PRINT_INFO("Wrote {} bytes to file {}"
                , bytes_written, fname);
            fclose(fp);
        }
    }
...
int SlowPub(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* av)
{
    ...
    if (0) FPS_PRINT_INFO("aname [{}] am {}  publish status RunPub {} am->vm {}"
        , aname
        , fmt::ptr(am)
        , runPub
        , fmt::ptr(am ? am->vm : nullptr)
    ); // spdlog
    ...
    if (0) FPS_PRINT_INFO("aname [{}] >> publish status RunPub {} -> {}"
        , am->name
        , am->amap["RunPub"]
        , runPub
    ); // spdlog
...
int SlowPubOne(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* av)
{
    ...
    if (0)FPS_PRINT_INFO("aname [{}] >> RunPub {} -> {} idx {} size {}"
        , am->name
        , am->amap["RunPub"]
        , runPub
        , idx
    ...
    if (runPub && am->syscVec)
    {
        if (idx >= (int)am->syscVec->size())
        {
            ...
            av->setParam("endTime", 1.0);  // stops slowpubone running and removes the schedItem
            return 0;
        }
        if (idx == -2)
        {
        ...
        if (idx == -1)
        {
            ...
            if (0) FPS_PRINT_INFO("aname [{}]>>publish [{}]>>elapsed {:2.3f}mS"
                , am->name
                , "/site/ess_ls"
                , (tThen - tNow) * 1000.0
            ); // spdlog
        ...
        else
        {
            ...
            // Jimmy - A hack right now. Don't really want to pub to /assets/site_controller/summary
            // at the moment since that is not in the config file (site_controller_manager.json)
            std::string sv = svp;
            if (sv != "site/summary")
            {
...
int FastPub(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* av)
{
    ...
    {
        runPub = am->amap["RunPub"]->getbVal();
        if (debug)FPS_PRINT_INFO("aname [{}] >> publish status RunPub {} -> {}"
            , am->name
            , am->amap["RunPub"]
            , runPub
        ); // spdlog
...
{
    auto tmp = 
          fmt::format("             git_branch: {}", GITBRANCH);
    printf("%s \n",tmp.c_str());
