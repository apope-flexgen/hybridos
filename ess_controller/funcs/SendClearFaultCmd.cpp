...
/**
 * @brief Sends a clear fault command to hardware units (ex.: CATL BMS) that contains
 * the clear fault register
 * 
...
int SendClearFaultCmd(varsmap &vmap, varmap &amap, const char* aname, fims* p_fims, assetVar* av)
{
    if (0) FPS_ERROR_PRINT("%s >> Incoming amap [%p] aname [%s]  Sending to assetVar [%s]\n"
                            , __func__
                            , &amap
                            , aname
                            , av->name.c_str()
    ...
    if (0) FPS_ERROR_PRINT("%s >> Corrected am [%p] amap %p aname [%s]\n"
            , __func__
            , am
            , &amap
            , am->name.c_str()
    ...
    if (reload < 2)
    {
        ...
        if (1) FPS_ERROR_PRINT("%s >>> %s Running reload 1 >> gpioFault [%d]\n", __func__, schedName, amap["gpioFault"]->getbVal());
    ...
    if (amap["gpioFault"]->getbVal())
    {
        ...
        if (amap["KeyCmd"]->getbVal()) amap["KeyCmd"]->setVal(false);
        ...
        if (temp_av) temp_av->sendEvent("ESS", am->p_fims, Severity::Info, "GPIO Fault seen at time %2.3f"
            , now.count()
            );
        vm->setVal(vmap, amap["SystemFaultCleared"]->comp.c_str(), amap["SystemFaultCleared"]->name.c_str(), bval);
    ...
    else if (!amap["SystemFault"]->getbVal())
    {
        if (amap["SystemFault"]->getbParam("Clearing"))
        {
            ...
            // Send Event - info
            const auto now = flex::get_time_dbl();
            ESSLogger::get().info("[{}] [Fault cleared]",
                aname);
            auto temp_av = amap["KeyCmd"];
            if (temp_av) temp_av->sendEvent("ESS", am->p_fims, Severity::Info, "Fault cleared at time %2.3f"
                , now.count()
                );
        }
        vm->setVal(vmap, amap["SystemFaultCleared"]->comp.c_str(), amap["SystemFaultCleared"]->name.c_str(), bval);
    ...
    else if (amap["KeyCmd"]->getbParam("KeyCmdDone"))
    {
        ...
        auto temp_av = amap["KeyCmd"];
        if (temp_av) temp_av->sendEvent("ESS", am->p_fims, Severity::Alarm, "Fault not clearing at time %2.3f"
            , now.count()
            );
        vm->setVal(vmap, amap["SystemFaultCleared"]->comp.c_str(), amap["SystemFaultCleared"]->name.c_str(), bval);
        vm->schedStop(vmap, amap, schedName, 0.0001);
    ...
    {
        RunKeyCmd(vmap, amap, aname, nullptr, av);
        if (!amap["SystemFault"]->getbParam("Clearing"))  amap["SystemFault"]->setParam("Clearing", true);
    }
