...
int HandleAssetHeartbeat(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, asset_manager* am)
{
    ...
    if (reload < 2)
    {
        ...
        // Jimmy - We probably don't need to set ess TOD here since that's already done in UpdateSysTime
        ...
        // Jimmy - We probably don't need to set asset TOD here since that's already done in UpdateSysTime
    ...
    if (0) FPS_ERROR_PRINT("%s >> Heartbeat value before %f\n", __func__, HBLast);
    if (0) FPS_ERROR_PRINT("%s >> Heartbeat value now %f\n", __func__, HBnow);
