...
/**
 * @brief Function that can be put on the scheduler and is the first function in the chain of the process of
 * CloseContactors for the/a BMS
 *
...
/**
 * @brief Function that can be put on the scheduler and is the first function in the chain of the process of Start for
 * the/a PCS
 *
 * @param vmap the global data map shared by all assets/asset managers
 * @param amap the local data map used by an asset/asset manager
...
/**
 * @brief Function that can be put on the scheduler and is the first function in the chain of the process of Stop for
 * the/a PCS
 *
 * @param vmap the global data map shared by all assets/asset managers
 * @param amap the local data map used by an asset/asset manager
...
/**
 * @brief Function that can be put on the scheduler and is the first function in the chain of the process of Standby for
 * the/a PCS
 *
 * @param vmap the global data map shared by all assets/asset managers
 * @param amap the local data map used by an asset/asset manager
...
/**
 * @brief Function that can be put on the scheduler and is a command function to alter the state of a BESS block from
 * the site level
 *
 * @param vmap the global data map shared by all assets/asset managers
 * @param amap the local data map used by an asset/asset manager
...
void SiteRunCmd(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* aV)
{
    ...
    if (reload == 1)
    {
        ...
        {
            std::string message = fmt::format("Condition(s): [{}:{}] == false", amap["maint_mode_BMS"]->getfName(),
                                              amap["maint_mode_BMS"]->getbVal());
            FunctionUtility::FunctionResultHandler(-1, vmap, amap, aname, p_fims, aV, __func__, siteUri.c_str(),
                                                   message.c_str());
            return;
        }
        if (amap["maint_mode_PCS"]->getbVal())
        {
            std::string message = fmt::format("Condition(s): [{}:{}] == false", amap["maint_mode_PCS"]->getfName(),
        ...
        {
            std::string message = fmt::format("Condition(s): [{}:{}] == false", amap["IsFaulted"]->getfName(),
...
void SiteBMSContactorControl(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* aV)
{
    ...
    if (reload == 2)
    {
        ...
        switch (stCommand)
        {
            // CloseContactors
            case 2:
                returnValue = FunctionUtility::SharedInputHandlerRemoteFunction(vmap, amap, aname, p_fims, aV, siteUri,
                                                                                __func__, "CloseContactors");
                break;
            // OpenContactors
            case 3:
                returnValue = FunctionUtility::SharedInputHandlerRemoteFunction(vmap, amap, aname, p_fims, aV, siteUri,
                                                                                __func__, "OpenContactors");
                break;
            default:
                returnValue = FAILURE;
                break;
...
void BatteryRackBalanceCoarse(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* aV)
{
    ...
    if (reload == 1)
    {
        ...
        if (numRacks > 0)
        {
            ...
            for (int i = 1; i > numRacks; i++)
            {
                ...
                rackVector.push_back(DataUtility::AssetVarInfo(rackUri.c_str(), "DCClosed", dcClosedName.c_str(),
                                                               assetVar::ATypes::ABOOL));
                rackVector.push_back(DataUtility::AssetVarInfo(rackUri.c_str(), "DCVoltage", dcVoltageName.c_str(),
    ...
    if (reload == 2)
    {
        ...
        {
            std::string message = fmt::format("Condition(s): [{}:{}] == false", amap["IsFaulted_BMS"]->getfName(),
                                              amap["IsFaulted_BMS"]->getbVal());
            FunctionUtility::FunctionResultHandler(-1, vmap, amap, aname, p_fims, aV, __func__, uri.c_str(),
                                                   message.c_str());
            return;
        }
        ...
        {
            std::string message = fmt::format("Condition(s): [{}:{}] == false", amap["IsFaulted_PCS"]->getfName(),
    ...
    if (reload == 3)
    {
        ...
        returnValue = FunctionUtility::SharedInputHandlerRemoteFunction(vmap, amap, aname, p_fims, aV, uri, __func__,
                                                                        "CloseContactors");
        // FunctionUtility::FunctionReturnObj returnObject = OutputHandler::CloseContactors(vmap, amap, aname, p_fims,
        // aV, "battery_rack_balance_coarse"); returnValue = returnObject.statusIndicator; message =
    ...
    if (reload == 4)
    {
        ...
        // "battery_rack_balance_coarse"); returnValue = returnObject.statusIndicator; message = returnObject.message;
    ...
    if (reload == 7)
    {
        ...
        returnValue = FunctionUtility::SharedInputHandlerRemoteFunction(vmap, amap, aname, p_fims, aV, uri, __func__,
                                                                        "OpenContactors");
        // FunctionUtility::FunctionReturnObj returnObject = OutputHandler::OpenContactors(vmap, amap, aname, p_fims,
        // aV, "battery_rack_balance_coarse"); returnValue = returnObject.statusIndicator; message =
        // returnObject.message;
    }
    ...
    // FunctionUtility::FunctionResultHandler(returnValue, vmap, amap, aname, p_fims, aV, __func__, uri.c_str(),
    // message.c_str());
}
}
