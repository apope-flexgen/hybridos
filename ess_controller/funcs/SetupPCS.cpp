...
int SetupPCS(varsmap& vmap, varmap& amap, const char* _aname, fims* p_fims, assetVar* aV)
{
    ...
    // Create vector and populate it with all filenames within given path from
    // above
    ...
    // Create vectors for each of the types of stdcfg pcs files. v_pcs_manager,
    // pcs_manager, pcs_module
    ...
    // for loop to cycle through all of the filenames and get their respective
    // file contents then convert them to strings  after that it pushes the data
    // to the correct vector dependent on what type of file it is
    for (int i = 0; i < fileNamesVectorSize; i++)
    {
        ...
        // this section should take the path + filename and end up giving the
        // contents string in "currentJsonString"
    ...
    // This section converts all the config contents vectors into arrays of the
    // same size
    ...
    for (int a = 1; a <= num_pcs; a++)
    {
        ...
        for (int c = 1; c <= num_modules; c++)
        {
            ...
            for (int d = 0; d < pcsModulesSize; d++)
            {
                ...
                currPcsModuleString = ReplaceString(currPcsModuleString, std::string("##MODULE_ID##"),
                                                    std::string(moduleId));
