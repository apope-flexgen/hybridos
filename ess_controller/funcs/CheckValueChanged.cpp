...
/**
 * @brief Write a value to a variable say /limits/pcs:CheckChanged this function
 * will then process all the other variables in that table to see if any values
 * have changed if any have changed then set the value of
 * /limits/pcs:CheckChanged to true , else st its value to false. The
 * CheckChanged variable will not be included in the check list and its
 * type will be coerced to bool.
 *
 *
...
int CheckValueChanged(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* av, bool noreset)
{
    ...
    if (debug)
        FPS_PRINT_INFO("checking  aname [{}] with assetVar [{}], noreset [{}]", aname, av->getfName(), noreset);
    bool changed = false;

    for (auto xx : vmap[av->comp])
    {
        if (!xx.second || (xx.second == av))
            continue;
        bool xy = false;
        if (noreset)
        {
        ...
        if (debug)
            FPS_PRINT_INFO("checking  av [{}] changed [{}] with assetVar [{}]", xx.second->getfName(), xy,
                           av->getfName());
