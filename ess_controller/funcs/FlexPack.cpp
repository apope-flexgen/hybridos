...
// fims_send -m set -r /$$ -u /gpio/full/sched/gpio
...
// int SimHandlePcs(varsmap &vmap, varmap &amap, const char* aname, fims*
// p_fims, assetVar* aV); int SimHandleBms(varsmap &vmap, varmap &amap, const
...
//    int SimHandleRizenBms(varsmap &vmap, varmap &amap, const char* aname,
//    fims* p_fims, assetVar* aV);
// int SimHandleRackVolts(varsmap& vmap, varmap& amap, const char* aname, fims*
// p_fims, assetVar* aV);  int SimHandleCellCurrent(varsmap& vmap, varmap& amap,
// const char* aname, fims* p_fims, assetVar* aV);  int SimProcessCells(varsmap&
// vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* aV);
...
//    int SimBmsManageContactor(varsmap& vmap, varmap& amap, const char* aname,
//    fims* p_fims, assetVar* aV);
// int SimBmsOpenContactor(varsmap& vmap, varmap& amap, const char* aname, fims*
// p_fims, assetVar* aV);
//    int SimBmsStartConnection(varsmap& vmap, varmap& amap, const char* aname,
//    fims* p_fims, assetVar* aV);

int SetMapping(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* av);

// int LinkVar(varsmap &vmap, varmap &amap, const char* aname, fims* p_fims,
// assetVar* aV);
...
// if(av->extras && av->extras && av->extras->optVec &&
// av->extras->optVec->cjopts)
// {
//         FPS_PRINT_INFO("@@@@@ found cjopts ready to run vlink options"); //
//         spdlog cJSON *cji; int idx  = -1; cJSON_ArrayForEach(cji,
//         av->extras->optVec->cjopts)
//         {
...
void VarMapUtils::setVLinkFromCj(varsmap& vmap, cJSON* cji, assetVar* avl, assetVar* aV)
{
    ...
    if (1)
        FPS_PRINT_INFO("aV [{}] vname [{}] lname [{}] ", aV->getfName(), vname ? vname : "no vname",
...
// int SetMapping(varsmap &vmap, varmap &amap, const char* aname, fims* p_fims,
// assetVar* aV)
...
int RunAVLink(varsmap& vmap, varmap& amap, VarMapUtils* vm, std::string& lname, assetVar* aV)
{
    ...
    if (debug)
        FPS_PRINT_INFO("aV [{}] lname [{}]", aV->getfName(), lname);
    ...
    if (vx != vmap.end())
    {
        for (auto xx : vmap[lname])
        {
            ...
            if (debug)
                FPS_PRINT_INFO("idx [{:0d}] aname [{}]  av [{}]", idx, xx.first, av->getfName());
            const char* vname = av->getcVal();
            if (!vname)
            {
                FPS_PRINT_INFO("idx [{:0d}]  ERROR av [{}] no vname, skipping", idx, av->getfName());
                idx++;
                continue;
            }
            bool setam = false;
            ...
            if (debug)
                FPS_PRINT_INFO("idx [{:0d}]  av [{}]  vname [{}] setam [{}]", idx, av->getfName(), vname, setam);
            ...
            {
                if (debug)
                    FPS_PRINT_INFO("idx [{:0d}]  av [{}]  vname [{}]  set default avv ", idx, av->getfName(), vname);
                if (av->gotParam("default"))
                {
                    // new function to set a value from a param
                ...
                // avv  = vm->setDefault(vmap,vname,nullptr, ltype?ltype:(const char
                // *)"double");
            }
            ...
            {
                if (debug)
                    FPS_PRINT_INFO("idx [{:0d}]  av [{}]  vname [{}]  set default avl ", idx, av->getfName(), lvar);
                if (av->gotParam("default"))
                {
                    // new function to set a value from a param
            ...
            {
                if (debug)
                    FPS_PRINT_INFO("idx [{:0d}]  ERROR >> [{}:{}] avv [{} @{}] avl [{} @{}]\n", idx, lname, xx.first,
                                   vname, fmt::ptr(avv), lvar ? lvar : "no lVar", fmt::ptr(avl));
            }
            if (avv && ai && setam)
            {
                ai->amap[av->comp] = avv;
            ...
            if (avv && avl)
            {
                // Set the link.
                if (debug)
                    FPS_PRINT_INFO("idx [{:0d}]  av [{}]  avl [{}]  set vlink ", idx, av->getfName(), avl->getfName());

                avv->linkVar = avl;
            }
            if (av->extras && av->extras && av->extras->optVec && av->extras->optVec->cjopts)
            {
                FPS_PRINT_INFO("@@@@@ [{}] found cjopts ready to run vlink options vm [{}]", av->getfName(),
                               fmt::ptr(vm));
                cJSON* cji;
                int iix = 0;
                ...
                {
                    if (vm)
                        vm->setVLinkFromCj(vmap, cji, avl, av);
                    iix++;
                }
                if (0)
                    FPS_PRINT_INFO("@@@@@ [{}] found cjopts ran vlink options idx [{}].[{}]", av->getfName(), idx, iix);
            }
            idx++;
        }
    }
...
int RunALink(varsmap& vmap, varmap& amap, VarMapUtils* vm, std::string& aname, std::string& lname, assetVar* aV)
{
    ...
    if (vx != vmap.end())
    {
        for (auto xx : vmap[lname])
        {
            ...
            const char* vname = av->getcVal();
            if (debug)
                FPS_PRINT_INFO(" 0==> xx [{}] found av [{}]  vname [{}]", xx.first, av->getfName(),
            ...
            if (am)
            {
                ...
                if (debug)
                    FPS_PRINT_INFO(" 1==> xx [{}] found av [{}]  vname [{}] from am ", xx.first, av->getfName(),
                                   vname ? vname : "No Vname");
            }
            if (ai)
            {
                ...
                if (debug)
                    FPS_PRINT_INFO(" 1==> xx [{}] found av [{}]  vname [{}] from ai ", xx.first, av->getfName(),
                                   vname ? vname : "No Vname");
            }
            // FPS_PRINT_INFO(" 2==> xx [{}] found av [{}]  vname [{}]", xx.first,
            // av->getfName(), vname?vname:"No Vname");
            //         if(!avv || !avl)
            //         {
            if (debug)
                FPS_PRINT_INFO(
                    "idx [{:0d}]  NOTE >> amap [{}] vname [{}] lname [{}] "
                    "avv [{}] avl [{}]",
                    idx, xx.first, vname ? vname : "no vname", lname ? lname : "no lname",
                    avv ? avv->getfName() : "no Avv found", avl ? avl->getfName() : "no Avl found");
            //         }
            // first case no amap setup but we have an av
            // "/components/bms_controls:start_connection"
            ...
            if (avv && !avl)
            {
                ...
                // second case no amap setup no av
                // "/components/bms_controls:start_connection" may have a default value
                //
            }
            else if (!avv && !avl)
            {
...
// int RunALink(varsmap &vmap, varmap &amap, VarMapUtils*vm, std::string&aname,
// std::string&lname/*, char* ltype*/)
// {
//     // //the amap is the key here /link/pcs
//     // // means use the pcs amap
//     // // if we
...
//     if(vx == vmap.end())
//     {
//         FPS_PRINT_ERROR(" MISSING lname : aname [{}] lname [{}]", aname,
//         lname);
//     }
//     if(vx != vmap.end())
//     {
...
//             if(!av)
//             {
//                 FPS_PRINT_ERROR(" MISSING av : xx [{}]  aname [{}] lname
//                 [{}]", xx.first, aname, lname);
//                 // we can make the var here
//                 char *ltype =  nullptr;//av->getcParam("ltype");
//                 av  = vm->setDefault(vmap,xx.first.c_str(),nullptr,
//                 ltype?ltype:(const char*)"double");
//                 if(1/*||debug*/)FPS_PRINT_INFO("idx [{:0d}]  av [{}]   set
//                 default avl "
//                     , idx, av->getfName()
//                     );
//                 //continue;
...
//     //         if(!avv || !avl)
//     //         {
//             FPS_PRINT_INFO("idx [{:0d}]  NOTE >> amap [{}] vname [{}] lname
//             [{}] avv [{}] avl [{}]"
//                      , idx
//                      , xx.first
//                      , vname ? vname:"no Vname"
    ...
    if (vx != vmap.end())
    {
        for (auto xx : vmap[lname])
        {
            ...
            if (!av)
            {
                ...
                if (1 /*||debug*/)
                    FPS_PRINT_INFO("idx [{:0d}]  av [{}]   set default avl ", idx, av->getfName());
                // continue;
            }
            const char* vname = av->getcVal();
            const char* lname = nullptr;
            ...
            //         {
            FPS_PRINT_INFO(
                "idx [{:0d}]  NOTE >> amap [{}] vname [{}] lname [{}] avv "
                "[{}] avl [{}]",
                idx, xx.first, vname ? vname : "no Vname", lname ? lname : "no Lname",
                avv ? avv->getfName() : "no Avv found", avl ? avl->getfName() : "no Avl found");
            //         }
...
int RunAllALists(varsmap& vmap, varmap& amap, const char* _aname, fims* p_fims, assetVar* aV)
{
    ...
    FPS_PRINT_INFO(" func running ", NULL);
    ...
    if (vmap.find(vkey) != vmap.end())
    {
        ...
        for (auto xx : al)
        {
            ...
            // vm->getVar(vmap, "_assetList", xx.first.c_str());//vm->setAlist(vmap,
            // xx.first.c_str());
            assetVar* av = xx.second;
            ...
            if (av && av->gotParam("name"))
            {
                ...
                if (av->extras && av->extras->optVec)
                {
                    ...
                    {
                        if (name)
                            aLv->setParam("name", name);
                        if (mode)
                            aLv->setParam("mode", mode);
                    }
                }
            }
            ...
            FPS_PRINT_INFO("found idx [{:02d}] >> alist [{}]  name [{}] cjopts [{}]", idx, xx.first,
                           name ? name : "No Name Param", fmt::ptr(cjopts));
            if (cjopts)
            {
                int idy = 0;
                cJSON* cji;
                ...
                cJSON_ArrayForEach(cji, cjopts)
                {
                    ...
                    if (cjaV && cjaV->valuestring)
                    {
                        ...
                        if (aaV)
                            alist->add(aaV);
                        FPS_PRINT_INFO("found  [{:02d}] >> [{:02d}] xy [{}] aaV [{}]", idx, idy, cjaV->valuestring,
                                       fmt::ptr(aaV));
                        idy++;
                    }
                }
...
int RunAllLinks(varsmap& vmap, varmap& amap, const char* _aname, fims* p_fims, assetVar* aV)
{
    ...
    for (auto xx : linkVec)
    {
        ...
        {
            FPS_PRINT_INFO(" processing idx [{:02d}]  >> xx->aname :[{}]->[{}] parent [{}]", idx, xx, aname,
                           aV->am->name);
            ...
            if (!am && !ai)
            {
                ...
                FPS_PRINT_INFO(" creating assetManager   [{}]", aname);
...
int RunSysVec(varsmap& vmap, varmap& amap, const char* _aname, fims* p_fims, assetVar* aV)
{
    ...
    if (xx == vmap.end())
    {
        FPS_PRINT_INFO(" Unable to find [config/load]", NULL);
        return 0;
    }
    ...
    for (const auto& xy : cl)
    {
        ...
        FPS_PRINT_INFO(" idx [{:02d}] name [{}]", idx, xy.first);
    ...
    if (avm)
    {
        FPS_PRINT_INFO(" idx [{:02d}] avm-> [{}]", idx, avm->getfName());
...
//                          "help": "Set up the amname, ainame, pname and amap
//                          for a varable ",
//                     "ifChanged":false,"enabled":true,
//                     "actions":{"onSet":[{"func":[{"func":"SetMapping"}]}]}}}'
// TODO add amap etc
...
bool loadClientMap(varsmap& vmap, VarMapUtils* vm,
                   const char* cfgname)  // cJSON* cjss)
{
    bool ret = false;
    assetVar* av;
    cJSON* cjss = nullptr;
    ...
    {
        if (1)
            FPS_PRINT_INFO("found components", NULL);
        cJSON* cjcomps = cjregs->child;
        while (cjcomps)
        {
            // FPS_PRINT_INFO(" cjregs->child {} ->child {}", fmt::ptr(cji),
            // fmt::ptr(cji->child));
            cJSON* cjmid = cJSON_GetObjectItem(cjcomps, "id");
            if (cjmid)
            {
                if (0)
                    FPS_PRINT_INFO("found reg id [{}]", cjmid->valuestring);
            ...
            if (cjreg)
            {
                // ret = true;
                if (0)
                    FPS_PRINT_INFO("found registers type {} child {} next {}", cjreg->type, fmt::ptr(cjreg->child),
                                   fmt::ptr(cjreg->next));
                ...
                if (cjmap)
                {
                    // ret = true;
                    if (0)
                        FPS_PRINT_INFO("found map type {} child type {} child next{}", cjmap->type, cjmap->child->type,
                                       fmt::ptr(cjmap->child->next));
                    ...
                    while (cjim)
                    {
                        ...
                        {
                            if (0)
                                FPS_PRINT_INFO("found map id [{}]", cjmid->valuestring);
                            ...
                            if (cjmnam)
                            {
                                ...
                                if (0)
                                    FPS_PRINT_INFO("found map name [{}]", cjmnam->valuestring);
                            }
...
bool loadSiteMap(varsmap& vmap, VarMapUtils* vm,
                 const char* cfgname)  // cJSON* cjss)
{
    bool ret = false;
    // assetVar*av;
...
bool loadSiteMapCj(varsmap& vmap, VarMapUtils* vm, cJSON* cjss)  // cJSON* cjss)
{
    ...
    if (cjregs)
    {
        FPS_PRINT_INFO("found registers", NULL);
        cji = cjregs->child;
        while (cji)
        {
            FPS_PRINT_INFO("found register item [{}]", cji->string);
        ...
        if (cjinput)
        {
            ret = true;
            if (0)
                FPS_PRINT_INFO("found input registers type {} child {}", cjinput->type, fmt::ptr(cjinput->child));
            cji = cjinput->child;
            while (cji)
            {
                // char*tmp = nullptr;
                ...
                {
                    if (0)
                        FPS_PRINT_INFO("found input register id [{}] uri [{}]", cjid->valuestring, cjuri->valuestring);
                    int ival = 0;
        ...
        if (cjhold)
        {
            ret = true;
            if (0)
                FPS_PRINT_INFO("found holding registers type {} child {}", cjinput->type, fmt::ptr(cjinput->child));
            cji = cjhold->child;
            while (cji)
            {
                // char*tmp = nullptr;
                ...
                {
                    if (1)
                        FPS_PRINT_INFO("found holding register id [{}] uri [{}]", cjid->valuestring,
                                       cjuri->valuestring);
                    int ival = 0;
                    vm->setVal(vmap, cjuri->valuestring, cjid->valuestring, ival);
                }
...
int RunTpl(varsmap& vmap, varmap& amap, const char* xaname, fims* p_fims, assetVar* aV)
{
    ...
    // __func__, (int)syscpVec->size()); now get the
    // asset_manager to configure itsself syscpVec needs to be solved
...
// fims_send -m set -r /$$ -u /flex/full/system/commands/loadCfg '
//              {"value":"someval","aname":"bms", "pname":essName,
//              "config":"bms_manager.json"}'
...
//                     "ifChanged":false,"enabled":true,
//                     "actions":{"onSet":[{"func":[{"func":"LoadConfig"}]}]}}}'
...
int DumpConfig(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* aV)
{
    ...
    if (aV->gotParam("outFile"))
    {
        ...
        auto fname = fmt::format("{}{}_{}_dump.json", am->vm->runCfgDir ? am->vm->runCfgDir : "run_configs",
                                 am->vm->getSysName(vmap), outFile);
        ...
        FPS_ERROR_FMT("{} >> dump config  av [{}] table [{}]  file [{}] len [{}]\n", __func__, aV->getfName(),
                      table ? table : "none", outFile, vmlen);
    }
    return 0;
}
...
//         }
//    }
// linkVals(*vm, vmap, amap, aname, "/controls", dval, "ActivePowerSetpoint",
// "ReactivePowerSetpoint");  amap["essPcsStatusFaults"] = vm->setLinkVal(vmap,
// essName, "/status", "essPcsStatusFaults", ival);
//    av = makeVar(vmap, comp, var, defvalue);
//    "var":"/debug/bms:test1"
//    "linkName":"/status/bms:MaxValue"
//    "vlinkName":"/status/bms:MaxValue"
// makes a var /debug/bms:test1 ...vmap["/debug/bms"]["test1"] = getVar(vmap,
// "/status:bms:MaxValue");
...
int MakeLink(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* aV)
{
    ...
    if (linkName)
    {
        ...
        if (!lVar && cjVal)
        {
            // lVar = vm->makeVar(vmap, linkName, nullptr);  // Note makeVar did not
            // work.....
    ...
    if (lVar)
    {
        ...
        //   __func__, varName, linkName); FPS_ERROR_FMT("{} >>
        //   Flex >>    aVar [{}] : [{}] \n"
        //         ,__func__, aVar->comp.c_str(),
        //         aVar->name.c_str());
        //   FPS_ERROR_FMT("{} >> Flex >>    lVar [{}] : [{}] \n"
        //         ,__func__, lVar->comp.c_str(),
        //         lVar->name.c_str());
    ...
    if (am)
    {
        ...
        // if(ai)
        // {
        //     ai->amap[varName] = vm->setLinkVal(vmap, ai->name.c_str(), sysName,
        //     varName, cjVal);
        // }
    }
    FPS_ERROR_FMT("{} >> Flex >>Setting link for [{}] to [{}] \n", __func__, varName, linkName);
    // //aV->am->vm->setLinkVal(vmap, flexName, baseName, varName, cjVal);
    // FPS_ERROR_FMT("{} >> Flex >>Done Setting
...
{
    if (0)
        FPS_ERROR_FMT("{} >> running  aV [{}] aname [{}] aV->am {}\n", __func__, aV->getfName(), aname,
                      fmt::ptr(aV->am));
    asset_manager* am = aV->am;
    int debug = 1;
    ...
    if (0)
        FPS_ERROR_FMT("{} >> ##2 running  aV [{}] aname [{}]\n", __func__, aV->getfName(), aname);
    ...
    char* uri = aV->getcParam("uri");
    if (0)
        FPS_ERROR_FMT("{} >> ##2a running  aV [{}] aname [{}] uri [{}]  am [{}]\n", __func__, aV->getfName(), aname,
                      uri, fmt::ptr(aV->am));

    double tNow = aV->am->vm->get_time_dbl();
    if (0)
        FPS_ERROR_FMT("{} >> ##2b running  aV [{}] aname [{}] uri [{}]  am [{}]\n", __func__, aV->getfName(), aname,
                      uri, fmt::ptr(aV->am->vm));

    assetVar* av = aV->am->vm->setVal(vmap, uri, nullptr, tNow);
    if (0)
        FPS_ERROR_FMT("{} >> ##3 running  aV [{}] uri [{}]\n", __func__, aV->getfName(), uri);
    if (!av)
    {
        FPS_ERROR_FMT("{} >> error setVal aV [{}] uri[{}]\n", __func__, aV->getfName(), uri);
        return 0;
    ...
    // if our target av does not have an am, set it to our current am (this
    // prevents vmap crashes)
    if (!av->am)
        av->am = am;
    ...
    if (0)
        FPS_ERROR_FMT("{} >> before HandleSchedLoad with av [{}] uri [{}]\n", __func__, av->getfName(), uri);
    int rc = HandleSchedLoad(vmap, amap, aname, p_fims, av);
    if (0)
        FPS_ERROR_FMT("{} >> after HandleSchedLoad with av [{}] uri [{}] rc {}\n", __func__, av->getfName(), uri, rc);
    return rc;
}
...
int RunMonitor(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* aV)
{
    ...
    // TODO use flexname
    if (0)
        FPS_ERROR_FMT("{} ##############>>  [{}] running aname [{}] monitor [{}]\n", __func__, aV->getfName(), amname,
                      monitor);
