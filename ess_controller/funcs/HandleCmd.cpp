...
/**
 * @brief Determines if the system command has executed by checking whether the value
 * has been updated
 * 
...
void checkCmd(varsmap& vmap, varmap& amap, const char* aname, assetVar* av)
{
    ...
    if (retrievedAv)
    {
        FPS_PRINT_DEBUG("We have an assetVar [{}:{}] in the cmdVar param in [{}:{}]", retrievedAv->comp, retrievedAv->name, av->comp, av->name);
        cmdAv = retrievedAv;
    }
    ...
    if (currCheckCmdHoldTime <= 0)
    {
        FPS_PRINT_DEBUG("Check command hold time reached 0. Now check if command value has been updated for asset [{}] with [{}:{}]"
        ...
        {
            // Wait until check command wait time reaches 0 before determining that the system command did not update successfully
            ...
            if (currCheckCmdTime <= 0)
            {
                ...
                FPS_PRINT_INFO("currCheckCmdTime reached 0 and currCmdTries is now [{}] for asset [{}] with [{}:{}]. Remaining number of command tries [{}]"
                ...
                if (remainCmdTries <= 0)
                {
                    ...
                    if (enableAlert)
                    {
                        const std::string msg = fmt::format("Reached max command tries [{}]. Cannot verify that command value [{}] has been sent to [{}:{}]"
                                                            , maxCmdTries, av->getdVal(), cmdAv->comp, cmdAv->name);
                        ESSUtils::record(vmap, amap, aname, av, msg, tNow, "ResponseFailure", Severity::Alarm);
                    }

                    // Set command result assetVar to false, indicating that the system command ran unsuccessfully
                ...
                // Reset hold and check times and increment current # of command tries . At this point, we're done checking the command
                ESSUtils::incrementTime(av, currCheckCmdHoldTime, checkCmdHoldTimeout, checkCmdHoldTimeout, "currCheckCmdHoldTime");
        ...
        {
            // Set command result assetVar to true, indicating that the system command ran successfully
            const std::string cmdSuccessAvName = fmt::format("{}Success", av->name);
            ESSUtils::setAvStatus(vmap, amap, aname, av, cmdSuccessAvName, true, cmdSuccessAvName, false);

            // Reset hold and check times and reset current # of command tries back to 0. At this point, we're done checking the command
            ESSUtils::incrementTime(av, currCheckCmdHoldTime, checkCmdHoldTimeout, checkCmdHoldTimeout, "currCheckCmdHoldTime");
            ESSUtils::incrementTime(av, currCheckCmdTime, checkCmdTimeout, checkCmdTimeout, "currCheckCmdTime");
            ...
            if (enableAlert)
            {
                const std::string msg = fmt::format("Received command value [{}] from [{}:{}]", cmdAv->getdVal(), cmdAv->comp, cmdAv->name);
                ESSUtils::record(vmap, amap, aname, av, msg, tNow, "ResponseSuccess", Severity::Info);
            }
        }
    }

    // Check if we have finished checking the command successfully or unsuccessfully
    if (checkCmdDone && av->gotParam("cmdSent"))
    {
        FPS_PRINT_DEBUG("Command check is done for [{}:{}]. Setting cmdSent to false and triggerCmd to {}", cmdAv->comp, cmdAv->name, triggerCmd);
...
void sendCmd(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* av)
{
    ...
    if (currSendCmdHoldTime <= 0)
    {
        FPS_PRINT_DEBUG("Send command hold time reached 0. Now attempt to send command to asset [{}] with [{}:{}]"
        ...
        if (av->gotParam("useExpr") && av->getbParam("useExpr"))
        {
            ...
            if (sendCmdOk)
                ...
                FPS_PRINT_DEBUG("Condition is not satisfied or valid for asset [{}] with [{}:{}]", cstr{aname}, av->comp, av->name);
        }

        // Send command
        // If conditions are defined, then only send command if all conditions are satisfied
        if (!sendCmdOk)
        {
            // Wait until send command wait time reaches 0 before determining that we cannot send a command
            ...
            {                
                ...
                if (enableAlert)
                {
                    std::string msg = fmt::format("Cannot send command value [{}] to [{}:{}]"
                                                    , av->getdVal(), av->comp, av->name);

                    assetVar* retrievedAv = ESSUtils::getAvFromParam(vmap, amap, av, "cmdVar");
                    if (retrievedAv)
                        msg = fmt::format("Cannot send command value [{}] to [{}:{}]"
                                                    , av->getdVal(), retrievedAv->comp, retrievedAv->name);

                    // If condition(s) are defined, include all conditions in the message for reporting
                    if (av->gotParam("useExpr") && av->getbParam("useExpr"))
                    {
                ...
                ESSUtils::incrementTime(av, currSendCmdHoldTime, sendCmdHoldTimeout, sendCmdHoldTimeout, "currSendCmdHoldTime");
        ...
        else
        {
            ...
            if (enableAlert)
            {
                const std::string msg = fmt::format("Sent command value [{}] to [{}:{}]", cmdAv->getdVal(), cmdAv->comp, cmdAv->name);
...
void setupCmdParams(varsmap& vmap, varmap& amap, const char* aname, assetVar* av)
{
    ...
    // Amount of times to send the command before determining the command did not send successfully
    if (!av->gotParam("maxCmdTries"))
        av->setParam("maxCmdTries", 0);
    if (!av->gotParam("currCmdTries"))
        av->setParam("currCmdTries", 0);

    // Amount of time to wait before sending system command - default to 0, indicating immediate send
    ...
    // Amount of time to wait before checking system command - default to 0, indicating immediate check
    ...
    // Amount of time to wait before determing that we cannot send the system command
    ...
    // Amount of time to wait before determing that running the system command was unsuccessful
    ...
    // Check if the assetVar has a cmdVar param. This param is used for sending a value
    ...
    if (av->gotParam("cmdVar") && av->getcParam("cmdVar"))
    {
        ...
        if (!avUri.empty())
        {
            ...
            // Otherwise, we may have a full uri and a variable name. Something like this: /component/asset_name:var_name
            else
            {
...
/**
 * @brief Runs the system command for a particular asset and updates the asset's state
 * 
...
int HandleCmd(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* av)
{
    if(0)FPS_PRINT_INFO("running  [{}]  ", "test");
    if(0)FPS_PRINT_INFO("assetVar [{}]  aname [{}]   val [{}]", av->getfName(), cstr{aname}, av->getdVal());
