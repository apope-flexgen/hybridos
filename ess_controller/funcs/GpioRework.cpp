...
int SetupRwGpioSched(scheduler*sched, asset_manager* am)
{
    ...
    if(1)FPS_ERROR_PRINT("%s >> Running  [%s] am %p \n", __func__, am->name.c_str() , am);
...
int GpioOut(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* aV)
{
    ...
    if(1)FPS_ERROR_PRINT("%s >> checking aV [%s] am %p \n", __func__, aV->name.c_str() , aV->am);
    if(!aV->am)
    {
        FPS_ERROR_PRINT("%s >>  aV [%s:%s] No am found %p \n", __func__,  aV->comp.c_str(), aV->name.c_str(), aV->am);
        return 0;
    }
    ...
    //The input config stuff would have set / cleared the bit in the  output register...
    ...
    if (amap["gpios"]->gotParam("i2cdevint"))
    {
        ...
        FPS_ERROR_PRINT("%s >> GPIO #3  aV [%s] setting bval [%s] i2cdev %d  cfgVal 0x%04x cfg 0x%04x \n", __func__,  aV->fname, bval?"true":"false"
                     , i2cdev, cfgVal, cfg);
...
int GpioScan(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* aV)
{
    if(0)FPS_ERROR_PRINT("%s >> checking aV [%s] am %p \n", __func__, aV->name.c_str() , aV->am);
    ...
    if (0)FPS_ERROR_PRINT("%s >> checking aV [%s] am %p reload %d \n"
            , __func__, aV->am->name.c_str() , aV->am, reload);
    if (reload < 2)
    {
        ...
        for (auto x :vmap["/components/gpio_out"])
        {
            ...
            if(av)
            {
                ...
                // linkVals(*vm, vmap, amap, aname, "/components", bval, av->name.c_str());
                // if(av->gotParam("dir"))
                // {
        ...
        linkVals(*vm, vmap, amap, aname, "/config", ival, "GPIOcfg");  // output config
        linkVals(*vm, vmap, amap, aname, "/status", ival, "GPIOout");  // output values
    ...
    else
    {
        ...
        if(0)FPS_ERROR_PRINT("%s >> checking aV [%s] res  %d  oldres %d i2cdev %d \n"
            , __func__, aV->getfName() , res, oldres, i2cdev);
    }
    //if(res != oldres)
    {
        ...
        gpio_bitres = res;
        int bits = res^oldres;
        if(1 && bits!=0)FPS_ERROR_PRINT("%s >> checking aV [%s] res  %d  oldres %d bf 0x%0x \n"
            , __func__, aV->getfName() , res, oldres, bits);
        ival = (int) res;
        amap.at("GPIObits")->setVal(ival);
        varsmap* vlist = vm->createVlist(); // for sending assetVars out through fims_server/modbus_client
        // runs multiple commands based on which switches are on:
        // NOTE: Remove " ~ " - from res if res is all zeroes by default, only here to zero out
        ...
        for (auto x :vmap["/components/gpio"])
        {
            ...
            {
                // bool dirin = true;
                // if (avg->gotParam("dir") && (strcmp(avg->getcParam("dir"),"output") ==0))
                // {
                //     dirin = false;
                // }
                if (avg->getbParam("enabled")&& avg!= GPIORefTime)
                {
                    ...
                    if(0)FPS_ERROR_PRINT("%s >> tNow [%f] found gpio [%s] pin [%d] bitfield [%s] bitres [%s] bval [%s]\n"
                        , __func__
                        , tNow
                        , avg->getfName()
                        , avg->getiParam("bit")
                        , bits & (1<<avg->getiParam("bit"))?"true":"false"
                        , res & (1<<avg->getiParam("bit"))?"true":"false"
                        , bval?"true":"false"
                        );
                    if(bits & (1<<avg->getiParam("bit")))
                    {
                        if(1)FPS_ERROR_PRINT("%s >> tNow [%f] toggled gpio [%s] pin [%d] pending [%s] res [%d] bits [%d]\n"
                            , __func__
                            , tNow
                            , avg->getfName()
                            , avg->getiParam("bit")
                        ...
                        if (((res & (1<<avg->getiParam("bit"))) ^ avg->getbParam("inverted")) != bval) 
                        {
                            ...
                            if(bval == true)
                            {
                                ...
                                if(1)FPS_ERROR_PRINT("%s >> tNow [%f] toggled gpio [%s] pin [%d] pending [true] SeenTime/Threshold %2.3f/%2.3f\n"
                                    , __func__
                                    , tNow
                                    , avg->getfName()
                                    , avg->getiParam("bit")
                                    , tNow - avg->getdParam("SeenTime")
                                    , avg->getdParam("SeenTimeThreshold")
                                );
                            }
                            else
                            {
                                avg->setVal(bval);                  // No timer on reset - function only measures high level time currently
                                if(1)FPS_ERROR_PRINT("%s >> tNow [%f] toggled gpio [%s] pin [%d] pending [false] SeenTime/Threshold %2.3f/%2.3f\n"
                                    , __func__
                                    , tNow
                                    , avg->getfName()
                                    , avg->getiParam("bit")
                    ...
                    if(bval && (avg->getdParam("SeenTime") > 0.0))
                    {
                        ...
                        {
                            if (1) FPS_ERROR_PRINT("tNow [%f] SeenTime/Threshold %2.3f/%2.3f has been reached for pin: [%s]\n"
                                , tNow
                                , tNow-avg->getdParam("SeenTime") 
                                , avg->getdParam("SeenTimeThreshold")
                                , avg->getName());
...
int GpioInit(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* aV)
{
    ...
    int force = 0;
    bool i2cOK = false;
    if (1)FPS_ERROR_PRINT(">>>>>>>>Init >>>>%s running for GPIO Manager\n", __func__);
    asset_manager* am = aV->am;
    am->vm->setTime();
    for (auto x :vmap["/components/gpio"])
    {
        ...
        if (av)
        {
            if (!av->gotParam("SeenTimeThreshold")) // no threshold exists for this GPIO pin, setting it to 15ms:
            {
                FPS_ERROR_PRINT(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>SeenTimeThreshold not setup for GPIO pin: %s, setting it to 15ms\n"
                , av->getfName());
    ...
    if(am->vm->argc != 2)system("sudo modprobe i2c_dev"); // make sure to start the program with sudo as well.
    ...
    aV->setParam("i2cOK", i2cOK);
    if (1) FPS_ERROR_PRINT(" i2cbus %d address %d i2cdev %d fname [%s] saddr %d force %d\n"
                , i2cbusint, address, i2cdev, fname
                , saddr
                , force
                );
    
    // this should be it....
    //               name          func,     initial delay, priority , repeat time, assetVar 
    //int SchedRunFunc(varsmap& vmap, const char* sname, const char* func,  double runAfter,  double refTime,  double repTime, assetVar *aV)
...
int GpioRwPub(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* aV)
{
    ...
    if(debug)FPS_ERROR_PRINT("%s  sysVec -> [%s] aV [%s]\n",__func__, auri.c_str(), aV->getfName());
    // varsmap* vlist = vm->createVlist(); // for sending assetVars out through fims_server/modbus_client
    // for (auto x :vmap["/components/gpio"])
    // {
    //     // this scan looks for the false -> true transition
