...
int RunKeyCmd(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* aV)
{
    ...
    {
        if (1)
            FPS_ERROR_PRINT("%s >> Running [%s] reload %d \n", __func__, rldName, reload);
    ...
    if (amap["KeyCmd"]->getdParam("KeyCmdStartTime") == 0.0)
    {
        ...
        bval = true;
        if (1)
            FPS_ERROR_PRINT("%s >>>> Starting [%s] at time %f\n", __func__, rldName, tNow);
    }
    ...
    if (0)
        FPS_ERROR_PRINT(
            "%s >>>> Running at time %f KeyCmdStartTime %f "
            "maxKeyCmdTime %f maxKeyCmdOnTime %f maxKeyCmdTries %d\n",
            rldName, tNow, KeyCmdStartTime, maxKeyCmdTime, maxKeyCmdOnTime,
            amap["KeyCmd"]->getiParam("maxKeyCmdTries"));

    amap["KeyCmdTries"]->setVal((int)((tNow - KeyCmdStartTime) / maxKeyCmdTime));

    if (0)
        FPS_ERROR_PRINT("%s >>>> KeyCmdStartTime %f KeyCmdTries %d\n", rldName,
                        amap["KeyCmd"]->getdParam("KeyCmdStartTime"), amap["KeyCmdTries"]->getiVal());
    ...
    if (!amap["KeyCmd"]->getbParam("KeyCmdDone"))
    {
        if (amap["KeyCmdTries"]->valueChangedReset() || bval)
        {
            if (amap["KeyCmdTries"]->getiVal() < amap["KeyCmd"]->getiParam("maxKeyCmdTries"))
            {
                ...
                if (1)
                    FPS_ERROR_PRINT("%s >>>> Setting KeyCmd On Try no. %d at elapsed time %f\n", rldName,
                                    amap["KeyCmdTries"]->getiVal() + 1, tNow - KeyCmdStartTime);
            }
            else
            {
        ...
        if ((tNow - KeyCmdStartTime) > ((maxKeyCmdTime * amap["KeyCmdTries"]->getiVal()) + maxKeyCmdOnTime) &&
            amap["KeyCmd"]->getbVal())
        {
            amap["KeyCmd"]->setVal(false);
            if (1)
                FPS_ERROR_PRINT("%s >>>> Setting KeyCmd Off Try no. %d at elapsed time %f\n", rldName,
                                amap["KeyCmdTries"]->getiVal() + 1, tNow - KeyCmdStartTime);
        }
    }
    if (rldName)
