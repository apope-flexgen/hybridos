...
/**
 * @brief Periodically check if we are able to communicate with the assets/asset managers
 * 
...
// TODO put state strings  into a config assetVar so we can set it up from config
    // There is an assetVar for log. The value for the assetVar log is the file name
    // Ex.: /config/ess: { CheckAmCommsLogFile: { value: "file:CheckAmComms.txt" } }
    ...
    if (reload < 2) 
    {
        ...
        FPS_ERROR_PRINT("%s >> Log file from CheckAmCommsLog is %s\n", __func__, amap[logName]->getcVal() ? amap[logName]->getcVal() : "none");
        FPS_ERROR_PRINT("%s >> param from CheckAmCommsLog is %s\n", __func__, amap[logName]->getbParam("enablePerf") ? "true" : "false");
        ...
        if(reload == 0) // complete restart 
        {
            ...
            amap["Timestamp"]     ->setParam("seenStalled", false);    // Ensure log msgs are only sent out once when comms stalled for the first time
            amap["Timestamp"]     ->setParam("seenInit", false);
            amap["Timestamp"]     ->setParam("initCnt", -1);

            amap["Timestamp"]     ->setParam("FaultTime", amap["essCommsFaultTimeout"]->getdVal());       // time remaining before fault
            amap["Timestamp"]     ->setParam("AlarmTime", amap["essCommsAlarmTimeout"]->getdVal());       // time reamining before alarm
            amap["Timestamp"]     ->setParam("ResetTime", amap["essCommsRecoverTimeout"]->getdVal());   // time remaining before reset
            ...
            if (!logFile)
            {
                ...
                if (amap[logName])
                {
                    ...
                    if (1) FPS_ERROR_PRINT("%s >> Log file name from amap %s is %s\n", __func__, aname, logFile);
                }
                else
                {
            ...
            if (0) FPS_ERROR_PRINT("%s >> enablePerf for %s is %s\n", __func__, amap[logName]->name.c_str(), amap[logName]->getbParam("enablePerf") ? "true" : "false");
            
            //amap[logName]->setVal(logFile);
            // amap[logName]->openLog(vm->runLogDir?vm->runLogDir:(char*)"run_logs", logFile);
            // amap[logName]->logAlways();
    ...
    if(!am->am)
    {
        ...
        for (auto ix : am->assetManMap)
        {
            ...
            {
                if (1) FPS_ERROR_PRINT("%s >> %s %d essCommsFaults %d essCommsAlarms detected\n"
        ...
        if(essCommsFaults > 0)
        {
            if (1) FPS_ERROR_PRINT("%s >> %d essCommsFaults detected\n", __func__, essCommsFaults);
            // if (0) amap[logName]->sendLog(amap["Timestamp"], "%s >> %d essCommsFaults detected\n"
            //                              , __func__
            //                              , essCommsFaults);
        ...
        if(essCommsAlarms > 0)
        {
            if (1) FPS_ERROR_PRINT("%s >> %d essCommsAlarms detected\n", __func__, essCommsAlarms);
            // if (0) amap[logName]->sendLog(amap["Timestamp"], "%s >> %d essCommsAlarms detected\n"
            //                              , __func__
            //                              , essCommsAlarms);
        }
        ...
        if(initCnt  !=  icnt)
        {
            ...
            if (1) FPS_ERROR_PRINT("%s >> icnt %d intiSeen %d  change detected\n", __func__, icnt, initCnt);
            // if (0) amap[logName]->sendLog(amap["Timestamp"], "%s >> icnt %d intiSeen %d  change detected at %s\n"
            //                              , __func__
            //                              , icnt
            //                              , initCnt
            //                              , strtime(local_tm));
    ...
    if (strcmp(currentTimestamp, initTimestamp) == 0)    
    {
        ...
        // if (0) amap[logName]->sendLog(amap["Timestamp"], "%s >> %s  NO Timestamp,  bypass [%s]\n"
        //                                  , __func__
        //                                  , aname
    ...
    else  // wait for comms to go past reset then set active or wait to alarm and then fault
    {
        ...
        // Has value changed ? If yes then count down RecoverTime to zero based on tNow - tLast
        // if we changed then increase alarm / fault timeout up to max decrease recovery time to 0
        // if recovery time == 0 and falut/alarm was set then reset alarm
        // not quite add the time since last changed this way we recover all the time slots we lost

        // did we see a change
        if (strcmp(currentTimestamp, lastTimestamp) != 0) 
        {
            if(0)FPS_ERROR_PRINT("%s >>  ts  change for %s from [%s] to [%s] at %2.3f \n"
                , __func__, aname, lastTimestamp, currentTimestamp, tNow);
            ...
            if(strcmp(aname,"pcs")==0)
                if(0)FPS_ERROR_PRINT("%s >>  ts  change for %s from [%s] to [%s] at %2.3f, (%2.3f)   RecoverTime %2.3f AlarmTime %2.3f toAlarm %2.3f FaultTime %2.3f\n"
                    , __func__, aname, lastTimestamp?lastTimestamp:"no last Value", currentTimestamp, tNow,  (tNow - lastChange), RecoverTime, AlarmTime, toAlarm, FaultTime);

            // Log timestamp change here
            // if (0) amap[logName]->sendLog(amap["Timestamp"], "%s >> Timestamp change for %s from [%s] to [%s]  RecoverTime now %2.3f diff %2.3f AlarmTime %2.3f FaultTime %2.3f\n"
            //                              , __func__
            //                              , aname
            //                              , lastTimestamp ? lastTimestamp : "no last value"
            //                              , currentTimestamp
            //                              , RecoverTime, (tNow - tLast), AlarmTime, FaultTime);

            ival = amap["CommsStateNum"]->getiVal();
            FPS_ERROR_PRINT("%s >> %s Current Comms State Num %d toReset Time %2.3f seenOk %s\n", __func__, aname, ival, RecoverTime, seenOk ? "true" : "false");
            ...
            if (RecoverTime <= 0.0 && !seenOk)
            {
                ...
                if(seenFault)
                {
                    ...
                    // if (0) amap[logName]->sendLog(amap["Timestamp"], "%s >>  Timestamp Comms fault for %s cleared at %s\n"
                    //                 , __func__
                    //                 , aname
                    //                 , strtime(local_tm));
                ...
                if(seenAlarm)
                {
                    ...
                    // if (0) amap[logName]->sendLog(amap["Timestamp"], "%s >>  Timestamp Comms alarm for %s cleared at %s\n"
                    //                 , __func__
                    //                 , aname
                    //                 , strtime(local_tm));
                ...
                // if (1) amap[logName]->sendLog(amap["Timestamp"], "%s >> Timestamp Comms for [%s] [%s] OK at %s\n"
                //                     , __func__
                //                     , aname
                //                     , amap["Timestamp"]->getcVal()
                //                     , strtime(local_tm));
                // Ensure comms is no longer stalled
                if (amap["Timestamp"]->getbParam("seenStalled")) amap["Timestamp"]->setParam("seenStalled", false);
        ...
        else   // No Change , start tracking faults and alarms
        {
            ...
            {
                if(1)FPS_ERROR_PRINT("%s >>  Timestamp  Fault (%2.3f) for %s at %2.3f \n", __func__, FaultTime, aname, tNow);

                // Log comms fault here
                // if (0) amap[logName]->sendLog(amap["Timestamp"], "%s >> Timestamp Comms Fault for %s at %s\n"
                //                          , __func__
                //                          , aname
                //                          , strtime(local_tm));
                char *tval = nullptr;
                ...
                ESSLogger::get().critical("Comms lost for: [{}]",
                    aname);
                std::string dirAndFile = fmt::format("{}/{}_{}.{}", LogDir, aname, "CommsFault", "txt"); // might just hard code the file name itself instead.
                ...
                if (0) FPS_ERROR_PRINT("%s >> Fault Sent dest [%s] msg [%s]  am %p \n", __func__, dest, msg, (void*)am);
                // if (0) amap[logName]->sendLog(amap["Timestamp"], "%s >> Timestamp Comms fault sent dest [%s] msg [%s]  am %p at %s\n"
                //                          , __func__
                //                          , dest
                //                          , msg
                //                          , (void*)am
                //                          , strtime(local_tm));
                if(1)FPS_ERROR_PRINT("%s >>  aname %s TimeStamp comp [%s] name [%s] \n"
                    , __func__
                    , aname
                    , amap["Timestamp"]->comp.c_str()
                    , amap["Timestamp"]->name.c_str()
            ...
            {
                if(1)FPS_ERROR_PRINT("%s >>  Timestamp  Alarm  (%2.3f) for %s at %2.3f \n", __func__, AlarmTime, aname, tNow);

                // Log timestamp alarm here
                // if (0) amap[logName]->sendLog(amap["Timestamp"], "%s >> Timestamp Comms Alarm for %s at %s\n"
                //                          , __func__
                //                          , aname
                //                          , strtime(local_tm));
                ...
                if (0) FPS_ERROR_PRINT("%s >> Alarm Sent dest [%s] msg [%s]  am %p \n", __func__, dest, msg, (void*)am);
                // if (0) amap[logName]->sendLog(amap["Timestamp"], "%s >> Timestamp Comms alarm sent dest [%s] msg [%s]  am %p at %s\n"
                //                          , __func__
                //                          , dest
                //                          , msg
                //                          , (void*)am
            ...
            {
                if(1)FPS_ERROR_PRINT("%s >> Comms for [%s] [%s] Stalled at %2.3f  Reset %2.3f Fault %2.3f Alarm %2.3f \n"
                        , __func__
                        , aname
                        , amap["Timestamp"]->getcVal()
                        , tNow 
                        , RecoverTime, FaultTime, AlarmTime);
            ...
            {
                // if (1) amap[logName]->sendLog(amap["Timestamp"], "%s >> Timestamp Comms for [%s] [%s] Stalled at %s\n"
                //             , __func__
                //             , aname
                //             , amap["Timestamp"]->getcVal()
                //             , strtime(local_tm));
