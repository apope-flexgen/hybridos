...
namespace CalculateVarUtils
{
    ...
    {
        "valChangedAny", "valChangedAll"
    };

    /**
     * @brief Sets the calculated value of type boolean to the assetVar. Converts boolean to double if the assetVar's data type is numeric
     * 
...
void setupCalculateVarParams(varsmap& vmap, varmap& amap, const char* aname, assetVar* av)
{
    ...
    for (int i = 1; i <= av->getiParam("numVars"); i++)
    {
        ...
        if (!avUri.empty())
        {
            ...
            // See if the assetVar exists in the data map if the uri is provided like so: var_name
            if (!my.Var)
            {
                if (av->gotParam(varParam.c_str()) && av->getcParam(varParam.c_str()) && !amap[avUri])
                {
                    ...
                    FPS_PRINT_WARN("Checking assetVar [{}]  fname [{}] am ptr [{}]", avUri, amap[avUri]->getfName(), fmt::ptr(amap[avUri]->am));
                }
            }
            else
            {
                // If we have the name of the asset and the name of the variable (ex.: bms:RackCurrent), this indicates we are doing an aggregation
                // In this case, check to make sure all asset instance's assetVars exist
                if (strcmp(aname, my.Uri) == 0 && strcmp(av->am->name.c_str(), my.Uri) == 0)
                {
                    const std::string avName = fmt::format("{}", my.Var);
                    FPS_PRINT_INFO("Found the name of the asset [{}] in param [{}] in [{}:{}]. This indicates we are including all of asset instance's assetVars with name {} for aggregation"
                    ...
                    if (!av->am->assetManMap.empty())
                    {
                        ...
                        for (auto& pair : av->am->assetManMap)
                        {
                            ...
                            FPS_PRINT_INFO("[{}:{}] parent asset manager [{}] asset manager [{}]", av->comp, av->name, aname, ami->name);
                            if (!ami->amap[avName])
                            {
                                double dval = 0.0;
                                FPS_PRINT_INFO("Creating AM, default double assetVar [{}] for [{}]", avName, ami->name);
                                ami->amap[avName] = vm->setLinkVal(vmap, ami->name.c_str(), "/status", avName.c_str(), dval);
                            }
                        }
                    }
                    else
                    {
                        ...
                        for (auto& pair : av->am->assetMap)
                        {
                            ...
                            FPS_PRINT_INFO("[{}:{}] asset manager [{}] asset instance [{}]", av->comp, av->name, avName, ai->name);
                            if (!ai->amap[avName])
                            {
                                double dval = 0.0;
                                FPS_PRINT_INFO("Creating AI default double assetVar [{}] for [{}]", avName, ai->name);
                                ai->amap[avName] = vm->setLinkVal(vmap, ai->name.c_str(), "/status", avName.c_str(), dval);
                                FPS_PRINT_INFO("Checking AI assetVar [{}]  fname [{}] ptr am [{}] ai [{}] "
                                                , avName, ai->amap[avName]->getfName()
                                                ,fmt::ptr(ai->amap[avName]->am)
                ...
                // We have a uri and a variable name. Something like this: /component/asset_name:var_name
                else
                {
                    assetVar* avParam = vm->getVar(vmap, avUri.c_str(), nullptr);
                    // If the assetVar does not exist, create a new one. This should be added in vmap
                    if (!avParam)
                    {
                        //TODO give it a manager
                        FPS_PRINT_INFO("assetVar param {} does not exist. Creating new one for am [{}]", avUri, av->am->name);
    ...
    // Note: if true, only the expression is evaluated. Otherwise, only the operation is used
...
/**
 * @brief Checks the math operator (+, -, *, /, **) and perform the operation using
 * the list of operands if valid. Results are then stored in assetVar
 * 
 * @param vmap the global data map shared by all assets/asset managers
 * @param amap the local data map used by an asset/asset manager
...
{
    // Parse the expression, making sure the expression is valid and replacing any placeholders (ex.: {1})
    // with the appropriate corresponding operands
    //const PSW v1.1.0 possible leak ??
    const std::string& expr = CalculatorUtils::parseExpr(vmap, amap, av, "expression");
    if (expr.empty())
    {
        FPS_PRINT_WARN("Unable to parse expression [{}] in [{}:{}]. Skipping expression evaluation", cstr{av->getcParam("expression")}, av->comp, av->name);
        return;
    }

    if(debug)FPS_PRINT_INFO("expr char* [{}] expr string [{}]", av->getcParam("expression"), expr);
    ...
    {
        if(debug)FPS_PRINT_INFO("{} = {}. Set to {}", expr, result.dval, av->name);
    ...
    {
        if(debug)FPS_PRINT_INFO("{} = {}. Set to {}", expr, result.bval, av->name);

        // If the av type is an int, then we'll store the resulting boolean value as an int
        CalculateVarUtils::setBoolVal(vmap, av, result.bval);
        break;
    }
    case Expression::DataType::STRING:
    {
        if(debug)FPS_PRINT_INFO("{} = {}. Set to {}", expr, result.sval, av->name);
    ...
    default:
        FPS_PRINT_ERROR("Type {} not supported for result. Supported expression data types are: NUMERIC, BOOL, and STRING", result.type);
    }
}

/**
 * @brief Parses the math expression (+, -, *, /, avg, etc.) and evaluates the expression
 * if valid. Results are then stored in assetVar
 * 
 * @param vmap the global data map shared by all assets/asset managers
 * @param amap the local data map used by an asset/asset manager
 * @param av the assetVar to store the results
...
{
    // Check if the operation is defined and if it is an operation that can be used
    std::string oper(av->getcParam("operation"));
    if (oper.empty() 
        || (std::find(Math::operators.begin(), Math::operators.end(), oper) == Math::operators.end() 
        && std::find(CalculateVarUtils::operators.begin(), CalculateVarUtils::operators.end(), oper) == CalculateVarUtils::operators.end()))
    {
        // TODO after MVP send undefined action to log as well 
        FPS_PRINT_WARN("operation [{}] not defined for assetVar [{}:{}]. Supported operations are:\n"\
                                "    Addition           (+)\n"\
                                "    Subtraction        (-)\n"\
                                "    Multiplication     (*)\n"\
                                "    Division           (/)\n"\
    ...
    if (std::find(CalculateVarUtils::operators.begin(), CalculateVarUtils::operators.end(), oper) != CalculateVarUtils::operators.end())
    {
        ...
        {
            FPS_PRINT_ERROR("Unable to retrieve list of assetVars as operands for [{}:{}]. Skipping calculations", av->comp, av->name);
            return;
        }
        ...
        {
            // Run valChanged function for all assetVar operands. Run valChangedReset here since we are only interested in seeing
            // at least one operand value change
            bool results = std::any_of(operands.begin(), operands.end(), [](assetVar* av) { return av->valueChanged(); });
            if (results != av->getbVal())
            {
                // Only set value through vm and trigger actions when the resulting value is true, which indicates all operand values have changed
                if (results)
                    vm->setVal(vmap, av->comp.c_str(), av->name.c_str(), results);

                // Run valChangedReset afterwards to reset the changd flag for all assetVars
        ...
        {
            // Run valChanged function for all assetVar operands. We do not want to run valChangedReset here until all operand values have changed
            bool results = std::all_of(operands.begin(), operands.end(), [](assetVar* av) { return av->valueChanged(); });
            if (results != av->getbVal())
            {
                // Only set value through vm and trigger actions when the resulting value is true, which indicates all operand values have changed
                if (results)
    ...
    // Grab the list of operands as type double. Only valid if all of the operands in the list
    // are type double
    std::vector<double> operandsDbl;
    if (!CalculatorUtils::getOperandsDbl(vmap, amap, av, operandsDbl))
    {
        FPS_PRINT_DEBUG("Unable to retrieve list of operands of type double for [{}:{}]. Skipping calculations", av->comp, av->name);
        return;
    }
    ...
    if (oper == "+")
    {
        ...
        if (av->gotParam("scale")) results = Math::scale(results, av->getdParam("scale"));
...
/**
 * @brief Calculate the results using either the math operation or the math expression
 * 
 * @param vmap the global data map shared by all assets/asset managers
 * @param amap the local data map used by an asset/asset manager
 * @param aname the name of the asset/asset manager
...
int CalculateVar(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* av)
{
    if (!av) 
    {
        FPS_PRINT_INFO("assetVar is null. Exiting function");
        return 0;
    }
    ...
    if(enabled)
    {
        // Perform calculation, depending on whether we got an expression or an operation
