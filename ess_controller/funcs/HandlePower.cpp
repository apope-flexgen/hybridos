...
int HandlePowerInit(varsmap& vmap, VarMapUtils* vm, varmap& amap, const char* aname, fims* p_fims, const char* p,
                    const char* b, int reload);
...
int CheckTableVar(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* av1, assetVar* av2,
                  assetVar* tblAv);
...
int AggregateManager(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* aV)
{
    ...
    if (!am || !am->vm)
    {
        FPS_PRINT_ERROR("no am ({}) (or vm) for [{}] aname [{}]", fmt::ptr(am), aV->getfName(), aname);  // spdlog
        return 0;
    }
    ...
    if (1)
    {
        ...
        for (auto& iy : am->assetMap)
        {
            asset* ami = iy.second;
            if (debug)
                FPS_PRINT_INFO(" Got asset instance {}. Running  now..", ami->name);
        ...
        {
            if (debug)
                FPS_PRINT_INFO("{} MaxVal {} in {} MinVal {} in {} select {}\n", cstr{ aV->getfName() }, maxVal,
                               cstr{ maxName }, minVal, cstr{ minName }, tmpsel);
        }
    }
    return 0;
}

int HandlePowerInit(varsmap& vmap, VarMapUtils* vm, varmap& amap, const char* aname, fims* p_fims, const char* pcsch,
                    const char* bmsch, int reload)
{
    UNUSED(p_fims);
    UNUSED(bmsch);
    double dval = 0.0;
    ...
    bool debug = true;
    if (debug)
        FPS_PRINT_INFO("RUNNING RELOAD {} for {}", reload, pcsch);
    // Rated power values may come from either configuration (/config/pcs) or
    // hardware (/components/pcs_info -> /limits/pcs) First we see if either exist
    // for P, Q, S and pf
    ...
    if (!Priority_ctrl)
        FPS_PRINT_INFO("No Priority found for {}", pcsch);
    if (debug)
        FPS_PRINT_INFO("Done checking hw and config", NULL);

    // First look for limits from hardware
    // If HW limit wasn't found at the first run, don't create "_HW" entry in
    // /limits
    if (Sr && (reload < 2 || amap["RatedApparentPower_HW"]))
    {
        S = Sr->getdVal();
        if (debug)
            FPS_PRINT_INFO("{} Found Sr_hw [{}]", aname, S);
    ...
    if (Pr && (reload < 2 || amap["RatedActivePower_HW"]))
    {
        P = Pr->getdVal();
        if (debug)
            FPS_PRINT_INFO("{} Found Pr_hw [{}]", aname, P);
    ...
    if (Qr && (reload < 2 || amap["RatedReactivePower_HW"]))
    {
        Q = Qr->getdVal();
        if (debug)
            FPS_PRINT_INFO("{} Found Qr_hw [{}]", aname, Q);
    ...
    if (pfr && (reload < 2 || amap["RatedPowerFactor_HW"]))
    {
        pf = pfr->getdVal();
        if (debug)
            FPS_PRINT_INFO("{} Found pfr_hw [{}]", aname, pf);
    ...
    if (PC && (reload < 2 || amap["MaxChargePower_HW"]))
    {
        P = PC->getdVal();
        if (debug)
            FPS_PRINT_INFO("{} Found PC_hw [{}]", aname, P);
    ...
    if (PD && (reload < 2 || amap["MaxDischargePower_HW"]))
    {
        P = PD->getdVal();
        if (debug)
            FPS_PRINT_INFO("{} Found PD_hw [{}]", aname, P);
    ...
    if (Pr_c)
    {
        P = Pr_c->getdVal();
        if (debug)
            FPS_PRINT_INFO("{} Found Pr_c [{}]", aname, P);
    ...
    if (Qr_c)
    {
        Q = Qr_c->getdVal();
        if (debug)
            FPS_PRINT_INFO("{} Found Qr_c [{}]", aname, Q);
    ...
    if (Sr_c)
    {
        S = Sr_c->getdVal();
        if (debug)
            FPS_PRINT_INFO("{} Found Sr_c [{}]", aname, S);
    ...
    if (pfr_c)
    {
        pf = pfr_c->getdVal();
        if (debug)
            FPS_PRINT_INFO("{} Found pfr_c [{}]", aname, pf);
    ...
    if (PMaxSet_c)
    {
        P = PMaxSet_c->getdVal();
        if (debug)
            FPS_PRINT_INFO("{} Found PmaxSet_c [{}]", aname, P);
        linkVals(*vm, vmap, amap, pcsch, "/limits", dval, "MaxActivePowerSetpoint_Config");
        amap["MaxActivePowerSetpoint_Config"]->setVal(P);
        if (!amap["RatedActivePower_HW"] && !amap["RatedActivePower_config"])
            Pr = PMaxSet_c;
    }
    if (debug)
        FPS_PRINT_INFO("Done initializing vars", NULL);

    if (Priority_ctrl)
        cval = Priority_ctrl->getcVal();
    if (Priority_c && !cval)
    {
        ...
        {
            if (debug)
                FPS_PRINT_INFO("{} Found Priority from config: {}", aname, cval);
        ...
        else
        {
            ...
            if (debug)
                FPS_PRINT_INFO(
                    "{} Priority from config invalid. Setting power "
                    "priority to default P {}",
                    aname, cval);
            linkVals(*vm, vmap, amap, pcsch, "/controls", cval, "PowerPriority");
            amap["PowerPriority"]->setVal(cval);
        }
    ...
    else if (!Priority_c || !cval || (cval[0] != 'p' && cval[0] != 'q' && cval[0] != 'P' && cval[0] != 'Q'))
    {
        ...
        if (debug)
            FPS_PRINT_INFO(
                "{} No priority found in /config or /controls. Setting "
                "power priority for {} to default P {}",
                aname, pcsch, cval);
        linkVals(*vm, vmap, amap, pcsch, "/controls", cval, "PowerPriority");
        amap["PowerPriority"]->setVal(cval);
    }
    if (debug)
        FPS_PRINT_INFO("Done initializing Priority", NULL);

    P = -1;
    Q = -1;
    S = -1;
    pf = 0.75;  // Default power factor (from UL1741)
    if (amap["RatedActivePower_HW"])
        P = Pr->getdVal();
    else if (amap["RatedActivePower_Config"])
        P = Pr_c->getdVal();
    if (amap["RatedReactivePower_HW"])
        Q = Qr->getdVal();
    else if (amap["RatedReactivePower_Config"])
        Q = Qr_c->getdVal();
    if (Sr && amap["RatedApparentPower_HW"])
        S = Sr->getdVal();
    else if (Sr_c && amap["RatedApparentPower_Config"])
        S = Sr_c->getdVal();
    if (amap["RatedPowerFactor_HW"])
        pf = pfr->getdVal();
    else if (amap["RatedPowerFactor_Config"])
        pf = pfr_c->getdVal();
    if (pf <= 0 || pf > 1)
        pf = 0.75;
    if (debug)
        FPS_PRINT_INFO("Done initializing Ps and Qs", NULL);

    linkVals(*vm, vmap, amap, pcsch, "/limits", dval, "MaxPCSReactivePower", "RatedApparentPower");

    if (P > -1 && Q > -1 && S > -1)
    {
        if (P > S && !amap["RatedActivePower_HW"])
            P = S;
        if (Q > S && !amap["RatedReactivePower_HW"])
            Q = S;
    ...
    else
    {
        ...
        {
            if (PC && PD)
                P = -PC->getdVal() > PD->getdVal() ? -PC->getdVal() : PD->getdVal();
            else if (PC)
                P = -PC->getdVal();
            else if (PD)
                P = PD->getdVal();
            ...
            Q = sqrt(S * S * (1 - (pf * pf)));
            FPS_PRINT_INFO(
                "Rated Active/Reactive/Apparent Powers set to max "
                "charge/discharge powers {}/{}/{}",
                P, Q, S);
        }
        else if (Q > -1)
        {
        ...
        else if (PMaxSet_c)
        {
            ...
            FPS_PRINT_ERROR(
                "PCS Config and hardware missing rated "
                "active/apparent/reactive power and/or power factor",
                NULL);
    }
    if (debug)
        FPS_PRINT_INFO("Done initing vars", NULL);
    ...
    if (debug)
        FPS_PRINT_INFO("Done setting max Ps", NULL);
...
int HandlePowerCmd(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* aV)
{
    ...
    if (reload < 2)
    {
        ...
        linkVals(*vm, vmap, amap, essch, "/limits", dval, "MaxChargePower", "MaxDischargePower", "MaxReactivePower",
                 "MaxApparentPower");

        linkVals(*vm, vmap, amap, pcsch, "/controls", dval, "ActivePowerSetpoint", "ReactivePowerSetpoint",
                 "ActivePowerCmdReal", "ReactivePowerCmdReal");
        linkVals(*vm, vmap, amap, pcsch, "/controls", cval, "PowerPriority");
    ...
    if (debug)
        FPS_PRINT_INFO("Running reload {} essch {} for {}", reload, essch, aV->getfName());

    // Update /pcs/limits with HandlePowerLimit if anything in the table or
    // anything in the config changes
    ...
    {
        if (debug)
            FPS_PRINT_INFO("Limits Changed  essch {} aname {}  aV {}", essch, aname, aV->getfName());
        HandlePowerInit(vmap, vm, amap, aname, p_fims, pcsch, bmsch, reload);
        bval = true;
        ...
        if (debug)
            FPS_PRINT_INFO("Amap check  amap {} aname {}->{}->{}  essch {}->{}->{} ", fmt::ptr(&amap), aname,
                           fmt::ptr(am1), fmt::ptr(amap1), essch, fmt::ptr(am2), fmt::ptr(amap2));
    }
    ...
    if (amap["ChargePowerLimit"]->valueChangedReset() | amap["DischargePowerLimit"]->valueChangedReset() || bval)
    {
        if (debug)
            FPS_PRINT_INFO(
                "MaxBMSChargePower [{}] MaxBMSDischargePower [{}] "
                "RatedApparentPower [{}]",
                amap["ChargePowerLimit"]->getdVal(), amap["DischargePowerLimit"]->getdVal(),
        ...
        {
            if (Smax_D > Smax_PCS)
                amap["MaxApparentPower"]->setVal(Smax_PCS);
            else
                amap["MaxApparentPower"]->setVal(Smax_D);
        }
        else
        {
            if (Smax_C > Smax_PCS)
                ...
                amap["MaxApparentPower"]->setVal(Smax_C);
        }
        ...
        if (debug)
            FPS_PRINT_INFO(
                "Max Charge Power [{}] kW Max Discharge Power [{}] kW "
                "Qmax [{}] kVAr Smax [{}] kVA ",
                amap["MaxChargePower"]->getdVal(), amap["MaxDischargePower"]->getdVal(),
                amap["MaxReactivePower"]->getdVal(), amap["MaxApparentPower"]->getdVal());
    }

    if (amap["MaxChargePower"]->valueChangedReset() | amap["MaxDischargePower"]->valueChangedReset() |
        amap["MaxReactivePower"]->valueChangedReset() | amap["ActivePowerSetpoint"]->valueChangedReset() |
        amap["ReactivePowerSetpoint"]->valueChangedReset() | amap["MaxApparentPower"]->valueChangedReset() |
        amap["PowerPriority"]->valueChanged())
    {
        char* clval = amap["PowerPriority"]->getcLVal();
        cval = amap["PowerPriority"]->getcVal();
        if (cval[0] != 'P' && cval[0] != 'Q' && cval[0] != 'p' && cval[0] != 'q')
            amap["PowerPriority"]->setVal(clval);
        amap["PowerPriority"]->valueChangedReset();

        if (debug)
            FPS_PRINT_INFO(
                "MaxChargePower [{}] MaxDischargePower [{}] MaxReactivePower [{}] MaxApparentPower [{}] \
                                    PowerPriority [{}] ActivePowerSetpoint [{}] ReactivePowerSetpoint [{}]",
                amap["MaxChargePower"]->getdVal(), amap["MaxDischargePower"]->getdVal(),
                amap["MaxReactivePower"]->getdVal(), amap["MaxApparentPower"]->getdVal(),
                amap["PowerPriority"]->getcVal(), amap["ActivePowerSetpoint"]->getdVal(),
                amap["ReactivePowerSetpoint"]->getdVal());

        // Start with setpoints, coerce them into range first based on BMS/PCS
        // derived limits, then Apparent power limit
        ...
        if (Pcmd_lim > Pmax_D)
        {
            ...
            {
                if (debug)
                    FPS_PRINT_INFO("Pset [{}] kW limited to Max Discharge Power [{}] kW", Pcmd_lim, Pmax_D);
                tm* local_tm = vm->get_local_time_now();
                ESSLogger::get().info("Max discharge kW limited to [{}]", Pmax_D);
                msg = fmt::format(
                    "[{}] kW setpoint [{}] limited to Discharge power "
                    "limit [{}] at [{}]",
        ...
        else if (Pcmd_lim < Pmax_C)
        {
            ...
            {
                if (debug)
                    FPS_PRINT_INFO("Pset [{}] kW limited to Max Charge Power [{}] kW", Pcmd_lim, Pmax_C);
                tm* local_tm = vm->get_local_time_now();
                ESSLogger::get().info("Max charge kW limited to [{}]", Pmax_C);
                msg = fmt::format("[{}] kW setpoint [{}] limited to Charge power limit [{}] at [{}]", __func__,
                                  Pcmd_lim, Pmax_C, strtime(local_tm));
                aV->sendEvent(aname, am->p_fims, Severity::Info, msg.c_str());
        ...
        if (Qcmd_lim > Qmax)
        {
            ...
            {
                if (debug)
                    FPS_PRINT_INFO("Qset [{}] kVAr limited to Max Reactive Power [{}] kVAr", Qcmd_lim, Qmax);
                tm* local_tm = vm->get_local_time_now();
                ESSLogger::get().info("Max reactive kVAr limited to [{}]", Qmax);
                msg = fmt::format(
                    "[{}] kVAr setpoint [{}] limited to reactive power "
                    "limit [{}] at [{}]",
        ...
        if (debug)
            FPS_PRINT_INFO(
                "After PCS/BMS limits: Pcmd_lim [{}] kW Qcmd_lim [{}] "
                "kVAr Smax [{}] kVA",
                Pcmd_lim, Qcmd_lim, Smax);

        // If S from P and Q requests is too large, decrease P or Q accordingly
        // depending on power priority (P priority by default)
        double Scmd_lim = sqrt((Pcmd_lim * Pcmd_lim) + (Qcmd_lim * Qcmd_lim));
        if (Scmd_lim > Smax)
        {
            if (debug)
                FPS_PRINT_INFO("S derived from Pcmd and Qcmd is greater than Smax [{}]", Smax);
            cval = amap["PowerPriority"]->getcVal();
            tm* local_tm = vm->get_local_time_now();
            if (cval[0] == 'Q' || cval[0] == 'q')
            {
                if (Pcmd_lim >= 0)
                    Pcmd_lim = sqrt((Smax * Smax) - (Qcmd_lim * Qcmd_lim));
                else
                    Pcmd_lim = -sqrt((Smax * Smax) - (Qcmd_lim * Qcmd_lim));
                ESSLogger::get().info("Max apparent kVA limited to [{}] under Q priority", Smax);
                msg = fmt::format(
                    "[{}] Apparent power limit [{}] exceeded by setpoint "
                    "[{}]. Active power limited to [{}] at [{}]",
                    __func__, Smax, Scmd_lim, Pcmd_lim, strtime(local_tm));
            }
            else
            {
                if (Qcmd_lim >= 0)
                    Qcmd_lim = sqrt((Smax * Smax) - (Pcmd_lim * Pcmd_lim));
                else
                    Qcmd_lim = -sqrt((Smax * Smax) - (Pcmd_lim * Pcmd_lim));
                ESSLogger::get().info("Max apparent kVA limited to [{}] under P priority", Smax);
                msg = fmt::format(
                    "[{}] Apparent power limit [{}] exceeded by setpoint "
                    "[{}]. Reactive power limited to [{}] at [{}]",
                    __func__, Smax, Scmd_lim, Qcmd_lim, strtime(local_tm));
            }
            if (debug)
                FPS_PRINT_INFO("Scmd [{}] kVA limited to Max Apparent Power [{}] kVA", Scmd_lim, Smax);
            aV->sendEvent(aname, am->p_fims, Severity::Info, msg.c_str());
        }

        if (debug)
            FPS_PRINT_INFO("After S limiting: Pcmd [{}] kW Qcmd [{}] kVAr S [{}] kVA", Pcmd_lim, Qcmd_lim,
                           sqrt((Pcmd_lim * Pcmd_lim) + (Qcmd_lim * Qcmd_lim)));
        if (debug)
            FPS_PRINT_INFO("After S limiting: Pcmd [{}] kW Qcmd [{}] kVAr", amap["ActivePowerCmdReal"]->getdVal(),
                           amap["ReactivePowerCmdReal"]->getdVal());
        ...
        if (amap["ActivePowerCmdReal"]->getdVal() != Pcmd_lim)
        {
            ...
            if (debug)
                FPS_PRINT_INFO("Pcmd set to {} kW", amap["ActivePowerCmdReal"]->getdVal());
        ...
        if (amap["ReactivePowerCmdReal"]->getdVal() != Qcmd_lim)
        {
            ...
            if (debug)
                FPS_PRINT_INFO("Qcmd set to {} kVAr", amap["ReactivePowerCmdReal"]->getdVal());
...
int EstimatePower(varsmap& vmap, varmap& amap, const char* aname, fims* p_fims, assetVar* aV)
{
    ...
    {
        if (0)
            FPS_ERROR_PRINT("%s >> Running reload %d \n", __func__, reload);
        reload = 0;  // complete reset  reload = 1 for remap ( links may have changed)
    }
    ...
    if (reload < 2)
    {
        ...
        epAv = amap[__func__];
        linkVals(*vm, vmap, amap, bmsch, "/status", dval, "MaxBMSDischargePowerRunning", "MaxBMSChargePowerRunning",
                 "MaxBMSDischargePower", "MaxBMSChargePower", "MaxBMSDischargePowerOffset", "MaxBMSChargePowerOffset");
        linkVals(*vm, vmap, amap, bmsch, "/status", dval, "MaxBMSDischargeIV", "MaxBMSChargeIV",
                 "MaxBMSChargePowerEstFiltIn", "MaxBMSDischargePowerEstFiltIn");
        linkVals(*vm, vmap, amap, pcsch, "/status", bval, "Derate");
        linkVals(*vm, vmap, amap, pcsch, "/status", bval, "ActivePowerSetpoint");

        if (reload == 0)
        {
            if (!amap["MaxBMSDischargePower"]->gotParam("done"))
                amap["MaxBMSDischargePower"]->setParam("done", false);
            if (!amap["MaxBMSChargePower"]->gotParam("done"))
                amap["MaxBMSChargePower"]->setParam("done", false);
            if (!amap["MaxBMSDischargePower"]->gotParam("active"))
                amap["MaxBMSDischargePower"]->setParam("active", false);
            if (!amap["MaxBMSChargePower"]->gotParam("active"))
    ...
    {
        // When filtered max power peaks for the first time, use that power offset
        // as the max
        if (!amap["MaxBMSDischargePower"]->getbParam("done"))
        {
            if (amap["ActivePowerSetpoint"]->getdVal() >= 0)
            {
                ...
                {
                    if (amap["MaxBMSDischargePower"]->gotParam("MaxPowerSent") &&
                        amap["MaxBMSDischargePower"]->getbParam("MaxPowerSent"))
                    {
                        if (amap["MaxBMSDischargePowerRunning"]->getdVal() <=
                            amap["MaxBMSDischargePowerRunning"]->getdLVal())
                        {
                            amap["MaxBMSDischargePowerOffset"]->setVal(amap["MaxBMSDischargeIV"]->getdVal() -
                                                                       amap["MaxBMSDischargePowerRunning"]->getdVal());
                            amap["MaxBMSDischargePower"]->setParam("done", true);
                            if (1)
                                FPS_ERROR_PRINT("%s >> Max BMS Discharge Power Offset set to %f\n", __func__,
                                                amap["MaxBMSDischargePowerOffset"]->getdVal());
                        }
                    }
                ...
                if (!amap["MaxBMSDischargePower"]->getbParam("active"))
                {
                    ...
                    // vm->setVecDepth(vmap, amap,
                    // "MaxBMSDischargePowerEstFiltIn","vecAv",120, &lVal);
            ...
            else if (amap["MaxBMSDischargePower"]->getbParam("active"))
            {
                ...
                amap["MaxBMSDischargePower"]->setVal(amap["MaxBMSDischargePower"]->getdParam("lastActiveVal"));
        }
        else
            amap["MaxBMSDischargePower"]->setVal(amap["MaxBMSDischargeIV"]->getdVal() -
                                                 amap["MaxBMSDischargePowerOffset"]->getdVal());
        ...
        if (!amap["MaxBMSChargePower"]->getbParam("done"))
        {
            if (amap["ActivePowerSetpoint"]->getdVal() <= 0)
            {
                ...
                {
                    if (amap["MaxBMSChargePower"]->gotParam("MaxPowerSent") &&
                        amap["MaxBMSChargePower"]->getbParam("MaxPowerSent"))
                    {
                        if (amap["MaxBMSChargePowerRunning"]->getdVal() >= amap["MaxBMSChargePowerRunning"]->getdLVal())
                        {
                            amap["MaxBMSChargePowerOffset"]->setVal(amap["MaxBMSChargeIV"]->getdVal() -
                                                                    amap["MaxBMSChargePowerRunning"]->getdVal());
                            amap["MaxBMSChargePower"]->setParam("done", true);
                            if (1)
                                FPS_ERROR_PRINT("%s >> Max BMS Charge Power Offset set to %f\n", __func__,
                                                amap["MaxBMSChargePowerOffset"]->getdVal());
                        }
                    }
                ...
                if (!amap["MaxBMSChargePower"]->getbParam("active"))
                {
                    ...
                    // vm->setVecDepth(vmap, amap, "MaxBMSChargePowerEstFiltIn", "vecAv",
                    // 120, &lVal);
            ...
            else if (amap["MaxBMSChargePower"]->getbParam("active"))
            {
                ...
                amap["MaxBMSChargePower"]->setVal(amap["MaxBMSChargePower"]->getdParam("lastActiveVal"));
        }
        else
            amap["MaxBMSChargePower"]->setVal(amap["MaxBMSChargeIV"]->getdVal() -
                                              amap["MaxBMSChargePowerOffset"]->getdVal());
...
{
    UNUSED(p_fims);
    FPS_PRINT_INFO("WE'RE DERATING :P:P:P:P:", NULL);
    double dval = 0.0;
    bool bval = false;
    int rc = 0;
    int reload = 0;
    ...
    if (reload < 2)
    {
        ...
        linkVals(*vm, vmap, amap, bmsch, "/status", bval, "OverCurrentD1", "OverCurrentD2", "UnderCurrentD1",
                 "UnderCurrentD2");

        if (reload == 0)
        {
            // amap["Derate"]->setDbVal(0.001);
            if (!amap["Derate"]->gotParam("Increment"))
                amap["Derate"]->setParam("Increment", 0);
            if (!amap["Derate"]->gotParam("Decrement"))
                amap["Derate"]->setParam("Decrement", 0);
            if (!amap["Derate"]->gotParam("OverCurrent"))
                amap["Derate"]->setParam("OverCurrent", false);
            if (!amap["Derate"]->gotParam("Recovery"))
                amap["Derate"]->setParam("Recovery", false);
            if (!amap["Derate"]->gotParam("Hold"))
                amap["Derate"]->setParam("Hold", false);
            if (!amap["Derate"]->gotParam("Done"))
                amap["Derate"]->setParam("Done", false);
            if (!amap["Derate"]->gotParam("LastDerate"))
                amap["Derate"]->setParam("LastDerate", 1.0);
            if (!amap["Derate"]->gotParam("HoldDerate"))
                amap["Derate"]->setParam("HoldDerate", 1.0);
        }
    }
    ...
    if (!strcmp(amap["SystemState"]->getcVal(), "Running"))
    {
        ...
        if (amap["OverCurrentD1"]->getbVal() || amap["UnderCurrentD1"]->getbVal())
        {
            if (amap["OverCurrentD2"]->getbVal() || amap["UnderCurrentD2"]->getbVal())
            {
                ...
                {
                    if (1)
                        FPS_ERROR_PRINT("%s >> Active Power Derating starting\n", __func__);
                    const auto now = flex::get_time_dbl();
                    ESSLogger::get().info("Active Power Derating starting");
                    assetVar* temp_av = amap["Derate"];
                    if (temp_av)
                        temp_av->sendEvent(aname, am->p_fims, Severity::Info,
                                           "Active Power Derating starting at time %2.3f", now.count());
            ...
            if (!amap["Derate"]->getbParam("Recovery"))
            {
                ...
                else
                {
                    ...
                    {
                        if (1)
                            FPS_ERROR_PRINT("%s >> Active Power Derating set to 0\n", __func__);
                        const auto now = flex::get_time_dbl();
                        ESSLogger::get().info("Active Power Derating set to 0%");
                        assetVar* temp_av = amap["Derate"];
                        if (temp_av)
                            temp_av->sendEvent(aname, am->p_fims, Severity::Info,
                                               "Active Power Derating set to 0 at time %2.3f", now.count());
                    }
            ...
            {
                if (1)
                    FPS_ERROR_PRINT("%s >> Active Power Derating holding at [%f]\n", __func__,
                                    amap["Derate"]->getdVal());
                const auto now = flex::get_time_dbl();
                ESSLogger::get().info("Active Power Derating holding at [{}]", amap["Derate"]->getdVal());
                assetVar* temp_av = amap["Derate"];
                if (temp_av)
                    temp_av->sendEvent(aname, am->p_fims, Severity::Info,
                                       "Active Power Derating holding at [%f] at time %2.3f", amap["Derate"]->getdVal(),
                                       now.count());
        ...
        else
        {
            ...
            {
                if (1)
                    FPS_ERROR_PRINT("%s >> Active Power Derating recovering at [%f]\n", __func__, NewDerate);
                const auto now = flex::get_time_dbl();
                ESSLogger::get().info("Active Power Derating recovering at [{}]", NewDerate);
                assetVar* temp_av = amap["Derate"];
                if (temp_av)
                    temp_av->sendEvent(aname, am->p_fims, Severity::Info,
                                       "Active Power Derating recovering at [%f] at time %2.3f", NewDerate,
                                       now.count());
            ...
            {
                if (1)
                    FPS_ERROR_PRINT("%s >> Active Power no longer Derating\n", __func__);
                const auto now = flex::get_time_dbl();
                ESSLogger::get().info("Active Power no longer Derating");
                assetVar* temp_av = amap["Derate"];
                if (temp_av)
                    temp_av->sendEvent(aname, am->p_fims, Severity::Info,
                                       "Active Power no longer Derating at time %2.3f", now.count());
