...
//two_way_tbl* SetupTableVar(VarMapUtils* vm, const char* fileName, const char* ext = "csv");
...
int HandlePowerLimit(varsmap &vmap, varmap &amap, const char* aname, fims* p_fims, assetVar* av)
{
    ...
    if (reload < 2)
    {
        ...
        amap["mbmu_avg_cell_temperature"]  = vm->setLinkVal(*am->vmap, essName, "/status", "mbmu_avg_cell_temperature", dval);

        if (!amap["pChargeTbl"]->getcVal())    amap["pChargeTbl"]->setVal(chargeTbl);
        if (!amap["pDischargeTbl"]->getcVal()) amap["pDischargeTbl"]->setVal(dischargeTbl);
        ...
        if (tblPtr)
        {
            if (1) FPS_ERROR_PRINT("%s >> We're setting up a new table, but table %s at %p already exists. Deleting current table now...\n", __func__, amap["pChargeTbl"]->name.c_str(), tblPtr);
        ...
        ival = 2; amap["HandlePowerLimit"]->setVal(ival);
    }

    double soc = amap["mbmu_soc"]->getdVal();
    double temp = amap["mbmu_avg_cell_temperature"]->getdVal();
    //printf("%s >> done 3\n"  , __func__);
    if(0)FPS_ERROR_PRINT("%s >> done 4 pChargeTbl %p pDischargeTbl %p\n"
            , __func__
            , amap["pChargeTbl"]->getvParam("tbl")
    ...
    if(0)FPS_ERROR_PRINT("%s >> done 5 pChargeTbl %p pDischargeTbl %p\n"
            , __func__
            , pChargeTbl
            , pDischargeTbl
    ...
    {
        if (1) FPS_ERROR_PRINT("%s >> Unable to load tables \n", __func__);
        return 0;
    }
    ...
    if (0) FPS_ERROR_PRINT("%s >> soc [%2.3f] temp [%2.3f]  pChargeVal [%2.3f] pDischargeVal [%2.3f]\n", __func__, soc, temp, pChargeVal, pDischargeVal);
