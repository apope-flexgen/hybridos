...
if (1 === 2 && fs.existsSync(mdoJsonPathAndFile)) {
...
function subscribeShadow(uri) {
    ...
    if (!subscribes.find((sub) => uri.startsWith(sub))) {
        subscribes.push(uri);
        subscribes = subscribes.filter((sub) => sub === uri || !sub.startsWith(uri));
    }
}
...
function updateAndPubMetrics() {
    ...
    Object.keys(mdo).forEach((pubUri) => {
        Object.keys(mdo[pubUri]).forEach((id) => {
            if (id.includes('OF_slew_override') && !id.includes('PFR_OF_slew_override')) {
                console.log('---- sending', id, 'to updateMetric from updateAndPubMetrics');
            }
            updateMetric(pubUri, id);
        });
        const msg = {
            method: 'pub', uri: pubUri, replyto: null, body: prepareBody(pubUri),
        };
        processFims(msg);
        fims.send(msg);
    });
...
function updateMetric(pubUri, id) {
    ...
    if (comp[entry.operation]) {
        ...
        if (id.includes('OF_slew_override') && !id.includes('PFR_OF_slew_override')) {
            console.log('\n---------- updateMetric:');
            console.log(pubUri, id, new Date());
            console.log('----');
        }
        mdo[pubUri][id] = entry;
...
function insertMdo(uri, key, value) {
    ...
    if (key.includes('OF_slew_override') && !key.includes('PFR_OF_slew_override')) {
        console.log('----', key, 'in insertMdo');
    }
    if (metrics) {
        if (key.includes('OF_slew_override') && !key.includes('PFR_OF_slew_override')) {
            console.log('----', key, 'in insertMdo, past if(metrics)');
        }
        metrics.forEach((metric) => {
            if (key.includes('OF_slew_override') && !key.includes('PFR_OF_slew_override')) {
                console.log('---- sending', key, 'to processOutputs from insertMdo');
            }
        ...
        if (key.includes('OF_slew_override') && !key.includes('PFR_OF_slew_override')) {
            console.log('---- FORCE SENDing', key, 'to processOutputs from insertMdo');
            processOutputs(uri, key);
            console.log('---- AFTER FORCE SENDing', key, 'to processOutputs from insertMdo');
        }
    }
}
...
function processOutputs(pubUri, id) {
    ...
    if (entry.hasOwnProperty('outputs')) {
        ...
        } else if (!entry.hasOwnProperty('lastValue')) {
            /* eslint-enable no-prototype-builtins */
            entry.state.lastValue = null;
        }
        // TODO: this doesn't happen without the input:
        if (id.includes('OF_slew_override') && !id.includes('PFR_OF_slew_override')) {
            console.log('---- sending', id, 'to updateMetric from processOutputs');
        }
        updateMetric(pubUri, id);
        const { value } = entry;
        const { lastValue } = entry.state;
        if (id.includes('OF_slew_override') && !id.includes('PFR_OF_slew_override')) {
            console.log('----', id, 'AFTER updateMetric in processOutputs');
        }
        if ((entry.param && entry.param.setOutputOnUpdate) || lastValue !== value) {
            if (id.includes('OF_slew_override') && !id.includes('PFR_OF_slew_override')) {
                console.log('---- sending', id, 'out via FIMS');
            }
