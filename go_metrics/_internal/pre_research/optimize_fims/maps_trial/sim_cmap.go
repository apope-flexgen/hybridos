...
func cmapDataPreparation(
	id int,
	jobs <-chan IncomingJob,
	capPointer *(cmap.ConcurrentMap),
	progress chan bool,
...
func cmapProcessing(ConcurrentMapPointer *(cmap.ConcurrentMap), jobsDone chan bool, uri string, timeCount chan ProcessingTimeInfo, wg *sync.WaitGroup, pointerToVmap *interface{}) {
	...
	for {
		...
		uriChannelMap := ConcurrentMapPointer
		x, _ := uriChannelMap.Get(uri)
		msgs, _ := x.(chan Msg)
...
func cmapSim(numJobs int, pointerToVmap *interface{}, uris []string) (info SimInfo) {

	hashMapPointer := cmap.New()
	uriChannelMap := hashMapPointer

	for index := range uris {
		uri := uris[index]
		uriChannelMap.Set(uri, make(chan Msg, numJobs))
	}
	...
	go func() {
		...
		for index := range uris {
			...
			go cmapProcessing(&hashMapPointer, jobsDone, uri, processingTimeChannel, &wgForProcessing, pointerToVmap)
		}
	}()
	...
	go func() {
		...
		for w := 1; w <= numJobs; w++ {
			...
			default:
				...
				go cmapDataPreparation(w, jobs, &hashMapPointer, progress, dataPreparationTimeChannel, &wgForDataPreparation, jobsDone, pointerToVmap)
				numWorkers++
			}
