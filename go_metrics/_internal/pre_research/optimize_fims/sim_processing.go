...
func syncMapProcessing(uriChannelMap sync.Map, jobsDone chan bool, uri string, timeCount chan ProcessingTimeInfo, wg *sync.WaitGroup, pointerToVmap *interface{}) {
	...
	for {
		...
		if len(msgs) > 0 {
			...
			// 	// fimsMsg.Body = vmap
			// 	//Send back fims msg created in syncMapDataPreparation method to client uri
			// 	f.Send(fimsMsg)
...
// func syncMapSim(numJobs int, minProcessTime int, maxProcessTime int, jobInputDelay time.Duration, workerCreationDelay time.Duration)(info SimInfo) {
func syncMapSim(numJobs int, pointerToVmap *interface{}) (info SimInfo) {
	...
	for x := 0; x < numJobs; x++ {
		currInfo := <-dataPreparationTimeChannel
	...
	for x := 0; x < numJobs; x++ {
		currInfo := <-processingTimeChannel
		currTime := currInfo.Time
...
func main() {
	...
	for i := 1; i <= SIMS; i++ {
		...
		currStats := syncMapSim(numJobs, &vmap)
		...
		totalTime += currStats.TotalTime
	}

	fmt.Println()
	fmt.Println()
	fmt.Printf(" Avg Total Time Across All Sims:                                    %v \n\n\n", (totalTime / time.Duration(SIMS)))
}
