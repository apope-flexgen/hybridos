...
// constants:
static constexpr u32 Max_Expected_Data_Size = 10000; // at most 10000 characters for json data being send to client
...
struct Thread_Workspace
{
    ...
    std::future<bool> thread_future; // worker thread future
...
struct Component_Workspace
{
    ...
    // TODO(WALKER): Make sure that this works properly for waking up the aggregator thread
    // also waking up aggregator thread might be tricky (as all the channels need to finish before waking him up)
    // this means that when pushing on to the "pub" channel 
    std::atomic_size_t pub_counter{}; // might just have "aggregator" thread sleep for frequency then go and check channels (don't know about drift and stuff)
    std::mutex aggregator_mutex;
    std::condition_variable aggregator_cond;
    rigtorp::SPSCQueue<Get_Chan_Msg> get_channel{1024}; // "listener" thread sends on this channel

    // Heartbeat stuff:
    bool heartbeat_enabled = false;
    u8 heartbeat_read_thread_idx;
    u8 heartbeat_read_decode_idx;
    u8 heartbeat_write_thread_idx;
    u8 heartbeat_write_decode_idx;
    // std::chrono::nanoseconds heartbeat_frequency; // how often we check (minimum is poll frequency implicitly, might just remove this)
    ...
    fmt::memory_buffer send_buf; // For sending "gets" and "pubs" out on fims (reusable buffer)

    std::atomic_bool keep_running{false}; // to tell whether or not this component needs to stop
    std::future<bool> comp_future; // aggregator thread future
    ...
    {
        keep_running = false;
        // tell all our worker threads to stop:
        for (u8 thread_idx = 0; thread_idx < num_threads; ++thread_idx)
        {
            decoded_caches[thread_idx].thread_workspace->stop_thread();
        }
        // wait for all of them to stop fully:
        for (u8 thread_idx = 0; thread_idx < num_threads; ++thread_idx)
        {
            decoded_caches[thread_idx].thread_workspace->thread_future.wait();
        }
        // notify aggregator thread:
        {
            std::lock_guard<std::mutex> lk{aggregator_mutex};
        }
        aggregator_cond.notify_one();
    }
...
{
    u8 num_threads = 0;
    Get_Chan_Msg get_msg;
    Set_Chan_Msg* set_msgs = nullptr;
    std::bitset<Max_Num_Threads> set_send_mask; // which thread channels to send off on after parsing an incoming message (for sets)
...
struct Mod_Conn
{
    ...
    ~Mod_Conn() { if (ctx) { modbus_close(ctx); modbus_free(ctx); } }
};
// size info:
// auto x = sizeof(Mod_Conn);

struct Ctx_Pool
{
    u8 max_num_conns = 255; // This is either "max_num_conns" in the config or "total_num_threads" after reading the config
    u8 num_conns = 0;
    Mod_Conn* pool;
    ...
    static std::size_t get_bytes_needed(const config_loader::Client_Config& config)
    {
        ...
        needed += sizeof(Mod_Conn) * config.connection.max_num_conns + alignof(Mod_Conn);

        return needed;
    }
};
// size info:
// auto x = sizeof(Ctx_Pool);
...
struct Connection_Workspace
{
    ...
    std::size_t total_num_threads; // only counts worker threads for this number (this is NOT the same as num_conns)
...
{
    Connection_Workspace conn_workspace;
    u8 num_comps = 0;
    ...
    bool startup_modbus_conns(String_Storage& string_storage)
    {
        ...
        for (u8 i = 0; i < conn_workspace.ctx_pool.num_conns; ++i)
        {
            auto ctx = conn_workspace.ctx_pool.pool[i].ctx;
            if (ctx) { modbus_close(ctx); modbus_free(ctx); }
        }
        conn_workspace.ctx_pool.num_conns = 0;

        // Now connect the modbus contexts for this client:
        for (std::size_t i = 0; i < conn_workspace.total_num_threads; ++i)
        {
            if (i >= conn_workspace.ctx_pool.max_num_conns) break; // cannot exceed max_num_conns for threads

            auto& curr_conn = conn_workspace.ctx_pool.pool[i];
            auto connected = -1;
            ...
            ++conn_workspace.ctx_pool.num_conns;
    ...
    {
        keep_running = false;
        for (u8 comp_idx = 0; comp_idx < num_comps; ++comp_idx)
        {
            msg_staging_areas[comp_idx].comp_workspace->stop_thread();
        }
        // confirm that each aggregator thread has returned (each aggregator thread tells all their worker threads to stop):
        for (u8 comp_idx = 0; comp_idx < num_comps; ++comp_idx)
        {
            msg_staging_areas[comp_idx].comp_workspace->comp_future.wait();
        }
...
struct Main_Workspace
{
    ...
    u8 num_clients = 0;
    bool reload = false; // main runs a do/while loop and checks for this
...
static bool initialize_arena_from_config(const config_loader::Client_Configs_Array& configs_array, Main_Workspace& main_workspace)
{
    ...
    for (const auto& config : configs_array.configs)
    {
        ...
        bytes_needed += Ctx_Pool::get_bytes_needed(config);
...
static bool allocate_main_workspace_from_config(const config_loader::Client_Configs_Array& configs_array, Main_Workspace& main_workspace)
{
    ...
    for (u8 client_idx = 0; client_idx < main_workspace.num_clients; ++client_idx)
    {
        ...
        curr_client_workspace.conn_workspace.ctx_pool.max_num_conns = static_cast<u8>(config.connection.max_num_conns);
        // connection context pool (for aggregator and worker threads -> ctx_pool stuff):
        if (!main_workspace.arena.allocate(curr_client_workspace.conn_workspace.ctx_pool.pool, curr_client_workspace.conn_workspace.ctx_pool.max_num_conns)) return false;
