...
public:
    ...
    Config_Validation_Result parse_json_config(cJSON* JSON_config, bool* p_flag, Input_Source_List* inputs, const Fims_Object& defaults, std::vector<Fims_Object*>& multiple_inputs) override;
    ...
    // The opposit of execute. Will zero the components. Prevents stale behavior when the feat is not enabled.
    void clear_all_component_outputs(); 
