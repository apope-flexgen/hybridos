...
void features::Manual::handle_fims_set(std::string uri_endpoint, const cJSON& msg_value) {
    if (msg_value.type == cJSON_Number) {
        ...
        // Update slew rates
        if (manual_solar_kW_slew_rate.set_fims_int(uri_endpoint.c_str(), range_check(msg_value.valueint, 100000000, 1)))
            manual_solar_kW_slew.set_slew_rate(manual_solar_kW_slew_rate.value.value_int);
        if (manual_ess_kW_slew_rate.set_fims_int(uri_endpoint.c_str(), range_check(msg_value.valueint, 100000000, 1)))
            manual_ess_kW_slew.set_slew_rate(manual_ess_kW_slew_rate.value.value_int);
        if (manual_gen_kW_slew_rate.set_fims_int(uri_endpoint.c_str(), range_check(msg_value.valueint, 100000000, 1)))
