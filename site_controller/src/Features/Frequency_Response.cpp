...
void Frequency_Response::handle_fims_set(const fims_message& msg) {
    ...
    if (enable_mask.set_fims_int(uri_endpoint.c_str(), body_int)) return;
    if (baseload_cmd_kw.set_fims_float(uri_endpoint.c_str(), body_float)) return;
    if (freq_offset_hz.set_fims_float(uri_endpoint.c_str(), body_float)) return;
