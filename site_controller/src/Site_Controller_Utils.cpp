...
std::string string_format( const std::string& format, Args ... args )
{
    ...
    if( size_s <= 0 ){ throw std::runtime_error( "Error calculating formatted string size." ); }
    ...
    if( buf.get() == nullptr ){ throw std::runtime_error( "Memory allocation error during formatting." ); }
...
void increment_timespec_ms(timespec& time_struct, int ms_inc)
{
    ...
    if(ns_left_in_sec <= ns_inc)
    {   // Time increment causes overflow
    ...
    {   // Time increment does not cause overflow
...
{
    bool in_range = ((tolerance_value + (tolerance_value * tolerance_percent * .01)) >= actual_value &&
                     (tolerance_value - (tolerance_value * tolerance_percent * .01)) <= actual_value);
    return in_range;
}
...
float get_curve_cmd(float ind_var, std::vector<std::pair<float,float>> &curve_points)
{
    ...
    {
        cmd = ((curve_points[1].second - curve_points[0].second)
            / (curve_points[1].first - curve_points[0].first))
            * (ind_var - curve_points[0].first) + curve_points[0].second;
    }
    else if (ind_var >= curve_points[n-1].first)
    {
        cmd = ((curve_points[n-1].second - curve_points[n-2].second)
            / (curve_points[n-1].first - curve_points[n-2].first))
            * (ind_var - curve_points[n-1].first) + curve_points[n-1].second;
    }
    else
    {
        for (int i = 0; i < n - 1; i++)
        {
            ...
            {
                cmd = ((curve_points[i+1].second - curve_points[i].second)
                    / (curve_points[i+1].first - curve_points[i].first))
                    * (ind_var - curve_points[i+1].first) + curve_points[i+1].second;
                break;
            }
        }
    }
