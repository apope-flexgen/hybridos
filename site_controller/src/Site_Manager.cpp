...
void Site_Manager::configure_feature_objects(void)
{
    ...
    energy_arbitrage_feature.feature_vars = {
        &energy_arb_obj.price,
        &energy_arb_obj.threshold_charge_1,
        &energy_arb_obj.threshold_charge_2,
        &energy_arb_obj.threshold_dischg_1,
        &energy_arb_obj.threshold_dischg_2,
        &energy_arb_obj.soc_min_limit,
        &energy_arb_obj.soc_max_limit,
        &energy_arb_obj.max_charge_1,
        &energy_arb_obj.max_charge_2,
        &energy_arb_obj.max_dischg_1,
        &energy_arb_obj.max_dischg_2
    };
    ...
    site_export_target.enable_flag = &export_target_mode_enable_flag;
    site_export_target.feature_vars = {
        &export_target_load_enable_flag,
        &export_target_kW_cmd,
        &export_target_kW_slew_rate
    };
    // Manual Power Mode
    manual_power_mode.enable_flag = &manual_mode_enable_flag;
    manual_power_mode.feature_vars = {
        &manual_solar_kW_cmd,
        &manual_ess_kW_cmd
    };
    // Grid Target Mode
    grid_target.enable_flag = &grid_target_mode_enable_flag;
    grid_target.feature_vars = {
        &grid_target_kW_cmd,
        &ess_charge_support_enable_flag
    };
    ...
    absolute_ess.enable_flag = &absolute_ess_enable_flag;
    absolute_ess.feature_vars = {
        &absolute_ess_kW_cmd,
        &absolute_ess_direction_flag
    };
    ess_calibration.enable_flag = &ess_calibration_enable_flag;
    ess_calibration.feature_vars = {
        &ess_calibration_kW_cmd,
        &ess_calibration_soc_limits_enable,
        &ess_calibration_min_soc_limit,
        &ess_calibration_max_soc_limit,
        &ess_calibration_voltage_limits_enable,
        &ess_calibration_min_voltage_limit,
        &ess_calibration_max_voltage_limit,
        &ess_calibration_num_setpoint,
        &ess_calibration_num_limited,
        &ess_calibration_num_zero
    };
    ...
    runmode1_kW_features_list.push_back(&site_export_target);
    runmode1_kW_features_list.push_back(&manual_power_mode);
    runmode1_kW_features_list.push_back(&grid_target);
    runmode1_kW_features_list.push_back(&frequency_response_feature);
    runmode1_kW_features_list.push_back(&absolute_ess);
    ...
    //
    // no runmode2 active power features yet
    ...
    charge_control.feature_vars = {
        &ess_charge_control_kW_request,
        &ess_charge_control_target_soc,
        &ess_charge_control_kW_limit,
        &ess_charge_control_charge_disable,
        &ess_charge_control_discharge_disable
    };
    ...
    active_voltage.feature_vars = {
        &active_voltage_deadband,
        &active_voltage_droop_percent,
        &active_voltage_cmd,
        &active_voltage_actual_volts,
        &active_voltage_status_flag,
        &active_voltage_rated_kVAR
    };
    // Watt-Var Mode
    watt_var.enable_flag = &watt_var_mode_enable_flag;
    watt_var.feature_vars = {
        &watt_var_points
    };
    // Reactive Setpoint Mode
    reactive_setpoint.enable_flag = &reactive_setpoint_mode_enable_flag;
    reactive_setpoint.feature_vars = {
        &reactive_setpoint_kVAR_cmd,
        &reactive_setpoint_kVAR_slew_rate
    };
    // Power Factor Mode
    power_factor.enable_flag = &power_factor_mode_enable_flag;
    power_factor.feature_vars = {
        &power_factor_cmd
    };
    // Watt-PF Mode
    watt_pf.enable_flag = &watt_pf_mode_enable_flag;
    // add features to list
    ...
    runmode1_kVAR_features_list.push_back(&watt_pf);
    // Disable all features as part of initial configuration in case any are accidentally enabled
    ...
    //
    // POI Limits
    poi_limits.enable_flag = &poi_limits_enable;
    poi_limits.feature_vars = {
        &poi_limits_max_kW,
        &poi_limits_min_kW,
        &poi_limits_load_enable,
        &soc_poi_limits_enable,
        &soc_poi_target_soc,
        &soc_poi_limits_low_min_kW,
        &soc_poi_limits_low_max_kW,
        &soc_poi_limits_high_min_kW,
        &soc_poi_limits_high_max_kW
    };
    poi_limits.summary_vars = {
        &poi_limits_max_kW,
        &poi_limits_min_kW
    };
    // PFR
    ...
    watt_watt.feature_vars = {
        &watt_watt_points
    };
    // LDSS
    ldss.enable_flag = &ldss_enable_flag;
    ldss.feature_vars = {
        &ldss_priority_setting,
        &ldss_max_load_threshold_percent,
        &ldss_min_load_threshold_percent,
        &ldss_warmup_time,
        &ldss_cooldown_time,
        &ldss_start_gen_time,
        &ldss_stop_gen_time
    ...
    load_shed.feature_vars = {
        &load_shed_value,
        &load_shed_max_value,
        &load_shed_max_shedding_threshold,
        &load_shed_high_threshold,
        &load_shed_low_threshold,
        &load_shed_spare_ess_kw,
        &load_shed_increase_timer_ms,
        &load_shed_decrease_timer_ms,
        &load_shed_increase_display_timer,
        &load_shed_decrease_display_timer
    };
    // Solar Shed
    solar_shed.enable_flag = &solar_shed_enable;
    solar_shed.feature_vars = {
        &solar_shed_value,
        &solar_shed_max_value,
        &solar_shed_max_shedding_threshold,
        &solar_shed_high_threshold,
        &solar_shed_low_threshold,
        &solar_shed_spare_ess_kw,
        &solar_shed_increase_timer_ms,
        &solar_shed_decrease_timer_ms,
        &solar_shed_increase_display_timer,
        &solar_shed_decrease_display_timer
    };
    // ESS Discharge Prevention
    ess_discharge_prevention.enable_flag = &edp_enable;
    ess_discharge_prevention.feature_vars = {
        &edp_soc
    };
    // Aggregated Asset Limit
    agg_asset_limit.enable_flag = &agg_asset_limit_enable;
    agg_asset_limit.feature_vars = {
        &agg_asset_limit_kw
    };
    // Closed Loop Control
    ...
    // add features to list
    standalone_power_features_list.push_back(&poi_limits);
...
{
    if (var == NULL) bufJSON_StartObject(buf); // JSON_object {
    ...
    if (var == NULL) bufJSON_EndObject(buf); // } JSON_object
}
...
void Site_Manager::parse_default_vals(cJSON* JSON_defaults, Fims_Object &default_vals)
{
    ...
    else throw std::runtime_error("invalid var_type field");
        ...
        if (msg->nfrags == 3) // This is where you would allow user to set parameters
        {
            if (body_type == cJSON_Number) //process all numeric sets here
            {
                if (uType == features_uri) //all /feature numeric sets here
                {
                    if (strncmp(msg->pfrags[1], "active_power", strlen("active_power")) == 0)
                    {
                        ...
                        asset_priority_runmode1.set_fims_int(msg->pfrags[2], range_check(body_int, 5, 0));
                        asset_priority_runmode2.set_fims_int(msg->pfrags[2], range_check(body_int, 5, 0));
                        ...
                        //Grid Target 
                        grid_target_kW_cmd.set_fims_float(msg->pfrags[2], body_float);

                        //Site Export Target 
                        export_target_kW_cmd.set_fims_float(msg->pfrags[2], body_float);
                        //update slew rate internally if changed
                        if(export_target_kW_slew_rate.set_fims_int(msg->pfrags[2], range_check(body_int, 100000000, 1)))
                            export_target_kW_cmd_slew.set_slew_rate(export_target_kW_slew_rate.value.value_int);
                        ...
                        // Absolute Power, absolute value of command
                        absolute_ess_kW_cmd.set_fims_float(msg->pfrags[2], fabsf(body_float));
                    ...
                    {
                        // poi limits
                        poi_limits_max_kW.set_fims_float(msg->pfrags[2], fabsf(body_float));
                        poi_limits_min_kW.set_fims_float(msg->pfrags[2], -1*fabsf(body_float));
                        soc_poi_target_soc.set_fims_float(msg->pfrags[2], fabsf(body_float));
                        soc_poi_limits_low_min_kW.set_fims_float(msg->pfrags[2], -1 * fabsf(body_float));
                        soc_poi_limits_low_max_kW.set_fims_float(msg->pfrags[2], fabsf(body_float));
                        soc_poi_limits_high_min_kW.set_fims_float(msg->pfrags[2], -1 * fabsf(body_float));
                        soc_poi_limits_high_max_kW.set_fims_float(msg->pfrags[2], fabsf(body_float));
                        // pfr
            ...
            else if (body_type == cJSON_False || body_type == cJSON_True) //process all boolean sets here
            {
                if (uType == features_uri) //all /features boolean sets here
                {
                    ...
                    {
                        frequency_response.handle_fims_set(*msg);
                        absolute_ess_direction_flag.set_fims_bool(msg->pfrags[2], body_bool);
                        ...
                        // Site Export Target
                        export_target_load_enable_flag.set_fims_bool(msg->pfrags[2], body_bool);
                        valid_set = true;
                    }
                    else if (strncmp(msg->pfrags[1], "reactive_power", strlen("reactive_power")) == 0)
                    {
                    ...
                    else if (strncmp(msg->pfrags[1], "standalone_power", strlen("standalone_power")) == 0)
                    {
                        ...
                        poi_limits_load_enable.set_fims_bool(msg->pfrags[2], body_bool);
                        soc_poi_limits_enable.set_fims_bool(msg->pfrags[2], body_bool);
                        valid_set = true;
                    }
                    else if (strncmp(msg->pfrags[1], "site_operation", strlen("site_operation")) == 0)
                    {
...
void Site_Manager::get_values()
{
    ...
    asset_cmd.set_ess_actual_kW(ess_actual_kW.value.value_float);
    asset_cmd.set_feeder_actual_kW(feeder_actual_kW.value.value_float);
    asset_cmd.set_gen_actual_kW(gen_actual_kW.value.value_float);
    asset_cmd.set_solar_actual_kW(solar_actual_kW.value.value_float);

    asset_cmd.set_ess_actual_kVAR(ess_actual_kVAR.value.value_float);
    asset_cmd.set_gen_actual_kVAR(gen_actual_kVAR.value.value_float);
    asset_cmd.set_solar_actual_kVAR(solar_actual_kVAR.value.value_float);

    asset_cmd.set_ess_max_potential_kW( max_potential_ess_kW.value.value_float);
    asset_cmd.set_feeder_max_potential_kW(max_potential_feeder_kW.value.value_float);
    asset_cmd.set_gen_max_potential_kW(max_potential_gen_kW.value.value_float);
    asset_cmd.set_solar_max_potential_kW(max_potential_solar_kW.value.value_float);

    asset_cmd.set_ess_min_potential_kW(min_potential_ess_kW.value.value_float);
    asset_cmd.set_feeder_min_potential_kW(min_potential_feeder_kW.value.value_float);
    asset_cmd.set_gen_min_potential_kW(min_potential_gen_kW.value.value_float);
    asset_cmd.set_solar_min_potential_kW(min_potential_solar_kW.value.value_float);

    asset_cmd.set_ess_potential_kVAR(potential_ess_kVAR.value.value_float);
    asset_cmd.set_gen_potential_kVAR(potential_gen_kVAR.value.value_float);
    asset_cmd.set_solar_potential_kVAR(potential_solar_kVAR.value.value_float);

    asset_cmd.set_ess_start_first_kW(start_first_ess_kW.value.value_float);
    asset_cmd.set_gen_start_first_kW(start_first_gen_kW.value.value_float);
    asset_cmd.set_solar_start_first_kW(start_first_solar_kW.value.value_float);

    //reset all asset kW and kVAR cmds to 0
    asset_cmd.reset_kW_cmd();
    asset_cmd.reset_kVAR_cmd();
    ...
    site_kW_load.value.set(asset_cmd.get_site_kW_load());
    ...
    ess_charge_control_kW_request.value.set(pAssets->charge_control(ess_charge_control_target_soc.value.value_float,
                                                                    ess_charge_control_charge_disable.value.value_bool,
                                                                    ess_charge_control_discharge_disable.value.value_bool));
}
...
void Site_Manager::set_values()
{
    ...
    export_target_kW_cmd_slew.update_slew_target(ess_kW_cmd.value.value_float + solar_kW_cmd.value.value_float + gen_kW_cmd.value.value_float);
    ...
    // If closed loop control is enabled, subtract it's contribution from the slew target to prevent feedback loop
    float reactive_slew_contribution = reactive_power_closed_loop_enable.value.value_bool * reactive_power_closed_loop_regulator.offset * reactive_power_closed_loop_step_size_kW.value.value_float;
    reactive_setpoint_kVAR_cmd_slew.update_slew_target(ess_kVAR_cmd.value.value_float + solar_kVAR_cmd.value.value_float - reactive_slew_contribution);
    ...
    pAssets->start_first_gen(asset_cmd.get_gen_start_first_flag()); 
    pAssets->start_first_solar(asset_cmd.get_solar_start_first_flag());
...
{
    min_potential_ess_kW.value.set(asset_cmd.get_ess_min_potential_kW());
    max_potential_ess_kW.value.set(asset_cmd.get_ess_max_potential_kW());

    min_potential_feeder_kW.value.set(asset_cmd.get_feeder_min_potential_kW());
    max_potential_feeder_kW.value.set(asset_cmd.get_feeder_max_potential_kW());

    min_potential_gen_kW.value.set(asset_cmd.get_gen_min_potential_kW());
    max_potential_gen_kW.value.set(asset_cmd.get_gen_max_potential_kW());

    min_potential_solar_kW.value.set(asset_cmd.get_solar_min_potential_kW());
    max_potential_solar_kW.value.set(asset_cmd.get_solar_max_potential_kW());

    site_kW_load_inclusion.value.set(asset_cmd.get_site_kW_load_inclusion());
    site_kW_demand.value.set(asset_cmd.get_site_kW_demand());
}
...
{
    feature_kW_demand.value.set(asset_cmd.get_feature_kW_demand());
    site_kW_charge_production.value.set(asset_cmd.get_site_kW_charge_production());
    site_kW_discharge_production.value.set(asset_cmd.get_site_kW_discharge_production());
}
...
{    
	...
	poi_limits_enable.ui_enabled = true;
    ...
    for (auto feature : standalone_power_features_list)
    {
        ...
        if (feature == &poi_limits) {
            ...
            bool poi_limits_enabled_status = feature->enable_flag->value.value_bool && !soc_poi_limits_enable.value.value_bool;
            // For soc-based POI limits, enable its variables based on the enable status of the feature and the subfeature enable status
            bool soc_limits_enabled_status = feature->enable_flag->value.value_bool && soc_poi_limits_enable.value.value_bool;
            ...
            soc_poi_limits_enable.ui_enabled = feature->enable_flag->value.value_bool;
            // Update soc-based subfeature to mirror it's enabled status
            soc_poi_target_soc.ui_enabled = soc_limits_enabled_status;
            soc_poi_limits_low_min_kW.ui_enabled = soc_limits_enabled_status;
            soc_poi_limits_low_max_kW.ui_enabled = soc_limits_enabled_status;
            soc_poi_limits_high_min_kW.ui_enabled = soc_limits_enabled_status;
            soc_poi_limits_high_max_kW.ui_enabled = soc_limits_enabled_status;
        }
...
bool Site_Manager::configure_runmode1_kVAR_features(void)
{
    ...
    if (!initial_gc_kVAR_feature_found)
    {
        ...
        FPS_ERROR_LOG("Initial runmode1 kVAR feature does not pass available active power features mask.\n");
        return false;
    }
    return true;
}
...
bool Site_Manager::call_sequence_functions(const char* target_asset, const char* asset_cmd, Value_Object* value, int tolerance_percent)
{
    ...
    {
        //sync breaker state
        if (strcmp(asset_cmd, "get_sync_feeder_state") == 0)
            return_value = pAssets->get_sync_feeder_status() ? (value->type == Bool) && (value->value_bool == true)
                           : (value->type == Bool) && (value->value_bool == false);
        //get poi breaker state
        else if (strcmp(asset_cmd, "get_poi_feeder_state") == 0) {
        ...
        else if (strcmp(asset_cmd, "set_poi_feeder_state_open") == 0)
            return_value = pAssets->set_poi_feeder_state_open();
        //set poi breaker state closed
        else if (strcmp(asset_cmd, "set_poi_feeder_state_closed") == 0)
            return_value = pAssets->set_poi_feeder_state_closed();
        else if (strcmp(asset_cmd, "set_sync_feeder_close_permissive_remove") == 0)
            return_value = pAssets->set_sync_feeder_close_permissive_remove();
        else if (strcmp(asset_cmd, "set_sync_feeder_close_permissive") == 0)
    ...
    {
        if (strcmp(asset_cmd, "clear_faults") == 0)
            clear_faults();
        else if (strcmp(asset_cmd, "get_standby_flag") == 0)
    ...
    {
        if (strcmp(asset_cmd, "reset_load_shed") == 0)
        {
            int configured_shed_value = int(value->value_float);
            if (configured_shed_value < 0 || configured_shed_value > load_shed_max_value.value.value_int) {
                FPS_ERROR_LOG("Invalid load shed value %d provided, must be between 0 and %d\n", configured_shed_value, load_shed_max_value.value.value_int);
                command_found = false;
            } else {
    ...
    //check for specific feeder id for setting and getting specific feeder states
        ...
        if (found_feeder == nullptr) {
            ...
            if (strcmp(asset_cmd, "set_feeder_state_open") == 0) {
                return_value = pAssets->set_feeder_state_open(found_feeder);
            }
            //set_feeder_state_closed
            else if (strcmp(asset_cmd, "set_feeder_state_closed") == 0) {
            ...
            else if (strcmp(asset_cmd, "get_feeder_state") == 0) {
    ...
    {
        FPS_ERROR_LOG("call_function command not found. target_asset: %s asset_cmd: %s \n", target_asset, asset_cmd);
        return false;
    }
...
void Site_Manager::init_state(void)
{
    ...
    export_target_kW_cmd_slew.reset_slew_target();
    ...
    load_shed_calculator.min_offset = 0;
    ...
    solar_shed_calculator.min_offset = 0;
...
void Site_Manager::runmode1_state(void)
{
    ...
    if (_ess_charge_control_enable_flag.value.value_bool)
        asset_cmd.set_ess_kW_request(range_check(ess_charge_control_kW_request.value.value_float,
                                                 ess_charge_control_kW_limit.value.value_float,
                                                 -1 * ess_charge_control_kW_limit.value.value_float));
    ...
    if (pfr_enable_flag.value.value_bool)
        primary_frequency_response(asset_cmd.get_site_kW_demand());

    // limit power values based on the amount of value that the POI can legally/physically handle
    if(poi_limits_enable.value.value_bool)
        apply_poi_limits();
    ...
    {
        asset_cmd.ess_overload_support(grid_target_kW_cmd.value.value_float);
    }
    ...
    asset_cmd.set_gen_start_first_flag((asset_cmd.get_gen_start_first_flag()) || (soc_avg_running.value.value_float <= start_first_gen_soc.value.value_float));

    // start any "startable" ESS/solar and take any that are in standby out of standby
    pAssets->start_available_ess_solar();
...
void Site_Manager::shutdown_state(void)
{
    ...
    asset_cmd.set_ess_start_first_flag(false);
    asset_cmd.set_gen_start_first_flag(false);
    asset_cmd.set_solar_start_first_flag(false);
...
void Site_Manager::process_runmode1_kW_feature()
{
    ...
    else if (export_target_mode_enable_flag.value.value_bool)
    {
        asset_cmd.site_export_target_mode(export_target_load_enable_flag.value.value_bool, &export_target_kW_cmd_slew, export_target_kW_cmd.value.value_float);
    }
    ...
    //GRID TARGET MODE determine site demand to be distributed to assets (plus baseload)
    else if (grid_target_mode_enable_flag.value.value_bool)
    {
        asset_cmd.grid_target_mode(grid_target_kW_cmd.value.value_float);
    }
    ...
    // Absolute ESS Mode takes an ESS kW_cmd and routes those commands through dispatch and charge control
    else if (absolute_ess_enable_flag.value.value_bool)
    {
        asset_cmd.absolute_ess(absolute_ess_direction_flag.value.value_bool, absolute_ess_kW_cmd.value.value_float);
    }
...
void Site_Manager::execute_frequency_response_feature() {
    ...
    Frequency_Response_Inputs ins {
        asset_cmd.get_ess_max_potential_kW(),
        asset_cmd.get_ess_min_potential_kW(),
        get_ess_total_rated_active_power(),
        site_frequency.value.value_float,
        current_time
    };
    // call frequency response algorithm
    Frequency_Response_Outputs outs = frequency_response.aggregate_response_components(ins);
    // set outputs
    asset_cmd.set_ess_max_potential_kW(outs.ess_max_potential);
    asset_cmd.set_ess_min_potential_kW(outs.ess_min_potential);
    asset_cmd.set_site_kW_demand(outs.output_kw);
    ...
    asset_cmd.set_solar_max_potential_kW(0);
}
...
void Site_Manager::energy_arbitrage_helper() {
    Energy_Arbitrage_Inputs Energy_Arbitrage_Parameters {
        ...
        asset_cmd.get_solar_max_potential_kW()
    };
    ...
    if (!active_alarm_array[3] && outputs.error_code == 1) {
        ...
        asset_cmd.set_solar_max_potential_kW(outputs.solar_max_potential_kW);
        // Feature output assigned to ESS
        asset_cmd.set_ess_kW_request(outputs.ess_kW_power);
        // Solar fully uncurtailed, but reserve a portion equal to ESS charge request (negative feature output) if present
        asset_cmd.set_solar_kW_request(outputs.solar_kW_request);
    }

    asset_cmd.set_site_kW_load_inclusion(true);
}
 ...
 * 
 * Note: right now there is no "list" because there is only one runmode2 calculation, and that calculation might end up
 * being considered a hard-coded protection.
 */
void Site_Manager::process_runmode2_kW_feature()
{
    ...
    asset_cmd.set_ess_kW_request(asset_cmd.get_ess_min_potential_kW());
...
void Site_Manager::process_runmode1_kVAR_feature()
{
    ...
    if (active_voltage_mode_enable_flag.value.value_bool)
    {
        ...
        asset_cmd.active_voltage_mode(active_voltage_deadband.value.value_float, active_voltage_cmd.value.value_float, active_voltage_actual_volts.value.value_float,
                                      active_voltage_droop_percent.value.value_float, active_voltage_rated_kVAR.value.value_float);

        //set status flag
        active_voltage_status_flag.value.set(asset_cmd.get_site_kVAR_demand() != 0);
    ...
    {
        asset_cmd.set_site_kVAR_demand(0);

        //this mode does uses power factor control
        asset_pf_flag = true;
    }
    //WATT-PF MODE tbd
    else if (watt_pf_mode_enable_flag.value.value_bool)
    {
        asset_cmd.set_site_kVAR_demand(0);
    ...
    site_kVAR_demand.value.set(asset_cmd.get_site_kVAR_demand());
}
 ...
 */
void Site_Manager::apply_poi_limits()
{
    float poi_demand = asset_cmd.get_site_kW_demand();
    // Determine whether load should be taken into account (includes uncontrollable)
    float load_correction = (poi_limits_load_enable.value.value_bool && asset_cmd.get_site_kW_load_inclusion()) * asset_cmd.get_site_kW_load();
    // Remove the aggregate power that exists outside the control of Site Controller
    poi_demand -= load_correction;

    // If importing, determine the actual amount of demand that will not be cancelled out by production from within the site
    if (poi_demand < 0)
        poi_demand = less_than_zero_check(poi_demand + asset_cmd.calculate_internal_charge_compensation(asset_priority_runmode1.value.value_float));

    float max_poi_limit = poi_limits_max_kW.value.value_float;
    float min_poi_limit = poi_limits_min_kW.value.value_float;
    ...
    if (soc_poi_limits_enable.value.value_bool)
    {
        ...
        if (soc_avg_running.value.value_float <= soc_poi_target_soc.value.value_float) {
            max_poi_limit = soc_poi_limits_low_max_kW.value.value_float;
            min_poi_limit = soc_poi_limits_low_min_kW.value.value_float;
        // Only apply over limits if ESS are above the configured threshold
        } else {
            max_poi_limit = soc_poi_limits_high_max_kW.value.value_float;
            min_poi_limit = soc_poi_limits_high_min_kW.value.value_float;
        }
    }

    float final_correction = (range_check(poi_demand, max_poi_limit, min_poi_limit) - poi_demand);

    asset_cmd.set_site_kW_demand(asset_cmd.get_site_kW_demand() + final_correction);
    // Do not take uncontrolled correction into account in the feeder limit as it represents the actual import of power of the POI
    asset_cmd.set_feeder_max_potential_kW(std::min(asset_cmd.get_feeder_max_potential_kW(), -1.0f * min_poi_limit));
    ...
    // Load must be included (conditional on params) as it would be implicit in the requested demand were the limit to be used
    total_site_kW_charge_limit.value.set(range_check(total_site_kW_charge_limit.value.value_float, max_poi_limit + load_correction, min_poi_limit + load_correction));
    total_site_kW_discharge_limit.value.set(range_check(total_site_kW_discharge_limit.value.value_float, max_poi_limit + load_correction, min_poi_limit + load_correction));
}
...
void Site_Manager::dispatch_active_power(int asset_priority)
{
    ...
    if (asset_cmd.get_site_kW_load_inclusion())
        aggregate_dispatch += asset_cmd.dispatch_site_kW_discharge_cmd(asset_priority, asset_cmd.get_site_kW_load(), LOAD);

    // Process and dispatch charge production
    aggregate_dispatch += asset_cmd.dispatch_site_kW_charge_cmd(asset_priority, charge_dispatch_solar_enable_flag.value.value_bool,
                                                                                charge_dispatch_gen_enable_flag.value.value_bool,
                                                                                charge_dispatch_feeder_enable_flag.value.value_bool);

    // Process and dispatch remaining discharge production to each asset type
    aggregate_dispatch += asset_cmd.dispatch_site_kW_discharge_cmd(asset_priority, asset_cmd.get_site_kW_discharge_production(), REQUESTS);
    aggregate_dispatch += asset_cmd.dispatch_site_kW_discharge_cmd(asset_priority, asset_cmd.get_site_kW_discharge_production(), DEMAND);
...
void Site_Manager::set_asset_power_commands()
{
    ...
    ess_kVAR_cmd.value.set(ess_kVAR_cmd_slew.get_slew_target(asset_cmd.get_ess_kVAR_cmd()));
    gen_kVAR_cmd.value.set(gen_kVAR_cmd_slew.get_slew_target(asset_cmd.get_gen_kVAR_cmd()));
    solar_kVAR_cmd.value.set(solar_kVAR_cmd_slew.get_slew_target(asset_cmd.get_solar_kVAR_cmd()));
}

//W-VAr mode function
void Site_Manager::watt_var_mode(float active_power, std::vector<std::pair<float,float>>& curve)
{
    asset_cmd.set_site_kVAR_demand(get_curve_cmd(active_power, curve));
}
...
void Site_Manager::primary_frequency_response(float kW_cmd)
{
    ...
    float reference_hz = (freq_input > pfr_site_nominal_hz.value.value_float)
                            ? pfr_site_nominal_hz.value.value_float + pfr_deadband.value.value_float
                            : pfr_site_nominal_hz.value.value_float - pfr_deadband.value.value_float;
    ...
    asset_cmd.set_site_kW_demand(calculated_response);
    total_site_kW_charge_limit.value.set(
        range_check(total_site_kW_charge_limit.value.value_float + response_kW, 0.0f, pfr_limits_min_kW.value.value_float)
    );
    total_site_kW_discharge_limit.value.set(
        range_check(total_site_kW_discharge_limit.value.value_float + response_kW, pfr_limits_max_kW.value.value_float, 0.0f)
    );
}
...
{
    float new_site_demand = get_curve_cmd( asset_cmd.get_site_kW_demand(), watt_watt_curve);

    // Set asset cmd value
    asset_cmd.set_site_kW_demand(new_site_demand);
}
...
{
    pAssets->update_ldss_settings(LDSS_Settings{
        ldss_enable_flag.value.value_bool,
        (LDSS_Priority_Setting)ldss_priority_setting.value.value_int,
        ldss_max_load_threshold_percent.value.value_float,
        ldss_min_load_threshold_percent.value.value_float,
        ldss_warmup_time.value.value_int,
        ldss_cooldown_time.value.value_int,
        ldss_start_gen_time.value.value_int,
        ldss_stop_gen_time.value.value_int
    });
}
...
void Site_Manager::calculate_solar_shed()
{
    ...
    solar_max = std::min(solar_max, asset_cmd.get_solar_max_potential_kW());

    asset_cmd.set_solar_max_potential_kW(solar_max);
...
{
    float current_cmd = asset_cmd.get_site_kW_demand();
    ...
    float total_correction = _active_power_closed_loop_manual_offset_kW.value.value_float + active_power_closed_loop_regulator.offset * active_power_closed_loop_step_size_kW.value.value_float;

    // Take the product of the offset and step size as the total correction applied to the active power feature's command
    asset_cmd.set_site_kW_demand(current_cmd + total_correction);
    // Give enough headroom to accomodate the full correction if needed
    ...
    asset_cmd.set_feeder_max_potential_kW(std::max(asset_cmd.get_feeder_max_potential_kW(), current_cmd + zero_check(-1.0f * total_correction)));
...
void Site_Manager::calculate_reactive_power_closed_loop_offset()
{
    ...
    float reactive_power_feature_cmd = asset_cmd.get_site_kVAR_demand();
    // Regulate based on configured deadbands compared to difference in dispatched commands (steady state) and difference in command and POI (accuracy)
    bool command_accepted = reactive_power_closed_loop_regulator.regulate_variable_offset(
        std::abs(reactive_power_feature_cmd - prev_reactive_power_feature_cmd), poi_value - reactive_power_feature_cmd
    );
    // Take the product of the offset and step size as the total correction applied to the reactive power feature's command
    float total_correction = reactive_power_closed_loop_regulator.offset * reactive_power_closed_loop_step_size_kW.value.value_float;
    asset_cmd.set_site_kVAR_demand(asset_cmd.get_site_kVAR_demand() + total_correction);
    ...
    if (command_accepted)
        prev_reactive_power_feature_cmd = reactive_power_feature_cmd;
}
...
void Site_Manager::prevent_ess_discharge()
{
    ...
    {
        if (asset_cmd.get_ess_max_potential_kW() > 0)
        {
        ...
        if (asset_cmd.get_ess_min_potential_kW() > 0)
        { 
...
{
    float solar_power = asset_cmd.get_solar_actual_kW() + uncontrolled_solar_kw;
    ...
    if (ess_limit < asset_cmd.get_ess_max_potential_kW()) {
        ...
        if (asset_cmd.get_ess_min_potential_kW() > max_potential_ess_kW.value.value_float) {
...
int Site_Manager::get_num_ess_available()
{
    return num_ess_available;
}

int Site_Manager::get_num_ess_running()
{
    return num_ess_running;
}
