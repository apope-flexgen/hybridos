...
Solar_Manager::~Solar_Manager() {
    if (!pSolar.empty())
        for (size_t j = 0; j < pSolar.size(); j++)
            if (pSolar[j] != NULL)
                delete pSolar[j];
}
...
void Solar_Manager::calculate_solar_total_potential_active_power(void) {
    switch (solar_curtailment_state) {
        ...
        case partial_curtailment:  // report prime inverter power, scaled by units controllable
            for (int i = 0; i < numParsed; i++) {
                if (pSolar[i]->is_controllable()) {  // if a non-prime inverter is generating less power than the prime inverter, give it the prime inverter's generation as its limit
...
std::tuple<float, float> Solar_Manager::calculate_solar_reactive_power_commands(void) {
    ...
    {
        if (sps != brs) {
            spq = ((pow(sps, 2) * tq) - sqrt((pow(brs, 2) * pow(sps, 2) * pow(spp, 2)) + (pow(brs, 2) * pow(sps, 2) * pow(bcp, 2)) + (pow(brs, 2) * pow(sps, 2) * pow(tq, 2)) - (pow(brs, 4) * pow(spp, 2)) - (pow(sps, 4) * pow(bcp, 2)))) /
                  (pow(sps, 2) - pow(brs, 2));
        } else {
