...
bool Type_Configurator::configure_ranged_templated_asset(void)
{
    ...
    for (const auto& asset_num : asset_list)
    {
        ...
        auto check_id = asset_config.asset_id_to_asset_number_map[asset_id->valuestring].insert
            (std::pair<int, bool>(asset_config.asset_num, false));
        auto check_name = asset_config.asset_name_to_asset_number_map[asset_name->valuestring].insert
            (std::pair<int, bool>(asset_config.asset_num, false));
        if (!check_id.second || !check_name.second)
        {
...
int Type_Configurator::entry_is_template(cJSON* asset_array_entry)
{
    ...
    if (num_instances_obj->valueint != 1) {
        FPS_ERROR_LOG("Error parsing asset with ID %s and name %s: if 'number_of_instances' is greater than 1, the asset entry is considered templated and must have a wildcard character in the ID and name.", asset_id->valuestring, asset_name->valuestring);
        return TEMPLATING_ERROR;
    }
    return NON_TEMPLATE;
}
    ...
    for (int i = 0; i < cJSON_GetArraySize(range); ++i)
    {
        ...
        if (range_entry->type == cJSON_String) {
            ...
            if (range_entry_str.find("..") != range_entry_str.rfind("..")) {
                FPS_ERROR_LOG("Error parsing asset %s: 'range' array must contain only numbers or strings \
        ...
        {
            FPS_ERROR_LOG("Error parsing asset %s: 'range' array must contain only numbers or \
            strings.", cJSON_GetObjectItem(asset_array_entry, "name")->valuestring);
            return std::vector<int>();
        }
    }
