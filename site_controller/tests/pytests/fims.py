...
def fims_set(uri: str, value: Any) -> Dict:
...
def fims_threaded_pub(uri: str, value: Any, publish_frequency=.2):
    """
    ...
    t.start()
    time.sleep(publish_frequency * 8)  # long enough for >=8 pubs
...
def poll_until_uri_is_at_value(uri: str, expected: float, tolerance: float = 0, timeout_seconds: int = 10) -> bool:
    for _ in range(timeout_seconds, 0, -1):
        actual = fims_get(uri)
