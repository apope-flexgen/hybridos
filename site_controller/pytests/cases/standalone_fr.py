    ...
    Setup(
        ...
        pre_lambda=[
            lambda: Site_Controller_Instance.get_instance().mig.upload(add_underfrequency_configs(
                trigger_freq_hz=59, droop_freq_hz=57, active_cmd_kw=-1000, slew_rate_kw=1000, cooldown_slew_rate_kw=1000)),
...
@ parametrize("test", [
    ...
    Setup(
        ...
        [],
        pre_lambda=[
            lambda: Site_Controller_Instance.get_instance().mig.upload(
                add_underfrequency_configs(
                    trigger_freq_hz=59, droop_freq_hz=57, active_cmd_kw=3000, instant_recovery_freq_hz=60, trigger_duration_sec=10,
                    recovery_freq_hz=59.5, recovery_duration_sec=5, cooldown_duration_sec=10)),
            lambda: Site_Controller_Instance.get_instance().restart_site_controller(True)
        ]
    ),
    Steps(
...
@ parametrize("test", [
    ...
    Steps(
        ...
        [
            Flex_Assertion(Assertion_Type.approx_eq, "/features/active_power/site_kW_load",
                           1050, wait_secs=7),  # load being annoying swinging use 7 seconds
            Flex_Assertion(Assertion_Type.approx_eq, "/features/active_power/feeder_actual_kW", 50, tolerance=0.5)
        ]
    ),
    Steps(
    ...
    Steps(
        ...
        [
            ...
            Flex_Assertion(Assertion_Type.approx_eq, "/features/active_power/feeder_actual_kW", -1000, tolerance=0.1, wait_secs=0),
            Flex_Assertion(Assertion_Type.approx_eq, "/features/standalone_power/uf_pfr_in_recovery",
                           True, wait_secs=0),  # will already be ticking the recovery timer
            Flex_Assertion(Assertion_Type.approx_eq, "/features/standalone_power/uf_pfr_in_cooldown", True,
                           wait_secs=3),  # timer finishes at 5 seconds so at 6 seconds we should be in_cooldown
            Flex_Assertion(Assertion_Type.approx_eq, "/features/active_power/feeder_actual_kW", 50,
                           tolerance=0.5, wait_secs=10)  # make sure we aren't pushing extra power anymore
        ]
    ),
    Teardown(
        {
...
@ parametrize("test", [
    ...
    Steps(
        ...
        [
            ...
            Flex_Assertion(Assertion_Type.approx_eq, "/features/active_power/feeder_actual_kW", -850,
                           tolerance=0.1, wait_secs=0),  # cooldown_slew_rate_kw * 3
        ]
    ),
    Steps(
        {
        ...
        [
            ...
            Flex_Assertion(Assertion_Type.approx_eq, "/features/active_power/feeder_actual_kW", -600,
                           tolerance=0.5, wait_secs=0),  # 850 - cooldown_slew_rate_kw * 5
        ]
    ),
    Teardown(
        {
...
# demonstrate event response correctly operates with changes on recovery_frequency_hz and instant_recovery_frequency_hz
@ fixture
@ parametrize("test", [
    Setup(
        "live_updates_recovery_frequency",
        {},
        [],
        # under-frequency event:
        # droop < trigger < target frequency (60Hz)
        pre_lambda=[
            lambda: Site_Controller_Instance.get_instance().mig.upload(add_underfrequency_configs(
                trigger_freq_hz=1,  # we will trigger it move out of the way
                droop_freq_hz=0,  # must be less than trigger
                active_cmd_kw=1000,  # simple small
                instant_recovery_freq_hz=62,  # place instant_recovery at 62, it will be changed later
                trigger_duration_sec=15,  # FR will last for 15 seconds
                cooldown_duration_sec=60,  # LONG cooldown to give time to play with slow ramp down
                recovery_freq_hz=61,
    ...
    Steps(
        ...
        [
            ...
            Flex_Assertion(Assertion_Type.approx_eq, "/features/active_power/active_power_setpoint_load_method", 0, wait_secs=0),
            Flex_Assertion(Assertion_Type.approx_eq, "/features/standalone_power/fr_mode_enable_flag", True, wait_secs=0),
        ]
    ),
    # Force start event
    # Event is now active
    Steps(
        ...
        [
            Flex_Assertion(Assertion_Type.approx_eq, "/features/standalone_power/uf_pfr_active_response_status", True, wait_secs=1),
            Flex_Assertion(Assertion_Type.approx_eq, "/features/standalone_power/uf_pfr_in_recovery", False, wait_secs=0),
        ]
    ),
    # Stop force start
    # Set recovery frequency to 59, This being below the site frequency of 60 Hz
    # Enters recovery state
    Steps(
        {
            "/features/standalone_power/uf_pfr_force_start": False,
            "/features/standalone_power/uf_pfr_recovery_freq_hz": 59,
        },
        [
            # verify it is starting to recover
            Flex_Assertion(Assertion_Type.approx_eq, "/features/standalone_power/uf_pfr_active_response_status", True, wait_secs=1),
            Flex_Assertion(Assertion_Type.approx_eq, "/features/standalone_power/uf_pfr_in_recovery", True, wait_secs=0),
        ]
    ),
    # Set instant recovery to 59, This being less than site frequency of 60
    # Cooldown phase begins and is no longer in recovery state
    Steps(
        {
            "/features/standalone_power/uf_pfr_instant_recovery_freq_hz": 59,
        },
        [
            Flex_Assertion(Assertion_Type.approx_eq, "/features/standalone_power/uf_pfr_active_response_status", False, wait_secs=1),
            Flex_Assertion(Assertion_Type.approx_eq, "/features/standalone_power/uf_pfr_in_recovery", False, wait_secs=0),
            Flex_Assertion(Assertion_Type.approx_eq, "/features/standalone_power/uf_pfr_in_cooldown", True, wait_secs=0),
        ]
    ),
    Teardown(
        ...
        [
            Flex_Assertion(Assertion_Type.approx_eq, "/features/active_power/site_frequency", 60, wait_secs=1),
            Flex_Assertion(Assertion_Type.approx_eq, "/features/standalone_power/fr_mode_enable_flag", False, wait_secs=0),
            Flex_Assertion(Assertion_Type.approx_eq, "/features/active_power/active_power_setpoint_load_method", 0, wait_secs=0),
...
def test_live_updates_recovery_frequency(test):
    return test


# demonstrate that frequency variables are settable
@ fixture
@ parametrize("test", [
    Setup(
        "frequency_variable_sets",
        {},
        [],
        pre_lambda=[
            lambda: Site_Controller_Instance.get_instance().mig.upload(add_underfrequency_configs(
                trigger_freq_hz=58,  # we will trigger it move out of the way
                droop_freq_hz=55,  # must be less than trigger
                active_cmd_kw=1000,  # simple small
                instant_recovery_freq_hz=62,  # place instant_recovery at 62 will change later
                trigger_duration_sec=15,  # FR will last for 15 seconds
                cooldown_duration_sec=60,  # LONG cooldown to give time to play with slow ramp down
                recovery_freq_hz=61,  # seems reasonable, but won't be able to recover
                recovery_duration_sec=60,
                slew_rate_kw=1000,
                cooldown_slew_rate_kw=50  # VERY SLOW cooldown ramp
            )),
    ...
    Steps(
        {
            "/features/standalone_power/uf_pfr_trigger_freq_hz": 5,  # Should not set to 5 due to being under previous droop_freq_hz value; should retain past value
            "/features/standalone_power/uf_pfr_droop_freq_hz": 40,
            "/features/standalone_power/uf_pfr_recovery_freq_hz": 45,
            "/features/standalone_power/uf_pfr_instant_recovery_freq_hz": 47,
        },
        [
            Flex_Assertion(Assertion_Type.approx_eq, "/features/standalone_power/uf_pfr_trigger_freq_hz", 58, wait_secs=1),  # Retained past value
            Flex_Assertion(Assertion_Type.approx_eq, "/features/standalone_power/uf_pfr_droop_freq_hz", 40, wait_secs=0),
            Flex_Assertion(Assertion_Type.approx_eq, "/features/standalone_power/uf_pfr_recovery_freq_hz", 45, wait_secs=0),
            Flex_Assertion(Assertion_Type.approx_eq, "/features/standalone_power/uf_pfr_instant_recovery_freq_hz", 47, wait_secs=0),
        ]
    ),
    Steps(
        {
            # "/features/standalone_power/uf_pfr_trigger_freq_hz": 58,
            "/features/standalone_power/uf_pfr_droop_freq_hz": 60,  # Should not set to 60 due to being above previous trigger_freq_hz; should retain past value
        },
        [
            Flex_Assertion(Assertion_Type.approx_eq, "/features/standalone_power/uf_pfr_trigger_freq_hz", 58, wait_secs=1),
            Flex_Assertion(Assertion_Type.approx_eq, "/features/standalone_power/uf_pfr_droop_freq_hz", 40, wait_secs=0),  # Retained past value
        ]
    ),
    Steps(
        {
            # "/features/standalone_power/uf_pfr_trigger_freq_hz": 58,
            "/features/standalone_power/uf_pfr_droop_freq_hz": 50,
        },
        [
            Flex_Assertion(Assertion_Type.approx_eq, "/features/standalone_power/uf_pfr_trigger_freq_hz", 58, wait_secs=1),
            Flex_Assertion(Assertion_Type.approx_eq, "/features/standalone_power/uf_pfr_droop_freq_hz", 50, wait_secs=0),
        ]
    ),
    Steps(
        {
            "/features/standalone_power/uf_pfr_trigger_freq_hz": 51,
            # "/features/standalone_power/uf_pfr_droop_freq_hz": 50,
        },
        [
            # Trigger_freq_hz is not set to 5 due to being under prev droop_freq_hz
            Flex_Assertion(Assertion_Type.approx_eq, "/features/standalone_power/uf_pfr_trigger_freq_hz", 51, wait_secs=1),
            Flex_Assertion(Assertion_Type.approx_eq, "/features/standalone_power/uf_pfr_droop_freq_hz", 50, wait_secs=0),
        ]
    ),
    Steps(
        {
            "/features/standalone_power/uf_pfr_trigger_freq_hz": 61,
            "/features/standalone_power/uf_pfr_droop_freq_hz": 62,
        },
        [
            # Attempt to change under-frequency to over-frequency will fail due to values being over the target_freq_hz
            Flex_Assertion(Assertion_Type.approx_eq, "/features/standalone_power/uf_pfr_trigger_freq_hz", 51, wait_secs=1),
            Flex_Assertion(Assertion_Type.approx_eq, "/features/standalone_power/uf_pfr_droop_freq_hz", 50, wait_secs=0),
        ]
    ),
...
def test_frequency_variable_sets(test):
    return test
