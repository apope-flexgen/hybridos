...
::Usage
	/path/to/psm /path/to/config [OPT Arguments]

	PSM will DEFAULT TO DBI LOAD BEHAVIOR unless provided a -f flag or if dbi load fails.
...
::Arguments
	...
	-u, --uri "collection":
		Run PSM with configuration read from a dbi collection. The following files are expected:
			psm_tree.json 
			psm_dflt.json
			psm_*_updt.json (optional, see confluence documentation)
		if "collection" is specified, psm will attempt to get files from /dbi/collection/
		otherwise the collection will default to /dbi/psm
		Cannot be used with -f
		If neither -u nor -f are provided then psm will first default to "-u" behavior
			if unable to connect to dbi, or if empty documents are received for the psm_tree or psm_dflt, then "-f" behavior will be attempted
	
	-f, --file "/path/to/config"
		Runs PSM with configurations stored in a psm.json at the filepath provided
		If no filepath provided, psm will default to searching for a filepath in the second command line argument (legacy behavior)
			If no psm.json found at this path or if second argument is not a valid filepath, psm will finally look in the current working directory
...
var configDocs map[string]bool 		//holds the name of the configuration documents in the /dbi/psm collection. 
var updtDocs []string			//holds the name of the psm_*_updt.json documents (subset of configDogs)
var collection string = "psm" //dbi collection. default is /dbi/psm, but can be overwritten with --uri ["collection"] flag
var cpath string
...
// goroutine in main() in psm.go handles the success or failure, and then replies back on configsync channel with results.
// if successful, the configs are placed in configMap, updateMap, or treeMap as appropriate. 
func arbitrateConfig() bool {
	var numUpdt int					//number of psm_*_updt.json files to parse. 
	...
	for doc,_ := range configDocs{
		match,_ := regexp.Match("^psm_[a-zA-Z]+_updt$", []byte(doc))
		if match {
			numUpdt++
	...
	f.Send(fims.FimsMsg {
		Method:  "get",
		Uri:     uristr,
		Replyto: "/psm/config_tree",
	})
	select {
	case ret := <- configsync:
		if ret == CONFIG_RECEIVE_FAILURE {
			log.Println("Unable to receive psm asset tree. Attempting to configure using local configuration file.")
			return false
		} else {
			configDocs[doc] = true
		}
		log.Println("Received psm asset tree")
	case <- time.After(5 * time.Second):
...
// init() initializes the data structures and configurations for PSM.
...
func runConfig() {
	...
	} else if !fromDBI { //if -u or --uri flag were provided, then do not allow psm to attempt to find a 'psm.json'
		readConfig(&config)
	} else {
		log.Fatalf("Unable to configure psm. Please check configurations and try again (maybe try the --help flag). Exiting...")
	}
	// Send completely expanded and parsed config set to dbi so it can be grabbed and easily analyzed.
	f.Send(fims.FimsMsg {
		Method:  "set",
		Uri:     "/dbi/psm/fullconfig",
		Body: 	configMap,
	})
...
func main() {
	...
	go func() {
		for {
			...
			//For now this is done here in the main recieve goroutine. If PSM has to recieve many fims messages this may slow down, so move this outside
			//It's nice to have here so that configs can be updated during run in the future. 
			if (msg.Method == "set" && ((msg.Uri == "/psm/config_dflt") || (msg.Uri == "/psm/config_update") || (msg.Uri == "/psm/config_tree") || (msg.Uri == "/psm/configdocs"))) {
				...
				//otherwise continue so that the psm process does not block if these uris get set to during operation. 
				if ok {
					select {
...
func receiveFims(msg fims.FimsMsg, fimsMap map[string]interface{}) {
	...
	if msg.Nfrags > 0 {
		...
		} else if msg.Frags[0] == "psm" || msg.Frags[0] == "twins" {
			switch msg.Nfrags {
			case 1:
				if msg.Method == "get" && msg.Replyto != "" {
					f.Send(fims.FimsMsg{
