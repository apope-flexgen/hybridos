...
// treeCfgNode is a struct to unmarshal the power system tree in psm.json into
...
// cfg is the struct to unmarshal all of psm.json into
type cfg struct {
	UpdateRate     uint
	PublishRate    uint
	TimeMultiplier uint
		...
		for _, id := range updt.ids {
			...
			for variable, value := range updt.vars {
				...
				if _, ok := configMap[updt.assetType].([]interface{}); !ok {
					log.Fatalln("Could not find update asset type [", updt.assetType, "] in configMap. Check spelling of top level asset type in psm_*_updt.json files. Exiting")
				}
...
func createIDS(tmplMap map[string]interface{}) ([]string, bool) {
	...
	// Allows the user to provide a single asset in the 'vars' field without requiring the user to add a 'reps' field.
	if reps, ok = tmplMap["reps"]; !ok {
		if _, ok = varsMap["id"]; ok {
			if temp, okay := varsMap["id"].(string); !okay {
				log.Println("[CreateIDs] Could not parse 'id' field in 'vars' object. Expected a string")
			} else {
				return []string{temp}, okay
			}
			if verbose {
				log.Println("Reps not added, includes Static token", varsMap["id"])
			}
		} else {
			return []string{""}, false
		}
...
// createConfig uses the tree structure to expand default configurations into a full 'psm.json'-like configuration containing variables and expressions
...
func readConfig(config *cfg) {
	...
	if cpath == "" { //use path from command line flag if specified, otherwise look for it in second argument (legacy load path for backwards compatibility)
		if len(os.Args) < 2 {
			...
			if os.IsNotExist(err) {
				log.Fatal("PSM configuration file not found at: ", os.Args[1])
			} else if info.IsDir() {
				files, err := os.ReadDir(os.Args[1])
				if err != nil {
					log.Fatalf("No files found in directory:", os.Args[1])
				}
				for i, file := range files {
					if file.Name() == "psm.json" || file.Name() == "twins.json" || file.Name() == "PSM.json" {
						cpath = path.Join(os.Args[1], file.Name())
						break
					}
					if i == len(files)-1 {
						log.Fatalf("No PSM configuration file found in directory:", os.Args[1])
					}
				}
			} else {
				cpath = os.Args[1]
	...
	log.Println("Reading in local file:", cpath)
...
func createPwrTree() {
	_, ok := treeMap["root"]
	if !ok {
		log.Fatal("Expecting psm tree to start with object named \"root\". Object not found, exiting")
	}
...
func receiveConfig(msg fims.FimsMsg) bool {
	...
	if msg.Uri == "/psm/config_dflt" {
	...
	} else if msg.Uri == "/psm/config_update" {
		...
		for k, v := range out { //append new values to updateMap for each psm_*_updt.json
			updateMap[k] = v
		}
	} else if msg.Uri == "/psm/config_tree" {
	...
	} else if msg.Uri == "/psm/configdocs" {
...
func combineConfigs(cfgMap map[string]interface{}, updtMap map[string]interface{}) map[string]interface{} {
	...
	for k, v := range updtMap {
		...
		if val.Kind() == reflect.Slice {
			...
			cfgMap[k] = v //This is for updateRate, etc. that live at the top level in psm.json
		}
	}
	return cfgMap
}
...
func buildState(config *cfg, fimsMap map[string]interface{}) (treeNode, map[string]asset, error) {
	...
	// assetsMap is populated by looking through assets.json for "psm" directives
		...
		for j := 0; j < thisAsset.NumField(); j++ {
			...
			if thisField.CanInterface() {
				...
				for k := 0; k < len(fieldStrings); k++ {
					found := false
					// check if there is a psm_id from assets.json associated with this field for this asset or its aliases
					for _, id := range ids {
...
// each component (corresponding to a PSM asset ID) and variable (corresponding
// to PSM asset struct fields)
func unpackInstance(inst instance, assetsMap map[string]map[string][]string) {
	for _, c := range inst.Components {
		for _, v := range c.Variables {
			if v.RegID != "" && v.PSMID != "" { // We've found an entry in assets.json with a psm_id
				if _, ok := assetsMap[c.ID]; !ok { // check if entry is present in map
					assetsMap[c.ID] = make(map[string][]string)
				}
				assetsMap[c.ID][v.PSMID] = append(assetsMap[c.ID][v.PSMID], v.RegID)
			}
		}
...
// buildAssetsMaps() loads assets.json to look for "psm_id" fields. It does this
// for each set of ESS, Feeders, Generators and Solar. This tells PSM to publish
...
func buildAssetsMaps(assetsMap map[string]map[string][]string) {
	...
	var cpath string
	if len(os.Args) < 2 {
		log.Print("Config path argument not found. Usage 'psm /path/to/config'. Trying current working directory")
		cpath = "assets.json"
	} else if len(os.Args) < 3 {
		info, err := os.Stat(os.Args[1])
		if os.IsNotExist(err) {
			log.Fatal("PSM asset file not found at: ", os.Args[1])
		} else if info.IsDir() {
		...
		if os.IsNotExist(err) {
			log.Fatal("PSM asset file not found at: ", os.Args[2])
		} else if info.IsDir() {
