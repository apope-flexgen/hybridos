// Basic TCP server with which to test C2C communication interface
package main
...
func listenToLocalPort() net.Conn {
	fmt.Println("Message start-word is", startWord)
	ln, err := net.Listen("tcp", ":8000")
	if err != nil {
		panic("Failed to listen to local port")
	}
	conn, err := ln.Accept()
	if err != nil {
		panic("Client connection attempt failed")
	}
	return conn
}
