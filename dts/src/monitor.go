...
func (monitor *archiveMonitor) run(group *errgroup.Group, groupContext context.Context) (startUpError error) {
...
func (monitor *archiveMonitor) monitorUntil(done <-chan struct{}) error {
	...
	for {
		...
		case ev := <-watcher.Event: // receive from channel
			...
			if !strings.Contains(filenameWithPath, config.Extension) { // ignore archives that don't match the extension
				continue
			}
...
func (monitor *archiveMonitor) readPreexistingArchives(group *errgroup.Group, groupContext context.Context) (StartUpError error) {
	...
	for _, dirEntry := range dirEntries {
		...
		matchesExtension, err := filepath.Match("*"+config.Extension, file.Name())
		if err != nil {
