...
func (writer *influxWriter) run(group *errgroup.Group, groupContext context.Context) (startUpError error) {
	...
	for i := 0; i < config.NumInfluxPrepareBatchesWorkers; i++ {
		group.Go(func() error { return writer.prepareBatchesUntil(groupContext.Done()) })
	}
	for i := 0; i < config.NumInfluxSendBatchesWorkers; i++ {
...
func (writer *influxWriter) initialize() error {
	...
	var address string
	switch config.InfluxAddr {
	case "":
		address = "localhost:8086"
	default:
		address = config.InfluxAddr
	}

	conn := influx.NewConnector(address, time.Minute/2, time.Duration(config.DbHealthCheckDelayS*float64(time.Second)), false)
...
func (writer *influxWriter) prepareBatches(data *archiveData) (*archiveBatches, error) {
	...
	err := writer.ensureDB(data.db) // set up database if it isn't already
	if err != nil {
	...
	for {
		...
		case preparedBatches := <-writer.preparedBatchesQueue:
			...
			if err == nil {
				...
				err := removeArchive(archiveFilePath, true, config.FailedWritePath)
				if err != nil {
...
// Ensures that the database we want to write to exists in the destination server
func (writer *influxWriter) ensureDB(dbName string) error {
	if _, exists := writer.knownDbs.Load(dbName); exists {
		return nil
	}
	//Bug fix: to take the dbname as string literal its enclosed in quotes ONLY to create db
	tmpName := "\"" + dbName + "\""
	log.Debugf("trying to create database: %s", tmpName)

	var retentionPolicies []influx.RetentionPolicy = nil
	if len(config.Retention) > 0 {
		retentionPolicies = []influx.RetentionPolicy{
			{Name: "DTS Retention Policy",
				Default:  true,
				Duration: config.Retention},
		}
	}

	err := writer.influxConn.CreateDatabase(tmpName, retentionPolicies)
	if err != nil {
