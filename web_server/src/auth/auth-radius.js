...
class AuthRadius {
    constructor(nasIpAddress, secret, port, connectionTimeout) {
        if (!nasIpAddress)
            throw new Error('nasIpAddress is not defined.');
        ...
        if (connectionTimeout > 30000) //30 seconds seems like absurdly long time to wait so don't allow it. could make it configurable
            throw new Error('connectionTimeout is higher than expected.');

        this.nasIpAddress = nasIpAddress;
        this.secret = secret;
        this.port = port;
        this.protocol = 'udp4'; //Dgram only works on udp so no need to accept any other type
        ...
        this.em.on('error', (err) => { });
        this.socket;
        this.timeoutId;
    }

    authenticate(username, password) {
        radius.add_dictionary(path.join(__dirname, '../lib/radius/dictionaries/dictionary.flexgen'));
        ...
        this.timeoutId = setTimeout(() => {
            this.close(this.Events.timedout); //TODO: what should we do if Radius times out? Try local?
        }, parseInt(this.connectionTimeout));

        this.socket.on("message", msg => {
            let decodedPacket = radius.decode({ packet: msg, secret: this.secret });
            ...
            } else {//Condition unlikely to be hit.
                this.error('connection status undertermined');
            }
        });

        this.socket.send(packet, 0, packet.length, this.port, this.nasIpAddress, (err, bytes) => {
            if (err) {
